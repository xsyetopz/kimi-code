...
- `packages/telemetry`: shared client-side telemetry infrastructure (target: no-op / stripped in the harness product line).
...
**Out of the harness product line** (migration targets for deletion or quarantine, not default-path dependencies): `apps/kimi-web`, `apps/vis`, `apps/kimi-inspect`, `packages/kap-server`, `packages/minidb`, `packages/server-e2e`. See [ARCHITECTURE.md](ARCHITECTURE.md) for the north star.
