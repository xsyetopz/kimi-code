...
- `bun -C packages/kap-server test` — server routes green.
- `bun -C packages/kap-server test` — server routes green (incl. any wire-schema guards).
- `bun -C packages/agent-core-v2 test` — native + Legacy Service tests green.
- `bun -C packages/agent-core-v2 run lint:imports` — the import boundaries (v1 ban, kosong subtree) still hold for a LegacyService.
- `bun -C packages/klient test` (optionally with `KIMI_SERVER_URL` for the live legacy suites) when a v1 parity scenario exists.
