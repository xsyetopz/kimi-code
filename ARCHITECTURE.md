...
- `REQ-003` — Strip novelty users of a harness never need (easter eggs, design-system demo chrome, browser UIs, HTTP server edge on the default path).
...
| E-004 | USER | CLI⇔TUI only; drop web/vis/inspect apps | user directive | Delete browser hosts |
| E-005 | USER | In-process klient/memory; no kap-server default | user directive | Remove HTTP edge from hot path |
| E-006 | USER | Bun SQLite search + ripgrep fallback | user directive | Replace minidb server index |
| E-007 | USER | Keep package dir names; change internals | user directive | No new top-level package names |
| E-008 | USER | CI after product | user directive | Do not block on CI |
...
| Term | Definition |
|---|---|
| Harness | Agent runtime + thin CLI/TUI client; no novelty chrome |
...
| QA-005 | Product-line LOC ≤250K | Inventory script | P1 |
| QA-006 | Swarm enter/exit and subagent delegation | Engine `swarm` domain only | P1 |
| QA-007 | Plugin/hook interceptors run through engine hook pipeline | No parallel ad-hoc middleware | P1 |
...
| 1 | Strip novelty: delete easter eggs / tips chrome | revert commits |
...
- kap-server as default harness transport → rejected (`E-005`).
- Browser-hosted session UI → rejected (`E-004`).
