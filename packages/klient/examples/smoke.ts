...
async function main(): Promise<void> {
  ...
  try {
    ...
    assert(
      env.platform.length > 0 && env.homeDir.length > 0,
      "env snapshot is populated",
    );
    console.log("[ok] env");
    ...
    assert(
      seen.includes(name),
      "kosong.providers.changed fired for the new provider",
    );
    await klient.global.kosong.removeProvider(name);
    sub.dispose();
    console.log(
      "[ok] kosong addProvider/getProvider/removeProvider + kosong.providers.changed",
    );
    ...
    while (!seenModels.includes(modelId) && Date.now() < modelDeadline)
      await tick(25);
    assert(
      seenModels.includes(modelId),
      "kosong.models.changed fired for the new model",
    );
    await klient.global.kosong.removeProvider(modelId);
    modelSub.dispose();
    console.log(
      "[ok] kosong anonymous addProvider/removeProvider + kosong.models.changed",
    );
    ...
    assert(
      ...
      "config.getAll returns an object",
    );
    console.log("[ok] config.getAll");

    assert(
      Array.isArray(await klient.global.flags.list()),
      "flags.list returns an array",
    );
    assert(
      Array.isArray(await klient.global.plugins.list()),
      "plugins.list returns an array",
    );
