...
async function probeWireEncoding(): Promise<void> {
  ...
  try {
    ...
    assert(
      ...
      "kimi: a wire message carries the loaded tool schemas",
    );
    assert(
      declaration.tools?.some(
        (t) => t.function?.name === "computer_screenshot",
      ) === true,
      "kimi: declaration embeds computer_screenshot",
    );
    assert(
      kimiBody?.tools?.some((t) => t.function?.name === "select_tools") ===
        true,
...
const SCENARIOS: readonly Scenario[] = [
  {
    prompt: "Take a screenshot of my screen right now.",
    expectTool: "computer_screenshot",
  },
  {
    prompt:
      "Click at the exact center of my screen. The screen resolution is 1920x1080.",
    expectTool: "computer_click",
  },
];

async function step1Select(
  requester: ModelRequester,
  scenario: Scenario,
...
async function step2UseLoadedTool(
  ...
  try {
    const result = await withTimeout(
      collect(requester, messages(true)),
      60_000,
      "step2",
    );
    ...
    try {
      await withTimeout(
        collect(requester, messages(false)),
        60_000,
        "step2-isolation",
      );
      return {
...
async function probeLiveKimiProviders(): Promise<void> {
  ...
  try {
    ...
    const filter = process.env["KIMI_SELECT_TOOLS_MODELS"]
      ?.split(",")
      .map((s) => s.trim());
    ...
    for (const m of targets) {
      ...
      for (const scenario of SCENARIOS) {
        ...
        summary.push(
          `${m.model.padEnd(40)} ${scenario.expectTool.padEnd(22)} ${row}`,
        );
      }
    }
...
function describeWireBody(raw: Buffer): string[] {
  ...
  for (const [i, m] of (body.messages ?? []).entries()) {
    ...
    if (content.length > 0)
      parts.push(`content=${content.slice(0, 60).replaceAll("\n", " ")}…`);
