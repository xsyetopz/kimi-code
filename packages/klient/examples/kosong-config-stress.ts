  ...
  try {
    ...
    await phase("sequential provider add read-after-write", 30, async () => {
      for (let i = 0; i < 30; i += 1) {
        ...
        assert(
          got !== undefined,
          `providers.${name} visible to the registry facade`,
        );
      }
    });
    ...
    await phase("same-name add/remove flip-flop", 30, async () => {
      for (let i = 0; i < 15; i += 1) {
        await kosong.addProvider(
          "flip",
          apiKeyProvider(`sk-flip-${String(i)}`),
        );
        let providers = await config.get<ProvidersSectionView>("providers");
        assert(
          providers["flip"]?.apiKey === `sk-flip-${String(i)}`,
          "flip add persisted",
        );
    ...
    await phase("default-model churn", 22, async () => {
      ...
      for (let i = 0; i < 20; i += 1) {
        ...
        assert(
          def === id,
          `defaultModel=${id} persisted before setDefaultModel resolved`,
        );
      }
    });
    ...
    await phase("concurrent burst adds", 40, async () => {
      await Promise.all(
        Array.from({ length: 40 }, (_, i) =>
          kosong.addProvider(
            `burst_${String(i)}`,
            apiKeyProvider(`sk-burst-${String(i)}`),
          ),
        ),
      );
      const providers = await config.get<ProvidersSectionView>("providers");
    ...
    await phase("concurrent mixed sections", 41, async () => {
      await Promise.all([
        ...Array.from({ length: 30 }, (_, i) =>
          kosong.removeProvider(`seq_${String(i)}`),
        ),
      ...
      for (let i = 0; i < 30; i += 1) {
        assert(
          providers[`seq_${String(i)}`] === undefined,
          `seq_${String(i)} removal persisted`,
        );
      }
      ...
      assert(
        (await config.get<string>("defaultModel")) === "churn_b",
        "default flip persisted",
      );
    });
    ...
    await phase(
      "config-originated replace visible to registry",
      4,
      async () => {
        ...
        assert(
          got !== undefined,
          "config.replace(providers) visible to kosong.getProvider",
        );
        ...
        assert(
          modelIds.includes("cfg_model"),
          "config.replace(models) visible to kosong.listModels",
        );

        await config.set({
          domain: "providers",
          patch: { cfg_only: { apiKey: "sk-cfg-updated" } },
        });
        const providers = await config.get<ProvidersSectionView>("providers");
        assert(
          providers["cfg_only"]?.apiKey === "sk-cfg-updated",
          "config.set merge persisted",
        );

        await kosong.removeProvider("cfg_only");
        assert(
          (await config.get<ProvidersSectionView>("providers"))["cfg_only"] ===
            undefined,
  ...
  await phase("restart durability", 1, async () => {
    ...
    try {
      ...
      const providerIds = new Set(
        (await klient2.global.kosong.listProviders()).map((p) => p.id),
      );
...
try {
  ...
  if (pinnedModelEnv !== undefined)
    process.env["KIMI_MODEL_NAME"] = pinnedModelEnv;
}
