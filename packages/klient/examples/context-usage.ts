...
async function main(): Promise<void> {
  ...
  if (seedModel === undefined || seedKey === undefined) {
    throw new Error(
      "KIMI_EXAMPLE_MODEL and KIMI_EXAMPLE_API_KEY are required (see header)",
    );
  }
  ...
  try {
    ...
    agent.events.on("agent.status.updated", (event) => {
      ...
      if ("contextTokens" in event)
        slice["contextTokens"] = event["contextTokens"];
      if ("maxContextTokens" in event)
        slice["maxContextTokens"] = event["maxContextTokens"];
      if ("contextUsage" in event)
        slice["contextUsage"] = event["contextUsage"];
      ...
      console.log(
        `[event]   ${elapsed()} agent.status.updated ->`,
        JSON.stringify(slice),
      );
    });
    agent.events.on("turn.started", (event) => {
      console.log(
        `[event]   ${elapsed()} turn.started         -> turnId=${String(event.turnId)}`,
      );
    });
    agent.events.on("turn.ended", (event) => {
      console.log(
        `[event]   ${elapsed()} turn.ended           -> reason=${event.reason}`,
      );
    });
    ...
    agent.events.onError((error) => {
      console.log(
        `[event-err] ${elapsed()} ${error.message.split("\n")[0] ?? error.message}`,
      );
    });

    const completed = new Promise<"completed" | "failed" | "timeout">(
      (resolve) => {
    ...
    console.log(
      "[note]    session left in the (disposed) temp home ->",
      session.id,
    );
