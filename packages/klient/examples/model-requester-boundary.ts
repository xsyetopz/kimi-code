...
async function probeRealConfig(): Promise<void> {
  ...
  const { app } = bootstrap(
    { homeDir, clientIdentity: EXAMPLE_CLIENT_IDENTITY },
  ...
  try {
    ...
    const filter = process.env["KIMI_BOUNDARY_MODELS"]
      ?.split(",")
      .map((s) => s.trim());
    const targets = models.filter(
      (m) => filter === undefined || filter.includes(m.model),
    );
...
): string => sseChunk({ tool_calls: toolCalls }, finishReason);
...
function describeCaught(error: unknown): {
  outcome: string;
  wrappedBy: string;
} {
  ...
  if (error instanceof ChatProviderError) {
    return {
      outcome: `raw ${error.name} (escaped translation!)`,
      wrappedBy: "ChatProvider only",
    };
  }
  return {
...
async function probeBoundaries(): Promise<void> {
  ...
  try {
    ...
    assert(
      lastAuth === "Bearer sk-probe",
      "requester injects per-request auth",
    );
    report(
      "happy-path",
      `events=${ok.events.join(">")} text=${JSON.stringify(ok.text)}`,
      "—",
    );
    ...
    const refreshable: AuthProvider = {
      ...
      getAuth: (options) => {
        ...
        return Promise.resolve({
          apiKey: options?.force === true ? "sk-good" : "sk-stale",
        });
      },
    };
    ...
    resetCounts();
    handler = (_req, res) =>
      writeJsonError(res, 429, "too many requests", { "retry-after": "2" });
    try {
      ...
      const { outcome, wrappedBy } = describeCaught(error);
      assert(
        isError2(error) && error.code === "provider.rate_limit",
    ...
    try {
      ...
      assert(
        ...
        "500 -> provider.api_error",
      );
      assert(
        !error.message.includes("<"),
        "HTML body sanitized to its <title>",
      );
    ...
    try {
      await collect(
        makeRequester(
          staticKey("sk-probe"),
          `http://127.0.0.1:${String(deadPort)}`,
        ),
      );
      throw new Error("expected a failure");
    } catch (error) {
    ...
    try {
      ...
      assert(
        isError2(error),
        "malformed stream still surfaces as a coded Error2",
      );
    ...
    const toolOk = await collect(
      makeRequester(staticKey("sk-probe")),
      undefined,
      TOOL_INPUT,
    );
    const wireTools = (
      lastRequestBody as { tools?: { function?: { name?: string } }[] }
    ).tools;
    assert(
    ...
    assert(
      toolOk.events.includes("part:function"),
      "function header part streamed",
    );
    assert(
      toolOk.events.includes("part:tool_call_part"),
      "argument fragments streamed",
    );
    assert(toolOk.toolCalls.length === 1, "one assembled tool call");
    assert(
      toolOk.toolCalls[0]?.name === "get_weather",
      "tool call name assembled",
    );
    assert(
    ...
    assert(
      toolOk.finishReason === "tool_calls",
      "finish reason 'tool_calls' normalized",
    );
    report(
      "toolcall-happy",
    ...
    assert(
      parallel.toolCalls.length === 2,
      "two parallel tool calls assembled",
    );
    assert(
      parallel.toolCalls[0]?.name === "tool_a" &&
        parallel.toolCalls[0]?.arguments === '{"a":1}',
      "index 0 arguments routed to tool_a",
    );
    assert(
      parallel.toolCalls[1]?.name === "tool_b" &&
        parallel.toolCalls[1]?.arguments === '{"b":2}',
    ...
    const wireMessages = (
      lastRequestBody as { messages?: Record<string, unknown>[] }
    ).messages;
    assert(
      wireMessages?.some(
    ...
    try {
      ...
      assert(
        ...
        "abort is NOT a ChatProviderError",
      );
