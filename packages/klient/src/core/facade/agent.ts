...
export type PromptLaunchResult = Awaited<
  ReturnType<IAgentRPCService["prompt"]>
>;
export type ShellCommandResult = Awaited<
  ReturnType<IAgentShellCommandService["run"]>
>;
export type SetModelResult = Awaited<
  ReturnType<IAgentProfileService["setModel"]>
>;
export type ThinkingLevel = ReturnType<
  IAgentProfileService["getEffectiveThinkingLevel"]
>;
export type UsageStatus = Awaited<ReturnType<IAgentUsageService["status"]>>;
export type AgentContextData = Awaited<
  ReturnType<IAgentRPCService["getContext"]>
>;
export type PlanData = Awaited<ReturnType<IAgentPlanService["status"]>>;
export type AgentTaskInfo = Awaited<
  ReturnType<IAgentTaskService["list"]>
>[number];
  ...
  return {
    ...
    runShellCommand: (input) =>
      call(scope, "agentShellCommandService", "run", [
        input,
      ]) as Promise<ShellCommandResult>,
    cancelShellCommand: (input) =>
      call(scope, "agentShellCommandService", "cancel", [
        input.commandId,
      ]) as Promise<void>,
    getModel: () =>
      call(scope, "agentProfileService", "getModel", []) as Promise<string>,
    setModel: (model) =>
      call(scope, "agentProfileService", "setModel", [
        model,
      ]) as Promise<SetModelResult>,
    getThinking: () =>
      call(
        scope,
        "agentProfileService",
    ...
    setThinking: (level) =>
      call(scope, "agentProfileService", "setThinking", [
        level,
      ]) as Promise<void>,
    setPermission: (mode) => rpc("setPermission", { mode }) as Promise<void>,
    getUsage: () =>
      call(scope, "agentUsageService", "status", []) as Promise<UsageStatus>,
    getContext: () => rpc("getContext", {}) as Promise<AgentContextData>,
    getPlan: () =>
      call(scope, "agentPlanService", "status", []) as Promise<PlanData>,
    enterPlan: () =>
      call(scope, "agentPlanService", "enter", []) as Promise<void>,
    clearPlan: () =>
      call(scope, "agentPlanService", "clear", []) as Promise<void>,
    cancelPlan: (input) =>
    ...
    stopTask: async (input) => {
      ...
      await call(scope, "agentTaskService", "stop", [
        input.taskId,
        input.reason,
      ]);
    },
    getTaskOutput: (input) =>
      call(scope, "agentTaskService", "readOutput", [
        input.taskId,
        input.tail,
      ]) as Promise<string>,
    getMcpServers: () =>
      call(scope, "agentMcpService", "list", []) as Promise<
