...
export function defineKlientConformance(
  ...
  describe(`klient conformance: ${transport}`, () => {
    ...
    it("workspaces round-trip through create/get/update/list/delete", async () => {
      ...
      const created = await workspaces.createOrTouch({
        root: process.cwd(),
        name: "conformance",
      });
      ...
      const updated = await workspaces.update({
        id: created.id,
        patch: { name: "conformance-2" },
      });
    ...
    it("sessions index responds with a page shape", async () => {
      ...
      const count = await target.klient.global.sessions.countActive([
        "no-such-workspace",
      ]);
    ...
    it("creates a titled session through implicit workspace materialization", async () => {
      ...
      try {
        ...
        expect(created.id.length).toBeGreaterThan(0);
        expect(
          await target.klient.global.sessions.get(created.id),
        ).toMatchObject({
          id: created.id,
          title: "conformance session",
        });
    ...
    it("session skills.list returns the workspace skills as summaries", async () => {
      ...
      try {
        await mkdir(join(workDir, ".kimi-code", "skills", "conf-skill"), {
          recursive: true,
        });
        await writeFile(
        ...
        try {
          ...
          expect(
            skills.find((skill) => skill.name === "conf-skill"),
          ).toMatchObject({
            name: "conf-skill",
            description: "conformance fixture skill",
            source: "project",
    ...
    it("providers.set/get/delete works and emits kosong.providers.changed", async () => {
      ...
      const sub = target.klient.events.on(
      ...
      try {
        ...
        await waitFor(
          () =>
            events.some((event) =>
              [...event.added, ...event.changed].includes(name),
            ),
          5_000,
        );
      } finally {
    ...
    it("config reads respond", async () => {
      ...
      expect(
        Array.isArray(await target.klient.global.config.diagnostics()),
      ).toBe(true);
    });
    ...
    it("config replaceSections writes several domains and clears undefined ones", async () => {
      ...
      const beforeProviders =
        await config.inspect<Record<string, unknown>>("providers");
      const beforeModels =
        await config.inspect<Record<string, unknown>>("models");
      try {
        ...
        expect((await config.inspect<string>("defaultModel")).userValue).toBe(
          "conf-provider/m1",
        );
        ...
        expect(
          (await config.inspect<string>("defaultModel")).userValue,
        ).toBeUndefined();
        const providers =
          await config.inspect<Record<string, unknown>>("providers");
    ...
    it("kosong lists models/providers and anonymous provider round-trips", async () => {
      ...
      try {
        ...
        await waitFor(
          () =>
            events.some((event) =>
              [...event.added, ...event.changed].includes(id),
            ),
          5_000,
        );
      } finally {
    ...
    it("flags / plugins / auth read models respond", async () => {
      ...
      expect(Array.isArray(await target.klient.global.flags.enabledIds())).toBe(
        true,
      );
      expect(typeof (await target.klient.global.flags.snapshot())).toBe(
        "object",
      );
      expect(Array.isArray(await target.klient.global.plugins.list())).toBe(
        true,
      );
