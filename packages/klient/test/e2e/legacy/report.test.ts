...
describe("server-e2e report", () => {
  it("renders HTTP and WS trace events into a readable HTML timeline", () => {
    ...
    expect(html).toContain(
      '<span class="event-title">fresh listSessions snapshot</span>',
    );
  ...
  it("records HttpClient request and response envelopes", async () => {
    ...
    const httpEvent = readReportEvents(reportDir).find(
      (event) => event.kind === "http",
    );
  ...
  it("records WsClient inbound and outbound frames", async () => {
    ...
    FakeWebSocket.instances[0]?.emit(
      ...
      JSON.stringify({
        type: "server_hello",
        payload: { heartbeat_ms: 30_000 },
      }),
    );

    const events = readReportEvents(reportDir).filter(
      (event) => event.kind === "ws",
    );
  ...
  it("records createCaseLogger entries under the active case", () => {
    ...
    try {
      ...
      const event = readReportEvents(reportDir).find(
