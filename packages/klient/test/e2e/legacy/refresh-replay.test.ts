...
async function getEnvelope<T>(
  ...
  expect(
    body.code,
    `${path} returned code=${body.code} msg=${body.msg ?? ""}`,
  ).toBe(0);
  return body.data;
}
...
async function openSocketWithHello(opts: {
  ...
  const serverHello = await ws.waitForFrame(
...
describeLive("refresh-replay (live server required)", () => {
  ...
  it(
    ...
    async () => {
      ...
      log("max seq before reconnect", {
        session_id: session.id,
        max_seq: maxSeq,
      });
      expect(
        maxSeq,
        "session must publish at least one event before reconnect",
      ).toBeGreaterThan(0);

      await client.close();
      log("closed initial socket");

      const refreshed = await openSocketWithHello({
        sid: session.id,
        lastSeq: maxSeq,
        log,
      });
  ...
  it(
    ...
    async () => {
      ...
      const { items } = await client.http.listMessages(session.id, {
        page_size: 100,
      });
