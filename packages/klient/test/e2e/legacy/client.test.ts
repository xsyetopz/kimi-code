...
describeLive("DaemonClient (live server required)", () => {
  ...
  it("completes handshake (server_hello + client_hello ack)", async () => {
    ...
    const client = new DaemonClient({ baseUrl: BASE_URL });
    log("connect request", {
      url: `${BASE_URL.replace(/^http/, "ws")}/api/v1/ws`,
    });
    const hello = await client.connect();
    log("server hello", hello);
    // heartbeat_ms is optional — kap-server omits it (no server heartbeat).
    expect(hello.heartbeat_ms === undefined || hello.heartbeat_ms > 0).toBe(
      true,
    );
  ...
  it("waitForFrame times out cleanly", async () => {
    ...
    await client.connect();
    log("wait request", {
      frame_type: "event.does.not.exist",
      timeout_ms: 100,
    });
    let caughtError: unknown;
    try {
      await client.waitForFrame((f) => f.type === "event.does.not.exist", {
        timeoutMs: 100,
      });
    } catch (error) {
      caughtError = error;
  ...
  it(
    ...
    async () => {
      ...
      if (compactError === undefined) {
        throw new Error(
          "expected compactSession to reject for an empty-history session",
        );
      }
      ...
      const populated = await client.createSession({
        metadata: { cwd: process.cwd() },
      });
      ...
      const beforeCompact = await client.listMessages(populated.id, {
        page_size: 100,
      });
      successLog("messages before compact", beforeCompact);
      expect(beforeCompact.items.some((m) => m.role === "user")).toBe(true);
      expect(beforeCompact.items.some((m) => m.role === "assistant")).toBe(
        true,
      );
      ...
      const completedPromise = client.waitForFrame(
        (f) =>
          f.type === "compaction.completed" && f.session_id === populated.id,
        { timeoutMs: PROMPT_TIMEOUT_MS },
      );
      const compactResponse = await client.compactSession(
        populated.id,
        populatedCompactRequest,
      );
      ...
      const afterCompact = await client.listMessages(populated.id, {
        page_size: 100,
      });
  ...
  it(
    ...
    async () => {
      ...
      const undoPrompt = await client.submitAndWait(
        ...
        {
          content: [
            { type: "text", text: 'Remember UNDO-ME. Reply with "OK".' },
          ],
        },
        { waitFor: "prompt.completed", timeoutMs: PROMPT_TIMEOUT_MS },
      );
      ...
      const beforeUndo = await client.listMessages(session.id, {
        page_size: 100,
      });
      log("messages before undo", beforeUndo);
      expect(textFromMessages(beforeUndo.items)).toContain("UNDO-ME");

      const result = await client.undoSession(session.id, {
        count: 1,
        page_size: 100,
      });
...
describe("DaemonClient session action helpers", () => {
  it("forkSession posts the action-suffix route and unwraps the returned session", async () => {
    ...
    const fork = testSession({
      id: "sess_fork",
      title: "Fork: Source session",
    });
    ...
    expect(calls[0]?.url).toBe(
      "http://server.example.test/api/v1/sessions/sess_source:fork",
    );
  ...
  it("compactSession posts the action-suffix route and preserves error envelope details", async () => {
    ...
    let caughtError: unknown;
    try {
      await client.compactSession("sess_source", {
    ...
    if (caughtError === undefined) {
      throw new Error(
        "expected compactSession to reject for a non-zero envelope",
      );
    }
    ...
    expect(calls).toHaveLength(1);
    expect(calls[0]?.url).toBe(
      "http://server.example.test/api/v1/sessions/sess_source:compact",
    );
    expect(calls[0]?.init.method).toBe("POST");
    expect(parseRecordedJsonBody(calls[0])).toEqual({
      instruction: "  focus on decisions  ",
  ...
  it("undoSession posts the action-suffix route and unwraps messages plus status", async () => {
    ...
    const result = await client.undoSession("sess_source", {
      count: 2,
      page_size: 25,
    });
    ...
    expect(calls).toHaveLength(1);
    expect(calls[0]?.url).toBe(
      "http://server.example.test/api/v1/sessions/sess_source:undo",
    );
    expect(calls[0]?.init.method).toBe("POST");
    expect(parseRecordedJsonBody(calls[0])).toEqual({
      count: 2,
  ...
  it("model catalog helpers call the catalog and action-suffix routes", async () => {
    ...
    expect(calls.map((call) => [call.init.method, call.url])).toEqual([
      ...
      [
        "POST",
        "http://server.example.test/api/v1/models/kimi-code%2Fkimi-for-coding:set_default",
      ],
  ...
  it("child-session and pending reverse-RPC helpers call recovery routes", async () => {
    ...
    await expect(client.listPendingApprovals("sess_parent")).resolves.toEqual({
      items: [],
    });
    await expect(client.listPendingQuestions("sess_parent")).resolves.toEqual({
      items: [],
    });
    await expect(
      client.dismissQuestion("sess_parent", "question_1"),
    ).resolves.toEqual({
      dismissed: true,
      dismissed_at: "2026-06-09T00:00:00.000Z",
    });
    ...
    expect(calls.map((call) => [call.init.method, call.url])).toEqual([
      [
        "POST",
        "http://server.example.test/api/v1/sessions/sess_parent/children",
      ],
      [
        "GET",
      ...
      [
        "GET",
        "http://server.example.test/api/v1/sessions/sess_parent/approvals?status=pending",
      ],
      [
        "GET",
        "http://server.example.test/api/v1/sessions/sess_parent/questions?status=pending",
      ],
      [
        "POST",
  ...
  it("uploadFile posts multipart form data and deleteFile hits the file route", async () => {
    ...
    const client = new DaemonClient({
      baseUrl: "http://server.example.test",
      fetchImpl: recordingFetchSequence(
        [okEnvelope(file), okEnvelope({ deleted: true })],
        calls,
      ),
    });
    ...
    await expect(client.deleteFile(file.id)).resolves.toEqual({
      deleted: true,
    });
...
function recordingFetchSequence(
  ...
  return (async (
    ...
    calls.push({ url: fetchInputUrl(input), init: init ?? {} });
    const responseBody =
      responseBodies[Math.min(index, responseBodies.length - 1)];
    index++;
    return new Response(JSON.stringify(responseBody), {
      status: 200,
      headers: { "content-type": "application/json" },
