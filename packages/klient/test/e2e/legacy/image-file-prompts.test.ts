    ...
    try {
        ...
        () =>
          client.submitPrompt(sid!, {
            content: [
              {
                type: "image",
                source: { kind: "file", file_id: "file_missing_e2e" },
              },
            ],
          }),
      ...
      await expectEnvelopeCode(
        () =>
          client.submitPrompt(sid!, {
            content: [
              { type: "image", source: { kind: "file", file_id: textFile.id } },
            ],
          }),
      ...
      const submit = await client.submitPrompt(sid, {
        content: [
          {
            type: "text",
            text: 'Reply with the single word "OK" after reading this image.',
          },
      ...
      try {
        ...
        if (
          ...
          log("prompt already terminal before abort", {
      ...
      await client.waitForSessionBusy(sid, false, {
