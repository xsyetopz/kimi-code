...
const TERMINAL_SHELL =
  process.env["KIMI_SERVER_E2E_TERMINAL_SHELL"] ?? "/bin/sh";
...
function payloadOf<T>(frame: AnyFrame): T {
  if (frame.payload === undefined)
    throw new Error(`${frame.type} frame should carry payload`);
...
describeLive("legacy: terminal controls", () => {
  it("terminal output, resize, and close round-trip", async () => {
    ...
    try {
      ...
      try {
        const attach = await client.attachTerminal(sessionId, terminal.id, {
          sinceSeq: 0,
        });
        ...
        const inputAck = await client.writeTerminalInput(
          sessionId,
          terminal.id,
          input,
        );
        ...
        const resizeAck = await client.resizeTerminal(
          sessionId,
          terminal.id,
          100,
          31,
        ...
        const exitFramePromise = client.waitForFrame(
          isTerminalExitFor(sessionId, terminal.id),
          {
            timeoutMs: EXIT_TIMEOUT_MS,
          },
        );
        const closeAck = await client.closeTerminalControl(
          sessionId,
          terminal.id,
        );
        terminalClosed = true;
