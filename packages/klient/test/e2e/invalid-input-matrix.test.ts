...
function onceEvent<
  ...
  E extends keyof TPayloadMap & string,
>(
...
function queueScript(
  ...replies: readonly (MockReply | undefined)[]
): MockHandler {
  const queue = [...replies];
  return () => {
  ...
  server = createServer((req: IncomingMessage, res: ServerResponse) => {
    void (async () => {
      ...
      try {
        ...
        res.end(
          JSON.stringify({
            error: { message: `mock handler: ${String(error)}` },
          }),
        );
        return;
      }
...
function openAiMessages(callIndex: number): Record<string, unknown>[] {
  const body = requests[callIndex]?.json as
    | { messages?: Record<string, unknown>[] }
    | undefined;
  expect(
    body?.messages,
    `request #${callIndex} should carry a messages array`,
  ).toBeDefined();
...
describe("l1: klient input validation", () => {
  it("rejects an image_url part missing `url` before any engine work (l1)", async () => {
    ...
    const failure = await ctx.agent
      .prompt({ input: badInput })
  ...
  it("accepts an empty input array; the empty user message is dropped by the engine (l2 toleration)", async () => {
    ...
    expect(messages.every((message) => message["role"] === "system")).toBe(
      true,
    );
...
describe("image blocks with invalid data", () => {
  it("a data-URL image with an unaccepted mime is replaced at prompt ingestion on EVERY provider (l2)", async () => {
    ...
    for (const { label, model, reply } of cases) {
      ...
      expect(ctx.payloads("prompt.completed")[0]?.["reason"], label).toBe(
        "completed",
      );
    }
  }, 60_000);
  ...
  it("a malformed data URL is replaced with a notice at prompt ingestion (l2)", async () => {
    ...
    await promptAndWait(ctx, [
      ...
      {
        type: "image_url",
        imageUrl: { url: "data:definitely-not-a-data-url" },
      },
    ]);
  ...
  it("openai: corrupt base64 with an accepted mime passes conversion; a 400 triggers media-stripped resend (l3 + engine fallback)", async () => {
    ...
    expect(JSON.stringify(secondContent)).toContain(
      "image omitted for provider compatibility",
    );
    expect(ctx.payloads("prompt.completed")[0]?.["reason"]).toBe("completed");
  }, 30_000);
  ...
  it("google-genai: corrupt base64 rides inlineData; a 400 triggers media-stripped resend (l3 + engine fallback)", async () => {
    ...
    expect(
      secondParts.some((part) => (part as { inlineData?: unknown }).inlineData),
    ).toBe(false);
    expect(ctx.payloads("prompt.completed")[0]?.["reason"]).toBe("completed");
  }, 30_000);
  ...
  it("anthropic: corrupt base64 with a legal mime passes conversion, then gets stripped after a 400", async () => {
    ...
    resetMock(
      queueScript(jsonError(400, "could not process the image"), OK_ANTHROPIC),
    );
    ...
    const firstBlocks = (
      (requests[0]?.json as { messages?: { content?: unknown[] }[] })
        .messages ?? []
    ).flatMap((message) =>
    ...
    const secondBlocks = (
      (requests[1]?.json as { messages?: { content?: unknown[] }[] })
        .messages ?? []
    ).flatMap((message) =>
      Array.isArray(message.content) ? message.content : [],
    );
...
describe("video blocks", () => {
  ...
  it("kimi (composed): ReadMediaFile on a video uploads via the files API (uploadVideo trait)", async () => {
    ...
    await writeFile(
      join(ctx.workDir, "clip.mp4"),
      Buffer.from(MP4_FTYP_HEX, "hex"),
    );
    ...
    const chatCalls = requests.filter(
      (request) => request.url === "/v1/chat/completions",
    );
    expect(chatCalls).toHaveLength(2);
    const secondBody = chatCalls[1]?.json as {
      messages: Record<string, unknown>[];
    };
  ...
  it("openai: no uploadVideo capability — the tool result is an error text, no files call", async () => {
    ...
    expect(
      requests.find((request) => request.url === "/v1/files"),
    ).toBeUndefined();
    const chatCalls = requests.filter(
      (request) => request.url === "/v1/chat/completions",
    );
...
describe("tool exchange structure", () => {
  it("openai: an unknown tool call still gets a paired (error) result on the wire", async () => {
    ...
    const toolMessage = messages.find(
      (message) =>
        message["role"] === "tool" &&
        message["tool_call_id"] === "call_unknown_1",
    );
  ...
  it("media tool result: plain openai extracts text + appends a media user message; kimi keeps parts in place", async () => {
    ...
    const openAiWire = await runMediaToolResultCase(
      M_OPENAI_VISION,
      "media-result-openai",
    );
    ...
    const trailingContent = trailing?.["content"] as {
      type: string;
      text?: string;
    }[];
    ...
    expect(trailingContent.some((part) => part.type === "image_url")).toBe(
      true,
    );
  ...
  it("tool call ids are sanitized (64-char, safe charset) consistently across call and result", async () => {
    ...
    for (const [model, label] of [
      ...
      const assistant = messages.find(
        (message) => message["tool_calls"] !== undefined,
      );
  ...
  it("after a user abort, an interruption reminder separates the next user message on the wire", async () => {
    ...
    resetMock((_req, callIndex) =>
      callIndex === 0 ? { kind: "hang" } : OK_OPENAI,
    );
    ...
    expect(requests).toHaveLength(2);
    const userMessages = openAiMessages(1).filter(
...
describe("provider HTTP errors", () => {
  it("a non-recoverable 422 fails the turn with provider.api_error + statusCode on the wire (l3)", async () => {
    ...
    resetMock(
      queueScript(jsonError(422, "validation exploded on field messages")),
    );
    ...
    expect(
      (wireError?.["details"] as Record<string, unknown> | undefined)?.[
        "statusCode"
      ],
    ).toBe(422);
