...
describe("SystemMetricsCollector", () => {
  it("emits a numeric system_metrics sample after the warmup delay", () => {
    ...
    expect(
      numberProperty(event.properties, "process_started_at"),
    ).toBeGreaterThan(0);
    expect(
      numberProperty(event.properties, "process_uptime_ms"),
    ).toBeGreaterThanOrEqual(0);
    expect(numberProperty(event.properties, "rss_bytes")).toBeGreaterThan(0);
    expect(numberProperty(event.properties, "heap_used_bytes")).toBeGreaterThan(
      0,
    );
    expect(
      numberProperty(event.properties, "heap_total_bytes"),
    ).toBeGreaterThan(0);
    expect(
      numberProperty(event.properties, "external_bytes"),
    ).toBeGreaterThanOrEqual(0);
    expect(
      numberProperty(event.properties, "array_buffers_bytes"),
    ).toBeGreaterThanOrEqual(0);
    expect(
      numberProperty(event.properties, "cpu_user_us"),
    ).toBeGreaterThanOrEqual(0);
    expect(
      numberProperty(event.properties, "cpu_system_us"),
    ).toBeGreaterThanOrEqual(0);
    expect(numberProperty(event.properties, "cpu_elapsed_us")).toBeGreaterThan(
      0,
    );
    expect(
      numberProperty(event.properties, "load_avg_1m"),
    ).toBeGreaterThanOrEqual(0);
    expect(
      numberProperty(event.properties, "free_mem_bytes"),
    ).toBeGreaterThanOrEqual(0);
    expect(numberProperty(event.properties, "total_mem_bytes")).toBeGreaterThan(
      0,
    );
    expect(
      numberProperty(event.properties, "cpu_count"),
...
describe("payload assembly", () => {
  ...
  it("rejects nested context and array property values before outbound send", () => {
    ...
    expect(() => buildPayload([nestedContext], "device-1")).toThrow(
      /context.nested/,
    );
    expect(() => buildPayload([arrayProperty], "device-1")).toThrow(
      /property.list/,
    );
  });
...
describe("server prefix application", () => {
  ...
  it("passes already-prefixed and invalid event names through unchanged", () => {
    ...
    const missingName = { ...sampleEvent("missing") } as unknown as Record<
      string,
      unknown
    >;
    ...
    expect(
      applyServerPrefix(emptyName as unknown as EnrichedTelemetryEvent),
    ).toBe(emptyName);
    expect(
      applyServerPrefix(missingName as unknown as EnrichedTelemetryEvent),
    ).toBe(missingName);
...
describe("AsyncTransport", () => {
  ...
  it("spools to disk when the anonymous 401 retry gets a transient response", async () => {
    ...
    const file = readFileSync(
      join(telemetryDir, readdirOne(telemetryDir)),
      "utf-8",
    );
...
describe("telemetry bootstrap", () => {
  it("matches the KIMI_DISABLE_TELEMETRY true-value semantics", () => {
    expect(isTelemetryDisabledByEnv({ KIMI_DISABLE_TELEMETRY: "1" })).toBe(
      true,
    );
    expect(isTelemetryDisabledByEnv({ KIMI_DISABLE_TELEMETRY: "yes" })).toBe(
      true,
    );
    expect(isTelemetryDisabledByEnv({ KIMI_DISABLE_TELEMETRY: "0" })).toBe(
      false,
    );
    expect(isTelemetryDisabledByEnv({ KIMI_DISABLE_TELEMETRY: "false" })).toBe(
      false,
    );
  });
  ...
  it("writes system metrics with the singleton session context", async () => {
    ...
    if (metrics === undefined)
      throw new Error("Expected a system_metrics event");

    expect(metrics.session_id).toBe("ses");
    expect(Number.isFinite(metrics.properties["process_started_at"])).toBe(
      true,
    );
...
describe("crash handler", () => {
  ...
  it("reflects startup, runtime, and shutdown phases in crash telemetry", () => {
    ...
    expect(
      transport.saved.map((batch) => batch[0]?.properties["where"]),
    ).toEqual(["startup", "runtime", "shutdown"]);
  });
...
function numberProperty(
  ...
  if (typeof value !== "number" || !Number.isFinite(value)) {
    throw new Error(
      `Expected property ${key} to be a finite number, got ${String(value)}`,
    );
  }
  return value;
}
...
function requestInitFrom(
  ...
  const call = fetchImpl.mock.calls[index] as
    | readonly [unknown, RequestInit?]
    | undefined;
  const init = call?.[1];
  if (init === undefined)
    throw new Error(`No request init for fetch call ${String(index)}`);
  return init;
}
