...
export class FsWatchBridge {
  ...
  async addWatch(
    ...
    if (resolved === undefined) {
      return {
        code: FS_WATCH_CODE.SESSION_NOT_FOUND,
    ...
    for (const raw of rawPaths) {
      ...
      if (rel === undefined) {
        return {
          code: FS_WATCH_CODE.PATH_ESCAPES,
          msg: "fs.path_escapes_session",
        };
      }
      normalized.push(rel);
    }
    ...
    if (current + toAdd.length > MAX_PATHS_PER_CONNECTION) {
      return {
        code: FS_WATCH_CODE.LIMIT_EXCEEDED,
        msg: "fs.watch_limit_exceeded",
      };
    }
  ...
  async removeWatch(
    ...
    this.connPathCount.set(
      conn.id,
      Math.max(0, this.countFor(conn.id) - removed),
    );
  ...
  detachConnection(conn: FsWatchConnection): void {
    for (const sw of Array.from(this.bySession.values())) {
      ...
      this.connPathCount.set(
        conn.id,
        Math.max(0, this.countFor(conn.id) - entry.paths.size),
      );
      this.recomputeAndApply(sw);
      if (sw.conns.size === 0) this.teardownSession(sw);
    }
  ...
  private recomputeAndApply(sw: SessionWatch): void {
    ...
    if (union.size > 0 && sw.sub === undefined) {
      sw.sub = sw.fsWatch.onDidChangeFiles((ev) =>
        this.onSessionEvent(sw.id, ev),
      );
    }
  }
