...
function goalContinuationStarts(
...
describe("server-v2 /api/v1/sessions", () => {
  ...
  it("downloads a ZIP with the supplied Web log and cleans up its temporary directory", async () => {
    ...
    const manifest = JSON.parse(
      entries.get("manifest.json")?.toString("utf8") ?? "null",
    ) as {
      sessionId: string;
  ...
  it("cleans up the temporary archive when the client cancels the download", async () => {
    ...
    await writeFile(
      join(sessionDir, "cancel-test.bin"),
  ...
  async function createStoppedGoalRig(status: "paused" | "blocked") {
    ...
    const stopped = await postJson<{ status: string }>(
      agentRpc(
        IAgentGoalService,
        status === "blocked" ? "markBlocked" : "pauseGoal",
        id,
      ),
      status === "blocked" ? { reason: "need credentials" } : {},
    );
    if (stopped.body.data.status !== status)
  ...
  it("creates a second session via workspace_id resolved from a prior cwd create", async () => {
    const cwd = home as string;
    const first = await postJson<SessionWire>("/api/v1/sessions", {
      metadata: { cwd },
    });
  ...
  it("lists created sessions", async () => {
    ...
    expect(body.code).toBe(0);
    expect(body.data.items.some((s) => s.id === created.body.data.id)).toBe(
      true,
    );
  ...
  it("supports exclude_empty when listing sessions", async () => {
    ...
    expect(all.body.data.items.some((s) => s.id === created.body.data.id)).toBe(
      true,
    );

    const filtered = await getJson<PageWire>(
      "/api/v1/sessions?exclude_empty=true",
    );
    expect(filtered.body.code).toBe(0);
    expect(
      filtered.body.data.items.some((s) => s.id === created.body.data.id),
    ).toBe(false);
  });
  ...
  it("paginates sessions with before_id and terminates on the last page", async () => {
    ...
    expect(page1.body.data.items.map((s) => s.id)).toEqual(
      ids.slice(4).reverse(),
    );
    ...
    expect(page2.body.data.items.map((s) => s.id)).toEqual(
      ids.slice(1, 4).reverse(),
    );
  ...
  it("gets a session by id and 404s for unknown", async () => {
    ...
    const got = await getJson<SessionWire>(
      `/api/v1/sessions/${created.body.data.id}`,
    );
  ...
  it("starts one continuation when the Web profile resumes a blocked goal", async () => {
    ...
    try {
      const resumed = await postJson<SessionWire>(
  ...
  it("restores an archived session via :restore and returns it to the default list", async () => {
    ...
    const restored = await postJson<SessionWire>(
    ...
    expect(listed.body.data.items.find((s) => s.id === id)?.archived).toBe(
      false,
    );
  });
  ...
  it("returns 40401 when restoring a missing session", async () => {
    const { body } = await postJson<null>(
      "/api/v1/sessions/sess_missing:restore",
    );
    expect(body.code).toBe(40401);
  });
  ...
  it("cold-loads a persisted session on :undo instead of 40401", async () => {
    ...
    const res = await postJson<{ messages: unknown }>(
  ...
  it("returns 40901 when :undo reports a busy session", async () => {
    ...
    const undo = vi
      ...
      .mockRejectedValue(
        new Error2(ErrorCodes.SESSION_BUSY, "session is busy"),
      );
  ...
  it("rejects an unsupported action suffix (40001)", async () => {
    ...
    const { body } = await postJson<null>(
      `/api/v1/sessions/${created.body.data.id}:restart`,
    );
    expect(body.code).toBe(40001);
  });
  ...
  it("creates a child session tagged with parent_session_id and child_session_kind", async () => {
    ...
    const child = await postJson<SessionWire>(
      `/api/v1/sessions/${parentId}/children`,
      {
        title: "child-title",
  ...
  it("lists direct children and omits grandchildren", async () => {
    ...
    const grandchild = await postJson<SessionWire>(
    ...
    const parentChildren = await getJson<PageWire>(
      `/api/v1/sessions/${parentId}/children`,
    );
    expect(parentChildren.body.code).toBe(0);
    expect(parentChildren.body.data.items.some((s) => s.id === childId)).toBe(
      true,
    );
    expect(
      parentChildren.body.data.items.some((s) => s.id === grandchildId),
    ).toBe(false);

    const childChildren = await getJson<PageWire>(
      `/api/v1/sessions/${childId}/children`,
    );
    expect(childChildren.body.code).toBe(0);
    expect(
      childChildren.body.data.items.some((s) => s.id === grandchildId),
    ).toBe(true);
  });
  ...
  it('does not list a plain fork as a child (kind must be "child")', async () => {
    ...
    const parentId = parent.body.data.id;
    const forked = await postJson<SessionWire>(
      `/api/v1/sessions/${parentId}:fork`,
      {},
    );
    expect(forked.body.code).toBe(0);

    const children = await getJson<PageWire>(
      `/api/v1/sessions/${parentId}/children`,
    );
    expect(children.body.code).toBe(0);
    expect(
      children.body.data.items.some((s) => s.id === forked.body.data.id),
    ).toBe(false);
  });

  it("returns 40401 when listing children of a missing parent", async () => {
    const { body } = await getJson<null>(
      "/api/v1/sessions/sess_missing_parent/children",
    );
    expect(body.code).toBe(40401);
  });
  ...
  it("returns an empty warnings list for an existing session", async () => {
    ...
    expect(sessionWarningsResponseSchema.parse(body.data)).toEqual({
      warnings: [],
    });
  });
  ...
  it("returns 40401 for warnings of a missing session", async () => {
    const { body } = await getJson<null>(
      "/api/v1/sessions/sess_missing_warnings/warnings",
    );
    expect(body.code).toBe(40401);
  });
  ...
  it("lists only archived sessions with archived_only", async () => {
    const cwd = home as string;
    const a = await postJson<SessionWire>("/api/v1/sessions", {
      metadata: { cwd },
    });
    const b = await postJson<SessionWire>("/api/v1/sessions", {
      metadata: { cwd },
    });
    ...
    const onlyArchived = await getJson<PageWire>(
      "/api/v1/sessions?archived_only=true",
    );
    expect(onlyArchived.body.code).toBe(0);
    expect(onlyArchived.body.data.items.some((s) => s.id === archivedId)).toBe(
      true,
    );
    expect(onlyArchived.body.data.items.some((s) => s.id === liveId)).toBe(
      false,
    );

    // include_archive shows both.
    const all = await getJson<PageWire>(
      "/api/v1/sessions?include_archive=true",
    );
  ...
  it("paginates archived_only without returning empty filtered pages", async () => {
    const cwd = home as string;
    const archivedOlder = await postJson<SessionWire>("/api/v1/sessions", {
      metadata: { cwd },
    });
    ...
    const archivedNewer = await postJson<SessionWire>("/api/v1/sessions", {
      metadata: { cwd },
    });
    await postJson<{ archived: boolean }>(
    ...
    const first = await getJson<PageWire>(
      "/api/v1/sessions?archived_only=true&page_size=1",
    );
    expect(first.body.code).toBe(0);
  ...
  it("keeps the after_id lower bound while a filtered drain pages for more candidates", async () => {
    ...
    expect(page.body.data.items.map((s) => s.id)).toEqual([
      archivedNewer.body.data.id,
    ]);
  ...
  it("returns a terminal empty page when archived_only busy filtering finds no match", async () => {
    ...
    await postJson<{ archived: boolean }>(
      `/api/v1/sessions/${first.body.data.id}:archive`,
    );
    await postJson<{ archived: boolean }>(
      `/api/v1/sessions/${second.body.data.id}:archive`,
    );
  ...
  it("rejects a malformed workspace_id when listing (40001)", async () => {
    const { body } = await getJson<null>(
      "/api/v1/sessions?workspace_id=not-a-workspace-id",
    );
    expect(body.code).toBe(40001);
  });
  ...
  it("returns 40410 for an unknown workspace_id when listing", async () => {
    const { body } = await getJson<null>(
      "/api/v1/sessions?workspace_id=wd_missing_000000000000",
    );
    expect(body.code).toBe(40410);
  });
  ...
  it("lists the union of legacy split buckets for one workspace, in recency order", async () => {
    ...
    const workspaces = await getJson<{ items: { id: string }[] }>(
      "/api/v1/workspaces",
    );
    ...
    expect(listed.body.code).toBe(0);
    expect(listed.body.data.items.map((s) => s.id)).toEqual([
      "s-lower",
      "s-typed",
    ]);
  ...
  it("filters child sessions by the busy query", async () => {
    ...
    const idle = await getJson<PageWire>(
      `/api/v1/sessions/${parentId}/children?busy=false`,
    );
    expect(idle.body.code).toBe(0);
    expect(idle.body.data.items.some((s) => s.id === childId)).toBe(true);

    const running = await getJson<PageWire>(
      `/api/v1/sessions/${parentId}/children?busy=true`,
    );
    expect(running.body.code).toBe(0);
  ...
  it("keeps a session listable and gettable with cwd after its workspace is unregistered (gap G3)", async () => {
    ...
    const del = await deleteJson<{ deleted: boolean }>(
    ...
    const profile = await getJson<SessionWire>(
      `/api/v1/sessions/${id}/profile`,
    );
  ...
  it("merges metadata via profile and keeps cwd authoritative", async () => {
    ...
    const first = await postJson<SessionWire>(
      `/api/v1/sessions/${id}/profile`,
      {
        metadata: { foo: "bar" },
      },
  ...
  it("replaces custom metadata on a second profile update (v1 semantics)", async () => {
    ...
    const second = await postJson<SessionWire>(
      `/api/v1/sessions/${id}/profile`,
      {
        metadata: { baz: 1 },
      },
  ...
  it("applies agent_config.permission_mode via profile idempotently", async () => {
    ...
    const again = await postJson<SessionWire>(
      `/api/v1/sessions/${id}/profile`,
      {
        agent_config: { permission_mode: "yolo" },
      },
  ...
  it("publishes session.meta.updated on the core bus when renaming via profile", async () => {
    ...
    expect((meta?.payload as { title?: string } | undefined)?.title).toBe(
      "renamed-via-profile",
    );
  });
  ...
  it("returns 40401 when updating the profile of a missing session", async () => {
    const { body } = await postJson<null>(
      "/api/v1/sessions/sess_missing_profile/profile",
      {
        title: "nope",
      },
    );
...
describe("server-v2 /api/v1/sessions (minidb read model)", () => {
  ...
  it("prepares the read model at boot and serves immediate reads", async () => {
    ...
    const paged = await getJson<PageWire>(
      `/api/v1/sessions?page_size=1&before_id=${id}`,
    );
    expect(paged.body.data.items).toEqual([]);
    expect(paged.body.data.has_more).toBe(false);

    await postJson<{ archived: boolean }>(`/api/v1/sessions/${id}:archive`);
    const archivedOnly = await getJson<PageWire>(
      "/api/v1/sessions?archived_only=true",
    );
    ...
    base = `http://127.0.0.1:${server.port}`;
    const relisted = await getJson<PageWire>(
      "/api/v1/sessions?include_archive=true",
    );
