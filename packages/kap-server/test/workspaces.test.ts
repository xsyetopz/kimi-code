...
describe("server-v2 /api/v1/workspaces", () => {
  ...
  it("creates a workspace with the full wire shape", async () => {
    ...
    const { status, body } = await postJson<WorkspaceWire>(
      "/api/v1/workspaces",
      {
        root,
        name: "proj",
  ...
  it("derives the default name from the root when name is omitted", async () => {
    const root = home as string;
    const { body } = await postJson<WorkspaceWire>("/api/v1/workspaces", {
      root,
    });
    expect(body.code).toBe(0);
  ...
  it("is idempotent on root (createOrTouch)", async () => {
    ...
    const second = await postJson<WorkspaceWire>("/api/v1/workspaces", {
      root,
    });
  ...
  it("rejects a relative root (40001)", async () => {
    const { body } = await postJson<null>("/api/v1/workspaces", {
      root: "relative/path",
    });
  ...
  it("lists registered workspaces", async () => {
    const root = home as string;
    const created = await postJson<WorkspaceWire>("/api/v1/workspaces", {
      root,
    });
    const { body } = await getJson<ListWire>("/api/v1/workspaces");
    expect(body.code).toBe(0);
    expect(body.data.items.some((w) => w.id === created.body.data.id)).toBe(
      true,
    );
  });
  ...
  it("renames a workspace via PATCH", async () => {
    ...
    const updated = await patchJson<WorkspaceWire>(`/api/v1/workspaces/${id}`, {
      name: "renamed",
    });
  ...
  it("returns 40410 when patching an unknown workspace", async () => {
    const { body } = await patchJson<null>(
      "/api/v1/workspaces/wd_missing_000000000000",
      {
        name: "nope",
      },
    );
