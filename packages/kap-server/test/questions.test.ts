...
describe("server-v2 /api/v1/sessions/{sid}/questions", () => {
  ...
  it("lists a pending question projected onto the wire shape", async () => {
    ...
    const { body } = await getJson<ListWire>(
      `/api/v1/sessions/${sid}/questions?status=pending`,
    );
    expect(body.code).toBe(0);
  ...
  it("resolves a pending question", async () => {
    ...
    const { body } = await postJson<ResolveWire>(
      `/api/v1/sessions/${sid}/questions/q-2`,
      {
  ...
  it("returns 40902 on a duplicate resolve (recently-resolved window)", async () => {
    ...
    const dup = await postJson<{ resolved: false }>(
      `/api/v1/sessions/${sid}/questions/q-5`,
      {
        answers: { q_0: { kind: "single", option_id: "opt_0_0" } },
      },
    );
  ...
  it("returns 40405 for an unknown question id", async () => {
    ...
    const { body } = await postJson<null>(
      `/api/v1/sessions/${sid}/questions/nope`,
      {
        answers: { q_0: { kind: "single", option_id: "opt_0_0" } },
      },
    );
  ...
  it("resolves a question whose id contains a colon (provider tool_call id)", async () => {
    ...
    const list = await getJson<ListWire>(
      `/api/v1/sessions/${sid}/questions?status=pending`,
    );
  ...
  it("keeps 40001 for a colon tail that matches no pending question", async () => {
    const sid = await createSession();
    const { body } = await postJson<null>(
      `/api/v1/sessions/${sid}/questions/q-9:0`,
      {
        answers: { q_0: { kind: "single", option_id: "opt_0_0" } },
      },
    );
  ...
  it("returns 40401 for an unknown session", async () => {
    const { body } = await getJson<null>(
      "/api/v1/sessions/nope/questions?status=pending",
    );
