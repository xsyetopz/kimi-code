...
describe("server-v2 /api/v1 fs folder picker", () => {
  ...
  beforeEach(async () => {
    ...
    instancesDir = await mkdtemp(
      join(tmpdir(), "kimi-server-v2-fs-instances-"),
    );
  ...
  it("defaults browse to $HOME when path is omitted", async () => {
    ...
    expect(
      typeof body.data.parent === "string" || body.data.parent === null,
    ).toBe(true);
  ...
  it("reflects registered workspace roots in recent_roots", async () => {
    const root = home as string;
    const created = await postJson<{ id: string }>("/api/v1/workspaces", {
      root,
    });
...
describe("server-v2 /api/v1 fs:mkdir", () => {
  ...
  beforeEach(async () => {
    ...
    instancesDir = await mkdtemp(
      join(tmpdir(), "kimi-server-v2-fsmkdir-instances-"),
    );
    server = await startServer({
      hostIdentity: TEST_HOST_IDENTITY,
      host: "127.0.0.1",
  ...
  it("creates a directory that fs:browse then lists", async () => {
    ...
    const { status, body } = await postJson<{ path: string }>(
      "/api/v1/fs:mkdir",
      {
        path: target,
      },
    ...
    expect(browseBody.data.entries.map((e) => e.name)).toContain(
      "fresh-folder",
    );
  });

  it("rejects a relative path (40001)", async () => {
    const { body } = await postJson<null>("/api/v1/fs:mkdir", {
...
describe("server-v2 /api/v1 fs:content", () => {
  ...
  beforeEach(async () => {
    ...
    instancesDir = await mkdtemp(
      join(tmpdir(), "kimi-server-v2-fscontent-instances-"),
    );
    server = await startServer({
      hostIdentity: TEST_HOST_IDENTITY,
      host: "127.0.0.1",
  ...
  it("serves binary files byte-for-byte with an octet-stream fallback mime", async () => {
    ...
    const original = Buffer.from([
      0x00, 0x01, 0x02, 0xff, 0xfe, 0x00, 0x10, 0x80,
    ]);
  ...
  it.skipIf(process.platform === "win32")(
    "rejects non-regular files (40001)",
    async () => {
