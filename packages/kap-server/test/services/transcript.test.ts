...
function turnOps(
  turnId: string,
  items: ReturnType<AgentTranscript["getItems"]>,
): TranscriptTurn {
...
describe("AgentTranscriptProjector", () => {
  it("projects a full turn: headers, delta appends, flush, tool frames", () => {
    ...
    const upserts = ops.filter(
      (op): op is FrameUpsertOp => op.op === "frame.upsert",
    );
  ...
  it("places late-attach deltas into the engine-reported active step", () => {
    ...
    const ops = projector.map(
      ev({ type: "assistant.delta", turnId: 0, delta: "late" }),
    );
    tx.apply(ops);
  ...
  it("adopts a backfilled stream frame on mid-turn attach instead of clobbering it", () => {
    ...
    // seeded offset.
    const ops = projector.map(
      ev({ type: "assistant.delta", turnId: 0, delta: "world" }),
    );
    tx.apply(ops);
    ...
    const next = projector.map(
      ev({ type: "thinking.delta", turnId: 0, delta: "hmm" }),
    );
  ...
  it("gives live markers their own namespace so they never collide with backfilled markers", () => {
    ...
    const ops = projector.map(
      ev({ type: "compaction.started", trigger: "auto" }),
    );
    tx.apply(ops);
  ...
  it("snapshotToOps anchors standalone items so backfill keeps history order against live turns", () => {
    ...
    expect(ops.find((op) => op.op === "marker.upsert")).toMatchObject({
      beforeTurn: 1,
    });
    expect(ops.find((op) => op.op === "taskref.upsert")).toMatchObject({
      beforeTurn: 2,
    });
  ...
  it("flushes open frames on turn.ended even without step completion", () => {
    ...
    const tx = new AgentTranscript("main");
    const feed = (event: DomainEvent): void =>
      void tx.apply(projector.map(event));
  ...
  it("sets retry on turn.step.retrying and clears it at the terminal upsert", () => {
    ...
    const feed = (event: DomainEvent): void =>
      void tx.apply(projector.map(event));
    const step = (): TranscriptTurn["steps"][number] =>
      turnOps("t1", tx.getItems()).steps[0]!;
  ...
  it("overwrites tool frame progress and drops progress for unknown calls", () => {
    ...
    const tool = (): TranscriptFrame | undefined =>
      turnOps("t1", tx.getItems()).steps[0]!.frames.find(
        (frame) => frame.kind === "tool",
      );
    expect(tool()).toMatchObject({
      progress: { kind: "stdout", text: "line1" },
    });
    ...
    expect(tool()).toMatchObject({
      progress: { kind: "progress", percent: 40 },
    });
    expect(
      (tool() as { progress?: Record<string, unknown> }).progress?.["text"],
    ).toBeUndefined();
  });
  ...
  it("marks tool.result errors and keeps the display payload", () => {
    ...
    const tool = turnOps("t1", tx.getItems()).steps[0]!.frames.find(
      (f) => f.kind === "tool",
    );
    expect(tool).toMatchObject({
      state: "error",
  ...
  it("projects process tasks as shell tasks with streaming output", () => {
    ...
    expect(
      ops.some(
        (op) => op.op === "taskref.upsert" && op.item.taskId === "bash-1",
      ),
    ).toBe(true);
    const appends = ops.filter((op): op is AppendOp => op.op === "append");
    expect(appends.map((op) => [op.offset, op.text])).toEqual([
    ...
    expect(
      projector.map(
        ev({
          type: "shell.output",
          commandId: "cmd-1",
          update: { kind: "progress", percent: 50 },
        }),
      ),
    ).toEqual([]);
  });
  ...
  it("fills the shell task output from late stderr chunks before completing", () => {
    ...
    tx.apply(
      projector.map(
        ev({ type: "shell.started", commandId: "c1", taskId: "task-1" }),
      ),
    );
    tx.apply(
      projector.map(
        ev({
          type: "shell.output",
          commandId: "c1",
          update: { kind: "stderr", text: "boom" },
        }),
      ),
    );
    tx.apply(
      projector.map(
        ev({ type: "shell.completed", commandId: "c1", isError: true }),
      ),
    );
  ...
  it("marks a foreground shell task terminal on shell.completed", () => {
    ...
    tx.apply(
      projector.map(
        ev({ type: "shell.completed", commandId: "c1", isError: false }),
      ),
    );
    expect(tx.getTask("task-1")).toMatchObject({
      kind: "shell",
    ...
    tx.apply(
      projector.map(
        ev({ type: "shell.started", commandId: "c2", taskId: "task-2" }),
      ),
    );
    tx.apply(
      projector.map(
        ev({ type: "shell.completed", commandId: "c2", isError: true }),
      ),
    );
  ...
  it("projects goal updates into meta.goal plus an inline marker", () => {
    ...
    const clearedOps = projector.map(
      ev({ type: "goal.updated", snapshot: null }),
    );
  ...
  it("mirrors plan / swarm mode slices into meta.modes (only when provided)", () => {
    ...
    tx.apply(
      projector.map(ev({ type: "agent.status.updated", planMode: true })),
    );
    tx.apply(
      projector.map(ev({ type: "agent.status.updated", swarmMode: true })),
    );
    expect(tx.getMeta().modes).toEqual({ plan: {}, swarm: {} });

    // Mode exit clears the badge (`null` deletes the key in the reducer).
    tx.apply(
      projector.map(ev({ type: "agent.status.updated", planMode: false })),
    );
    expect(tx.getMeta().modes).toEqual({ swarm: {} });
    tx.apply(
      projector.map(ev({ type: "agent.status.updated", swarmMode: false })),
    );
  ...
  it("mirrors status slices into meta.agent (shallow-merged across slices)", () => {
    ...
    const usageOnly = projector.map(
      ev({ type: "agent.status.updated", usage: {} }),
    );
    expect(usageOnly).toEqual([
      { op: "meta.merge", meta: { agent: { usage: {} } } },
    ]);

    feed(
      ev({ type: "agent.status.updated", model: "k2", thinkingEffort: "high" }),
    );
    feed(
      ev({
    ...
    expect(tx.getMeta().agent).toMatchObject({
      model: "k3",
      thinkingEffort: "high",
    });
  });
  ...
  it("maps agent.activity.updated into meta.agent.phase", () => {
    ...
    feed(
      ev({
        ...
        turn: turn({
          pendingApprovals: [
            { approvalId: "ap1", toolCallId: "c1", since: 1500 },
          ],
        }),
        background: [],
      }),
  ...
  it("projects plan.revision as a marker and refines the active plan badge", () => {
    ...
    tx.apply(
      projector.map(
        ev({ ...revision, version: 2, path: "agents/main/plan/plan-1/v2.md" }),
      ),
    );
    ...
    expect(
      markers.map((item) => item.kind === "marker" && item.markerId),
    ).toEqual(["live-m1", "live-m2"]);
  ...
  it("projects skill / plugin-command / cron / compaction / hook / undo markers", () => {
    ...
    feed(
      ev({ type: "context.spliced", start: 1, deleteCount: 2, messages: [] }),
    );
  ...
  it("projects error / warning events as notice markers outside any step", () => {
    ...
    tx.apply(
      projector.map(ev({ type: "warning", message: "AGENTS.md oversized" })),
    );
  ...
  it("emits interactions as global entities only (no inline frame), back-links on resolve", () => {
    ...
    expect(
      turnOps("t2", tx.getItems()).steps[0]!.frames.map((f) => f.kind),
    ).toEqual(["tool"]);
    ...
    tx.apply(
      projector.mapInteractionResolved("apr-1", {
        decision: "approved",
        scope: "session",
      }),
    );

    const tool = turnOps("t2", tx.getItems()).steps[0]!.frames.find(
      (f) => f.kind === "tool",
    );
  ...
  it("surfaces a mid-turn task notification as a user input frame linked to the task", () => {
    ...
    expect(frame?.kind === "text" && frame.text).toContain(
  ...
  it("replaces the global todo document on a confirmed TodoList write", () => {
    ...
    feed(
      ev({ type: "tool.result", toolCallId: "call_read", output: "2 todos" }),
    );
    ...
    feed(
      ev({ type: "tool.result", toolCallId: "call_write", output: "updated" }),
    );
  ...
  it("projects prompt submitted/completed/aborted/steered as global queue entities", () => {
    ...
    feed(
      ev({
        type: "prompt.aborted",
        promptId: "p3",
        abortedAt: "2026-01-01T00:00:03.000Z",
      }),
    );
        ...
        accessor: {
          get: (token: unknown) => {
            ...
            if (token === ISessionIndex)
              return { get: async () => ({ workspaceId: "ws" }) };
            return undefined;
          },
        },
      } as unknown as Scope,
  ...
  it("readColdSnapshot folds task/todo/goal/plan/interaction records into the cold snapshot", async () => {
    ...
    try {
      ...
      expect(snapshot!.meta.goal).toMatchObject({
        objective: "fix the bug",
        status: "active",
      });
  ...
  it("folds blocked turn endings into failed (engine wire contract)", () => {
    ...
    tx.apply(
      projector.map(
        ev({ type: "turn.started", turnId: 0, origin: { kind: "user" } }),
      ),
    );
    tx.apply(
      projector.map(ev({ type: "turn.ended", turnId: 0, reason: "blocked" })),
    );
...
describe("bindSessionTranscript", () => {
  ...
  class FakeAgents {
    ...
    private readonly createHandlers = new Set<
      (handle: FakeAgentHandle) => void
    >();
    ...
    onDidCreate(cb: (handle: FakeAgentHandle) => void): {
      dispose: () => void;
    } {
  ...
  it("keeps the materialized transcript and roster entry when an agent is disposed", () => {
    ...
    expect(
      store.agents().find((a) => a.agentId === "main")?.disposedAt,
    ).toBeUndefined();
    binding.dispose();
  });
  ...
  it("heals missing tool frames and missed results, keeps richer live ones", () => {
    const makeTurn = (
      frames: TranscriptTurn["steps"][number]["frames"],
    ): TranscriptTurn => ({
      kind: "turn",
      turnId: "t0",
      ordinal: 0,
    ...
    expect(frames).toContainEqual(
      expect.objectContaining({ frameId: "t0.1.call_3", output: "y" }),
    );
  });
  ...
  it("subscribes the bus for an agent whose projector was seeded before its handle existed", () => {
    ...
    binding.seedPendingInteractions("sub-1");
    expect(byAgent.get("sub-1")?.map((op) => op.op)).toEqual([
      "interaction.upsert",
    ]);
    ...
    const sub = agents.add("sub-1");
    sub.bus.emit(
      ev({ type: "turn.started", turnId: 1, origin: { kind: "user" } }),
    );
  ...
  it("merges the backfill live-first: live frame fields and longer text survive", async () => {
    ...
    try {
      ...
      bus.emit(
        ev({ type: "assistant.delta", turnId: 0, delta: "Hello world" }),
      );
      bus.emit(
        ev({
          type: "tool.call.started",
  ...
  describe("op journal", () => {
    it("assigns consecutive per-agent seqs and serves catch-up from the journal", async () => {
      ...
      expect(catchup?.batches.map((batch) => batch.seq)).toEqual([
        base + 1,
        base + 2,
      ]);
      ...
      expect(
        service
          .getOpsSince("s1", "sub-1", 0)
          ?.batches.map((batch) => batch.seq),
      ).toEqual([1]);
    ...
    it("marks catch-up incomplete once the bounded journal evicts old batches", async () => {
      ...
      for (
        let turnId = 1;
        turnId <= TRANSCRIPT_OPS_JOURNAL_CAPACITY + 1;
        turnId++
      ) {
        main.bus.emit(
          ev({ type: "turn.started", turnId, origin: { kind: "user" } }),
        );
      }
