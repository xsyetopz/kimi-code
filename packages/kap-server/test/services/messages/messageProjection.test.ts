...
describe("toProtocolMessage", () => {
  ...
  it("prefers the stored message id and falls back to the transcript index", () => {
    ...
    expect(toProtocolMessage(SESSION_ID, 7, withId, CREATED_AT).id).toBe(
      "msg_custom",
    );
  ...
  it("stamps created_at from the override or the session-created fallback", () => {
    ...
    expect(
      toProtocolMessage(SESSION_ID, 3, msg, CREATED_AT, CREATED_AT + 999)
        .created_at,
    ).toBe(new Date(CREATED_AT + 999).toISOString());
  ...
  it("carries origin into metadata and omits metadata otherwise", () => {
    const withOrigin: ContextMessage = {
      ...userText("a"),
      origin: { kind: "user" },
    };
    expect(
      toProtocolMessage(SESSION_ID, 0, withOrigin, CREATED_AT).metadata,
    ).toEqual({
      origin: { kind: "user" },
    });
    expect(
      toProtocolMessage(SESSION_ID, 0, userText("a"), CREATED_AT),
