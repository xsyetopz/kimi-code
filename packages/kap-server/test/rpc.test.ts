...
function rpc(
  ...
  if (scope === "session")
    return `/api/v1/debug/session/${ids.sid}/${String(service)}/${method}`;
...
describe("server-v2 /api/v1/debug RPC", () => {
  ...
  it("archives a session", async () => {
    const id = await createSession(home as string);
    const workspaceId = (await server!.core.accessor
      .get(ISessionIndex)
      .get(id))!.workspaceId;
    const { body } = await call<null>(
      "POST",
      rpc("workspace", ISessionLifecycleService, "archive", {
        wid: workspaceId,
      }),
      id,
    );
  ...
  it("rejects reflected goal helper access for subagents", async () => {
    ...
    const { body } = await call<null>(
      "POST",
      rpc("agent", IAgentGoalService, "clearInternal", {
        sid: id,
        aid: "sub-1",
      }),
      "user",
  ...
  it("lists and installs plugins through RPC", async () => {
    const pluginRoot = await mkdtemp(
      join(tmpdir(), "server-v2-plugin-source-"),
    );
    try {
      await writeFile(
      ...
      const commands = await call<
        readonly { pluginId: string; name: string }[]
      >("GET", rpc("core", IPluginService, "listPluginCommands"));
      ...
      const activated = await call<null>(
        "POST",
        rpc("agent", IAgentRPCService, "activatePluginCommand", {
          sid: sessionId,
          aid: "main",
        }),
  ...
  it("returns 40401 when the agent does not exist", async () => {
    ...
    const { body } = await call<null>(
      "POST",
      rpc("agent", IAgentRPCService, "prompt", {
        sid: id,
        aid: "does-not-exist",
      }),
      { input: [{ type: "text", text: "hello" }] },
  ...
  it("rejects unknown service (40001)", async () => {
    const { body } = await call<null>(
      "POST",
      "/api/v1/debug/does-not-exist/list",
    );
    expect(body.code).toBe(40001);
  });
  ...
  it("rejects oversized body", async () => {
    ...
    try {
      const res = await fetch(
        `${base}${rpc("core", ISessionIndex, "listRecent")}`,
        {
          method: "POST",
          headers: {
            "content-type": "application/json",
...
describe("server-v2 /api/v1/debug RPC (dev-only, whitelist-free)", () => {
  ...
  it("rejects an unknown service with 40001", async () => {
    const { body } = await call(
      "POST",
      "/api/v1/debug/noSuchService/whatever",
      [],
    );
    expect(body.code).toBe(40001);
  });
