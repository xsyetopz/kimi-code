...
class FakeSessionActivityView {
  private readonly listeners = new Set<
    (change: SessionActivityChangedEvent) => void
  >();
  ...
  onDidChange(listener: (change: SessionActivityChangedEvent) => void): {
    dispose(): void;
  } {
...
function makeCore(
    ...
    const sessionAccessor = {
      get: (t: unknown) => {
        ...
        if (t === ISessionMetadata)
          return { read: async () => ({ agents: metaAgents }) };
        return undefined;
      },
    };
    return {
  ...
  const handler = {
    ...
    accessor: {
      get: (t: unknown) =>
        t === ISessionLifecycleService ? sessionLifecycle : undefined,
    },
    dispose: () => {},
  };
  const accessor = {
...
describe("SessionEventBroadcaster", () => {
  ...
  it("fans out volatile events with the current watermark + offset, not journaled", async () => {
    ...
    main.bus.emit(
      agentEvent("assistant.delta", { turnId: 1, delta: " there" }),
    ); // volatile
    await bc.getCursor("s1");

    const vol = envelopes.filter(
      (e) => e.volatile === true && e.type === "assistant.delta",
    );
  ...
  it("projects main-agent status and context changes into complete v1 status events", async () => {
    ...
    contextSize = 20;
    main.bus.emit(
      agentEvent("context.spliced", { start: 0, deleteCount: 0, messages: [] }),
    );
    main.bus.emit(
      agentEvent("context.spliced", { start: 0, deleteCount: 0, messages: [] }),
    );
    ...
    const statuses = envelopes.filter(
      (envelope) => envelope.type === "agent.status.updated",
    );
  ...
  it("omits maxContextTokens when no model is bound and no default model resolves", async () => {
    ...
    expect(statuses).toHaveLength(1);
    expect(JSON.stringify(statuses[0]!.payload)).not.toContain(
      "maxContextTokens",
    );
  });
  ...
  it("returns buffer_overflow when the gap exceeds the cap", async () => {
    ...
    for (let i = 0; i < 5; i++)
      main.bus.emit(agentEvent("turn.started", { turnId: i }));
  ...
  it("returns epoch_changed for a mismatched epoch", async () => {
    ...
    const result = await bc.getBufferedSince("s1", {
      seq: 0,
      epoch: "ep_wrong",
    });
  ...
  it("subscribes to agents created after activation (onDidCreate)", async () => {
    ...
    expect(
      envelopes.filter((e) => e.volatile !== true).map((e) => e.seq),
    ).toEqual([1, 2, 3]);
  ...
  it("broadcasts agent.disposed only for agents this state attached", async () => {
    ...
    expect((disposed[0]!.payload as { agentId: string }).agentId).toBe(
      "agent-0",
    );
  ...
  it("getSnapshotState returns the in-flight turn", async () => {
    ...
    expect(snap.inFlightTurn).toMatchObject({
      turn_id: 1,
  ...
  describe("global fan-out to unsubscribed connections", () => {
    ...
    it("delivers work_changed to a global-only target while a subscriber drives the session", async () => {
      ...
      await bc.getCursor("s1"); // drain between the turn boundaries
      main.bus.emit(
        agentEvent("turn.ended", { turnId: 1, reason: "completed" }),
      );
      await bc.getCursor("s1"); // drain
      ...
      expect(workChanged[0]).toMatchObject({
        session_id: "s1",
        payload: { busy: true },
      });
    ...
    it("delivers event.config.warning to a global-only target that never subscribed", async () => {
      ...
      const warnings = [
        ...
        {
          message:
            "Environment variable OLD_VAR is deprecated; use NEW_VAR instead.",
        },
      ];
    ...
    it("drops malformed event.config.warning payloads", async () => {
      ...
      eventBus.emit({
        type: "event.config.warning",
        payload: { warnings: [{ message: 42 }] },
      });
      eventBus.emit({
        type: "event.config.warning",
        payload: { warnings: "nope" },
      });
  ...
  it("maps the main turn outcome into last_turn_reason on the post-turn work_changed", async () => {
    ...
    const workChanged = durable.filter(
      (e) => e.type === "event.session.work_changed",
    );
  ...
  it("flips busy from background tasks alone (no turn involved)", async () => {
    ...
    const workChanged = envelopes.filter(
      (e) => e.type === "event.session.work_changed",
    );
    expect(workChanged.map((e) => e.payload)).toMatchObject([
      { busy: true, last_turn_reason: undefined },
    ...
    expect(
      envelopes
        .filter((e) => e.type === "agent.status.updated")
        .map((e) => e.payload),
  ...
  it("emits the first background-work change from an agent created after activation", async () => {
    ...
    const workChanged = envelopes.filter(
      (event) => event.type === "event.session.work_changed",
    );
  ...
  it("flips busy but never touches last_turn_reason from sub-agent turn boundaries", async () => {
    ...
    expect(
      workChanged.every(
        (e) =>
          (e.payload as { last_turn_reason?: string }).last_turn_reason !==
          "cancelled",
      ),
    ).toBe(true);
  ...
  it("broadcasts question requested / answered as durable v1 events", async () => {
    ...
    expect(
      (envelopes[3]!.payload as { resolved_at?: string }).resolved_at,
    ).toBeTypeOf("string");
  });
  ...
  it("broadcasts question dismissed when resolved with null", async () => {
    ...
    expect(
      (envelopes[3]!.payload as { dismissed_at?: string }).dismissed_at,
    ).toBeTypeOf("string");
  });
  ...
  it("does not re-announce interactions already pending at activation, but still broadcasts their resolution", async () => {
    ...
    lc.interactions.enqueue({
      ...
      kind: "question",
      payload: {
        questions: [{ question: "Early", options: [{ label: "A" }] }],
      },
    });
  ...
  it("fans out the legacy background.task.* alias alongside native task.* for v1 clients", async () => {
    ...
    main.bus.emit(
      agentEvent("task.terminated", { info: { ...info, status: "completed" } }),
    );
    await bc.getCursor("s1");
  ...
  it("delivers only the allowlisted agent events on live fan-out", async () => {
    ...
    const agentEnvs = envelopes.filter(
      (e) => e.type === "turn.started" || e.type === "turn.ended",
    );
    expect(agentEnvs).toHaveLength(2);
    expect(
      agentEnvs.every(
        (e) => (e.payload as { agentId: string }).agentId === "main",
      ),
    ).toBe(true);
  ...
  it("replays only the allowlisted agent events while keeping the global sequence", async () => {
    ...
    try {
      ...
      await bc2.getCursor("s1");
      main.bus.emit(
        agentEvent("turn.ended", { turnId: 2, reason: "completed" }),
      );
      await bc2.getCursor("s1");

      const result = await bc2.getBufferedSince(
        "s1",
        { seq: 0 },
        new Set(["main"]),
      );
      ...
      expect(
        result.events.every(
          (e) => (e.envelope.payload as { agentId: string }).agentId === "main",
        ),
      ).toBe(true);
    } finally {
      await bc2.close();
  ...
  describe("transcript streaming", () => {
    ...
    it("sends transcript.reset on first subscription, then fans ops out filtered per grade", async () => {
      ...
      main.bus.emit(
        agentEvent("turn.started", { turnId: 1, origin: { kind: "user" } }),
      );
      for (const view of [deltaView, blockView, turnView]) {
        ...
        expect(
          batches.map((ops) =>
            (ops.payload as OpsPayload).ops.map((o) => o.op),
          ),
        ).toEqual([["turn.upsert"], ["meta.merge"]]);
      }
      expect(transcriptEnvelopes(plainView.envelopes)).toHaveLength(0);
      ...
      const deltaOps = transcriptEnvelopes(deltaView.envelopes).at(-1)!
        .payload as OpsPayload;
      expect(deltaOps.ops.map((o) => o.op)).toEqual(["frame.upsert", "append"]);
      const blockOps = transcriptEnvelopes(blockView.envelopes).at(-1)!
        .payload as OpsPayload;
      expect(blockOps.ops.map((o) => o.op)).toEqual(["frame.upsert"]);
      expect(transcriptEnvelopes(turnView.envelopes)).toHaveLength(
        turnBatchesBefore,
      );
      ...
      const flushed = transcriptEnvelopes(blockView.envelopes).at(-1)!
        .payload as OpsPayload;
      ...
      expect(
        transcriptEnvelopes(deltaView.envelopes).every(
          (e) => e.volatile === true,
        ),
      ).toBe(true);
    });
    ...
    it("re-sends transcript.reset on grade upgrade, not on equal or downgraded re-subscribe", async () => {
      ...
      expect(transcriptEnvelopes(view.envelopes).at(-1)!.type).toBe(
        "transcript.reset",
      );
    });
    ...
    it("keeps delivering ops across a no-reset resubscribe", async () => {
      ...
      const resub = bc.subscribe("s1", view.target, undefined, {
        main: "delta",
      });
    ...
    it("sends no resets when the target downgrades while the seed is in flight", async () => {
      ...
      const pending = bc.subscribe("s1", view.target, undefined, {
        "*": "delta",
      });
      await bc.subscribe("s1", view.target, undefined, { "*": "off" });
      await pending;
    ...
    it("delivers no transcript.ops before the baseline reset has landed", async () => {
      ...
      const pending = bc.subscribe("s1", second.target, undefined, {
        "*": "delta",
      });
      main.bus.emit(agentEvent("assistant.delta", { turnId: 1, delta: "x" }));
      await pending;
      ...
      expect(types.indexOf("transcript.ops")).toBeGreaterThan(
        types.indexOf("transcript.reset"),
      );
    });
    ...
    it("seeds transcript resets for every graded agent regardless of the agent filter", async () => {
      ...
      const view = collectingTarget();
      await bc.subscribe("s1", view.target, new Set(["main"]), {
        "*": "delta",
      });
      const resets = transcriptEnvelopes(view.envelopes).filter(
        (e) => e.type === "transcript.reset",
      ...
      expect(
        resets.map((e) => (e.payload as { agent_id: string }).agent_id),
      ).toEqual(["main", "sub-1"]);
      ...
      await bc.subscribe("s1", view.target, undefined, { "*": "delta" });
      expect(
        transcriptEnvelopes(view.envelopes).filter(
          (e) => e.type === "transcript.reset",
        ),
      ).toHaveLength(2);
    });
    ...
    it("sends an items-empty baseline reset marking older history, with global state and the watermark", async () => {
      ...
      main.bus.emit(agentEvent("turn.step.started", { turnId: 1, step: 1 }));
      main.bus.emit(
        agentEvent("assistant.delta", { turnId: 1, delta: "secret body" }),
      );
      main.bus.emit(agentEvent("turn.step.completed", { turnId: 1, step: 1 }));
      main.bus.emit(
        agentEvent("turn.ended", { turnId: 1, reason: "completed" }),
    ...
    it("honours per-agent grade overrides over the wildcard", async () => {
      ...
      const view = collectingTarget();
      await bc.subscribe("s1", view.target, undefined, {
        "*": "off",
        main: "delta",
      });
    ...
    it("stamps ops payloads with the batch seq and resets with the watermark", async () => {
      ...
      const ops = transcriptEnvelopes(view.envelopes).filter(
        (e) => e.type === "transcript.ops",
      );
      ...
      expect(seqs.every((seq) => seq !== undefined && seq > watermark!)).toBe(
        true,
      );
      ...
      expect(ops.every((e) => e.volatile === true && e.seq === reset.seq)).toBe(
        true,
      );
    });
    ...
    it("replays journaled batches instead of a reset when transcript_since is covered", async () => {
      ...
      main.bus.emit(
        agentEvent("assistant.delta", { turnId: 1, delta: "again" }),
      );
      expect(transcriptEnvelopes(second.envelopes).at(-1)!.type).toBe(
        "transcript.ops",
      );
    });
    ...
    it("suppresses per agent — agents outside the spec keep their session_events", async () => {
      ...
      expect(
        turns.map((e) => (e.payload as { agentId: string }).agentId),
      ).toEqual(["agent-0"]);
    });
    ...
    it("filters the replayed backlog by transcript grades", async () => {
      ...
      const filtered = await bc.getBufferedSince("s1", { seq: 1 }, undefined, {
        "*": "delta",
      });
      ...
      const offSpec = await bc.getBufferedSince("s1", { seq: 1 }, undefined, {
        "*": "off",
      });
    ...
    it("unsubscribeTranscript detaches per agent: ops stop and legacy events resume for that agent only", async () => {
      ...
      expect(
        new Set(opsBefore.map((e) => (e.payload as OpsPayload).agent_id)),
      ).toEqual(new Set(["main", "agent-0"]));

      bc.unsubscribeTranscript("s1", view.target, ["main"]);

      main.bus.emit(
        agentEvent("turn.started", { turnId: 2, origin: { kind: "user" } }),
      );
      sub.bus.emit(
        agentEvent("turn.started", { turnId: 2, origin: { kind: "user" } }),
      );
      ...
      expect(
        new Set(opsAfter.map((e) => (e.payload as OpsPayload).agent_id)),
      ).toEqual(new Set(["agent-0"]));
    });
