...
function syncNow(service: GlobalSearchService): Promise<void> {
  return (
    service as unknown as { ensureSyncStarted(): Promise<void> }
  ).ensureSyncStarted();
}
...
function refreshNow(service: GlobalSearchService): Promise<void> {
  return (
    service as unknown as { refreshReadonly(): Promise<void> }
  ).refreshReadonly();
}
...
interface ServiceInternals {
  db: {
    ...
    batch(
      ops: { op: "set" | "del"; key: string; value?: unknown }[],
    ): Promise<void>;
  ...
  syncSession(
    db: NonNullable<ServiceInternals["db"]>,
...
describe("GlobalSearchService", () => {
  ...
  it("filters by container (session and agent)", async () => {
    ...
    await writeWire(home!, "s2", "agent-1", [
      userLine("苹果 from s2 subagent", T1),
    ]);
    ...
    const inS2 = await service.search({
      query: "苹果",
      container: { sessionId: "s2" },
    });
  ...
  it("filters by role and time range", async () => {
    ...
    const ranged = await service.search({
      query: "苹果",
      startTime: T2,
      endTime: T2,
    });
  ...
  it("paginates with an opaque cursor and rejects changed conditions", async () => {
    ...
    // Malformed token.
    await expect(
      service.search({ query: "苹果", pageToken: "!!!" }),
    ).rejects.toBeInstanceOf(GlobalSearchError);
  });
  ...
  it("rejects a pageToken that decodes to a non-object", async () => {
    ...
    for (const payload of ["null", "42", '"str"', "[1,2]"]) {
      ...
      await expect(
        service.search({ query: "苹果", pageToken: token }),
  ...
  it("drops docs of a wire file that disappears while its session remains", async () => {
    ...
    const subFile = await writeWire(home!, "s1", "agent-1", [
      userLine("苹果 sub agent", T2),
    ]);
    const service = track(makeService(home!, staticIndex([s1])));
    await service.reindex();
  ...
  it("rejects reindex on a read-only instance", async () => {
    ...
    const reader = track(makeService(home!, index));
    await expect(reader.reindex()).rejects.toMatchObject({
      reason: "readonly_index",
    });
  });
  ...
  it("serves the building page while the index base is rebuilding, real hits after commit", async () => {
    ...
    const db = internals(service).db as unknown as {
    ...
    try {
      ...
      const buildingLiteral = await service.search({
        query: "苹果",
        mode: "literal",
      });
  ...
  it("read-only instance serves the building page while its base is rebuilding", async () => {
    ...
    const db = internals(reader).db as unknown as {
      textIndexBuilding(name: string): boolean;
    };
    const original = db.textIndexBuilding.bind(db);
    db.textIndexBuilding = () => true;
  ...
  it("assigns 0-based turn ordinals to user and assistant hits", async () => {
    ...
    const assistants = await service.search({
      query: "苹果",
      role: "assistant",
      sort: "time_asc",
    });
  ...
  it("restarts the turn counter when a shrunk file is rescanned", async () => {
    ...
    await service.reindex();
    expect(
      (await service.search({ query: "苹果", sort: "time_asc" })).items.map(
        (h) => h.turn,
      ),
    ).toEqual([0, 1, 2]);
  ...
  it("rewinds the counter on context.undo and renumbers after it", async () => {
    ...
    const page = await service.search({ query: "苹果", sort: "time_asc" });
    const bySnippet = (needle: string) =>
      page.items.find((h) => h.snippet.includes(needle));
  ...
  it("keeps step attribution across incremental sync passes", async () => {
    ...
    await appendFile(
      file,
      `${assistantStepLine("苹果 reply", "u1", T2)}\n`,
      "utf8",
    );
    await settleSync(service);
    const page = await service.search({ query: "苹果", role: "assistant" });
  ...
  it("rescans a wire file whose meta predates step tracking", async () => {
    ...
    await appendFile(
      file,
      `${assistantStepLine("苹果 reply two", "u1", T2)}\n`,
      "utf8",
    );
    await settleSync(service);
    const page = await service.search({
  ...
  describe("literal mode", () => {
    ...
    it("finds symbol substrings exactly; distractors without the exact substring do not hit", async () => {
      ...
      const passed = await service.search({
        query: "**已通过**",
        mode: "literal",
      });
      ...
      const latex = await service.search({
        query: "$\\frac{a}{b}$",
        mode: "literal",
      });
      ...
      const korean = await service.search({
        query: "한국어 문구",
        mode: "literal",
      });
    ...
    it("rejects queries shorter than 2 normalized characters", async () => {
      const service = await literalFixture();
      await expect(
        service.search({ query: "c", mode: "literal" }),
      ).rejects.toMatchObject({
    ...
    it("flags candidate-cap truncation as incomplete", async () => {
      ...
      expect(page.items.every((h) => h.snippet.includes("cap-target"))).toBe(
        true,
      );
    ...
    it("paginates and rejects page tokens after a mode change", async () => {
      ...
      const termsPage = await service.search({
        query: "page-target",
        pageSize: 2,
      });
      await expect(
  ...
  describe("stage-4 bounded lifecycle", () => {
    it("serves the published generation without waiting for a blocked background sync", async () => {
      ...
      const page = await Promise.race([
        ...
        new Promise<never>((_, reject) =>
          setTimeout(
            () => reject(new Error("search waited for the blocked sync")),
            2_000,
          ),
        ),
      ]);
    ...
    it("scopes one session sync to its own file-meta keys among 10k sessions", async () => {
      ...
      db.query = (criteria) => {
        ...
        if (criteria.key.prefix.startsWith("\0meta\\file\\"))
          metaRowsScanned += rows.length;
        return rows;
      };
    ...
    it("migrates legacy hash-only file-meta keys to the session-scoped format", async () => {
      ...
      const main = await writeWire(home!, "s1", "main", [
        userLine("苹果 main", T1),
      ]);
    ...
    it("paginates by keyset without duplicates or gaps under concurrent additive writes", async () => {
      ...
      const lines: string[] = [];
      for (let i = 0; i < 25; i++)
        lines.push(userLine(`苹果 doc ${i}`, T1 + i));
      ...
      const more: string[] = [];
      for (let i = 25; i < 30; i++)
        more.push(`${userLine(`苹果 doc ${i}`, T1 + i)}\n`);
      ...
      const times = [...page1.items, ...page2.items, ...page3.items].map(
        (h) => h.time,
      );
    ...
    it("rejects page tokens from an older generation after a rescan or a reindex", async () => {
      ...
      const lines: string[] = [];
      for (let i = 0; i < 30; i++)
        lines.push(userLine(`苹果 doc ${i} padding`, T1 + i));
      const file = await writeWire(home!, "s1", "main", lines);
      const service = track(makeService(home!, staticIndex([s1])));
      await service.reindex();
    ...
    it("terminates a hot 2-character literal query within the postings budget", async () => {
      ...
      const lines: string[] = [];
      for (let i = 0; i < 400; i++)
        lines.push(userLine(`的汉 filler ${i} about stuff`, T1 + i));
    ...
    it("accepts legacy v1 offset tokens and upgrades them to v2 keyset tokens", async () => {
      ...
      const lines: string[] = [];
      for (let i = 0; i < 30; i++)
        lines.push(userLine(`苹果 legacy ${i}`, T1 + i));
      await writeWire(home!, "s1", "main", lines);
      const service = track(makeService(home!, staticIndex([s1])));
      await service.reindex();
    ...
    it("paginates score sort by (score, time, key) without duplicates", async () => {
      ...
      for (let p = 0; p < 3; p++) {
        const page: Awaited<ReturnType<typeof service.search>> =
        ...
        for (let i = 1; i < page.items.length; i++) {
          expect(page.items[i]!.score).toBeLessThanOrEqual(
            page.items[i - 1]!.score,
          );
        }
    ...
    it("rejects over-budget queries: too many terms, oversized literal", async () => {
      ...
      await expect(
        service.search({ query: "aa bb cc dd" }),
      ).rejects.toMatchObject({
        reason: "invalid_query",
      });
      await expect(service.search({ query: "aa bb cc dd" })).rejects.toThrow(
        /too many terms/,
      );
      ...
      await expect(
        service.search({ query: oversized, mode: "literal" }),
      ).rejects.toMatchObject({
        reason: "invalid_query",
      });
      await expect(
        service.search({ query: oversized, mode: "literal" }),
      ).rejects.toThrow(/limited to 1024 characters/);
    });
    ...
    it("flags deadline and text-budget stops as incomplete instead of truncating silently", async () => {
      ...
      const lines: string[] = [];
      for (let i = 0; i < 200; i++)
        lines.push(userLine(`苹果 deadline ${i}`, T1 + i));
      await writeWire(home!, "s1", "main", lines);
      const service = track(makeService(home!, staticIndex([s1])));
      await service.reindex();
      ...
      const textStopped = await service.search({
        query: "苹果",
        mode: "literal",
      });
      ...
      const textComplete = await service.search({
        query: "苹果",
        mode: "literal",
      });
  ...
  describe("live route", () => {
    ...
    it("accepts single-character literal queries on the live route", async () => {
      ...
      service.setLiveTranscriptSource(
        fakeLiveSource(new Map([["s1", makeLiveStore("s1")]])),
      );
      ...
      expect(page.items.length).toBe(3);
      expect(page.items.map((h) => h.role).sort()).toEqual([
        "assistant",
        "title",
        "user",
      ]);
      ...
      await expect(
        service.search({ query: "苹", mode: "literal" }),
      ).rejects.toMatchObject({
        reason: "invalid_query",
      });
    });
    ...
    it("serves terms queries from the live store and orders hits by tf score", async () => {
      ...
      const dup = await service.search({
        query: "苹果 苹果",
        container: { sessionId: "s1" },
      });
      expect(dup.items.map((h) => h.time)).toEqual(
        page.items.map((h) => h.time),
      );
    });
    ...
    it("returns matching terms result sets on both routes for equivalent data", async () => {
      ...
      for (const page of [live, index]) {
        for (let i = 1; i < page.items.length; i++) {
          expect(page.items[i - 1]!.score).toBeGreaterThanOrEqual(
            page.items[i]!.score,
          );
        }
      }
    ...
    it("applies role, time, agent and sort filters on the live route", async () => {
      ...
      addLiveTurn(store, "main", {
        ...
        steps: [
          { stepId: "t0.1", startedAt: T2, texts: ["苹果 assistant answer"] },
        ],
      });
      addLiveTurn(store, "sub", {
        ordinal: 0,
        startedAt: T3,
        prompt: "苹果 subagent prompt",
      });
      const service = track(makeService(home!, gettableIndex([s1])));
      service.setLiveTranscriptSource(fakeLiveSource(new Map([["s1", store]])));
    ...
    it("scopes container.agentId queries to that agent only", async () => {
      ...
      const service = track(makeService(home!, gettableIndex([s1])));
      service.setLiveTranscriptSource(
        fakeLiveSource(new Map([["s1", store]]), calls),
      );
    ...
    it("searches the partial text of an in-flight turn", async () => {
      ...
      expect(
        page.items.some(
          (h) => h.role === "user" && h.snippet.includes("running"),
        ),
      ).toBe(true);
      expect(
        page.items.some(
          (h) => h.role === "assistant" && h.snippet.includes("partial"),
        ),
      ).toBe(true);
    });
  ...
  describe("lifecycle drain correctness (plan 13)", () => {
    it("closes the handle when post-open index setup fails, and the next open becomes the writer again (review #19)", async () => {
      ...
      await expect(service.reindex()).rejects.toThrow(
        "injected createTextIndex failure",
      );
      spy.mockRestore();
    ...
    it("dispose drains an in-flight read-only refresh before closing the db (review #20)", async () => {
      ...
      expect(
        (internals(reader).db as unknown as { readOnly: boolean } | null)
          ?.readOnly,
      ).toBe(true);
...
describe("baseline: synthetic corpus", () => {
  ...
  it("indexing and search latency scale within a linear budget from 100 to 400 sessions", async () => {
    ...
    const terms100 = await medianMs(() =>
      service.search({ query: "compaction" }),
    );
    ...
    expect(
      (await service.search({ query: "message 3 about", mode: "literal" }))
        .items.length,
    ).toBeGreaterThan(0);
