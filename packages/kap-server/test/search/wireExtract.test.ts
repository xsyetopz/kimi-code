...
describe("extractFromWireLine", () => {
  it("extracts user text messages", () => {
    ...
    expect(out).toEqual([
      { role: "user", text: "帮我重构搜索模块", time: 1_700_000_000_000 },
    ]);
  });

  it("keeps user messages without an origin", () => {
    const out = extractFromWireLine(
      userRecord("hello world", 1_700_000_000_000),
    );
  ...
  it.each([
    ...
    "compaction_summary",
    "plugin_command",
  ])("filters out origin kind %s", (kind) => {
    expect(
      extractFromWireLine(
        userRecord("not user input", 1_700_000_000_000, { kind }),
      ),
    ).toEqual([]);
  });
  ...
  it.each(["skill_activation", "plugin_command"])(
    ...
    (kind) => {
      ...
      expect(out).toEqual([
        { role: "user", text: "/commit 整理提交", time: 1_700_000_000_000 },
      ]);
    },
  );
  ...
  it.each(["model-tool", "nested-skill"])(
    ...
    (trigger) => {
      expect(
        extractFromWireLine(
          userRecord("skill body", 1_700_000_000_000, {
  ...
  it("ignores non-user append_message roles", () => {
    const out = extractFromWireLine(
      line({
        ...
        message: {
          role: "assistant",
          content: [{ type: "text", text: "assistant as message" }],
        },
      }),
    );
    expect(out).toEqual([]);
  ...
  it("extracts assistant text content parts from loop events", () => {
    const out = extractFromWireLine(
      line({
        ...
        event: {
          type: "content.part",
          part: { type: "text", text: "Here is the refactor plan." },
        },
      }),
    );
    expect(out).toEqual([
    ...
    expect(
      extractFromWireLine(
        line({
          ...
          event: {
            type: "content.part",
            part: { type: "thinking", thinking: "hmm" },
          },
        }),
      ),
    ).toEqual([]);
  ...
  it("ignores other record types, blank lines and unparseable JSON", () => {
    expect(
      extractFromWireLine(line({ type: "metadata", protocol_version: "1.4" })),
    ).toEqual([]);
  ...
  it("drops messages whose text is empty after trimming", () => {
    expect(
      extractFromWireLine(
        userRecord("   ", 1_700_000_000_000, { kind: "user" }),
      ),
    ).toEqual([]);
  });
  ...
  it("normalizes second-based timestamps to epoch ms", () => {
    const out = extractFromWireLine(
      userRecord("seconds time", 1_700_000_000, { kind: "user" }),
    );
...
describe("analyzeWireLine turn effects", () => {
  ...
  it("user messages without an origin open an anchor turn", () => {
    ...
    expect(turnOf(userRecord("hi", 1, { kind: "user" }))).toEqual({
      kind: "open",
      anchor: true,
    });
  });
  ...
  it("hidden origins do not open turns, except turn-opening system triggers", () => {
    expect(
      turnOf(userRecord("x", 1, { kind: "injection", variant: "v" })),
    ).toEqual({
      kind: "none",
    });
    expect(turnOf(userRecord("x", 1, { kind: "retry" }))).toEqual({
      kind: "none",
    });
    expect(
      turnOf(userRecord("x", 1, { kind: "system_trigger", name: "reminder" })),
    ).toEqual({
      kind: "none",
    });
    expect(
    ...
    expect(
      turnOf(userRecord("x", 1, { kind: "system_trigger", name: "subagent" })),
    ).toEqual({
      kind: "open",
      anchor: false,
    });
  ...
  it("marker origins open a turn only for user-slash prompts", () => {
    ...
    expect(
      turnOf(
        userRecord("/s", 1, { kind: "plugin_command", trigger: "user-slash" }),
      ),
    ).toEqual({
      kind: "open",
      anchor: true,
    ...
    expect(
      turnOf(
        userRecord("s", 1, { kind: "skill_activation", trigger: "model-tool" }),
      ),
    ).toEqual({
      kind: "none",
    });
    expect(
      turnOf(userRecord("sum", 1, { kind: "compaction_summary" })),
    ).toEqual({ kind: "none" });
  });

  it("cron / task / hook / shell_command origins open non-anchor turns", () => {
    expect(
      turnOf(userRecord("cron", 1, { kind: "cron_job", jobId: "j" })),
    ).toEqual({
      kind: "open",
      anchor: false,
    });
    ...
    expect(
      turnOf(userRecord("h", 1, { kind: "hook_result", event: "stop" })),
    ).toEqual({
      kind: "open",
      anchor: false,
    });
    expect(
      turnOf(userRecord("!", 1, { kind: "shell_command", phase: "input" })),
    ).toEqual({
      kind: "open",
      anchor: false,
    });
  ...
  it("compaction and clear do NOT renumber; undo carries its count; invalid undo is none", () => {
    ...
    expect(turnOf(line({ type: "context.undo", count: 2 }))).toEqual({
      kind: "undo",
      count: 2,
    });
    expect(turnOf(line({ type: "context.undo", count: 0 }))).toEqual({
      kind: "none",
    });
  ...
  it("other record types have no effect", () => {
    expect(turnOf(line({ type: "metadata", protocol_version: "1.4" }))).toEqual(
      { kind: "none" },
    );
