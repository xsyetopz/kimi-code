...
describe("server-v2 GET /api/v1/sessions/:id/snapshot", () => {
  ...
  it("reflects the durable watermark and in-flight turn after events", async () => {
    ...
    emit(sid, {
      ...
      turnId: 1,
      delta: "Hello",
    } as unknown as DomainEvent); // volatile
  ...
  it("returns 404 for an unknown session", async () => {
    const res = await fetch(
      `${base}/api/v1/sessions/sess_does_not_exist/snapshot`,
      {
        headers: authHeaders(server as RunningServer),
      } as never,
    );
  ...
  it("returns the persisted transcript for a cold session", async () => {
    ...
    expect((snap.messages.items[0]!.content[0] as { text: string }).text).toBe(
      "hello-from-disk",
    );
    expect((snap.messages.items[1]!.content[0] as { text: string }).text).toBe(
      "hi-from-disk",
    );
    expect(snap.epoch).toMatch(/^ep_/);
  });
  ...
  it("serves a v1-layout session (ISO timestamps, no id field) without crashing", async () => {
    ...
    if (resumed === undefined)
      throw new Error(`session ${sid} failed to resume`);
    const main = await resumed.accessor
