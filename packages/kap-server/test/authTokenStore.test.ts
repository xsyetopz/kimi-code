...
describe("privateFiles", () => {
  ...
  it.skipIf(process.platform === "win32")(
    "readPrivateFile throws on a 0644 file",
    async () => {
...
describe("persistentToken", () => {
  ...
  it.skipIf(process.platform === "win32")(
    "writes server.token with mode 0600",
    async () => {
      const home = join(tmpDir, "home");
  ...
  it("rotateServerToken writes a new, different token to server.token", async () => {
    ...
    expect(readFileSync(join(home, "server.token"), "utf8").trim()).toBe(
      rotated,
    );
  });
});
...
describe("password", () => {
  it("resolvePasswordHash returns undefined when env is unset or empty", async () => {
    ...
    expect(
      await resolvePasswordHash({ KIMI_CODE_PASSWORD: "" }),
    ).toBeUndefined();
  });
