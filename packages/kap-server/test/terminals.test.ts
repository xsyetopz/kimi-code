...
describe("server-v2 /api/v1/sessions/{sid}/terminals", () => {
  ...
  it("creates terminals for multiple sessions using each session workspace cwd", async () => {
    ...
    try {
      ...
      const termA = (
        await post<Terminal>(`/api/v1/sessions/${sidA}/terminals`, {
          cols: 100,
          rows: 30,
        })
      ).data;
      const termB = (
        await post<Terminal>(`/api/v1/sessions/${sidB}/terminals`, {})
      ).data;
      ...
      expect(spawnOptions.map((o) => o.cwd)).toEqual([
        resolve(rootA),
        resolve(rootB),
      ]);

      const listA = (
        await get<{ items: Terminal[] }>(`/api/v1/sessions/${sidA}/terminals`)
      ).data;
      const listB = (
        await get<{ items: Terminal[] }>(`/api/v1/sessions/${sidB}/terminals`)
      ).data;
  ...
  it("resolves an explicit relative cwd against the session workspace", async () => {
    ...
    const term = (
      await post<Terminal>(`/api/v1/sessions/${sid}/terminals`, { cwd: "sub" })
    ).data;
  ...
  it("gets and closes a terminal by session id", async () => {
    const sid = await createSession(work as string);
    const terminal = (
      await post<Terminal>(`/api/v1/sessions/${sid}/terminals`, {})
    ).data;

    const got = (
      await get<Terminal>(`/api/v1/sessions/${sid}/terminals/${terminal.id}`)
    ).data;
  ...
  it("maps terminal-not-found, cwd-escape and unknown-session to protocol codes", async () => {
    ...
    const missing = await get<unknown>(
      `/api/v1/sessions/${sid}/terminals/term_missing`,
    );
    ...
    const noSession = await get<unknown>(
      `/api/v1/sessions/sess_missing/terminals`,
    );
