...
describe("requestLogging", () => {
  ...
  it("logs the envelope code instead of the HTTP status code", async () => {
    ...
    const res = await fetch(
      `http://127.0.0.1:${String(server.port)}/api/v1/healthz`,
    );
    ...
    const completed = parseEntries(lines).filter(
...
describe("extractEnvelopeCode", () => {
  it("extracts a leading code from an envelope body", () => {
    expect(
      extractEnvelopeCode(
        '{"code":0,"msg":"success","data":null,"request_id":"r"}',
      ),
    ).toBe(0);
    expect(
      extractEnvelopeCode(
        '{"code":40001,"msg":"validation.failed","data":null,"request_id":"r"}',
      ),
    ).toBe(40001);
  });
