...
describe("InFlightTurnTracker", () => {
  it("accumulates assistant text and reports pre-append offsets", () => {
    ...
    expect(
      t.apply(SID, ev({ type: "assistant.delta", turnId: 1, delta: "Hello" })),
    ).toEqual({
      offset: 0,
    });
    expect(
      t.apply(SID, ev({ type: "assistant.delta", turnId: 1, delta: " world" })),
    ).toEqual({
      offset: 5,
    });
  ...
  it("tracks thinking offsets independently", () => {
    ...
    t.apply(SID, ev({ type: "turn.started", turnId: 1 }));
    expect(
      t.apply(SID, ev({ type: "thinking.delta", turnId: 1, delta: "abc" })),
    ).toEqual({
      offset: 0,
    });
    expect(
      t.apply(SID, ev({ type: "thinking.delta", turnId: 1, delta: "de" })),
    ).toEqual({
      offset: 3,
    });
    expect(t.get(SID)).toMatchObject({
  ...
  it("ignores deltas for a mismatched turn", () => {
    ...
    t.apply(SID, ev({ type: "turn.started", turnId: 1 }));
    expect(
      t.apply(SID, ev({ type: "assistant.delta", turnId: 99, delta: "stale" })),
    ).toEqual({});
    expect(t.get(SID)?.assistant_text).toBe("");
  });
    ...
    expect(t.get(SID)?.running_tools).toEqual([
      {
        tool_call_id: "tc1",
        name: "bash",
        last_progress: { kind: "stdout", text: "hi" },
      },
    ]);
  ...
  it("reports step-relative offsets that restart at 0 each step", () => {
    ...
    t.apply(SID, ev({ type: "turn.step.started", turnId: 1, step: 1 }));
    expect(
      t.apply(SID, ev({ type: "assistant.delta", turnId: 1, delta: "ab" })),
    ).toEqual({ offset: 0 });
    expect(
      t.apply(SID, ev({ type: "assistant.delta", turnId: 1, delta: "cd" })),
    ).toEqual({ offset: 2 });
