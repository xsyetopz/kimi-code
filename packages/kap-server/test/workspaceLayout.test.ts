...
describe("local/local on-disk layout (byte compatibility)", () => {
  ...
  it("persists the pre-refactor layout byte-for-byte and serves it through the snapshot reader", async () => {
    const created = await postJson<{ id: string; workspace_id: string }>(
      "/api/v1/sessions",
      {
    ...
    const workspacesFile = JSON.parse(
    ...
    expect(workspacesFile.workspaces[workspaceId]).toMatchObject({
      root: workDir,
    });
    ...
    expect((JSON.parse(firstWireLine) as { type: string }).type).toBe(
      "metadata",
    );
    const metaWithAgent = JSON.parse(
      await readFile(join(sessionDir, "state.json"), "utf8"),
    ) as {
      agents: Record<string, { homedir: string }>;
    };
    expect(metaWithAgent.agents["main"]?.homedir).toBe(
      join(sessionDir, "agents", "main"),
    );

    // The snapshot reader serves the layout straight from disk (auto mode).
    const snapshot = await fetch(
      `${base}/api/v1/sessions/${sessionId}/snapshot`,
      {
        headers: authHeaders(server!),
      },
    );
    const snapshotBody = (await snapshot.json()) as Envelope<{
      session: { id: string };
    }>;
    ...
    const second = await postJson<{ id: string; workspace_id: string }>(
      "/api/v1/sessions",
      {
        metadata: { cwd: workDir },
      },
    ...
    expect(indexAfter.map((entry) => entry.sessionId)).toEqual([
      sessionId,
      second.data.id,
    ]);
  });
