...
describe("server-v2 /api/v1/sessions/{sid}/messages", () => {
  ...
  it("lists spliced messages newest-first with stable ids and mapped content", async () => {
    ...
    expect(tool).toMatchObject({
      role: "tool",
      content: [
        { type: "tool_result", tool_call_id: "call_1", output: "file.txt" },
      ],
    });
  });
  ...
  it("returns 40401 for an unknown session on both endpoints", async () => {
    ...
    const got = await getJson<null>(
      "/api/v1/sessions/nope/messages/msg_does_not_exist",
    );
  ...
  it("paginates with page_size and before_id / after_id cursors", async () => {
    ...
    const all = await getJson<PageWire>(
      `/api/v1/sessions/${id}/messages?page_size=100`,
    );
    ...
    const first = await getJson<PageWire>(
      `/api/v1/sessions/${id}/messages?page_size=1`,
    );
    ...
    expect(older.body.data.items.map((m) => m.id)).toEqual([
      idsDesc[1],
      idsDesc[2],
    ]);
    ...
    expect(newer.body.data.items.map((m) => m.id)).toEqual([
      idsDesc[0],
      idsDesc[1],
    ]);
  ...
  it("filters the page by role after pagination", async () => {
    ...
    const { body } = await getJson<PageWire>(
      `/api/v1/sessions/${id}/messages?role=user`,
    );
    expect(body.code).toBe(0);
  ...
  it("reads the persisted full transcript for a cold session", async () => {
    ...
    const livePage = await getJson<PageWire>(
      `/api/v1/sessions/${id}/messages?page_size=100`,
    );
    ...
    const got = await getJson<MessageWire>(
      `/api/v1/sessions/${id}/messages/${m1.id}`,
    );
    expect(got.body.code).toBe(0);
    expect(got.body.data).toMatchObject({
      id: m1.id,
