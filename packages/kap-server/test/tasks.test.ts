...
describe("server-v2 /api/v1/sessions/{sid}/tasks", () => {
  ...
  it("filters the list by wire status", async () => {
    ...
    const running = await getJson<ListWire>(
      `/api/v1/sessions/${id}/tasks?status=running`,
    );
    ...
    const completed = await getJson<ListWire>(
      `/api/v1/sessions/${id}/tasks?status=completed`,
    );
  ...
  it("gets a single task by id and 40406 for an unknown task", async () => {
    ...
    const got = await getJson<TaskWire>(
      `/api/v1/sessions/${id}/tasks/${taskId}`,
    );
  ...
  it("cancelling an unknown task returns 40406", async () => {
    ...
    const { body } = await postJson<null>(
      `/api/v1/sessions/${id}/tasks/nope:cancel`,
    );
  ...
  it("returns 40401 for an unknown session on all three endpoints", async () => {
    ...
    const cancelled = await postJson<null>(
      "/api/v1/sessions/nope/tasks/tid:cancel",
    );
