...
describe("LocalKaos", () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = toPosix(
      await realpath(await mkdtemp(join(tmpdir(), "kaos-test-"))),
    );
  ...
  describe("iterdir + glob", () => {
    it("should list directory entries and match glob patterns", async () => {
      ...
      expect(new Set(names)).toEqual(
        new Set(["alpha", "bravo.txt", "charlie.TXT"]),
      );
  ...
  describe("glob hidden files", () => {
    ...
    it("should glob through hidden directories with ** pattern", async () => {
      ...
      await kaos.writeText(
        join(tempDir, "src", ".config", "settings.yml"),
        "debug: true",
      );
  ...
  describe("readLines streaming", () => {
    async function collectLines(
      path: string,
      options?: Parameters<LocalKaos["readLines"]>[1],
    ) {
      const lines: string[] = [];
    ...
    it("keeps utf16le and hex on the decode-then-split path", async () => {
      ...
      await expect(
        collectLines(utf16Path, { encoding: "utf16le" }),
      ).resolves.toEqual(["a\n", "ੁ\n"]);

      const hexPath = join(tempDir, "hex.txt");
      await kaos.writeBytes(hexPath, Buffer.from("a\nb"));
      await expect(collectLines(hexPath, { encoding: "hex" })).resolves.toEqual(
        ["610a62"],
      );
    });
  ...
  describe("scanTextFile", () => {
    ...
    it("detects NUL and invalid UTF-8", async () => {
      ...
      await expect(kaos.scanTextFile(nul)).resolves.toMatchObject({
        hasNul: true,
      });
  ...
  describe("readLineRange", () => {
    async function collectRange(
      ...
      const lines: string[] = [];
      for await (const line of kaos.readLineRange(path, {
        startLine,
        maxLines,
      })) {
        lines.push(line);
    ...
    it("reads only the requested line window", async () => {
      ...
      await expect(collectRange(filePath, 2, 2)).resolves.toEqual([
        "b\n",
        "c\n",
      ]);
    ...
    it("preserves U+FEFF at the start of a ranged non-first line", async () => {
      ...
      await kaos.writeText(filePath, "a\n\uFEFFb\n");
      await expect(collectRange(filePath, 2, 1)).resolves.toEqual([
        "\uFEFFb\n",
      ]);
    });
  });
  ...
  describe("readText errors parameter (Python compat)", () => {
    ...
    it('throws on invalid utf-8 with errors="strict" (default)', async () => {
      ...
      await expect(
        kaos.readText(filePath, { errors: "strict" }),
      ).rejects.toThrow();
    });
  ...
  describe("mkdir recursive", () => {
    ...
    it("should throw when parents:true + existOk:false on existing dir", async () => {
      ...
      await expect(
        kaos.mkdir(existing, { parents: true, existOk: false }),
      ).rejects.toThrow();
    });
    ...
    it("should succeed when parents:true + existOk:true on existing dir", async () => {
      ...
      await expect(
        kaos.mkdir(existing, { parents: true, existOk: true }),
    ...
    it("should throw when existOk:true but conflicting path is a file", async () => {
      ...
      await expect(
        kaos.mkdir(filePath, { existOk: true }),
  ...
  describe("glob ** pattern", () => {
    ...
    it("should not duplicate matches for deep ** patterns", async () => {
      ...
      await kaos.mkdir(join(tempDir, "l1", "l2", "l3", "l4"), {
        parents: true,
      });
      await kaos.writeText(
        join(tempDir, "l1", "l2", "l3", "l4", "deep.txt"),
        "x",
      );
  ...
  describe("glob symlink cycle safety", () => {
    ...
    it("T-C6 two non-cyclic symlinks to same target both traverse (path-local visited)", async () => {
      ...
      expect(
        matches.some(
          (p) => p.endsWith(`a/shared.txt`) || p.endsWith(`a\\shared.txt`),
        ),
      ).toBe(true);
      expect(
        matches.some(
          (p) => p.endsWith(`b/shared.txt`) || p.endsWith(`b\\shared.txt`),
        ),
      ).toBe(true);
    });
  });
  ...
  describe("exec streaming", () => {
    it("should run a command and stream stdout/stderr", async () => {
      ...
      expect(Buffer.concat(stdoutChunks).toString("utf-8").trim()).toBe(
        "hello",
      );
      expect(Buffer.concat(stderrChunks).toString("utf-8").trim()).toBe(
        "stderr line",
      );
    });
  });
  ...
  describe("exec spawn failure", () => {
    it("should reject when the binary does not exist", async () => {
      await expect(
        kaos.exec("/absolutely/non-existent/binary"),
      ).rejects.toThrow();
    });
    ...
    it("should reject execWithEnv() with an empty args array", async () => {
      ...
      await expect(kaos.execWithEnv([])).rejects.toThrow(
        /at least one argument/,
      );
    });
  });
  ...
  describe("exec timeout", () => {
    ...
    it("should allow killing a long-running process", async () => {
      ...
      const result = await Promise.race([
        proc
          .wait()
          .then((exitCode) => ({ kind: "exited" as const, code: exitCode })),
...
describe("LocalKaos instance isolation", () => {
  test("instances have isolated cwds (no process.cwd pollution)", async () => {
    ...
    const tmpA = toPosix(
      await realpath(await mkdtemp(join(tmpdir(), "kaos-a-"))),
    );
    const tmpB = toPosix(
      await realpath(await mkdtemp(join(tmpdir(), "kaos-b-"))),
    );
    ...
    try {
      ...
      const procA = await kaosA.exec(
        "node",
        "-e",
        "process.stdout.write(process.cwd())",
      );
      const procB = await kaosB.exec(
        "node",
        "-e",
        "process.stdout.write(process.cwd())",
      );
...
describe("LocalProcess.kill safety", () => {
  ...
  test.skipIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const tmp = await realpath(
        await mkdtemp(join(tmpdir(), "kaos-killtree-")),
      );
      try {
        ...
        const scriptPath = fileURLToPath(
          new URL("./fixtures/killtree.cjs", import.meta.url),
        );
  ...
  test.skipIf(process.platform === "win32")(
    ...
    async () => {
      const kaos = await LocalKaos.create();
      const tmp = await realpath(
        await mkdtemp(join(tmpdir(), "kaos-killtree-posix-")),
      );
      try {
