...
describe("Anthropic model profile matching", () => {
  ...
  it("does not claim an official profile for an unrecognized compatible model", () => {
    expect(
      matchKnownAnthropicModelProfile("Example Compatible Model"),
    ).toBeUndefined();
  });
  ...
  ])("does not claim the unknown-Claude fallback for %s", (model) => {
...
async function captureRequestBody(
  ...
  if (
    ...
    capturedOptions["headers"] !== undefined
  ) {
...
async function captureAnthropicMessages(
  ...
  let provider = new AnthropicChatProvider({
    ...
    stream: false,
    clientFactory: () =>
      ({ messages: { create }, beta: { messages: { create } } }) as never,
  });
...
async function captureBetaRequestBody(
  ...
  if (capturedParams === undefined) {
    throw new Error(
      "Expected provider.generate() to call beta.messages.create",
    );
  }
...
describe("betaApi", () => {
  ...
  it("routes to client.beta.messages.create with betas in the body and no beta header", async () => {
    ...
    const headers = body["_extra_headers"] as
...
describe("withThinkingKeep (context_management)", () => {
  ...
  it("does not duplicate the context-management beta or the clear_thinking edit across repeated calls", () => {
    ...
    expect(
      betas.filter((b) => b === "context-management-2025-06-27"),
    ).toHaveLength(1);
    expect(state.contextManagement).toEqual({
      edits: [{ type: "clear_thinking_20251015", keep: "all" }],
    });
...
describe("AnthropicChatProvider", () => {
  ...
  describe("message conversion", () => {
    ...
    it("maps json_schema response format to output_config.format", async () => {
      ...
      const history: Message[] = [
        {
          role: "user",
          content: [{ type: "text", text: "Extract contact" }],
          toolCalls: [],
        },
      ];
      const schema = {
    ...
    it("video url content rejects unsupported media type", async () => {
      ...
      const history: Message[] = [
        {
          ...
          content: [
            {
              type: "video_url",
              videoUrl: { url: "data:image/png;base64,AAAA" },
            },
          ] satisfies ContentPart[],
          toolCalls: [],
        },
      ...
      await expect(
        captureRequestBody(provider, "", [], history),
    ...
    it("tool result with video content", async () => {
      ...
      const lastContent = messages.at(-1)!.content as Array<{
        type: string;
        content: unknown[];
      }>;
    ...
    it("user audio parts degrade to placeholder text, video parts convert to video blocks", async () => {
      ...
      const history: Message[] = [
        {
          ...
          content: [
            ...
            {
              type: "audio_url",
              audioUrl: { url: "https://example.com/b.mp3" },
            },
            {
              type: "video_url",
              videoUrl: { url: "data:video/mp4;base64,AAAA" },
      ...
      expect(messages[0]?.["content"]).toEqual([
        ...
        {
          type: "text",
          text: "(audio omitted: not supported by this provider)",
        },
        {
          type: "video",
          source: { type: "base64", media_type: "video/mp4", data: "AAAA" },
    ...
    it("tool result audio degrades to placeholder text inside tool_result", async () => {
      ...
      const history: Message[] = [
        ...
        },
        {
          ...
          content: [],
          toolCalls: [
            { type: "function", id: "call_tts", name: "tts", arguments: "{}" },
          ],
        },
        {
          role: "tool",
          content: [
            {
              type: "audio_url",
              audioUrl: { url: "https://example.com/hi.mp3" },
            },
          ] satisfies ContentPart[],
          toolCallId: "call_tts",
          toolCalls: [],
    ...
    it("three parallel tool_results merged in order", async () => {
      ...
      const history: Message[] = [
        {
          role: "user",
          content: [{ type: "text", text: "Do three things" }],
          toolCalls: [],
        },
        {
          role: "assistant",
    ...
    it("merges a follow-up text turn into the preceding tool_results", async () => {
      ...
      expect(
        msgs[2]!.content.slice(0, 2).every((b) => b.type === "tool_result"),
      ).toBe(true);
    ...
    it("merges consecutive plain-text user messages into one", async () => {
      ...
      expect(msgs[0]!.role).toBe("user");
      expect(msgs[0]!.content.map((block) => block.text)).toEqual([
        "First",
        "Second",
        "Third",
      ]);
    ...
    it.each([
      ...
      async (model) => {
        ...
        const history: Message[] = [
          {
            role: "user",
            content: [{ type: "text", text: "Search for 429" }],
            toolCalls: [],
          },
          {
            role: "assistant",
  ...
  describe("with thinking", () => {
    ...
    it("future 4.6+ model uses adaptive thinking and passes xhigh through", async () => {
      const provider =
        createProvider("claude-sonnet-4-8").withThinking("xhigh");
      const body = await captureRequestBody(provider, "", [], thinkHistory);
    ...
    it("adaptive thinking off clears stale output_config", async () => {
      const provider = createProvider("opus-4-6")
        .withThinking("high")
        .withThinking("off");
      const body = await captureRequestBody(provider, "", [], thinkHistory);
    ...
    it("replaces the previous thinking config when called again", () => {
      ...
      expect(getGenerationState(provider).thinking).toEqual({
        type: "disabled",
      });
    });
    ...
    it("haiku-4-5 legacy: medium sends budget thinking without output_config", async () => {
      const provider = createProvider("claude-haiku-4-5-20251001").withThinking(
        "medium",
      );
      const body = await captureRequestBody(provider, "", [], thinkHistory);
    ...
    it.each([
      ...
      "adaptive-capable model %s routes through adaptive path",
      async (model) => {
        const provider = createProvider(model).withThinking("high");
        const body = await captureRequestBody(provider, "", [], thinkHistory);
    ...
    it.each([
      ...
      "non-adaptive model %s does not emit adaptive thinking",
      async (model) => {
        const provider = createProvider(model).withThinking("high");
        const body = await captureRequestBody(provider, "", [], thinkHistory);
    ...
    describe("supports adaptive thinking matrix", () => {
      ...
      it.each([
        ...
        expect((body["thinking"] as { type: string }).type).not.toBe(
          "adaptive",
        );
      });
  ...
  describe("stream response parsing", () => {
    it("yields text delta from stream events", async () => {
      ...
      (provider as any)._client.messages.create = vi
        .fn()
        .mockResolvedValue(stream) as never;
    ...
    it("streaming: generate() assembles parallel tool calls via index routing", async () => {
      ...
      expect(
        (message.toolCalls[0] as ToolCall & { _streamIndex?: number })
          ._streamIndex,
      ).toBeUndefined();
      expect(
        (message.toolCalls[1] as ToolCall & { _streamIndex?: number })
          ._streamIndex,
      ).toBeUndefined();
    });
    ...
    it("converts stream errors to ChatProviderError", async () => {
      ...
      (provider as any)._client.messages.create = vi
        .fn()
        .mockResolvedValue(errorStream) as never;
...
describe("resolveDefaultMaxTokens", () => {
  ...
  it("matches vendor-prefixed and suffixed third-party variants", () => {
    // Bedrock / Vertex / proxy prefixes
    expect(resolveDefaultMaxTokens("anthropic.claude-opus-4-7-v1:0")).toBe(
      128000,
    );
    ...
    expect(
      resolveDefaultMaxTokens("bedrock/anthropic.claude-opus-4-6-v1:0"),
    ).toBe(128000);
    ...
    expect(
      resolveDefaultMaxTokens("anthropic.claude-3-5-sonnet-20240620-v1:0"),
    ).toBe(8192);
  });
...
describe("AnthropicChatProvider constructor max_tokens", () => {
  ...
  it("honors defaultMaxTokens for unknown models", async () => {
    expect(
      await maxTokensFor("unknown-model", { defaultMaxTokens: 12345 }),
    ).toBe(12345);
  });
  ...
  it("lets defaultMaxTokens lower the budget for known models", async () => {
    expect(
      await maxTokensFor("claude-opus-4-7", { defaultMaxTokens: 200 }),
    ).toBe(200);
  });

  it("honors explicit defaultMaxTokens above the ceiling for known models", async () => {
    expect(
      await maxTokensFor("claude-opus-4-7", { defaultMaxTokens: 999999 }),
    ).toBe(999999);
  });
