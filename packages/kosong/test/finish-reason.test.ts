  ...
  it.each<[string | null | undefined, FinishReason | null, string | null]>([
    ...
    (raw, expectedFinish, expectedRaw) => {
      ...
      expect(result).toEqual({
        finishReason: expectedFinish,
        rawFinishReason: expectedRaw,
      });
    },
  );
});
function makeKimiStream(
  rawFinish: string | null | undefined,
...
function createOpenAILegacyProvider(
  response: unknown,
  stream: boolean,
...
function createAnthropicStreamProvider(
  events: unknown[],
...
function createAnthropicNonStreamProvider(
  response: unknown,
): AnthropicChatProvider {
  return new AnthropicChatProvider({
    model: "k25",
...
function createGoogleGenAIProvider(
  response: unknown,
  stream: boolean,
): GoogleGenAIChatProvider {
...
describe("GoogleGenAIStreamedMessage finish reason", () => {
  ...
  it.each<[string, FinishReason | null, string | null]>([
    ...
    "raw Gemini finishReason %j maps to %j (raw=%j)",
    async (raw, expectedFinish, expectedRaw) => {
