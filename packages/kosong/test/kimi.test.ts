...
describe("KimiChatProvider", () => {
  describe("message conversion", () => {
    ...
    it("adds Kimi-only types to JetBrains-like enum-only tool parameters without mutation", async () => {
      const provider = createProvider();
      const originalParameters = structuredClone(
        JETBRAINS_ENUM_ONLY_TOOL.parameters,
      );
      const history: Message[] = [
        {
          role: "user",
          ...
          function: {
            ...
            parameters: {
              ...
              properties: {
                ...
                externalHint: {
                  description: "Optional vendor-specific hint.",
                  type: "string",
                },
                replacement: { type: "string" },
              },
        ...
        chat: {
          completions: {
            create: vi
              .fn()
              .mockImplementation(() =>
                mockCreateResult(makeChatCompletionResponse("kimi-k2")),
              ),
          },
        },
      };
  ...
  describe("clone client sharing", () => {
    ...
    it("withGenerationKwargs clone shares the same OpenAI client as the original", () => {
      ...
      const clone = original.withGenerationKwargs({
        max_completion_tokens: 1024,
      });
      expect(getInternalClient(clone)).not.toBeUndefined();
      expect(
        Object.is(getInternalClient(clone), getInternalClient(original)),
      ).toBe(true);
    });
      ...
      const chunks = [
        {
          id: "chatcmpl-empty-reasoning",
          choices: [
            { index: 0, delta: { reasoning_content: "" }, finish_reason: null },
          ],
        },
      ];
      (
