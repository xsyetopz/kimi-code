...
describe("inferWireType (deprecated compatibility wrapper)", () => {
  it("answers only the wire, or undefined when the entry is not importable", () => {
    ...
    expect(inferWireType({ id: "x", type: "openai_responses" })).toBe(
      "openai_responses",
    );
    expect(
      inferWireType({ id: "amazon-bedrock", npm: "@ai-sdk/amazon-bedrock" }),
    ).toBeUndefined();
    expect(
      inferWireType({ id: "cohere", npm: "@ai-sdk/cohere" }),
    ).toBeUndefined();
    expect(
      inferWireType({
        id: "x",
...
describe("resolveCatalogImport — wire resolution", () => {
  ...
  it("infers anthropic from npm or id", () => {
    expect(
      resolveCatalogImport({ id: "anthropic", npm: "@ai-sdk/anthropic" }),
    ).toMatchObject({
      kind: "ok",
      wire: "anthropic",
      guessed: false,
  ...
  it("infers google-genai and vertexai", () => {
    expect(
      resolveCatalogImport({ id: "gemini", npm: "@ai-sdk/google" }),
    ).toMatchObject({
      kind: "ok",
      wire: "google-genai",
    });
  ...
  it("falls back to openai for vendor-specific SDKs models.dev does not type", () => {
    ...
    expect(
      resolveCatalogImport({ id: "x", type: "openai_responses" }),
    ).toMatchObject({
      guessed: false,
    });
  });
  ...
  it("refuses SDKs known to be proprietary instead of guessing", () => {
    ...
    expect(
      resolveCatalogImport({ id: "cohere", npm: "@ai-sdk/cohere" }),
    ).toEqual({
      kind: "invalid",
      reason: "proprietary-sdk",
    });
...
describe("resolveCatalogImport — endpoint resolution", () => {
  it("resolves without asking for official SDKs and env-resolved wires", () => {
    expect(
      resolveCatalogImport({ id: "openai", npm: "@ai-sdk/openai" }),
    ).toEqual({
      kind: "ok",
      wire: "openai",
      guessed: false,
  ...
  it("rejects blank and placeholder user URLs", () => {
    expect(
      resolveCatalogImport({ id: "xai", npm: "@ai-sdk/xai" }, "   "),
    ).toEqual({
      kind: "invalid",
...
describe("catalogBaseUrl", () => {
  it("strips a trailing /v1 for anthropic so the official SDK does not double it", () => {
    expect(
      catalogBaseUrl(
        { id: "k", api: "https://api.kimi.com/coding/v1" },
        "anthropic",
      ),
  ...
  it("leaves anthropic base URLs without a bare /v1 suffix untouched", () => {
    expect(
      catalogBaseUrl(
        { id: "a", api: "https://api.anthropic.com" },
        "anthropic",
      ),
    ).toBe("https://api.anthropic.com");
    expect(
      catalogBaseUrl({ id: "a", api: "https://host/v1beta" }, "anthropic"),
    ).toBe("https://host/v1beta");
  });

  it("passes openai-family base URLs through unchanged (SDK appends /chat/completions)", () => {
    expect(
      catalogBaseUrl({ id: "o", api: "https://api.openai.com/v1" }, "openai"),
  ...
  it("returns undefined for env-placeholder URLs the config cannot express", () => {
    expect(
      catalogBaseUrl({ id: "neon", api: "${NEON_BASE_URL}/v1" }, "openai"),
    ).toBeUndefined();
    expect(
      catalogBaseUrl(
        {
  ...
  it("adaptBaseUrlForWire strips a trailing /v1 only for the Anthropic wire", () => {
    expect(
      adaptBaseUrlForWire("https://gateway.example.test/v1", "anthropic"),
    ).toBe("https://gateway.example.test");
    expect(
      adaptBaseUrlForWire("https://gateway.example.test/v1/", "anthropic"),
    ).toBe("https://gateway.example.test");
    expect(
      adaptBaseUrlForWire("https://gateway.example.test/v1", "openai"),
    ).toBe("https://gateway.example.test/v1");
    expect(adaptBaseUrlForWire("https://host/v1beta", "anthropic")).toBe(
      "https://host/v1beta",
    );
  });
});
...
describe("catalogModelToCapability", () => {
  ...
  it("defaults tool_use to true and skips models without a positive context", () => {
    expect(
      catalogModelToCapability({ id: "m", limit: { context: 1000 } })
        ?.capability.tool_use,
    ).toBe(true);
    expect(catalogModelToCapability({ id: "m" })).toBeUndefined();
    expect(
      catalogModelToCapability({ id: "m", limit: { context: 0 } }),
    ).toBeUndefined();
  });
  ...
  it.each<[CatalogModelEntry["interleaved"], string | undefined]>([
    ...
    "derives reasoningKey from interleaved=%j → %j",
  ...
  it("strips the always-thinking inference only where the wire encodes a true off", () => {
    ...
    expect(anthropic[0]?.supportEfforts).toEqual([
      "low",
      "medium",
      "high",
      "max",
  ...
  it("tracks limit.input separately from the total context window", () => {
    ...
    const weird = catalogModelToCapability({
      id: "m",
      limit: { context: 1000, input: 5000 },
    });
    ...
    const placeholder = catalogProviderModels({
      ...
      models: {
        "vendor/claude-model": {
          ...
          provider: {
            npm: "@ai-sdk/anthropic",
            api: "${NEON_BASE_URL}/anthropic/v1",
          },
        },
      },
    });
    ...
    const models = catalogProviderModels({
      ...
      models: {
        "tenant-model": {
          ...
          provider: {
            npm: "@ai-sdk/openai-compatible",
            api: "https://tenant.example.test/v1",
          },
        },
