...
const POST_COMPACTION_SHAPE: Message[] = [
  {
    role: "user",
    content: [{ type: "text", text: "An earlier user prompt" }],
    toolCalls: [],
  },
  {
    role: "user",
    content: [
      { type: "text", text: "<summary>Conversation so far…</summary>" },
    ],
    toolCalls: [],
  },
  {
    ...
    content: [
      {
        type: "text",
        text: "<system-reminder>Stay on task.</system-reminder>",
      },
    ],
    toolCalls: [],
  },
...
const STEER_AFTER_TOOL_RESULT: Message[] = [
  ...
  {
    role: "user",
    content: [{ type: "text", text: "Now multiply them instead" }],
    toolCalls: [],
  },
];
...
async function anthropicWireRoles(history: Message[]): Promise<string[]> {
  ...
  if (captured === undefined)
    throw new Error("Anthropic provider did not call messages.create");
...
async function googleWireRoles(history: Message[]): Promise<string[]> {
  ...
  if (captured === undefined)
    throw new Error("Google provider did not call a model endpoint");
