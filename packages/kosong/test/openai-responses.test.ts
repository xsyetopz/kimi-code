...
async function captureRequestBody(
  ...
  ((provider as any)._client.responses as unknown as Record<string, unknown>)[
    "create"
...
describe("OpenAIResponsesChatProvider", () => {
  describe("message conversion", () => {
      ...
      const history: Message[] = [
        {
          ...
          content: [
            ...
            {
              type: "audio_url",
              audioUrl: { url: "data:audio/mp3;base64,QUJD" },
            },
          ],
          toolCalls: [],
        },
      ...
      const history: Message[] = [
        {
          ...
          content: [
            {
              type: "audio_url",
              audioUrl: { url: "data:audio/wav;base64,V0FW" },
            },
          ],
          toolCalls: [],
        },
      ...
      const history: Message[] = [
        {
          ...
          content: [
            {
              type: "audio_url",
              audioUrl: { url: "https://example.com/speech.mp3" },
            },
          ],
          toolCalls: [],
        },
      ...
      const history: Message[] = [
        {
          ...
          content: [
            ...
            {
              type: "audio_url",
              audioUrl: { url: "data:audio/ogg;base64,T0dH" },
            },
          ],
          toolCalls: [],
        },
      ...
      const history: Message[] = [
        ...
        },
        {
          ...
          content: [
            {
              type: "image_url",
              imageUrl: { url: "https://example.com/shot.png" },
            },
          ],
    ...
    it("video_url in tool result degrades to placeholder text in function_call_output", async () => {
      ...
      const history: Message[] = [
        {
          role: "user",
          content: [{ type: "text", text: "Record it" }],
          toolCalls: [],
        },
        {
          role: "assistant",
        ...
        },
        {
          ...
          content: [
            {
              type: "video_url",
              videoUrl: { url: "https://example.com/rec.mp4" },
            },
          ],
          toolCallId: "call_rec",
          toolCalls: [],
  ...
  describe("reasoning configuration", () => {
    ...
    it('with_thinking("max") passes max through to the wire', async () => {
      ...
      expect((body["reasoning"] as Record<string, unknown>)["effort"]).toBe(
        "max",
      );
    });
    ...
    it("does not filter concrete efforts through a client-side allow-list", async () => {
      ...
      const maxBody = await captureRequestBody(
        provider.withThinking("max"),
        "",
        [],
        history,
      );
      const xhighBody = await captureRequestBody(
        provider.withThinking("xhigh"),
        "",
        [],
        history,
      );

      expect((maxBody["reasoning"] as Record<string, unknown>)["effort"]).toBe(
        "max",
      );
      expect(
        (xhighBody["reasoning"] as Record<string, unknown>)["effort"],
      ).toBe("xhigh");
    });
  });
  ...
  describe("response parsing", () => {
    it("yields text from non-stream response", async () => {
      ...
      (provider as any)._stream = false;
      (
        (provider as any)._client.responses as unknown as Record<
          string,
          unknown
        >
      )["create"] = vi.fn().mockResolvedValue(makeResponsesAPIResponse());
      ...
      const history: Message[] = [
        {
          ...
          content: [
            ...
            {
              type: "audio_url",
              audioUrl: { url: "file:///path/to/audio.mp3" },
            },
          ],
          toolCalls: [],
        },
  ...
  describe("streaming", () => {
    ...
    it("uses final arguments from function_call_arguments.done when no deltas are emitted", async () => {
      ...
      (
        (provider as any)._client.responses as unknown as Record<
          ...
          unknown
        >
      )["create"] = vi.fn().mockResolvedValue(makeAsyncIterable(events));
    ...
    it("yields ThinkPart from response.output_item.done reasoning item (with encrypted_content)", async () => {
      ...
      expect(parts).toEqual([
        { type: "think", think: "", encrypted: "enc_done" },
      ]);
    });
    ...
    it("ignores unknown stream event types and continues with known events", async () => {
      ...
      await expect(collectStreamParts(stream)).resolves.toEqual([
        { type: "text", text: "hi" },
      ]);
    });
    ...
    it("normalizes malformed gateway error frames with nested rate-limit JSON", async () => {
      ...
      expect((caughtError as Error).message).not.toContain(
        "stream event.type must be a string",
      );
    });
    ...
    it("fails fast on response.failed with an insufficient_quota code", async () => {
      ...
      expect((caughtError as APIProviderQuotaExhaustedError).statusCode).toBe(
        429,
      );
