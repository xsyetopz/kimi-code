...
describe("providers without message-level tool declarations", () => {
  ...
  it("openai responses skips the message", async () => {
    ...
    let captured: Record<string, unknown> | undefined;
    ((provider as any)._client.responses as Record<string, unknown>)["create"] =
...
describe("dynamically_loaded_tools capability bit", () => {
  ...
  it("catalog entries map the capability, defaulting to false", () => {
    ...
    expect(
      catalogModelToCapability(base)?.capability.dynamically_loaded_tools,
    ).toBe(false);
    expect(
      catalogModelToCapability({ ...base, dynamically_loaded_tools: true })
