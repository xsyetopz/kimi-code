...
const ANTHROPIC_THINKING_VISION_TOOL_CAPABILITY: ModelCapability =
  Object.freeze({
    image_in: true,
    video_in: false,
    audio_in: false,
...
const GEMINI_THINKING_MULTIMODAL_TOOL_CAPABILITY: ModelCapability =
  Object.freeze({
    image_in: true,
    video_in: true,
    audio_in: true,
...
export function getOpenAILegacyModelCapability(
  modelName: string,
): ModelCapability {
  return capabilityFromCatalog(modelName, OPENAI_LEGACY_CAPABILITY_CATALOG);
}

export function getOpenAIResponsesModelCapability(
  modelName: string,
): ModelCapability {
...
export function getGoogleGenAIModelCapability(
  modelName: string,
): ModelCapability {
  const normalized = normalizeModelName(modelName);
