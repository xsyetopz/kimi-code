...
describe("Session.listBackgroundTasks / getBackgroundTaskOutput", () => {
  ...
  it("returns empty output for an unknown task id", async () => {
    ...
    try {
      ...
      await expect(
        session.getBackgroundTaskOutput("bash-deadbeef"),
      ).resolves.toBe("");
    } finally {
      await harness.close();
    }
  ...
  it("rejects after the session is closed", async () => {
    ...
    try {
      ...
      await expect(
        session.getBackgroundTaskOutput("bash-aaaaaaaa"),
      ).rejects.toMatchObject({
        name: "KimiError",
        code: "session.closed",
      } satisfies Partial<KimiError>);
      await expect(
        session.stopBackgroundTask("bash-aaaaaaaa"),
      ).rejects.toMatchObject({
        name: "KimiError",
        code: "session.closed",
      } satisfies Partial<KimiError>);
