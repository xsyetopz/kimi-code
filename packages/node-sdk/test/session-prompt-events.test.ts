...
describe("Session.prompt events", () => {
  ...
  it("persists sanitized prompt metadata without marking the title custom", async () => {
    ...
    try {
      ...
      const mediaState = JSON.parse(
        await readFile(statePath, "utf-8"),
      ) as Record<string, unknown>;
      expect(mediaState["title"]).toBe(
        "use api_key=[redacted] for the request",
      );
  ...
  it("includes persisted subagent replay only when resume explicitly requests it", async () => {
    ...
    try {
      ...
      if (spawned?.type !== "subagent.spawned")
        throw new Error("Expected persisted subagent");
      await session.close();

      const defaultResume = await harness.resumeSession({ id: session.id });
      expect(defaultResume.getResumeState()?.agents).not.toHaveProperty(
        spawned.subagentId,
      );
      ...
      expect(
        fullResume.getResumeState()?.agents[spawned.subagentId]?.replay,
      ).toContainEqual(
        expect.objectContaining({
          type: "message",
  ...
  it("starts btw through RPC as a forked subagent without prompt metadata updates", async () => {
    ...
    try {
      ...
      fakeProviderState.responseText =
        "The main agent is working from the existing context.";
      events.length = 0;
      done = waitForEvent(
        session,
      ...
      expect(events).not.toContainEqual(
        expect.objectContaining({ type: "subagent.failed" }),
      );
      expect(events).not.toContainEqual(
        expect.objectContaining({
          type: "session.meta.updated",
      ...
      const btwHistoryText = JSON.stringify(
        fakeProviderState.calls[1]?.history,
      );
  ...
  it("persists only conversation through the selected turn across resume", async () => {
    ...
    try {
      ...
      const replayText = visibleReplayText(
        resumed.getResumeState()?.agents["main"]?.replay ?? [],
      );
  ...
  it("omits subagents created after the selected historical turn", async () => {
    ...
    try {
      ...
      expect(
        Object.keys(fork.getResumeState()?.sessionMetadata.agents ?? {}),
      ).toEqual(["main"]);
    } finally {
      await harness.close();
    }
