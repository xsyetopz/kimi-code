...
`);
    ...
    expect("unsupportedProviderField" in config.providers["local"]!).toBe(
      false,
    );
    expect("customModelField" in config.models!["camel-model"]!).toBe(false);

    const rawProviders = config.raw?.["providers"] as Record<
      string,
      Record<string, unknown>
    >;
    const rawModels = config.raw?.["models"] as Record<
      string,
      Record<string, unknown>
    >;
    expect(rawProviders["local"]?.["unsupported_provider_field"]).toBe(
      "raw-only",
    );
