...
describe("Session context", () => {
  it("restores a session-only additional directory after close and resume", async () => {
    ...
    const additionalDir = await makeTempDir(
      tempDirs,
  ...
  it("clears context without replacing the session", async () => {
    ...
    try {
      const session = await harness.createSession({
        id: "ses_context_clear",
        workDir,
      });
      await session.importContext(
        "Earlier context to clear.",
        "file 'notes.md'",
      );
      ...
      await expect(session.getContext()).resolves.toEqual({
        history: [],
        tokenCount: 0,
      });
    } finally {
  ...
  it("restores the imported message with its estimated token count after resume", async () => {
    ...
    try {
      const session = await harness.createSession({
        id: "ses_context_resume",
        workDir,
      });
      await session.importContext(
        "A decision from the previous session.",
        "session 'old-session'",
      );
  ...
  it("rejects an import that would push existing context beyond the model limit", async () => {
    const homeDir = await makeTempDir(
      tempDirs,
      "kimi-sdk-context-overflow-home-",
    );
    const workDir = await makeTempDir(
      tempDirs,
