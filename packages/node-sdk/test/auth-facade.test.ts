...
describe("KimiHarness.auth", () => {
  ...
  it("exposes a cached access token without refreshing auth state", async () => {
    ...
    await expect(harness.auth.getCachedAccessToken()).resolves.toBe(
      "oauth-access-token",
    );
  });
...
`,
  ...
  it("provisions SDK config using an existing Kimi OAuth token", async () => {
    ...
    expect(
      new ProviderManager({ config }).resolveProviderConfig(
        config.defaultModel!,
      ),
    ).toMatchObject({
      modelCapabilities: {
...
`,
  ...
  it("logs in against environment OAuth host and base URL over persisted config", async () => {
    ...
    const storage = new FileTokenStorage(join(homeDir, "credentials"));
    await storage.save(
      resolveKimiTokenStorageName({ oauthKey: configuredOauthKey }),
      {
        ...freshToken(),
...
`,
    ...
    expect(
      warnings.some((w) => w.includes("models.kimi-code/kimi-for-coding")),
    ).toBe(true);
  });
  ...
  it("submitFeedback surfaces HTTP errors without throwing", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn<FetchMock>(
        async () =>
          new Response(
            JSON.stringify({ message: "feedback API rejected the request" }),
            {
              status: 401,
              headers: { "Content-Type": "application/json" },
            },
