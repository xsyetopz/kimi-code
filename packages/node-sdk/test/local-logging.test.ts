...
import {
  ...
  getRootLogger,
} from "../../agent-core/src/logging/logger";
...
function snapshotLogEnv(): Record<
  (typeof LOG_ENV_KEYS)[number],
  string | undefined
> {
  return Object.fromEntries(
    LOG_ENV_KEYS.map((key) => [key, process.env[key]]),
  ) as Record<(typeof LOG_ENV_KEYS)[number], string | undefined>;
}
...
describe("Local logging — harness integration", () => {
  ...
  it("default export includes rotated session log files without requiring active kimi-code.log", async () => {
    ...
    try {
      ...
      const sessionLogEntries = [...entries.keys()].filter(
        (entry) =>
          entry === "logs/kimi-code.log" ||
          entry.startsWith("logs/kimi-code.log."),
      );
  ...
  it("--include-global-log bundles global active and sets manifest field", async () => {
    ...
    expect(
      entries.get("logs/global/kimi-code.log")!.toString("utf-8"),
    ).toContain("untagged probe");
  });
  ...
  it("--include-global-log bundles the active root global log path", async () => {
    ...
    const workDir = await makeTempDir("kimi-log-work-");
    const first = createKimiHarness({
      identity: TEST_IDENTITY,
      homeDir: firstHome,
    });
    const firstSession = await first.createSession({
      id: "ses_first_global_export",
      workDir,
    });
    const second = createKimiHarness({
      identity: TEST_IDENTITY,
      homeDir: secondHome,
    });
    ...
    const entries = readZipEntries(await readFile(result.zipPath));
    const globalLog = entries
      .get("logs/global/kimi-code.log")!
      .toString("utf-8");
    const firstLog = await readOptionalFile(
      join(firstHome, "logs", "kimi-code.log"),
    );
  ...
  it("multiple KimiHarness constructions in the same process do not throw", async () => {
    const homeDir = await makeTempDir("kimi-log-home-");
    expect(() =>
      createKimiHarness({ identity: TEST_IDENTITY, homeDir }),
    ).not.toThrow();
    expect(() =>
      createKimiHarness({ identity: TEST_IDENTITY, homeDir }),
    ).not.toThrow();
  ...
  it("uses the latest harness homeDir for global diagnostic logging", async () => {
    ...
    const secondLog = await readFile(
      join(secondHome, "logs", "kimi-code.log"),
      "utf-8",
    );
