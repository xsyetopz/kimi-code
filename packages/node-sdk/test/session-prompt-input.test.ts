...
import type { CoreAPI, RPCMethods } from "../src/compat";
...
describe("Session.prompt input normalization", () => {
  ...
  it("scopes interactive agent id across awaited session operations", async () => {
    ...
    expect(rpc.enterPlanCalls).toEqual([
      { sessionId: "ses_scoped_agent", agentId: "agent-btw" },
    ]);
    expect(rpc.getPlanCalls).toEqual([
      { sessionId: "ses_scoped_agent", agentId: "agent-btw" },
    ]);
    expect(rpc.clearPlanCalls).toEqual([
      { sessionId: "ses_scoped_agent", agentId: "agent-btw" },
    ]);
  ...
  it("isolates overlapping interactive agent scopes while RPC resolution is pending", async () => {
    ...
    const first = rpc.withInteractiveAgent("agent-a", () =>
      session.setModel("model-a"),
    );
    const second = rpc.withInteractiveAgent("agent-b", () =>
