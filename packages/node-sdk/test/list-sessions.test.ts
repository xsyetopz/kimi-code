...
describe("SessionStore.list", () => {
  ...
  it("creates workDir-scoped session directories and a root session index", async () => {
    ...
    expect(summary.sessionDir).not.toBe(
      join(homeDir, "sessions", "ses_list_full"),
    );
  ...
  it("forks a session directory, rewrites metadata, and drops reserved goal state", async () => {
    ...
    expect(existsSync(join(fork.sessionDir, "upcoming-goals.json"))).toBe(
      false,
    );
    expect(existsSync(join(source.sessionDir, "upcoming-goals.json"))).toBe(
      true,
    );
  ...
  it("reads title from customTitle before title", async () => {
    ...
    const sessions = await store.list({ workDir });
    expect(sessions.find((session) => session.id === custom.id)?.title).toBe(
      "Custom Title",
    );
    expect(sessions.find((session) => session.id === fallback.id)?.title).toBe(
      "Fallback Title",
    );
  });
  ...
  it("keeps sessions visible when state.json is missing or malformed", async () => {
    ...
    await writeFile(
      join(malformed.sessionDir, "state.json"),
      "{bad json",
      "utf-8",
    );
  ...
  it("does not scan legacy flat session directories", async () => {
    const homeDir = await makeTempDir();
    const workDir = await makeTempDir();
    await mkdir(join(homeDir, "sessions", "ses_legacy_flat"), {
      recursive: true,
    });
