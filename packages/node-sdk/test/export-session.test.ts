...
import { createKimiHarness, KimiError, type SessionSummary } from "#/index";
    ...
    const data =
      method === 0
        ...
        : method === 8
          ...
          : null;
    if (data === null)
      throw new Error(`unsupported compression method: ${String(method)}`);
...
describe("exportSessionDirectory", () => {
  it("writes a zip with manifest and every session file", async () => {
    ...
    expect(result.manifest.exportedAt).toMatch(
  ...
  it("uses a timestamped default output path when outputPath is omitted", async () => {
    ...
    expect(basename(result.zipPath)).toMatch(
      /^kimi-debug-session_-\d{8}-\d{6}\.zip$/,
    );
...
describe("KimiHarness.exportSession", () => {
  ...
  it("rejects missing session ids", async () => {
    ...
    const missingExport = harness.exportSession({
      id: "ses_missing",
      version: "1.0.0-test",
    });
