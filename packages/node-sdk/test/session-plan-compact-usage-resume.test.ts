...
describe("Session plan, compact, usage, and resume APIs", () => {
  it("sets plan mode through manualEnterPlan and clears the active plan file", async () => {
    ...
    try {
      ...
      const planOn = waitForSessionEvent(
        session,
        (event) =>
          event.type === "agent.status.updated" && event.planMode === true,
      );
      ...
      const planOff = waitForSessionEvent(
        session,
        (event) =>
          event.type === "agent.status.updated" && event.planMode === false,
      );
  ...
  it("rejects manual compaction on an empty session with compaction.unable", async () => {
    ...
    try {
      const session = await harness.createSession({
        id: "ses_compact_runtime",
        workDir,
      });

      await expect(
        session.compact({ instruction: "Keep important facts." }),
  ...
  it.todo("marks resumed plan mode active when the restored plan has no plan data", async () => {
    const homeDir = await makeTempDir(
      tempDirs,
      "kimi-sdk-resume-legacy-plan-home-",
    );
  ...
  it("forks a session, drops goal state, and returns an active fork session", async () => {
    ...
    try {
      ...
      await expect(fork.getStatus()).resolves.toMatchObject({
        model: "test-model",
      });
      ...
      const enterRecord = forkRecords.find(
        (record) => record["type"] === "plan_mode.enter",
      );
      ...
      expect(
        forkRecords.some((record) => record["type"] === "goal.clear"),
      ).toBe(false);
