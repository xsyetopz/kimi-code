import type { KimiConfig } from "../src/compat";
...
describe("fetchCatalog", () => {
  it("fetches and returns the catalog map", async () => {
    const catalog = {
      anthropic: {
        id: "anthropic",
        models: { x: { id: "x", limit: { context: 1000 } } },
      },
    };
...
describe("applyCatalogProvider", () => {
  ...
  it("maps always-thinking models to always_thinking and carries the off encoding", () => {
    ...
    expect(config.models?.["gateway/gpt-5"]).toMatchObject({
      capabilities: ["thinking", "tool_use"].map((c) =>
        c === "thinking" ? "always_thinking" : c,
      ),
      supportEfforts: ["low", "medium", "high"],
    });
    expect(config.models?.["gateway/gpt-5"]?.capabilities).not.toContain(
      "thinking",
    );
