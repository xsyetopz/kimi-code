...
function scrubHomePrefixes(value: unknown, home: HomePair): unknown {
  ...
  for (const prefix of [home.raw, home.real].toSorted(
...
const KNOWN_DIFFS = {
  ...
  listPluginCommands: (
    ...
    home: HomePair,
  ): unknown => scrubHomePrefixes(defs, home),
  ...
  getPlan: (plan: SessionPlan, home: HomePair): unknown => {
    ...
    const projected = scrubHomePrefixes(
      { content: plan.content, path: plan.path },
      home,
    ) as {
      content: string;
      path: string;
    ...
    return {
      ...projected,
      path: projected.path.replaceAll(plan.id, "<PLAN_ID>"),
    };
  },
  ...
  // call sites.
  createGoal: (snapshot: GoalSnapshot | null): unknown =>
    projectGoalSnapshot(snapshot),
  getGoal: (result: GoalToolResult): unknown =>
    projectGoalSnapshot(result.goal),
  pauseGoal: (snapshot: GoalSnapshot | null): unknown =>
    projectGoalSnapshot(snapshot),
  resumeGoal: (snapshot: GoalSnapshot | null): unknown =>
    projectGoalSnapshot(snapshot),
  cancelGoal: (snapshot: GoalSnapshot | null): unknown =>
    projectGoalSnapshot(snapshot),
  ...
  exportSession: (result: ExportSessionResult, home: HomePair): unknown => {
    const projected = scrubHomePrefixes(result, home) as Record<
      string,
      unknown
    >;
...
function projectResumedSession(
  resumed: ResumedSessionSummary,
  home: HomePair,
): unknown {
  const projected = projectSessionSummary(resumed, home) as Record<
    string,
    unknown
  >;
...
function projectResumedAgent(
  ...
  const context = projected["context"] as {
  ...
  const replay = (projected["replay"] as readonly Record<string, unknown>[])
    ...
    .map((record) => {
      ...
      if (rest["type"] === "goal_updated") {
        rest["snapshot"] = projectGoalSnapshot(
          rest["snapshot"] as GoalSnapshot,
        );
      }
      return rest;
    });
  projected["replay"] = replay;
  const plan = projected["plan"] as {
    id: string;
    content: string;
    path: string;
  } | null;
...
function expectConfigParity(v1Config: KimiConfig, v2Config: KimiConfig): void {
  ...
  expect(normalize(project(v2Config), "")).toEqual(
    normalize(project(v1Config), ""),
  );
}
...
describe("v1↔v2 return-value parity", () => {
  ...
  it("listWorkspaceSkills returns the same skills on the same fixture", async () => {
    ...
    await writeSkill(
      join(homeDir, "skills", "parity-user-skill"),
      "parity-user-skill",
    );
    await writeSkill(
  ...
  it("setConfig returns the same merged config and reads back identically", async () => {
    ...
    try {
      ...
      const [v1ReadBack, v2ReadBack] = await Promise.all([
        v1.getConfig(),
        v2.getConfig(),
      ]);
  ...
  it("removeProvider cascades to models and default pointers identically", async () => {
    ...
    try {
      ...
      for (const config of [v1Config, v2Config]) {
        ...
        expect(
          config.models !== undefined && "fixture-model" in config.models,
        ).toBe(false);
  ...
  it("getConfigDiagnostics is empty on a valid config", async () => {
    ...
    try {
      ...
      expect(normalize(v2Diagnostics, "")).toEqual(
        normalize(v1Diagnostics, ""),
      );
...
describe("v1↔v2 plugin parity", () => {
  ...
  it("getPluginInfo returns the same info modulo per-home paths and install stamps", async () => {
    ...
    try {
      ...
      expect(project(v2Info, pair.v2Home)).toEqual(
        project(v1Info, pair.v1Home),
      );
  ...
  it("setPluginMcpServerEnabled toggles one server identically on both engines", async () => {
    ...
    try {
      await installFixtureOnBoth(pair);
      await Promise.all([
        pair.v1.setPluginMcpServerEnabled(
          FIXTURE_PLUGIN_ID,
          "parity-stdio",
          false,
        ),
        pair.v2.setPluginMcpServerEnabled(
          FIXTURE_PLUGIN_ID,
          "parity-stdio",
          false,
        ),
      ...
      expect(
        v1Info.mcpServers.find((server) => server.name === "parity-stdio")
          ?.enabled,
      ).toBe(false);
    } finally {
      await closePluginPair(pair);
  ...
  it("listPluginCommands returns the same enabled commands", async () => {
    ...
    try {
      ...
      await Promise.all([
        (async () => {
          await pair.v1.setPluginMcpServerEnabled(
            FIXTURE_PLUGIN_ID,
            "parity-stdio",
            false,
          );
        ...
        (async () => {
          await pair.v2.setPluginMcpServerEnabled(
            FIXTURE_PLUGIN_ID,
            "parity-stdio",
            false,
          );
...
async function makeSessionParityPair(
  configToml?: string,
 ...
 *  roster to copy. */
async function materializeMainAgentOnBoth(
  pair: SessionParityPair,
  id: string,
): Promise<void> {
  await Promise.all([
  ...
  await Promise.all([
    pair.v1.resumeSession({ id }),
    pair.v2.resumeSession({ id }),
  ]);
}
...
async function appendMainWireRecord(
  ...
  throw new Error(
    `wire.jsonl for ${sessionId} not found under ${sessionsRoot}`,
  );
}
...
describe("v1↔v2 session lifecycle parity", () => {
  it("createSession returns the same summary for the same input", async () => {
    ...
    try {
      ...
      expect(project(v2Summary, pair.v2Home)).toEqual(
        project(v1Summary, pair.v1Home),
      );
      expect(v1Summary.additionalDirs).toEqual([extraDir]);
      expect(v1Summary.metadata).toEqual({
        origin: "parity",
        nested: { n: 1 },
      });
  ...
  it("listSessions reflects the same creates and renames across filters", async () => {
    ...
    try {
      await createOnBoth(pair, {
        id: "session_parity_one",
        metadata: { tag: "one" },
      });
      await Promise.all([
        pair.v1.createSession({
          id: "session_parity_two",
      ...
      await Promise.all([
        pair.v1.renameSession({
          id: "session_parity_one",
          title: "Parity Session One",
        }),
        pair.v2.renameSession({
          id: "session_parity_one",
          title: "Parity Session One",
        }),
      ]);
      ...
      expect(v1Filtered.map((summary) => summary.id)).toEqual([
        "session_parity_one",
      ]);
      ...
      expect(v1ById.map((summary) => summary.id)).toEqual([
        "session_parity_two",
      ]);
    } finally {
      await closeSessionPair(pair);
  ...
  it("forkSession copies the session with merged metadata on both engines", async () => {
    ...
    try {
      ...
      expect(v1Fork.metadata).toEqual({
        origin: "source",
        shared: "fork",
        fork: "yes",
      });
      expect(v2Fork.metadata).toEqual({
        origin: "source",
        shared: "fork",
        fork: "yes",
      });
  ...
  it("resumeSession returns the same session state modulo the pinned agents gap", async () => {
    ...
    try {
      ...
      const [v1Resumed, v2Resumed] = await Promise.all([
        pair.v1.resumeSession({
          id: "session_parity_resume",
          additionalDirs: [extraDir],
        }),
        pair.v2.resumeSession({
          id: "session_parity_resume",
          additionalDirs: [extraDir],
        }),
      ]);
      const project = KNOWN_DIFFS.resumeSession;
      expect(project(v2Resumed, pair.v2Home)).toEqual(
        project(v1Resumed, pair.v1Home),
      );
      ...
      await expect(
        pair.v1.resumeSession({ id: "session_missing" }),
      ).rejects.toMatchObject({
        code: ErrorCodes.SESSION_NOT_FOUND,
      });
      await expect(
        pair.v2.resumeSession({ id: "session_missing" }),
      ).rejects.toMatchObject({
        code: ErrorCodes.SESSION_NOT_FOUND,
      });
    } finally {
...
describe("v1↔v2 agent interaction parity", () => {
  ...
  it("setModel rejects an unknown alias with the same code on both engines", async () => {
    ...
    try {
      ...
      const input = {
        sessionId: "session_parity_agent_setmodel_bad",
        model: "missing-model",
      };
  ...
  it("setThinking applies and rejects unlisted efforts identically", async () => {
    ...
    try {
      ...
      await expect(
        pair.v1.setThinking({ ...input, effort: "bogus" }),
      ).rejects.toMatchObject({
      ...
      await expect(
        pair.v2.setThinking({ ...input, effort: "bogus" }),
      ).rejects.toMatchObject({
        code: ErrorCodes.MODEL_CONFIG_INVALID,
      });
      await expect(
  ...
  it("plan mode lifecycle matches: enter / getPlan / write / clear / cancel", async () => {
    ...
    try {
      ...
      expect(projectPlan(v2Plan, pair.v2Home)).toEqual(
        projectPlan(v1Plan, pair.v1Home),
      );
      ...
      expect(projectPlan(v2Filled, pair.v2Home)).toEqual(
        projectPlan(v1Filled, pair.v1Home),
      );
      ...
      expect(projectPlan(v2Cleared, pair.v2Home)).toEqual(
        projectPlan(v1Cleared, pair.v1Home),
      );
      expect(v1Cleared?.content).toBe("");
      // A second enter while active rejects with the same plain error.
      await expect(
        pair.v1.setPlanMode({ ...input, enabled: true }),
      ).rejects.toThrowError("Already in plan mode");
      await expect(
        pair.v2.setPlanMode({ ...input, enabled: true }),
      ).rejects.toThrowError("Already in plan mode");
      await Promise.all([
  ...
  it("importContext appends the byte-identical message and validates identically", async () => {
    ...
    try {
      ...
      expect(normalize(v2Context.history, "")).toEqual(
        normalize(v1Context.history, ""),
      );
      expect(v1Context.history).toHaveLength(1);
      expect(v1Context.history[0]).toMatchObject({
        role: "user",
        origin: { kind: "user" },
      });
      ...
      expect(normalize(v2After.history, "")).toEqual(
        normalize(v1After.history, ""),
      );
  ...
  it("importContext rejects an overflowing import with context.overflow on both engines", async () => {
    ...
    try {
      ...
      const input = {
        ...
        content:
          "This import is far too large for the tiny fixture context window.",
        source: "file 'big.md'",
      } as const;
  ...
  it("resumeSession replays the same per-agent history and state snapshot", async () => {
    ...
    try {
      ...
      expect(project(v2Limited, pair.v2Home)).toEqual(
        project(v1Limited, pair.v1Home),
      );
      const v1LimitedTypes = v1Limited.agents["main"]!.replay.map(
        (record) => record.type,
      );
      expect(v1LimitedTypes).toEqual(["message", "message"]);
      expect(
        v2Limited.agents["main"]!.replay.map((record) => record.type),
      ).toEqual(v1LimitedTypes);
    } finally {
      await closeSessionPair(pair);
      restoreEnv();
    }
  ...
  it("clearContext empties both histories", async () => {
    ...
    try {
      ...
      await Promise.all([
        pair.v1.clearContext(input),
        pair.v2.clearContext(input),
      ]);
      const [v1Context, v2Context] = await Promise.all([
        pair.v1.getContext(input),
        pair.v2.getContext(input),
  ...
  it("undoHistory failure modes differ as pinned (v1 partial + request.invalid, v2 atomic session.undo_unavailable)", async () => {
    ...
    try {
      ...
      await expect(
        pair.v1.undoHistory({ ...input, count: 2 }),
      ).rejects.toMatchObject({
        code: ErrorCodes.REQUEST_INVALID,
      });
      await expect(
        pair.v2.undoHistory({ ...input, count: 2 }),
      ).rejects.toMatchObject({
        code: "session.undo_unavailable",
      });
      const [v1Context, v2Context] = await Promise.all([
  ...
  it("createSession applies model / thinking / permission to the main agent", async () => {
    ...
    try {
      ...
      await Promise.all([
        pair.v1.createSession(drifted),
        pair.v2.createSession(drifted),
      ]);
  ...
  it("prompt launches a turn and updates title/lastPrompt identically", async () => {
    ...
    try {
      ...
      await Promise.all([
        pair.v1.prompt({
          ...input,
          input: [{ type: "text", text: "hello parity" }],
        }),
        pair.v2.prompt({
          ...input,
          input: [{ type: "text", text: "hello parity" }],
  ...
  it("activateSkill renders, launches, and rejects identically", async () => {
    ...
    await writeSkill(
      join(pair.workDir, ".kimi-code", "skills", "parity-skill"),
      "parity-skill",
    );
    try {
  ...
  it("getSessionWarnings reports an oversized AGENTS.md identically", async () => {
    ...
    try {
      ...
      expect(v1Warnings[0]).toMatchObject({
  ...
  it("getSessionWarnings flags a creation-time broken secondary recipe on both engines", async () => {
    const restoreEnv = scrubConfigEnv();
    const pair = await makeSessionParityPair(
      SECONDARY_MODEL_BROKEN_CONFIG_TOML,
    );
    try {
...
describe("v1↔v2 goal parity", () => {
  it("goal lifecycle matches: create / get / pause / resume / replace / cancel", async () => {
    ...
    try {
      ...
      expect(v1Created).toMatchObject({
        status: "active",
        turnsUsed: 0,
        tokensUsed: 0,
      });
      ...
      const [v1Got, v2Got] = await Promise.all([
      ...
      expect(v1Replaced).toMatchObject({
        status: "active",
        objective: "Replacement goal.",
      });
  ...
  it("getCronTasks returns the same empty list on a fresh session", async () => {
    ...
    try {
      ...
      await expect(
        pair.v1.getCronTasks({ sessionId: "session_missing" }),
      ).rejects.toMatchObject({
        code: ErrorCodes.SESSION_NOT_FOUND,
      });
      await expect(
        pair.v2.getCronTasks({ sessionId: "session_missing" }),
      ).rejects.toMatchObject({
        code: ErrorCodes.SESSION_NOT_FOUND,
      });
    } finally {
...
async function waitForBackgroundTask(
  ...
  for (;;) {
    ...
    if (Date.now() >= deadline)
      throw new Error("timed out waiting for a background task");
...
async function stopAndSettle(
  ...
  await waitForBackgroundTask(
    ...
    sessionId,
    (candidate) =>
      candidate.taskId === taskId && candidate.status !== "running",
  );
}
  ...
  it("task lifecycle matches: list / detach / output / stop", async () => {
    ...
    try {
      ...
      const [v1Tail, v2Tail] = await Promise.all([
        pair.v1.getBackgroundTaskOutput({
          ...input,
          taskId: v1Running.taskId,
          tail: 4,
        }),
        pair.v2.getBackgroundTaskOutput({
          ...input,
          taskId: v2Running.taskId,
          tail: 4,
        }),
      ]);
      ...
      const [v1Killed, v2Killed] = await Promise.all([
        waitForBackgroundTask(
          ...
          input.sessionId,
          (task) =>
            task.taskId === v1Running.taskId && task.status !== "running",
        ),
        waitForBackgroundTask(
          pair.v2,
          input.sessionId,
          (task) =>
            task.taskId === v2Running.taskId && task.status !== "running",
        ),
      ]);
      expect(projectList([v2Killed])).toEqual(projectList([v1Killed]));
      expect(v1Killed).toMatchObject({
        status: "killed",
        stopReason: "parity stop",
      });
...
describe("v1↔v2 print policy parity", () => {
  ...
  it("drain mode waits for pending tasks and then finishes", async () => {
    const restoreEnv = scrubConfigEnv();
    const pair = await makeAgentParityPair(
      printConfig('print_background_mode = "drain"'),
    );
    try {
  ...
  it("keep_alive_on_exit = true keeps the legacy drain mapping", async () => {
    const restoreEnv = scrubConfigEnv();
    const pair = await makeAgentParityPair(
      printConfig("keep_alive_on_exit = true"),
    );
    try {
  ...
  it("exit mode finishes immediately with tasks still running", async () => {
    const restoreEnv = scrubConfigEnv();
    const pair = await makeAgentParityPair(
      printConfig('print_background_mode = "exit"'),
    );
    try {
  ...
  it("steer mode finishes at the max-turns cap and the wall-clock ceiling", async () => {
    const restoreEnv = scrubConfigEnv();
    const maxTurnsPair = await makeAgentParityPair(
      printConfig("print_max_turns = 1"),
    );
    const ceilingPair = await makeAgentParityPair(
      printConfig("print_wait_ceiling_s = 1"),
    );
    try {
      ...
      const maxTurnsInput = {
        sessionId: "session_parity_print_maxturns",
      } as const;
      ...
      const ceilingInput = {
        sessionId: "session_parity_print_ceiling",
      } as const;
      ...
      await Promise.all([
        stopAndSettle(
          ceilingPair.v1,
          ceilingInput.sessionId,
          v1CeilingTask.taskId,
        ),
        stopAndSettle(
          ceilingPair.v2,
          ceilingInput.sessionId,
          v2CeilingTask.taskId,
        ),
      ]);
      // Print-policy calls require a live session on both engines.
      await expect(
        maxTurnsPair.v1.handlePrintMainTurnCompleted({
          sessionId: "session_missing",
        }),
      ).rejects.toMatchObject({ code: ErrorCodes.SESSION_NOT_FOUND });
      await expect(
        maxTurnsPair.v2.handlePrintMainTurnCompleted({
          sessionId: "session_missing",
        }),
      ).rejects.toMatchObject({ code: ErrorCodes.SESSION_NOT_FOUND });
      await expect(
        maxTurnsPair.v1.waitForBackgroundTasksOnPrint({
          sessionId: "session_missing",
        }),
      ).rejects.toMatchObject({ code: ErrorCodes.SESSION_NOT_FOUND });
      await expect(
        maxTurnsPair.v2.waitForBackgroundTasksOnPrint({
          sessionId: "session_missing",
        }),
      ).rejects.toMatchObject({ code: ErrorCodes.SESSION_NOT_FOUND });
    } finally {
...
async function makeGlobalMcpParityPair(
  mcpJson?: unknown,
): Promise<GlobalMcpParityPair> {
  const v1HomeDir = await makeTempDir("kimi-sdk-parity-v1-home-");
  const v2HomeDir = await makeTempDir("kimi-sdk-parity-v2-home-");
...
describe("v1↔v2 global MCP parity", () => {
  it("CRUD round-trips identically and writes byte-identical mcp.json files", async () => {
    ...
    try {
      ...
      expect(normalize(v2Initial, "name")).toEqual(
        normalize(v1Initial, "name"),
      );
      ...
      expect(normalize(v2Updated, "name")).toEqual(
        normalize(v1Updated, "name"),
      );
      ...
      expect(normalize(v2Removed, "name")).toEqual(
        normalize(v1Removed, "name"),
      );
      expect(v1Removed.map((server) => server.name)).toEqual([
        "existing-http",
        "added",
      ]);
  ...
  it("a malformed mcp.json rejects every read with the same config.invalid", async () => {
    const pair = await makeGlobalMcpParityPair();
    await writeFile(
      join(pair.v1HomeDir, "mcp.json"),
      "{ not valid json",
      "utf-8",
    );
    await writeFile(
      join(pair.v2HomeDir, "mcp.json"),
      "{ not valid json",
      "utf-8",
    );
    try {
  ...
  it("OAuth guards and flow bookkeeping match without contacting a server", async () => {
    ...
    try {
      ...
      await expectSameMcpRejection(
        pair,
        (client) =>
          client.completeGlobalMcpServerAuth({ flowId: "flow_missing" }),
        (client) =>
          client.completeGlobalMcpServerAuth({ flowId: "flow_missing" }),
      );
      await Promise.all([
...
describe("v1↔v2 session MCP parity", () => {
  ...
  async function makeSessionMcpPair(
    mcpJson?: unknown,
  ): Promise<SessionParityPair> {
    const pair = await makeSessionParityPair();
    if (mcpJson !== undefined) {
  ...
  it("listMcpServers reports the same entries after the initial connect", async () => {
    ...
    try {
      ...
      const [v1Servers, v2Servers] = await listMcpServersWhenSettled(
        pair,
        input,
      );
      expect(normalize(v2Servers, "name")).toEqual(
        normalize(v1Servers, "name"),
      );
      ...
      expect(byName.get("broken")).toMatchObject({
        status: "failed",
        toolCount: 0,
      });
      expect(byName.get("off")).toMatchObject({
        status: "disabled",
        toolCount: 0,
      });
  ...
  it("getMcpStartupMetrics agrees, exactly 0 without servers", async () => {
    ...
    try {
      ...
      const emptyPair = await makeSessionMcpPair();
      try {
        await createOnBoth(emptyPair, {
          id: "session_parity_mcp_metrics_empty",
        });
        const emptyInput = {
          sessionId: "session_parity_mcp_metrics_empty",
        } as const;
        const [v1Empty, v2Empty] = await Promise.all([
  ...
  it("reconnectMcpServer rejects and reconnects identically", async () => {
    ...
    try {
      ...
      const [v1Unknown, v2Unknown] = await Promise.all([
        captureRejection(
          pair.v1.reconnectMcpServer({ ...input, name: "missing" }),
        ),
        captureRejection(
          pair.v2.reconnectMcpServer({ ...input, name: "missing" }),
        ),
      ]);
...
describe("v1↔v2 event & interaction parity", () => {
  it("forwards the same shell.* event sequence for a `!` command", async () => {
    ...
    try {
      ...
      const v2Completed = projectEventStream(v2Events, input.sessionId).filter(
        (projected) =>
          (projected as { type: string }).type === "shell.completed",
      );
  ...
  it("keeps exactly one event subscription per session across a reload", async () => {
    ...
    try {
      ...
      await Promise.all([
        pair.v1.reloadSession(input),
        pair.v2.reloadSession(input),
      ]);
      await Promise.all([
        pair.v1.runShellCommand({
          ...input,
      ...
      expect(
        v2Projected.filter(
          (projected) =>
            (projected as { type: string }).type === "shell.started",
        ),
      ).toHaveLength(2);
    } finally {
      await closeSessionPair(pair);
    ...
    try {
      ...
      const projectFailure = (events: readonly Event[]): unknown[] =>
        projectEventStream(events, input.sessionId).flatMap((projected) => {
          ...
          if (entry.type === "error")
            return { type: entry.type, code: entry.code };
          return entry;
        });
      ...
      expect(
        v1Projected.map((event) => (event as { type: string }).type),
      ).toEqual(
...
describe("v1↔v2 residual surface parity", () => {
  it("exportSession packages the same entry list and manifest on both engines", async () => {
    ...
    try {
      ...
      expect(normalize(v2Result.entries, "")).toEqual(
        normalize(v1Result.entries, ""),
      );
      ...
      expect(project(v2Result, pair.v2Home)).toEqual(
        project(v1Result, pair.v1Home),
      );
  ...
  it("startBtw forks a side-question child with the parent context on both engines", async () => {
    ...
    try {
      ...
      const [v1Context, v2Context] = await Promise.all([
        pair.v1.withInteractiveAgent(v1ChildId, () =>
          pair.v1.getContext({ sessionId }),
        ),
        pair.v2.withInteractiveAgent(v2ChildId, () =>
          pair.v2.getContext({ sessionId }),
        ),
      ]);
  ...
  it("setSwarmMode toggles swarm mode with the same reminder lifecycle on both engines", async () => {
    ...
    try {
      ...
      expect(JSON.stringify(v1Entered.history[0])).toContain(
        "<system-reminder>",
      );
  ...
  it("swarm() enters task-triggered swarm mode, prompts, and auto-exits after the turn", async () => {
    ...
    try {
      ...
      await Promise.all([
        pair.v1.swarm({
          ...input,
          input: [{ type: "text", text: "parity swarm prompt" }],
        }),
        pair.v2.swarm({
          ...input,
          input: [{ type: "text", text: "parity swarm prompt" }],
  ...
  it("listSkills returns the same merged catalog on the same fixture", async () => {
    ...
    try {
      ...
      await writeSkill(
        join(pair.v1Home.raw, "skills", "parity-user-skill"),
        "parity-user-skill",
      );
      await writeSkill(
        join(pair.v2Home.raw, "skills", "parity-user-skill"),
        "parity-user-skill",
      );
      await writeSkill(
