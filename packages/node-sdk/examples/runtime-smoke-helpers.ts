...
export function smokeIdentityFromEnv(): KimiHostIdentity {
  ...
  if (version === undefined || version.trim().length === 0) {
    throw new Error(
      "KIMI_CODE_SMOKE_VERSION is required for Kimi SDK smoke examples.",
    );
  }
  return {
...
export async function runPromptToEnd(
  session: Session,
  prompt: string,
): Promise<Event> {
  const stream = await startPromptStream(session, prompt, {
    waitForDelta: false,
  });
  return stream.ended;
}
...
function watchPromptStream(
  session: Session,
  waitForDelta: boolean,
): PromptStreamWatcher {
  ...
  unsubscribe = session.onEvent((event) => {
    ...
    if (event.type === "turn.ended") {
      if (firstDelta !== undefined && !sawDelta) {
        firstDelta.reject(
          new Error("Turn ended before any streaming delta was emitted"),
        );
      }
      ended.resolve(event);
      settled = true;
