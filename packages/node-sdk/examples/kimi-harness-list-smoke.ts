...
async function main(): Promise<void> {
  ...
  try {
    ...
    await delay(2);
    const sessionC = await harness.createSession({
      id: "ses_list_c",
      workDir: workDirA,
    });
    const sessionCSummary = (
      await harness.listSessions({ workDir: workDirA })
    ).find((item) => item.id === sessionC.id);
    ...
    if (workDirASessions.length !== 2) {
      throw new Error(
        `expected 2 workDirA sessions, got ${String(workDirASessions.length)}`,
      );
    }
    if (workDirBSessions.length !== 1) {
      throw new Error(
        `expected 1 workDirB session, got ${String(workDirBSessions.length)}`,
      );
    }
