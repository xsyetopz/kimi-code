...
async function readSessionLogs(sessionDir: string): Promise<{
  ...
  const files = (await readdir(logsDir))
    .filter(
      (file) => file === "kimi-code.log" || file.startsWith("kimi-code.log."),
    )
    .toSorted();
  const chunks = await Promise.all(
    files.map((file) => readFile(join(logsDir, file), "utf-8")),
  );
...
async function assertRotatedFilesStayBounded(
  ...
  assert(
    files.includes("kimi-code.log.1"),
    "session log did not rotate after exceeding max bytes",
  );
  assert(
    !files.includes("kimi-code.log.2"),
    "session log kept more archives than configured",
  );
  for (const file of files) {
    const size = (await stat(join(sessionDir, "logs", file))).size;
    assert(
      size <= MAX_LOG_BYTES,
      `${file} exceeded ${MAX_LOG_BYTES} bytes: ${size}`,
    );
  }
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    assert(
      exportedSessionLogs.length > 0,
      "default export did not include any session log files",
    );
    assert(
      !sessionOnly.entries.includes(GLOBAL_LOG),
      "default export included global log",
    );
    if (sessionOnly.entries.includes(SESSION_LOG)) {
      assert(
        sessionOnly.manifest.sessionLogPath === SESSION_LOG,
        "bad sessionLogPath",
      );
    } else {
      assert(
    ...
    assert(
      sessionOnly.manifest.globalLogPath === undefined,
      "default export set globalLogPath",
    );
    ...
    assert(
      !sessionLog.includes(globalOnly),
      "session log included global-only entry",
    );
    assert(
      !sessionLog.includes(evictedEntry),
      "oldest session log entry was not evicted",
    );
    assert(
      !sessionLog.includes("must-not-leak"),
      "session log leaked sensitive value",
    );
    assert(
      sessionLog.includes("[REDACTED]"),
      "session log did not redact sensitive value",
    );
    assert(
      sessionLog.includes(longEntry),
      "session log missed long-entry marker",
    );
    assert(
      sessionLog.includes("…"),
      "session log did not show truncation for long entry",
    );
    assert(
      sessionLog.includes(finalEntry),
      "session log missed final entry after rotation",
    );
    ...
    assert(
      withGlobal.manifest.globalLogPath === GLOBAL_LOG,
      "bad globalLogPath",
    );
    ...
    const globalLog = (
      await Promise.all(
        globalFiles.map((file) =>
          readFile(join(TEST_HOME, "logs", file), "utf-8"),
        ),
      )
    ).join("\n");
    assert(
      globalLog.includes(globalOnly),
      "global log missed global-only entry",
    );
    ...
    const globalSizeLines = await describeFiles(
      join(TEST_HOME, "logs"),
      globalFiles,
    );
    await writeFile(
