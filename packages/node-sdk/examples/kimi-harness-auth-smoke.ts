...
async function main(): Promise<void> {
  ...
  const homeDir =
    explicitHomeDir ??
    (await mkdtemp(join(tmpdir(), "kimi-sdk-auth-smoke-home-")));
  const workDir =
    explicitWorkDir ??
    (await mkdtemp(join(tmpdir(), "kimi-sdk-auth-smoke-work-")));
  ...
  try {
    ...
    if (
      config.providers[MANAGED_KIMI_CODE_PROVIDER]?.oauth?.key !==
      "oauth/kimi-code"
    ) {
...
function printDeviceCode(auth: {
  ...
  process.stdout.write(
    [
      ...
      auth.expiresIn === null
        ? undefined
        : `  Expires in: ${String(auth.expiresIn)}s`,
      "",
    ]
...
function printUsage(
  usage: Awaited<ReturnType<KimiHarness["auth"]["getManagedUsage"]>>,
): void {
  if (usage.kind === "error") {
  ...
  if (summary === null) {
    process.stdout.write(
      `usage: no summary, limits=${String(usage.limits.length)}\n`,
    );
    return;
  }
  ...
  const label =
    w !== undefined
      ? `${String(w.duration)}${w.unit} window`
      : (summary.name ?? "usage");
  process.stdout.write(
    `usage: ${label} ${String(summary.used)}/${String(summary.limit)}\n`,
  );
}
