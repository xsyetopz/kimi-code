...
const OUT = path.resolve(
  arg("out", path.join(os.tmpdir(), "minidb-kimi-code-" + Date.now())),
);
...
async function main() {
  ...
  console.log(
    `  db size on disk  : ${mib(sz)}  (${(sz / totalTextBytes).toFixed(2)}x raw text)`,
  );
  ...
  for (const q of queries) {
    ...
    for (const r of res.slice(0, 3)) {
      ...
      console.log(
        `     [${r.score.toFixed(3)}] ${r.value.workspaceName} :: ${title.slice(0, 60)}`,
      );
    }
  }
