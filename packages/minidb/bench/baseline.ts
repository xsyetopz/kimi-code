...
async function main() {
  ...
  // One JSONL record per set, O_APPEND, no fsync (rely on OS flush).
  {
    ...
    const ms = await bench(
      "naive set concurrent, fsync=no (O_APPEND)",
      async () => {
    ...
    await fh.close();
    console.log(
      `    -> ${ops(N, ms)}   (${(bytes / 1024 / 1024).toFixed(2)} MiB written)`,
    );
  ...
  // --- naive durable writes, sequential fsync=always --------------------
  {
    ...
    const fh = await fs.open(file, "a");
    const ms = await bench(
      `naive set sequential, fsync=always (N=${fmt(NSMALL)})`,
      async () => {
  ...
  // of minidb's compact()).
  {
    ...
    for (let i = 0; i < N; i++) m.set(`k${i}`, VALUE);
    const ms = await bench(
      `naive JSON snapshot of ${fmt(N)} keys`,
      async () => {
