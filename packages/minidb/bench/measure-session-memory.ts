...
async function main(): Promise<void> {
  ...
  for (let i = 0; i < N; i += CHUNK) {
    ...
    for (let j = i; j < end; j++) {
      ...
      const doc: SessionDoc = {
        ...
        lastPrompt:
          "please refactor the " + LATIN[(j * 7) % LATIN.length] + " module",
        text,
      ...
      ops.push({
        ...
        value: doc,
        dt: {
          updatedAt: 1700000000000 + j * 1000,
          createdAt: 1699000000000 + j * 1000,
        },
      });
    }
    await db.batch(ops);
  ...
  for (const e of ti.postings.values())
    postingsEntries += (e as { df: number }).df;
  ...
  console.log(
    `  avg encoded doc    : ${fmt((totalEncoded / N) | 0)} bytes  (JSON of value)`,
  );
  ...
  const rows: [string, Snap][] = [
    ...
    [
      "+ data: store + value buf + key skiplist + dt(updatedAt,createdAt)",
      afterData,
    ],
  ...
  console.log(
    `  total per session   : ${fmt(totalPerDoc | 0)} B  (heap + external)`,
  );
