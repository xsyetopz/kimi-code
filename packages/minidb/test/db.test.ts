...
test("backup + restore preserves data, indexes, and text search", async () => {
  ...
  try {
    ...
    const restored = await MiniDb.restore(backupDir, restoreDir, {
      valueCodec: "json",
    });
...
test("backup fences writes at a linearization point: every pre-fence ack is in, concurrent writes reject (review #22)", async () => {
  ...
  try {
    ...
    assert.equal(
      restored.get("inflight"),
      "c",
      "the write drained before the fence is included",
    );
    ...
    assert.equal(
      restored.get("after"),
      undefined,
      "a post-backup write is not included",
    );
    await restored.close();
  } finally {
    await fs.rm(dir, { recursive: true, force: true });
...
test("backup copy failure leaves no partial backup behind and reopens the write gate (review #22)", async () => {
  ...
  try {
    ...
    (fs as unknown as { copyFile: unknown }).copyFile = () =>
      Promise.reject(boom);
    try {
      await assert.rejects(
        db.backup(backupDir, { compact: false }),
    ...
    const leftovers = (await fs.readdir(parent)).filter((n) =>
      n.includes(".backup-"),
    );
    assert.deepEqual(
      leftovers,
      [],
      `stranded temp dirs: ${leftovers.join(", ")}`,
    );
...
test("query with skip/limit returns the same rows as slicing the unbounded result", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      db.query({ key: { prefix: "k1" }, limit: 10 }),
      full.slice(0, 10),
    );
    assert.deepEqual(
      db.query({ key: { prefix: "k1" }, skip: 5, limit: 3 }),
      full.slice(5, 8),
    );
    const eq = db.query({ filter: { grp: 2 } }); // indexed equality path
    assert.deepEqual(
      db.query({ filter: { grp: 2 }, limit: 7 }),
      eq.slice(0, 7),
    );
  ...
  test(`async reads match sync reads (valueMode: ${valueMode})`, async () => {
    ...
    try {
      ...
      for (let i = 0; i < 300; i++) {
        await db.set(
          `k${i}`,
          {
            kind: `t${i % 5}`,
            score: i,
            text: `hello world doc ${i} 异步 读取 ${i % 7}`,
          },
      ...
      for (const q of shapes) {
        assert.deepEqual(
          await db.queryAsync(q),
          db.query(q),
          `queryAsync(${JSON.stringify(q)})`,
        );
      }
      await db.close();
    } finally {
