...
test("createIndex validates field and duplicate names", async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => db.createIndex("bad", {} as never),
      /requires a field/,
    );
    await db.createIndex("byCity", { field: "city" });
    await assert.rejects(
      () => db.createIndex("byCity", { field: "city" }),
      /already exists/,
    );
    await db.close();
  } finally {
...
test("secondary indexes require the json codec", async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => db.createIndex("x", { field: "n" }),
      /require valueCodec: "json"/,
    );
    await db.close();
  } finally {
    await fs.rm(dir, { recursive: true, force: true });
...
function stubPersist(
  ...
  const priv = db as unknown as Record<
    typeof key,
    (defs: { name: string }[]) => Promise<void>
  >;
...
async function sidecarNames(dir: string, file: string): Promise<string[]> {
  try {
    return (
      JSON.parse(await fs.readFile(path.join(dir, file), "utf8")) as {
        name: string;
      }[]
    )
      .map((d) => d.name)
...
test("dropIndex persist failure: the live index stays usable and the sidecar is unchanged", async () => {
  ...
  try {
    ...
    assert.equal(
      await fs.readFile(path.join(dir, "db.indexes.json"), "utf8"),
      before,
    );
...
test("createIndex rebuild failure: no phantom, existing indexes keep serving, unique rollback unchanged", async () => {
  ...
  try {
    ...
    await assert.rejects(
      db.createIndex("byU", { field: "u", unique: true }),
      /unique/i,
    );
    assert.deepEqual(
      db.listIndexes().map((x) => x.name),
...
test("mixed create/drop across the three sidecar types: every op lands serialized; memory == sidecars == reopen", async () => {
  ...
  try {
    ...
    await db.createCompoundIndex("cmpOld", {
      groupBy: "ws",
      orderBy: "updatedAt",
    });
    ...
    assert.deepEqual(
      await sidecarNames(dir, "db.compound-indexes.json"),
      expectCmp,
    );
...
test("set/batch/del do not share the sidecar mutation chain (writes flow while a create is parked)", async () => {
  ...
  try {
    ...
    assert.equal(
      db.findEq("byA", 0).length,
      0,
      "k0 was deleted before publish",
    );
    assert.deepEqual(
...
test("toJSON/getter/Proxy docs: get, secondary/compound/text indexes and unique checks all see the persisted view (and reopen agrees)", async () => {
  ...
  try {
    ...
    let reads = 0;
    const getterDoc: Record<string, unknown> = {
      u: "u1",
      g: "G",
      o: 5,
      t: "alpha bravo",
