...
test("RESP: SET with EX / PX sets a TTL", async () => {
  ...
  try {
    const sock = await connect(srv.port);
    assert.equal(
      await send(sock, encode("SET", "ex", "v", "EX", "10")),
      "+OK\r\n",
    );
    ...
    assert.equal(
      await send(sock, encode("SET", "px", "v", "PX", "5000")),
      "+OK\r\n",
    );
...
function collectUntil(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(
      () =>
        reject(
          new Error(`timed out waiting for reply; got ${buf.length} bytes`),
        ),
      20_000,
    );
    sock.on("data", (d) => {
...
test("RESP: an oversized request gets -ERR and the connection recovers", {
  timeout: 30_000,
}, async () => {
  const dir = await tmpDir();
  const srv = await startServer({ dir, port: 0, fsyncPolicy: "no" });
  try {
    ...
    assert.ok(
      pong > tooLarge,
      "PING after the oversized request must be answered",
    );
    sock.end();
  } finally {
    await srv.close();
...
test("RESP: one bad command does not starve its pipelined siblings", async () => {
  ...
  try {
    ...
    const data = await collectUntil(sock, (s) =>
      s.endsWith("+PONG\r\n+PONG\r\n"),
    );
