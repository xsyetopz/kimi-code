...
test("postings codec: roundtrip + delta compression", () => {
  ...
  assert.ok(
    enc.length < entries.length * 8,
    `expected compression, got ${enc.length} bytes`,
  );
});
...
test("PostingsFile: rebuild + positioned read", async () => {
  ...
  try {
    ...
    const { dict: dict2 } = await PostingsFile.rebuild(p, [
      { term: "only", entries: [[7, 1]] },
    ]);
...
test("MiniDb: text postings written to disk, search survives reopen", async () => {
  ...
  try {
    ...
    assert.ok(
      fssync.existsSync(path.join(dir, "db.text-bio.postings")),
      "postings file exists",
    );
...
for (const indexGenerations of [false, true]) {
  test(`MiniDb: compaction rebuilds postings (file reclaimed) [indexGenerations: ${indexGenerations}]`, async () => {
    ...
    try {
      ...
      await db.createTextIndex("bio", { fields: ["bio"] });
      for (let i = 0; i < 50; i++)
        await db.set("k" + i, { bio: "hello world " + i });
      ...
      if (indexGenerations) {
        ...
        assert.ok(
          fssync.existsSync(p) || inGen,
          "postings persisted (root or generation)",
        );
      } else {
      ...
      for (let i = 0; i < 50; i++)
        await db.set("k" + i, { bio: "goodbye world " + i });
      for (let i = 50; i < 80; i++)
        await db.set("k" + i, { bio: "hello again " + i });
      ...
      if (indexGenerations) {
        ...
        assert.ok(
          !fssync.existsSync(p),
          "root postings reclaimed after generation publish",
        );
        const gen = db.getIndexGeneration()!;
        assert.ok(gen, "compaction published a generation");
        assert.ok(
          fssync.existsSync(
            path.join(dir, "generations", gen.id, "text-bio.postings"),
          ),
        ...
        assert.ok(
          fssync.existsSync(p),
          "legacy path keeps the root postings file",
        );
      }
      await db.close();
    } finally {
...
for (const indexGenerations of [false, true]) {
  test(`MiniDb: writes during a compaction postings rebuild stay consistent [indexGenerations: ${indexGenerations}]`, async () => {
    ...
    try {
      ...
      await db.createTextIndex("bio", { fields: ["bio"] });
      for (let i = 0; i < 50; i++)
        await db.set(`d${i}`, { bio: `hello doc${i}` });
...
test("trigram: normalization can change length (single ligature ﬀ becomes a legal query)", () => {
  ...
  assert.deepEqual(createNgramTokenizer({ forQuery: true })("ﬀ"), [
    ngramTerm("ff"),
  ]);
});
...
test("MiniDb: n-gram text index persists tokenizer, survives reopen", async () => {
  ...
  try {
    ...
    const defs = JSON.parse(
      await fs.readFile(path.join(dir, "db.textindexes.json"), "utf8"),
    ) as {
      name: string;
      tokenizer?: string;
    }[];
...
test("TextIndex: top-K over many candidates matches the full-ranking reference", async () => {
  ...
  try {
    ...
    assert.equal(
      all.length,
      matching,
      "unbounded search returns every scoring doc",
    );
    for (let i = 1; i < all.length; i++) {
      const [p, c] = [all[i - 1]!, all[i]!];
      assert.ok(
        p.score > c.score || (p.score === c.score && p.key < c.key),
        `rank order at ${i}`,
      );
    }
...
test("TextIndex: overwrite/remove prune the delta via the doc reverse map", () => {
  ...
  assert.equal(
    ti.termCount(),
    2,
    "pruned terms leave the vocabulary (cherry, mango)",
  );
...
test("TextIndex: maxVisits truncates a hot term and reports visits (disk-backed)", async () => {
  ...
  try {
    ...
    assert.ok(
      bounded.visits <= 100,
      `visits stay within the budget, got ${bounded.visits}`,
    );
    ...
    for (const h of bounded.hits)
      assert.ok(fullKeys.has(h.key), `${h.key} is a real match`);
...
test("TextIndex: maxVisits caps the memory base the same way", async () => {
  const ti = new TextIndex(); // memory base
  for (let i = 0; i < 200; i++)
    ti.add(`k${String(i).padStart(4, "0")}`, { bio: "x pad" });
  ...
  assert.equal(
    ti.search("x", { limit: 1_000 }).length,
    200,
    "unbudgeted search unaffected",
  );
  ti.close();
});
...
test("TextIndex: AND under a budget yields a subset with complete per-doc scores", async () => {
  ...
  try {
    ...
    const bounded = ti.searchBounded("hot rare", {
      limit: 1_000,
      maxVisits: 60,
    });
    assert.equal(bounded.truncated, true);
...
test("MiniDb: searchBounded surfaces values, visits and the truncated flag", async () => {
  ...
  try {
    ...
    const bounded = db.searchBounded("body", "x", {
      limit: 1_000,
      maxVisits: 80,
    });
    assert.equal(bounded.truncated, true);
...
test("MiniDb: dropTextIndex persist failure: the index stays searchable and the sidecar is unchanged", async () => {
  ...
  try {
    ...
    const before = await fs.readFile(
      path.join(dir, "db.textindexes.json"),
      "utf8",
    );
    const boom = new Error("injected sidecar write failure");
    const restore = stubTextPersist(db, async () => {
...
test("MiniDb: dropTextIndex persist window: a compaction postings rebuild skips the dropping index (no orphan, no leaked handle)", async () => {
  ...
  try {
    ...
    assert.equal(
      ti.needsRebuild(),
      true,
      "setup: the index is a rebuild candidate",
    );
    ...
    assert.equal(
      ti.building,
      false,
      "a dropping index must not start a postings build",
    );
    release();
...
test("TextIndex: a throwing tokenizer leaves the live view, delta and buildQueue untouched (review #24)", () => {
  ...
  assert.equal(
    (priv.buildQueue as unknown[] | null)?.length,
    0,
    "the failed add was never queued",
  );
  assert.deepEqual(
    ti.search("hello").map((h) => h.key),
    ["k"],
...
test("MiniDb: a throwing tokenizer rejects set with zero side effects; the old doc stays searchable (review #24)", async () => {
  ...
  try {
    ...
    const ti = (
      db as unknown as {
        text: Map<string, { tokenizer: (s: string) => string[] }>;
      }
    ).text.get("ft")!;
  ...
  try {
    ...
    const ti = (
      db as unknown as {
        text: Map<
          string,
          { tokenizer: (s: string) => string[]; customTokenizer: boolean }
        >;
      }
    ).text.get("ft")!;
    const origTokenizer = ti.tokenizer;
    ...
    assert.equal(
      db.get("bad"),
      undefined,
      "the poisonous doc never enters the store",
    );
...
test("decoded postings cache honors the byte budget, not just the term count", async () => {
  ...
  try {
    ...
    for (let i = 0; i < 200; i++) {
      ti.add(`k${i}`, {
        text: `alpha beta gamma doc${i} shared${i % 10} common`,
      });
    }
    await ti.build(
      [...Array(200)].map((_, i) => ({
    ...
    ti.search("gamma");
    ti.search("common");
    const cacheSize = (ti as unknown as { cache: Map<string, unknown> }).cache
      .size;
    assert.ok(
      cacheSize < 4,
      `byte budget evicted big lists (cache holds ${cacheSize})`,
    );
