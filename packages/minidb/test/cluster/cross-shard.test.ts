...
test("crossShard 'none' rejects multi-shard writes but allows single-shard ones", async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => ClusterDb.open({ dir, crossShard: "2pc" }),
      /2pc/,
    );
  } finally {
    await rmrf(dir);
...
test("secondary index: create, findEq/findRange merged across shards, maintained on new writes, drop", async () => {
  ...
  try {
    ...
    const db = await ClusterDb.open<U>({
      dir,
      shardCount: 8,
      valueCodec: "json",
    });
    ...
    assert.deepEqual(found.map((r) => r.key).sort(), [...expectedBj].sort());

    const ranged = await db.findRange("by-age", {
      min: 25,
      max: 30,
      maxExclusive: true,
    });
    ...
    for (let i = 1; i < ranged.length; i++)
      assert.ok(ranged[i]!.field >= ranged[i - 1]!.field);
    ...
    assert.deepEqual(defs.map((d) => d.name).sort(), ["by-age", "by-city"]);
...
test("text index: search merges per-shard results by score", async () => {
  ...
  try {
    ...
    const db = await ClusterDb.open<D>({
      dir,
      shardCount: 8,
      valueCodec: "json",
    });
    const docs: [string, D][] = [
      [
        "d:a",
        { title: "rust wal", body: "write ahead log durability in rust" },
      ],
      [
        "d:b",
        { title: "go wal", body: "write ahead log implementations compared" },
      ],
      ["d:c", { title: "garden", body: "tomatoes and basil on the balcony" }],
      ["d:d", { title: "log blog", body: "a blog about logging and logs" }],
      [
        "d:e",
        { title: "rust garden", body: "rust in the garden shed with tools" },
      ],
    ];
    await db.mset(docs);
    ...
    assert.ok(
      keys.includes("d:b") || keys.includes("d:a") || keys.includes("d:d"),
    );

    const limited = await db.search("txt", "rust OR log", {
      op: "OR",
      limit: 2,
    });
...
test("index definitions survive reopen (registry applied on shard open)", async () => {
  ...
  try {
    ...
    const db2 = await ClusterDb.open<U>({
      dir,
      shardCount: 4,
      valueCodec: "json",
    });
...
test("findRange applies offset/count/reverse to the globally merged result", async () => {
  ...
  try {
    ...
    const rows = entries
      .map(([key, d]) => ({ key, n: d.n }))
      .sort((a, b) => a.n - b.n || cmpKey(a.key, b.key));
    ...
    assert.deepEqual(
      await query({ offset: 12, count: 8 }),
      keys(rows.slice(12, 20)),
    );
...
test("concurrent createIndex from two instances keeps both registry entries", async () => {
  ...
  try {
    ...
    const shardCount = 4;
    const dbA = await ClusterDb.open<D>({
      dir,
      shardCount,
      valueCodec: "json",
    });
    const dbB = await ClusterDb.open<D>({
      dir,
      shardCount,
      valueCodec: "json",
    });
    ...
    const fresh = await ClusterDb.open<D>({
      dir,
      shardCount,
      valueCodec: "json",
    });
    assert.deepEqual((await fresh.listIndexes()).map((d) => d.name).sort(), [
      "ia",
      "ib",
    ]);
