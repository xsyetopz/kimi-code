...
test("cluster open creates meta + shard dirs; reload validates topology", async () => {
  ...
  try {
    ...
    const meta = JSON.parse(
      await fs.readFile(path.join(dir, "cluster.meta.json"), "utf8"),
    );
    ...
    for (let i = 0; i < 4; i++)
      assert.ok(entries.includes(`shard-0${i}`), `shard-0${i} exists`);
    await db.close();
    ...
    await assert.rejects(
      () => ClusterDb.open({ dir, shardCount: 8 }),
      /shardCount=4/,
    );
    await assert.rejects(
      () => ClusterDb.open({ dir, valueCodec: "string" }),
      /valueCodec=json/,
    );
    ...
    try {
      await assert.rejects(
        () => ClusterDb.open({ dir: dir2, shardCount: 0 }),
        RangeError,
      );
    } finally {
      await rmrf(dir2);
...
test("single-key ops round-trip across shards", async () => {
  const dir = await tmpDir("minidb-cluster-");
  try {
    const db = await ClusterDb.open<User>({
      dir,
      shardCount: 4,
      valueCodec: "json",
    });
...
test("mset/mget/mdel span shards (atomic per shard)", async () => {
  ...
  try {
    ...
    assert.ok(
      grouped.size >= 3,
      `keys hit multiple shards, got ${grouped.size}`,
    );
...
test("scan/prefix merge across shards: sorted, bounded, limited, reversible", {
  timeout: 60_000,
}, async () => {
  const dir = await tmpDir("minidb-cluster-");
  try {
...
test("hash routing distributes keys over all shards and is deterministic", async () => {
  ...
  for (let s = 0; s < 16; s++) {
    ...
    assert.ok(
      counts[s]! < avg * 3,
      `shard ${s} not hot (${counts[s]} vs avg ${avg})`,
    );
  }
...
test("query() merges filter + sort + skip/limit across shards", async () => {
  const dir = await tmpDir("minidb-cluster-");
  try {
    const db = await ClusterDb.open<QueryDoc>({
      dir,
      shardCount: 4,
      valueCodec: "json",
    });
    ...
    const sorted = (rows: { value: QueryDoc | undefined }[]) =>
      rows.map((e) => e.value!.n);
...
test("compound index definitions fan out to all shards and survive reopen", async () => {
  ...
  try {
    ...
    const info = [
      { name: "byGN", groupBy: "g", orderBy: "n", orderType: "number" },
    ];
    ...
    await db.close();
    const db2 = await ClusterDb.open<QueryDoc>({
      dir,
      shardCount: 4,
      valueCodec: "json",
    });
