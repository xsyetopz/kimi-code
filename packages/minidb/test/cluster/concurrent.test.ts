...
test("P=4 processes write disjoint keyspaces concurrently (S=8); all data survives", {
  timeout: 180_000,
}, async () => {
  ...
  try {
    ...
    const runs = Array.from({ length: P }, (_, p) =>
      runWorkerOk(["write", dir, "8", `p${p}`, String(N)], {
        timeoutMs: 120_000,
      }),
    );
    ...
    await Promise.all(
      Array.from({ length: P }, (_, p) =>
        runWorkerOk(["verify", dir, "8", `p${p}`, String(N)], {
          timeoutMs: 120_000,
        }),
      ),
    );
    ...
    const db = await ClusterDb.open<{ p: string; i: number }>({
      dir,
      readOnly: true,
    });
    let count = 0;
...
test("concurrent writers on the SAME shard serialize safely with no lost writes", {
  timeout: 180_000,
}, async () => {
  const dir = await tmpDir("minidb-cluster-mp-");
  try {
    ...
    const keysPerProc = Array.from({ length: procs }, (_, p) =>
      Array.from({ length: perProc }, (_, i) =>
        keyOnShard(`hot${p}:${i}`, targetShard, shards),
      ),
    );
    const runs = keysPerProc.map((keys) =>
      runWorkerOk(["writekeys", dir, String(shards), keys.join(",")], {
        timeoutMs: 150_000,
      }),
    );
    const reports = await Promise.all(runs);
    ...
    const db = await ClusterDb.open<{ key: string }>({
      dir,
      shardCount: shards,
      readOnly: true,
    });
...
test("a reader process observes commits from a concurrently running writer process", {
  timeout: 180_000,
}, async () => {
  const dir = await tmpDir("minidb-cluster-mp-");
  try {
    ...
    const waiter = runWorkerOk(
      ["wait-read", dir, "4", "live:k", "42", "120000"],
      { timeoutMs: 150_000 },
    );
...
test("mixed read/write storm across processes keeps all committed data readable", {
  timeout: 240_000,
}, async () => {
  const dir = await tmpDir("minidb-cluster-mp-");
  try {
    const writers = Array.from({ length: 3 }, (_, p) =>
      runWorkerOk(["write", dir, "8", `storm${p}`, "100"], {
        timeoutMs: 150_000,
      }),
    );
    ...
    const racingReaders = Array.from({ length: 2 }, () =>
      runWorker(["wait-read", dir, "8", "storm0:0", "0", "90000"], {
        timeoutMs: 120_000,
      }),
    );
    ...
    for (let p = 0; p < 3; p++) {
      await runWorkerOk(["verify", dir, "8", `storm${p}`, "100"], {
        timeoutMs: 120_000,
      });
    }
  } finally {
...
async function assertSameAsFreshOpen(
  db: ClusterDb<Doc>,
  dir: string,
  hot: number,
): Promise<void> {
  ...
  try {
    assert.deepEqual(
      await db.scan(),
      ref.scan(),
      "scan equality with a fresh full open",
    );
    assert.deepEqual(
      (await db.findEq("c", "c3")).sort(sortByKey),
    ...
    assert.deepEqual(
      crange.sort(sortByKey),
      rrange.sort(sortByKey),
      "findRange equality",
    );
    assert.deepEqual(
...
test("reader WAL catch-up: incremental tail apply equals a full reopen (multi-process)", {
  timeout: 180_000,
}, async () => {
  const dir = await tmpDir("minidb-cluster-mp-");
  try {
    ...
    assert.equal(
      stats3.readerReopens,
      stats2.readerReopens,
      "no reopens without writes",
    );
    await db.close();
  } finally {
    await rmrf(dir);
...
test("reader falls back to a full reopen on compaction rotation, then resumes incrementally", {
  timeout: 180_000,
}, async () => {
  const dir = await tmpDir("minidb-cluster-mp-");
  try {
    ...
    await runWorkerOk(
      ["storm", dir, String(shards), String(hot), "rot", "1500", "60000", "1"],
      { timeoutMs: 120_000 },
    );
    await sleep(150);
    ...
    const stats1 = db.stats();
    assert.ok(
      stats1.readerReopens - stats0.readerReopens >= 1,
      "rotation forced a full reopen",
    );
    assert.equal(
    ...
    assert.equal(
      stats2.readerReopens - stats1.readerReopens,
      0,
      "no further reopen",
    );
    assert.equal(
...
test("fingerprint: a compound-index-only definition change refreshes the cached reader (multi-process)", {
  timeout: 180_000,
}, async () => {
  const dir = await tmpDir("minidb-cluster-mp-");
  try {
    ...
    const compoundNames = () =>
      pool.withReader(hot, shardDir, (db) =>
    ...
    await runWorkerOk(["compound-defs", dir, String(shards), "create"], {
      timeoutMs: 120_000,
    });
    assert.deepEqual(
      await compoundNames(),
    ...
    await runWorkerOk(["compound-defs", dir, String(shards), "drop"], {
      timeoutMs: 120_000,
    });
    assert.deepEqual(
      await compoundNames(),
      [],
      "the refreshed reader sees the drop",
    );
