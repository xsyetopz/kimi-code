...
test("two writers contend on the same shard; loser times out with LockError", async () => {
  ...
  try {
    const db1 = await ClusterDb.open({
      dir,
      shardCount: 4,
      valueCodec: "json",
    });
...
test("read-only instance coexists with a live writer and sees its commits", async () => {
  const dir = await tmpDir("minidb-cluster-");
  try {
    const writer = await ClusterDb.open({
      dir,
      shardCount: 4,
      valueCodec: "json",
    });
    ...
    const reader = await ClusterDb.open({
      dir,
      shardCount: 4,
      valueCodec: "json",
      readOnly: true,
    ...
    await assert.rejects(
      ...
      (e: unknown) =>
        ...
        String(e.errors[0]).includes("read-only"),
    );
...
test("lock lease: db.lock timestamp advances while a writer is held", async () => {
  ...
  try {
    ...
    assert.ok(
      second.ts > first.ts,
      `timestamp renewed (${first.ts} -> ${second.ts})`,
    );
    await db.close();
  } finally {
    await rmrf(dir);
...
test("closeAll() drains in-flight callbacks before closing handles — no MiniDb-is-closed leak (review #18)", async () => {
  ...
  try {
    ...
    assert.equal(
      closeReturned,
      false,
      "closeAll waits for the in-flight callback",
    );
    ...
    assert.equal(
      opSettled,
      true,
      "every callback is settled by the time closeAll returns",
    );
