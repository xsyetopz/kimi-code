...
function crashWriter(dir: string, shardCount: number): Promise<void> {
  return new Promise((resolve) => {
    ...
    child.stdout.on("data", () => {
      ...
      heartbeats++;
      if (!killTimer && heartbeats >= 3)
        killTimer = setTimeout(() => child.kill("SIGKILL"), 20);
    });
...
test("kill -9 a writer mid-flight: every shard reopens clean and data is a contiguous prefix", {
  timeout: 120_000,
}, async () => {
  const runs = 3;
  for (let r = 0; r < runs; r++) {
    ...
    try {
      ...
      assert.ok(
        last >= 2,
        `run${r}: expected several durable keys, got k0..k${last}`,
      );
...
test("a db.lock left by a SIGKILLed holder is taken over by the next process", {
  timeout: 60_000,
}, async () => {
  ...
  try {
    ...
    assert.ok(
      elapsed < 5_000,
      `takeover was prompt (${Math.round(elapsed)}ms)`,
    );
