...
test("index-consistency: indexes stay consistent with the store under random ops", {
  timeout: 60_000,
}, async () => {
  ...
  await db.createCompoundIndex("byCityAge", {
    groupBy: "city",
    orderBy: "age",
  });
  ...
  try {
    ...
    const parisOrdered = db
      .compoundRange("byCityAge", "Paris")
      .map((r) => r.key);
    const expectedParis = [...live.entries()]
      .filter(([, d]) => d.city === "Paris")
      .sort(
        (a, b) =>
          a[1].age - b[1].age || (a[0] < b[0] ? -1 : a[0] > b[0] ? 1 : 0),
      )
      .map(([k]) => k);
    assert.deepEqual(
      parisOrdered,
      expectedParis,
      "compound Paris ordered by age",
    );
