...
async function driveStormWithRotation(
  ...
  acked: Map<string, { n: number; pad: string }>;
  failures: unknown[];
}> {
...
test("stress: acknowledged writes must never be lost by a WAL rotation (memory mode)", {
  timeout: 120_000,
}, async () => {
  ...
  const db2 = await MiniDb.open<{ n: number; pad: string }>({
    dir,
    valueCodec: "json",
  });
  const lost: string[] = [];
  ...
  expect(
    lost,
    `${lost.length} acknowledged writes lost after rotation`,
  ).toEqual([]);
  await db2.close();
  await rmrf(dir);
});
...
test("stress: acknowledged writes must stay readable and exact under rotation (disk mode)", {
  timeout: 120_000,
}, async () => {
  ...
  expect(
    wrong,
    `${wrong.length} acknowledged keys return corrupted values`,
  ).toEqual([]);
  await db.close();
...
test("stress: torn-tail recovery must not desync later writes (disk mode)", {
  timeout: 60_000,
}, async () => {
  ...
  const torn = Buffer.concat([
    Buffer.from([0x4d, 0x44, 1, 0, 5, 0]),
    Buffer.alloc(97, 0xff),
  ]);
  ...
  try {
    ...
    for (let i = 0; i < 40; i++)
      expect(db.get(`post${i}`), `get(post${i})`).toEqual(post(i));
    for (let i = 0; i < 50; i++)
      expect(db.get(`seed${i}`), `get(seed${i})`).toEqual(seedVal);
    await db.compact();
    for (let i = 0; i < 40; i++)
...
test("stress: a stale lock must be taken over by exactly one process", {
  timeout: 180_000,
}, async () => {
  ...
  try {
    ...
    for (let r = 0; r < ROUNDS; r++) {
      await fs.writeFile(
        lockPath,
        JSON.stringify({ pid: DEAD_PID, ts: Date.now() }),
      );
      ...
      for (;;) {
        ...
        if (lines.length >= RACERS) {
          ...
          if (holders.length !== 1)
            violations.push(
              `round ${r}: ${holders.length} holders (${holders.join(", ")})`,
            );
          break;
        }
...
test("stress: a read-only open must never modify the database or break a live writer", {
  timeout: 60_000,
}, async () => {
  ...
  for (let i = 0; i < 20; i++)
    await writer.set(`before${i}`, { i, pad: "x".repeat(400) });
  ...
  expect(reader.stats.compactions, "read-only instance ran a compaction").toBe(
    0,
  );
  await reader.close();
  ...
  let beforeLive = 0;
  for (let i = 0; i < 10; i++)
    if (check.get(`after${i}`) !== undefined) afterLive++;
  for (let i = 0; i < 20; i++)
    if (check.get(`before${i}`) !== undefined) beforeLive++;
  expect(beforeLive).toBe(20);
  expect(
    afterLive,
    "writes acknowledged by the live writer after a read-only open",
  ).toBe(10);
  await check.close();
  await rmrf(dir);
});
...
test("stress: a read-only open of an empty dir creates no files", {
  timeout: 30_000,
}, async () => {
...
test("stress: a single oversized token must not poison the text index and compaction", {
  timeout: 60_000,
}, async () => {
  ...
  await expect(
    db.compact(),
    "compaction failed because of a document",
...
test("stress: dt-ordered fast path must agree with the general path on offset/count", {
  timeout: 30_000,
}, async () => {
  const dir = await tmpDir("minidb-stress-dtfast-");
  const db = await MiniDb.open({ dir, valueCodec: "json" });
  for (let i = 0; i < 10; i++)
    await db.set(`d${i}`, { v: i }, { dt: { t: i * 10 } });
  const general = db
    .query({ dt: { t: { offset: 3, count: 4 } } })
    .map((r) => r.key);
  const fast = db
    .query({ dt: { t: { offset: 3, count: 4 } }, limit: 10 })
    .map((r) => r.key);
...
test("stress: RESP server must serialize replies per connection", {
  timeout: 60_000,
}, async () => {
  ...
  try {
    for (let r = 0; r < ROUNDS; r++) {
      ...
      expect(reply, `round ${r}: replies out of order or incomplete`).toBe(
        WANT,
      );
    }
  } finally {
...
// ===========================================================================
test("stress: torn-tail recovery in memory mode stays correct across compaction", {
  timeout: 60_000,
}, async () => {
  ...
  try {
    ...
    for (const [key, value] of want)
      expect(db2.get(key), `recovered ${key}`).toEqual(value);
  } finally {
    await db2.close();
    await rmrf(dir);
  }
...
// ===========================================================================
test("stress: mass TTL expiry across compaction and reopen", {
  timeout: 90_000,
}, async () => {
  ...
  try {
    ...
    for (let i = 0; i < 120; i++)
      await db.set(`churn${i}`, { i, pad: "q".repeat(900) });
  ...
  try {
    ...
    for (let i = 0; i < EPHEM; i++)
      expect(db2.has(`temp${i}`), `temp${i} resurrected`).toBe(false);
...
// ===========================================================================
test("stress: full-text index stays consistent under churn, compaction, reopen", {
  timeout: 120_000,
}, async () => {
      ...
      if (iter % 250 === 249) {
        for (const term of ["tok5", "tok77"]) {
          const hits = db
            ...
            .map((h) => h.key)
            .sort();
          expect(hits, `iter ${iter}: search("${term}")`).toEqual(
            liveHits(term),
          );
        }
      }
...
// ===========================================================================
test("stress: evict-lru under compaction churn — no resurrection, strict budget", {
  timeout: 120_000,
}, async () => {
  ...
  const db2 = await MiniDb.open<{ i: number; pad: string }>({
    dir,
    valueCodec: "json",
  });
  try {
    const after = new Map(db2.scan().map((e) => [e.key, e.value] as const));
    expect(after.size, "reopened key set must equal pre-close key set").toBe(
      before.size,
    );
    for (const [key, value] of before) {
      expect(after.get(key), `value of ${key} changed across reopen`).toEqual(
        value,
      );
    }
  } finally {
...
// ===========================================================================
test("stress: maxMemory reject leaves the db consistent and writable", {
  timeout: 60_000,
}, async () => {
...
// ===========================================================================
test("stress: read-only openers alongside a live compacting writer", {
  timeout: 90_000,
}, async () => {
  ...
  try {
    ...
    for (let i = 0; i < n; i++)
      expect(db.get(`rk${i}`)).toEqual({ n: i, pad: "w".repeat(700) });
  } finally {
    await db.close();
    await rmrf(dir);
  }
...
// ===========================================================================
test("stress: batch durability under compaction churn", {
  timeout: 120_000,
}, async () => {
...
// ===========================================================================
test("stress: a >65535-op batch rejects cleanly and applies nothing", {
  timeout: 60_000,
}, async () => {
...
// ===========================================================================
test("stress soak: full-feature model consistency with churn, compaction, reopen", {
  timeout: 300_000,
}, async () => {
  ...
  const checkAll = async (ctx: string, dbi: MiniDb<Doc>): Promise<void> => {
    ...
    for (const e of scanned)
      expect(e.value, `${ctx}: value of ${e.key}`).toEqual(
        model.get(e.key)!.doc,
      );
    const byDt = dbi
      .dtRange("created")
  ...
  try {
    ...
    for (let iter = 0; iter < 3000; iter++) {
      ...
      if (roll < 0.5) {
        ...
        const dt =
          rng() < 0.4
            ? 1_700_000_000_000 + Math.floor(rng() * 1_000_000)
            : null;
