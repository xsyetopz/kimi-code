...
test("soak: sustained random ops do not leak memory or corrupt", {
  skip: !SOAK,
}, async () => {
  ...
  try {
    ...
    console.log(
      `  soak: ${ops} ops, heap ${mib(firstHeap)} -> ${mib(finalHeap)} MiB`,
    );
    assert.ok(
    ...
    for (const k of KEYS)
      assert.deepEqual(db.get(k), model.get(k), `soak reopen ${k}`);
  } finally {
