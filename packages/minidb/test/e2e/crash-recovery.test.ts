...
function crashWriter(
  ...
  return new Promise((resolve) => {
    const child = spawn(
      process.execPath,
      ["--import", "tsx", WRITER, dir, String(compactEvery)],
      {
...
test("crash-recovery: kill mid-write, recovery yields a contiguous correct prefix", {
  timeout: 60_000,
}, async () => {
  ...
  for (let r = 0; r < runs; r++) {
    ...
    try {
      ...
      assert.ok(
        last >= 2,
        `run${r}: expected several durable keys, got up to k${last}`,
      );
    } finally {
      await rmrf(dir);
    }
...
test("crash-recovery: kill during compaction, still consistent", {
  timeout: 60_000,
}, async () => {
  const runs = 3;
  for (let r = 0; r < runs; r++) {
    ...
    try {
      ...
      assert.ok(
        last >= 2,
        `compact-run${r}: expected several durable keys, got up to k${last}`,
      );
    } finally {
      await rmrf(dir);
    }
