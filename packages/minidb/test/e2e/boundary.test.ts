...
test("boundary: large value round-trips and recovers", {
  timeout: 30_000,
}, async () => {
  const dir = await tmpDir();
...
test("boundary: many keys survive compaction + recovery", {
  timeout: 30_000,
}, async () => {
  const dir = await tmpDir();
  const N = 20000;
...
test("boundary: overwrite same key many times keeps size 1", {
  timeout: 30_000,
}, async () => {
  const dir = await tmpDir();
  const db = await MiniDb.open({ dir, valueCodec: "string" });
