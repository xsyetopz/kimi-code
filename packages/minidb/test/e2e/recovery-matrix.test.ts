    ...
    test(`recovery-matrix: WAL corrupt at ${where}, mode=${mode}`, async () => {
      ...
      try {
        ...
        const present = new Set(
          Array.from({ length: 10 }, (_, i) => "k" + i).filter(
            (k) => db.get(k) !== undefined,
          ),
        );
        ...
        if (where === "tail") {
          ...
          for (let i = 0; i < 10; i++) {
            if (i < idx)
              assert.ok(present.has("k" + i), `k${i} should survive`);
            else
              assert.ok(!present.has("k" + i), `k${i} should be lost (strict)`);
          }
        }
        await db.close();
      } finally {
