...
function failFileWrites(
  ...
  let fired = false;
  (fs as unknown as Record<string, unknown>).open = async (
    p: fsSync.PathLike,
    flags?: string,
  ) => {
    ...
    if (!fired && pred(s)) {
      fired = true;
      (handle as unknown as Record<string, unknown>)[fail] = () =>
...
function failRenames(
  pred: (src: string, dst: string) => boolean,
  error: unknown,
): () => void {
  const original = fs.rename;
  (fs as unknown as Record<string, unknown>).rename = (
    src: string,
    dst: string,
  ) =>
...
async function seedIndexedDb(
  ...
  await db.createCompoundIndex("byKindTime", {
    groupBy: "kind",
    orderBy: "ts",
  });
...
describe("skiplist bulkLoad", () => {
  test("matches one-by-one inserts on content, rank, and range behavior", () => {
    ...
    const bulk = SkipList.bulkLoad(entries, {
      compareKey: cmpNumber,
      compareVal: cmpString,
    });
    const inc = new SkipList<number, string>({
      compareKey: cmpNumber,
      compareVal: cmpString,
    });
...
describe("gen-codec", () => {
  test("store image round-trips memory + disk refs, expiry and dt metadata", async () => {
    ...
    const records: StoreImageRecord[] = [
      {
        kstr: "a",
        ref: { kind: "memory", value: Buffer.from("v1") },
        expireAt: 0,
        dt: null,
      },
      {
    ...
    expect((out[3]!.ref as { value: Buffer }).value.toString("utf8")).toBe(
      "utf8 value ✓",
    );
    expect(Buffer.from(out[3]!.kstr, "binary").toString("utf8")).toBe(
      "é-key-键",
    );
  });
  ...
  test("a single-byte flip anywhere makes the crc check fail", async () => {
    ...
    for (const pos of [0, 5, buf.length - 5]) {
      ...
      expect(() => parseGenerationBuffer(bad, "MDGS", 4)).toThrow(
        GenerationCorruptError,
      );
    }
    // Wrong magic / unsupported version are structured errors too.
    expect(() => parseGenerationBuffer(buf, "XXXX", 1)).toThrow(
      GenerationCorruptError,
    );
    expect(() => parseGenerationBuffer(buf, "MDGS", 99)).toThrow(
      GenerationCorruptError,
    );
  });
  ...
  test("dt / secondary / compound / text-docs images round-trip", async () => {
    ...
    const dt = readDtIndexImage(
      parseGenerationBuffer(await fs.readFile(path.join(dir, "dt")), "MDGD", 1)
        .payload,
    );
    expect(dt).toEqual([
      {
    ...
    const sec = readSecondaryIndexImage(
      parseGenerationBuffer(await fs.readFile(path.join(dir, "sec")), "MDSI", 1)
        .payload,
    );
    ...
    expect(cmp[0]!.groups[1]).toEqual({
      group: 42,
      entries: [{ order: 1, pk: "z" }],
    });
    expect(cmp[0]!.groups[2]).toEqual({ group: null, entries: [] });
    expect(cmp[0]!.groups[3]).toEqual({
      group: true,
      entries: [{ order: 2, pk: "y" }],
    });
    ...
    const docs = readTextDocsImage(
      parseGenerationBuffer(
        await fs.readFile(path.join(dir, "docs")),
        "MDTC",
        1,
      ).payload,
    );
...
describe("generation build + load", () => {
  test("explicit build publishes; reopen loads the generation with zero rebuilds", async () => {
    ...
    expect(
      fsSync.existsSync(
        path.join(dir, "generations", gen!.id, "manifest.json"),
      ),
    ).toBe(true);
  ...
  test("legacy database (generations disabled) gets a background first generation on open", async () => {
    ...
    await waitFor(
      () => db.stats.generationBuilds >= 1,
      "generationBuilds stat",
    );
    await db.close();
  ...
  test("compaction publishes the generation transactionally (no sync postings tail)", async () => {
    ...
    // are reclaimed.
    expect(
      fsSync.readdirSync(dir).filter((f) => /^db\.text-.*\.postings$/.test(f)),
    ).toEqual([]);
      ...
      try {
        return (
          fsSync
            ...
            .filter((g) => !g.includes(".tmp-")).length <= 2
        );
      } catch {
        return false;
      }
    }, "retention sweep");
    const left = (await listGenerations(dir))
      .filter((g) => !g.tmp)
      .map((g) => g.id);
  ...
  test("clean text index: background build then a second build re-links (no ENOENT)", async () => {
    ...
    await db.createTextIndex("ft", { fields: ["text"] });
    for (let i = 0; i < 200; i++)
      await db.set(`k${i}`, { text: `hello world doc ${i}` });
    await db.close();
  ...
  test("clean text index loaded from a generation survives consecutive builds", async () => {
    ...
    await db.createTextIndex("ft", { fields: ["text"] });
    for (let i = 0; i < 100; i++)
      await db.set(`k${i}`, { text: `hello doc ${i}` });
  ...
  test("read-only reader keeps serving through a writer generation switch", async () => {
    ...
    let writer = await MiniDb.open<Record<string, unknown>>({
      dir,
      valueCodec: "json",
    });
...
describe("generation fault matrix", () => {
  test("unknown manifest format version falls back without deleting anything", async () => {
    ...
    for (const g of await listGenerations(dir)) {
      ...
      const manifest = JSON.parse(await fs.readFile(p, "utf8")) as {
        format: number;
      };
      manifest.format = 99;
  ...
  test("corrupt store image discards the generation, never the snapshot/WAL", async () => {
    ...
    for (const g of await listGenerations(dir)) {
      if (!g.tmp)
        expect(
          fsSync.existsSync(path.join(dir, "generations", g.id, "store")),
        ).toBe(true);
    }
    await db.close();
  });
  ...
  test("interrupted store-image write: no publish, CURRENT keeps the previous generation, next build succeeds", async () => {
    ...
    await db.close();
    const db2 = await MiniDb.open<Record<string, unknown>>({
      dir,
      valueCodec: "json",
    });
  ...
  test("crash after the dir rename but before CURRENT: old CURRENT wins, stray dir cleaned by the next publish", async () => {
    ...
    await db.rebuildGeneration();
    const first = db.getIndexGeneration()!.id;
    const before = (await listGenerations(dir))
      .filter((g) => !g.tmp)
      .map((g) => g.id);
    const restore = failRenames(
    ...
    const gens = (await listGenerations(dir))
      .filter((g) => !g.tmp)
      .map((g) => g.id);
  ...
  test("old-generation cleanup failure after publish is harmless", async () => {
    ...
    (fs as unknown as Record<string, unknown>).rm = (
      p: string,
      opts?: unknown,
    ) =>
  ...
  test("WAL rollback during a build aborts it (expected churn, not an error)", async () => {
    ...
    (db.wal as unknown as { appendLoc: unknown }).appendLoc = () => ({
      ...
      batchId: -1,
        ...
        Object.assign(new Error("injected WAL failure"), {
          code: "WAL_POISONED",
        }),
      ),
    });
  ...
  test("backup/restore carries the generation tree and safely falls back on inode change", async () => {
    ...
    // data.
    const restored = await MiniDb.restore<Record<string, unknown>>(
      backupDir,
      destDir,
      { valueCodec: "json" },
    );
  ...
  test("seal flush: a WAL write failing after being imaged aborts the build instead of publishing a resurrectable write", async () => {
    ...
    (fh as { writev: unknown }).writev = async (
      bufs: readonly Uint8Array[],
    ) => {
...
describe("stage 6: maintenance shutdown semantics", () => {
  ...
  test("close() safely cancels a queued maintenance task (backpressure + drain)", async () => {
    ...
    expect(
      ["completed"].includes(outcome as string) || outcome instanceof Error,
    ).toBe(true);
    const db2 = await MiniDb.open<Record<string, unknown>>({
      dir,
      valueCodec: "json",
...
describe("generation availability triggers (wal-growth + close publish)", () => {
  ...
  test("wal-growth trigger publishes a generation without any compaction", async () => {
    ...
    await waitFor(
      () => db.getIndexGeneration() !== null,
      "wal-growth generation build",
    );
  ...
  test("a failed build backs the wal-growth trigger off instead of re-kicking every write", async () => {
    ...
    await growWalPast(db);
    await waitFor(
      () => db.stats.generationBuildErrors >= 1,
      "first kicked build to fail",
    );
    (fs as unknown as Record<string, unknown>).rename = original;
    expect(db.getIndexGeneration()).toBeNull();
