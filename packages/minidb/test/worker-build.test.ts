...
describe("worker orchestration (thread hosting)", () => {
  test("typed runtime configuration validates, configures once, and resets", () => {
    expect(() =>
      configureTextBuildWorkerRuntime("relative-worker.mjs"),
    ...
    expect(configureTextBuildWorkerRuntime(source)).toEqual(
      getTextBuildWorkerRuntimeState(),
    );
    expect(() =>
      configureTextBuildWorkerRuntime(
        new URL("../src/worker/text-build-core.ts", import.meta.url),
      ),
  ...
  test("worker thread and inline fallback produce identical artifacts", async () => {
    ...
    const [ra, rb] = await Promise.all([
      workerHandle.promise,
      buildTextArtifacts(specB),
    ]);
    ...
    const handle = startWorkerTextBuild(await specForLiveDb(db, dir, tmp), {
      workerFactory: () => {
        const worker = new Worker(
          new URL("./missing-worker.mjs", import.meta.url),
        );
        worker.once("exit", () => {
...
describe("build core (segmented external merge)", () => {
  test("tiny memory budget forces segments; merged output equals the unsegmented build", {
    timeout: 120_000,
  }, async () => {
    ...
    const big = await buildTextArtifacts(
      await specForLiveDb(db, dir, tmpBig, {
    ...
    for (let i = 0; i < big.indexes.length; i++) {
      expect(small.indexes[i]!.postingsInfo).toEqual(
        big.indexes[i]!.postingsInfo,
      );
      expect(small.indexes[i]!.dictionaryInfo).toEqual(
        big.indexes[i]!.dictionaryInfo,
      );
      expect(small.indexes[i]!.baseDocsInfo).toEqual(
        big.indexes[i]!.baseDocsInfo,
      );
    }
    // Segment files were cleaned up after the merge.
    expect(
      (await fs.readdir(tmpSmall)).filter((f) => f.includes(".seg-")),
    ).toEqual([]);
    await db.close();
  });
  ...
  test("expired documents are dropped exactly like recovery drops them", async () => {
    ...
    await db.createTextIndex("ft", { fields: ["text"] });
    for (let i = 0; i < 100; i++)
      await db.set(`live${i}`, { text: `live doc ${i}` });
    for (let i = 0; i < 50; i++)
      await db.set(`dead${i}`, { text: `dead doc ${i}` }, { ttl: 1 });
...
describe("MiniDb worker build integration", () => {
  test("worker build publishes; get/set/search stay available throughout", async () => {
    ...
    expect(
      db
        .maintenanceStatus()
        .some((t) => t.kind === "generation-build" && t.state === "complete"),
    ).toBe(true);
    await db.close();
    ...
    await waitFor(
      () =>
        db
          .maintenanceStatus()
          .some(
            (task) => task.kind === "text-build" && task.state === "running",
          ),
  ...
  test("fallback reopen defers the text base build (writer): building → committed", async () => {
    ...
    expect(db.get("k42")).toMatchObject({ score: 42 });
    expect(() => db.search("ft", "hello")).toThrowError(/still building/);
    await expect(db.searchBoundedAsync("ft", "hello")).rejects.toThrowError(
      /still building/,
    );
    ...
    expect(
      db
        .maintenanceStatus()
        .some((t) => t.kind === "text-build" && t.state === "complete"),
    ).toBe(true);
    await db.close();
  }, 60000);
  ...
  test("fallback reopen defers the text base build (read-only): scratch dir outside the db dir", async () => {
    ...
    await writer.set("mid", {
      kind: "t1",
      score: 1,
      text: "mid build write 中途",
    });
    const walEnd = (await fs.stat(path.join(dir, "db.wal"))).size;
    expect(
      await reader.catchUpFromWal(reader.recoveryInfo!.walScanEnd),
    ).not.toBeNull();
    ...
    const postings = await fs.readdir(
      path.join(scratchRoot, scratchEntries[0]!),
    );
    expect(
      postings.some(
        (f) => f.startsWith("db.text-ft") && f.endsWith(".postings"),
      ),
    ).toBe(true);
    // close() drops the scratch dir (handles released first).
    await reader.close();
    expect(
      fsSync.existsSync(scratchRoot)
        ? (await fs.readdir(scratchRoot)).length
        : 0,
    ).toBe(0);
    await writer.close();
  }, 60000);
  ...
  test("small corpus keeps the staged inline rebuild (no deferral window)", async () => {
    ...
    expect(db.search("ft", "hello").length).toBe(50);
    expect(db.maintenanceStatus().some((t) => t.kind === "text-build")).toBe(
      false,
    );
    await db.close();
  }, 60000);
  ...
  test("a failed worker keeps the current generation and the live index intact", async () => {
    ...
    expect(await fs.readFile(path.join(dir, "CURRENT"), "utf8")).toBe(
      currentBefore,
    );
    expect(db.search("ft", "hello").length).toBe(50);
...
describe("bounded createTextIndex (data-first corpus)", () => {
  test("large existing corpus builds via the worker; concurrent writes replay on commit", async () => {
    ...
    while (!settled && ops < 200) {
      await db.set(`c${ops}`, {
        kind: "concurrent",
        text: `concurrent write ${ops} 并发 hello`,
      });
      ops++;
    }
    await createP;
...
describe("single-file deployment (worker entry absent)", () => {
  test("bounded builds still run — hosted inline, never the unbounded staged path", async () => {
    vi.doMock("../src/worker/text-build.js", async (importOriginal) => {
      const orig =
        await importOriginal<typeof import("../src/worker/text-build.js")>();
    ...
    try {
      ...
      const db = await SeaMiniDb.open<Record<string, unknown>>({
        dir,
        valueCodec: "json",
      });
      await seedDataOnly(
        db as unknown as MiniDb<Record<string, unknown>>,
        5000,
      );
