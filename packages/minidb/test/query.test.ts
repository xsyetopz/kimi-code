...
test("key range + prefix scan", async () => {
  ...
  try {
    ...
    for (const k of ["user:3", "user:1", "user:2", "order:1"])
      await db.set(k, k);
    assert.deepEqual(
...
test("dt-ordered limit fast path matches a reference (no ties)", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      db
        .query({ dt: { ts: { gte, lte } }, filter: { role: "user" }, limit: 2 })
        .map((r) => r.key),
...
test("dt-ordered limit fast path orders equal ts by key (tie-break)", async () => {
  ...
  try {
    ...
    for (const k of ["t3", "t1", "t4", "t2"])
      await db.set(k, { v: k }, { dt: { ts: 1000 } });
    // descending -> key desc; ascending -> key asc
    assert.deepEqual(
      db
        .query({
          dt: { ts: { gte: 1000, lte: 1000 } },
          sort: { ts: -1 },
          limit: 4,
        })
        .map((r) => r.key),
...
test("query uses value indexes for equality/range filters", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      db
        .query({ filter: { $and: [{ city: "Paris" }, { age: { $gte: 18 } }] } })
        .map((r) => r.key),
      ["a"],
    );
