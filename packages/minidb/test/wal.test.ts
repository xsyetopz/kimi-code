...
test("append then read back preserves frames and order", async () => {
  ...
  try {
    ...
    await wal.open();
    await wal.append(
      encodeFrame({ type: TYPE_SET, key: B("a"), value: B("1") }),
    );
    await wal.append(
      encodeFrame({ type: TYPE_SET, key: B("b"), value: B("2") }),
    );
...
test("group commit: many concurrent appends all land in order", async () => {
  ...
  try {
    ...
    for (let i = 0; i < N; i++) {
      ops.push(
        wal.append(
          encodeFrame({ type: TYPE_SET, key: B(`k${i}`), value: B(`${i}`) }),
        ),
      );
    }
...
test("fsyncPolicy 'always' works", async () => {
  ...
  try {
    ...
    await wal.open();
    await wal.append(
      encodeFrame({ type: TYPE_SET, key: B("durable"), value: B("yes") }),
    );
    await wal.close();
    const frames = parseAll(await fs.readFile(file));
...
test("recovery truncates a torn/corrupt tail at the error offset", async () => {
  ...
  try {
    ...
    for (let i = 0; i < N; i++) {
      await wal.append(
        encodeFrame({ type: TYPE_SET, key: B(`key${i}`), value: B("v") }),
      );
    }
    await wal.close();
    ...
    assert.ok(
      err instanceof CorruptFrameError,
      "expected a corrupt-frame error",
    );
    assert.equal(
      err.offset,
      validSize,
      "error offset should equal end of valid data",
    );
    assert.equal(
      frames.length,
      N,
      "all valid frames before the tail are recovered",
    );
...
test("everysec: idle WAL performs zero background fsyncs; only dirty intervals sync", async () => {
  ...
  try {
    ...
    await wal.append(
      encodeFrame({ type: TYPE_SET, key: B("k"), value: B("v") }),
    );
    await tick(wal);
...
test("background sync failure is recorded but neither rejects writes nor clears dirty", async () => {
  ...
  try {
    ...
    await tick(wal);
    assert.equal(
      stats.walFsyncErrors,
      1,
      "the failing tick records exactly one error",
    );
    ...
    assert.equal(
      stats.lastWalFsyncError,
      boom,
      "sticky error is not cleared by a later success",
    );
    assert.equal(
      wal.poison,
      null,
      "a background sync failure must never poison the WAL",
    );
...
test("close() waits for an in-flight background sync before closing the fd (review #13)", async () => {
  ...
  try {
    ...
    assert.equal(
      closed,
      false,
      "close() must wait for the in-flight background sync",
    );
    ...
    assert.equal(
      stats.walFsyncs,
      2,
      "the background sync plus close() final sync",
    );
  } finally {
    await fs.rm(dir, { recursive: true, force: true });
  }
...
test("queue depth and group-commit counters track the append buffer", async () => {
  ...
  try {
    ...
    for (let i = 0; i < 5; i++) {
      await wal.append(
        encodeFrame({ type: TYPE_SET, key: B(`s${i}`), value: B("v") }),
      );
    }
    ...
    for (let i = 0; i < N; i++) {
      ops.push(
        wal.append(
          encodeFrame({ type: TYPE_SET, key: B(`c${i}`), value: B("v") }),
        ),
      );
    }
    await Promise.all(ops);
    ...
    assert.ok(
      stats.walMaxQueuedBytes > 0,
      "high-water mark recorded the burst",
    );
...
test("appendLoc stamps frames with the id of the flush batch carrying them", async () => {
  ...
  try {
    const wal = new WAL(file, { fsyncPolicy: "no" });
    await wal.open();
    const a = wal.appendLoc(
      encodeFrame({ type: TYPE_SET, key: B("a"), value: B("1") }),
    );
    const b = wal.appendLoc(
      encodeFrame({ type: TYPE_SET, key: B("b"), value: B("2") }),
    );
    assert.equal(
      a.batchId,
      b.batchId,
      "same-tick appends share the next batch id",
    );
    assert.ok(a.batchId > 0);
    await Promise.all([a.done, b.done]);
    const c = wal.appendLoc(
      encodeFrame({ type: TYPE_SET, key: B("c"), value: B("3") }),
    );
...
test("a failed flush poisons the WAL: WAL_POISONED rejections until truncate + refreshSize + clearPoison", async () => {
  ...
  try {
    ...
    await wal.open();
    await wal.append(
      encodeFrame({ type: TYPE_SET, key: B("ok"), value: B("1") }),
    );

    const fh = (
      wal as unknown as {
        fh: { writev: (...a: unknown[]) => Promise<unknown> };
      }
    ).fh;
    ...
    const batchFrame = wal.append(
      encodeFrame({ type: TYPE_SET, key: B("bad"), value: B("x") }),
    );
    ...
    await assert.rejects(
      wal.append(
        encodeFrame({ type: TYPE_SET, key: B("later"), value: B("z") }),
      ),
      (err) => {
...
test("an fsync failure with fsyncPolicy 'always' poisons the WAL and counts as walFsyncErrors", async () => {
  ...
  try {
    ...
    assert.equal(
      stats.walFsyncErrors,
      1,
      "fsync-class failure counted by sync() itself",
    );
    assert.equal(stats.walWriteErrors, 0, "not a writev-class failure");
    assert.ok(
      wal.poison,
      "an unacknowledged batch poisons even though its bytes landed",
    );

    // A poisoned close() does not throw and skips the final flush/fsync.
    await wal.close();
    assert.equal(
      (wal as unknown as { fh: unknown }).fh,
      null,
      "the handle is still released",
    );
  } finally {
    await fs.rm(dir, { recursive: true, force: true });
  }
