...
test("$in / $nin membership", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      db
        .query({ filter: { city: { $in: "Paris" as unknown as string[] } } })
        .map((r) => r.key),
      [],
    );
    assert.deepEqual(
      db
        .query({ filter: { city: { $nin: "Paris" as unknown as string[] } } })
        .map((r) => r.key),
      [],
    );
    await db.close();
...
test("$regex with string, [pattern, flags], RegExp and stateful global RegExp", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      db
        .query({
          filter: { name: { $regex: ["^a", "i"] as unknown as string } },
        })
        .map((r) => r.key),
      ["a"],
    );
...
test("logical operators $and $or $nor $not", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      db
        .query({ filter: { $nor: [{ city: "Paris" }, { city: "London" }] } })
        .map((r) => r.key),
      ["d"],
    );
    assert.deepEqual(
...
test("unknown operator and non-array logical args never match", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      db
        .query({ filter: { $and: { city: "Paris" } as never } })
        .map((r) => r.key),
      [],
    );
    assert.deepEqual(
      db
        .query({ filter: { $or: { city: "Paris" } as never } })
        .map((r) => r.key),
      [],
    );
    assert.deepEqual(
      db
        .query({ filter: { $nor: { city: "Paris" } as never } })
        .map((r) => r.key),
      [],
    );
    await db.close();
...
test("nested dot + bracket paths in filter and projection", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      db.query({ filter: { "user.addr.zip": "x" } }).map((r) => r.key).length,
      0,
    );
