...
test("auto-compaction triggers when the WAL crosses the threshold", async () => {
  ...
  try {
    ...
    for (let i = 0; i < 200; i++)
      await db.set(`k${i}`, `v${i}`.padEnd(50, "x"));
...
test("open-time compaction runs in the background — open() returns with the full dataset", async () => {
  ...
  try {
    ...
    assert.equal(db.size, 200);
    for (let i = 0; i < 200; i++)
      assert.equal(db.get(`k${i}`), `v${i}`.padEnd(50, "x"));
    ...
    await waitFor(
      () => db.stats.compactions >= 1,
      "open-time background compaction",
    );
    if (db.compacting) await db._compactDone;
    assert.ok(
      (await fs.stat(path.join(dir, "db.wal"))).size < 1024,
      "WAL shrank",
    );
    assert.equal(db.size, 200);
    await db.close();
...
test("concurrent SET/UPDATE/DEL during compaction survive recovery", {
  timeout: 30_000,
}, async () => {
  ...
  try {
    ...
    for (let i = 0; i < N; i++) {
      if (i % 2 === 0)
        assert.equal(db.get("k" + i), "updated-" + i, `updated k${i}`);
      else if (i % 4 === 1)
        assert.equal(db.get("k" + i), undefined, `deleted k${i}`);
      else assert.equal(db.get("k" + i), "v" + i, `unchanged k${i}`);
    }
    for (let i = 0; i < M; i++)
      assert.equal(db.get("new" + i), "n" + i, `new${i}`);
...
test("compaction with no concurrent writes produces an empty WAL tail", async () => {
  ...
  try {
    ...
    assert.equal(
      walSize,
      0,
      "WAL tail is empty when no writes raced compaction",
    );
    await db.close();
...
test("disk-mode compaction with continuous writes stays consistent for sync and async readers", async () => {
  ...
  try {
    ...
    for (let i = 300; i < 380; i++)
      await db.set(`k${i}`, { n: i, text: `tail ${i}` });
    await compactP;
