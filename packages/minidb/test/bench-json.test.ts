...
test("bench --quick emits a JSON report with a stable schema", async () => {
  ...
  try {
    ...
    assert.equal(
      idle.extra.walFsyncs,
      0,
      "idle everysec db: zero background fsyncs",
    );
    ...
    assert.ok(
      dirty.extra.walFsyncs >= 1,
      "a write triggers a background fsync",
    );
