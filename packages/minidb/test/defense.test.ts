...
test("encodeFrame rejects an oversized key", () => {
  ...
  assert.throws(
    () => encodeFrame({ type: TYPE_SET, key: big, value: B("v") }),
    /key too large/,
  );
});
...
test("decodeBatchOps rejects a body shorter than the count field", () => {
  ...
  assert.throws(
    () => decodeBatchOps(Buffer.alloc(0)),
    /batch body truncated: op count/,
  );
  assert.throws(
    () => decodeBatchOps(Buffer.alloc(1)),
    /batch body truncated: op count/,
  );
});
...
test("WAL constructor rejects an unknown fsyncPolicy", () => {
  assert.throws(
    () => new WAL("/tmp/x", { fsyncPolicy: "bogus" as never }),
    /unknown fsyncPolicy/,
  );
});
...
test("WAL append rejects a non-buffer frame", async () => {
  ...
  try {
    await assert.rejects(
      () => wal.append("x" as unknown as Buffer),
