...
test("compound index orders sessions within a workspace by updatedAt", async () => {
  ...
  await db.createCompoundIndex("byWsUpdated", {
    groupBy: "workspaceId",
    orderBy: "updatedAt",
  });
  try {
    await db.set(
      "a",
      { workspaceId: "W1", title: "a" },
      { dt: { updatedAt: 300 } },
    );
    await db.set(
      "b",
      { workspaceId: "W1", title: "b" },
      { dt: { updatedAt: 100 } },
    );
    await db.set(
      "c",
      { workspaceId: "W1", title: "c" },
      { dt: { updatedAt: 200 } },
    );
    await db.set(
      "d",
      { workspaceId: "W2", title: "d" },
      { dt: { updatedAt: 500 } },
    );
    ...
    const desc = db.compoundRange("byWsUpdated", "W1", {
      reverse: true,
      count: 10,
    });
    assert.deepEqual(
    ...
    // pagination
    const page = db.compoundRange("byWsUpdated", "W1", {
      reverse: true,
      offset: 1,
      count: 1,
    });
...
test("multiple dt columns each get their own compound index", async () => {
  ...
  await db.createCompoundIndex("byWsCreated", {
    groupBy: "workspaceId",
    orderBy: "createdAt",
  });
  try {
    await db.set(
      "a",
      { workspaceId: "W1" },
      { dt: { updatedAt: 300, createdAt: 10 } },
    );
    await db.set(
      "b",
      { workspaceId: "W1" },
      { dt: { updatedAt: 100, createdAt: 30 } },
    );
    await db.set(
      "c",
      { workspaceId: "W1" },
      { dt: { updatedAt: 200, createdAt: 20 } },
    );
...
test("remove() reaps emptied groups: the groups map stays bounded after high-cardinality churn (review #25)", async () => {
  ...
  const entry = (
    db.compound as unknown as {
      indexes: Map<
        string,
        { groups: Map<unknown, unknown>; byPk: Map<string, unknown> }
      >;
    }
  ...
  try {
    ...
    assert.equal(
      entry.groups.size,
      0,
      "every emptied group is reaped, del and overwrite alike",
    );
...
async function compoundSidecarNames(dir: string): Promise<string[]> {
  try {
    return (
      JSON.parse(
        await fs.readFile(path.join(dir, "db.compound-indexes.json"), "utf8"),
      ) as { name: string }[]
    )
      .map((d) => d.name)
      .sort();
...
test("createCompoundIndex persist failure: no phantom in memory or sidecar; retry succeeds", async () => {
  ...
  try {
    ...
    assert.throws(
      () => db.compoundRange("byWsUpdated", "W1"),
      /no such compound index/,
    );
...
test("dropCompoundIndex persist failure: the live index stays usable and the sidecar is unchanged", async () => {
  ...
  try {
    ...
    const before = await fs.readFile(
      path.join(dir, "db.compound-indexes.json"),
      "utf8",
    );
    const boom = new Error("injected sidecar write failure");
    const restore = stubCompoundPersist(db, async () => {
    ...
    await assert.rejects(
      db.dropCompoundIndex("byWsUpdated"),
      (e) => e === boom,
    );
    restore();
    assert.deepEqual(
