...
test("walFsyncs counts one fsync per flush under fsyncPolicy 'always'", async () => {
  ...
  try {
    ...
    assert.ok(
      db.stats.walFsyncs >= N,
      `expected >= ${N} fsyncs, got ${db.stats.walFsyncs}`,
    );
  } finally {
    await db.close();
    await fs.rm(dir, { recursive: true, force: true });
...
test("everysec: idle db performs zero background fsyncs; a dirty window syncs once then goes quiet", async () => {
  ...
  const tick = (): Promise<void> | null =>
    (
      db.wal as unknown as { backgroundTick(): Promise<void> | null }
    ).backgroundTick();
  try {
    ...
    assert.equal(
      db.stats.walFsyncs,
      0,
      "idle everysec db must not fsync in the background",
    );
    ...
    assert.equal(
      db.stats.walFsyncs,
      1,
      "no fsync growth after the writes stopped",
    );
    ...
    assert.equal(
      db.stats.walFsyncs,
      2,
      "the next dirty window fsyncs once more",
    );
  } finally {
...
test("everysec background sync failure is observable in stats but does not change write semantics", async () => {
  ...
  try {
    const fh = (db as unknown as { wal: { fh: { sync: () => Promise<void> } } })
      .wal.fh;
    ...
    assert.equal(
      ...
      boom,
      "the failure is observable via stats",
    );
    ...
    assert.equal(
      db.stats.walFsyncs,
      1,
      "background sync recovers once the failure clears",
    );
    assert.equal(
      db.stats.lastWalFsyncError,
      boom,
      "sticky error survives later successes",
    );
  } finally {
    await db.close();
    await fs.rm(dir, { recursive: true, force: true });
...
test("recovery stats capture scanned bytes, frames and duration at open", async () => {
  ...
  try {
    assert.equal(
      reopened.stats.recoveryFrames,
      N,
      "one frame per set replayed",
    );
    assert.equal(
      reopened.stats.recoveryBytes,
      walBytes,
      "WAL bytes accounted (no snapshot yet)",
    );
...
test("compaction phase stats break down into snapshot/rotation/postings/total", async () => {
  ...
  try {
    ...
    assert.ok(
      db.stats.compactionSnapshotDurationMs > 0,
      "snapshot phase recorded",
    );
    assert.ok(
      db.stats.compactionRotationDurationMs > 0,
      "rotation phase recorded",
    );
    assert.ok(
      db.stats.compactionPostingsDurationMs > 0,
      "postings rebuild recorded",
    );
    assert.ok(
      db.stats.textRebuildDurationMs > 0,
      "postings rebuild also counts as a text rebuild",
    );
    assert.ok(
...
test("query stats count candidates, decodes and sorted rows", async () => {
  ...
  try {
    for (let i = 0; i < 100; i++)
      await db.set(`k${i}`, { n: i, grp: i % 10 === 0 ? "x" : "y" });
    ...
    assert.equal(
      db.stats.queryCandidates - c0,
      100,
      "every record was a candidate",
    );
    assert.equal(
    ...
    assert.equal(
      db.stats.querySortedRows - s0,
      10,
      "only matched rows are sorted",
    );
    ...
    assert.ok(
      db.stats.queryCandidates - c1 < 100,
      "early exit stops before the full scan",
    );
  } finally {
    await db.close();
    await fs.rm(dir, { recursive: true, force: true });
...
test("index rebuild stats: values decoded once per record, 0 without value-derived indexes", async () => {
  ...
  for (let i = 0; i < 20; i++)
    await db.set(`k${i}`, { n: i }, { dt: { created: i } });
  await db.close();
  ...
  try {
    assert.equal(
      ...
      0,
      "no decodes without value-derived indexes",
    );
    assert.equal(
