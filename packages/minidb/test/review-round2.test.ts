...
test("compound index: re-set with same group+order does not duplicate", async () => {
  ...
  await db.createCompoundIndex("byWs", {
    groupBy: "workspaceId",
    orderBy: "updatedAt",
  });
  try {
...
test("WAL and snapshot tolerate short writes (fragmented writev)", async () => {
  ...
  const proto = Object.getPrototypeOf(probe) as {
    writev: (...a: unknown[]) => Promise<unknown>;
  };
  await probe.close();
  ...
  try {
    ...
    for (let i = 0; i < 30; i++)
      await db.set(`k${i}`, `value-${i}-${"x".repeat(40)}`);
    await db.compact();
    ...
    for (let i = 0; i < 30; i++)
      assert.equal(db2.get(`k${i}`), `value-${i}-${"x".repeat(40)}`);
...
test("set rolls back store + indexes when the WAL write fails", async () => {
  ...
  const fh = (
    db as unknown as {
      wal: { fh: { writev: (...a: unknown[]) => Promise<unknown> } };
    }
  ).wal.fh;
  ...
  assert.deepEqual(
    db.findEq("byCity", "London"),
    [],
    "index must not contain rolled-back key",
  );
...
test("batch rolls back atomically when the WAL write fails", async () => {
  ...
  assert.deepEqual(
    db.get("a"),
    { n: 1 },
    "a must be restored to pre-batch value",
  );
  ...
  try {
    const raw = await new Promise<Buffer>((resolve, reject) => {
      ...
      sock.on("connect", () => {
        ...
        const set = `*3\r\n$3\r\nSET\r\n$1\r\nk\r\n$${v.length}\r\n`;
        sock.write(
          Buffer.concat([Buffer.from(set, "binary"), v, Buffer.from("\r\n")]),
        );
...
test("batch still rejects genuine unique violations", async () => {
  ...
  await assert.rejects(
    db.batch([
      { op: "set", key: "u2", value: { email: "duplicate@example.test" } },
    ]),
    /unique/i,
  );
...
test("createIndex(unique) rejects existing duplicate data", async () => {
  ...
  await assert.rejects(
    db.createIndex("byMail", { field: "email", unique: true }),
    /unique/i,
  );
  assert.equal(
    db.listIndexes().length,
    0,
    "index must not persist after failed create",
  );
  await db.close();
  await fs.rm(dir, { recursive: true, force: true });
});
...
test("batch rejects keys longer than 128 chars", async () => {
  ...
  await assert.rejects(
    db.batch([{ op: "set", key: longKey, value: { v: 1 } }]),
    /key too long/i,
  );
...
test("repeated TTL updates on one key do not bloat the heap", {
  timeout: 30_000,
}, async () => {
  const dir = await tmpDir();
  const db = await MiniDb.open({
  ...
  const heapSize = (db as unknown as { store: { heap: { size: number } } })
    .store.heap.size;
...
test("open failure on corrupt index JSON releases the lock", async () => {
  ...
  await assert.rejects(
    MiniDb.open({ dir, valueCodec: "json" }),
    /JSON|Unexpected/i,
  );
...
test("WAL poison: a failed writev is truncated away — the rejected key never reappears and later writes stay consistent (disk mode)", async () => {
  ...
  assert.equal(
    db.get("failed"),
    undefined,
    "rejected write must not reappear after reopen",
  );
  assert.equal(db.get("ok"), "persisted");
  await db.close();
  await fs.rm(dir, { recursive: true, force: true });
...
test("WAL poison: a half-written group is fully revoked — in-memory and reopen agree that both keys are absent", async () => {
  ...
  assert.equal(
    db.get("first"),
    undefined,
    "in-memory state must match what reopen replays",
  );
...
test("WAL poison: an fsync failure (fsyncPolicy 'always') revokes the rejected write — no reappears after reopen", async () => {
  ...
  assert.equal(
    db.stats.walFsyncErrors,
    1,
    "fsync-class failure counted separately",
  );
...
test("WAL poison: same-key ops failing in one group restore the pre-group value; a cross-group failure keeps the committed value", async () => {
  ...
  assert.equal(
    db.get("k"),
    "A",
    "a failed later group must not roll back a committed value",
  );
  await db.close();
  db = await MiniDb.open<string>({ dir: dir2, ...MEM_OPTS });
...
test("WAL poison: an applyOp contract violation poisons the WAL and rolls the group back — no half-commit anywhere", async () => {
  ...
  const ti = (
    db as unknown as {
      text: Map<
        string,
        { addPrepared: (k: string, t: readonly string[]) => void }
      >;
    }
  ).text.get("ft")!;
  ...
  assert.equal(
    db.get("doc"),
    undefined,
    "the half-applied write never reached disk",
  );
  assert.equal(db.get("after")?.t, "fine");
  await db.close();
  await fs.rm(dir, { recursive: true, force: true });
...
test("WAL poison: when the recovery truncate fails the instance is write-disabled but stays readable and closable", async () => {
  ...
  await assert.rejects(
    db.batch([{ op: "set", key: "b2", value: "x" }]),
    /writes are disabled/,
  );
...
test("everysec background sync failure neither poisons the WAL nor rejects writes (stage-1 semantics regression)", async () => {
  ...
  try {
    ...
    await db.set("k", "v");
    await waitFor(
      () => db.stats.walFsyncErrors >= 1,
      "walFsyncErrors to be counted",
    );
    assert.equal(
      db.stats.lastWalFsyncError,
      boom,
      "sticky error is observable",
    );
    assert.equal(
      db.wal.poison,
      null,
      "a background sync failure must not poison the WAL",
    );
    await db.set("k2", "v2");
...
test("WAL poison: an applyOp violation queued behind an in-flight batch still lets that batch commit", async () => {
  ...
  await waitFor(
    () => db.wal.poison === null,
    "the in-place recovery to finish",
  );
  ...
  assert.equal(
    db.get("a")?.t,
    "first",
    "the committed in-flight batch survives reopen",
  );
  assert.equal(db.get("b"), undefined);
...
test("WAL poison: in-place recovery skips the truncate when the WAL file was replaced (stale coordinates)", async () => {
  ...
  await (
    db as unknown as { recoverWalInPlace(w: unknown): Promise<void> }
  ).recoverWalInPlace(db.wal);
  assert.equal(
...
test("WAL poison: close() in the same tick as a failing write resolves cleanly and leaves a consistent file", async () => {
  ...
  assert.equal(
    db.get("bad"),
    undefined,
    "the revoked write did not survive close+reopen",
  );
  await db.close();
  await fs.rm(dir, { recursive: true, force: true });
});
...
test("backup waits out an in-flight WAL recovery instead of copying the un-acked tail", async () => {
  ...
  assert.equal(
    restored.get("bad"),
    undefined,
    "the un-acked tail never reached the backup",
  );
...
test("a unique-conflicting insert is rejected before any eviction side effect (review #6)", async () => {
  ...
  try {
    ...
    await assert.rejects(
      db.set("bad", { u: "taken", pad: "z".repeat(100) }),
    ...
    assert.ok(
      db.get("victim"),
      "the failed insert must not evict the LRU victim",
    );
    assert.equal(
      db.stats.evictions,
      evictionsBefore,
      "zero side effects: no eviction ran",
    );
    ...
    assert.ok(
      db.stats.evictions > evictionsBefore,
      "a legal write still evicts under pressure",
    );
    await db.close();
  } finally {
    await fs.rm(dir, { recursive: true, force: true });
...
function rawBatchBody(
  ops: { type: number; key: string; value?: string }[],
): Buffer {
  ...
  for (const op of ops) {
    ...
    const value =
      op.value === undefined ? Buffer.alloc(0) : Buffer.from(op.value);
...
async function reopenWithAppendedBatch(
  body: Buffer,
): Promise<{ dir: string; db: MiniDb<string> }> {
  const dir = await tmpDir();
  let db = await MiniDb.open<string>({ dir, ...MEM_OPTS });
  await db.close();
  await fs.appendFile(
...
test("recovery skips a batch with an unknown sub-op type wholesale (review #9)", async () => {
  ...
  try {
    assert.equal(
      db.get("accepted"),
      undefined,
      "the whole batch is skipped, not half-applied",
    );
    assert.equal(db.get("unknown"), undefined);
    assert.equal(
      db.recoveryInfo!.corruptBatches,
      1,
      "the skipped batch is accounted",
    );
...
test("recovery skips a batch with trailing bytes wholesale (review #9)", async () => {
  ...
  try {
    assert.equal(
      db.get("accepted"),
      undefined,
      "trailing bytes invalidate the whole batch",
    );
...
test("a mid-batch applyOp violation rolls the whole batch back — memory and reopen agree (stage 7 group rollback)", async () => {
  ...
  assert.equal(
    db.get("x1"),
    undefined,
    "the op applied before the failure is rolled back too",
  );
  assert.equal(db.get("x2"), undefined);
  assert.deepEqual(
    db.search("ft", "first"),
    [],
    "no half-batch in the text index",
  );
