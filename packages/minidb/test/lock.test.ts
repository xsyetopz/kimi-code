...
test("two same-process contenders over a stale corpse: exactly one wins, zero double-wins", {
  timeout: 120_000,
}, async () => {
  const dir = await tmpDir();
  try {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      assert.equal(
        ...
        1,
        `iteration ${i}: exactly one winner`,
      );
      await a.release();
      await b.release();
...
test("a racing renew() never re-publishes a released lock (100 iterations)", {
  timeout: 60_000,
}, async () => {
  const dir = await tmpDir();
  try {
...
test("acquire() on an already-held lock is an idempotent true and does not re-mint the token", async () => {
  ...
  try {
    ...
    assert.equal(
      await lock.acquire(),
      true,
      "re-entrant acquire reports the held lock",
    );
    const after = JSON.parse(await fs.readFile(lockPath, "utf8")) as {
      token?: string;
    };
...
test("a legacy tokenless lock file is respected while alive and taken over when dead", async () => {
  ...
  try {
    ...
    assert.ok(
      typeof taken.token === "string" &&
        taken.token.startsWith(`${process.pid}:`),
