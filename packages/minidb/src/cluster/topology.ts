...
type TopologyOpts = Pick<
  ClusterOpenOptions,
  "shardCount" | "valueCodec" | "fsyncPolicy"
>;
...
export class Topology {
  ...
  static async open(dir: string, opts: TopologyOpts): Promise<Topology> {
    ...
    if (
      ...
      throw new RangeError(
        `shardCount must be a positive integer, got ${opts.shardCount}`,
      );
    }
    ...
    if (loaded.version !== META_VERSION) {
      throw new Error(
        `unsupported cluster meta version ${loaded.version} in ${metaPath}`,
      );
    }
    ...
    if (
      opts.shardCount !== undefined &&
      opts.shardCount !== loaded.shardCount
    ) {
      throw new RangeError(
    ...
    if (
      opts.valueCodec !== undefined &&
      opts.valueCodec !== loaded.valueCodec
    ) {
      throw new RangeError(
    ...
    if (
      opts.fsyncPolicy !== undefined &&
      opts.fsyncPolicy !== loaded.fsyncPolicy
    ) {
      throw new RangeError(
  ...
  allShardDirs(): string[] {
    return Array.from({ length: this.meta.shardCount }, (_, i) =>
      this.shardDir(i),
    );
  }
  ...
  async ensureShardDirs(): Promise<void> {
    await Promise.all(
      this.allShardDirs().map((d) => fs.mkdir(d, { recursive: true })),
    );
  }
}
