...
function assertRegularFile(filePath: string): void {
  ...
  if (!stat.isFile()) {
    throw new TypeError(
      "MiniDb text-build worker entry must be a regular file",
    );
  }
}

/** Configure the packaged worker entry once during process startup. */
export function configureTextBuildWorkerRuntime(
  entry: string | URL,
): TextBuildWorkerRuntimeState {
  let next: TextBuildWorkerRuntimeEntry;
  if (typeof entry === "string") {
    if (!path.isAbsolute(entry)) {
      throw new TypeError(
        "MiniDb packaged text-build worker entry must be an absolute path",
      );
    }
  ...
  if (configuredEntry !== null) {
    const current =
      configuredEntry.kind === "packaged"
        ? configuredEntry.path
        : configuredEntry.url.href;
