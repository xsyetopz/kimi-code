...
class CapturingClient implements Client {
  ...
  async requestPermission(
    ...
    throw new Error(
      "CapturingClient.requestPermission should not be called in session-control test",
    );
  }
  ...
  async writeTextFile(
    ...
    throw new Error(
      "CapturingClient.writeTextFile should not be called in session-control test",
    );
  }
  async readTextFile(_p: ReadTextFileRequest): Promise<ReadTextFileResponse> {
    throw new Error(
      "CapturingClient.readTextFile should not be called in session-control test",
    );
  }
}
...
describe("AcpServer session/unstable_setSessionModel", () => {
  ...
  it("rejects unknown sessionId with invalid_params and does not call setModel or emit notifications", async () => {
    ...
    await expect(
      client.unstable_setSessionModel({
