...
class CollectingClient implements Client {
  ...
  async requestPermission(
    ...
    throw new Error(
      "CollectingClient.requestPermission should not be called in tool-call-stream test",
    );
  }
  ...
  async writeTextFile(
    ...
    throw new Error(
      "CollectingClient.writeTextFile should not be called in tool-call-stream test",
    );
  }
  async readTextFile(_p: ReadTextFileRequest): Promise<ReadTextFileResponse> {
    throw new Error(
      "CollectingClient.readTextFile should not be called in tool-call-stream test",
    );
  }
}
...
describe("AcpServer tool-call streaming", () => {
  ...
  it("uses turn-prefixed toolCallId so identical SDK ids across turns do not collide", async () => {
    ...
    const ids = startUpdates.map(
      (n) => (n.update as { toolCallId: string }).toolCallId,
    );
