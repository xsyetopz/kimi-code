...
class StubClient implements Client {
  async requestPermission(
    ...
    throw new Error(
      "StubClient.requestPermission should not be called in ext-methods test",
    );
  }
  async sessionUpdate(_n: SessionNotification): Promise<void> {
    throw new Error(
      "StubClient.sessionUpdate should not be called in ext-methods test",
    );
  }
  async writeTextFile(
    ...
    throw new Error(
      "StubClient.writeTextFile should not be called in ext-methods test",
    );
  }
  async readTextFile(_p: ReadTextFileRequest): Promise<ReadTextFileResponse> {
    throw new Error(
      "StubClient.readTextFile should not be called in ext-methods test",
    );
  }
}
...
describe("AcpServer ext method surface", () => {
  ...
  it("over-the-wire extMethod surfaces -32601 to a remote ACP client", async () => {
    ...
    await expect(
      client.extMethod("myorg.unsupported", {}),
