...
class StubClient implements Client {
  async requestPermission(
    ...
    throw new Error(
      "StubClient.requestPermission should not be called in error-mapping test",
    );
  }
  ...
  async writeTextFile(
    ...
    throw new Error(
      "StubClient.writeTextFile should not be called in error-mapping test",
    );
  }
  async readTextFile(_p: ReadTextFileRequest): Promise<ReadTextFileResponse> {
    throw new Error(
      "StubClient.readTextFile should not be called in error-mapping test",
    );
  }
}
...
describe("AcpServer error mapping", () => {
  it("maps a turn.ended failed event with auth.login_required to authRequired (-32000)", async () => {
    ...
    new AgentSideConnection(
      (c) => new AcpServer(makeHarnessWithSession(session), c),
      agentStream,
    );
  ...
  it("maps blocked turn-end reasons to ACP stopReason refusal", () => {
    ...
    expect(
      turnEndReasonToStopReason("failed", { code: "provider.filtered" }),
    ).toBe("refusal");
