...
describe("outcomeToQuestionAnswer", () => {
  ...
  it("maps a selected q0_opt_<i> to { question: options[i].label }", () => {
    expect(
      outcomeToQuestionAnswer(sampleQuestion, selected("q0_opt_2")),
    ).toEqual({
      "Pick a flavour": "Mint chip",
    });
    expect(
      outcomeToQuestionAnswer(sampleQuestion, selected("q0_opt_0")),
    ).toEqual({
  ...
  it("maps cancelled to null", () => {
    expect(
      outcomeToQuestionAnswer(sampleQuestion, {
