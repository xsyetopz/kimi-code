...
class StubClient implements Client {
  async requestPermission(
    ...
    throw new Error(
      "StubClient.requestPermission should not be called in kaos-activation test",
    );
  }
...
describe("AcpServer FS-capability activation (boundary injection)", () => {
  it("passes an AcpKaos to createSession when the client advertises fs.readTextFile", async () => {
    ...
    expect(captured[0]?.options.persistenceKaos).not.toBe(
      captured[0]?.options.kaos,
    );
  });
