...
describe("acpBlocksToPromptParts", () => {
  ...
  it("keeps the XML wrapper for non-file:// resource_link schemes", () => {
    const out = acpBlocksToPromptParts([
      resourceLinkBlock(
        "zed:///agent/terminal-selection?lines=10",
        "Terminal (10 lines)",
      ),
    ...
    expect(out).toEqual([
      {
        type: "text",
        text: '<resource_link uri="zed:///agent/terminal-selection?lines=10" name="Terminal (10 lines)" />',
      },
      {
        type: "text",
  ...
  it("falls back to the XML wrapper for unparseable resource_link uris", () => {
    const out = acpBlocksToPromptParts([
      resourceLinkBlock("not a url", "weird"),
    ]);
    expect(out).toEqual([
  ...
  it("escapes XML-special characters in non-file:// resource_link attributes", () => {
    const out = acpBlocksToPromptParts([
      resourceLinkBlock(
        ...
        'name with "quotes" & <angle>',
      ),
    ]);
    expect(out).toEqual([
      {
        type: "text",
        text: '<resource_link uri="https://example.com/a&amp;b" name="name with &quot;quotes&quot; &amp; &lt;angle&gt;" />',
      },
    ]);
  });
...
describe("compressPromptImageParts", () => {
  async function pngBase64(width: number, height: number): Promise<string> {
    const buf = await new Jimp({ width, height, color: 0x3366ccff }).getBuffer(
      "image/png",
    );
  ...
  it("downsamples an oversized inline image part and announces the compression", async () => {
    ...
    const parts = acpBlocksToPromptParts([
      imageBlock(originalBase64, "image/png"),
    ]);
