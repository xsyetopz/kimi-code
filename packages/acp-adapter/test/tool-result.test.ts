...
class CollectingClient implements Client {
  ...
  async requestPermission(
    ...
    throw new Error(
      "CollectingClient.requestPermission should not be called in tool-result test",
    );
  }
  ...
  async writeTextFile(
    ...
    throw new Error(
      "CollectingClient.writeTextFile should not be called in tool-result test",
    );
  }
  async readTextFile(_p: ReadTextFileRequest): Promise<ReadTextFileResponse> {
    throw new Error(
      "CollectingClient.readTextFile should not be called in tool-result test",
    );
  }
}
...
describe("AcpServer tool.result → tool_call_update", () => {
  ...
  it("emits status=failed when isError is true", async () => {
    ...
    const toolUpdates = collecting.updates.filter(
      (u) =>
        (u.update as { sessionUpdate?: string }).sessionUpdate !==
        "available_commands_update",
    );
