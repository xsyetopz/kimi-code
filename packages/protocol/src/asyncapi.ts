...
function operationMessageRefs(
  ...
  return ASYNCAPI_OPERATIONS.filter(
    (operation) => operation.direction === direction,
  ).map((operation) => ({
    $ref: `#/components/messages/${messageId(operation.type)}`,
  }));
}

function ackMessageRefs(): Array<{ $ref: string }> {
  return ASYNCAPI_OPERATIONS.filter(
    (operation) => operation.ackSchema !== undefined,
  ).map((operation) => ({
    $ref: `#/components/messages/${messageId(operation.type)}_ack`,
  }));
}
