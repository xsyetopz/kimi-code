 ...
 * Run: `bun --filter @moonshot-ai/protocol exec vitest run src/__tests__/envelope.test.ts`.
 */
...
describe("envelope", () => {
  ...
  it("envelopeSchema rejects non-integer code", () => {
    ...
    expect(
      schema.safeParse({ code: 1.5, msg: "x", data: null, request_id: "r" })
        .success,
    ).toBe(false);
  });
  ...
  it("envelopeSchema rejects missing request_id", () => {
    const schema = envelopeSchema(z.unknown());
    expect(
      schema.safeParse({ code: 0, msg: "success", data: null }).success,
    ).toBe(false);
  });
  ...
  it("wire shape matches the daemon helper byte-for-byte", () => {
    ...
    expect(oursJson).toBe(
      '{"code":0,"msg":"success","data":{"id":"sess_1"},"request_id":"req_y"}',
    );

    const errJson = JSON.stringify(
      errEnvelope(40001, "validation failed", "req_z"),
    );
    expect(errJson).toBe(
...
describe("error-codes", () => {
  ...
  it("ErrorCodeReason maps every numeric code to its domain.reason label", () => {
    expect(ErrorCodeReason[ErrorCode.SESSION_NOT_FOUND]).toBe(
      "session.not_found",
    );
    expect(ErrorCodeReason[ErrorCode.PROVIDER_NOT_FOUND]).toBe(
      "provider.not_found",
    );
    expect(ErrorCodeReason[ErrorCode.MODEL_NOT_FOUND]).toBe("model.not_found");
    expect(ErrorCodeReason[ErrorCode.VALIDATION_FAILED]).toBe(
      "validation.failed",
    );
    expect(ErrorCodeReason[ErrorCode.FS_WATCH_LIMIT_EXCEEDED]).toBe(
      "fs.watch_limit_exceeded",
    );
    expect(ErrorCodeReason[ErrorCode.GOAL_UNSUPPORTED_AGENT]).toBe(
