...
describe("messageContentSchema variants", () => {
  ...
  it("rejects unknown content type", () => {
    expect(
      messageContentSchema.safeParse({ type: "audio", text: "" }).success,
    ).toBe(false);
  });
});
...
describe("messageSchema", () => {
  ...
  it("rejects empty id", () => {
    expect(messageSchema.safeParse({ ...validMessage, id: "" }).success).toBe(
      false,
    );
  });
  ...
  it("rejects bad ISO timestamp", () => {
    expect(
      messageSchema.safeParse({ ...validMessage, created_at: "yesterday" })
        .success,
    ).toBe(false);
  });
