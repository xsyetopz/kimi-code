...
describe("terminal REST schemas", () => {
  ...
  it("accepts empty create requests and optional relative cwd", () => {
    ...
    expect(
      createTerminalRequestSchema.parse({ cwd: "packages/server", cols: 100 }),
    ).toEqual({
      cwd: "packages/server",
      cols: 100,
    });
  ...
  it("rejects absolute create cwd overrides", () => {
    expect(
      createTerminalRequestSchema.safeParse({ cwd: "/tmp/outside" }).success,
    ).toBe(false);
  });

  it("wraps terminal lists under items", () => {
    expect(
      listTerminalsResponseSchema.parse({ items: [sampleTerminal] }).items,
    ).toHaveLength(1);
  });

  it("requires closed:true for close responses", () => {
    expect(closeTerminalResponseSchema.parse({ closed: true })).toEqual({
      closed: true,
    });
    expect(
      closeTerminalResponseSchema.safeParse({ closed: false }).success,
    ).toBe(false);
  });
});
