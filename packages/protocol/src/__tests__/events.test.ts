...
type _AssertToolInputDisplayNonNever = ToolInputDisplay extends never
  ? never
  : true;
...
describe("events / display re-exports", () => {
  ...
  it("preserves detached on task events", () => {
    ...
    expect((parsed as { info: { detached?: boolean } }).info.detached).toBe(
      false,
    );
  });
  ...
  it("validates event.session.status_changed events", () => {
    ...
    expect((parsed as { previous_status: string }).previous_status).toBe(
      "idle",
    );
    expect((parsed as { current_prompt_id: string }).current_prompt_id).toBe(
      "prompt_1",
    );
  });
