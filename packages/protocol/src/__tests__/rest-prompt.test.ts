...
describe("promptSubmitResultSchema", () => {
  ...
  it("rejects empty prompt_id", () => {
    expect(
      promptSubmitResultSchema.safeParse({
        prompt_id: "",
        user_message_id: "msg",
      }).success,
    ).toBe(false);
...
describe("promptSteerRequestSchema", () => {
  it("requires at least one prompt id", () => {
    expect(
      promptSteerRequestSchema.parse({ prompt_ids: ["prompt_a"] }).prompt_ids,
    ).toEqual(["prompt_a"]);
    expect(promptSteerRequestSchema.safeParse({ prompt_ids: [] }).success).toBe(
      false,
    );
  });
});
...
describe("promptAbortResponseSchema", () => {
  it("parses { aborted: true } success shape", () => {
    const parsed = promptAbortResponseSchema.parse({
      aborted: true,
      at_seq: 7,
    });
