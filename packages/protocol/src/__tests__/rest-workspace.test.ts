...
describe("workspaceIdSchema", () => {
  it("accepts a wd_<slug>_<hash12> string", () => {
    expect(workspaceIdSchema.parse("wd_kimi_0123456789ab")).toBe(
      "wd_kimi_0123456789ab",
    );
  });
  ...
  it("rejects missing wd_ prefix", () => {
    expect(workspaceIdSchema.safeParse("kimi_0123456789ab").success).toBe(
      false,
    );
  });

  it("rejects non-hex tail", () => {
    expect(workspaceIdSchema.safeParse("wd_kimi_xyzxyzxyzxyz").success).toBe(
      false,
    );
  });

  it("rejects truncated hash", () => {
    expect(workspaceIdSchema.safeParse("wd_kimi_0123456789").success).toBe(
      false,
    );
  });
});
...
describe("createWorkspaceRequestSchema (POST /api/v1/workspaces)", () => {
  it("accepts a root-only body", () => {
    expect(
      createWorkspaceRequestSchema.parse({ root: "/Users/foo/code" }),
    ).toEqual({
      root: "/Users/foo/code",
    });
  });
  ...
  it("rejects empty root", () => {
    expect(createWorkspaceRequestSchema.safeParse({ root: "" }).success).toBe(
      false,
    );
  });

  it("rejects missing root", () => {
    expect(createWorkspaceRequestSchema.safeParse({} as unknown).success).toBe(
      false,
    );
  });
...
describe("updateWorkspaceRequestSchema (PATCH /api/v1/workspaces/{id})", () => {
  ...
  it("rejects empty body (name is required for the patch)", () => {
    expect(updateWorkspaceRequestSchema.safeParse({} as unknown).success).toBe(
      false,
    );
  });
});
...
describe("workspaceIdParamSchema", () => {
  it("accepts a wd_-shaped workspace_id", () => {
    expect(
      workspaceIdParamSchema.parse({ workspace_id: "wd_kimi_0123456789ab" })
        .workspace_id,
...
describe("listWorkspacesResponseSchema", () => {
  it("accepts an empty list", () => {
    expect(listWorkspacesResponseSchema.parse({ items: [] })).toEqual({
      items: [],
    });
  });
  ...
  it("accepts a non-empty list", () => {
    expect(
      listWorkspacesResponseSchema.parse({ items: [sampleWorkspace] }).items[0]
        ?.id,
...
describe("deleteWorkspaceResponseSchema", () => {
  ...
  it("rejects {deleted:false}", () => {
    expect(
      deleteWorkspaceResponseSchema.safeParse({ deleted: false }).success,
    ).toBe(false);
  });
});
