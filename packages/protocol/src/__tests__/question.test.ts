...
describe("questionItemSchema (SCHEMAS §6.2)", () => {
  ...
  it("rejects more than 4 options", () => {
    ...
    expect(() =>
      questionItemSchema.parse({ ...baseItem, options: tooMany }),
    ).toThrow();
  });
});
...
describe("questionRequestSchema (SCHEMAS §6.2)", () => {
  ...
  it("accepts a 1-question request", () => {
    expect(questionRequestSchema.parse(baseReq).question_id).toBe(
      "01J_QUESTION",
    );
  });
  ...
  it("rejects 0 questions", () => {
    expect(() =>
      questionRequestSchema.parse({ ...baseReq, questions: [] }),
    ).toThrow();
  });
  ...
  it("rejects more than 4 questions", () => {
    ...
    expect(() =>
      questionRequestSchema.parse({ ...baseReq, questions: tooMany }),
    ).toThrow();
  });
...
describe("questionAnswerSchema 5-kind discriminated union (SCHEMAS §6.2)", () => {
  ...
  it("rejects single with empty option_id", () => {
    expect(() =>
      questionAnswerSchema.parse({ kind: "single", option_id: "" }),
    ).toThrow();
  });

  it("rejects multi with empty option_ids array", () => {
    expect(() =>
      questionAnswerSchema.parse({ kind: "multi", option_ids: [] }),
    ).toThrow();
  });

  it("rejects unknown kind", () => {
    expect(() =>
      questionAnswerSchema.parse({ kind: "rangefinder", value: 42 }),
    ).toThrow();
  });
});

describe("questionAnswerMethodSchema", () => {
  it.each(["enter", "space", "number_key", "click"] as const)(
    "accepts %s",
    (m) => {
...
describe("questionResponseSchema (SCHEMAS §6.2)", () => {
  it("accepts a single-answer response with all optional fields", () => {
    ...
    expect(parsed.answers["q_1"]).toEqual({
      kind: "single",
      option_id: "opt_1",
    });
...
describe("questionResolveResultSchema (REST §3.6)", () => {
  ...
  it("rejects resolved:false", () => {
    expect(() =>
      questionResolveResultSchema.parse({
        resolved: false,
        resolved_at: "2026-06-04T10:31:00Z",
      }),
    ).toThrow();
...
describe("questionAlreadyResolvedDataSchema (REST §3.6 idempotent 40902)", () => {
  it("accepts resolved:false", () => {
    expect(
      questionAlreadyResolvedDataSchema.parse({ resolved: false }),
    ).toEqual({
      resolved: false,
    });
  });
  ...
  it("rejects resolved:true", () => {
    expect(() =>
      questionAlreadyResolvedDataSchema.parse({ resolved: true }),
    ).toThrow();
  });
});
...
describe("listPendingQuestionsResponseSchema (REST pending recovery)", () => {
  ...
  it("accepts status=pending query", () => {
    expect(
      listPendingQuestionsQuerySchema.parse({ status: "pending" }),
    ).toEqual({
      status: "pending",
    });
  });
