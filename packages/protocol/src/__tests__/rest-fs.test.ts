...
describe("fsListManyRequestSchema", () => {
  it("requires at least one path", () => {
    expect(fsListManyRequestSchema.safeParse({ paths: [] }).success).toBe(
      false,
    );
  });
...
describe("fsStatManyRequestSchema", () => {
  ...
  it("rejects empty paths array", () => {
    expect(fsStatManyRequestSchema.safeParse({ paths: [] }).success).toBe(
      false,
    );
  });
});
...
describe("fsSearchRequestSchema (W11.1)", () => {
  ...
  it("caps limit at 200", () => {
    expect(
      fsSearchRequestSchema.safeParse({ query: "a", limit: 201 }).success,
    ).toBe(false);
    expect(
      fsSearchRequestSchema.safeParse({ query: "a", limit: 200 }).success,
    ).toBe(true);
  });
...
describe("fsSearchResponseSchema (W11.1)", () => {
  it("round-trips an empty response", () => {
    expect(
      fsSearchResponseSchema.parse({ items: [], truncated: false }),
...
describe("fsGrepRequestSchema (W11.1)", () => {
  ...
  it("rejects context_lines > 10", () => {
    expect(
      fsGrepRequestSchema.safeParse({ pattern: "a", context_lines: 11 })
        .success,
    ).toBe(false);
  });
...
describe("fsGitStatusRequestSchema (W11.2)", () => {
  ...
  it("rejects empty path strings inside paths", () => {
    expect(fsGitStatusRequestSchema.safeParse({ paths: [""] }).success).toBe(
      false,
    );
  });
});
...
describe("fsMkdirRequestSchema", () => {
  ...
  it("accepts recursive:true", () => {
    expect(
      fsMkdirRequestSchema.parse({ path: "a/b", recursive: true }),
    ).toEqual({
      path: "a/b",
      recursive: true,
    });
