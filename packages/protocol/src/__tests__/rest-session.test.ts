...
describe("exportSessionRequestSchema", () => {
  it("accepts an empty body and an optional Web log", () => {
    ...
    expect(
      exportSessionRequestSchema.parse({ web_log: '{"event":"connected"}\n' }),
    ).toEqual({
  ...
  it("accepts the desktop log flag", () => {
    expect(exportSessionRequestSchema.parse({ desktop: true })).toEqual({
      desktop: true,
    });
  });
  ...
  it("accepts a Web log at the 256 KiB UTF-8 boundary", () => {
    expect(
      exportSessionRequestSchema.safeParse({ web_log: "a".repeat(256 * 1024) })
        .success,
    ).toBe(true);
  });
  ...
  it("rejects a Web log over the 256 KiB UTF-8 boundary", () => {
    expect(
      exportSessionRequestSchema.safeParse({
        web_log: `${"a".repeat(256 * 1024)}b`,
      }).success,
    ).toBe(false);
  });
  ...
  it("measures the Web log limit in UTF-8 bytes instead of JavaScript characters", () => {
    expect(
      exportSessionRequestSchema.safeParse({ web_log: "你".repeat(87_382) })
        .success,
    ).toBe(false);
  });
  ...
  it("rejects fields that the server owns", () => {
    expect(
      exportSessionRequestSchema.safeParse({ outputPath: "/tmp/export.zip" })
        .success,
    ).toBe(false);
  });
});
...
describe("exportSessionParamsSchema", () => {
  it("requires a non-empty session_id", () => {
    ...
    expect(
      exportSessionParamsSchema.safeParse({ session_id: "" }).success,
    ).toBe(false);
  });
});
...
describe("createSessionRequestSchema", () => {
  ...
  it("rejects extra unknown agent_config keys via partial schema (zod is permissive but the partial holds known keys)", () => {
    const parsed = createSessionRequestSchema.parse({
      metadata: { cwd: "/tmp/foo" },
      agent_config: { model: "m", unknown_key: "x" } as unknown as {
        model: string;
      },
    });
    expect(parsed.agent_config?.model).toBe("m");
    expect(
      (parsed.agent_config as Record<string, unknown>)["unknown_key"],
    ).toBeUndefined();
  });
});
...
describe("listSessionsQuerySchema", () => {
  ...
  it("rejects page_size > 100", () => {
    expect(listSessionsQuerySchema.safeParse({ page_size: 101 }).success).toBe(
      false,
    );
  });

  it("accepts a busy filter", () => {
    expect(listSessionsQuerySchema.parse({ busy: true })).toEqual({
      busy: true,
    });
  });

  it("rejects a non-boolean busy value", () => {
    expect(listSessionsQuerySchema.safeParse({ busy: "frozen" }).success).toBe(
      false,
    );
  });
...
describe("listSessionChildrenQuerySchema", () => {
  it("does not advertise exclude_empty (child lists do not filter by it)", () => {
    const parsed = listSessionChildrenQuerySchema.parse({
      exclude_empty: true,
    });
...
describe("updateSessionProfileRequestSchema", () => {
  it("accepts a metadata patch (without cwd)", () => {
    expect(
      updateSessionProfileRequestSchema.parse({
...
describe("forkSessionRequestSchema", () => {
  ...
  it("rejects non-object metadata", () => {
    expect(forkSessionRequestSchema.safeParse({ metadata: "x" }).success).toBe(
      false,
    );
  });
});
...
describe("createSessionChildRequestSchema", () => {
  ...
  it("rejects non-object metadata", () => {
    expect(
      createSessionChildRequestSchema.safeParse({ metadata: "x" }).success,
    ).toBe(false);
  });
});
...
describe("compactSessionRequestSchema", () => {
  ...
  it("accepts an optional instruction string", () => {
    expect(
      compactSessionRequestSchema.parse({
  ...
  it("rejects a non-string instruction", () => {
    expect(
      compactSessionRequestSchema.safeParse({ instruction: 123 }).success,
    ).toBe(false);
  });
});
...
describe("undoSessionRequestSchema", () => {
  ...
  it("rejects zero count and oversized page size", () => {
    expect(undoSessionRequestSchema.safeParse({ count: 0 }).success).toBe(
      false,
    );
    expect(undoSessionRequestSchema.safeParse({ page_size: 101 }).success).toBe(
      false,
    );
  });
});
...
describe("archiveSessionResponseSchema", () => {
  it("accepts the canonical { archived: true } shape", () => {
    expect(archiveSessionResponseSchema.parse({ archived: true })).toEqual({
      archived: true,
    });
  });
  ...
  it("rejects { archived: false }", () => {
    expect(
      archiveSessionResponseSchema.safeParse({ archived: false }).success,
    ).toBe(false);
  });
});
...
describe("deleteSessionResponseSchema (deprecated alias)", () => {
  it("accepts the canonical { archived: true } shape", () => {
    expect(deleteSessionResponseSchema.parse({ archived: true })).toEqual({
      archived: true,
    });
  });
  ...
  it("rejects { archived: false }", () => {
    expect(
      deleteSessionResponseSchema.safeParse({ archived: false }).success,
    ).toBe(false);
  });
});
