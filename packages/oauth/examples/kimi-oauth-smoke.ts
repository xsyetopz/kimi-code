...
async function main(): Promise<void> {
  ...
  const homeDir =
    explicitHomeDir ?? (await mkdtemp(join(tmpdir(), "kimi-oauth-smoke-")));
  ...
  try {
    ...
    const accessToken = await toolkit
...
function smokeIdentityFromEnv(): KimiHostIdentity {
  ...
  if (version === undefined || version.trim().length === 0) {
    throw new Error(
      "KIMI_CODE_SMOKE_VERSION is required for Kimi OAuth smoke.",
    );
  }
  return {
...
function printUsage(
  usage: Awaited<
    ReturnType<KimiOAuthToolkit<ManagedKimiConfigShape>["getManagedUsage"]>
  >,
): void {
  if (usage.kind === "error") {
