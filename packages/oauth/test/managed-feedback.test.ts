...
describe("kimiCodeFeedbackUrl", () => {
  it("appends /feedback to the default base URL", () => {
    expect(kimiCodeFeedbackUrl()).toBe(
      "https://api.kimi.com/coding/v1/feedback",
    );
  });
...
describe("fetchSubmitFeedback", () => {
  ...
  it("preserves the kimi-code- version prefix in the request body", async () => {
    ...
    const calls = fetchMock.mock.calls as unknown as [string, RequestInit?][];
    const sent = JSON.parse(calls[0]?.[1]?.body as string) as Record<
      string,
      unknown
    >;
  ...
  it("surfaces API error messages from failed submissions", async () => {
    vi.stubGlobal(
      ...
      vi.fn(
        async () =>
          new Response(
            JSON.stringify({ error: { message: "feedback rejected" } }),
            {
              status: 400,
              headers: { "Content-Type": "application/json" },
            },
