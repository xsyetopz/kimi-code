...
describe("FileTokenStorage", () => {
  ...
  it("refuses path-traversal names on save (B1)", async () => {
    await expect(
      storage.save("../../etc/passwd", sampleToken()),
  ...
  it("refuses leading-dot names (hidden file abuse)", async () => {
    await expect(storage.save(".hidden", sampleToken())).rejects.toThrow(
      /Invalid token name/,
    );
  });

  it("refuses empty name", async () => {
    await expect(storage.save("", sampleToken())).rejects.toThrow(
      /Invalid token name/,
    );
  });
  ...
  it("save() leaves no *.tmp.* sibling once the rename completes", async () => {
    ...
    const tmps = entries.filter((name) =>
      name.startsWith("kimi-code.json.tmp."),
    );
    expect(tmps).toEqual([]);
