...
describe("parseManagedUserInfoPayload", () => {
  ...
  it("drops a phone record whose fields are all empty or non-string", () => {
    expect(
      parseManagedUserInfoPayload({
        user_id: "u_1",
        phone: { country_code: 86 },
      })?.phone,
    ).toBeUndefined();
    expect(
      parseManagedUserInfoPayload({ user_id: "u_1", phone: "86" })?.phone,
    ).toBeUndefined();
  });
  ...
  it("keeps email only when it is a non-empty string", () => {
    expect(
      parseManagedUserInfoPayload({ user_id: "u_1", email: "user@example.com" })
        ?.email,
    ).toBe("user@example.com");
    expect(
      parseManagedUserInfoPayload({ user_id: "u_1", email: "" })?.email,
    ).toBeUndefined();
    expect(
      parseManagedUserInfoPayload({ user_id: "u_1", email: 42 })?.email,
    ).toBeUndefined();
  });
});
...
describe("fetchManagedUserInfo", () => {
  it("sends only Authorization and Accept headers", async () => {
    ...
    await expect(
      fetchManagedUserInfo("https://api.example/me", "access-token"),
    ).resolves.toEqual({
      kind: "ok",
      userInfo: {
        userId: "u_123",
  ...
  it("wraps network failures with the profile prefix", async () => {
    ...
    expect(result).toEqual({
      kind: "error",
      message: "Failed to fetch profile: socket hang up",
    });
  });
});
...
describe("managedUserInfoResultSchema", () => {
  ...
  it("accepts a full camelCase domain payload", () => {
    expect(
      managedUserInfoResultSchema.parse({ kind: "ok", userInfo: fullUserInfo }),
    ).toEqual({
      kind: "ok",
      userInfo: fullUserInfo,
    });
    expect(
      managedUserInfoResultSchema.parse({
        kind: "error",
        message: "nope",
        status: 401,
      }),
