...
describe("isManagedKimiCodeBaseUrl", () => {
  it("matches the default managed endpoint, with or without a trailing slash", () => {
    expect(isManagedKimiCodeBaseUrl("https://api.kimi.com/coding/v1")).toBe(
      true,
    );
    expect(isManagedKimiCodeBaseUrl("https://api.kimi.com/coding/v1/")).toBe(
      true,
  ...
  it("matches against the KIMI_CODE_BASE_URL override", () => {
    ...
    expect(isManagedKimiCodeBaseUrl("https://gw.example.com/coding/v1")).toBe(
      true,
    );
    expect(isManagedKimiCodeBaseUrl("https://api.kimi.com/coding/v1")).toBe(
      false,
  ...
  it("rejects other paths on the managed host and other hosts entirely", () => {
    expect(isManagedKimiCodeBaseUrl("https://api.kimi.com/coding/v2")).toBe(
      false,
    );
    expect(isManagedKimiCodeBaseUrl("https://api.kimi.com/v1")).toBe(false);
    expect(
      isManagedKimiCodeBaseUrl("https://gateway.example.com/coding/v1"),
    ).toBe(false);
...
describe("parseManagedUsagePayload", () => {
  it("returns empty when payload is not an object", () => {
    expect(parseManagedUsagePayload(null)).toEqual({
      summary: null,
      limits: [],
      extraUsage: null,
    });
  ...
  it("parses the numeric strings the platform reports", () => {
    const parsed = parseManagedUsagePayload({
      usage: {
        used: "17",
        limit: "100",
        resetTime: "2030-01-01T00:00:00.000Z",
      },
    });
  ...
  it("normalizes window duration and timeUnit from the window record", () => {
    const parsed = parseManagedUsagePayload({
      limits: [
        ...
        {
          detail: { used: 3, limit: 60 },
          window: { duration: 7, timeUnit: "TIME_UNIT_DAY" },
        },
        {
          detail: { used: 4, limit: 30 },
  ...
  it("passes through `name` from the item or detail", () => {
    ...
    expect(parsed.limits.map((l) => l.name)).toEqual([
      "Daily cap",
      "Detail named",
    ]);
  });
  ...
  it("returns null extra usage when boosterWallet is missing or invalid", () => {
    expect(
      parseManagedUsagePayload({ usage: { used: 1, limit: 10 } }).extraUsage,
    ).toBeNull();
    expect(
      parseManagedUsagePayload({
        usage: { used: 1, limit: 10 },
        boosterWallet: {
          balance: { type: "OTHER", amount: "100", amountLeft: "50" },
        },
      }).extraUsage,
    ).toBeNull();
    expect(
...
describe("fetchManagedUsage", () => {
  it("sends only Authorization and Accept headers", async () => {
    ...
    await expect(
      fetchManagedUsage("https://api.example/usages", "access-token"),
    ).resolves.toEqual({
      kind: "ok",
      parsed: {
  ...
  it("surfaces JSON API error messages with status", async () => {
    ...
    const result = await fetchManagedUsage(
      "https://api.example/usages",
      "access-token",
    );
  ...
  it("surfaces nested JSON API error messages", async () => {
    vi.stubGlobal(
      ...
      vi.fn(
        async () =>
          new Response(
            JSON.stringify({ error: { message: "usage endpoint moved" } }),
            {
              status: 404,
              headers: { "Content-Type": "application/json" },
            },
