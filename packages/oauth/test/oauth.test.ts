...
function requestAuth(
  ...
  return requestDeviceAuthorization(config, {
...
function pollToken(
  ...
  return pollDeviceToken(config, deviceCode, {
    deviceHeaders: TEST_DEVICE_HEADERS,
  });
}
...
describe("requestDeviceAuthorization", () => {
  ...
  it("posts client_id as form-encoded body", async () => {
    ...
    expect(recorded.headers["content-type"]).toContain(
  ...
  it("surfaces message fields from failed device authorization responses", async () => {
    ...
    await expect(requestAuth()).rejects.toThrow(
      /device authorization disabled/,
    );
  });
...
describe("pollDeviceToken", () => {
  ...
  it("throws on 500 server error", async () => {
    ...
    await expect(pollToken(flowConfig(), "d")).rejects.toBeInstanceOf(
      OAuthError,
    );
  });
  ...
  it("surfaces nested API error messages from failed polling responses", async () => {
    server.enqueue("/api/oauth/token", {
      status: 400,
      body: {
        error: { code: "invalid_request", message: "poll rejected by server" },
      },
    });

    await expect(pollToken(flowConfig(), "d")).rejects.toThrow(
      /poll rejected by server/,
    );
  });
...
describe("refreshAccessToken", () => {
  ...
  it("does not retry after a 401 refresh response", async () => {
    ...
    await expect(
      refreshToken(flowConfig(), "old-rt", {
        maxRetries: 3,
        backoffMs: () => 0,
      }),
  ...
  it("surfaces nested API error messages from unauthorized refresh responses", async () => {
    ...
    await expect(refreshToken(flowConfig(), "old-rt")).rejects.toThrow(
