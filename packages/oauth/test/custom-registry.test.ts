...
describe("fetchCustomRegistry", () => {
  it("parses a kokub-shaped 200 response into three providers", async () => {
    const fetchMock = vi.fn(async () =>
      makeJsonResponse(makeKokubResponseBody()),
    );
  ...
  it("sends the given User-Agent, and none by default", async () => {
    ...
    const withoutUa = fetchMock.mock.calls[0] as unknown as [
      string,
      RequestInit,
    ];
    expect(
      (withoutUa[1].headers as Record<string, string>)["User-Agent"],
    ).toBeUndefined();
  });
...
describe("applyCustomRegistryProvider", () => {
  it("writes provider + model aliases for a kokub-shaped entry with default fallbacks", () => {
    ...
    expect((claude as { displayName: string }).displayName).toBe(
      "Claude Opus 4.7",
    );
  });
  ...
  it("falls back to the model id for displayName when name is absent", () => {
    ...
    expect(
      (config.models?.["demo/m-1"] as { displayName: string }).displayName,
    ).toBe("m-1");
  });
  ...
  it("clears stale aliases for the same provider before re-populating", () => {
    ...
    expect(
      config.models?.["registry_chat-completions/stale-model"],
    ).toBeUndefined();
  ...
  it("maps support_efforts / default_effort onto the model alias", () => {
    ...
    const alias = config.models?.["rich/rich-thinker"] as Record<
      string,
      unknown
    >;
  ...
  it("treats support_efforts as a thinking capability hint without reasoning: true", () => {
    ...
    const alias = config.models?.["rich/rich-effort-only"] as Record<
      string,
      unknown
    >;
...
describe("removeCustomRegistryProvider", () => {
  it("removes the provider and every alias for it, and clears matching defaultModel", () => {
    ...
    expect(
      config.models?.["registry_chat-completions/gpt-5.5"],
    ).toBeUndefined();
    expect(
      config.models?.["registry_chat-completions/claude-opus-4-7"],
    ).toBeUndefined();
    expect(config.models?.["other/keepme"]).toBeDefined();
