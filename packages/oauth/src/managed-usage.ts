...
export function kimiCodeBaseUrl(): string {
  ...
  return (
    process.env["KIMI_CODE_BASE_URL"] ?? DEFAULT_KIMI_CODE_BASE_URL
  ).replace(/\/+$/, "");
}
...
export function isManagedKimiCodeBaseUrl(baseUrl: string | undefined): boolean {
  ...
  return (
    managed !== undefined && candidate !== undefined && managed === candidate
  );
}
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return {
        kind: "error",
        status,
        message: await readApiErrorMessage(res, hint),
      };
    }
