 ...
 *
 * Usage:
 *   bun --filter @moonshot-ai/agent-core-v2 gen:wire-manifest          # write the file
 *   bun --filter @moonshot-ai/agent-core-v2 gen:wire-manifest --check  # freshness check (CI-style)
 *
...
function scanMigrationChain(): string {
  ...
  const chain = pairs.flatMap((p, i) =>
    i === 0 ? [p.source, p.target] : [p.target],
  );
  return chain.join(" -> ");
}
  ...
  if (s !== undefined && depth < 4) {
    if (isRecord(s.properties) && Object.keys(s.properties).length > 0) {
      ...
      for (const [name, prop] of Object.entries(s.properties)) {
        dict[required.has(name) ? name : `${name}?`] = sketchFromJsonSchema(
          prop,
          root,
          depth + 1,
        );
...
function renderTsType(
  ...
  const doc =
    typeof sketch[TYPE_KEY] === "string" ? sketch[TYPE_KEY] : undefined;
  const lines = ["{"];
...
function findImportSource(file: string, name: string): string | undefined {
  ...
  for (const m of source.matchAll(re)) {
    for (const part of m[1]!.split(",")) {
      const named = /^(?:type\s+)?([\w$]+)(?:\s+as\s+([\w$]+))?$/.exec(
        part.trim(),
      );
  ...
  if (bodies.length > 0 && bodies.every((b) => b !== undefined)) {
    ...
    for (const [name, info] of fieldMaps[0]!) {
      if (
        ...
        const values = fieldMaps.map((fm) =>
          tsQuote(fm.get(name)!.type.slice(1, -1)),
        );
    ...
    return truncate(
      fieldMaps
        .map((fm) =>
          stringifySketch(
            renderTsFields(fm, file, budget, charBudget, depth + 1),
          ),
        )
        .join(" | "),
      charBudget * 2,
...
function summarizeTsTypeExpr(
  ...
  if (text.endsWith("[]")) {
    const inner = summarizeTsTypeExpr(
      text.slice(0, -2),
      file,
      budget,
      charBudget,
  ...
  if (intersections.length > 1) {
    ...
    if (
      sides.every((side) => typeof side !== "string" && !Array.isArray(side))
    ) {
  ...
  if (text.startsWith("{")) {
    ...
    if (body !== undefined) {
      return renderTsFields(
        splitTsTypeFields(body),
        file,
        budget,
        charBudget,
        depth,
...
function summarizeTsType(
  ...
  for (const m of readCached(fromFile).matchAll(
    /export\s+\*\s+from\s*'([^']+)'/g,
  )) {
    const target =
      m[1] === undefined ? undefined : resolveModuleFile(fromFile, m[1]);
...
function friendlyZodExpr(expr: string, ownerFile: string, depth = 0): Sketch {
  ...
  if (literal?.[1] !== undefined)
    return friendlyZodExpr(literal[1], ownerFile, depth + 1);
  const union = /^z\.union\((.+)\)$/.exec(text);
  if (union?.[1] !== undefined)
    return friendlyZodUnion(union[1], ownerFile, depth);
  const enumMatch = /^z\.enum\((.+)\)$/.exec(text);
  if (enumMatch?.[1] !== undefined) {
    const body = enumMatch[1].trim().replace(/^\[/, "").replace(/\]$/, "");
    return splitTopLevel(body)
      .map((member) =>
        stringifySketch(friendlyZodExpr(member, ownerFile, depth + 1)),
      )
      .join(" | ");
  }
  ...
  if (/^z\.\w*[oO]bject\(/.test(text)) {
    ...
    for (const [key, fieldExpr] of splitObjectFields(body)) {
      ...
      dict[`${key}${optional ? "?" : ""}`] = friendlyZodExpr(
        fieldExpr,
        ownerFile,
        depth + 1,
      );
...
function friendlyZodUnion(
  ...
  if (members.length > 0 && bodies.every((b) => b !== undefined)) {
    ...
    const spreadSets = fieldMaps.map((fm) =>
      [...fm.keys()].filter((k) => fm.get(k) === ""),
    );
    ...
    const prefix =
      commonSpreads.length > 0 ? `${commonSpreads.join(" + ")} & ` : "";
    return truncate(`${prefix}${sketches.join(" | ")}`, 320);
  }
  return truncate(
    members
      .map((m) => stringifySketch(friendlyZodExpr(m, ownerFile, depth + 1)))
      .join(" | "),
    320,
  );
}
...
function sketchPayloadFromSource(
  ...
  const callRe = new RegExp(
    `defineOp\\(\\s*'${type.replaceAll(".", "\\.")}'\\s*,\\s*\\{`,
  );
  ...
  const afterSchema = optionsBody
    .slice(schemaField.index + schemaField[0].length)
    .trimStart();
...
export async function buildWireManifest(): Promise<string> {
  ...
  const { WIRE_PROTOCOL_VERSION } = (await import(
    "#/wire/migration/migration"
  )) as {
    WIRE_PROTOCOL_VERSION: string;
  };

  const entries = [...OP_REGISTRY.values()].toSorted((a, b) =>
  ...
  for (const entry of entries) {
    ...
    if (crossReducers.length > 0)
      flags.push(`cross-reducers: ${crossReducers.join(", ")}`);
    const owner = owners.get(entry.type);
    const staticSketch =
      owner === undefined
        ? undefined
        : sketchPayloadFromSource(owner, entry.type);
...
async function main(): Promise<void> {
  ...
  console.log(
    `[gen-wire-manifest] wrote ${relative(process.cwd(), MANIFEST_PATH)}`,
  );
}
