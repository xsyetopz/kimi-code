 ...
 *
 * Usage:
 *   bun --filter @moonshot-ai/agent-core-v2 gen:state-manifest          # write the file
 *   bun --filter @moonshot-ai/agent-core-v2 gen:state-manifest --check  # freshness check (CI-style)
 *
  ...
  for (const sf of project.getSourceFiles()) {
    ...
    for (const call of sf.getDescendantsOfKind(SyntaxKind.CallExpression)) {
      ...
      if (
        ...
        expression.getName() !== "register"
      ) {
        continue;
      }
      const args = call.getArguments();
      const [arg] = args;
      if (args.length !== 1 || arg === undefined || !Node.isIdentifier(arg))
        continue;
...
class TypeRenderer {
  ...
  private renderType(
    ...
    if (type.isUnion() && (type.getFlags() & ts.TypeFlags.Boolean) !== 0)
      return "boolean";
  ...
  private renderUnionMembers(
    ...
    const rest = collapseBoolean
      ? members.filter((m) => !m.isBooleanLiteral())
      : members;
    const rank = (type: MorphType): number =>
      type.isNull() ? 1 : type.isUndefined() ? 2 : 0;
    const rendered = rest
  ...
  private needsParensInUnion(type: MorphType): boolean {
    return (
      type.isIntersection() ||
      (type.isObject() && type.getCallSignatures().length > 0)
    );
  }
  ...
  private enumDeclOf(type: MorphType): Node | undefined {
    ...
    if (memberDecl === undefined || !Node.isEnumMember(memberDecl))
      return undefined;
  ...
  private tryRenderEnumUnion(type: MorphType): string | undefined {
    ...
    for (const member of members) {
      if ((member.getFlags() & ts.TypeFlags.EnumLiteral) === 0)
        return undefined;
  ...
  private noteExternal(sym: MorphSymbol): void {
    const isTsLib = (file: string) =>
      /[\\/]node_modules[\\/]typescript[\\/]lib[\\/]/.test(file);
    if (
      sym
        .getDeclarations()
        .some((d) => !isTsLib(d.getSourceFile().getFilePath()))
    ) {
  ...
  private tryRenderAlias(
    ...
    if (alias === undefined || alias.compilerSymbol === skipSymbol)
      return undefined;
    ...
    if (kind === "named") {
      ...
      if (decl === undefined || !Node.isTypeAliasDeclaration(decl))
        return undefined;
  ...
  private renderObjectType(
    ...
    const anonymous =
      sym === undefined || /^__(type|object)$/.test(sym.getName());
    if (!anonymous && sym.compilerSymbol !== skipSymbol) {
      const kind = this.classify(sym);
      if (kind === "named" && typeArgs.length === 0) {
        return this.renderNamed(sym, () =>
          this.renderStructural(type, location, depth),
        );
      }
      if (kind === "ambient") {
    ...
    try {
      ...
      if (
        ...
        return callSignatures
          .map(
            (sig) => `(${this.renderSignature(sig, location, depth, "arrow")})`,
          )
          .join(" & ");
      }
  ...
  private renderObjectBody(
    ...
    for (const prop of props ??
      type.getProperties().filter((p) => this.isPublic(p))) {
    ...
    if (stringIndex !== undefined) {
      lines.push(
        `  [key: string]: ${this.renderType(stringIndex, location, depth + 1)};`,
      );
    }
    const numberIndex = type.getNumberIndexType();
    if (numberIndex !== undefined) {
      lines.push(
        `  [key: number]: ${this.renderType(numberIndex, location, depth + 1)};`,
      );
    }
    return lines;
  }
  ...
  private renderSignature(
    ...
    for (const param of sig.getParameters()) {
      ...
      params.push(
        `${rest ? "..." : ""}${param.getName()}${optional ? "?" : ""}: ${rendered}`,
      );
    }
    const returnType = this.renderType(
      sig.getReturnType(),
      location,
      depth + 1,
    );
...
function renderManifest(
  ...
  for (const scope of SCOPES) {
    ...
    lines.push("");
    lines.push(
      `export type ${scope.keyUnionName} = keyof ${scope.interfaceName};`,
    );
    sections.push(lines.join("\n"));
  }

  const counts = SCOPES.map(
    (s) => `${s.label}: ${byScope.get(s.dir)?.length ?? 0} keys`,
  ).join(" · ");
  const out: string[] = [
    "// App, Workspace, Session & Agent State Manifest",
    "//",
    "// Generated by scripts/gen-state-manifest.mts — do not edit by hand.",
    "// Regenerate with: bun --filter @moonshot-ai/agent-core-v2 gen:state-manifest",
    "//",
...
function main(): void {
  ...
  console.log(
    `[gen-state-manifest] wrote ${relative(process.cwd(), MANIFEST_PATH)}`,
  );
