...
import {
  buildStreamTiming,
  ModelRequesterImpl,
} from "#/kosong/model/modelRequesterImpl";
...
describe("ModelRequesterImpl request execution", () => {
      ...
      modelWith({
        ...
        getAuth: (options) => {
          ...
          return Promise.resolve({
            apiKey: authCalls.length === 1 ? "tok-1" : "tok-2",
          });
        },
      }),
      registryReturning(provider),
  ...
  it("surfaces a replay-surviving 401 as provider.auth_error", async () => {
    const provider = new FakeChatProvider();
    provider.handler = () =>
      Promise.reject(new APIStatusError(401, "account rejected"));
    const requester = new ModelRequesterImpl(
      modelWith({
        canRefresh: true,
    ...
    const failure = await collect(requester.request(INPUT)).catch(
      (error: unknown) => error,
    );
  ...
  it("translates other provider failures and rethrows aborts untouched", async () => {
    ...
    const aborted = await collect(requester.request(INPUT)).catch(
      (error: unknown) => error,
    );
  ...
  it("uploadVideo presence is the capability declaration", async () => {
    ...
    await expect(requester.uploadVideo("file-id")).rejects.toThrow(
...
describe("effectiveMaxCompletionTokens", () => {
  it("reads the folded budget back from the params", () => {
    ...
    expect(effectiveMaxCompletionTokens({ maxCompletionTokens: 512 })).toBe(
      512,
    );
  });
});
