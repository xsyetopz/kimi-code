...
describe("registry-driven vendor verdicts", () => {
  ...
  it("usesTraitDrivenThinking: native traits and the (kimi, anthropic) pair registration", () => {
    ...
    expect(usesTraitDrivenThinking(registry, "anthropic", "anthropic")).toBe(
      false,
    );
    // Kimi registers no google-genai definition — the pair contributes nothing.
    expect(usesTraitDrivenThinking(registry, "google-genai", "kimi")).toBe(
      false,
    );
  });
  ...
  it("requiresStrictThinkingValidation: only the strict-validation thinking driver", () => {
    ...
    expect(requiresStrictThinkingValidation(registry, "openai", "kimi")).toBe(
      true,
    );
    expect(
      requiresStrictThinkingValidation(registry, "anthropic", "kimi"),
    ).toBe(false);
    expect(requiresStrictThinkingValidation(registry, "openai", "openai")).toBe(
      false,
    );
    expect(
      requiresStrictThinkingValidation(registry, "openai", undefined),
    ).toBe(false);
    expect(
      requiresStrictThinkingValidation(registry, "anthropic", "anthropic"),
    ).toBe(false);
  });
});
...
describe("resolveThinkingEffortForModel", () => {
  ...
  it("prefers the normalized request, then config, then the model default", () => {
    expect(
      resolveThinkingEffortForModel("HIGH", undefined, thinkingModel, true),
    ).toBe("high");
    expect(
      resolveThinkingEffortForModel(
        undefined,
    ...
    expect(
      resolveThinkingEffortForModel(undefined, undefined, thinkingModel, true),
    ).toBe("high");
    expect(
      resolveThinkingEffortForModel(
        undefined,
  ...
  it("picks the middle effort when the model declares no default", () => {
    ...
    expect(defaultThinkingEffortForModel({ capabilities: ["thinking"] })).toBe(
      "on",
    );
  ...
  it("normalizes unknown efforts back to the model default under kimi semantics", () => {
    expect(
      resolveThinkingEffortForModel("extreme", undefined, thinkingModel, true),
    ).toBe("high");
    expect(
      resolveThinkingEffortForModel("extreme", undefined, thinkingModel, false),
    ).toBe("extreme");
    expect(
      resolveThinkingEffortForModel("on", undefined, thinkingModel, true),
    ).toBe("high");
  });
  ...
  it("keeps always-thinking models on under kimi semantics", () => {
    ...
    expect(resolveThinkingEffortForModel("off", undefined, always, true)).toBe(
      "low",
    );
    expect(
      resolveThinkingEffortForModel("off", undefined, thinkingModel, true),
    ).toBe("off");
  });
  ...
  it("modelSupportsThinkingEffort validates against the declared effort list", () => {
    ...
    expect(modelSupportsThinkingEffort("extreme", thinkingModel, true)).toBe(
      false,
    );
    expect(modelSupportsThinkingEffort("off", thinkingModel, true)).toBe(true);
    expect(modelSupportsThinkingEffort("extreme", thinkingModel, false)).toBe(
      true,
    );
  });
});
...
describe("resolveForcedThinkingEffort", () => {
  it("applies the forced effort only for trait-driven vendors with thinking on", () => {
    ...
    expect(
      resolveForcedThinkingEffort(undefined, "high", true),
    ).toBeUndefined();
  });
});
