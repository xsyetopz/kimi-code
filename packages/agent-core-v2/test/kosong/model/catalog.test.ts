...
function hostHeadersPort(spec: {
  ...
  return new HostRequestHeadersAdapter(
    ...
    stubAgentIdentity({
      slug: spec.identitySlug,
      hostRequestHeaders: spec.headers,
    }),
  );
}
 ...
 * longer see).
 */
function silentModelWrite(
  models: IModelService,
  records: Record<string, ModelRecord>,
): void {
  (models as unknown as { models: Record<string, ModelRecord> }).models =
    records;
}
...
describe("Model assembly (pure data)", () => {
  ...
  describe("custom identity", () => {
    ...
    const OFFICIAL = {
      ...
      models: {
        k2: { provider: "kimi", model: "kimi-k2", maxContextSize: 200000 },
      },
    };
  ...
  it("resolves provider env-bag credentials and endpoints through the registry", () => {
    const { host, catalog } = createHost({
      providers: {
        kimi: {
          type: "kimi",
          env: {
            KIMI_API_KEY: "env-token",
            KIMI_BASE_URL: "https://kimi-env.example.test/v1",
          },
        },
    ...
    try {
      ...
      return expect(kimi.authProvider.getAuth()).resolves.toEqual({
        apiKey: "env-token",
      });
    } finally {
      host.dispose();
  ...
  it("passes a declared offEffort through providerOptions for the OpenAI wires", () => {
    ...
    try {
      expect(catalog.get("grok").providerOptions).toEqual({
        offEffort: "none",
      });
      expect(catalog.get("grokResponses").providerOptions).toEqual({
        offEffort: "none",
      });
  ...
  it("throws config.invalid for unknown models, missing providers, and incomplete records", () => {
    ...
    expectInvalid(
      {
        models: {
          ghost: { provider: "missing", model: "m", maxContextSize: 1 },
        },
      },
      "ghost",
    );
    ...
    const { host, catalog } = createHost(
      {
        ...
        models: {
          k1: { provider: "kimi", model: "kimi-k2", maxContextSize: 1 },
        },
      },
...
describe("ModelCatalog inspect", () => {
  it("builds the god object with per-field provenance (kimi structured model)", () => {
    ...
    try {
      ...
      expect(view.resolved.auth).toEqual({
        kind: "apiKey",
        apiKey: "••••test",
      });
      expect(view.sources["resolved.protocol"]).toMatchObject({
        kind: "builtin",
      });
      ...
      expect(
        view.sources["resolved.capabilities.max_context_tokens"],
      ).toMatchObject({
        kind: "synthesized",
      });
      expect(
        view.sources["resolved.capabilities.max_input_tokens"],
      ).toMatchObject({
        kind: "none",
      });
      expect(view.sources["resolved"]).toMatchObject({ kind: "synthesized" });
      // Kimi's definition capability is UNKNOWN — nothing is detected.
      expect(view.sources["resolved.capabilities.tool_use"]).toMatchObject({
        kind: "none",
      });
    } finally {
      host.dispose();
  ...
  it("attributes profile-filled efforts and capabilities to builtin", () => {
    ...
    try {
      ...
      expect(view.sources["model.effective.defaultEffort"]).toMatchObject({
        kind: "builtin",
      });
      expect(view.sources["resolved.supportEfforts"]).toMatchObject({
        kind: "builtin",
      });
      expect(view.sources["resolved.capabilities.thinking"]).toMatchObject({
        kind: "builtin",
      });
      expect(
        view.sources["resolved.providerOptions.supportEfforts"],
      ).toMatchObject({
        kind: "builtin",
      });
    } finally {
  ...
  it("attributes override fields to the overrides block", () => {
    ...
    try {
      ...
      expect(view.sources["model.effective.maxContextSize"]).toMatchObject({
        kind: "override",
      });
      expect(view.sources["resolved.maxContextSize"]).toMatchObject({
        kind: "override",
      });
      expect(view.sources["model.effective.model"]).toMatchObject({
        kind: "config",
      });
    } finally {
      host.dispose();
  ...
  it("attributes the input cap to config, its clamp, and its absence", () => {
    ...
    try {
      ...
      expect(
        declaredView.sources["model.effective.maxInputSize"],
      ).toMatchObject({ kind: "config" });
      expect(
        declaredView.sources["resolved.capabilities.max_input_tokens"],
      ).toMatchObject({
        kind: "config",
      });
      ...
      expect(
        clampedView.sources["resolved.capabilities.max_input_tokens"],
      ).toMatchObject({
        kind: "synthesized",
      });
      ...
      expect(
        clampedOverrideView.sources["model.effective.maxInputSize"],
      ).toMatchObject({
        kind: "synthesized",
        detail: expect.stringContaining("clamped"),
      });
      ...
      expect(
        clampedOverrideView.sources["resolved.maxInputSize"],
      ).toMatchObject({
        kind: "synthesized",
      });
      ...
      expect(
        plainView.sources["resolved.capabilities.max_input_tokens"],
      ).toMatchObject({
        kind: "none",
      });
    } finally {
...
describe("wire projection (pure)", () => {
  it("toProtocolModel projects the materialized Model into the snake_case wire shape", () => {
    ...
    try {
      const record = (catalogSections["models"] as Record<string, ModelRecord>)[
        "k2"
      ]!;
...
describe("ModelCatalog enumeration", () => {
  ...
  it("projects official Anthropic effort metadata inferred from the model name", async () => {
    const sections = structuredClone(catalogSections);
    (sections["providers"] as Record<string, ProviderConfig>)["anthropic"] = {
      type: "anthropic",
    };
    ...
    const { host, catalog } = createHost(sections);
    try {
      const opus = (await catalog.listModels()).find(
  ...
  it("projects latest Opus efforts for unknown Claude-marked Anthropic-compatible models", async () => {
    const sections = structuredClone(catalogSections);
    (sections["providers"] as Record<string, ProviderConfig>)["custom"] = {
      type: "anthropic",
    };
  ...
  it("detects env-bag credentials through the vendor endpoint declarations", async () => {
    ...
    try {
      ...
      expect(byId["empty"]).toMatchObject({
        has_api_key: false,
        status: "unconfigured",
      });
    } finally {
  ...
  it("marks an OAuth provider connected when a cached token exists", async () => {
    const { host, catalog } = createHost(
      {
        providers: {
          acme: { type: "kimi", oauth: { storage: "file", key: "oauth/acme" } },
        },
        models: {},
      },
