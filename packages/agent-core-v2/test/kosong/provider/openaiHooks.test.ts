...
const context: TraitContext = {
  config: { protocol: "openai", modelName: "m" },
};
    ...
    const hooks = composeOpenAIChatHooks([
      ...
      resolved({
        mergeHistory: (messages) =>
          messages.some((m) => m["marker"] === "a")
            ? [...messages, { marker: "b" }]
            : undefined,
        buildParams: (params) => ({
          ...params,
          b: (params["a"] as number) + 1,
        }),
      }),
    ]);
  ...
  it("lets the last declarer win", () => {
    const hooks = composeOpenAIChatHooks([
      ...
      resolved({
        ...
        withThinking: (_effort, _options, kwargs) => ({
          ...kwargs,
          second: true,
        }),
      }),
    ...
    expect(hooks?.cacheKey?.("session-1")).toEqual({
...
describe("traitEndpoint / firstProcessEnv", () => {
  ...
  it("returns undefined when no trait declares an endpoint", () => {
    ...
    expect(
      traitEndpoint([resolved({ endpoint: () => undefined })]),
    ).toBeUndefined();
