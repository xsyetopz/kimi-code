...
describe("kimiOpenAITrait reasoning hooks", () => {
  ...
  it("force-replays reasoning only in keep:all sessions with thinking enabled", () => {
    ...
    expect(
      call(kimiOpenAITrait.preserveThinking, keepAllDisabled, context),
    ).toBeUndefined();
    ...
    expect(
      call(kimiOpenAITrait.preserveThinking, keepSome, context),
    ).toBeUndefined();
...
describe("kimiOpenAITrait.extractUsage", () => {
  ...
  it("finds usage inside choices[0].usage", () => {
    ...
    expect(
      call(kimiOpenAITrait.extractUsage, { choices: [{ usage }] }, context),
    ).toBe(usage);
  });

  it("defers to the base default when the chunk carries no usage", () => {
    expect(
      call(kimiOpenAITrait.extractUsage, { choices: [] }, context),
    ).toBeUndefined();
    expect(
      call(kimiOpenAITrait.extractUsage, { choices: [{}] }, context),
    ).toBeUndefined();
...
describe("kimiOpenAITrait request params", () => {
  ...
  it("encodes thinking into extra_body.thinking, carrying keep", () => {
    ...
    expect(
      call(kimiOpenAITrait.withThinking, "high", { keep: "all" }, {}, context),
    ).toEqual({
      extra_body: {
  ...
  it("applies no 128k ceiling in withMaxCompletionTokens", () => {
    expect(
      call(kimiOpenAITrait.withMaxCompletionTokens, 200_000, context),
    ).toEqual({
...
describe("kimiAnthropicTrait (the (kimi, anthropic) registration)", () => {
  const seeded = {
    betaFeatures: ["interleaved-thinking-2025-05-14", "other-beta"],
  };

  it("encodes thinking:{type:enabled} + output_config.effort and strips the interleaved beta", () => {
    const out = call(
      kimiAnthropicTrait.withThinking,
      "high",
      {},
      seeded,
      context,
  ...
  it("omits output_config for on", () => {
    expect(
      call(kimiAnthropicTrait.withThinking, "on", {}, seeded, context),
    ).toEqual({
      thinking: { type: "enabled" },
  ...
  it("encodes off as disabled", () => {
    expect(
      call(kimiAnthropicTrait.withThinking, "off", {}, seeded, context),
    ).toEqual({
