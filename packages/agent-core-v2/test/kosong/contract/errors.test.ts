...
describe("isAbortError", () => {
  ...
  it("rejects provider errors and plain values", () => {
    expect(isAbortError(new APIConnectionError("Connection error."))).toBe(
      false,
    );
...
describe("throwIfAbortError", () => {
  ...
  it("does nothing for non-abort errors", () => {
    expect(() =>
      throwIfAbortError(new APIConnectionError("Connection error.")),
    ).not.toThrow();
...
describe("isRetryableGenerateError", () => {
  ...
  it("retries transient failures", () => {
    expect(
      isRetryableGenerateError(new APIConnectionError("Connection error.")),
    ).toBe(true);
    expect(
      isRetryableGenerateError(new APITimeoutError("Request timed out.")),
    ).toBe(true);
    expect(
      isRetryableGenerateError(
        new APIProviderRateLimitError("Too many requests"),
      ),
    ).toBe(true);
    expect(
      isRetryableGenerateError(
    ...
    expect(
      isRetryableGenerateError(new APIStatusError(503, "Service unavailable")),
    ).toBe(true);
  });

  it("does not retry deterministic client failures", () => {
    expect(
      isRetryableGenerateError(new APIStatusError(400, "Bad request")),
    ).toBe(false);
    expect(
      isRetryableGenerateError(new APIStatusError(401, "Unauthorized")),
    ).toBe(false);
  });
});
...
describe("classifyApiError", () => {
  it("classifies typed errors and carries the status code", () => {
    ...
    expect(
      classifyApiError(new APIProviderRateLimitError("Too many requests")),
    ).toEqual({
      kind: "rate_limit",
      statusCode: 429,
    });
    expect(
      classifyApiError(new APIProviderQuotaExhaustedError("quota exhausted")),
    ).toEqual({
      kind: "quota_exhausted",
      statusCode: 429,
    });
    expect(
      classifyApiError(new APIProviderOverloadedError(529, "Overloaded")),
    ).toEqual({
      kind: "overloaded",
      statusCode: 529,
    });
    expect(
      classifyApiError(new APIConnectionError("Connection error.")),
    ).toEqual({
      kind: "network",
  ...
  it("classifies status errors by status code", () => {
    expect(classifyApiError(new APIStatusError(401, "Unauthorized")).kind).toBe(
      "auth",
    );
    expect(classifyApiError(new APIStatusError(403, "Forbidden")).kind).toBe(
      "auth",
    );
    expect(classifyApiError(new APIStatusError(500, "Internal")).kind).toBe(
      "5xx_server",
    );
    expect(classifyApiError(new APIStatusError(422, "Nope")).kind).toBe(
      "4xx_client",
    );
    // A 413 phrased as token overflow routes to compaction, not 4xx.
    expect(
      classifyApiError(
        new APIStatusError(413, "Request exceeds the maximum size"),
      ).kind,
    ).toBe("4xx_client");
    expect(
      classifyApiError(normalizeAPIStatusError(413, "context length exceeded"))
        .kind,
