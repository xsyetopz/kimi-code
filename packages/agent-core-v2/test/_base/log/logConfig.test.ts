...
describe("path resolution", () => {
  it("resolves the global log path under homeDir/logs", () => {
    expect(resolveGlobalLogPath("/home/kimi")).toBe(
      "/home/kimi/logs/kimi-code.log",
    );
  });

  it("resolves the session log path under sessionDir/logs", () => {
    expect(resolveSessionLogPath("/sessions/s1")).toBe(
      "/sessions/s1/logs/kimi-code.log",
    );
  });
});
