...
import {
  ...
  StateRegistry,
  type StateChange,
} from "#/_base/state/stateRegistry";
...
describe("StateRegistry", () => {
  ...
  it("snapshots non-string-keyed Maps as entry arrays", () => {
    const id = { id: 1 };
    const pairKey = defineState(
      "test.pairs",
      () => new Map<object, string>([[id, "one"]]),
    );
    const registry = new StateRegistry();
  ...
  it("shares referenced objects across branches without false circular marks", () => {
    const shared = { v: 1 };
    const sharedKey = defineState("test.shared", () => ({
      a: shared,
      b: shared,
    }));
    const registry = new StateRegistry();
  ...
  it("collapses class instances to a marker in snapshots but keeps recursing plain data", () => {
    ...
    const mixedKey = defineState("test.mixed", () => ({
      ...
      tool: service,
      nullProto: Object.assign(Object.create(null) as Record<string, unknown>, {
        v: 1,
      }),
    }));
    const registry = new StateRegistry();
...
describe("state services (scoped)", () => {
  ...
  it("keeps registered state invisible to sibling scope tiers", () => {
    ...
    expect(workspace.accessor.get(IWorkspaceStateService).has(sessionKey)).toBe(
      false,
    );
