...
describe("registerScopedService / getScopedServiceDescriptors", () => {
  ...
  it("filters registrations by scope layer", () => {
    ...
    expect(
      getScopedServiceDescriptors(LifecycleScope.App).map((e) => e.id),
    ).toEqual([IApp]);
    expect(
      getScopedServiceDescriptors(LifecycleScope.Session).map((e) => e.id),
    ).toEqual([ISession]);
    expect(
      getScopedServiceDescriptors(LifecycleScope.Agent).map((e) => e.id),
    ).toEqual([IAgent]);
  });
  ...
  it("allows the same id to coexist at different scopes", () => {
    ...
    expect(getScopedServiceDescriptors(LifecycleScope.Session)[0]?.id).toBe(
      IDual,
    );
  });
});
