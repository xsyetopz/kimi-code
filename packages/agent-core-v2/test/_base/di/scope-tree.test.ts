...
describe("Scope tree", () => {
  ...
  it("rejects a child whose kind is not strictly greater", () => {
    const app = createAppScope();
    const session = app.createChild(LifecycleScope.Session, "s1");
    expect(() => session.createChild(LifecycleScope.Session, "s2")).toThrow(
      /greater/,
    );
    expect(() => session.createChild(LifecycleScope.App, "c2")).toThrow(
      /greater/,
    );
    app.dispose();
  });
  ...
  it("rejects duplicate child ids within a parent", () => {
    ...
    expect(() => app.createChild(LifecycleScope.Session, "s1")).toThrow(
      /already has a child/,
    );
    app.dispose();
  });
  ...
  it("extra seed injects a context token resolvable from that scope", () => {
    ...
    const ISessionContext =
      createDecorator<ISessionContext>("tree-session-ctx");
    _clearScopedRegistryForTests();

    const app = createAppScope();
    const session = app.createChild(LifecycleScope.Session, "s1", {
      extra: [
        [ISessionContext as ServiceIdentifier<unknown>, { sessionId: "s1" }],
      ],
    });
  ...
  it("use-after-dispose throws on createChild", () => {
    ...
    const session = app.createChild(LifecycleScope.Session, "s1");
    session.dispose();
    expect(() => session.createChild(LifecycleScope.Agent, "a1")).toThrow(
      /disposed/,
    );
    app.dispose();
  });
