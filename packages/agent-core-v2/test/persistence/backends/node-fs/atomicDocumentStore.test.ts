...
describe("JsonAtomicDocumentStore", () => {
  ...
  it("set + get round-trips a value", async () => {
    await config.set<State>("session", "state.json", {
      title: "hello",
      count: 1,
    });
    expect(await config.get<State>("session", "state.json")).toEqual({
      title: "hello",
      count: 1,
    });
  });
  ...
  it("keys are independent", async () => {
    ...
    expect(await config.get<State>("session", "a.json")).toEqual({
      title: "A",
    });
    expect(await config.get<State>("session", "b.json")).toEqual({
      title: "B",
    });
  });
  ...
  it("delete removes a key; missing delete is a no-op", async () => {
    ...
    expect(await config.get("session", "state.json")).toBeUndefined();
    await expect(
      config.delete("session", "state.json"),
  ...
  it("list returns keys, optionally filtered by prefix", async () => {
    ...
    expect((await config.list("session", "job-")).toSorted()).toEqual([
      "job-1",
      "job-2",
    ]);
  });
  ...
  it("value is persisted through the underlying IFileSystemStorageService", async () => {
    ...
    const raw = new TextDecoder().decode(
      await storage.read("session", "state.json"),
    );
  ...
  it("throws storage.decode_failed when the stored bytes are not valid JSON", async () => {
    ...
    await expect(config.get("session", "bad.json")).rejects.toSatisfy(
      (error: unknown) => {
...
describe("TomlAtomicDocumentStore", () => {
  ...
  it("set + get round-trips a value as TOML", async () => {
    await config.set<State>("session", "config.toml", {
      title: "hello",
      count: 1,
    });
    expect(await config.get<State>("session", "config.toml")).toEqual({
      title: "hello",
      count: 1,
    });
  });
  ...
  it("value is persisted as TOML through the underlying IFileSystemStorageService", async () => {
    ...
    const raw = new TextDecoder().decode(
      await storage.read("session", "config.toml"),
    );
  ...
  it("throws storage.decode_failed when the stored bytes are not valid TOML", async () => {
    await storage.append(
      "session",
      "bad.toml",
      new TextEncoder().encode("key = [unclosed"),
    );
