...
describe("FileStorageService — file permissions", () => {
  ...
  it.skipIf(isWin)(
    "creates scope directories with dirMode (0700)",
    async () => {
  ...
  it.skipIf(isWin)(
    "defaults to the process umask when modes are omitted",
    async () => {
...
describe("FileStorageService — error translation", () => {
  ...
  it.skipIf(isWin)(
    "translates non-ENOENT failures into StorageError(io_failed)",
    async () => {
      const svc = new FileStorageService(dir);
      await mkdir(join(dir, "scope", "adir"), { recursive: true });
      await expect(svc.read("scope", "adir")).rejects.toSatisfy(
        (error: unknown) => {
  ...
  it.skipIf(isWin)(
    "translates write failures into StorageError(io_failed)",
    async () => {
      const svc = new FileStorageService(dir);
      await writeFile(join(dir, "blocked"), "x");
      await expect(
        svc.write("blocked", "k.json", encoder.encode("{}")),
...
describe("FileStorageService — writeStream", () => {
  ...
  it("writes a chunked source and replaces the whole value", async () => {
    ...
    for await (const chunk of svc.readStream("scope", "k.bin"))
      chunks.push(chunk);
