...
describe("MiniDbQueryStore", () => {
  ...
  it("shares the store with a second cluster instance instead of locking it out", async () => {
    ...
    try {
      ...
      expect(await peer.get(`${COLLECTION}${SEP}mine`)).toEqual({
        id: "mine",
        v: 2,
      });
      await store.close();
  ...
  it("wipes and rebuilds the store after the cluster registry is corrupted", async () => {
    ...
    await first.ensureIndex(COLLECTION, {
      kind: "value",
      name: "byV",
      field: "v",
    });
  ...
  it("opens a 16-shard cluster under the cache dir", async () => {
    ...
    const meta = JSON.parse(
      await fsp.readFile(join(storeDir, "cluster.meta.json"), "utf8"),
    ) as {
      shardCount: number;
    };
  ...
  it("getMany returns present values and skips missing keys", async () => {
    ...
    const found = await store.getMany<{ v: number }>(COLLECTION, [
      "a",
      "missing",
      "b",
    ]);
  ...
  it("pageByColumn walks the ordered column with bounds, filter and limit", async () => {
    ...
    const first = await store.pageByColumn<{ id: string; updatedAt: number }>(
      COLLECTION,
      {
        column: "updatedAt",
        dir: "desc",
    ...
    const bounded = await store.pageByColumn<{ id: string; updatedAt: number }>(
      COLLECTION,
      {
        column: "updatedAt",
        dir: "desc",
    ...
    const asc = await store.pageByColumn<{ id: string; updatedAt: number }>(
      COLLECTION,
      {
        column: "updatedAt",
        bounds: { gte: 45 },
