...
function storageServiceSuite(
  ...
  describe(name, () => {
    ...
    it("list returns keys, optionally filtered by prefix", async () => {
      ...
      expect((await service.list("s")).toSorted()).toEqual([
        "alpha",
        "alphabet",
        "beta",
      ]);
      expect((await service.list("s", "alpha")).toSorted()).toEqual([
        "alpha",
        "alphabet",
      ]);
    });
...
describe("FileStorageService", () => {
  it("treats directory fsync on a removed scope dir as a no-op", async () => {
    ...
    try {
      ...
      await expect(
        (
          service as unknown as { syncDirOnce(dir: string): Promise<void> }
        ).syncDirOnce(join(dir, "missing")),
