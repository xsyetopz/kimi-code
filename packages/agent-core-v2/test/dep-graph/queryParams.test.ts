...
describe("readQueryParams", () => {
  ...
  it("parses a comma-separated domain list, trimming and deduping", () => {
    expect(
      readQueryParams("?domain=session, sessionMetadata ,session"),
    ).toEqual({
  ...
  it("drops empty entries from a domain list", () => {
    expect(readQueryParams("?domain=,session,")).toEqual({
      domains: ["session"],
    });
  });
  ...
  it("honors explicit false-ish hideOrphans values", () => {
    expect(readQueryParams("?hideOrphans=false")).toEqual({
      hideOrphans: false,
    });
  ...
  it("parses groupByScope as a boolean flag", () => {
    expect(readQueryParams("?groupByScope=true")).toEqual({
      groupByScope: true,
    });
  });
