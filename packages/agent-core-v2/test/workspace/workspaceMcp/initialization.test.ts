...
describe("Workspace MCP initialization", () => {
  ...
  function createWorkspaceMcpService(
    ready: Promise<void>,
  ...
  it("times out tool calls using the session MCP timeout preference", async () => {
    ...
    const service = createWorkspaceMcpService(Promise.resolve(), {
      toolTimeoutMs: 1,
    });
    await service.ready;
    manager = service.connectionManager();
    ...
    await expect(client.callTool("slow_echo", { text: "hi" })).rejects.toThrow(
      /timed out/i,
    );
  }, 15000);
});
