...
function makeStack(fixture: Fixture, opts?: StackOptions) {
  ...
  if (opts?.extraAgentDirs !== undefined)
    config.setExtraAgentDirs(opts.extraAgentDirs);
...
describe("agent profile loaders + session catalog", () => {
  ...
  it("lists builtin profiles when no agent directories exist", async () => {
    await withFixture(async (fixture) => {
      await withStack(fixture, undefined, async (stack) => {
        ...
        expect(stack.catalog.get(DEFAULT_AGENT_PROFILE_NAME)?.description).toBe(
          "builtin default",
        );
        expect(stack.catalog.getDefault().name).toBe(
          DEFAULT_AGENT_PROFILE_NAME,
        );
        expect(stack.catalog.list().length).toBeGreaterThan(0);
        expect(
          stack.catalog.inspect(DEFAULT_AGENT_PROFILE_NAME)?.sourceId,
        ).toBe("builtin");
      });
    });
  });
  ...
  it("orders sources user < extra < workspace < explicit", async () => {
    await withFixture(async (fixture) => {
      ...
      await writeAgent(
        fixture.extraDir,
        "shared.md",
        agentMd("shared", "from extra"),
      );
      await writeAgent(
        fixture.extraDir,
        "user-extra.md",
        agentMd("user-extra", "from extra"),
      );
      await writeAgent(
        join(fixture.workDir, ".kimi-code", "agents"),
        "shared.md",
      ...
      await withStack(
        ...
        async (stack) => {
          ...
          expect(stack.catalog.get("shared")?.description).toBe(
            "from explicit",
          );
          expect(stack.catalog.get("user-extra")?.description).toBe(
            "from extra",
          );
  ...
  it("merges plugin agents below user; user wins on name collision", async () => {
    await withFixture(async (fixture) => {
      ...
      await writeAgent(
        pluginAgentsDir,
        "shared.md",
        agentMd("shared", "from plugin"),
      );
      await writeAgent(
        pluginAgentsDir,
        "plugin-only.md",
        agentMd("plugin-only", "from plugin"),
      );
      await writeAgent(
        join(fixture.homeDir, "agents"),
        "shared.md",
      ...
      await withStack(
        ...
        async (stack) => {
          ...
          expect(stack.catalog.get("plugin-only")?.description).toBe(
            "from plugin",
          );
        },
      );
  ...
  it("fails ready when an explicit agent file is invalid", async () => {
    await withFixture(async (fixture) => {
      ...
      await withStack(fixture, { explicitFiles: [bad] }, async (stack) => {
        await expect(stack.explicitLoader.ready).rejects.toThrow(
          /description/i,
        );
      });
    });
  ...
  it("resolves relative explicit files against the workspace root", async () => {
    await withFixture(async (fixture) => {
      ...
      await withStack(
        fixture,
        { explicitFiles: ["agents/solo.md"] },
        async (stack) => {
          await stack.ready();

          expect(stack.catalog.get("solo")?.description).toBe(
            "relative explicit",
          );
        },
      );
  ...
  it("reloads the extra loader when extraAgentDirs changes", async () => {
    await withFixture(async (fixture) => {
      await writeAgent(
        fixture.extraDir,
        "from-extra.md",
        agentMd("from-extra", "extra agent"),
      );
      await withStack(fixture, undefined, async (stack) => {
        await stack.ready();
        ...
        expect(stack.catalog.get("from-extra")?.description).toBe(
          "extra agent",
        );
      });
    });
  ...
  it("keeps the builtin default when a same-name file does not opt in to override", async () => {
    await withFixture(async (fixture) => {
      ...
      await withStack(fixture, undefined, async (stack) => {
        ...
        expect(stack.catalog.getDefault().description).not.toBe(
  ...
  it("lets a file profile explicitly override the builtin default", async () => {
    await withFixture(async (fixture) => {
      ...
      await withStack(fixture, undefined, async (stack) => {
        ...
        expect(stack.catalog.getDefault().description).toBe(
          "project default override",
        );
        expect(
          stack.catalog.inspect(DEFAULT_AGENT_PROFILE_NAME)?.sourceId,
  ...
  it("falls back to a valid lower-priority override when the higher candidate does not opt in", async () => {
    await withFixture(async (fixture) => {
      ...
      await withStack(fixture, undefined, async (stack) => {
        ...
        expect(stack.catalog.getDefault().description).not.toBe(
          "project default without override",
        );
        expect(
          stack.catalog.inspect(DEFAULT_AGENT_PROFILE_NAME)?.sourceId,
        ).toBe("user");
  ...
  it("keeps builtin profiles and warns when a non-fatal loader fails its first load", async () => {
    await withFixture(async (fixture) => {
      ...
      await withStack(fixture, { hostFs }, async (stack) => {
        ...
        expect(
          stack.warnings.some(
            (w) => w.includes('"user"') && w.includes("load failed"),
          ),
        ).toBe(true);
        expect(
          stack.registry.entries().some((e) => e.sourceId === "user"),
        ).toBe(false);
      });
    });
  });
  ...
  it("keeps the previous contribution when a non-fatal loader reload fails", async () => {
    await withFixture(async (fixture) => {
      ...
      await withStack(fixture, { hostFs }, async (stack) => {
        ...
        expect(stack.warnings.some((w) => w.includes("load failed"))).toBe(
          true,
        );
        expect(stack.catalog.get("file-agent")?.description).toBe("from file");
        const userEntry = stack.registry
          .entries()
          .find((e) => e.sourceId === "user");
        expect(userEntry?.contribution.profiles.map((p) => p.name)).toContain(
          "file-agent",
        );
      });
    });
      ...
      await withStack(
        ...
        async (stack) => {
          ...
          void stack.explicitLoader
            ...
            .catch((error) =>
              stack.warnings.push(
                `agent profile loader "explicit" reload failed: ${String(error)}`,
              ),
            );
          await vi.waitFor(() => {
            expect(
              stack.warnings.some((w) => w.includes("reload failed")),
            ).toBe(true);
          });

          expect(stack.catalog.get("exp-agent")?.description).toBe("explicit");
          const explicitEntry = stack.registry
            .entries()
            .find((e) => e.sourceId === "explicit");
          expect(
            explicitEntry?.contribution.profiles.map((p) => p.name),
          ).toContain("exp-agent");
        },
      );
    });
  ...
  it("replaces the builtin default system prompt with user-level SYSTEM.md", async () => {
    await withFixture(async (fixture) => {
      ...
      await withStack(fixture, undefined, async (stack) => {
        ...
        const prompt = stack.catalog
          .getDefault()
          .systemPrompt({ cwd: "/work/dir" });
  ...
  it("lets a same-name workspace agent file win over user-level SYSTEM.md", async () => {
    await withFixture(async (fixture) => {
      ...
      await withStack(fixture, undefined, async (stack) => {
        ...
        expect(stack.catalog.getDefault().systemPrompt({})).not.toContain(
          "system md prompt",
        );
      });
    });
  ...
  it("rescans the workspace source when a project agent file changes on disk", async () => {
    await withFixture(async (fixture) => {
      ...
      await mkdir(join(fixture.workDir, ".kimi-code", "agents"), {
        recursive: true,
      });
      await withStack(
        fixture,
        ...
        async (stack) => {
          ...
          const timedOut = new Promise<never>((_resolve, reject) => {
            setTimeout(
              () => reject(new Error("watch-driven refresh timed out")),
              10000,
            );
          });
          ...
          await expect(Promise.race([refreshed, timedOut])).resolves.toBe(
            "workspace",
          );
          expect(stack.catalog.get("watched-agent")?.description).toBe(
            "from watch",
          );
        },
      );
