...
describe("agentRoots", () => {
  ...
  describe("projectRoots", () => {
    it("resolves the brand .kimi-code/agents directory at the .git root", async () => {
      ...
      expect(
        roots.some(
          (r) => r.path.endsWith(".kimi-code/agents") && r.source === "project",
        ),
      ).toBe(true);
    });
    ...
    it("falls back to the generic .agents/agents directory", async () => {
      ...
      expect(
        roots.some(
          (r) => r.path.endsWith(".agents/agents") && r.source === "project",
        ),
      ).toBe(true);
      expect(roots.some((r) => r.path.endsWith(".kimi-code/agents"))).toBe(
        false,
      );
    });
    ...
    it("orders the brand directory before the generic directory", async () => {
      ...
      const roots = await projectAgentRoots(hostFs, root);
      const brandIdx = roots.findIndex((r) =>
        r.path.endsWith(".kimi-code/agents"),
      );
      const genericIdx = roots.findIndex((r) =>
        r.path.endsWith(".agents/agents"),
      );
  ...
  describe("userRoots", () => {
    it("resolves the brand agents directory under homeDir", async () => {
      ...
      expect(
        roots.some((r) => r.path.endsWith("/agents") && r.source === "user"),
      ).toBe(true);
    });
    ...
    it("falls back to the generic .agents/agents under osHomeDir", async () => {
      ...
      expect(
        roots.some(
          (r) => r.path.endsWith(".agents/agents") && r.source === "user",
        ),
      ).toBe(true);
    });
  });
  ...
  describe("configuredRoots", () => {
    ...
    it("skips an unreadable configured root and keeps later roots", async () => {
      ...
      expect(roots.map((candidate) => candidate.path)).toEqual([
        await realpath(availableDir),
      ]);
      expect(warnings.some((warning) => warning.includes(blockedDir))).toBe(
        true,
      );
    });
  });
