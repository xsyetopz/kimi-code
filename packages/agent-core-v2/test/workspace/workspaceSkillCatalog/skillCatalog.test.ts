...
import {
  BuiltinSkillSource,
  IBuiltinSkillSource,
} from "#/app/skillCatalog/builtinSkillSource";
import {
  IUserFileSkillSource,
  UserFileSkillSource,
} from "#/app/skillCatalog/userFileSkillSource";
...
function configStub(): IConfigService & {
  ...
  return {
    ...
    get: (domain: string) => {
      ...
      if (domain === MERGE_ALL_AVAILABLE_SKILLS_SECTION)
        return mergeAllAvailableSkills;
      return undefined;
    },
    inspect: () => ({
      value: undefined,
...
function makeHost(
  ...
  const host = createScopedTestHost([
    ...
    stubPair(
      IBootstrapService,
      stubBootstrap("/home", {}, { skillDirs: explicitDirs }),
    ),
...
describe("WorkspaceSkillCatalogService", () => {
  ...
  it("merges global and project skills; project wins on name collision", async () => {
    ...
    expect(catalog.catalog.getSkill("shared")?.description).toBe(
      "from project",
    );
    host.dispose();
  });
  ...
  it("registers contributions and the merged view into the workspace state container", async () => {
    ...
    expect(states.snapshot()["workspaceSkillCatalog.merged"]).toBe(
  ...
  it("orders project, user and plugin skills as project > user > plugin", async () => {
    ...
    expect(catalog.catalog.getSkill("user-plugin")?.description).toBe(
      "from user",
    );
    expect(catalog.catalog.getSkill("extra-plugin")?.description).toBe(
      "from extra",
    );
    host.dispose();
  });
  ...
  it("replaces default user and project discovery with explicitDirs", async () => {
    const store = new InMemorySkillDiscovery();
    store.setUserSkills([
      stubSkill("from-explicit", { description: "from explicit" }),
    ]);
    store.setProjectSkills([
      stubSkill("project-only", { description: "from project" }),
    ]);
    store.setExtraSkills([
      stubSkill("extra-only", { description: "from extra", source: "extra" }),
    ]);
    store.setPluginSkills([
    ...
    const ws = workspaceContextStub("/work");
    const { host, workspace, config } = makeHost(
      store,
      ws,
      [pluginRoot],
      ["/"],
    ...
    expect(catalog.catalog.getSkill("from-explicit")?.description).toBe(
      "from explicit",
    );
    expect(catalog.catalog.getSkill("project-only")).toBeUndefined();
    expect(catalog.catalog.getSkill("extra-only")?.description).toBe(
      "from extra",
    );
    expect(catalog.catalog.getSkill("plugin-only")?.description).toBe(
      "from plugin",
    );
    host.dispose();
  });
    ...
    class ExtraRootStore implements ISkillDiscovery {
      ...
      async discover(roots: readonly SkillRoot[]) {
        ...
        const pluginSkills = roots
          ...
          .map((root) =>
            stubSkill("demo-skill", { source: "extra", plugin: root.plugin }),
          );
  ...
  it("fires onDidChange with the plugin source id after a plugin reload re-pulls plugin skills", async () => {
    ...
    try {
      ...
      await catalog.load();
      expect(
        catalog.catalog.getPluginSkill("demo", "demo-skill"),
      ).toBeDefined();
  ...
  it("queues a plugin refresh during initial load so the refreshed contribution remains active", async () => {
    ...
    try {
      ...
      expect(
        catalog.catalog.getPluginSkill("demo", "stale-skill"),
      ).toBeUndefined();
      expect(
        catalog.catalog.getPluginSkill("demo", "fresh-skill"),
      ).toBeDefined();
    } finally {
      host.dispose();
  ...
  it("rescans the workspace-root source when a project skill file changes on disk", async () => {
    ...
    try {
      ...
      await mkdir(join(workDir, ".agents", "skills", "watched-skill"), {
        recursive: true,
      });
      await writeFile(
      ...
      expect(catalog.catalog.getSkill("watched-skill")?.description).toBe(
        "from watch",
      );
    } finally {
      host.dispose();
