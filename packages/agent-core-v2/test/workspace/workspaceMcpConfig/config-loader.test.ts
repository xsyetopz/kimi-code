 ...
 *
 * Exercises the real loader against temporary JSON files. Run with `bun
...
describe("loadMcpServers", () => {
  ...
  it("loads root .mcp.json from the repo root and lets project-local override it", async () => {
    ...
    expect(servers["projectOnly"]).toEqual({
      transport: "http",
  ...
  it("throws Error2(config.invalid) on invalid JSON", async () => {
    ...
    await expect(
      loadMcpServers({ fs, cwd, homeDir: home }),
    ).rejects.toBeInstanceOf(Error2);
    await expect(
      loadMcpServers({ fs, cwd, homeDir: home }),
  ...
  it("throws Error2(config.invalid) on schema violation with unknown transport", async () => {
    ...
    await writeJson(join(home, "mcp.json"), {
      mcpServers: {
        bad: { transport: "websocket", url: "https://x.example.com" },
      },
    });
    await expect(
      loadMcpServers({ fs, cwd, homeDir: home }),
  ...
  it("honors KIMI_CODE_HOME env var when homeDir is not supplied", async () => {
    ...
    try {
      ...
      expect(servers["from_env"]).toEqual({
        transport: "stdio",
        command: "env-cmd",
      });
    } finally {
