...
function modelOf(
  target: IWireService,
): ReadonlyMap<string, UserToolRegistration> {
...
describe("AgentUserToolService (wire-backed)", () => {
  ...
  it("preserves deferred disclosure in the wire model and runtime registry", async () => {
    ...
    expect(
      registry.list().find((tool) => tool.name === deferredTool.name)
        ?.disclosure,
    ).toBe("deferred");
  ...
  it("inherits currently registered parent user tools into another agent service", async () => {
    ...
    const childWire = registerTestAgentWire(
      ixChild,
      testWireScope(SCOPE, "user-tool-child"),
      {
