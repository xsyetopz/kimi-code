 ...
 * Run: `bun --filter @moonshot-ai/agent-core-v2 exec vitest run test/agent/goal/tools/goal-tools.test.ts`.
 */
...
describe("goal tools", () => {
  ...
  it("SetGoalBudget reports no current goal without failing", async () => {
    const execution = setGoalBudgetTool.resolveExecution({
      value: 20,
      unit: "turns",
    });
    if (execution.isError === true)
  ...
  it("SetGoalBudget ignores a stale call from a replaced goal turn", async () => {
    ...
    const results = await executeGoalCalls(
      [
        goalToolCall("call_old_budget", "SetGoalBudget", {
          value: 5,
          unit: "turns",
        }),
      ],
  ...
  it("SetGoalBudget applies a delayed execution to a goal created earlier in the same batch", async () => {
    ...
    const results = await executeGoalCalls(
      [
        ...
        goalToolCall("call_budget", "SetGoalBudget", {
          value: 5,
          unit: "turns",
        }),
      ],
    ...
    expect(
      results.find((result) => result.toolName === "SetGoalBudget")?.result
        .output,
    ).toBe("Goal budget set: 5 turns.");
    expect(goals.getGoal().goal).toMatchObject({
      objective: "new task",
      budget: { turnBudget: 5 },
    });
  ...
  it.each([
    ...
    async (updateStatus, expectedCurrentStatus, expectedOutput) => {
      ...
      const outcome = results.find(
        (result) => result.toolName === "UpdateGoal",
      )?.result;
  ...
  it("UpdateGoal reports no active goal when completing/blocking/resuming without one", async () => {
    ...
    if (resumed.isError === true)
      throw new Error("execution should not be an error");
    const resumedResult = await resumed.execute({
      turnId: 0,
      toolCallId: "call_n3",
      signal,
    });
  ...
  async function executeGoalCalls(
    ...
    for await (const result of toolExecutor.execute(calls, {
      turnId,
      signal,
    })) {
      results.push(result);
...
describe("goal tool main-agent gating", () => {
  ...
  it.each(gatedTools)(
    "%s is contributed with a main-agent-only guard",
    (name, ctor) => {
