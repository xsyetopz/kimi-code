  ...
  advanceBy(deltaMs: number): void {
    ...
    while (true) {
      const due = [...this.deadlines]
        .filter(
          (deadline) =>
            !deadline.cancelled && deadline.dueAt <= this.currentTime,
        )
  ...
  return {
    ...
    signal: () => {
      if (activeSignal === undefined)
        throw new Error("LLM request has not started");
      return activeSignal;
    },
  };
}
...
function goalRecords(records: readonly WireRecord[]): readonly GoalRecord[] {
  return records.filter((record): record is GoalRecord =>
    record.type.startsWith("goal."),
  );
}
...
function recordStepUsage(
  ...
  usageService.record("mock-model", usage, {
    type: "turn",
    turnId: turn.id,
    step: 1,
  });
...
function endTurn(
  ...
  const error =
    result.error !== undefined ? toKimiErrorPayload(result.error) : undefined;
  eventBus.publish({
    type: "turn.ended",
    turnId: turn.id,
...
describe("AgentGoalService", () => {
  ...
  describe("AgentGoalService creation", () => {
    ...
    it("rejects empty and too-long objectives", async () => {
      await expect(
        goals.createGoal({ objective: "   " }),
      ).rejects.toMatchObject({
        code: ErrorCodes.GOAL_OBJECTIVE_EMPTY,
      });
      await expect(
        goals.createGoal({ objective: "x".repeat(4001) }),
      ).rejects.toMatchObject({
    ...
    it("rejects duplicate active, paused, and blocked goals without replace", async () => {
      ...
      await expect(
        goals.createGoal({ objective: "second" }),
      ).rejects.toMatchObject({
    ...
    it("replaces an existing goal when replace is set", async () => {
      ...
      const second = await goals.createGoal({
        objective: "second",
        replace: true,
      });
  ...
  describe("AgentGoalService lifecycle", () => {
    it("emits typed lifecycle and completion changes", async () => {
      ...
      const completion = events.find(
        (event) => event.change?.kind === "completion",
      )?.change;
    ...
    it("pauseOnInterrupt parks active goals and no-ops for stopped goals", async () => {
      ...
      const paused = await goals.pauseOnInterrupt({
    ...
    it("cancelGoal discards the goal and throws when missing", async () => {
      ...
      await expect(goals.cancelGoal()).rejects.toMatchObject({
    ...
    it("forbids model-driven goal pauses", async () => {
      ...
      for (const status of ["active", "complete", "blocked"]) {
        expect(UpdateGoalToolInputSchema.safeParse({ status }).success).toBe(
          true,
        );
      }
  ...
  describe("AgentGoalService accounting and budgets", () => {
    ...
    it("tracks telemetry without goal text", async () => {
      ...
      await goals.resumeGoal();
      await goals.markComplete(
        { reason: "private completion reason" },
        "model",
      );
  ...
  describe("AgentGoalService records", () => {
    ...
    it("normalizes active replayed goals to paused", async () => {
      ...
      expect(
        goalRecords(records).filter((record) => record.type === "goal.update"),
      ).toEqual([
        expect.objectContaining({
          type: "goal.update",
          status: "paused",
...
describe("AgentGoalService goal-start review", () => {
  ...
  it("does not review CreateGoal without a goal_start display", async () => {
    setup("manual");
    const hookCtx = createGoalHookContext({
      kind: "generic",
      summary: "Creating a goal",
    });
...
describe("AgentGoalService core workflow hooks", () => {
  ...
  async function startLiveContinuation(
    ...
    const receipt: EnqueueReceipt = {
      assigned: Promise.resolve({ turn, step }),
      abort,
    };
  ...
  it("keeps a replacement goal isolated from late user-turn accounting", async () => {
    ...
    const replacement = await goals.createGoal({
      objective: "new task",
      replace: true,
    });
    await loopService.hooks.onWillBeginStep.run({
  ...
  it.each([
    ...
    "rejects a stale $name call from a replaced goal turn",
    async ({ name, args }) => {
      await goals.createGoal({ objective: "old task" });
  ...
  it.each([
    ...
    async (result) => {
      ...
      recordStepUsage(usageService, goals, continuationTurn, {
        ...zeroUsage,
        output: 7,
      });
  ...
  it.each(["turn", "token", "wall-clock"] as const)(
    ...
    async (budget) => {
      ...
      if (budget === "turn") {
        ...
        await goals.setBudgetLimits(
          { budgetLimits: { wallClockBudgetMs: 1 } },
          "model",
        );
        clock.advanceBy(1);
      }
      ...
      endTurn(eventBus, turn);
      expect(loopService.status()).toMatchObject({
        state: "idle",
      ...
      expect(resumed.terminalReason).toMatch(
  ...
  it("requests one final outcome turn after a terminal UpdateGoal tool result", async () => {
    ...
    expect(goals.getGoal().goal).toBeNull();
    expect(loopService.launches).toEqual([]);
    expect(JSON.stringify(context.get())).not.toContain(
  ...
  it("continues the goal when a goal turn hits the per-turn step limit", async () => {
    ...
    endTurn(eventBus, turn, {
      reason: "failed",
      error: createMaxStepsExceededError(1),
    });
...
describe("AgentGoalService agent eligibility", () => {
  ...
  it.each([
    ...
    [
      "isGoalToolTarget",
      (goals: IAgentGoalService) => goals.isGoalToolTarget(1, "goal-1"),
    ],
    [
      "createGoal",
      (goals: IAgentGoalService) => goals.createGoal({ objective: "work" }),
    ],
    ...
    async (_name, call) => {
      ...
      await expect(
        Promise.resolve().then<unknown>(() => call(goals)),
      ).rejects.toMatchObject({
  ...
  it("does not continue a previously persisted goal when the agent is a subagent", async () => {
    await ctx.restore([
      {
        type: "goal.create",
        goalId: "legacy-subagent-goal",
        objective: "work",
      },
    ]);
    ctx.mockNextResponse({
      type: "text",
      text: "Handled as one normal subagent turn.",
    });
...
describe("goal pause classification on provider errors", () => {
  ...
  function singleAttemptAgentOptions(): Pick<
    TestAgentOptions,
    "initialConfig"
  > {
    return {
  ...
  it("pauses the goal on provider authentication errors", async () => {
    ...
    expect(goal).toMatchObject({
      status: "paused",
      terminalReason:
        "Paused after provider authentication error: Unauthorized",
    });
  });
  ...
  it("pauses the goal on model configuration errors", async () => {
    ...
    expect(goal).toMatchObject({
      status: "paused",
      terminalReason:
        'Paused after model configuration error: LLM not set, send "/login" to login',
    });
  });
  ...
  it("aborts an in-flight LLM request when the wall-clock budget expires", async () => {
    ...
    try {
      ...
      await ctx
        ...
        .setBudgetLimits(
          { budgetLimits: { wallClockBudgetMs: 1_000 } },
          "user",
        );
      ...
      expect((await ctx.rpc.getGoal({})).goal).toMatchObject({
        ...
        terminalReason:
          "Blocked after goal budget reached: wall-clock budget 1000ms",
      });
    } finally {
      await ctx.dispose();
    }
...
describe("AgentGoalService mid-turn budget stop", () => {
  it("grants one tool-free grace step when a token budget is reached mid-turn", async () => {
    ...
    try {
      ...
      ctx.mockNextResponse({
        type: "text",
        text: "Final status: budget exhausted.",
      });
      ctx.mockNextResponse({
        type: "text",
        text: "This step should never run.",
      });
      ...
      const reminderIndex = history.findIndex(
        (message) =>
          ...
          message.origin.name === "goal_budget_stop",
      );
      ...
      expect(goal?.terminalReason).toMatch(
        /^Blocked after goal budget reached/,
      );
  ...
  it("runs the prompt as a normal turn when the goal's turn budget was reached at launch", async () => {
    ...
    try {
      ...
      ctx.mockNextResponse({
        type: "text",
        text: "Answering the prompt normally.",
      });
      ...
      expect(goal?.terminalReason).toBe(
        "Blocked after goal budget reached: turn budget 1",
      );
...
describe("AgentGoalService goal outcome tool result flow", () => {
  it("lets an automatic continuation explain the blocker after UpdateGoal blocks the goal", async () => {
    ...
    try {
      ...
      ctx.mockNextResponse({
        type: "text",
        text: "Blocked because credentials are unavailable.",
      });
      ...
      const history = ctx.get(IAgentContextMemoryService).get();
      expect(JSON.stringify(history)).toContain(
        "Blocked because credentials are unavailable.",
      );
  ...
  it("does not force a goal outcome summary after maxStepsPerTurn is exhausted", async () => {
    ...
    try {
      ...
      ctx.mockNextResponse({
        type: "text",
        text: "This summary should not run.",
      });
      ...
      const history = ctx.get(IAgentContextMemoryService).get();
      expect(JSON.stringify(history)).toContain(
        "Write a concise final message",
      );
      expect(JSON.stringify(history)).not.toContain(
        "This summary should not run.",
      );
