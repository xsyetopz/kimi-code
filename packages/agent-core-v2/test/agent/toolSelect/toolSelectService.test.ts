...
function registerUser(
  ...
  const registration = h.registry.register(tool, {
    source: "user",
    disclosure,
  });
...
describe("AgentToolSelectService S0 baseline (gate closed)", () => {
  ...
  it("keeps deferred user tools inline while the disclosure gate is closed", () => {
    ...
    expect(
      shaped.find((entry) => entry.name === USER_DEFERRED)?.deferred,
    ).toBeUndefined();
  });
...
describe("AgentToolSelectService view shaping (gate open)", () => {
  ...
  it("defers only opted-in user tools and restores them after selection", () => {
    ...
    expect(
      afterLoad.find((entry) => entry.name === USER_DEFERRED)?.deferred,
    ).toBe(true);
    expect(
      afterLoad.find((entry) => entry.name === USER_INLINE)?.deferred,
    ).toBeUndefined();
  });
  ...
  it("hides select_tools when an explicit policy disables disclosure", () => {
    ...
    expect(shaped.map((entry) => entry.name)).not.toContain(
      SELECT_TOOLS_TOOL_NAME,
    );
  });
  ...
  it("shapeHistory removes loaded schemas when the profile disables them", () => {
    ...
    h.contextMemory.history.push(
      schemaMessage(MCP_ALPHA, MCP_BETA),
      userMessage("keep"),
    );
...
describe("AgentToolSelectService.load", () => {
  ...
  it("settles per name: toLoad, alreadyAvailable, unknown", () => {
    ...
    expect(appended.origin).toEqual({
      kind: "injection",
      variant: DYNAMIC_TOOL_SCHEMA_VARIANT,
    });
  });
  ...
  it("loads the schema of an opted-in user tool", () => {
    ...
    expect(
      h.contextMemory.appended[0]?.tools?.map((tool) => tool.name),
    ).toEqual([USER_DEFERRED]);
  });
  ...
  it("pending ledger leads the history inside the defer window", () => {
    ...
    expect(
      h.contextMemory.get().some((message) => message.tools !== undefined),
    ).toBe(false);
  ...
  it("renders the select_tools tool output per name for mixed load results", async () => {
    ...
    const mixed = selectTools.resolveExecution({
      names: [MCP_BETA, MCP_ALPHA, MCP_GONE],
    });
    if (mixed.isError === true)
      throw new Error("expected a runnable execution");
