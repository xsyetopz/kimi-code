...
describe("Agent context", () => {
  ...
  it("stores prompt origins without leaking them to LLM projection", () => {
    ...
    ctx.appendSystemReminder("Remember this.", {
      kind: "injection",
      variant: "host",
    });
    context.append({
      ...
      content: [],
      toolCalls: [
        { type: "function", id: "call_origin", name: "Run", arguments: "{}" },
      ],
    });
    context.append({
      role: "tool",
  ...
  it("get(start, end) returns the size of a context-message range", () => {
    ...
    expect(tokenCounting.get(0, 2)).toEqual({
      size: 1_000,
      measured: 1_000,
      estimated: 0,
    });
