...
describe("AgentToolDedupeService", () => {
  ...
  describe("repeat telemetry", () => {
    ...
    it("does not keep interrupted cross-step history just for duplicate telemetry", async () => {
      ...
      const [result] = await runStep(h, 7, 3, [
        toolCall("a2", "Read", { path: "/a" }),
      ]);

      expect(result!.result.output as string).not.toContain(
        "<system-reminder>",
      );
      expect(
        telemetryEvents.filter((e) => e.event === "tool_call_dedup_detected"),
      ).toHaveLength(0);
      expect(
        telemetryEvents.filter((e) => e.event === "tool_call_repeat"),
      ).toHaveLength(0);
    });
    ...
    it("emits tool_call_repeat with the streak count starting at the second occurrence", async () => {
      ...
      const repeats = telemetryEvents.filter(
        (e) => e.event === "tool_call_repeat",
      );
      expect(repeats.map((e) => e.properties?.["repeat_count"])).toEqual([
        2, 3,
      ]);
      expect(repeats.every((e) => e.properties?.["tool_name"] === "Read")).toBe(
        true,
      );
    ...
    it("labels the action as r1/r2/r3 according to the reminder tier from streak 3 through 11", async () => {
      ...
      for (const e of telemetryEvents) {
        ...
        byCount.set(
          e.properties?.["repeat_count"] as number,
          e.properties?.["action"] as string,
        );
      }
    ...
    it('labels the action as "stop" at streak 12+', async () => {
      ...
      const at12 = telemetryEvents.find(
        (e) =>
          e.event === "tool_call_repeat" &&
          e.properties?.["repeat_count"] === 12,
      );
      const at13 = telemetryEvents.find(
        (e) =>
          e.event === "tool_call_repeat" &&
          e.properties?.["repeat_count"] === 13,
      );
    ...
    it("resets repeat state at turn boundaries", async () => {
      ...
      const [firstInNewTurn] = await runStep(h, 2, 1, [
        toolCall("b1", "Read", { p: 1 }),
      ]);

      expect(firstInNewTurn!.result.output as string).not.toContain(
        "<system-reminder>",
      );
      expect(
        telemetryEvents.filter((e) => e.event === "tool_call_repeat"),
