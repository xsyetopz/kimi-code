...
function permissionRulesStub(
  ...
  const sessionApprovalRulePatterns =
    options.sessionApprovalRulePatterns ?? (() => []);
  return {
    get rules() {
      return rules();
    },
...
function policyContext(
  ...
  const toolCall = toolCallFor(
    input.id ?? `call_${input.toolName}`,
    input.toolName,
    input.args,
  );
  ...
  return {
    ...
    execution: {
      ...
      approvalRule:
        subject === undefined
          ? input.toolName
          : literalRulePattern(input.toolName, subject),
      matchesRule:
        subject === undefined
          ? undefined
...
function matchesRuleSubject(
  ...
  switch (toolName) {
    ...
    case "Write":
    case "Edit":
      return matchesPathRuleSubject(ruleArgs, subject, {
        cwd: "/workspace",
        pathClass: "posix",
      });
    default:
...
function workspaceStub(initialWorkDir: string): {
  ...
  return {
    ...
    addAdditionalDir: (dir) => {
      if (!additionalDirs.includes(dir))
        additionalDirs = [...additionalDirs, dir];
    },
  };
}
