 ...
 * compression. Run: bun test -- test/agent/media/tools/read-media.test.ts
 */
...
describe("ReadMediaFileTool", () => {
  it("has name, parameters, and a path-scoped read access", () => {
    ...
    const execution = tool.resolveExecution({
      path: "/workspace/sample.png",
    }) as Extract<ToolExecution, { execute: unknown }>;
  ...
  it("reflects model capabilities in its description", () => {
    expect(
      makeTool({}, capabilities({ image_in: true, video_in: true }))
        .description,
    ).toContain("image and video files");
    expect(
      makeTool({}, capabilities({ image_in: true, video_in: false }))
        .description,
    ).toContain("Video files are not supported");
    expect(
      makeTool({}, capabilities({ image_in: false, video_in: true }))
        .description,
    ).toContain("Image files are not supported");
    expect(
      makeTool({}, capabilities({ image_in: false, video_in: false }))
        .description,
    ).toContain("does not support image or video input");
  });
  ...
  it("returns a text/image/text wrap plus a <system> note for PNG files", async () => {
    const result = await execute(
      makeTool({ "/workspace/sample.png": { data: pngBuffer() } }),
      {
        path: "/workspace/sample.png",
      },
    );
    ...
    expect(systemText).toContain(
      `Original dimensions: ${PNG_WIDTH}x${PNG_HEIGHT}`,
    );
  ...
  it("downsamples large images and points the model to region readback", async () => {
    ...
    const result = await execute(
      makeTool({ "/workspace/big.png": { data: big } }),
      {
  ...
  it("reads only the sniff header when the default image exceeds the safe decode allocation", async () => {
    ...
    expect(vi.mocked(fs.readBytes)).toHaveBeenCalledWith(
      "/workspace/huge.png",
      512,
    );
  });
  ...
  it("does not treat the decode allocation cap as a hard limit when the configured delivery budget accepts the file", async () => {
    ...
    expect(vi.mocked(fs.readBytes)).toHaveBeenLastCalledWith(
      "/workspace/large.png",
    );
  });
  ...
  it("does not claim downsampling for an image sent untouched", async () => {
    ...
    const result = await execute(
      makeTool({ "/workspace/small.png": { data: png } }),
      {
  ...
  it("reports an EXIF-rotated original in the decoded coordinate space", async () => {
    ...
    const result = await execute(
      makeTool({ "/workspace/portrait.jpg": { data: portrait } }),
      {
  ...
  it("wraps a video as a data URL when no uploader is provided", async () => {
    const result = await execute(
      makeTool({ "/workspace/clip.mp4": { data: mp4Buffer() } }),
      {
    ...
    expect(parts[0]).toEqual({
      type: "text",
      text: '<video path="/workspace/clip.mp4">',
    });
    expect(parts[1]).toMatchObject({
      type: "video_url",
  ...
  it("falls back to an inline base64 video part when the upload fails", async () => {
    ...
    expect(parts[1]).toEqual({
      type: "video_url",
      videoUrl: {
        url: `data:video/mp4;base64,${mp4Buffer().toString("base64")}`,
      },
    });
  });
  ...
  it("rejects files larger than the media limit", async () => {
    ...
    const result = await execute(
      makeTool({
        "/workspace/sample.png": { data: pngBuffer(), size: oversized },
      }),
      { path: "/workspace/sample.png" },
    );
    expect(result.isError).toBe(true);
...
describe("AgentMediaToolsRegistrar", () => {
  ...
  it("registers nothing until a media-capable model binds, then registers ReadMediaFile", () => {
    ...
    expect((tool as ReadMediaFileTool).description).toContain(
...
describe("createVideoUploader", () => {
  ...
  it("returns undefined when the model does not support video upload", () => {
    ...
    expect(
      createVideoUploader({} as Pick<ModelRequester, "uploadVideo">),
    ).toBeUndefined();
  });
  ...
  function heicBytes(): Buffer {
    return Buffer.from([
      ...
      0x00, 0x00, 0x00, 0x00, 0x68, 0x65, 0x69, 0x63, 0x00, 0x00, 0x00, 0x00,
    ]);
  }

  it("refuses HEIC with a conversion command for the execution environment", async () => {
    const result = await execute(
      makeTool({ "/workspace/photo.heic": { data: heicBytes() } }),
      {
