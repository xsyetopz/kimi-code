...
function stubSessionContext(sessionId = "test-session"): ISessionContext {
  return {
    ...
    cwd: "/sessions/test",
    scope: (subKey?: string) =>
      subKey ? `sessions/test/${subKey}` : "sessions/test",
  };
}
...
describe("AgentSkillService", () => {
  ...
  beforeEach(() => {
    ...
    ix = createServices(disposables, {
      additionalServices: (reg) => {
        ...
        registerTestAgentWireServices(reg, "wire/skill-test");        reg.definePartialInstance(IAgentToolRegistryService, {
          register: () => ({ dispose: () => {} }),
        });
  ...
  it("activate throws for an unknown skill", async () => {
    const svc = ix.get(IAgentSkillService);
    await expect(svc.activate({ name: "missing" })).rejects.toThrow(
      /not found/i,
    );
  });
...
describe("SkillTool", () => {
  ...
  it("rejects skills that disable model invocation", async () => {
    skills.register(
      stubSkill("private", { metadata: { disableModelInvocation: true } }),
    );
