...
describe("findInactiveToolPatterns", () => {
  ...
  it("flags a bare * as never matching, and the evaluator agrees", () => {
    expect(findInactiveToolPatterns(["*"])).toEqual([
    ...
    expect(
      isToolActive({ tools: ["*"] }, "mcp__github__create_pr", "mcp"),
    ).toBe(false);
  ...
  it("passes a full mcp__server__tool literal", () => {
    expect(
      findInactiveToolPatterns(["mcp__github__create_issue"], isKnown),
    ).toEqual([]);
  });
