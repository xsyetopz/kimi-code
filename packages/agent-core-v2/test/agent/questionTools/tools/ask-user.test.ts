 ...
 * constructor (`ISessionQuestionService` stubs instead
 * of a fake `Agent`).
 */
...
function makeTool(
  ...
  const request = vi.fn(
    options.request ?? (async () => ({ Postgres: true }) as QuestionResult),
  );
  ...
  const tool = new AskUserQuestionTool(question, tasks, scopeContext);
  return {
    tool,
    request,
    registerTask,
...
describe("AskUserQuestionTool", () => {
  it("exposes current metadata and schema", () => {
    ...
    expect(
      AskUserQuestionInputSchema.safeParse({ questions: [] }).success,
    ).toBe(false);
    expect(
  ...
  it("rejects duplicate question texts across questions (schema + execution)", async () => {
    ...
    expect(AskUserQuestionInputSchema.safeParse(duplicated).success).toBe(
      false,
    );
  ...
  it("describes the no-Other rule on options and the Recommended hint on label", () => {
    ...
    expect(optionsSchema.description).toContain(
  ...
  it("builds the v1-aligned schema including an optional background flag", () => {
    ...
    const params = tool.parameters as {
      properties: {
        background?: { type?: string; default?: boolean; description?: string };
      };
    };
  ...
  it("returns structured answers without leaking the answer method into output", async () => {
    const { tool } = makeTool({
      request: async () => ({
  ...
  it("forwards the request trace through a successful question answer", async () => {
    const { tool } = makeTool();
  ...
  it("returns a dismissed message when every question is dismissed", async () => {
    const { tool } = makeTool({ request: async () => null });
  ...
  describe("background mode", () => {
    ...
    it("runs the question in the background task and settles completed with the answer", async () => {
      ...
      expect(outputs).toEqual([
        JSON.stringify({ answers: { Postgres: true } }),
      ]);
