 ...
 * Run: `bun --filter @moonshot-ai/agent-core-v2 exec vitest run test/agent/plan/planGuard.test.ts`.
 */
  ...
  beforeEach(() => {
    ...
    const toolApproval: IAgentToolApprovalService = {
      ...
      resolvePermissionResolution: async () => {
        throw new Error(
          "resolvePermissionResolution is not used by the plan-guard listener",
        );
      },
    ...
    it("approves with the chosen option prefix and tracks the chosen option", async () => {
      ...
      const decision = await run(
        hookContext("ExitPlanMode", {
          display: planReviewDisplay({ options }),
        }),
      );
