...
describe("EnterPlanModeTool telemetry", () => {
  ...
  it("returns an error when plan mode is already active", async () => {
    ...
    const result = await executeTool(
      new EnterPlanModeTool(planService(), telemetry),
      {
        turnId: 0,
  ...
  it("uses inline guidance when no plan file path is available", async () => {
    ...
    const result = await executeTool(
      new EnterPlanModeTool(planMode, telemetry),
      {
        turnId: 0,
        toolCallId: "call_enter_plan",
        args: {},
  ...
  it("uses plan-file guidance when the host provides a plan file path", async () => {
    ...
    vi.mocked(planMode.status).mockImplementation(async () =>
      active ? ACTIVE_PLAN : null,
    );
    const { telemetry } = recordingTelemetry();
    ...
    expect(result.output).toContain(
      "Write the plan to the plan file with Write or Edit",
    );
  });
...
describe("ExitPlanModeTool telemetry", () => {
  ...
  it("exposes options[].description as optional with a default of empty string", () => {
    const { telemetry } = recordingTelemetry();
    const parameters = new ExitPlanModeTool(
      planService(),
      permissionMode(),
      telemetry,
    ).parameters as {
    ...
    expect(optionSchema.properties["description"]).toMatchObject({
      default: "",
    });
