...
const injectorStub: IAgentContextInjectorService = {
  ...
  register: (name, provider) => {
    ...
    return {
      dispose: () => {
        if (registeredInjection?.provider === provider)
...
beforeEach(() => {
  ...
  ix.set(
    ...
    new SyncDescriptor(AgentPermissionModeService),
  );
...
async function runRegisteredInjection(): Promise<string | undefined> {
  ...
  if (provider === undefined)
    throw new Error("expected permission mode injection provider");
  ...
  if (typeof content !== "string" && content !== undefined) {
    throw new Error(
      "expected permission mode injection provider to return text",
    );
  }
...
describe("AgentPermissionModeService (wire-backed)", () => {
  ...
  it("registers auto-mode reminder injection through the injection service", async () => {
    ...
    expect(autoReminder).toContain(
      "ExitPlanMode is also approved automatically",
    );
    expect(await runRegisteredInjection()).toBeUndefined();

    svc.setMode("manual");
    expect(await runRegisteredInjection()).toContain(
  ...
  it("re-announces auto mode on a fresh instance even with a live reminder in history (restore)", async () => {
    ...
    if (restoredProvider === undefined)
      throw new Error("expected restored provider");
