...
import {
  ToolScheduler,
  type ToolCallTask,
} from "#/agent/toolExecutor/toolScheduler";
...
describe("ToolScheduler", () => {
  it("starts read accesses on the same path concurrently", async () => {
    ...
    const second = makeControlledTask(
      "second",
      readPath("/repo/a.ts"),
      started,
    );
  ...
  it("waits when read and write accesses intersect", async () => {
    ...
    const drained: string[] = [];
    const scheduler = makeScheduler(drained);
    const writer = makeControlledTask(
      "writer",
      writePath("/repo/a.ts"),
      started,
    );
    const reader = makeControlledTask(
      "reader",
      readPath("/repo/a.ts"),
      started,
    );
  ...
  it("serializes write accesses on the same path", async () => {
    ...
    const drained: string[] = [];
    const scheduler = makeScheduler(drained);
    const firstWriter = makeControlledTask(
      "first-writer",
      writePath("/repo/a.ts"),
      started,
    );
    const secondWriter = makeControlledTask(
      "second-writer",
      writePath("/repo/a.ts"),
      started,
    );
  ...
  it("serializes path accesses that differ only by case", async () => {
    ...
    const writer = makeControlledTask(
      "writer",
      writePath("C:\\Repo\\a.ts"),
      started,
    );
    const reader = makeControlledTask(
      "reader",
      readPath("c:/repo/A.ts"),
      started,
    );
  ...
  it("treats recursive path accesses as covering descendants", async () => {
    ...
    const drained: string[] = [];
    const scheduler = makeScheduler(drained);
    const treeReader = makeControlledTask(
      "tree-reader",
      readTree("/repo/src"),
      started,
    );
    const childWriter = makeControlledTask(
      "child-writer",
      writePath("/repo/src/a.ts"),
      started,
    );
  ...
  it("does not start later tasks that conflict with queued accesses", async () => {
    ...
    const exclusive = makeControlledTask(
      "exclusive",
      ToolAccesses.all(),
      started,
    );
