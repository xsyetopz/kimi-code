...
function pluginServiceStub(options: PluginServiceStubOptions): IPluginService {
  ...
  return {
    ...
    removePlugin: async () => {},
    reloadPlugins: async (): Promise<ReloadSummary> => ({
      added: [],
      removed: [],
      errors: [],
    }),
...
function waitForPluginSessionStartMessage(
  ctx: TestAgentContext,
): Promise<void> {
  return new Promise((resolve) => {
    const subscription = ctx
      .get(IEventBus)
      .subscribe("context.spliced", (event) => {
        if (
          event.messages.some(
            (message) =>
              message.origin?.kind === "injection" &&
...
async function injectRegistered(ctx: TestAgentContext): Promise<void> {
  await (
    ctx.get(IAgentContextInjectorService) as unknown as {
      inject(): Promise<void>;
    }
  ).inject();
}
...
describe("AgentPluginService plugin session-start wiring", () => {
  ...
  it("injects the plugin session-start reminder through the real service registration", async () => {
    ...
    expect(text).toContain(
      '<plugin_session_start plugin="demo" skill="demo-skill">',
    );
