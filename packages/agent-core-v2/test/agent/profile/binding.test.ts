...
describe("AgentProfileService.bind", () => {
  ...
  it("binds a profile + model atomically and becomes runnable", async () => {
    ...
    const catalog = new BuiltinAgentProfileLoaderService(
      new AgentProfileRegistryService(),
    );
  ...
  it("restores the subagent allowlist from the binding record without catalog resolution", async () => {
    ...
    const bindingRecord = persistence.records.find(
      (record) => record.type === "profile.bind",
    );
  ...
  it("rejects binding a different profile once bound", async () => {
    ...
    await expect(
      svc.bind({ profile: "coder", model: MOCK_MODEL }),
...
describe("AgentToolPolicyService tool denylist", () => {
  ...
  async function bindProfile(name: string): Promise<IAgentToolPolicyService> {
    ...
    await ctx
      ...
      .bind({ profile: name, model: MOCK_MODEL });
  ...
  it("lists available profiles when binding an unknown profile", async () => {
    ...
    await expect(
      ctx
        .get(IAgentProfileService)
        .bind({ profile: "does-not-exist", model: MOCK_MODEL }),
  ...
  it("persists the denylist in the bind records", async () => {
    ...
    await ctx
      .get(IAgentProfileService)
      .bind({ profile: "deny-builtin", model: MOCK_MODEL });
    await ctx.get(IWireService).flush();

    const record = persistence.records.find(
      (candidate) => candidate.type === "profile.bind",
    );
    expect(record).toMatchObject({
      profileName: "deny-builtin",
      disallowedTools: ["Bash"],
    });
  });
...
describe("AgentToolPolicyService global [tools] config", () => {
  ...
  it("intersects the global config with the profile policy instead of overriding it", async () => {
    const svc = await bindWithToolsConfig(
      { enabled: ["Read", "Bash"] },
      "config-intersect",
    );
...
describe("AgentToolPolicyService.setSessionDisabledTools", () => {
  ...
  it("rejects when no profile is bound yet", async () => {
    ...
    await expect(toolPolicy.setSessionDisabledTools(["Bash"])).rejects.toThrow(
      /not bound/,
    );
...
describe("AgentToolPolicyService executor enforcement", () => {
  ...
  it.each([
    ...
    "blocks a direct builtin call through $name",
    async ({ options, profile, disable }) => {
  ...
  it("blocks a direct MCP call by glob before execution", async () => {
    ...
    await ctx
      .get(IAgentProfileService)
      .bind({ profile: "executor-deny-mcp", model: MOCK_MODEL });
  ...
  it("applies the workspace gate in the prompt projection (skillActive)", async () => {
    ...
    const profileService = ctx.get(IAgentProfileService);
    await profileService.bind({
      profile: "gate-skill-active",
      model: MOCK_MODEL,
    });
  ...
  it.each([
    {
      ...
      options: {
        initialConfig: { tools: { disabled: [SELECT_TOOLS_TOOL_NAME] } },
      },
      disable: undefined,
    },
    {
    ...
    "blocks select_tools through an explicit $name",
    async ({ options, disable }) => {
      ctx = createTestAgent(options, hostEnvironmentServices(homeDir));
      await ctx.get(IAgentProfileService).bind({
        profile: DEFAULT_AGENT_PROFILE_NAME,
        model: MOCK_MODEL,
...
describe("AgentProfileService tool-pattern warnings", () => {
  ...
  function toolPatternWarnings(): readonly {
    code?: string;
    message?: string;
  }[] {
  ...
  it("warns about profile entries that can never activate anything", async () => {
    ...
    const messages = toolPatternWarnings().map(
      (warning) => warning.message ?? "",
    );
    expect(
      messages.some(
        (m) => m.includes('"Bashh"') && m.includes('profile "bad-patterns"'),
      ),
    ).toBe(true);
    expect(
      messages.some(
        (m) => m.includes('"mcp__github"') && m.includes("mcp__github__*"),
      ),
    ).toBe(true);
    expect(
      messages.some((m) => m.includes('"*"') && m.includes("disallowedTools")),
    ).toBe(true);
  });
  ...
  it("warns about global [tools] config entries that can never activate anything", async () => {
    ...
    expect(
      messages.some(
        (m) =>
          m.includes('"*"') &&
          m.includes("the global [tools] config") &&
          m.includes("enabled"),
      ),
    ).toBe(true);
  });
  ...
  it("bind also publishes the warnings", async () => {
    ...
    await ctx
      .get(IAgentProfileService)
      .bind({ profile: "bind-bad-patterns", model: MOCK_MODEL });
...
describe("agentsMdReminder seeding", () => {
  ...
  it("seeds the known-set with the injected paths after a successful bind", async () => {
    const seedInjected =
      vi.fn<(paths: readonly string[], cwd: string) => void>();
    ...
    expect(profile.data().agentsMdPaths).toEqual([
      normalize(join(workDir, "AGENTS.md")),
    ]);
  });
  ...
  it("does not seed when the prompt build fails before the bind commits", async () => {
    ...
    seedInjected.mockClear();
    await expect(
      profile.bind({ profile: "throws-on-prompt", model: MOCK_MODEL }),
    ).rejects.toThrow("prompt build boom");
