...
const kimiEffortModel = {
  ...effortModel,
  protocol: "openai",
  providerType: "kimi",
};
const kimiBooleanModel = {
  ...booleanModel,
  protocol: "openai",
  providerType: "kimi",
};
...
describe("defaultThinkingEffortForModel", () => {
  ...
  it("returns on for boolean thinking models (thinking support without supportEfforts)", () => {
    ...
    expect(
      defaultThinkingEffortForModel({ capabilities: ["always_thinking"] }),
    ).toBe("on");
    expect(defaultThinkingEffortForModel({ adaptiveThinking: true })).toBe(
      "on",
    );
  });
});
...
describe("resolveThinkingEffortForModel", () => {
  it("returns the requested effort verbatim when one is provided", () => {
    expect(resolveThinkingEffortForModel("low", undefined, effortModel)).toBe(
      "low",
    );
    expect(
      resolveThinkingEffortForModel("on", { enabled: false }, booleanModel),
    ).toBe("on");
    expect(resolveThinkingEffortForModel("off", undefined, booleanModel)).toBe(
      "off",
    );
    expect(
      resolveThinkingEffortForModel("on", { effort: "medium" }, effortModel),
    ).toBe("medium");
  });

  it("returns off when config.enabled is false and no effort is requested", () => {
    expect(
      resolveThinkingEffortForModel(undefined, { enabled: false }, effortModel),
    ).toBe("off");
    expect(
      resolveThinkingEffortForModel(
        undefined,
  ...
  it("uses config.effort as the default effort", () => {
    expect(
      resolveThinkingEffortForModel(undefined, { effort: "high" }, effortModel),
    ).toBe("high");
    expect(
      resolveThinkingEffortForModel(
        undefined,
  ...
  it("falls back to defaultThinkingEffortForModel(model) when no effort is configured", () => {
    expect(
      resolveThinkingEffortForModel(undefined, undefined, effortModel),
    ).toBe("medium");
    expect(resolveThinkingEffortForModel(undefined, {}, booleanModel)).toBe(
      "on",
    );
    expect(resolveThinkingEffortForModel(undefined, undefined, undefined)).toBe(
      "off",
    );
  });
  ...
  it("clamps always-thinking models to their default effort even without strict validation", () => {
    ...
    expect(
      resolveThinkingEffortForModel("off", undefined, alwaysThinkingModel),
    ).toBe("on");
  });
  ...
  it("normalizes a configured off value (case/whitespace) instead of sending it upstream", () => {
    ...
    expect(
      resolveThinkingEffortForModel(undefined, { effort: "Off" }, booleanModel),
    ).toBe("off");
    expect(
      resolveThinkingEffortForModel(
        undefined,
  ...
  it("carries custom requested efforts through", () => {
    expect(resolveThinkingEffortForModel("xhigh", undefined, undefined)).toBe(
      "xhigh",
    );
    expect(
      resolveThinkingEffortForModel("bogus", { effort: "low" }, undefined),
    ).toBe("bogus");
  });

  it("normalizes requested effort case and whitespace", () => {
    expect(
      resolveThinkingEffortForModel("  Medium ", undefined, undefined),
    ).toBe("medium");
    expect(
      resolveThinkingEffortForModel("OFF", { effort: "high" }, undefined),
    ).toBe("off");
  });

  it("falls back to the model default for an unsupported Kimi effort", () => {
    expect(
      resolveThinkingEffortForModel("ultra", undefined, kimiEffortModel, true),
    ).toBe("medium");
  });

  it("projects a concrete effort to on for a boolean-only Kimi model", () => {
    expect(
      resolveThinkingEffortForModel("ultra", undefined, kimiBooleanModel, true),
    ).toBe("on");
  });

  it("reports unsupported concrete efforts only for Kimi effort models", () => {
    expect(modelSupportsThinkingEffort("ultra", kimiEffortModel, true)).toBe(
      false,
    );
    expect(modelSupportsThinkingEffort("ultra", openaiEffortModel, false)).toBe(
      true,
    );
  });
});
