...
describe("permissionRules/matchPermissionRule", () => {
  ...
  it("matches rules against tool-specific argument fields through execution matchers", () => {
    ...
    expect(
      matches(rule("mcp__github__*"), "mcp__github__list_issues", noArgs),
    ).toBe(true);
    expect(
      matches(rule("Bash(git *)"), "Bash", {
  ...
  it("does not match rule arguments without an execution matcher", () => {
    expect(matches(rule('Custom("query":"a.b")'), "Custom", noArgs)).toBe(
      false,
    );
    expect(matches(rule('Bash("command":"git status")'), "Bash", noArgs)).toBe(
      false,
    );
    expect(matches(rule("Bash(^git status$)"), "Bash", noArgs)).toBe(false);
    expect(matches(rule("Read([invalid"), "Read", noArgs)).toBe(false);
    expect(matches(rule("AgentSwarm(swarm)"), "AgentSwarm", noArgs)).toBe(
      false,
    );
  });
...
function matches(
  ...
  return (
    matchPermissionRule({ rule: permissionRule, toolName, execution }) !==
    undefined
  );
}
