...
describe("stepRetry plugin", () => {
  ...
  function rpcEvents(name: string) {
    return ctx.allEvents.filter(
      (event) => event.type === "[rpc]" && event.event === name,
    );
  }
  ...
  async function runTurn(turnId: number, signal?: AbortSignal) {
    ctx
      .get(IEventBus)
      .publish({ type: "turn.started", turnId, origin: { kind: "user" } });
  ...
  it("retries a retryable provider error and resumes the same step number", async () => {
    ...
    expect(
      rpcEvents("turn.step.started").map(
        (event) => (event.args as { step: number }).step,
      ),
    ).toEqual([1, 2]);
  ...
  it("honors the provider retry-after delay before retrying", async () => {
    ...
    ctx = createTestAgent(
      llmGenerateServices(async () => {
        calls += 1;
        if (calls === 1)
          throw new APIProviderRateLimitError("slow down", null, 1);
        return {
