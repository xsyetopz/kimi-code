...
function expectModelFacingParity(
  ...
  expect(JSON.stringify(actual.parameters)).toBe(
...
class FakeTaskService implements IAgentTaskService {
  ...
  track(
    _handle: ITaskHandle,
    _options: AgentTaskTrackOptions,
  ): IAgentTaskEntry {
  ...
  list(activeOnly = true, limit?: number): readonly AgentTaskInfo[] {
    ...
    for (const entry of this.entries.values()) {
      ...
      if (
        !activeOnly &&
        TERMINAL_STATUSES.has(info.status) &&
        info.detached === false
      )
        continue;
      result.push(info);
  ...
  async stopAll(reason?: string): Promise<readonly AgentTaskInfo[]> {
    const stopped = await Promise.all(
      Array.from(this.entries.keys()).map((taskId) =>
...
describe("TaskListTool", () => {
  it("has name and accepts the current schema", () => {
    ...
    expect(
      TaskListInputSchema.safeParse({ active_only: true, limit: 1 }).success,
    ).toBe(true);
    expect(
      TaskListInputSchema.safeParse({ active_only: true, limit: 0 }).success,
    ).toBe(false);
  ...
  it("does not wait when listing a running task", async () => {
    const tasks = new FakeTaskService();
    tasks.add(
      processTask({ taskId: "bash-running2", description: "running task" }),
    );
...
describe("TaskOutputTool", () => {
  ...
  it("returns live output when no persisted log is available", async () => {
    ...
    expect(output).toContain(
      `output_size_bytes: ${Buffer.byteLength(payload).toString()}`,
    );
  ...
  it("returns persisted output path and guidance when a log is available", async () => {
    ...
    expect(output).toContain("status: completed");
    expect(output).toContain(
      "output_path: /tmp/session/tasks/bash-persist1/output.log",
    );
...
describe("TaskStopTool", () => {
  it("has name and accepts the current schema", () => {
    ...
    expect(
      TaskStopInputSchema.safeParse({ task_id: "bash-1", reason: "" }).success,
    ).toBe(true);
  ...
  it.each([
    ...
    expect(tasks.stopCalls).toEqual([
      { taskId, reason: "Stopped by TaskStop" },
    ]);
  ...
  it("returns info when task is already terminal without suppressing notification", async () => {
    ...
    expect(tasks.getTask(taskId)?.terminalNotificationSuppressed).not.toBe(
      true,
    );
  });
...
describe("task tool descriptions", () => {
  ...
  it("matches the v1 model-facing contract exactly", () => {
    expectModelFacingParity(
      new TaskListTool(tasks),
      new V1TaskListTool({} as never),
    );
    expectModelFacingParity(
      new TaskOutputTool(tasks),
      new V1TaskOutputTool({} as never),
    );
    expectModelFacingParity(
      new TaskStopTool(tasks),
      new V1TaskStopTool({} as never),
    );
  });
