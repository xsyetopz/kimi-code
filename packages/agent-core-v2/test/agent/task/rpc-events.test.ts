...
function firstAppendedContextMessage(agent: FakeTaskAgent): TestContextMessage {
  const call = agent.context.appendUserMessage.mock
    .calls[0] as unknown as TestContextMessage[];
  const message = call.at(-1);
  if (message === undefined)
    throw new Error("Expected an appended context message");
  return message;
}
...
function notificationMessageFor(
  agent: FakeTaskAgent,
  taskId: string,
): TestContextMessage {
  for (const call of agent.context.appendUserMessage.mock
    ...
    for (const message of call) {
      if (message.origin?.kind === "task" && message.origin.taskId === taskId)
        return message;
    }
  }
...
describe("AgentTaskService — event emission", () => {
  ...
  it("emits task.terminated on stop", async () => {
    ...
    expect(
      agent.emittedEvents.filter((e) => e.type === "task.terminated"),
    ).toEqual([
      {
        type: "task.terminated",
        info: expect.objectContaining({
  ...
  it("emits task.terminated when a restored task is marked lost", async () => {
    const sessionDir = await mkdtemp(
      join(tmpdir(), "kimi-bg-agent-reconcile-"),
    );
...
describe("AgentTaskService — notification delivery", () => {
  ...
  it("enqueues stopped process task notifications into the turn flow", async () => {
    ...
    expect(message.content[0]!.text).toContain(
      "long shell task was stopped by user.",
    );
  });
  ...
  it("TaskStopTool persists stop reason and suppression across reload", async () => {
    ...
    try {
      ...
      const result = await executeTool(
        ...
        toolContext("task_stop_persisted", {
          task_id: taskId,
      ...
      expect(agent.context.appendUserMessage).not.toHaveBeenCalled();
      expect(
        readerFixture.ctx.get(IAgentLoopService).hasPendingRequests(),
      ).toBe(false);
    } finally {
  ...
  it("re-delivers a terminal task notification removed by undo when output is unavailable", async () => {
    ...
    try {
      ...
      expect(
        ctx.context.get().some((message) => message.origin?.kind === "user"),
      ).toBe(false);
      expect(
  ...
  it("preserves a queued notification when undo rejects an active turn", async () => {
    ...
    try {
      ...
      await expect(
        ctx.get(IAgentConversationUndoService).undo(1),
