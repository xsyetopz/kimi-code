...
function createAgentTaskService(
  options: { sessionDir?: string; maxRunningTasks?: number } = {},
): TaskServiceFixture {
  const persistence =
...
describe("AgentTaskService", () => {
  ...
  it("honours the list limit parameter", () => {
    const { manager } = createAgentTaskService();
    const first = registerProcess(
      manager,
      pendingProcess().proc,
      "sleep 1",
      "one",
    );
    const second = registerProcess(
      manager,
      pendingProcess().proc,
      "sleep 2",
      "two",
  ...
  it("also terminates a detached process task that exceeds the output limit", async () => {
    ...
    const taskId = manager.registerTask(
      new ProcessTask(proc, "producer", "bg"),
      {
        detached: true,
        timeoutMs: 60_000,
      },
  ...
  it("escalates a wall-clock timeout to SIGKILL when the process ignores SIGTERM", async () => {
    ...
    const taskId = manager.registerTask(
      new ProcessTask(proc, "runaway", "timeout sigkill"),
      {
        timeoutMs: 1,
      },
    );
  ...
  it("reports timed_out when a timed-out process exits to SIGTERM within the grace window", async () => {
    ...
    const { proc, killSpy } = pendingProcess();
    const taskId = manager.registerTask(
      new ProcessTask(proc, "sleep 60", "timeout graceful"),
      {
        timeoutMs: 1,
      },
    );
  ...
  it("applies the SIGTERM grace + SIGKILL escalation to a detachTimeout deadline", async () => {
    ...
    const taskId = manager.registerTask(
      new ProcessTask(proc, "runaway", "detach timeout"),
      {
        detached: false,
        detachTimeoutMs: 1,
      },
  ...
  it("auto-backgrounds a foreground task instead of killing it when its deadline fires", async () => {
    ...
    const { proc, killSpy } = pendingProcess();
    const taskId = manager.registerTask(
      new ProcessTask(proc, "sleep 60", "auto background"),
      {
        detached: false,
        timeoutMs: 1_000,
  ...
  it("kills a foreground task on timeout when auto-background is not enabled", async () => {
    ...
    const { proc, killSpy } = pendingProcess();
    const taskId = manager.registerTask(
      new ProcessTask(proc, "sleep 60", "plain timeout"),
      {
        detached: false,
        timeoutMs: 1_000,
        detachTimeoutMs: 5_000,
  ...
  it("getTask on an unknown id does not create persisted state", async () => {
    ...
    try {
      const { ctx, manager, persistence } = createAgentTaskService({
        sessionDir,
      });
