...
describe("AgentTaskPersistence", () => {
  ...
  it("listTasks enumerates all persisted entries", async () => {
    ...
    await persistence.writeTask(
      sample({ taskId: "bash-22222222", command: "pnpm test" }),
    );
  ...
  it("rejects path-traversal task ids", async () => {
    ...
    await expect(persistence.readTask("../etc/passwd")).rejects.toThrow(
      /Invalid task id/,
    );
    expect(() => persistence.taskOutputFile("../etc/passwd")).toThrow(
      /Invalid task id/,
    );
  });
  ...
  describe("readTaskOutputBytes / taskOutputSizeBytes", () => {
    ...
    it("readTaskOutputBytes returns the exact byte window for offset + maxBytes", async () => {
      await persistence.appendTaskOutput(
        "bash-page0000",
        "abcdefghijklmnopqrstuvwxyz",
      );

      expect(
        await persistence.readTaskOutputBytes("bash-page0000", 5, 10),
      ).toBe("fghijklmno");
      expect(await persistence.readTaskOutputBytes("bash-page0000", 0, 3)).toBe(
        "abc",
      );
      expect(
        await persistence.readTaskOutputBytes("bash-page0000", 20, 100),
      ).toBe("uvwxyz");
      expect(
        await persistence.readTaskOutputBytes("bash-page0000", 26, 10),
      ).toBe("");
    });

    it("readTaskOutputBytes returns empty string when output.log is absent", async () => {
      expect(
        await persistence.readTaskOutputBytes("bash-none0001", 0, 100),
      ).toBe("");
    });
  });
  ...
  describe("legacy session-root fallback", () => {
    ...
    it("keeps agent-local task and output authoritative without changing either root", async () => {
      ...
      expect(await legacy.readTaskOutputBytes(taskId, 0, 100)).toBe(
        "legacy output",
      );
      expect(await primary.readTaskOutputBytes(taskId, 0, 100)).toBe(
        "local output",
      );
    });
    ...
    it("treats a corrupt agent-local task key as authoritative over legacy data", async () => {
      ...
      await writeFile(
        join(sessionDir, AGENT_SCOPE, "tasks", `${taskId}.json`),
        "{not json",
      );
    ...
    it("treats an unrecognized agent-local task document as authoritative over legacy data", async () => {
      ...
      const primary = rootedPersistence(AGENT_SCOPE, sessionRoot());
      await legacy.writeTask(sample({ taskId, description: "legacy task" }));
      await docs.set(`${AGENT_SCOPE}/tasks`, `${taskId}.json`, {
        unexpected: true,
      });
