...
const persistedOp = testModel.defineOp("test.op.persisted", {
  ...
  apply: (state, payload: { value: number }) => ({
...
function typecheckRegisteredOps(): void {
  ...
  testModel.defineOp(dynamicType, {
    schema: z.object({}),
    apply: (state) => state,
  });

  const unionType = "test.op.persisted" as
    | "test.op.persisted"
    | "test.op.unregistered";
  // @ts-expect-error Op definitions reject unions that could mix registered and legacy types
  testModel.defineOp(unionType, {
    schema: z.object({}),
    apply: (state) => state,
  });
  ...
  const brandedType = "test.op.persisted" as string & {
    readonly opType: unique symbol;
  };
...
describe("op-uniqueness", () => {
  it("defineOp throws DuplicateOpError when a type is registered twice", () => {
    ...
    try {
      model.defineOp("lint.duplicate.runtime", {
        schema: z.object({}),
        apply: (s) => s,
      });
      expect(() =>
