...
describe("cron-fired steer turn context", () => {
  ...
  beforeEach(async () => {
    ...
    ctx = createTestAgent(
      sessionService(IAgentLifecycleService, lifecycleStub),
    );
  ...
  it("carries earlier tool results into the cron-fired steer turn request", async () => {
    ...
    const lastUser = fireRequest.history
    ...
    const requestToolTexts = fireRequest.history
      ...
      .map((part) => (part.type === "text" ? part.text : ""));
    expect(
      requestToolTexts.some((text) => text.includes(`id: ${jobId!}`)),
    ).toBe(true);
  });
});
