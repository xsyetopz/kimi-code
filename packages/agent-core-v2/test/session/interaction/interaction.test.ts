...
function makeFakeAgent(agentId: string): FakeAgent {
  ...
  const accessor: ServicesAccessor = {
    get: <T>(id: ServiceIdentifier<T>): T => {
      ...
      throw new Error(
        `unexpected service request in fake agent: ${String(id)}`,
      );
    },
  };
  return {
...
describe("SessionInteractionService", () => {
  ...
  beforeEach(() => {
    ...
    ix.set(
      ...
      new SyncDescriptor(SessionInteractionService),
    );
  });
  ...
  it("cancelPendingForTurn resolves cancelled interactions through onDidResolve", () => {
    ...
    expect(seen).toEqual([
      { id: "a1", response: { cancelled: true, reason: "turn_ended" } },
    ]);
    expect(svc.listPending()).toHaveLength(0);
  });
  ...
  it("request journals an interaction.request op to the origin agent wire", () => {
    ...
    expect(
      sub.dispatched.map((op) => ({ type: op.type, payload: op.payload })),
    ).toEqual([
      {
  ...
  it("journals to the main agent wire when the origin has no agentId", () => {
    ...
    expect(
      main.dispatched.map((op) => ({ type: op.type, payload: op.payload })),
    ).toEqual([
      {
        type: "interaction.request",
        payload: {
  ...
  it("kernel semantics are unchanged when the origin agent is absent", async () => {
    const svc = ix.get(ISessionInteractionService);
    const pending = svc.request<unknown, string>({
      kind: "question",
      payload: {},
    });
    svc.respond(svc.listPending()[0]!.id, "ok");
...
describe("interaction ops (wire-backed)", () => {
  ...
  it("request/resolved persist to the journal and fold into the model by id", async () => {
    ...
    wire.dispatch(
      interactionResolved({ id: "i1", response: { decision: "approved" } }),
    );
  ...
  it("replay rebuilds the interaction map from persisted records", async () => {
    ...
    const wire2 = registerTestAgentWire(
      ix2,
      testWireScope(SCOPE, "interaction-replay"),
      {
        log: log2,
      },
    );
    ...
    expect(model.get("i1")).toMatchObject({
      resolved: true,
      response: { answer: "a" },
    });
    expect(model.get("i2")).toMatchObject({
      resolved: false,
      toolCallId: "call-2",
    });
  });
