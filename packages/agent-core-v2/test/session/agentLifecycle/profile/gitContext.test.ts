...
describe("collectGitContext", () => {
  ...
  it("returns an unavailable block when the directory is not a git repository", async () => {
    ...
    await expect(
      collectGitContext(runner, "/not-a-repo", logger),
    ).resolves.toBe('<git-context status="unavailable" reason="not-a-repo"/>');
  ...
  it("omits both Remote and Project for a disallowed remote host", async () => {
    const { runner } = gitRunner({
      ...
      "remote get-url origin": {
        stdout: "git@internal.example.test:secret/repo.git",
      },
      "symbolic-ref --short HEAD": { stdout: "main" },
...
describe("remote url helpers", () => {
  it("sanitizes allowed https remotes and drops credentials", () => {
    expect(
      sanitizeRemoteUrl("https://user:token@github.com/owner/repo.git"),
    ).toBe("https://github.com/owner/repo.git");
  });

  it("rejects private hosts", () => {
    expect(
      sanitizeRemoteUrl("https://git.example.test/owner/repo.git"),
    ).toBeNull();
  });

  it("parses project names from ssh and https urls", () => {
    expect(parseProjectName("git@github.com:owner/repo.git")).toBe(
      "owner/repo",
    );
    expect(parseProjectName("https://github.com/owner/repo.git")).toBe(
      "owner/repo",
    );
  });
});
