...
function catalogOf(
  ...skills: readonly ReturnType<typeof stubSkill>[]
...
describe("SessionSkillCatalogService (seed view)", () => {
  ...
  it("merges ad-hoc sink contributions over the seed and drops them on remove", async () => {
    const seed = dataSeed(
      catalogOf(stubSkill("shared", { description: "from workspace" })),
    );
    const { host, catalog } = makeSession(seed.data);
    await catalog.load();
    ...
    sink.set(
      ...
      {
        skills: [
          stubSkill("shared", { description: "from adhoc" }),
          stubSkill("adhoc-only"),
        ],
      },
      { priority: 40 },
    );
  ...
  it("list returns plain summaries of the merged catalog after ready", async () => {
    ...
    const seeded = summaries.find(
      (summary) => summary.name === "from-workspace",
    );
    ...
    expect(summaries.some((summary) => summary.name === "adhoc-only")).toBe(
      true,
    );
    host.dispose();
  });
