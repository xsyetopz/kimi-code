...
function processWithOutput(
  ...
  const stdout = Readable.from(
    options.stdout === undefined ? [] : [options.stdout],
  );
...
function createTestRunner(proc: IProcess | ReturnType<typeof vi.fn>) {
  const exec =
    typeof proc === "function" ? proc : vi.fn().mockResolvedValue(proc);
...
function createFakeTaskService(options: { maxRunningTasks?: number } = {}): {
  ...
  const service: IAgentTaskService = {
    ...
    registerTask(
      task: AgentTask,
      registerOptions: RegisterAgentTaskOptions = {},
    ): string {
      ...
      entry.lifecyclePromise = Promise.resolve()
        .then(() =>
          task.start({
            ...
            settle: async (settlement: AgentTaskSettlement) =>
              settleTask(entry, settlement),
          }),
        )
        .catch((error: unknown) => {
    ...
    async getOutputSnapshot(taskId: string): Promise<AgentTaskOutputSnapshot> {
      ...
      return {
        outputPath: fullOutputAvailable
          ? `/fake/tasks/${taskId}/output.log`
          : undefined,
    ...
    async stopAll(reason?: string): Promise<readonly AgentTaskInfo[]> {
      ...
      return results.filter(
        (info): info is AgentTaskInfo => info !== undefined,
      );
    },
...
describe("BashTool", () => {
  it("exposes current metadata and schema", () => {
    ...
    expect(BashInputSchema.safeParse({ command: "echo hello" }).success).toBe(
      true,
    );
    expect(BashInputSchema.safeParse({ command: "" }).success).toBe(false);
    expect(
      BashInputSchema.safeParse({ command: "echo x", timeout: 0 }).success,
    ).toBe(false);
    expect(
      BashInputSchema.safeParse({ command: "echo x", timeout: 300 }).success,
    ).toBe(true);
    expect(
      BashInputSchema.safeParse({ command: "echo x", timeout: 301 }).success,
    ).toBe(false);
    expect(
      BashInputSchema.safeParse({ command: "echo x", timeout: 300_000 })
        .success,
    ).toBe(false);
    expect(
      BashInputSchema.safeParse({ command: "echo x", timeout: 300_001 })
        .success,
    ).toBe(false);
    expect(
  ...
  it("exposes a default timeout in the JSON Schema", () => {
    ...
    const tool = bashTool(runner);
    const properties = (
      tool.parameters as { properties: Record<string, { default?: number }> }
    ).properties;
  ...
  it("runs through runner.exec, injects cwd, noninteractive env, and closes stdin", async () => {
    ...
    const result = await executeTool(
      tool,
      context({ command: "printf ok", timeout: 60 }),
    );
  ...
  it("uses args.cwd when provided", async () => {
    const { runner, exec } = createTestRunner(
      processWithOutput({ stdout: "sub\n" }),
    );
    const tool = bashTool(runner);

    await executeTool(
      tool,
      context({ command: "pwd", cwd: "/tmp/project", timeout: 60 }),
    );
  ...
  it("uses Git Bash semantics on Windows", async () => {
    ...
    const tool = bashTool(
      runner,
      windowsBashEnv,
      createTestCtx("C:\\Users\\me\\project"),
    );

    const result = await executeTool(
      tool,
      context({ command: "echo ok 2>nul", timeout: 60 }),
    );
  ...
  it("returns stderr and marks non-zero exit codes as tool errors", async () => {
    const { runner } = createTestRunner(
      processWithOutput({ stderr: "boom\n", exitCode: 2 }),
    );
    const tool = bashTool(runner);

    const result = await executeTool(
      tool,
      context({ command: "exit 2", timeout: 60 }),
    );
  ...
  it("returns both stdout and stderr when a command succeeds", async () => {
    const { runner } = createTestRunner(
      processWithOutput({ stdout: "out\n", stderr: "warn\n" }),
    );
    const tool = bashTool(runner);

    const result = await executeTool(
      tool,
      context({ command: "mixed", timeout: 60 }),
    );
  ...
  it("returns both stdout and stderr when a command fails", async () => {
    ...
    const result = await executeTool(
      tool,
      context({ command: "mixed fail", timeout: 60 }),
    );
  ...
  it("returns the service failure reason when foreground process wait rejects", async () => {
    ...
    const result = await executeTool(
      tool,
      context({ command: "wait fails", timeout: 60 }),
    );
  ...
  it("interprets small timeout values as seconds at runtime", async () => {
    ...
    try {
      ...
      const running = executeTool(
        tool,
        context({ command: "sleep 3", timeout: 2 }),
      );
  ...
  it("reports a timed-out command with the timeout message when auto-background is disabled", async () => {
    ...
    try {
      ...
      const running = executeTool(
        tool,
        context({ command: "sleep 2", timeout: 1 }),
      );
  ...
  it("reports a stream read error as a tool error even when the process exits with code 0", async () => {
    ...
    const result = await executeTool(
      tool,
      context({ command: "remote-cmd", timeout: 60 }),
    );
  ...
  it("does not spawn when the signal is already aborted", async () => {
    ...
    const result = await executeTool(
      tool,
      context({ command: "echo nope" }, controller.signal),
    );
  ...
  it("kills the process and returns an abort result when aborted while running", async () => {
    ...
    const running = executeTool(
      tool,
      context({ command: "sleep 10" }, controller.signal),
    );
    await vi.waitFor(() => {
  ...
  it("adds a truncation note when stdout exceeds the cap", async () => {
    ...
    const result = await executeTool(
      tool,
      context({ command: "yes", timeout: 60 }),
    );
  ...
  it("truncates output with the sentinel even when the command fails", async () => {
    ...
    const { runner } = createTestRunner(
      processWithOutput({ stdout: huge, exitCode: 1 }),
    );
    const tool = bashTool(runner);

    const result = await executeTool(
      tool,
      context({ command: "fail-and-flood", timeout: 60 }),
    );
  ...
  it("saves full foreground output when the inline result is truncated", async () => {
    ...
    const { runner } = createTestRunner(
      processWithOutput({ stdout: fullOutput }),
    );
    ...
    const result = await executeTool(
      tool,
      context({ command: "flood", timeout: 60 }),
    );
    const output = result.output as string;
  ...
  it("does not inject GIT_SSH_COMMAND into the spawn environment", async () => {
    ...
    try {
      ...
      expect(Object.prototype.hasOwnProperty.call(env, "GIT_SSH_COMMAND")).toBe(
        false,
      );
    } finally {
  ...
  it("describes timeout behavior according to the auto-background config", () => {
    ...
    expect(autoBg.description).toContain(
    ...
    expect(legacyKillOnTimeout.description).toContain(
      "hits its timeout is killed",
    );
  ...
  it("resolves the detach timeout from the bashTaskTimeoutS config", async () => {
    ...
    await expect(
      detachTimeoutMsFor({ task: { bashTaskTimeoutS: 30 } }),
    ).resolves.toBe(30_000);
    await expect(
      detachTimeoutMsFor({ background: { bashTaskTimeoutS: 45 } }),
    ).resolves.toBe(45_000);
    await expect(
      detachTimeoutMsFor({ task: { bashTaskTimeoutS: 0 } }),
    ).resolves.toBe(0);
  });
});
...
describe("BashTool background mode", () => {
  it("can detach a foreground command through the background service", async () => {
    ...
    const running = executeTool(
      tool,
      context({ command: "sleep 10", timeout: 60 }),
    );
    await vi.waitFor(() => {
    ...
    await vi.waitFor(() => {
      expect(
        (proc.stdout as PassThrough).listenerCount("data"),
    ...
    expect((result as { brief?: string }).brief).toBe(
      `Backgrounded ${task.taskId}`,
    );
    expect(service.getTask(task.taskId)).toMatchObject({ detached: true });
    await vi.waitFor(async () => {
      await expect(service.readOutput(task.taskId)).resolves.toContain(
        "after detach\n",
      );
    });
  ...
  it("moves a timed-out foreground command to the background instead of killing it", async () => {
    ...
    try {
      ...
      const running = executeTool(
        tool,
        context({ command: "sleep 30", timeout: 1 }),
      );
      ...
      expect(service.getTask(taskId!)).toMatchObject({
        status: "running",
        detached: true,
      });
      ...
      await expect(service.readOutput(taskId!)).resolves.toContain(
        "after timeout\n",
      );
    } finally {
      vi.useRealTimers();
  ...
  it("keeps task metadata independent when noisy foreground output is capped before detach", async () => {
    ...
    const running = executeTool(
      tool,
      context({ command: "yes noisy", timeout: 60 }),
    );
    await vi.waitFor(() => {
      expect(service.list(false)).toHaveLength(1);
    });
    ...
    (proc.stdout as PassThrough).write(
      Array.from(
        { length: 6000 },
        (_, index) => `noisy output line ${String(index)}\n`,
      ).join(""),
    );
    service.detach(task.taskId);
    const result = await running;
  ...
  it("registers background commands and returns a task id", async () => {
    ...
    expect(result.output).toContain("automatic_notification: true");
    expect((result as { brief?: string }).brief).toMatch(
      /^Started bash-[0-9a-z]{8}$/,
    );
    expect(result.output).toContain(
  ...
  it("kills a spawned background command when the task limit is reached", async () => {
    ...
    service.registerTask(
      new ProcessTask(processWithOutput(), "sleep 10", "existing task"),
    );
  ...
  it("timeout-stops a background task that has not settled even if process exit is visible", async () => {
    vi.useFakeTimers();
    try {
      const { proc, finishWait, markExited } =
        processWithVisibleExitBeforeWait(0);
      const { runner } = createTestRunner(proc);
      const { service } = createFakeTaskService();
      const tool = bashTool(runner, createTestEnv(), createTestCtx(), service);
...
describe("BashTool prompt / runtime consistency", () => {
  it("reports unavailable background using only tools the prompt documents", async () => {
    ...
    const promptToolNames = new Set(
      [...enabledTool.description.matchAll(/`(Task[A-Za-z]+)`/g)].map(
        (match) => match[1],
      ),
    );
    ...
    const errorToolNames = [
      ...(result.output as string).matchAll(/\b(Task[A-Za-z]+)\b/g),
    ].map((match) => match[1]);
  ...
  it("does not claim failure exit codes appear in a system tag", () => {
    ...
    expect(tool.description).not.toMatch(
      /exit code will be provided in a system tag/,
    );
  });
});
