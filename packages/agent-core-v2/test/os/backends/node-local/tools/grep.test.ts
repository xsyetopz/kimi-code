...
describe("GrepTool", () => {
  ...
  it("exposes current metadata and schema", () => {
    ...
    expect(tool.description).toContain(
      "unknown content or unknown file locations",
    );
    expect(tool.description).toContain(
      "Do not use shell `grep` or `rg` directly",
    );
    ...
    expect(
      GrepInputSchema.safeParse({ pattern: "needle", output_mode: "content" })
        .success,
    ).toBe(true);
    expect(
      GrepInputSchema.safeParse({ pattern: "needle", output_mode: "bad" })
        .success,
    ).toBe(false);
  });
  ...
  describe("output_mode enum value", () => {
    ...
    it("rejects the legacy count value", () => {
      expect(
        GrepInputSchema.safeParse({ pattern: "needle", output_mode: "count" })
          .success,
      ).toBe(false);
    });
  ...
  describe("parameter descriptions", () => {
    ...
    it("mentions count_matches in the output_mode description", () => {
      ...
      expect(params.properties["output_mode"]?.description).toContain(
        "count_matches",
      );
      expect(params.properties["output_mode"]?.description).toContain(
        "per-file",
    ...
    it("describes include_ignored as covering all ignore files, not just .gitignore", () => {
      ...
      const description =
        params.properties["include_ignored"]?.description ?? "";
  ...
  it("can search an additional directory when path is explicit", async () => {
    const exec = vi
      .fn()
      .mockResolvedValue(processWithOutput("/extra/pkg/b.ts\n"));
    const tool = new GrepTool(createFakeKaos({ exec }), workspace);

    const result = await executeTool(
      tool,
      context({ pattern: "hit", path: "/extra" }),
    );
  ...
  it("sorts files_with_matches by mtime before pagination after sensitive filtering", async () => {
    ...
    const tool = new GrepTool(
      createFakeKaos({
        exec: vi.fn().mockResolvedValue(processWithOutput(stdout)),
        stat,
      }),
    ...
    const result = await executeTool(
      tool,
      context({ pattern: "hit", head_limit: 1 }),
    );
  ...
  it("keeps v1 tie order for files modified within the same second", async () => {
    ...
    const result = await executeTool(
      tool,
      context({ pattern: "hit", head_limit: 0 }),
    );

    expect(toolContentString(result)).toBe(
      ["src/c.ts", "src/a.ts", "src/b.ts"].join("\n"),
    );
  });
  ...
  it("limits concurrent mtime stats while sorting files_with_matches", async () => {
    ...
    const tool = new GrepTool(
      createFakeKaos({
        exec: vi
          .fn()
          .mockResolvedValue(processWithOutput(`${filePaths.join("\n")}\n`)),
        stat,
      }),
      { workspaceDir: "/workspace", additionalDirs: [] },
    );
  ...
  it("uses count-matches and ignores context flags outside content output mode", async () => {
    const exec = vi
      .fn()
      .mockResolvedValue(processWithOutput("/workspace/src/a.ts:2\n"));
    const tool = new GrepTool(createFakeKaos({ exec }), workspace);
  ...
  it("retries EAGAIN ripgrep failures with a single-threaded search", async () => {
    const exec = vi
      ...
      .mockResolvedValueOnce(
        processWithOutput(
          "",
          "rg: failed to spawn worker: Resource temporarily unavailable\n",
          2,
        ),
      )
  ...
  it("gives -C precedence over before and after context in content mode", async () => {
    const exec = vi
      .fn()
      .mockResolvedValue(processWithOutput("/workspace/src/a.ts:2:hit\n"));
    const tool = new GrepTool(createFakeKaos({ exec }), workspace);
  ...
  describe("column cap by output mode", () => {
    it("does not cap columns in content output mode so long matching lines are returned in full", async () => {
      const exec = vi
        .fn()
        .mockResolvedValue(processWithOutput("/workspace/src/a.ts:1:hit\n"));
      const tool = new GrepTool(createFakeKaos({ exec }), workspace);

      await executeTool(
        tool,
        context({ pattern: "hit", output_mode: "content" }),
      );
  ...
  it("rejects relative path escapes before spawning ripgrep", async () => {
    ...
    const result = await executeTool(
      tool,
      context({ pattern: "hit", path: "../outside" }),
    );
  ...
  it("appends sensitive prefilter globs after user glob filters", async () => {
    const exec = vi
      .fn()
      .mockResolvedValue(
        processWithOutput(nullRecord("/workspace/src/main.ts")),
      );
    const tool = new GrepTool(createFakeKaos({ exec }), workspace);

    const result = await executeTool(
      tool,
      context({ pattern: "hit", glob: "**/.env" }),
    );
  ...
  it("does not prefilter public key files that the sensitive policy allows", async () => {
    ...
    expect(toolContentString(result)).toContain(
      "id_ed25519.pub:1:ssh-ed25519 hit",
    );
    expect(toolContentString(result)).toContain(
      "id_ecdsa.pub:1:ecdsa-sha2 hit",
    );
  ...
  it("preserves content lines that look like workspace paths when no grep path prefix is present", async () => {
    const exec = vi
      .fn()
      .mockResolvedValue(processWithOutput("/workspace/not-a-path\n"));
    const tool = new GrepTool(createFakeKaos({ exec }), workspace);
  ...
  it("uses null-delimited Windows content paths for sensitive filtering", async () => {
    ...
    expect(toolContentString(result)).toBe(
      [
        "src/main.ts:1:hit",
        "Filtered 1 sensitive file(s): foo-10-/.aws/credentials",
      ].join("\n"),
    );
  });
  ...
  it("explains when every grep result is filtered as sensitive", async () => {
    const stdout = [
      "/workspace/.env:1:SECRET=hit",
      "/workspace/.aws/credentials:2:hit",
      "",
    ].join("\n");
    const exec = vi.fn().mockResolvedValue(processWithOutput(stdout));
    const tool = new GrepTool(createFakeKaos({ exec }), {
  ...
  it("normalizes content separators after filtering sensitive grep results", async () => {
    ...
    expect(content).toBe(
      ["src/main.ts:10:hit", "--", "src/other.ts:7:hit"].join("\n"),
    );
    expect(output).not.toContain("SECRET=hit");
    expect(output).toContain(
      "Filtered 3 sensitive file(s): .env, .aws/credentials, credentials",
    );
  });
  ...
  it("limits grep output to 250 lines by default", async () => {
    const paths = Array.from(
      { length: 251 },
      (_, index) => `/workspace/src/${String(index)}.ts`,
    );
  ...
  it("parses null-delimited files_with_matches output", async () => {
    const stdout =
      nullRecord("/workspace/src/main.ts") + nullRecord("/workspace/.env");
    const tool = new GrepTool(
      createFakeKaos({
        exec: vi.fn().mockResolvedValue(processWithOutput(stdout)),
      }),
  ...
  it("drops incomplete trailing null-delimited content records after timeout", async () => {
    ...
    const resultPromise = executeTool(
      tool,
      context({ pattern: "slow", output_mode: "content" }),
    );
    await vi.advanceTimersByTimeAsync(20_000);
    const result = await resultPromise;
  ...
  it("parses null-delimited count output before sensitive filtering and summary", async () => {
    const stdout =
      [
        nullRecord("/workspace/src/a.ts", "3"),
        nullRecord("/workspace/.env", "99"),
      ].join("\n") + "\n";
    const tool = new GrepTool(
      createFakeKaos({
        exec: vi.fn().mockResolvedValue(processWithOutput(stdout)),
  ...
  it("surfaces ripgrep parse errors with stderr detail", async () => {
    ...
    expect(result.output).toContain(
      "Failed to grep: error: unclosed character class",
    );
  ...
  it("surfaces ripgrep failures even when stderr is empty", async () => {
    ...
    expect(result).toEqual({
      isError: true,
      output: "Failed to grep: ripgrep exited with code 2",
    });
  });
  ...
  it("marks ripgrep stderr as truncated when error output exceeds the cap", async () => {
    ...
    expect(result).toMatchObject({ isError: true });
    expect(result.output).toContain(
      "Failed to grep: error: very large failure",
    );
  ...
  it("returns a locator error when ripgrep cannot be resolved", async () => {
    ...
    expect(result).toEqual({
      isError: true,
      output: "rg unavailable: download failed",
    });
    expect(exec).not.toHaveBeenCalled();
  });
  ...
  it("returns an install hint when spawning the resolved ripgrep path hits ENOENT", async () => {
    const error = Object.assign(new Error("spawn /mock/rg ENOENT"), {
      code: "ENOENT",
    });
  ...
  it("aborts while resolving the ripgrep path without spawning", async () => {
    ...
    const exec = vi.fn();
    vi.mocked(ensureRgPath).mockImplementationOnce(
      (_probe, { signal: locatorSignal } = {}) => {
    ...
    const resultPromise = executeTool(
      tool,
      context({ pattern: "hit" }, controller.signal),
    );
    controller.abort();
  ...
  it("appends the count-mode summary and pagination to the model-visible output", async () => {
    ...
    expect(output).toContain(
      "Results truncated to 3 lines (total: 10). Use offset=3 to see more.",
    );
  });
  ...
  it("returns content when searching a single file path", async () => {
    const exec = vi
      .fn()
      .mockResolvedValue(
        processWithOutput("/workspace/target.py:1:hello world\n"),
      );
    const tool = new GrepTool(createFakeKaos({ exec }), workspace);
  ...
  it("returns a clean no-match result when offset exceeds total entries", async () => {
    const exec = vi
      .fn()
      .mockResolvedValue(processWithOutput("/workspace/only.txt\n"));
    const tool = new GrepTool(createFakeKaos({ exec }), workspace);
  ...
  it("emits line numbers by default in content mode", async () => {
    const exec = vi
      .fn()
      .mockResolvedValue(processWithOutput("/workspace/a.txt:1:hello\n"));
    const tool = new GrepTool(createFakeKaos({ exec }), workspace);

    await executeTool(
      tool,
      context({ pattern: "hello", output_mode: "content" }),
    );
  ...
  it('drops the line-number column when "-n" is explicitly false', async () => {
    const exec = vi
      .fn()
      .mockResolvedValue(processWithOutput("/workspace/a.txt:hello\n"));
    const tool = new GrepTool(createFakeKaos({ exec }), workspace);
  ...
  it("maps schema flags onto ripgrep equivalents and tilde-expands ~ in path", async () => {
    const exec = vi
      .fn()
      .mockResolvedValue(processWithOutput("/workspace/a.ts:1:hello\n"));
    const tool = new GrepTool(createFakeKaos({ exec }), workspace);
  ...
  it("strips workspace prefix from rg output across record kinds (posix)", async () => {
    ...
    const result = await executeTool(
      tool,
      context({ pattern: "code", output_mode: "content" }),
    );
  ...
  it("treats a trailing-slash workspace dir the same as one without", async () => {
    const withSep = new GrepTool(
      createFakeKaos({
        exec: vi
          .fn()
          .mockResolvedValue(processWithOutput("/tmp/dir/file.py\n")),
      }),
  ...
  it("does not strip a workspace dir prefix when it would match a sibling name", async () => {
    ...
    expect(output.split("\n")).toEqual(
      expect.arrayContaining(["file.py", "/tmp/abc/file.py"]),
    );
  });
  ...
  it("locks the grep description to ripgrep-tip phrasing about hidden files and include_ignored", () => {
    ...
    expect(tool.description).toMatch(
      /ALWAYS use Grep tool instead of running `grep` or `rg`/,
    );
  });
  ...
  it("returns an abort error without spawning when the signal is already aborted", async () => {
    ...
    expect(result).toEqual({
      isError: true,
      output: "Aborted before search started",
    });
    expect(exec).not.toHaveBeenCalled();
  });
});
