...
import {
  type WriteInput,
  WriteInputSchema,
} from "#/agent/tools/os/write/write";
...
function createWriteFs(options: WriteFsOptions = {}) {
  ...
  const stat = vi.fn(
    options.stat ??
      (async () => ({ isFile: false, isDirectory: true, size: 0 })),
  );
...
function makeTool(
  ...
  workspace = PERMISSIVE_WORKSPACE,
) {
...
describe("WriteTool", () => {
  it("exposes current metadata and schema", () => {
    ...
    expect(tool.description).toContain(
      "append adds content at EOF without adding a newline",
    );
    expect(tool.description).toContain("\\n stays LF, \\r\\n stays CRLF");
    expect(tool.description).toContain(
      "Write is NOT ALLOWED for incremental changes",
    );
    ...
    expect(
      WriteInputSchema.safeParse({ path: "/tmp/out.txt", content: "hello" })
        .success,
    ).toBe(true);
    expect(
    ...
    expect(WriteInputSchema.safeParse({ path: "/tmp/out.txt" }).success).toBe(
      false,
    );
  });
  ...
  it("matches permission args with negated glob path semantics", () => {
    ...
    const insideSrc = tool.resolveExecution({
      path: "./src/a.ts",
      content: "x",
    });
    const outsideSrc = tool.resolveExecution({
      path: "./README.md",
      content: "x",
    });
  ...
  it("guides batching large content across multiple write calls", () => {
    ...
    expect(tool.description).toMatch(
      /overwrite[^.]*first chunk[^.]*then[^.]*append/i,
    );
  });
  ...
  it("expands leading tilde paths using the kaos home directory", async () => {
    ...
    expect(fakes.writeText).toHaveBeenCalledWith(
  ...
  it("creates missing parent directories automatically before writing", async () => {
    ...
    const { tool, mkdir, writeText } = makeTool({
      stat: vi.fn().mockRejectedValue(enoent),
    });

    const result = await execute(tool, {
      path: "/tmp/missing-dir/file.txt",
      content: "data",
    });
  ...
  it("rejects writing when the parent path is not a directory", async () => {
    const { tool, writeText } = makeTool({
      stat: vi
        .fn()
        .mockResolvedValue({ isFile: true, isDirectory: false, size: 0 }),
    });

    const result = await execute(tool, {
      path: "/tmp/a-file/child.txt",
      content: "data",
    });
  ...
  it("writes when the parent directory exists", async () => {
    const { tool, writeText } = makeTool({
      stat: vi
        .fn()
        .mockResolvedValue({ isFile: false, isDirectory: true, size: 0 }),
    });
  ...
  it("allows absolute writes to a sibling dir that merely shares the work-dir prefix", async () => {
    ...
    const result = await execute(tool, {
      path: "/workspace-sneaky/file.txt",
      content: "content",
    });

    expect(result.isError).toBeFalsy();
    expect(writeText).toHaveBeenCalledWith(
      "/workspace-sneaky/file.txt",
      "content",
    );
  });
});
