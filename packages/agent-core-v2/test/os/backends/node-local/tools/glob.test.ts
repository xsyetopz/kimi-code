...
vi.mock("#/os/backends/node-local/tools/rgLocator", () => ({
  ensureRgPath: vi.fn(
    async (): Promise<{ path: string; source: string }> => ({
      path: "rg",
      source: "system-path",
    }),
  ),
...
function createTestFs(
  ...
  const readdir =
    opts.readdir ?? vi.fn(async (): Promise<readonly string[]> => []);
...
function createTestEnv(
  opts: { home?: string; pathClass?: PathClass } = {},
): IHostEnvironment {
  return {
    _serviceBrand: undefined,
    osKind: "Linux",
...
function createTestProcessService(
...
function makeTool(
  ...
  tool: GlobTool;
  exec: ReturnType<typeof vi.fn>;
  withCwd: ReturnType<typeof withCwdOf>;
} {
...
describe("GlobTool", () => {
  ...
  it("retries once single-threaded when rg fails with EAGAIN (os error 11)", async () => {
    const exec = vi
      ...
      .mockResolvedValueOnce(
        fakeProcess(
          "",
          "rg: thread pool: Resource temporarily unavailable (os error 11)",
          2,
        ),
      )
    ...
    const retryArgs = (
      exec.mock.calls[1] as ReadonlyArray<unknown>
    )[1] as string[];
  ...
  it("mentions Windows path forms in the description on win32 backends", () => {
    const { tool } = makeTool(stubWorkspaceContext("C:\\workspace"), {
      pathClass: "win32",
    });
  ...
  beforeAll(async () => {
    try {
      ...
      const resolution = await actual.ensureRgPath(
        createRealRgProbe(probeProcessService),
        {
