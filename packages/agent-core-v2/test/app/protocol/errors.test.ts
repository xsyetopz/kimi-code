...
describe("translateProviderError", () => {
  ...
  it("maps 401 / 403 to provider.auth_error", () => {
    expect(
      translateProviderError(new APIStatusError(401, "Unauthorized")).code,
    ).toBe("provider.auth_error");
    expect(
      translateProviderError(new APIStatusError(403, "Forbidden")).code,
    ).toBe("provider.auth_error");
  });

  it("maps other status codes to provider.api_error", () => {
    expect(translateProviderError(new APIStatusError(500, "oops")).code).toBe(
      "provider.api_error",
    );
  });
  ...
  it("maps provider-overload errors to provider.overloaded at birth, keeping HTTP details", () => {
    const raw = new APIProviderOverloadedError(
      529,
      "Overloaded",
      "req-overload",
    );
  ...
  it("maps other empty responses to provider.api_error", () => {
    expect(
      translateProviderError(new APIEmptyResponseError("empty")).code,
    ).toBe("provider.api_error");
  });

  it("maps a plain ChatProviderError to provider.api_error", () => {
    expect(translateProviderError(new ChatProviderError("bad")).code).toBe(
      "provider.api_error",
    );
  });
  ...
  describe("message sanitization", () => {
    it("extracts the <title> from an nginx 413 HTML body and strips CR", () => {
      const error = translateProviderError(
        new APIStatusError(413, NGINX_413_HTML),
      );
      expect(error.code).toBe("provider.api_error");
    ...
    it("extracts the <title> from other nginx HTML error pages", () => {
      ...
      expect(
        translateProviderError(new APIStatusError(502, html)).message,
      ).toBe("502 Bad Gateway");
    });

    it("leaves a plain-text message unchanged", () => {
      expect(
        translateProviderError(new APIStatusError(500, "Internal Server Error"))
          .message,
    ...
    it("strips carriage returns from a non-HTML message", () => {
      expect(
        translateProviderError(new APIStatusError(500, "line1\r\nline2\r"))
          .message,
      ).toBe("line1\nline2");
    });
    ...
    it("falls back to the original message when the <title> is empty", () => {
      ...
      expect(
        translateProviderError(new APIStatusError(500, html)).message,
      ).toContain("<html>");
    });
