...
describe("FileSkillDiscovery", () => {
  ...
  function skillRoot(
    rel: string,
    source: SkillRoot["source"] = "project",
  ): SkillRoot {
  ...
  it("dedupes same-named skills across roots from the same plugin", async () => {
    await writeSkill(
      "plugin-a-first/dup/SKILL.md",
      "name: DUP\ndescription: from first root",
    );
    await writeSkill(
      "plugin-a-second/dup/SKILL.md",
  ...
  it("preserves same-named skills from different plugins", async () => {
    await writeSkill(
      "plugin-a/dup/SKILL.md",
      "name: DUP\ndescription: from plugin A",
    );
    await writeSkill(
      "plugin-b/dup/SKILL.md",
  ...
  it("discovers sub-skills of a parent that opts in", async () => {
    ...
    expect(
      result.skills.find((s) => s.name === "parent.child")?.metadata.isSubSkill,
    ).toBe(true);
  });
  ...
  it("discovers nested SKILL.md files when has-sub-skill is nested under metadata", async () => {
    ...
    expect(
      result.skills.find((s) => s.name === "outer.child")?.metadata.isSubSkill,
    ).toBe(true);
  });
