  ...
  ix.set(
    ...
    new StubConfigService(
      configured === undefined
        ? {}
        : { [BUILTIN_PRODUCT_SKILLS_SECTION]: configured },
    ),
  );
...
describe("BuiltinSkillSource product-skill switch", () => {
  ...
  it("exposes the same filter the session-less listings compose with", () => {
    ...
    expect(visibleBuiltinSkills(false).map((s) => s.name)).toEqual(
      NEUTRAL_SKILLS,
    );
  });
  ...
  it("signals a change when the switch is toggled", async () => {
    const config = new StubConfigService({
      [BUILTIN_PRODUCT_SKILLS_SECTION]: true,
    });
    const ix = new TestInstantiationService();
    ...
    expect(fired).toBe(1);
    expect((await source.load()).skills.map((s) => s.name)).toEqual(
      NEUTRAL_SKILLS,
    );
