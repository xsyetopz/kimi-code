...
describe("skillRoots", () => {
  ...
  describe("projectRoots", () => {
    it("resolves the brand .kimi-code/skills directory at the .git root", async () => {
      ...
      expect(
        roots.some(
          (r) => r.path.endsWith(".kimi-code/skills") && r.source === "project",
        ),
      ).toBe(true);
    });
    ...
    it("falls back to the generic .agents/skills directory", async () => {
      ...
      expect(
        roots.some(
          (r) => r.path.endsWith(".agents/skills") && r.source === "project",
        ),
      ).toBe(true);
      expect(roots.some((r) => r.path.endsWith(".kimi-code/skills"))).toBe(
        false,
      );
    });
    ...
    it("orders the brand directory before the generic directory", async () => {
      ...
      const brandIdx = roots.findIndex((r) =>
        r.path.endsWith(".kimi-code/skills"),
      );
      const genericIdx = roots.findIndex((r) =>
        r.path.endsWith(".agents/skills"),
      );
  ...
  describe("userRoots", () => {
    it("resolves the brand skills directory under homeDir", async () => {
      ...
      expect(
        roots.some((r) => r.path.endsWith("/skills") && r.source === "user"),
      ).toBe(true);
    });
    ...
    it("falls back to the generic .agents/skills under osHomeDir", async () => {
      ...
      expect(
        roots.some(
          (r) => r.path.endsWith(".agents/skills") && r.source === "user",
        ),
      ).toBe(true);
    });
  });
