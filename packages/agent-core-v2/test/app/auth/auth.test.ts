...
describe("OAuthService", () => {
  ...
  beforeEach(() => {
    ...
    toolkit = {
      ...
      logout: vi
        .fn()
        .mockResolvedValue({ providerName: OAUTH_PROVIDER, ok: true }),
    ...
    ix = createServices(disposables, {
      ...
      additionalServices: (reg) => {
        ...
        reg.definePartialInstance(IConfigService, {
          get: ((domain: string) =>
            configBacking()[domain]) as IConfigService["get"],
          ...
          reload: vi
            ...
            .mockResolvedValue(
              undefined,
            ) as unknown as IConfigService["reload"],
  ...
  it("startLogin rejects when the device authorization fails before onDeviceCode", async () => {
    toolkit.login.mockRejectedValue(
      new Error("device authorization request failed"),
    );
    const svc = createService();
  ...
  it("startLogin returns authenticated when model refresh fails on the already-authenticated fast path", async () => {
    ...
    expect(configReplace).not.toHaveBeenCalledWith(
      "defaultModel",
      expect.any(String),
    );
  });
  ...
  it("aborts an in-flight OAuth flow when its provider is removed from config", async () => {
    ...
    providerChangedEmitter.fire({
      added: [],
      removed: [OAUTH_PROVIDER],
      changed: [],
    });

    const flow = svc.getFlow(OAUTH_PROVIDER);
    expect(flow?.status).toBe("cancelled");
    expect(flow?.error_message).toBe(
      "Provider configuration changed during login.",
    );
  });
  ...
  it("logout surfaces managed provider cleanup write failures", async () => {
    ...
    await expect(svc.logout(OAUTH_PROVIDER)).rejects.toThrow(
      "config write failed",
    );
    expect(toolkit.logout).toHaveBeenCalledWith(
      OAUTH_PROVIDER,
  ...
  it("resolveTokenProvider re-derives the managed provider oauth ref from the current base url", () => {
    const svc = createService();
    svc.resolveTokenProvider(OAUTH_PROVIDER, {
      storage: "file",
      key: "stale-key",
    });
    ...
    expect(toolkit.tokenProvider).toHaveBeenCalledWith(
      OAUTH_PROVIDER,
      expectedRef,
    );
  });
  ...
  it("getManagedUserInfo resolves the managed runtime auth and delegates to the toolkit", async () => {
    ...
    await expect(svc.getManagedUserInfo(OAUTH_PROVIDER)).resolves.toBe(
      userInfo,
    );
...
describe("WebSearchProviderService", () => {
  ...
  it("prefers the services.moonshot_search config over the managed oauth provider", async () => {
    servicesConfig = {
      moonshotSearch: {
        baseUrl: "https://config.example.com/search",
        apiKey: "config-key",
      },
    };
    providers = {
...
describe("AuthSummaryService", () => {
    ...
    ix = createServices(disposables, {
      additionalServices: (reg) => {
        ...
        reg.definePartialInstance(IModelService, {
          ...
          getDefaultModel: (() =>
            defaultModel) as IModelService["getDefaultModel"],
        });
        reg.definePartialInstance(IConfigService, {
          get: ((domain: string) => {
  ...
  it("ensureReady propagates cached token read failures", async () => {
    ...
    await expect(createSummary().ensureReady()).rejects.toThrow(
  ...
  it("ensureReady accepts provider api keys", async () => {
    await expect(
      createSummary().ensureReady("openai"),
