...
describe("AgentIdentityService", () => {
  ...
  it.each([
    ...
    { slug: "" },
    { name: "", slug: "  " },
  ])("treats blank config values as unset: %j", async (section) => {
  ...
  it.each(["", "   "])(
    "treats a blank host display name as unset: %j",
    async (hostName) => {
  ...
  it("trims a padded host display name", async () => {
    expect((await resolve(undefined, "  Embedding Host  ")).displayName).toBe(
      "Embedding Host",
    );
  });
...
describe("buildAgentIdentitySnapshot products", () => {
  const HOST = {
    "User-Agent": "kimi-code-cli/1.2.3 (darwin)",
  ...
  it.each([
    ...
    "outboundUserAgent for host %j and slug %j is %j",
    (headers, slug, expected) => {
      expect(
        buildAgentIdentitySnapshot({ slug, hostRequestHeaders: headers })
          .outboundUserAgent,
      ).toBe(expected);
    },
  );
