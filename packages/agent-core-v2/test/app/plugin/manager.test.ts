 ...
 * Run: bun --filter @moonshot-ai/agent-core-v2 exec vitest run test/app/plugin/manager.test.ts
 */
  ...
  it("installs a github plugin through codeload", async () => {
    ...
    try {
      ...
      const stored = JSON.parse(
        ...
        plugins: Array<{ id: string; github?: { installedSha?: string } }>;
      };
      expect(
        stored.plugins.find((plugin) => plugin.id === "github-plugin")?.github
          ?.installedSha,
      ).toBe("1111111111111111111111111111111111111111");
      expect(manager.get("github-plugin")?.manifest?.name).toBe(
        "github-plugin",
      );
    } finally {
      await rm(sourceRoot, { recursive: true, force: true });
  ...
  it("checks github plugin updates against latest release", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn().mockResolvedValue({
        ...
        ok: false,
        headers: new Headers({
          location: "https://github.com/owner/repo/releases/tag/v2",
        }),
      }),
    );
    const manager = new PluginManager({ kimiHomeDir: home });
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: Parameters<typeof fetch>[0]) => {
        ...
        return new Response(null, {
          status: 302,
          headers: {
            location: "https://github.com/owner/good/releases/tag/v2",
          },
        });
      }) as typeof fetch,
    );
