...
async function persistedPluginIds(homeDir: string): Promise<readonly string[]> {
  ...
  return (JSON.parse(contents) as InstalledFile).plugins.map(
    (plugin) => plugin.id,
  );
}
...
describe("PluginService (plugin boundary)", () => {
  ...
  it("keeps the first load failure latched after the file is fixed", async () => {
    ...
    try {
      ...
      await writeInstalledFile(
        home,
        JSON.stringify(installedFile("recovery-demo", pluginRoot)),
      );

      await expect(svc.listPlugins()).rejects.toMatchObject({
  ...
  it("recovers the management plane through an explicit reload after the file is fixed", async () => {
    ...
    try {
      ...
      expect(reloads).toEqual([
        { added: ["recovery-demo"], removed: [], errors: [] },
      ]);
    } finally {
      host.dispose();
    }
  ...
  it("serves enabled plugin system-prompt sections on the consumption plane", async () => {
    const home = await makeHome();
    const pluginRoot = await makePluginDir("prompt-demo", {
      systemPrompt: "Always cite sources.",
    });
    createdDirs.push(pluginRoot);
    await writeInstalledFile(
      home,
      JSON.stringify(installedFile("prompt-demo", pluginRoot)),
    );
    const host = makeHost(home);
    try {
      const svc = host.app.accessor.get(IPluginService);
  ...
  it("keeps the last valid consumption snapshot after a reload failure", async () => {
    const home = await makeHome();
    const pluginRoot = await makePluginDir("stable-demo", {
      skills: "./skills/",
    });
    createdDirs.push(pluginRoot);
    ...
    await writeInstalledFile(
      home,
      JSON.stringify(installedFile("stable-demo", pluginRoot)),
    );
    const host = makeHost(home);
    try {
      const svc = host.app.accessor.get(IPluginService);
      await expect(svc.pluginSkillRoots()).resolves.toEqual([
        expect.objectContaining({
          plugin: expect.objectContaining({ id: "stable-demo" }),
        }),
      ]);

      await writeInstalledFile(home, "{ not json");
      await expect(svc.reloadPlugins()).rejects.toMatchObject({
        code: "plugin.load_failed",
      });
      await expect(svc.listPlugins()).rejects.toMatchObject({
        code: "plugin.load_failed",
  ...
  it("uses one initial plugin snapshot for concurrent readers", async () => {
    ...
    const pluginRoot = await makePluginDir("snapshot-demo", {
      skills: "./skills/",
    });
    createdDirs.push(pluginRoot);
    await mkdir(path.join(pluginRoot, "skills"));
    readInstalled.mockImplementationOnce(async () =>
      installedFile("snapshot-demo", pluginRoot),
    );
    const host = makeHost(home);
    try {
      const svc = host.app.accessor.get(IPluginService);
      ...
      expect(plugins).toEqual([
        expect.objectContaining({ id: "snapshot-demo" }),
      ]);
  ...
  it("waits for a pending install before reading managed plugin files", async () => {
    ...
    try {
      ...
      const install = svc.installPlugin({
        source: "https://downloads.example.test/plugin.zip",
      });
      ...
      downloadResponse.resolve(
        new Response("not a zip archive", { status: 200 }),
      );
  ...
  it("restores the previous managed copy when reinstall persistence fails", async () => {
    ...
    try {
      ...
      await expect(svc.installPlugin({ source: nextSource })).rejects.toThrow(
        "persist failed",
      );
      ...
      await expect(
        readdir(path.join(home, "plugins", "managed")),
  ...
  it("does not block consumption reads while an update check is pending", async () => {
    const home = await makeHome();
    const pluginRoot = await makePluginDir("github-demo", {
      skills: "./skills/",
    });
    createdDirs.push(pluginRoot);
    await mkdir(path.join(pluginRoot, "skills"));
    await writeInstalledFile(
      home,
      JSON.stringify(githubInstalledFile("github-demo", pluginRoot)),
    );
    ...
    try {
      ...
      await expect(svc.pluginSkillRoots()).resolves.toEqual([
        expect.objectContaining({
          plugin: expect.objectContaining({ id: "github-demo" }),
        }),
      ]);
      ...
      lookupResponse.resolve(
        new Response(null, {
          ...
          headers: {
            location:
              "https://github.com/example/github-demo/releases/tag/v2.0.0",
          },
        }),
      );
  ...
  it("keeps a queued removal when reload is already reading the installed file", async () => {
    ...
    await writeInstalledFile(
      home,
      JSON.stringify(installedFile("demo", pluginRoot)),
    );
    const host = makeHost(home);
    try {
      const svc = host.app.accessor.get(IPluginService);
  ...
  it("injects the managed Kimi endpoint env into stdio plugin MCP servers only", async () => {
    ...
    try {
      ...
      expect(JSON.stringify(servers["plugin-demo:docs"])).not.toContain(
        "KIMI_CODE_BASE_URL",
      );
    } finally {
      host.dispose();
    ...
    try {
      ...
      providerConfigs[KIMI_CODE_PROVIDER_NAME] = {
        ...
        oauth: {
          storage: "file",
          key: "kimi",
          oauthHost: "https://auth.ready.example.test",
        },
      };
  ...
  it("does not inject managed env when neither env nor the kimi provider supplies it", async () => {
    ...
    try {
      ...
      const servers = await svc.enabledMcpServers();
      const env =
        (servers["plugin-demo:finance"] as { env?: Record<string, string> })
          .env ?? {};
