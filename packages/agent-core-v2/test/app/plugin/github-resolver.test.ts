 ...
 * Run: bun --filter @moonshot-ai/agent-core-v2 exec vitest run test/app/plugin/github-resolver.test.ts
 */
...
import {
  ...
  resolveGithubSource,
} from "#/app/plugin/github-resolver";
  ...
  it("resolves a branch head from the GitHub commit feed", async () => {
    ...
    vi.stubGlobal(
      ...
      vi
        ...
        .mockResolvedValue(
          new Response(
            `<entry><id>tag:github.com,2008:Grit::Commit/${sha}</id></entry>`,
          ),
        ),
    );

    await expect(
      resolveGithubCommitSha("owner", "repo", "feature/demo"),
    ).resolves.toBe(sha);
