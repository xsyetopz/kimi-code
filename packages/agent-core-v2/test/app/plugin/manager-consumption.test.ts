...
async function zipTempLeftovers(dir: string): Promise<readonly string[]> {
  return (await readdir(dir)).filter((entry) =>
    entry.startsWith("kimi-plugin-zip-"),
  );
}
...
async function managedPluginRoot(
...
function mockGithubFetch(options: MockGithubFetchOptions): void {
  ...
  vi.stubGlobal(
    ...
    vi.fn(async (input: Parameters<typeof fetch>[0], init?: RequestInit) => {
      ...
      if (
        /^https:\/\/github\.com\/[^/]+\/[^/]+\/commits\/.+\.atom$/.test(url)
      ) {
        return new Response(
...
describe("PluginManager consumption plane", () => {
  ...
  it("counts nested sub-skills discovered through has-sub-skill bundles", async () => {
    ...
    await mkdir(path.join(root, "skills", "parent", "child"), {
      recursive: true,
    });
    await writeFile(
  ...
  it("removes the zip temp dir and reports the original source when a zip plugin has no manifest", async () => {
    ...
    const sourceRoot = await mkdtemp(
      path.join(tmpdir(), "plugin-no-manifest-"),
    );
    await writeFile(
  ...
  it("reports the GitHub URL when a GitHub plugin tarball has no manifest", async () => {
    const home = await makeKimiHome();
    const sourceRoot = await mkdtemp(
      path.join(tmpdir(), "plugin-gh-no-manifest-"),
    );
    await writeFile(
      path.join(sourceRoot, "README.md"),
      "no manifest here",
    ...
    mockGithubFetch({
      releaseTag: "v1.0.0",
      tarball: await zipDir(sourceRoot),
    });
    const manager = new PluginManager({ kimiHomeDir: home });
    await manager.load();
  ...
  it("reports the real local path when a local-path plugin has no manifest", async () => {
    ...
    expect(message).toContain(
      `Cannot install plugin at ${await realpath(sourceRoot)}`,
    );
    await rm(sourceRoot, { recursive: true, force: true });
  });
  ...
  it("uses unambiguous runtime names for plugin MCP servers", async () => {
    ...
    const first = await makePlugin("a-b", {
      mcpServers: { c: { command: "first-mcp" } },
    });
    const second = await makePlugin("a", {
      mcpServers: { "b-c": { command: "second-mcp" } },
    });
    const manager = new PluginManager({ kimiHomeDir: home });
    await manager.load();
  ...
  it("install() from github /tree/<branch> bypasses the GitHub API", async () => {
    ...
    mockGithubFetch({
      tarball: zipBuffer,
      onReleaseLookup: () => releaseLookups++,
    });
