...
describe("plugin command parser", () => {
  ...
  it("loads a command file and returns undefined for missing files", async () => {
    ...
    await expect(
      loadPluginCommand({ commandPath, pluginId: "demo" }),
    ...
    await expect(
      loadPluginCommand({
        commandPath: join(dir, "missing.md"),
        pluginId: "demo",
      }),
  ...
  it("expands $ARGUMENTS and appends args when no placeholder exists", () => {
    expect(expandCommandArguments("deploy $ARGUMENTS now", "prod")).toBe(
      "deploy prod now",
    );
    expect(expandCommandArguments("deploy now", "prod")).toBe(
      "deploy now\n\nARGUMENTS: prod",
    );
