...
describe("CapabilityService", () => {
  ...
  it("isolates a failing detector to its own entry", async () => {
    ...
    const service = fakeService([
      ...
      fakeEntry({
        id: "kimi-webbridge",
        detect: { steps: [{ id: "daemon", state: "ok" }] },
      }),
    ]);
    ...
    expect(cu?.steps).toEqual([
      { id: "detect", state: "failed", detail: "probe timed out" },
    ]);
  });
  ...
  it("reports not_installed when no step is ok, and unsupported as-is", async () => {
    const service = fakeService([
      fakeEntry({
        id: "kimi-cu",
        detect: { steps: [{ id: "plugin", state: "missing" }] },
      }),
  ...
  it("rejects install on an unsupported entry", async () => {
    const service = fakeService([
      fakeEntry({ id: "kimi-cu", supported: false }),
    ]);
