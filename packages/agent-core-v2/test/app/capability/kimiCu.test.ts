...
function fakeHostProcess(
  ...
  const service: IHostProcessService = {
    ...
    spawn: (command: string, args: readonly string[] = []) => {
      ...
      return Promise.resolve(
        fakeProc(hit?.code ?? 0, hit?.stdout ?? "", hit?.stderr ?? ""),
      );
    },
...
function fakePlugins(
  ...
  const service = {
    ...
    setPluginMcpServerEnabled: (input: {
      id: string;
      server: string;
      enabled: boolean;
    }) => {
...
describe("parsePermissionStatus", () => {
  it("parses the machine-readable request-permissions output", () => {
    ...
    expect(
      parsePermissionStatus(
        "permissionStatus: accessibility=false screenRecording=true",
      ),
    ).toEqual({
      accessibility: false,
      screenRecording: true,
...
describe("elevatedDittoScript", () => {
  it("shell-quotes both paths so spaces and metacharacters stay literal", () => {
    ...
    expect(
      elevatedDittoScript("/tmp/kimi cu/app", "/Applications/KimiCU.app"),
    ).toBe("/usr/bin/ditto '/tmp/kimi cu/app' '/Applications/KimiCU.app'");
    const script = elevatedDittoScript(
      "$(touch /tmp/pwned); echo '",
      "/Applications/KimiCU.app",
    );
    expect(script).toBe(
...
<string>0.4.18</string>
  ...
  it("returns undefined for a missing file", async () => {
    expect(
      await readAppBundleVersion(path.join(root, "nope.plist")),
    ).toBeUndefined();
  });
});
...
describe("kimi-cu entry", () => {
  ...
  function makeCtx(
    overrides: Partial<CapabilityEntryContext> = {},
  ...
  it("is supported only on macOS", () => {
    ...
    expect(createKimiCuEntry(makeCtx({ platform: "linux" })).supported).toBe(
      false,
    );
    expect(createKimiCuEntry(makeCtx({ platform: "win32" })).supported).toBe(
      false,
    );
  });
  ...
  it("detects all four layers with details", async () => {
    ...
    const host = fakeHostProcess([
      {
        ...
        code: 0,
        stdout:
          "SMAppService status=1 (1=enabled); fallback plist exists=false",
      },
      {
        match: "xpc-ping",
        code: 0,
    ...
    expect(
      host.calls.some((call) => call.includes("request-permissions")),
    ).toBe(false);
  });

  it("reports missing layers on a bare machine", async () => {
    const entry = createKimiCuEntry(
      makeCtx({ applicationsDir: path.join(root, "Applications") }),
    );
    const detected = await entry.detect();
  ...
  it("rejects install on non-macOS before any side effect", async () => {
    const plugins = fakePlugins([]);
    const entry = createKimiCuEntry(
      makeCtx({ platform: "linux", plugins: plugins.service }),
    );
  ...
  it("resumes a partial install without repeating completed runtime layers", async () => {
    ...
    const entry = createKimiCuEntry(
      makeCtx({
        ...
        hostProcess: host.service,
        fetchImpl: (() =>
          Promise.reject(new Error("download should be skipped"))) as never,
      }),
    );
    ...
    expect(
      host.calls.every(
        (call) => call.includes("service-status") || call.includes("xpc-ping"),
      ),
    ).toBe(true);
  });
  ...
  it("re-enables a previously disabled wiring plugin during setup", async () => {
    const applicationsDir = await fakeAppBundle();
    const plugins = fakePlugins([
      { id: "kimi-cu", enabled: false, state: "ok", version: "0.5.4" },
    ]);
    const host = fakeHostProcess([
      { match: "service-status", code: 0, stdout: "SMAppService status=1" },
      {
  ...
  it("re-enables disabled MCP servers during setup", async () => {
    ...
    await entry.install(() => {});
    expect(plugins.mcpEnabledCalls).toEqual([
      { id: "kimi-cu", server: "mac", enabled: true },
    ]);
  });
  ...
  it("reads a non-executable leftover app binary as a broken install", async () => {
    ...
    await chmod(
      path.join(applicationsDir, "KimiCU.app", "Contents", "MacOS", "kimi-cu"),
      0o644,
    );
    const entry = createKimiCuEntry(makeCtx({ applicationsDir }));
