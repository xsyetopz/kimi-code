...
function fakeFetch(opts: {
  ...
  const fetchImpl = (async (url: string | URL): Promise<Response> => {
    ...
    if (u === `${DAEMON_BASE}/status`) {
      const step =
        opts.statusSequence?.[
          Math.min(statusCalls, (opts.statusSequence?.length ?? 1) - 1)
        ];
      statusCalls += 1;
...
describe("kimi-webbridge entry", () => {
  ...
  it("detects a fully installed daemon with extension as soft gate", async () => {
    ...
    await mkdir(path.join(userHome, ".kimi-webbridge", "bin"), {
      recursive: true,
    });
    ...
    const { fetchImpl } = fakeFetch({
      statusSequence: [
        { running: true, version: "v1.11.3", extension_connected: false },
      ],
    });
    const entry = createKimiWebbridgeEntry(
      makeCtx({ plugins: plugins.service, fetchImpl }),
    );
  ...
  it("reports user skill shadows for manual cleanup without deleting them", async () => {
    ...
    const userHome = path.join(root, "user-home");
    await mkdir(path.join(kimiHome, "skills", "kimi-webbridge"), {
      recursive: true,
    });
    await writeFile(
      path.join(kimiHome, "skills", "kimi-webbridge", "SKILL.md"),
      "old",
    );
    await mkdir(path.join(userHome, ".agents", "skills", "kimi-webbridge"), {
      recursive: true,
    });
    await writeFile(
      path.join(userHome, ".agents", "skills", "kimi-webbridge", "SKILL.md"),
      "old",
    );
    const plugins = fakePlugins([
      { id: "kimi-webbridge", enabled: true, state: "ok" },
    ]);
    const { fetchImpl } = fakeFetch({
      statusSequence: [
  ...
  it("installs end-to-end: download, start-if-down, and plugin wiring", async () => {
    ...
    expect(host.calls.map((c) => `${c.command} ${c.args.join(" ")}`)).toEqual([
      `${binPath} start`,
    ]);
  ...
  it("re-enables a previously disabled wiring plugin during setup", async () => {
    ...
    await entry.install(() => {});
    expect(plugins.enabledCalls).toEqual([
