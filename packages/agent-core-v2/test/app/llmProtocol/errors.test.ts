...
describe("APIContextOverflowError", () => {
  it("extends APIStatusError and preserves HTTP details", () => {
    const err = new APIContextOverflowError(
      400,
...
describe("APIProviderOverloadedError", () => {
  it("extends APIStatusError and preserves HTTP details", () => {
    const err = new APIProviderOverloadedError(
      529,
      "Overloaded",
      "req-overload",
    );
...
describe("APIRequestTooLargeError", () => {
  it("extends APIStatusError and preserves HTTP details", () => {
    const err = new APIRequestTooLargeError(
      413,
...
describe("isRetryableGenerateError", () => {
  it("matches transient provider errors and empty generate responses", () => {
    ...
    expect(isRetryableGenerateError(new APIEmptyResponseError("empty"))).toBe(
      true,
    );
  });
  ...
  it.each([408, 409, 429, 500, 502, 503, 504, 529])(
    ...
    (statusCode) => {
      expect(
        isRetryableGenerateError(new APIStatusError(statusCode, "retryable")),
      ).toBe(true);
    },
  );
  ...
  it.each([400, 401, 403, 404, 422])(
    ...
    (statusCode) => {
      expect(
        isRetryableGenerateError(
          new APIStatusError(statusCode, "non-retryable"),
        ),
      ).toBe(false);
    },
  );
  ...
  it("retries an unclassified provider error as a transient fallback", () => {
    expect(
      isRetryableGenerateError(new ChatProviderError("upstream failure")),
    ).toBe(true);
  });
  ...
  it.each([
    ...
    "does not retry deterministic provider validation error: %s",
    (message) => {
      expect(isRetryableGenerateError(new ChatProviderError(message))).toBe(
        false,
      );
    },
  );
...
describe("isImageFormatError", () => {
  it("matches documented provider image format/data rejections", () => {
    ...
    expect(
      isImageFormatError(new APIStatusError(400, "Could not process image")),
    ).toBe(true);
    expect(
      isImageFormatError(
        new APIStatusError(
          400,
          "Invalid request: unsupported image url: /tmp/photo.avif",
        ),
      ),
    ).toBe(true);
    expect(
      isImageFormatError(new APIStatusError(400, "unsupported image format")),
    ).toBe(true);
    expect(
      isImageFormatError(
        new APIStatusError(400, "Unable to process input image"),
      ),
    ).toBe(true);
    expect(
      isImageFormatError(
        new APIStatusError(
          400,
          "The mime_type must accurately match the actual image format",
        ),
      ),
    ).toBe(true);
  });
  ...
  it("matches client-side image whitelist throws", () => {
    ...
    expect(
      isImageFormatError(
        new ChatProviderError(
          "Invalid data URL for image: data:image/avif;BASE64,AAA",
        ),
      ),
    ).toBe(true);
  });
  ...
  it("does not match a non-image 400, an unrelated status, or overflow/413 subclasses", () => {
    ...
    expect(isImageFormatError(new APIStatusError(422, "image is bad"))).toBe(
      false,
    );
    expect(isImageFormatError(new APIStatusError(401, "invalid api key"))).toBe(
      false,
    );
    expect(
      isImageFormatError(
        new APIContextOverflowError(
          400,
          "context length exceeded for image model",
        ),
      ),
    ).toBe(false);
    expect(
      isImageFormatError(
        new APIRequestTooLargeError(413, "image request too large"),
      ),
    ).toBe(false);
    expect(isImageFormatError(new ChatProviderError("connection reset"))).toBe(
      false,
    );
  ...
  it("does not match image count/size/support errors that stripping media cannot fix", () => {
    expect(
      isImageFormatError(new APIStatusError(400, "too many images in request")),
    ).toBe(false);
    expect(
      isImageFormatError(
        new APIStatusError(400, "image dimension 5000 exceeds maximum 2048"),
      ),
    ).toBe(false);
    expect(
      isImageFormatError(
        new APIStatusError(400, "image input is disabled for this model"),
      ),
    ).toBe(false);
    expect(
      isImageFormatError(new APIStatusError(400, "image_url is not allowed")),
    ).toBe(false);
    expect(
      isImageFormatError(
        new APIStatusError(
    ...
    expect(
      isImageFormatError(new APIStatusError(400, "Image Input Not Supported")),
    ).toBe(false);
    expect(
      isImageFormatError(
        new APIStatusError(400, "`inlineData` isn't supported by this model."),
      ),
    ).toBe(false);
    expect(
      isImageFormatError(
    ...
    expect(
      isImageFormatError(
        new APIStatusError(400, "unsupported media type for audio input"),
      ),
    ).toBe(false);
    expect(
      isImageFormatError(new APIStatusError(400, "invalid media type")),
    ).toBe(false);
  });

  it("is excluded from the transient-retry fallback so dedicated recovery fires first", () => {
    expect(
      isRetryableGenerateError(new ChatProviderError("transient blip")),
    ).toBe(true);
    expect(
      isRetryableGenerateError(
        new ChatProviderError(
    ...
    expect(
      isRetryableGenerateError(
        new APIStatusError(400, "unsupported image format"),
      ),
    ).toBe(false);
  });
});
...
describe("error hierarchy instanceof checks", () => {
  ...
  it("can catch with ChatProviderError and inspect subtype", () => {
    const err: ChatProviderError = new APIStatusError(
      404,
      "not found",
      "req-123",
    );
...
describe("normalizeAPIStatusError", () => {
  ...
  it.each([
    ...
    [
      400,
      "Invalid request: Your request exceeded model token limit: 262144 (requested: 274613)",
    ],
  ...
  it.each([
    ...
    'normalizes %i "%s" to APIProviderOverloadedError',
    (statusCode, message) => {
      const error = normalizeAPIStatusError(statusCode, message);
      expect(error).toBeInstanceOf(APIProviderOverloadedError);
      expect(error.statusCode).toBe(statusCode);
  ...
  it.each([
    ...
    'keeps bare %i "%s" as APIStatusError (not overload)',
    (statusCode, message) => {
      const error = normalizeAPIStatusError(statusCode, message);
      expect(error).toBeInstanceOf(APIStatusError);
...
describe("parseRetryAfterMs", () => {
  it("converts integer retry-after seconds to milliseconds", () => {
    expect(parseRetryAfterMs(new Headers({ "retry-after": "12" }))).toBe(
      12_000,
    );
  });
...
describe("isToolExchangeAdjacencyError", () => {
  ...
  it("matches the missing-tool_result 400", () => {
    expect(
      isToolExchangeAdjacencyError(
        new APIStatusError(400, ANTHROPIC_MISSING_RESULT),
      ),
    ).toBe(true);
  });
  ...
  it("matches the reverse unexpected-tool_result 400", () => {
    ...
    expect(
      isToolExchangeAdjacencyError(
        new APIStatusError(400, "unexpected `tool_result` block"),
      ),
    ).toBe(true);
  });
  ...
  it("also matches a 422 with the same shape", () => {
    expect(
      isToolExchangeAdjacencyError(
        new APIStatusError(422, ANTHROPIC_MISSING_RESULT),
      ),
    ).toBe(true);
  });
  ...
  it("matches the OpenAI/Moonshot tool_call_id-not-found 400", () => {
    expect(
      isToolExchangeAdjacencyError(
        new APIStatusError(400, MOONSHOT_TOOL_CALL_ID_NOT_FOUND),
      ),
    ).toBe(true);
    expect(
      isToolExchangeAdjacencyError(
  ...
  it("also matches a 422 tool_call_id-not-found", () => {
    expect(
      isToolExchangeAdjacencyError(
        new APIStatusError(422, MOONSHOT_TOOL_CALL_ID_NOT_FOUND),
      ),
    ).toBe(true);
  });
  ...
  it("does not match a context-overflow 400 or unrelated errors", () => {
    ...
    expect(
      isToolExchangeAdjacencyError(new APIStatusError(400, "Bad request")),
    ).toBe(false);
    expect(
      isToolExchangeAdjacencyError(
        new APIStatusError(400, "resource not found"),
      ),
    ).toBe(false);
    expect(
      isToolExchangeAdjacencyError(
        new APIStatusError(
          400,
          "400 Not supported model mimo-v2.5-pro-ultraspeed",
        ),
      ),
    ).toBe(false);
    expect(
      isToolExchangeAdjacencyError(
        new APIStatusError(500, ANTHROPIC_MISSING_RESULT),
      ),
    ).toBe(false);
    expect(
      isToolExchangeAdjacencyError(new Error(ANTHROPIC_MISSING_RESULT)),
    ).toBe(false);
...
describe("isRecoverableRequestStructureError", () => {
  ...
  it("matches the OpenAI/Moonshot tool_call_id-not-found 400", () => {
    expect(
      isRecoverableRequestStructureError(
        new APIStatusError(400, "400 tool_call_id  is not found"),
      ),
    ).toBe(true);
  });
  ...
  it("does not match context overflow, auth, or non-status errors", () => {
    ...
    expect(
      isRecoverableRequestStructureError(
        new APIStatusError(401, "unauthorized"),
      ),
    ).toBe(false);
    expect(
      isRecoverableRequestStructureError(
    ...
    expect(
      isRecoverableRequestStructureError(new Error("roles must alternate")),
    ).toBe(false);
  });
});
...
describe("isProviderRateLimitError", () => {
  it("matches explicit HTTP 429 status errors", () => {
    expect(
      isProviderRateLimitError(new APIProviderRateLimitError("rate limited")),
    ).toBe(true);
    expect(
      isProviderRateLimitError(new APIStatusError(429, "rate limited")),
    ).toBe(true);
    expect(isProviderRateLimitError({ response: { status: 429 } })).toBe(true);
    expect(
      isProviderRateLimitError({ statusCode: 503, message: "rate limit" }),
    ).toBe(false);
  });
  ...
  it("does not match non-rate-limit provider errors", () => {
    expect(
      isProviderRateLimitError(new APIStatusError(401, "unauthorized")),
    ).toBe(false);
    expect(isProviderRateLimitError("APIStatusError: 401 unauthorized")).toBe(
      false,
    );
    expect(isProviderRateLimitError(new Error("context length exceeded"))).toBe(
      false,
    );
  });
});
...
describe("quota-exhausted error contract", () => {
  it.each([
    ...
    'keeps the vendor-neutral 429 normalization a rate limit for "%s"',
    (message) => {
  ...
  it("is neither retryable nor a provider rate limit", () => {
    const quota = new APIProviderQuotaExhaustedError(
      "quota exhausted",
      "req-quota",
      1,
    );
    ...
    expect(
      isRetryableGenerateError(new APIProviderRateLimitError("rate limited")),
    ).toBe(true);
    expect(
      isProviderRateLimitError(new APIProviderRateLimitError("rate limited")),
    ).toBe(true);
