...
function contribution(marker: string): AgentProfileContribution {
  return {
    profiles: [
      normalizeAgentProfile({ name: marker, systemPrompt: () => marker }),
    ],
  };
}
...
describe("AgentProfileRegistryService", () => {
  ...
  it("keeps same-sourceId entries with different workspaceKeys coexisting", () => {
    ...
    registry.register("workspace", contribution("wd_a"), {
      workspaceKey: "wd_a",
    });
    registry.register("workspace", contribution("wd_b"), {
      workspaceKey: "wd_b",
    });
    // Re-registering one key replaces only that key's entry.
    registry.register("workspace", contribution("wd_a-v2"), {
      workspaceKey: "wd_a",
    });
  ...
  it("unregister(sourceId, workspaceKey) removes only the matching entry", () => {
    ...
    expect(registry.entries().map((entry) => entry.workspaceKey)).toEqual([
      "wd_b",
      undefined,
    ]);
  ...
  it("a dispose handle removes only the entry it registered", () => {
    const registry = new AgentProfileRegistryService();
    const stale = registry.register("workspace", contribution("old"), {
      workspaceKey: "wd_a",
    });
    registry.register("workspace", contribution("new"), {
      workspaceKey: "wd_a",
    });
