...
describe("FileSessionIndex (read model)", () => {
  ...
  it("paginates exactly through same-millisecond ties", async () => {
    ...
    const summaries = specs.map(([id, updatedAt]) =>
  ...
  it("walks all pages of a large listing without duplicates", async () => {
    ...
    for (let i = 0; i < 25; i++) {
      specs.push(
        summary(`s${String(i).padStart(2, "0")}`, {
          createdAt: i,
          updatedAt: i,
        }),
      );
      await seedSession(`s${String(i).padStart(2, "0")}`, {
        createdAt: i,
        updatedAt: i,
      });
    }
  ...
  it("cursor-less pages merge the mirror queue for read-your-writes", async () => {
    ...
    mirror.record(
      summary("pending-one", { title: "pending", createdAt: 3, updatedAt: 10 }),
    );
    const page = await store.listRecent({
      workspaceIds: [workspaceId],
      limit: 20,
  ...
  it("a crashed re-projection keeps readers on the previous generation", async () => {
    ...
    await fsp.rm(join(sessionsDir, workspaceId, "b"), {
      recursive: true,
      force: true,
    });
    (queryStore as FlakyQueryStore).failNextBatch = true;
  ...
  it("reprojects automatically after the store is wiped, without per-request backfill", async () => {
    ...
    await drainSessionIndexMirror();
    await drainQueryStoreDisposals();
    await fsp.rm(join(homeDir, "cache", "query-store"), {
      recursive: true,
      force: true,
    });
  ...
  it("reconciliation repairs external edits and deletions of state.json", async () => {
    ...
    await fsp.rm(join(sessionsDir, workspaceId, "gone"), {
      recursive: true,
      force: true,
    });
  ...
  it(
    "baseline: warm listRecent(limit=20) at 1k vs 10k vs 50k sessions",
    baseline,
    async () => {
      registerScopedService(
        LifecycleScope.App,
      ...
      const measure = async () => {
        const countOp = async (op: () => Promise<unknown>) => {
          ...
          await op();
          return {
            counts: countingStore.snapshotCounts(),
            fsLists: fileStorage.listCalls - listed,
          };
        };
        const ops = {
      ...
      for (const measured of [at1k, at10k, at50k]) {
        ...
        for (const op of Object.values(measured.ops))
          expect(op.fsLists).toBe(0);
        ...
        expect(sessionOps(measured.ops.list.counts)).toEqual([
          `pageByColumn:${collection}`,
        ]);
        expect(sessionRows(measured.ops.list.counts)).toBeLessThanOrEqual(
          2 * (LIST_LIMIT + 1),
        );
        // get is a single point lookup.
        expect(measured.ops.get.counts[`get:${collection}`]).toEqual({
          calls: 1,
          rows: 1,
        });
        expect(sessionOps(measured.ops.get.counts)).toEqual([
          `get:${collection}`,
        ]);
