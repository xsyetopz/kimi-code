  ...
  const host = createScopedTestHost([
    ...
    [
      ...
      stubBootstrap(
        "/tmp/kimi-home",
        {},
        { requestHeaders: { "User-Agent": "kimi-test/1.0" } },
      ),
    ],
    [
      IAgentIdentity,
      stubAgentIdentity({
        hostRequestHeaders: { "User-Agent": "kimi-test/1.0" },
      }),
    ],
  ]);
...
describe("refreshProviderModels modelSource short-circuit", () => {
  ...
  it("hides static entries from the orchestrator and merges them back verbatim", async () => {
    ...
    const { host, config, discovery, events, providers, models } =
      await createHost({
        providers: {
          ...staticProviders,
          acme: {
    ...
    try {
      ...
      expect(Object.keys(providerRecords).toSorted()).toEqual([
        "acme",
        "static-p",
      ]);
  ...
  it("throws provider.not_found for an unknown scoped provider", async () => {
    ...
    try {
      await expect(
        discovery.refreshProviderModels({ providerId: "missing" }),
      ).rejects.toSatisfy(
...
describe("refreshProviderModels write behavior", () => {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          headers: expect.objectContaining({
            Authorization: "Bearer sk-distributed-key",
          }),
        }),
      );
  ...
  it("never exposes a halfway-removed catalog: the registries stay untouched until the single atomic write", async () => {
    ...
    try {
      ...
      vi.spyOn(config, "replaceSections").mockImplementation(
        async (sections) => {
          seenDuringWrite = {
...
describe("modelCatalog config section", () => {
  it("self-registers and validates", () => {
    ...
    expect(() =>
      registry.validate(MODEL_CATALOG_SECTION, { refreshIntervalMs: -1 }),
    ).toThrow();
  });
});
