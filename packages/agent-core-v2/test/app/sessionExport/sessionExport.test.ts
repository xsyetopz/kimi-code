...
const fsOpenHook = vi.hoisted(() => ({
  afterOpen: undefined as
    | ((path: string, handle: FileHandle) => Promise<void>)
    | undefined,
}));
...
describe("sessionExport", () => {
  ...
  it("uses a timestamped default output path when outputPath is omitted", async () => {
    ...
    try {
      ...
      expect(basename(result.zipPath)).toMatch(
        /^kimi-debug-ses_defa-\d{8}-\d{6}\.zip$/,
      );
      await expect(stat(result.zipPath)).resolves.toMatchObject({
        size: expect.any(Number),
      });
    } finally {
  ...
  it("does not overwrite a previous default-path export when run again", async () => {
    ...
    await new Promise((resolvePromise) =>
      setTimeout(resolvePromise, 1100 - (Date.now() % 1000)),
    );
    ...
    try {
      ...
      await expect(stat(first.zipPath)).resolves.toMatchObject({
        size: expect.any(Number),
      });
      await expect(stat(second.zipPath)).resolves.toMatchObject({
        size: expect.any(Number),
      });
    } finally {
  ...
  it("keeps the session log bound when it rotates as wire scanning starts", async () => {
    ...
    await writeFile(
      wirePath,
      `${JSON.stringify({ type: "metadata", time: 1_700_000_000 })}\n`,
    );
    let rotated = false;
    ...
    try {
      ...
      await expect(
        readZipEntry(outputPath, "logs/kimi-code.log"),
  ...
  it("keeps the global log bound when it rotates as wire scanning starts", async () => {
    ...
    try {
      ...
      await expect(
        readZipEntry(outputPath, "logs/global/kimi-code.log"),
      ).resolves.toEqual(log);
    } finally {
      fsOpenHook.afterOpen = undefined;
    }
  ...
  it("closes pre-opened logs when manifest creation fails before writer ownership", async () => {
    ...
    fsOpenHook.afterOpen = async (path, handle) => {
      if (path === sessionLogPath || path === globalLogPath)
    ...
    expect(
      (await readdir(tmp)).filter((entry) =>
        entry.startsWith(".kimi-session-export-"),
      ),
    ).toEqual([]);
  });
  ...
  it("archives more than 300 session files without exhausting file handles", async () => {
    ...
    await expect(
      readZipEntry(join(tmp, "many-files.zip"), "entry-319.txt"),
    ).resolves.toEqual(Buffer.from("319\n", "utf8"));
  });
  ...
  it("includes a bounded Web log in the exported archive", async () => {
    ...
    await expect(
      readZipEntry(outputPath, "logs/kimi-web.jsonl"),
    ).resolves.toEqual(Buffer.from(webLog, "utf8"));
  });
  ...
  it("includes the desktop app log when given", async () => {
    ...
    await expect(
      readZipEntry(outputPath, "logs/kimi-desktop.log"),
    ).resolves.toEqual(Buffer.from(desktopLog, "utf8"));
  });
  ...
  it("archives the opened file size when the source is appended during compression", async () => {
    ...
    await expect(readZipEntry(outputPath, "live.log")).resolves.toEqual(
      initialContent,
    );
  });
  ...
  it("does not follow an output symlink swapped during compression", async () => {
    ...
    await expect(readZipEntry(outputPath, "controlled.bin")).resolves.toEqual(
      payload,
    );
    expect((await readdir(tmp)).toSorted()).toEqual([
      "export.zip",
      "safe-output",
      "state.json",
    ]);
...
function registerSessionExportServices(
  ...
  reg.defineInstance(ISessionIndex, {
    ...
    listRecent: async () => ({
      items: options.summary === undefined ? [] : [options.summary],
    }),
    get: async () => options.summary,
    count: async () =>
      options.summary === undefined || options.summary.archived ? 0 : 1,
    remove: async () => {},
  });
          ...
          accessor: accessorFrom([
            [
              ISessionLifecycleService,
              {
                ...
                create: async () => {
                  throw new Error(
                    "create should not be called by session export",
                  );
                },
                get: () => options.lifecycleHandle,
                list: () =>
                  options.lifecycleHandle === undefined
                    ? []
                    : [options.lifecycleHandle],
                ...
                fork: async () => {
                  throw new Error(
                    "fork should not be called by session export",
                  );
                },
                createChild: async () => {
                  throw new Error(
                    "createChild should not be called by session export",
                  );
                },
