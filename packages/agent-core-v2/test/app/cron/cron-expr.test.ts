...
describe("parseCronExpression", () => {
  ...
  it("parses ranges, lists, and steps", () => {
    expect(
      [...parseCronExpression("0 9-17 * * 1-5").hours].toSorted(
        (a, b) => a - b,
      ),
    ).toEqual([9, 10, 11, 12, 13, 14, 15, 16, 17]);
    expect(
      [...parseCronExpression("0 9,12,17 * * *").hours].toSorted(
        (a, b) => a - b,
      ),
    ).toEqual([9, 12, 17]);
    expect(
      [...parseCronExpression("*/5 * * * *").minutes].toSorted((a, b) => a - b),
    ).toEqual([0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55]);
    expect(
      [...parseCronExpression("0-30/10 * * * *").minutes].toSorted(
        (a, b) => a - b,
      ),
    ).toEqual([0, 10, 20, 30]);
  });
...
describe("hasFireWithinYears", () => {
  ...
  it("returns true for expressions with a fire inside the window", () => {
    ...
    expect(hasFireWithinYears(everyMinute, 1, localDate(2024, 0, 1))).toBe(
      true,
    );
  });
