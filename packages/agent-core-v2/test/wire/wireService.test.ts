...
describe("WireService", () => {
  ...
  it("replays silently: apply runs, no event or persist, onDidRestore once", async () => {
    ...
    const replayed = registerTestAgentWire(
      ix2,
    ...
    let restored = 0;
    disposables.add(
      replayEventBus.subscribe("wire.test.counter_changed", (event) =>
    ...
    expect((await readRecords(log2, SCOPE, "replay")).slice(1)).toEqual(
      records,
    );
  });
  ...
  it("throws CycleError when a dispatch cascade exceeds MAX_DRAIN", () => {
    disposables.add(
      eventBus.subscribe("wire.test.counter_changed", () =>
        wire.dispatch(otherInc({})),
      ),
    );
    disposables.add(
      eventBus.subscribe("wire.test.other_changed", () =>
        wire.dispatch(counterAdd({ by: 1 })),
      ),
    );
