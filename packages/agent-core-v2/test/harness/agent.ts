...
export interface ExecEnvOverride {
  ...
  readonly processRunner?:
    | ISessionProcessRunner
    | Partial<ISessionProcessRunner>;
}

export function execEnvServices(
...
function resolveHostFsOverride(
  input: IHostFileSystem | Partial<IHostFileSystem>,
): IHostFileSystem {
...
function isFullHostFs(input: unknown): boolean {
  ...
  return keys.every(
    (k) => typeof (input as Record<string, unknown>)[k] === "function",
  );
}
...
export function homeDirServices(
...
export function additionalDirServices(
...
export function llmGenerateServices(
  generate: GenerateFn,
): TestAgentServiceOverride {
  return appService(
    IProtocolAdapterRegistry,
...
export function externalHookServices(
  ...
  return [
    appService(
      IExternalHooksRunnerService,
      resolveExternalHooksRunner(hookRunner),
    ),
    agentService(
      IAgentExternalHooksService,
...
export function permissionModeServices(
  mode: PermissionMode,
): TestAgentServiceOverride {
  return agentService(
    IAgentPermissionModeService,
...
export function permissionRulesServices(
  ...
  return agentService(
    IAgentPermissionRulesService,
    createPermissionRulesStub(rules),
  );
}
...
function createSessionSkillCatalog(
...
export class AgentTestContext {
        ...
        [
          (reg) => {
            ...
            reg.defineInstance(
              ...
              >(["onDidCreateSession", "onWillCloseSession"]),
            );
            reg.defineInstance(
              ISessionInteractionService,
              this.createInteractionService(),
            );
            reg.defineInstance(
              ISessionApprovalService,
            ...
            reg.defineInstance(
              ...
              this.createInstructionsProvider(),
            );
            ...
            reg.defineInstance(IAgentLifecycleService, {
              ...
              create: () =>
                Promise.reject(
                  new Error(
                    "IAgentLifecycleService.create is not supported in the test harness",
                  ),
                ),
              fork: () =>
                Promise.reject(
                  new Error(
                    "IAgentLifecycleService.fork is not supported in the test harness",
                  ),
                ),
              get: () => undefined,
              list: () => [],
      ...
      extra: collectScopeSeed(
        [
          (reg) => {
            ...
            reg.defineDescriptor(
              IAgentProfileService,
              new SyncDescriptor(AgentProfileService),
            );
            reg.defineDescriptor(
              IAgentLLMRequesterService,
            ...
            reg.defineDescriptor(
              IAgentGoalService,
              new SyncDescriptor(AgentGoalService),
            );
            reg.defineDescriptor(
              IAgentSkillService,
              new SyncDescriptor(AgentSkillService),
            );
            reg.defineDescriptor(
              IAgentUserToolService,
  ...
  private async restoreRecordsOnly(
  ...
  private async dispatchRecordsOnly(
    records: readonly WireRecord[],
  ): Promise<void> {
    for (const record of records) {
      ...
      if (descriptor === undefined) {
        throw new Error(
          `Unknown wire record type in test harness: ${record.type}`,
        );
      }
      this.wire.dispatch({
        type: record.type,
  ...
  toolsData(): Array<
    ReturnType<IAgentToolRegistryService["list"]>[number] & {
  ...
  async takeApprovalRequest(): Promise<{
    ...
    const approval =
      await this.snapshots.take<ApprovalResponse>("requestApproval");
    return {
  ...
  appendRichToolExchange(): void {
    ...
    this.appendAssistantMessage({
      ...
      toolCalls: [
        toolCall("call_lookup", "Lookup", { query: "moon", limit: 2 }),
      ],
    });
    this.coverUsage(60);
  ...
  async expectResumeMatches(): Promise<void> {
    ...
    await this.drainWirePersistence();
    const configSnapshot = structuredClone(
      this.get(IConfigService).getAll() as KimiConfig,
    );
  ...
  private createApprovalService(): ISessionApprovalService {
    return {
      ...
      enqueue: (request) => {
        const id =
          request.id ?? request.toolCallId ?? `${request.toolName}:test`;
        const {
          sessionId: _sessionId,
          agentId: _agentId,
          ...payload
  ...
  private createRpcPassthroughAdapters(): AgentRpcPassthroughAPI {
    return {
      runShellCommand: (payload) =>
        this.get(IAgentShellCommandService).run(payload),
      cancelShellCommand: (payload) =>
        this.get(IAgentShellCommandService).cancel(payload.commandId),
      setThinking: (payload) =>
        this.get(IAgentProfileService).setThinking(payload.level),
      setModel: (payload) =>
        this.get(IAgentProfileService).setModel(payload.model),
      ...
      enterSwarm: (payload) =>
        this.get(IAgentSwarmService).enter(payload.trigger),
      ...
      registerTool: (payload) =>
        this.get(IAgentUserToolService).register(payload),
      unregisterTool: (payload) =>
        this.get(IAgentUserToolService).unregister(payload.name),
      setActiveTools: (payload) =>
        this.get(IAgentProfileService).update({
          activeToolNames: payload.names,
        }),
      stopTask: (payload) => {
        const tasks = this.get(IAgentTaskService);
      ...
      detachTask: (payload) =>
        this.get(IAgentTaskService).detach(payload.taskId),
      ...
      getTasks: (payload) =>
        this.get(IAgentTaskService).list(
...
function createWorkspaceContextStub(
  ...
  return {
    ...
    assertAllowed: (absPath: string, op: PathAccessOperation) => {
      const target = isAbsolute(absPath)
        ? resolve(absPath)
        : resolve(workDir, absPath);
      if (!isWithin(target)) {
...
function createPermissionModeService(
  ...
  return {
    ...
    onDidChangeMode:
      Event.None as IAgentPermissionModeService["onDidChangeMode"],
  };
}
  ...
  return {
    ...
    checkpointedModels: Object.fromEntries(
      CHECKPOINTED_MODELS.map((model) => [
        model.name,
        (ctx.get(IWireService).getModel(model) as Checkpointed<unknown>)
          .current,
      ]),
    ),
...
function pendingTaskNotificationKeys(
  records: readonly WireRecord[],
): readonly string[] {
  ...
  for (const record of records) {
    ...
    for (const message of contextMessagesFromRecord(record)) {
      ...
      if (isTaskOriginLike(origin)) {
        delivered.add(
          `${origin.taskId}\0${origin.status}\0${origin.notificationId}`,
        );
      }
    }
  }
...
function contextMessagesFromRecord(
  record: WireRecord,
...
function isTaskInfoLike(value: unknown): value is {
  ...
  return (
    typeof info["taskId"] === "string" && typeof info["status"] === "string"
  );
}
...
function configStateSnapshot(
...
): ResumeStateSnapshot["config"] {
  const profile = ctx.get(IAgentProfileService);
  const data = profile.data();
  let model: Model | undefined;
  try {
    model =
      data.modelAlias === undefined
        ? undefined
        : ctx.get(IModelCatalog).get(data.modelAlias);
  } catch {
    model = undefined;
  }
  const providerConfig =
    model === undefined
      ? undefined
      : ctx.get(IProviderService).get(model.providerName);
  return {
...
function configService(readConfig: () => KimiConfig): IConfigService {
  ...
  const effectiveConfig = () => {
    const effective = { ...configWithEnvOverrides(readConfig()) } as Record<
      string,
      unknown
    >;
    secondaryModelOverlay.apply(
      effective,
      () => undefined,
      (_domain, value) => value,
    );
...
function cronEnvOverrides(
  ...
  const pollIntervalMs = parseEnvCronPollIntervalMs(
    process.env["KIMI_CRON_POLL_INTERVAL_MS"],
  );
...
function parseEnvCronPollIntervalMs(
...
function createLogService(
  ...
  return {
    ...
    child: (childBindings) =>
      createLogService(
        logger?.child?.(childBindings) ??
          logger?.createChild?.(childBindings) ??
          logger,
  ...
  return {
    ...
    createChatProvider: (input: ProtocolAdapterConfig) => {
      if (
        ...
        hasProviderDefinition(input.providerType)
      ) {
        return replaceProviderGenerate(
          real.createChatProvider(input),
          generate,
        );
      }
...
function replaceProviderGenerate(
  ...
  if (provider.uploadVideo !== undefined) {
    replaced.uploadVideo = (input, options) =>
      provider.uploadVideo!(input, options);
  }
  return replaced;
}
...
function normalizeProviderStreamParts(
  ...
  const pendingIndexedDeltas = new Map<
    number | string,
    StreamedMessagePart[]
  >();
