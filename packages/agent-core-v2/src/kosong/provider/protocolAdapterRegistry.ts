...
export class ProtocolAdapterRegistry implements IProtocolAdapterRegistry {
  ...
  resolveAdapterIdentity(
    ...
    const definition =
      providerType === undefined
        ? undefined
        : getProviderDefinition(providerType, protocol);
    ...
    const resolved: ResolvedTrait[] = traits.map((trait) => ({
      trait,
      context,
    }));
  ...
  explainCapability(
    ...
    const baseCapability = getProtocolBase(identity.baseId)?.capability?.(
      modelName,
    );
    if (baseCapability !== undefined) {
      return {
        ...
        source: {
          kind: "builtin",
          detail: `protocol base '${identity.baseId}' catalog`,
        },
      };
    }
    return {
  ...
  createChatProvider(config: ProtocolAdapterConfig): ChatProvider {
    const identity = this.resolveAdapterIdentity(
      config.protocol,
      config.providerType,
    );
