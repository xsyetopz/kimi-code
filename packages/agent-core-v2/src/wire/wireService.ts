...
export class WireService extends Disposable implements IWireService {
  ...
  async restore(): Promise<void> {
    if (
      ...
      throw new BugIndicatingError(
        `Agent wire restore called while phase is ${this.restorePhase}`,
      );
    }
    this.restorePhase = "restoring";
    try {
      const source = this.log.read<WireRecord>(
        this.wireScope,
        AGENT_WIRE_RECORD_KEY,
      );
    ...
    for (const op of group.ops) {
      ...
      if (crossReducers !== undefined) {
        for (const entry of crossReducers) {
          ...
          crossInst.state = Object.freeze(
            entry.reducer(crossInst.state, op.payload),
          );
        }
      }
    }
  ...
  private appendToJournal(record: WireRecord, model: ModelDef<any>): void {
    ...
    const transform: PartsTransformer = (parts) =>
      this.blobService.offloadParts(parts as readonly ContentPart[]) as Promise<
        readonly unknown[]
      >;
  ...
  private async rehydrateModels(): Promise<void> {
    const transform: PartsTransformer = (parts) =>
      this.blobService.loadParts(parts as readonly ContentPart[]) as Promise<
        readonly unknown[]
      >;
