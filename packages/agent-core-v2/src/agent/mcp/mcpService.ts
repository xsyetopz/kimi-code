...
export class AgentMcpService extends Disposable implements IAgentMcpService {
  ...
  private async joinHealedOrReconnect(
    ...
    return current !== undefined && current !== staleClient
      ? current
      : undefined;
  }
  ...
  onStatusChange(listener: Parameters<IAgentMcpService["onStatusChange"]>[0]) {
    const unsubscribe =
      this.mcpHandle.connectionManager.onStatusChange(listener);
    return {
      dispose: unsubscribe,
    };
  }
  ...
  private registerNeedsAuthMcpServer(entry: McpServerEntry): void {
    ...
    const disposable = this._register(
      this.registry.register(tool, { source: "mcp" }),
    );
  ...
  private recordDiscovery(
    ...
    const enabledNamesSnapshot = [...enabledNames].toSorted((a, b) =>
      a.localeCompare(b),
    );
    const work = (): void => {
