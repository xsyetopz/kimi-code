...
function normalizeTimeoutMs(
  ...
  const defaultSeconds = isBackground
    ? DEFAULT_BACKGROUND_TIMEOUT_S
    : DEFAULT_TIMEOUT_S;
...
export class BashTool implements IBashTool {
  ...
  private autoBackgroundOnTimeout(): boolean {
    return (
      resolveAgentTaskConfig(this.config)?.bashAutoBackgroundOnTimeout ?? true
    );
  }

  private detachTimeoutMs(): number {
    const configuredS = resolveAgentTaskConfig(this.config)?.bashTaskTimeoutS;
    if (configuredS === undefined)
      return DEFAULT_BACKGROUND_TIMEOUT_S * MS_PER_SECOND;
    return configuredS * MS_PER_SECOND;
  }

  get description(): string {
    if (!this.allowBackground())
      return withoutBackgroundDescription(this.renderedDescription);
  ...
  private async foregroundCompletionResult(
    ...
    if (current?.status === "timed_out") {
      ...
      result = builder.error("Interrupted by user", {
        brief: "Interrupted by user",
      });
    } else if (
