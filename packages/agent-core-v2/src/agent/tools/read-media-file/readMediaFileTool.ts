...
function shouldSurfaceVideoUploadError(
  error: unknown,
  inlineVideoSupported: boolean,
): boolean {
  if (error instanceof VideoUploadUnsupportedError)
    return !inlineVideoSupported;
  ...
  private async videoContentPart(
    ...
    if (this.videoUploader !== undefined) {
      try {
        ...
        if (shouldSurfaceVideoUploadError(error, this.inlineVideoSupported))
          throw error;
      }
    }
