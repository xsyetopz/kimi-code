...
export class ToolResultTruncationService
  implements IAgentToolResultTruncationService
{
  ...
  async truncateForModel<T extends ExecutableToolResult>(
    ...
    if (text === undefined || text.length <= TOOL_RESULT_MAX_CHARS)
      return input.result;
    if (input.result.truncated === true) return input.result;

    const saved = await this.saveToolResult(
      input.toolName,
      input.toolCallId,
      text,
    );
  ...
  private async saveToolResult(
    ...
    try {
      ...
      await this.storage.write(this.storageScope, key, encoder.encode(text), {
        atomic: true,
      });
      return {
...
function persistableToolResultText(
