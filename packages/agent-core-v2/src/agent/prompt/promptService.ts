...
export const promptLaunchingKey = defineState<boolean>(
  "prompt.launching",
  () => false,
);
...
export class AgentPromptService implements IAgentPromptService {
  ...
  async enqueue(input: PromptInput): Promise<PromptHandle> {
    ...
    if (this.active === undefined && !this.launching) {
      if (
        this.fullCompaction.compacting !== null &&
      ...
      void this.startNext();
      await Promise.race([
        record.launchedDeferred.promise,
        record.completionDeferred.promise,
      ]);
    } else {
      this.publishQueued(record);
    }
  ...
  async steer(promptIds: readonly string[]): Promise<readonly PromptHandle[]> {
    ...
    if (this.active === undefined)
      throw new Error2(
        ...
        "no active prompt to steer into",
      );
    ...
    const { message: rerouted, captions } =
      this.extractCompressionCaptions(message);
    ...
    this.steered.set(this.active.id, [
      ...(this.steered.get(this.active.id) ?? []),
      ...selected,
    ]);
  ...
  abort(promptId: string, reason: Error = userCancellationReason()): boolean {
    ...
    item.completionDeferred.resolve({
      promptId,
      result: undefined,
      state: "cancelled",
    });
  ...
  private async startNext(): Promise<void> {
    ...
    try {
      ...
      const { message, captions } = this.extractCompressionCaptions(
        item.message,
      );
  ...
  private async blockedByHook(
    promptMessage: ContextMessage,
    isSteer: boolean,
  ): Promise<boolean> {
  ...
  private extractCompressionCaptions(message: ContextMessage): {
    ...
    return {
      message: captions.length === 0 ? message : { ...message, content: parts },
      captions,
    };
  }
  private appendPrompt(
    ...
    if (message.content.length > 0)
      this.context.append({ ...message, id: ownerPromptId });
  }
  private async deliverToolResult(ctx: ToolDidExecuteContext): Promise<void> {
    ...
    if (delivery.kind === "steer")
      await this.inject(delivery.message as ContextMessage);
  }
