...
export class AgentUsageService
  extends Disposable
  implements IAgentUsageService
{
  declare readonly _serviceBrand: undefined;

  private readonly _onDidRecord = this._register(
    new Emitter<UsageRecordedContext>(),
  );
  ...
  record(
    ...
    const usageScope: UsageRecordScope =
      source?.type === "turn" ? "turn" : "session";
  ...
  status(): UsageStatus {
    return usageStatusFromState(
      this.wire.getModel(UsageModel),
      this.currentTurn,
    );
  }
}
