...
export class RuntimeCompactionStrategy implements CompactionStrategy {
  constructor(
    ...
    private readonly estimateMessage: (
      message: Message,
    ) => number = estimateTokensForMessage,
  ) {}
  ...
  private delegate(): DefaultCompactionStrategy {
    ...
    return new DefaultCompactionStrategy(
      () =>
        model.modelCapabilities.max_input_tokens ??
  ...
  private config(
    model: ProfileModelContext = this.context(),
  ): CompactionConfig {
    const triggerRatio =
      model.compactionTriggerRatio ?? DEFAULT_COMPACTION_CONFIG.triggerRatio;
    const blockRatio = Math.max(
      triggerRatio,
...
export class DefaultCompactionStrategy implements CompactionStrategy {
  constructor(
    ...
    protected readonly estimateMessage: (
      message: Message,
    ) => number = estimateTokensForMessage,
  ) {}
...
function prefixEndsWithOpenToolExchange(
  messages: readonly Message[],
  index: number,
): boolean {
  ...
  for (let i = index; i >= 0; i--) {
    ...
    return (
      message.role === "assistant" && message.toolCalls.length > toolResultCount
    );
  }
  return false;
}
