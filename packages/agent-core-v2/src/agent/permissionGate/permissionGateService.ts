...
export class AgentPermissionGate
  extends Disposable
  implements IAgentPermissionGate
{
  ...
  constructor(
    ...
    super();
    this._register(
      toolExecutor.onBeforeExecuteTool((event) => this.adjudicate(event)),
    );
  }
  ...
  private async adjudicate(event: BeforeToolExecuteEvent): Promise<void> {
    ...
    if (result.kind === "ask") {
      event.waitUntil(() =>
        this.toolApproval.requestToolApproval(event, result, policyName),
      );
      return;
    }
