...
export class AgentLoopServiceCore extends Disposable implements IAgentLoopService {
  ...
  private cancelActiveTurn(turnId: number | undefined, cancellation: unknown): boolean {
    ...
    this.wire.dispatch(
      cancelTurn({ turnId: job.turn.id, target: 'active', reason: lt.cancelReasonFor(cancellation) }),
    );
  ...
  private cancelQueuedTurn(turnId: number, cancellation: unknown): boolean {
    ...
    this.wire.dispatch(cancelTurn({ turnId, target: 'queued', reason: lt.cancelReasonFor(cancellation) }));
  ...
  private assignStep(job: lt.TurnJob, request: StepRequest, options?: StepEnqueueOptions): Step {
  ...
  private enqueueStep(job: lt.TurnJob, request: StepRequest, options?: StepEnqueueOptions): Step {
  ...
  private cancelStep(job: lt.TurnJob, step: lt.MutableStep, request: StepRequest, reason?: unknown): boolean {
    ...
    try {
      ...
      if (result !== undefined) {
        ...
        const interruptReason =
          result.type === 'completed' ? undefined : lt.interruptReasonFor(result);
