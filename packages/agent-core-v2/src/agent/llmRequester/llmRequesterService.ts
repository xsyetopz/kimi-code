...
export const llmRequesterLastConfigLogSignatureKey = defineState<
  string | undefined
>("llmRequester.lastConfigLogSignature", () => undefined as string | undefined);
export const llmRequesterTurnConfigsKey = defineState<
  Map<number, TurnRequestConfig>
>("llmRequester.turnConfigs", () => new Map());
...
export const llmRequesterMediaStrippedTurnsKey = defineState<
  Map<number, MediaStripSnapshot>
>("llmRequester.mediaStrippedTurns", () => new Map());
export const llmRequesterEmittedThinkingEffortWarningsKey = defineState<
  Set<string>
>("llmRequester.emittedThinkingEffortWarnings", () => new Set());
...
export class AgentLLMRequesterService implements IAgentLLMRequesterService {
  ...
  constructor(
    ...
    private readonly videoResolver: IAgentVideoResolverService,
  ...
  private async runRequest(
    ...
    const projectionOptions: ContextProjectionOptions = {
      protocolProfile: request.model.protocolProfile,
    };
    const requestInput = (projection: RequestProjection) => {
      return {
        ...
        messages:
          projection === "strict"
            ? this.projector.projectStrict(shaped, projectionOptions)
            : projection === "media-degraded"
              ? this.projector.projectMediaDegraded(shaped, projectionOptions)
              : projection === "media-stripped"
                ? this.projector.projectMediaStripped(
                    shaped,
                    (mediaStripSnapshot ??=
                      this.projector.captureMediaStripSnapshot(
                        shaped,
                        projectionOptions,
                      )),
                    projectionOptions,
                  )
                : this.projector.project(shaped, projectionOptions),
      };
    };
      ...
      try {
        ...
        if (
          ...
          if (projection === "normal") {
            ...
            this.markMediaStrippedRecoveryTurn(
              mediaStripSnapshot,
              request.source,
            );
            projection = "media-stripped";
        ...
        if (
          projection === "normal" &&
          isRecoverableRequestStructureError(raw)
        ) {
          signal?.throwIfAborted();
          this.log.warn(
  ...
  private warnAboutAnthropicThinkingEffort(request: ResolvedLLMRequest): void {
    ...
    const supportEfforts = request.model.supportEfforts?.filter(
  ...
  private resolveRequest(
    ...
    const baseParams =
      turnConfig?.params ?? this.profile.resolveRequestParams();
    const budgetParams = completionBudgetParams({
      budget: resolveCompletionBudget({
        ...
        maxCompletionTokensCap:
          this.config.get<ModelOverrides>("modelOverrides")
            ?.maxCompletionTokens,
      }),
  ...
  private recordRequest(input: LLMRequestLogInput): void {
    ...
    if (
      !this.wire
        .getModel(LlmRequestTraceModel)
        .seenToolsHashes.includes(toolsHash)
    ) {
    ...
    const modelConfig =
      input.modelAlias === undefined
        ? undefined
        : this.modelService.get(input.modelAlias);
  ...
  private logResponse(
    ...
    if (timing.requestBuildMs !== undefined)
      payload["requestBuildMs"] = timing.requestBuildMs;
    if (timing.serverFirstTokenMs !== undefined) {
      payload["serverFirstTokenMs"] = timing.serverFirstTokenMs;
    }
    if (timing.serverDecodeMs !== undefined)
      payload["serverDecodeMs"] = timing.serverDecodeMs;
    if (timing.clientConsumeMs !== undefined)
      payload["clientConsumeMs"] = timing.clientConsumeMs;
