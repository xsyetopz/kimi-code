...
export type MergeAllAvailableSkillsConfig = z.infer<
  typeof MergeAllAvailableSkillsConfigSchema
>;
...
export type BuiltinProductSkillsConfig = z.infer<
  typeof BuiltinProductSkillsConfigSchema
>;
...
export const stripBuiltinProductSkillsEnv: ConfigStripEnv<
  BuiltinProductSkillsConfig
> = (value, raw, getEnv) => {
  if (getEnv === undefined) return value;
  if (parseBooleanEnv(getEnv(BUILTIN_PRODUCT_SKILLS_ENV)) === undefined)
    return value;
...
export function builtinProductSkillsEnabled(config: IConfigService): boolean {
  return (
    config.get<BuiltinProductSkillsConfig>(BUILTIN_PRODUCT_SKILLS_SECTION) !==
    false
  );
}
