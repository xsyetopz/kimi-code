...
export class CloudTransport {
  ...
  async retryDiskEvents(): Promise<void> {
    ...
    for (const key of keys) {
      if (!key.startsWith(FAILED_PREFIX) || !key.endsWith(JSONL_SUFFIX))
        continue;
...
function flattenNested(
  target: Record<string, CloudPrimitive>,
  prefix: string,
  value: unknown,
) {
...
function assertPrimitive(
  key: string,
  value: unknown,
): asserts value is CloudPrimitive {
