...
export class LocalFetchURLProvider implements UrlFetcher {
  ...
  async fetch(
    ...
    try {
      ...
      await Promise.all(
        dispatchers.map((dispatcher) => dispatcher.close().catch(() => {})),
      );
    }
  }
  ...
  private extractMainContent(html: string): string {
    ...
    try {
      const reader = new Readability(
        primary.document as unknown as ReadabilityDocument,
        {
          charThreshold: 0,
        },
      );
...
function isBlockedAddress(address: string): boolean {
  ...
  return (
    isIP(normalized) === 6 &&
    PRIVATE_ADDRESS_BLOCKLIST.check(normalized, "ipv6")
  );
}
...
async function resolveSafeFetchTarget(
  ...
  try {
    ...
    throw new Error2(ErrorCodes.WEB_INVALID_URL, `Invalid URL: "${url}"`, {
      details: { url },
    });
  }
