...
describe("CombinedAutocompleteProvider", () => {
  ...
  describe("fd @ file suggestions", { skip: !isFdInstalled }, () => {
    ...
    test("returns the same @ suggestions when the cwd path contains the query", async () => {
      ...
      const query = "@plan";
      const normalProvider = new CombinedAutocompleteProvider(
        [],
        normalBaseDir,
        requireFdPath(),
      );
      const queryInPathProvider = new CombinedAutocompleteProvider(
        [],
        queryInPathBaseDir,
        requireFdPath(),
      );

      const normalResult = await getSuggestions(
        normalProvider,
        [query],
        0,
        query.length,
      );
      const queryInPathResult = await getSuggestions(
        queryInPathProvider,
        [query],
        0,
        query.length,
      ...
      assert.ok(
        normalize(normalResult).includes(
          "plan.md :: packages/tui/docs/plan.md",
        ),
      );
    });
  ...
  describe("dot-slash path completion", () => {
    ...
    test("preserves ./ prefix when completing paths", async () => {
      ...
      const values = result?.items.map((item) => item.value);
      assert.ok(
        values?.includes("./update.sh"),
        `Expected ./update.sh in ${JSON.stringify(values)}`,
      );
    });
    ...
    test("preserves ./ prefix for directory completions", async () => {
      ...
      assert.ok(
        values?.includes("./src/"),
        `Expected ./src/ in ${JSON.stringify(values)}`,
      );
    });
  });
