...
describe("TUI Kitty image cleanup", () => {
  ...
  it("falls back to full redraw when Kitty image pre-clear would scroll", async () => {
    ...
    try {
      ...
      assert.ok(
        tui.fullRedraws > redrawsBeforeImage,
  ...
  it("does not use cursor-up placement for Kitty images taller than the viewport", async () => {
    ...
    try {
      ...
      const imageSequence = imageLines[0];
      assert.ok(
        imageLines.length > terminal.rows,
  ...
  it("deletes changed image ids before drawing moved placements", async () => {
    ...
    const oldImage = encodeKitty("AAAA", {
      columns: 2,
      rows: 2,
      imageId: 42,
      moveCursor: false,
    ...
    const newImage = encodeKitty("BBBB", {
      columns: 2,
      rows: 1,
      imageId: 42,
      moveCursor: false,
...
describe("TUI differential rendering", () => {
  ...
  it("clears stale content when maxLinesRendered was inflated by a transient component", async () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      assert.ok(
        !line.includes("Chat 12"),
        `Stale "Chat 12" at viewport row ${i}`,
      );
      assert.ok(
        !line.includes("Chat 13"),
        `Stale "Chat 13" at viewport row ${i}`,
      );
      assert.ok(
        !line.includes("Chat 14"),
...
describe("TUI steady-frame processed-line reuse", () => {
  it("writes only the changed line when one component updates in a long transcript", async () => {
    ...
    staticComponent.lines = Array.from(
      { length: 30 },
      (_, i) => `static-${String(i).padStart(2, "0")}`,
    );
    ...
    assert.ok(
      !writes.includes("static-"),
      "unchanged transcript lines must not be rewritten",
    );
    assert.ok(
      !writes.includes("\x1b[2J"),
      "no full clear for an in-viewport change",
    );
  ...
  it("fully redraws when the terminal width changes", async () => {
    ...
    assert.ok(
      writes.includes("\x1b[2J"),
      "width change should clear and redraw",
    );
    assert.ok(
      writes.includes("one") &&
        writes.includes("two") &&
        writes.includes("three"),
    );
    tui.stop();
  });
  ...
  it("does not rewrite a kitty image on an identical frame", async () => {
    ...
    try {
      ...
      assert.ok(
        !writes.includes(imageSequence),
        "identical frame must not rewrite the image",
      );
      assert.strictEqual(writes.replaceAll("\x1b[?25l", ""), "");
      tui.stop();
    } finally {
