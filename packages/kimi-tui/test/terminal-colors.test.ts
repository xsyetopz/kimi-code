...
describe("parseOsc11BackgroundColor", () => {
  it("parses 16-bit OSC 11 rgb responses", () => {
    assert.deepStrictEqual(
      parseOsc11BackgroundColor("\x1b]11;rgb:0000/8000/ffff\x07"),
      {
        r: 0,
        g: 128,
        b: 255,
  ...
  it("parses OSC 11 hex responses", () => {
    ...
    assert.deepStrictEqual(parseOsc11BackgroundColor("\x1b]11;#000000\x07"), {
      r: 0,
      g: 0,
      b: 0,
    });
  ...
  it("rejects non-strict OSC 11 responses", () => {
    ...
    assert.strictEqual(
      parseOsc11BackgroundColor("\x1b]10;#ffffff\x07"),
      undefined,
    );
    assert.strictEqual(
      parseOsc11BackgroundColor("\x1b]11;#ffffff\x07x"),
      undefined,
    );
  });
});
...
describe("parseTerminalColorSchemeReport", () => {
  it("parses color scheme reports", () => {
    ...
    assert.strictEqual(
      parseTerminalColorSchemeReport("\x1b[?997;3n"),
      undefined,
    );
    assert.strictEqual(parseTerminalColorSchemeReport("\x1b[?996n"), undefined);
    assert.strictEqual(
      parseTerminalColorSchemeReport("x\x1b[?997;1n"),
      undefined,
    );
  });
});
...
describe("TUI.queryTerminalBackgroundColor", () => {
  ...
  it("dispatches non-matching input normally while waiting for an OSC 11 reply", async () => {
    ...
    try {
      let settled = false;
      const query = tui
        .queryTerminalBackgroundColor({ timeoutMs: 1000 })
        .then((rgb) => {
          settled = true;
          return rgb;
        });
