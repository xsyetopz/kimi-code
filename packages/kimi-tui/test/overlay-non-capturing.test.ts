    ...
    it("microtask-deferred sub-overlay pattern (showExtensionCustom simulation) restores focus", async () => {
      ...
      try {
        ...
        const overlayPromise = new Promise<void>((resolve) => {
          doneFn = () => {
            if (!timerHandle)
              throw new Error("timerHandle was not initialized");
            timerHandle.hide();
            tui.hideOverlay();
            resolve();
          };
  ...
  describe("rendering order", () => {
    it("focus() on already-focused overlay bumps visual order", async () => {
      ...
      try {
        ...
        tui.showOverlay(new StaticOverlay(["B"]), {
          row: 0,
          col: 0,
          width: 1,
          nonCapturing: true,
        ...
        aHandle.focus();
        tui.showOverlay(new StaticOverlay(["C"]), {
          row: 0,
          col: 0,
          width: 1,
          nonCapturing: true,
    ...
    it("default rendering order for overlapping overlays follows creation order", async () => {
      ...
      tui.start();
      try {
        tui.showOverlay(new StaticOverlay(["A"]), {
          row: 0,
          col: 0,
          width: 1,
          nonCapturing: true,
    ...
    it("capturing overlay hidden and shown again renders on top after unhide", async () => {
      ...
      try {
        ...
        const capturing = tui.showOverlay(new StaticOverlay(["B"]), {
          row: 0,
          col: 0,
          width: 1,
        });
