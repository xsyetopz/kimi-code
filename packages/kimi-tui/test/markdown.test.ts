...
- Item 2`,
  ...
  - Level 2
    - Level 3
      ...
      assert.ok(
        plainLines.some((line) => line.includes("            - Level 4")),
      );
    });
...
2. Second`,
      ...
      assert.ok(
        plainLines.some((line) => line.includes("    1. Nested first")),
      );
      assert.ok(
        plainLines.some((line) => line.includes("    2. Nested second")),
      );
...
3. Third item`,
      ...
      const lines = markdown.render(80);
      const plainLines = lines.map((line) =>
        line.replace(/\x1b\[[0-9;]*m/gu, "").trim(),
      );

      // Find all lines that start with a number and period
      const numberedLines = plainLines.filter((line) => /^\d+\./u.test(line));
...
| npm run build | Build the project | npm run build |`,
      ...
      const plainLines = lines.map((line) =>
        line.replace(/\x1b\[[0-9;]*m/gu, "").trimEnd(),
      );
...
| prefix ${url} |`,
      ...
      const extracted = plainLines.join("").replace(/[│├┤─\s]/gu, "");
...
> - nested bullet`,
  ...
  describe("Heading with inline code", () => {
    it("should preserve heading styling after inline code", () => {
      ...
      const precedingChunk = joinedOutput.slice(
        Math.max(0, afterCodeIndex - 40),
        afterCodeIndex,
      );
      assert.ok(
    ...
    it("should preserve heading styling after inline code for h1", () => {
      ...
      assert.ok(
        precedingChunk.includes("\x1b[1m"),
        `Should re-apply bold for h1: ${precedingChunk}`,
      );
      assert.ok(
        precedingChunk.includes("\x1b[36m"),
        `Should re-apply cyan for h1: ${precedingChunk}`,
      );
      assert.ok(
        precedingChunk.includes("\x1b[4m"),
        `Should re-apply underline for h1: ${precedingChunk}`,
      );
    });
    ...
    it("should not leak h1 underline into padding when inline code is the last token", async () => {
      ...
      const contentWidth = renderedLine
      ...
      for (let col = contentWidth; col < 80; col++) {
        assert.strictEqual(
          getCellUnderline(terminal, 0, col),
          0,
    ...
    it("should preserve heading styling after bold text", () => {
      ...
      const precedingChunk = joinedOutput.slice(
        Math.max(0, afterBoldIndex - 40),
        afterBoldIndex,
      );
      assert.ok(
        precedingChunk.includes("\x1b[1m"),
        `Should re-apply bold for h2: ${precedingChunk}`,
      );
      assert.ok(
        precedingChunk.includes("\x1b[36m"),
        `Should re-apply cyan for h2: ${precedingChunk}`,
      );
    });
  });
  ...
  describe("Strikethrough syntax", () => {
    it("should render ~~text~~ as strikethrough", () => {
      ...
      const lines = markdown.render(80);
      const joinedOutput = lines.join("\n");
      const joinedPlain = lines
        .map((line) => line.replace(/\x1b\[[0-9;]*m/gu, ""))
        .join(" ");

      assert.ok(
        joinedOutput.includes("\x1b[9m"),
        "Should apply strikethrough styling",
      );
      assert.ok(
        joinedPlain.includes("strikethrough"),
  ...
  describe("Links", () => {
    ...
    it("should not duplicate URL for bare URLs", () => {
      ...
      const urlCount = (joinedPlain.match(/https:\/\/example\.com/gu) || [])
        .length;
    ...
    it("should emit OSC 8 hyperlink sequence when terminal supports hyperlinks", () => {
      ...
      assert.ok(
        joined.includes("\x1b]8;;https://example.com\x1b\\"),
        "Should contain OSC 8 open sequence",
      );
      // OSC 8 close: ESC ] 8 ; ; ESC \
      assert.ok(
        joined.includes("\x1b]8;;\x1b\\"),
        "Should contain OSC 8 close sequence",
      );
      // Visible text is present
      const plainLines = lines.map((line) =>
        line.replace(/\x1b[^a-zA-Z]*[a-zA-Z]|\x1b\].*?\x1b\\/gu, ""),
      );
      assert.ok(
        plainLines.join("").includes("click here"),
        "Should contain link text",
      );
      // URL is NOT printed inline as plain text
      const rawPlain = lines.map((line) =>
        line
          .replace(/\x1b\]8;;[^\x1b]*\x1b\\/gu, "")
          .replace(/\x1b\[[0-9;]*m/gu, ""),
      );
      assert.ok(
        !rawPlain.join("").includes("(https://example.com)"),
  ...
  describe("Streaming code fences", () => {
    it("stabilizes partial closing fence rendering", () => {
      ...
      const partial = new Markdown(
        "```ts\nconst x = 1;\n``",
        0,
        0,
        defaultMarkdownTheme,
      );
      const complete = new Markdown(
        "```ts\nconst x = 1;\n```",
        0,
        0,
        defaultMarkdownTheme,
