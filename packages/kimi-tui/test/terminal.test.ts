...
describe("normalizeAppleTerminalInput", () => {
  ...
  it("leaves non-Return input unchanged", () => {
    assert.equal(
      normalizeAppleTerminalInput("\x1b[13;2u", true, true),
      "\x1b[13;2u",
    );
...
describe("ProcessTerminal Kitty keyboard protocol negotiation", () => {
  ...
  function setupNegotiation(): NegotiationHarness {
    ...
    (
      terminal as unknown as { queryAndEnableKittyProtocol(): void }
  ...
  it("activates Kitty mode for non-zero negotiated flags", () => {
    ...
    try {
      ...
      harness.cleanup();
      assert.equal(
        harness.writes.filter((write) => write === "\x1b[<u").length,
        1,
      );
      assert.equal(harness.writes.includes("\x1b[>4;0m"), false);
    } finally {
  ...
  it("falls back to modifyOtherKeys for zero Kitty flags", () => {
    ...
    try {
      ...
      assert.equal(harness.terminal.kittyProtocolActive, false);
      assert.equal(
        harness.writes.filter((write) => write === "\x1b[>4;2m").length,
        1,
      );

      harness.cleanup();
      assert.equal(
        harness.writes.filter((write) => write === "\x1b[>4;0m").length,
        1,
      );
    } finally {
      harness.cleanup();
...
describe("ProcessTerminal dimensions", () => {
  it("falls back to COLUMNS and LINES before default dimensions", () => {
    const previousColumnsDescriptor = Object.getOwnPropertyDescriptor(
      process.stdout,
      "columns",
    );
    const previousRowsDescriptor = Object.getOwnPropertyDescriptor(
      process.stdout,
      "rows",
    );
