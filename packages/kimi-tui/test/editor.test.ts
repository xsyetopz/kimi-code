...
describe("Editor component", () => {
  ...
  describe("onRecall", () => {
    it("uses the returned text instead of the stored entry", () => {
      ...
      editor.onRecall = (entry) =>
        entry.startsWith("!") ? entry.slice(1) : undefined;
      ...
      for (const text of ["ำabc", "ຳabc"]) {
        ...
        for (const line of editor.render(width)) {
          assert.strictEqual(
            visibleWidth(line),
            width,
            `line width drift for ${JSON.stringify(text)}: ${line}`,
          );
        }
      }
    });
  ...
  describe("Undo", () => {
    ...
    it("decodes CSI-u Ctrl+letter sequences inside bracketed paste (tmux popup)", () => {
      ...
      editor.handleInput(
        "\x1b[200~line1\x1b[106;5uline2\x1b[106;5uline3\x1b[201~",
      );
      assert.strictEqual(editor.getText(), "line1\nline2\nline3");
    });
...
describe("wordWrapLine narrow width", () => {
  ...
  it("still re-wraps multi-grapheme atomic segments at narrow widths", () => {
    ...
    const preSegmented: Intl.SegmentData[] = [
      { segment: marker, index: 0, input: marker },
    ];
...
describe("Editor narrow width rendering", () => {
  ...
  it("recalls history without crashing after rendering at width 1", () => {
    ...
    assert.doesNotThrow(() => {
      (
        editor as unknown as { navigateHistory(direction: 1 | -1): void }
      ).navigateHistory(-1);
