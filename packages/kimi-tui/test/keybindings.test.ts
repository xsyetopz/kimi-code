...
describe("KeybindingsManager", () => {
  it("binds Ctrl+J as a default newline alias", () => {
    ...
    assert.deepStrictEqual(keybindings.getKeys("tui.input.newLine"), [
      "shift+enter",
      "ctrl+j",
    ]);
    assert.strictEqual(keybindings.matches("\n", "tui.input.newLine"), true);
    assert.strictEqual(
      keybindings.matches("\x1b[106;5u", "tui.input.newLine"),
      true,
    );
  });
  ...
  it("does not evict selector confirm when input submit is rebound", () => {
    ...
    assert.deepStrictEqual(keybindings.getKeys("tui.input.submit"), [
      "enter",
      "ctrl+enter",
    ]);
    assert.deepStrictEqual(keybindings.getKeys("tui.select.confirm"), [
      "enter",
    ]);
  });
  ...
  it("does not evict cursor bindings when another action reuses the same key", () => {
    ...
    assert.deepStrictEqual(keybindings.getKeys("tui.select.up"), [
      "up",
      "ctrl+p",
    ]);
  ...
  it("still reports direct user binding conflicts without evicting defaults", () => {
    ...
    assert.deepStrictEqual(keybindings.getKeys("tui.editor.cursorLeft"), [
      "left",
      "ctrl+b",
    ]);
  });
