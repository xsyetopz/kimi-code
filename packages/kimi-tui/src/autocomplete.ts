  ...
  private resolveScopedFuzzyQuery(
    ...
    if (displayBase.startsWith("~/") || displayBase.startsWith("/")) {
      const baseDir = displayBase.startsWith("~/")
        ? this.expandHomePath(displayBase)
        : displayBase;
      try {
    ...
    try {
      ...
      for (const group of perRoot) {
        for (const { entry, target } of group) {
          const score = target.fdQuery
            ? this.scoreEntry(entry.path, target.fdQuery, entry.isDirectory)
            : 1;
          if (score <= 0) continue;
          ...
          const absPath = target.absolute
            ...
            : toDisplayPath(
                join(target.pathRoot, target.displayBase, pathWithoutSlash),
              );
      ...
      for (const item of topEntries) {
        const pathWithoutSlash = item.isDirectory
          ? item.path.slice(0, -1)
          : item.path;
        const entryName = basename(pathWithoutSlash);
        let displayPath: string;
        if (item.target.absolute) {
          displayPath = this.scopedPathForDisplay(
            item.target.displayBase,
            pathWithoutSlash,
          );
          ...
          displayPath = item.target.displayBase
            ? this.scopedPathForDisplay(
                item.target.displayBase,
                pathWithoutSlash,
              )
            : pathWithoutSlash;
        }
        const completionPath = item.isDirectory
          ? `${displayPath}/`
          : displayPath;
