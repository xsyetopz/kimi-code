...
export class ProcessTerminal implements Terminal {
  ...
  private setupStdinBuffer(): void {
    ...
    this.stdinBuffer.on("data", (sequence) => {
      const negotiationSequence =
        this.readKeyboardProtocolNegotiationSequence(sequence);
  ...
  private readKeyboardProtocolNegotiationSequence(
    ...
    if (this.keyboardProtocolNegotiationBuffer) {
      const bufferedSequence =
        this.keyboardProtocolNegotiationBuffer + sequence;
      const negotiationSequence =
        parseKeyboardProtocolNegotiationSequence(bufferedSequence);
    ...
    const negotiationSequence =
      parseKeyboardProtocolNegotiationSequence(sequence);
  ...
  private enableWindowsVTInput(): void {
    ...
    try {
      ...
      for (const modulePath of candidates) {
        try {
          const helper = cjsRequire(modulePath) as {
  ...
  async drainInput(maxMs = 1000, idleMs = 50): Promise<void> {
    const shouldDisableKittyProtocol =
      this.keyboardProtocolPushed || this._kittyProtocolActive;
    this.clearKeyboardProtocolNegotiationBuffer();
