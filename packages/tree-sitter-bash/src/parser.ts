...
const FILE_REDIRECT_OP_SET: ReadonlySet<string> = new Set(
  FILE_REDIRECT_OPERATORS,
);
const DECLARATION_COMMAND_SET: ReadonlySet<string> = new Set(
  DECLARATION_COMMAND_KEYWORDS,
);
...
export class Parser {
  ...
  private parseStatementList(options: StatementListOptions = {}): Frame[] {
    ...
    for (;;) {
      ...
      if (
        token.type === "word" &&
        options.stopWords?.has(this.tokenText(token)) === true
      ) {
        if (needTerminator && options.terminatorRequired === true)
          this.hasError = true; // missing separator
        break;
      }
      ...
      if (
        ...
        this.lexer.next();
        children.push(
          this.frame("ERROR", token.start, token.end, [
  ...
  private parseSubshell(): Frame {
    ...
    if (this.scopeDepth >= MAX_PARSE_DEPTH) {
      ...
      return this.frame("ERROR", open.start, end, [
  ...
  private parseIfStatement(): Frame {
    ...
    if (!this.consumeKeyword(kids, "then")) {
      ...
      return this.frame(
        "if_statement",
        ifToken.start,
        this.endOf(kids, ifToken.end),
        kids,
      );
    }
    kids.push(
      ...this.parseStatementList({
        stopWords: STOP_IF_BODY,
        terminatorRequired: true,
      }),
    );
  ...
  private parseForStatement(): Frame {
    ...
    if (
      keyword === "for" &&
      next.type === "word" &&
      this.tokenText(next).startsWith("((")
    ) {
  ...
  private parseCaseItem(): Frame {
    ...
    for (;;) {
      ...
      kids.push(
        ...this.parseCasePattern(token.start, alt.end, firstAlternative),
      );
  ...
  private parseCasePattern(
    ...
    if (group !== null) {
      if (this.extglobGroupAccepted(start, end))
        return [this.frame("extglob_pattern", start, end)];
      this.hasError = true;
      return [this.frame("ERROR", start, end)];
    }
    if (!/["'$`]/.test(raw)) {
      // Bare pattern.
      if (this.isBareGlobPattern(start, end))
        return [this.frame("extglob_pattern", start, end)];
  ...
  private isFunctionDefinitionAhead(): boolean {
    ...
    if (!FUNCTION_NAME_RE.test(text) || RESERVED_WORD_SET.has(text))
      return false;
  ...
  private parseDeclarationCommand(): Frame {
    const kwToken = this.lexer.next();
    const kids: Frame[] = [
      this.anon(this.tokenText(kwToken), kwToken.start, kwToken.end),
    ];
    for (;;) {
      const token = this.lexer.peek();
  ...
  private parseCommand(): Frame | null {
    ...
    const start =
      prefix.length > 0 ? prefix[0]!.start : this.lexer.peek().start;
  ...
  private parseCommandArgument(): Frame[] {
    ...
    if (
      ...
      return [
        this.anon("$", start, start + 1),
        this.parseLiteral(start + 1, end),
      ];
    }
    return [this.parseLiteral(start, end)];
  }
  ...
  private parseRedirect(maxDestinations = Number.POSITIVE_INFINITY): Frame {
    ...
    if (isFileRedirectOp(op))
      return this.parseFileRedirect(descriptor, maxDestinations);
    this.hasError = true;
  ...
  private parseHeredocRedirect(descriptor: Frame | null): Frame {
    ...
    this.addKid(
      redirect,
      this.frame("heredoc_start", startToken.start, startToken.end),
    );
  ...
  private swallowAfterHeredoc(redirect: Frame): void {
    for (;;) {
      ...
      if (op === "|" || op === "|&") {
        ...
        let end = token.end;
        if (this.isStatementStart(this.lexer.peek())) {
          const inner = this.parsePipelineTail(
            this.parseStatementNotPipeline(),
          );
          kids.push(inner);
          end = inner.end;
        } else {
  ...
  private completeHeredocs(bodies: HeredocBody[]): void {
    for (const body of bodies) {
      ...
      const content = spec.quoted
        ? []
        : this.parseHeredocContent(body.bodyStart, body.bodyEnd);
      this.addKid(
        frame,
        this.frame("heredoc_body", body.bodyStart, body.bodyEnd, content),
      );
      let end = body.bodyEnd;
      if (body.found) {
        this.addKid(
          frame,
          this.frame("heredoc_end", body.endStart, body.endEnd),
        );
        end = body.endEnd;
      } else {
  ...
  private parseLiteralPieces(start: number, end: number): Frame {
    ...
    while (i < end) {
      ...
      if (ch === "'") {
        ...
        if (close >= end && this.source[close - 1] !== "'")
          this.hasError = true; // unterminated raw string
      ...
      if (ch === "$") {
        ...
        if (this.source[i + 1] === '"' && i === start) {
          ...
          pieces.push(
            this.frame("translated_string", i, next, [
              this.anon("$", i, i + 1),
              translated,
            ]),
          );
    ...
    if (pieces.length === 1) {
      ...
      if (
        only.type === "word" &&
        NUMBER_RE.test(this.text(only.start, only.end))
      ) {
  ...
  private parseString(start: number, rangeEnd: number): [Frame, number] {
    const string = this.frame("string", start, start + 1, [
      this.anon('"', start, start + 1),
    ]);
  ...
  private parseDollar(i: number, rangeEnd: number): [Frame, number] | null {
    ...
    if (next === "(" && i + 1 < rangeEnd) {
      if (this.source[i + 2] === "(")
        return this.parseArithmeticExpansion(i, rangeEnd);
      return this.parseCommandSubstitution(i, rangeEnd);
    }
    if (next === "{" && i + 1 < rangeEnd)
      return this.parseExpansion(i, rangeEnd);
    if (next === "[" && i + 1 < rangeEnd)
      return this.parseBracketArithmetic(i, rangeEnd);
    if (next === "'" && i + 1 < rangeEnd)
      return this.parseAnsiCString(i, rangeEnd);
    ...
    if (
      next !== undefined &&
      i + 1 < rangeEnd &&
      SPECIAL_VARIABLE_CHARS.includes(next)
    ) {
  ...
  private parseExpansionInner(i: number, rangeEnd: number): [Frame, number] {
    ...
    const expansion = this.frame("expansion", i, close, [
      this.anon("${", i, i + 2),
    ]);
    let j = i + 2;
    ...
    if (j < innerEnd && /[\w]/.test(this.source[j]!)) {
      let nameEnd = j;
      while (nameEnd < innerEnd && /[\w]/.test(this.source[nameEnd]!))
        nameEnd++;
      // ${0} is a special_variable_name; other digits are variable_name.
      ...
      this.addKid(
        expansion,
        this.frame(
          special ? "special_variable_name" : "variable_name",
          j,
          nameEnd,
        ),
      );
      ...
      j < innerEnd &&
      SPECIAL_VARIABLE_CHARS.includes(this.source[j]!)
    ) {
    ...
    if (j < innerEnd && this.source[j] === "[" && hasName) {
      ...
      if (sub.balanced)
        this.addKid(subscript, this.anon("]", subEnd - 1, subEnd));
  ...
  private parseExpansionInfix(
    ...
    if (two === "##" || two === "%%" || one === "#" || one === "%") {
      ...
      return this.parseExpansionPattern(
        expansion,
        j + operator.length,
        innerEnd,
      );
    ...
    if (two === "//" || two === "/#" || two === "/%" || one === "/") {
      ...
      this.addKid(expansion, this.anon(operator, j, j + operator.length));
      let pos = this.parseExpansionPattern(
        expansion,
        j + operator.length,
        innerEnd,
        "/",
    ...
    for (const operator of [":-", ":=", ":+", ":?", "-", "=", "+", "?"]) {
      if (
        this.source.startsWith(operator, j) &&
        j + operator.length <= innerEnd
      ) {
        this.addKid(expansion, this.anon(operator, j, j + operator.length));
  ...
  private parseExpansionPattern(
    ...
    if (
      pos > j &&
      (pos >= innerEnd || (stop !== undefined && this.source[pos] === stop))
    ) {
  ...
  private parseExpansionValue(start: number, end: number): Frame[] {
    ...
    const pieces =
      value.type === "concatenation" ? [...value.children] : [value];
    ...
    for (let p = 0; p < pieces.length; p++) {
      ...
      if (
        ...
        trimmed.push(
          newEnd === piece.end
            ? piece
            : this.frame("word", piece.start, newEnd),
        );
      } else {
        trimmed.push(piece);
      }
    ...
    if (trimmed.length === 2 && trimmed[0]!.type === "command_substitution")
      return trimmed;
  ...
  private parseMaxLengthValue(
    ...
    if (end > j) {
      ...
      if (negative !== null) {
        ...
        this.addKid(
          expansion,
          this.frame("number", start, start + negative[1]!.length),
        );
        return end;
      }
  ...
  private parseCommandSubstitution(
    ...
    const scan = scanBalancedStatements(
      this.source,
      this.budget,
      i + 1,
      rangeEnd,
    ...
    const substitution = this.frame("command_substitution", i, close, [
      this.anon("$(", i, i + 2),
    ]);
    ...
    if (scan.balanced)
      this.addKid(substitution, this.anon(")", close - 1, close));
  ...
  private parseArithmeticExpansion(
    ...
    const closed =
      scan.balanced && close - 2 >= i + 3 && this.source[close - 2] === ")";
    if (!closed) this.hasError = true;
  ...
  private parseParenArithmetic(i: number, rangeEnd: number): [Frame, number] {
    ...
    const close = scan.end;
    const closed =
      scan.balanced && close - 2 >= i + 2 && this.source[close - 2] === ")";
    if (!closed) this.hasError = true;
  ...
  private parseExpression(st: ExprState, minPrecedence: number): Frame | null {
    ...
    if (head.kind === "op" && (head.text === "++" || head.text === "--")) {
      ...
      if (demote) {
        ...
        const kids: Frame[] = [
          this.frame("test_operator", head.start, head.end),
        ];
    ...
    for (;;) {
      ...
      const operator =
        token.kind === "testop"
          ...
          : this.anon(token.text, token.start, token.end);
      const kids: Frame[] =
        right === null ? [left, operator] : [left, operator, right];
      left = this.frame(
        "binary_expression",
        left.start,
        this.endOf(kids, token.end),
  ...
  private parseTestRegex(st: ExprState): Frame[] | null {
    ...
    if (ch === "'") {
      ...
      if (/^'[^']*'\w*$/.test(this.text(i, j)))
        return [this.parseLiteral(i, j)];
   ...
   */
  private tryParseTestPattern(
    st: ExprState,
  ): { frames: Frame[]; end: number } | null {
    let i = st.pos;
    while (
      i < st.end &&
      (this.source[i] === " " ||
  ...
  private convertTestRightSide(
    ...
    if (
      right.type !== "word" &&
      !(right.type === "concatenation" && this.isBarePieces(right))
    ) {
      return right;
    }
  ...
  private isBarePieces(frame: Frame): boolean {
    return frame.children.every(
      (child) => child.type === "word" || child.type === "number",
    );
  }
  ...
  private isTestExtglob(right: Frame): boolean {
    ...
    if (i >= text.length) {
      ...
      if (
        (c0 === ")" || c0 === "*") &&
        nextChar === ")" &&
        isSpace(this.source[right.end + 1])
      ) {
    ...
    if (c0 !== "[" && !isAlnum(text[i]!) && !"[?/\\_*".includes(text[i]!))
      return false;
    for (; i < text.length; i++) {
      const ch = text[i]!;
      if (ch === "\\") {
  ...
  private scanExprToken(st: ExprState): ExprToken {
    ...
    if (ch >= "0" && ch <= "9") {
      ...
      if (
        ch === "0" &&
        (this.source[i + 1] === "x" || this.source[i + 1] === "X")
      ) {
        j = i + 2;
  ...
  private parseTestCommand(): Frame {
    ...
    if (
      ...
      const statements = this.parseScopedStatements(
        exprStart,
        scan.closerStart,
      );
