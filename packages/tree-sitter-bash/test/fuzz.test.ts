...
function fixtureSources(): string[] {
  ...
  for (const file of readdirSync(dir)
    ...
    .toSorted()) {
    for (const sample of parseFixtureFile(
      file,
...
describe("fuzz: token soup", () => {
  it("never throws and always yields a sound tree or a clean abort", () => {
    ...
    console.log(
      `token soup: ${count} inputs, ${parsed} parsed / ${count - parsed} aborted`,
    );
  });
});
  ...
  it("never throws and always yields a sound tree or a clean abort", () => {
    ...
    for (let n = 0; n < count; n++) {
      ...
      if (mode < 0.4) {
        ...
        mutated =
          base.slice(0, pos) + String.fromCodePoint(code) + base.slice(pos + 1);
      } else if (mode < 0.7) {
        mutated = base.slice(0, pos) + base.slice(pos + 1);
      } else {
        const code = Math.floor(rand() * 256);
        mutated =
          base.slice(0, pos) + String.fromCodePoint(code) + base.slice(pos);
      }
      if (checkContract(mutated).ok) parsed++;
    }
    expect(parsed).toBeGreaterThan(0);
    console.log(
      `byte mutations: ${count} inputs, ${parsed} parsed / ${count - parsed} aborted`,
    );
  });
});

describe("fuzz: nesting bombs degrade locally per the documented depth caps", () => {
  const substitution = (depth: number): string =>
    `echo ${"$(".repeat(depth)}x${")".repeat(depth)}`;
  const subshell = (depth: number): string =>
    `${"(".repeat(depth)}x${")".repeat(depth)}`;
  const expansion = (depth: number): string =>
    `echo ${"${a:-".repeat(depth)}z${"}".repeat(depth)}`;
  const ifs = (depth: number): string =>
    `${"if x; then ".repeat(depth)}y${"; fi".repeat(depth)}`;
