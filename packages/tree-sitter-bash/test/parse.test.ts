...
function sexp(node: SyntaxNode): string {
  if (node.children.length === 0) {
    return node.isNamed
      ? `(${node.type} ${JSON.stringify(node.text)})`
...
describe("parse entry contract", () => {
  ...
  it('returns { ok: false, reason: "aborted" } when the deadline has passed', () => {
    expect(parse("echo hello", { timeoutMs: 0 })).toEqual({
      ok: false,
      reason: "aborted",
    });
  });
  ...
  it("aborts instead of hanging on tens of thousands of statements", () => {
    expect(parse("ls; ".repeat(20_000))).toEqual({
      ok: false,
      reason: "aborted",
    });
  });
...
describe("simple commands", () => {
  it("parses a command with arguments", () => {
    expectTree(
      "ls -la",
      `(program (command (command_name (word "ls")) (word "-la")))`,
    );
  });
  ...
  it("treats a lone $ as an anonymous argument", () => {
    expectTree(
      "echo $",
      `(program (command (command_name (word "echo")) "$"))`,
    );
  });
});
...
describe("redirects", () => {
  ...
  it("parses a redirect-only statement", () => {
    expectTree(
      "> out",
      `(program (redirected_statement (file_redirect ">" (word "out"))))`,
    );
  });
...
describe("statement lists", () => {
  ...
  it("parses a subshell", () => {
    expectTree(
      "(ls)",
      `(program (subshell "(" (command (command_name (word "ls"))) ")"))`,
    );
  });
  ...
  it("recovers a trailing && with a partial list and hasError", () => {
    expectTree(
      "ls &&",
      `(program (list (command (command_name (word "ls"))) "&&"))`,
      true,
    );
  });
  ...
  it("recovers a trailing | with a partial pipeline and hasError", () => {
    expectTree(
      "ls |",
      `(program (pipeline (command (command_name (word "ls"))) "|"))`,
      true,
    );
  });
...
describe("adversarial input", () => {
  ...
  it("parses a multi-megabyte single word without throwing", () => {
    ...
    expect(result.hasError).toBe(false);
    const word =
      result.rootNode.namedChildren[0]!.namedChildren[0]!.namedChildren[0]!;
