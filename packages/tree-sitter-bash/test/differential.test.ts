...
import {
  KNOWN_DIFFERENCES,
  isKnownDifferenceId,
} from "./helpers/known-differences";
...
const corpusCases = readdirSync(CORPUS_DIR)
  ...
  .toSorted()
  .flatMap((file) =>
    parseCorpusFile(readFileSync(path.join(CORPUS_DIR, file), "utf8")).map(
      (c) => ({ ...c, file }),
    ),
  );
    ...
    "known-diff": () => {
      it(label(sample), async () => {
        const [ours, reference] = [
          ourDump(sample.source),
          await referenceDump(sample.source),
        ];
        expect(
          ours !== reference,
...
function registerCorpusTest(corpusCase: {
  file: string;
  name: string;
  input: string;
}): void {
...
describe("known-difference registry consistency", () => {
  ...
  it("every @known-diff id used in fixtures exists in the registry", () => {
    for (const id of usedIds) {
      expect(
        isKnownDifferenceId(id),
        `fixture uses unregistered id '${id}'`,
      ).toBe(true);
    }
  });
  ...
  it("every registry id is exercised by at least one fixture or corpus case", () => {
    for (const entry of KNOWN_DIFFERENCES) {
      expect(
        ...
        `registry id '${entry.id}' has no fixture sample`,
      ).toBe(true);
    }
  });
  ...
  it("every registry entry is anchored in README.md Known differences", () => {
    ...
    expect(
      sectionMatch,
      'README.md has no "## Known differences" section',
    ).not.toBeNull();
