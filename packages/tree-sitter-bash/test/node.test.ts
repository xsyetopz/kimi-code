...
function buildTree() {
  ...
  name1.addChild(
    createNode({ type: "word", source: SOURCE, startIndex: 0, endIndex: 4 }),
  );
  cmd1.addChild(
    createNode({ type: "word", source: SOURCE, startIndex: 5, endIndex: 8 }),
  );
  program.addChild(
    createNode({
      type: ";",
  ...
  cmd2.addChild(
    createNode({ type: "word", source: SOURCE, startIndex: 15, endIndex: 18 }),
  );
  return program;
}
...
describe("createNode", () => {
  ...
  it("rejects out-of-range offsets", () => {
    expect(() =>
      createNode({ type: "word", source: SOURCE, startIndex: -1, endIndex: 2 }),
    ).toThrow(RangeError);
    expect(() =>
      createNode({ type: "word", source: SOURCE, startIndex: 3, endIndex: 2 }),
    ).toThrow(RangeError);
    expect(() =>
      createNode({
        type: "word",
  ...
  it("rejects overlapping or out-of-order siblings", () => {
    ...
    parent.addChild(
      createNode({ type: "word", source: SOURCE, startIndex: 2, endIndex: 5 }),
    );
    expect(() =>
      parent.addChild(
        createNode({
...
describe("children vs namedChildren", () => {
  it("keeps anonymous nodes out of namedChildren but in children", () => {
    ...
    expect(program.children.map((c) => c.type)).toEqual([
      "command",
      ";",
      "command",
    ]);
    expect(program.namedChildren.map((c) => c.type)).toEqual([
      "command",
      "command",
    ]);
