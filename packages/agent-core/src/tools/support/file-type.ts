...
export const IMAGE_MIME_BY_SUFFIX: Readonly<Record<string, string>> =
  Object.freeze({
    ".png": "image/png",
    ".jpg": "image/jpeg",
    ".jpeg": "image/jpeg",
...
export const VIDEO_MIME_BY_SUFFIX: Readonly<Record<string, string>> =
  Object.freeze({
    ".mp4": "video/mp4",
    ".mpg": "video/mpeg",
    ".mpeg": "video/mpeg",
...
export function sniffMediaFromMagic(
  data: Buffer | Uint8Array,
): FileType | null {
  const buf = toBuffer(data);
  const header =
    buf.length > MEDIA_SNIFF_BYTES ? buf.subarray(0, MEDIA_SNIFF_BYTES) : buf;
  ...
  if (
    startsWith(header, Buffer.from("GIF87a")) ||
    startsWith(header, Buffer.from("GIF89a"))
  ) {
...
export function sniffImageDimensions(
  ...
  // offsets 6 and 8.
  if (
    (startsWith(buf, Buffer.from("GIF87a")) ||
      startsWith(buf, Buffer.from("GIF89a"))) &&
    buf.length >= 10
  ) {
