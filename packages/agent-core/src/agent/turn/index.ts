...
export class TurnFlow {
  ...
  private readonly interruptedTraceIdByTurn = new Map<
    number,
    string | undefined
  >();
  private readonly stepFailureByTurn = new Map<
    number,
  ...
  private async driveGoal(
    ...
    while (true) {
      ...
      if (
        goalBeforeTurn?.status === "active" &&
        goalBeforeTurn.budget.overBudget
      ) {
        await this.agent.goal.markBlocked({
          reason: "A configured budget was reached",
        });
      ...
      if (
        ...
        end.blockedByUserPromptHook === true
      ) {
        await this.agent.goal.markBlocked({
    ...
    try {
      ...
      if (isAbortError(error)) {
        ...
        if (this.shouldTrackApiError(turnId)) {
          ...
          const inputTokens = currentTurnInputTokens(
            this.agent.usage.data().currentTurn,
          );
  ...
  private async applyUserPromptHook(
    ...
    signal.throwIfAborted();
    const promptHookResults = await this.agent.hooks?.trigger(
      "UserPromptSubmit",
      {
        matcherValue: input,
        signal,
    ...
    this.agent.context.appendUserMessage(
      [{ type: "text", text: hookResult.text }],
      {
        kind: "hook_result",
        event: "UserPromptSubmit",
      },
  ...
  private async runStepLoop(
    ...
    while (true) {
      ...
      const maxStepsPerTurn = resolveMaxStepsPerTurn(
        loopControl?.maxStepsPerTurn,
      );
      const maxRetriesPerStep = resolveMaxRetriesPerStep(
      ...
      try {
        const result = await runTurn({
          ...
          buildMessagesMediaDegraded: () =>
            this.agent.context.mediaDegradedMessages,
          ...
          describeMissingTool: (name) =>
            this.agent.tools.missingToolMessage(name),
          log: this.agent.log,
          maxSteps: maxStepsPerTurn,
          maxRetryAttempts: maxRetriesPerStep,
          recordStepUsage: async (usage) => {
            try {
              const snapshot = await this.agent.goal.recordTokenUsage(
                usage.output,
              );
          ...
          hooks: {
            ...
            finalizeToolResult: async (ctx) => {
              ...
              const event =
                isError === true ? "PostToolUseFailure" : "PostToolUse";
              void this.agent.hooks?.fireAndForgetTrigger(event, {
                ...
                inputData: {
                  ...
                  error:
                    isError === true
                      ? toKimiErrorPayload(toolOutputText(output))
                      : undefined,
                  toolOutput:
                    isError === true
                      ? undefined
                      : toolOutputText(output).slice(0, 2000),
                },
              });
        ...
        if (
          isContextOverflow ||
          this.agent.fullCompaction.shouldRecoverFromContextOverflow(
            error,
  ...
  private trackToolLifecycle(event: LoopEvent, turnId: number): void {
    ...
    if (event.type === "tool.result") {
      ...
      const errorType =
        outcome === "error" ? telemetryToolErrorType(event.result) : undefined;
 ...
 */
export function resolveMaxStepsPerTurn(
  configValue?: number,
): number | undefined {
 ...
 */
export function resolveMaxRetriesPerStep(
  configValue?: number,
): number | undefined {
...
function toolOutputText(output: ExecutableToolResult["output"]): string {
  ...
  return output
    .filter(
      (part): part is Extract<(typeof output)[number], { type: "text" }> => {
        return (
...
function telemetryInterruptReason(
  reason:
    | LoopTurnInterruptedEvent["reason"]
    | Exclude<TurnEndedEvent["reason"], "completed">,
...
function classifyApiError(
  ...
  if (statusCode !== undefined) {
    ...
    if (statusCode === 401 || statusCode === 403)
      return { errorType: "auth", statusCode };
  ...
  if (summary.code === ErrorCodes.PROVIDER_RATE_LIMIT)
    return { errorType: "rate_limit" };
  if (summary.code === ErrorCodes.PROVIDER_AUTH_ERROR)
    return { errorType: "auth" };
  if (summary.code === ErrorCodes.CONTEXT_OVERFLOW)
  ...
  if (isApiEmptyResponseError(error, summary))
...
function isApiConnectionError(
  error: unknown,
  summary: KimiErrorPayload,
): boolean {
  return (
    error instanceof APIConnectionError || summary.name === "APIConnectionError"
  );
}
...
function isApiEmptyResponseError(
  error: unknown,
  summary: KimiErrorPayload,
): boolean {
  return (
...
type ToolTelemetryResult = Extract<
  LoopEvent,
  { type: "tool.result" }
>["result"];

function telemetryToolOutcome(
...
function telemetryToolErrorType(result: ToolTelemetryResult): string {
  ...
  if (text.startsWith('Tool "') && text.includes('" not found'))
    return "ToolNotFound";
