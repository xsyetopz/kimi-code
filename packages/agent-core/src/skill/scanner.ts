...
export async function resolveSkillRoots(
  ...
  const brandHomeDir =
    options.paths.brandHomeDir ?? path.join(userHomeDir, ".kimi-code");
...
export function extendWorkspaceWithSkillRoots<
  T extends WorkspaceWithAdditionalDirs,
>(workspace: T, skillRoots: readonly string[]): T {
  const additionalDirs = [...workspace.additionalDirs];
  for (const root of skillRoots) {
    if (isWithin(root, workspace.workspaceDir)) continue;
    if (additionalDirs.some((dir) => root === dir || isWithin(root, dir)))
      continue;
