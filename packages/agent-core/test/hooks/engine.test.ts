...
describe("HookEngine", () => {
  ...
  it("preserves a PreToolUse block result even when telemetry throws (no fail-open)", async () => {
    ...
    const telemetry = await import("../../src/utils/telemetry" as string).catch(
      () => null,
    );
    const { HookEngine } = await importEngine();
    const engine = new HookEngine([
    ...
    const spy =
      telemetry &&
      typeof (telemetry as { track?: unknown }).track === "function"
        ? vi
            .spyOn(
              telemetry as { track: (...args: unknown[]) => unknown },
              "track",
            )
            .mockImplementation(() => {
    ...
    const engine = new HookEngine([
      {
        event: "PreToolUse",
        command:
          "node -e \"process.stdout.write(process.env.KIMI_PLUGIN_TEST ?? 'missing')\"",
        timeout: 5,
