...
describe("resolveGithubSource", () => {
    ...
    mockSequence([
      {
        status: 302,
        headers: {
          location: "https://github.com/o/r/releases/tag/feat%2Frelease",
        },
      },
    ]);
    ...
    mockSequence([
      {
        status: 302,
        headers: {
          location: "https://github.com/o/r/releases/tag/release%231",
        },
      },
    ]);
    ...
    mockSequence([
      {
        status: 302,
        headers: {
          location: "https://github.com/wbxl2000/superpowers/releases",
        },
      },
      { status: 200 }, // codeload HEAD probe
    ]);
  ...
  it("bare URL: 5xx on /releases/latest throws instead of silently falling back", async () => {
    mockSequence([{ status: 503, statusText: "Service Unavailable" }]);
  ...
  it("bare URL: 429 (rate-limit-style) on /releases/latest throws, not falls back", async () => {
    mockSequence([{ status: 429, statusText: "Too Many Requests" }]);
  ...
  it("bare URL: 403 (WAF/abuse-detection-style) on /releases/latest throws, not falls back", async () => {
    mockSequence([{ status: 403, statusText: "Forbidden" }]);
  ...
  it("release-lookup error message hints at the /tree/<ref> escape hatch", async () => {
    mockSequence([{ status: 502, statusText: "Bad Gateway" }]);
  ...
  it("does not call api.github.com at all on bare URL", async () => {
    ...
    expect(calls.every((u) => !u.startsWith("https://api.github.com"))).toBe(
      true,
    );
  });
});
