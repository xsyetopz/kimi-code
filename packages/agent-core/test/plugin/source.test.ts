...
describe("resolveInstallSource", () => {
  ...
  it("recognizes http:// as zip-url", () => {
    ...
    expect(result).toEqual({
      kind: "zip-url",
      path: "http://example.com/plugin.zip",
    });
  });
  ...
  it("throws for relative local paths", () => {
    expect(() => resolveInstallSource("relative/path")).toThrow(
      /absolute path/i,
    );
  });
  ...
  describe("GitHub URL recognition", () => {
    ...
    it("recognizes www.github.com as a synonym", () => {
      const result = resolveInstallSource(
        "https://www.github.com/wbxl2000/superpowers",
      );
      expect(result).toEqual({
        kind: "github",
        owner: "wbxl2000",
    ...
    it("recognizes /tree/<short-sha> as sha", () => {
      const result = resolveInstallSource(
        "https://github.com/obra/superpowers/tree/45b441d",
      );
      expect(result).toEqual({
        kind: "github",
        owner: "obra",
    ...
    it("recognizes /tree/<full-sha> as sha", () => {
      ...
      const result = resolveInstallSource(
        `https://github.com/obra/superpowers/tree/${sha}`,
      );
      expect(result).toEqual({
        kind: "github",
        owner: "obra",
    ...
    it("preserves slashes inside branch names under /tree/", () => {
      const result = resolveInstallSource(
        "https://github.com/owner/repo/tree/feat/foo-bar",
      );
      expect(result).toEqual({
        kind: "github",
        owner: "owner",
