...
describe("parseManifest", () => {
  ...
  it("prefers root kimi.plugin.json when .kimi-plugin/plugin.json also exists", async () => {
    ...
    expect(result.shadowedManifestPath).toBe(
      ...
      expect(result.diagnostics).toContainEqual(
        expect.objectContaining({
          ...
          message: expect.stringContaining(
            `"${field}" is present but not supported`,
          ),
        }),
      );
    }
  ...
  it("parses plugin mcpServers", async () => {
    ...
    await writeFile(
      path.join(root, "bin", "finance-mcp"),
      "#!/bin/sh\n",
      "utf8",
    );
    const result = await parseManifest(root);
  ...
  it("warns and skips invalid plugin mcpServers entries", async () => {
    ...
    expect(result.diagnostics).toContainEqual(
      expect.objectContaining({
        severity: "warn",
        message: expect.stringContaining(
          'must be a PATH command or start with "./"',
        ),
      }),
    );
  });
  ...
  it("recurses into nested command directories and preserves the namespace", async () => {
    ...
    expect(result.manifest?.commands).toEqual([
      { path: path.join(root, "commands/deploy.md"), name: "deploy" },
      {
        path: path.join(root, "commands/frontend/component.md"),
        name: "frontend/component",
      },
      {
        path: path.join(root, "commands/frontend/deep/nested.md"),
        name: "frontend/deep/nested",
      },
    ]);
  });
  ...
  it("warns on invalid systemPromptPath and keeps the inline systemPrompt", async () => {
    ...
    expect(nonStringResult.diagnostics).toContainEqual(
      expect.objectContaining({
        severity: "warn",
        message: '"systemPromptPath" must be a string',
      }),
    );
  ...
  it("warns on a blank systemPromptPath", async () => {
    ...
    expect(result.diagnostics).toContainEqual(
      expect.objectContaining({
        severity: "warn",
        message: '"systemPromptPath" must not be blank',
      }),
    );
  });
  ...
  it("accepts system-prompt content exactly at the byte limit", async () => {
    ...
    const result = await parseManifest(root);
    expect(result.manifest?.systemPrompt).toBe(
      "x".repeat(PLUGIN_SYSTEM_PROMPT_MAX_BYTES),
    );
    expect(result.diagnostics).toEqual([]);
  });
});
