...
function toolWithContent(
  ...
  return new ReadTool(
    createFakeKaos({
      ...
      readBytes: vi
        .fn<Kaos["readBytes"]>()
        .mockImplementation(async (_path, n) => {
          return n === undefined ? bytes : bytes.subarray(0, n);
        }),
      readLines: vi
        .fn<Kaos["readLines"]>()
        .mockImplementation(readLinesFromContent(content)),
...
describe("ReadTool", () => {
  it("exposes current metadata and schema", () => {
    ...
    expect(ReadInputSchema.safeParse({ path: "/tmp/test.txt" }).success).toBe(
      true,
    );
    expect(
    ...
    expect(
      ReadInputSchema.safeParse({ path: "/tmp/test.txt", line_offset: 0 })
        .success,
    ).toBe(false);
    expect(
      ReadInputSchema.safeParse({
        path: "/tmp/test.txt",
        line_offset: -(MAX_LINES + 1),
      }).success,
    ).toBe(false);
  });
  ...
  it("returns an empty successful output when line_offset is beyond EOF", async () => {
    ...
    const result = await executeTool(
      tool,
      context({ path: "/tmp/a.txt", line_offset: 20 }),
    );
  ...
  it("supports negative line_offset as tail mode with absolute line numbers", async () => {
    ...
    const result = await executeTool(
      tool,
      context({ path: "/tmp/a.txt", line_offset: -3 }),
    );
  ...
  it("returns a friendly error for missing files before sniffing bytes", async () => {
    ...
    const readBytes = vi
      .fn<Kaos["readBytes"]>()
      .mockRejectedValue(
        new Error("readBytes should not be called for missing files"),
      );
    const readLines = vi
      .fn<Kaos["readLines"]>()
      .mockImplementation(readLinesFromContent(""));
    const tool = new ReadTool(
      createFakeKaos({
    ...
    const result = await executeTool(
      tool,
      context({ path: "/workspace/missing.txt" }),
    );
  ...
  it("returns a friendly error for directories before sniffing bytes", async () => {
    const readBytes = vi
      .fn<Kaos["readBytes"]>()
      .mockRejectedValue(
        new Error("readBytes should not be called for directories"),
      );
    const readLines = vi
      .fn<Kaos["readLines"]>()
      .mockImplementation(readLinesFromContent(""));
  ...
  it("rejects an image-extension file whose bytes are not an image as not readable", async () => {
    ...
    const readText = vi
      .fn<Kaos["readText"]>()
      .mockRejectedValue(
        new Error("readText should not be called for non-image files"),
      );
    const tool = new ReadTool(
      createFakeKaos({
        stat: vi.fn<Kaos["stat"]>().mockResolvedValue(REGULAR_FILE_STAT),
  ...
  it("rejects extensionless image files using magic-byte sniffing", async () => {
    ...
    const readText = vi
      .fn<Kaos["readText"]>()
      .mockRejectedValue(
        new Error("readText should not be called for extensionless images"),
      );
    const tool = new ReadTool(
      createFakeKaos({
        stat: vi.fn<Kaos["stat"]>().mockResolvedValue(REGULAR_FILE_STAT),
  ...
  it("rejects NUL-containing binary files before text decoding", async () => {
    const header = Buffer.concat([
      Buffer.from("plain prefix"),
      Buffer.from([0x00, 0x01]),
    ]);
    const readText = vi
      .fn<Kaos["readText"]>()
      .mockRejectedValue(
        new Error("readText should not be called for binary files"),
      );
    const tool = new ReadTool(
      createFakeKaos({
        stat: vi.fn<Kaos["stat"]>().mockResolvedValue(REGULAR_FILE_STAT),
  ...
  it("rejects NUL bytes that appear after the preflight header", async () => {
    ...
    const readLines = vi
      .fn<Kaos["readLines"]>()
      .mockImplementation(async function* readLines() {
        yield "safe text\n";
    ...
    const result = await executeTool(
      tool,
      context({ path: "/tmp/blob-with-late-nul" }),
    );
    const output = toolContentString(result);
  ...
  it("rejects invalid UTF-8 instead of returning replacement characters", async () => {
    ...
    const tool = new ReadTool(
      createFakeKaos({
        ...
        readBytes: vi
          .fn<Kaos["readBytes"]>()
          .mockResolvedValue(Buffer.from("text header")),
        readLines,
      }),
      PERMISSIVE_WORKSPACE,
    );
  ...
  it("uses text preview for sniffing before falling back to readBytes", async () => {
    ...
    const readBytes = vi
      .fn<Kaos["readBytes"]>()
      .mockImplementation(async () => Buffer.from([0x89, 0x50, 0x4e, 0x47]));
    const readTextPreview = vi.fn(async (_path: string, n: number) =>
      Buffer.from(content.slice(0, n), "utf8"),
    );
    const tool = new ReadTool(
      createFakeKaos({
        stat: vi.fn<Kaos["stat"]>().mockResolvedValue(REGULAR_FILE_STAT),
    ...
    expect(readTextPreview).toHaveBeenCalledWith(
      "/tmp/acp.txt",
      MEDIA_SNIFF_BYTES,
    );
    expect(readBytes).not.toHaveBeenCalled();
  ...
  it("reads through bounded byte preflight and streams line iteration without full readText", async () => {
    ...
    const readText = vi
      ...
      .mockRejectedValue(new Error("full readText should not be called"));
    let consumed = 0;
    const readLines: Kaos["readLines"] =
    ...
    expect(result.note).toContain(
      `Total lines in file: ${String(MAX_LINES + 5)}.`,
    );
  ...
  it("uses range reader when available without consuming readLines", async () => {
    ...
    const readLineRange = vi.fn(async function* readLineRange(
      ...
      for (
        let i = options.startLine;
        i < options.startLine + options.maxLines;
        i += 1
      ) {
        yield `line ${String(i)}\n`;
      }
    ...
    const result = await executeTool(
      tool,
      context({ path: "/tmp/range.txt", line_offset: 5, n_lines: 3 }),
    );
    const output = toolContentString(result);
  ...
  it("uses tail reader when available without consuming readLines", async () => {
    ...
    const readTailLines = vi.fn(
      async function* readTailLines(): AsyncGenerator<string> {
        yield "line 18\n";
        yield "line 19\n";
        yield "line 20";
      },
    ...
    const result = await executeTool(
      tool,
      context({ path: "/tmp/tail.txt", line_offset: -3 }),
    );
    const output = toolContentString(result);

    expect(output).toContain("18\tline 18");
    expect(output).toContain("20\tline 20");
    expect(readTailLines).toHaveBeenCalledWith("/tmp/tail.txt", {
      tailCount: 3,
      errors: "strict",
    });
    expect(readLines).not.toHaveBeenCalled();
  ...
  it("short-circuits on scan NUL before range read", async () => {
    ...
    const tool = new ReadTool(
      createFakeKaos({
        ...
        readBytes: vi
          .fn<Kaos["readBytes"]>()
          .mockResolvedValue(Buffer.from("text")),
  ...
  it("caps default reads at MAX_LINES", async () => {
    ...
    expect(result.note).toContain(`Max ${String(MAX_LINES)} lines reached.`);
    expect(result.output).toContain(
      `${String(MAX_LINES)}\tline ${String(MAX_LINES)}`,
    );
    expect(result.output).not.toContain(
      `${String(MAX_LINES + 1)}\tline ${String(MAX_LINES + 1)}`,
    );
  });
  ...
  it("description pins line/byte caps, tail mode, and the Grep-over-Read preference", () => {
    ...
    expect(tool.description).toMatch(
      /negative line_offset|reads from the end/i,
    );
  ...
  it("reports nonexistent files with the expected does-not-exist phrasing", async () => {
    ...
    const result = await executeTool(
      tool,
      context({ path: "/workspace/ghost.txt" }),
    );
  ...
  it("schema validation rejects n_lines=0 and n_lines=-1 with an n_lines-keyed error", () => {
    ...
    const negative = ReadInputSchema.safeParse({
      path: "/tmp/a.txt",
      n_lines: -1,
    });
  ...
  it("schema validation accepts -1 and -MAX_LINES but rejects -(MAX_LINES + 1)", () => {
    expect(
      ReadInputSchema.safeParse({ path: "/tmp/a.txt", line_offset: -1 })
        .success,
    ).toBe(true);
    expect(
      ReadInputSchema.safeParse({ path: "/tmp/a.txt", line_offset: -MAX_LINES })
        .success,
    ).toBe(true);
    expect(
      ReadInputSchema.safeParse({
  ...
  it("reads non-sensitive dotfiles like .gitignore successfully", async () => {
    ...
    const result = await executeTool(
      tool,
      context({ path: "/workspace/.gitignore" }),
    );
  ...
  it("negative line_offset exceeding total lines returns the entire file", async () => {
    ...
    const result = await executeTool(
      tool,
      context({ path: "/tmp/short.txt", line_offset: -100 }),
    );
  ...
  it("line_offset=-1 returns only the last line with its absolute line number", async () => {
    ...
    const result = await executeTool(
      tool,
      context({ path: "/tmp/last.txt", line_offset: -1 }),
    );

    expect(result.isError).toBeFalsy();
    expect(result.output).toContain("5\te");
    expect(result.note).toContain(
      "1 line read from file starting from line 5.",
    );
  });
...
describe("ReadTool description and schema parity", () => {
  it("encourages reading multiple files in parallel", () => {
    ...
    expect(tool.description).toMatch(
      /multiple `Read` calls in a single response/i,
    );
  });
  ...
  it("warns that sensitive files are refused", () => {
    ...
    expect(tool.description).toMatch(
      /sensitive|credential|secret|\.env|SSH key/i,
    );
  });
