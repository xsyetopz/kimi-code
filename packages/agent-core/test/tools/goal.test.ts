...
describe("CreateGoalTool", () => {
  ...
  it("omits the internal goalId from the model-facing output", async () => {
    ...
    expect(result.output).not.toContain(
      store.getGoal().goal?.goalId ?? "no-id",
    );
  });
  ...
  it("rejects empty and too-long objectives via the store", async () => {
    ...
    const tool = new CreateGoalTool(fakeAgent({ goal: store }));
    await expect(
      executeTool(tool, ctx({ objective: "   " })),
    ).rejects.toMatchObject({
      code: ErrorCodes.GOAL_OBJECTIVE_EMPTY,
    });
    await expect(
      executeTool(tool, ctx({ objective: "x".repeat(4001) })),
    ).rejects.toMatchObject({
  ...
  it("warns that creating fails when a goal already exists", () => {
    const description = new CreateGoalTool(
      fakeAgent(),
...
describe("GetGoalTool", () => {
  ...
  it("returns paused and blocked snapshots", async () => {
    ...
    let parsed = JSON.parse(
      (await executeTool(tool, ctx({}))).output as string,
    );
...
describe("SetGoalBudgetTool", () => {
  ...
  it("advertises an object parameter schema for OpenAI-compatible providers", () => {
    ...
    expect(
      validateToolArgs(validator, { value: 1.5, unit: "turns" }),
    ).toBeNull();
    expect(
      validateToolArgs(validator, { value: 1.5, unit: "hours" }),
    ).toBeNull();
  });
  ...
  it("accepts a value with a supported budget unit", () => {
    for (const unit of [
      ...
      expect(
        SetGoalBudgetToolInputSchema.safeParse({ value: 20, unit }).success,
      ).toBe(true);
    }
    expect(
      SetGoalBudgetToolInputSchema.safeParse({ value: 0, unit: "turns" })
        .success,
    ).toBe(false);
    expect(
      SetGoalBudgetToolInputSchema.safeParse({ value: 1, unit: "years" })
        .success,
    ).toBe(false);
    expect(
      SetGoalBudgetToolInputSchema.safeParse({ value: 1.5, unit: "turns" })
        .success,
    ).toBe(true);
    expect(
      SetGoalBudgetToolInputSchema.safeParse({ value: 1.5, unit: "hours" })
        .success,
    ).toBe(true);
  });
  ...
  it("sets turn, token, and time budgets on the current goal", async () => {
    ...
    expect(
      (await executeTool(tool, ctx({ value: 20, unit: "turns" }))).output,
    ).toBe("Goal budget set: 20 turns.");
    expect(store.getGoal().goal?.budget.turnBudget).toBe(20);

    expect(
      (await executeTool(tool, ctx({ value: 500_000, unit: "tokens" }))).output,
    ).toBe("Goal budget set: 500000 tokens.");
    expect(store.getGoal().goal?.budget.tokenBudget).toBe(500_000);

    expect(
      (await executeTool(tool, ctx({ value: 30, unit: "minutes" }))).output,
    ).toBe("Goal budget set: 30 minutes.");
  ...
  it("rounds fractional turn and token budgets before setting them", async () => {
    ...
    expect(
      (await executeTool(tool, ctx({ value: 1.5, unit: "turns" }))).output,
    ).toBe("Goal budget set: 2 turns.");
    expect(store.getGoal().goal?.budget.turnBudget).toBe(2);

    expect(
      (await executeTool(tool, ctx({ value: 0.4, unit: "tokens" }))).output,
    ).toBe("Goal budget set: 1 token.");
  ...
  it("ignores unreasonable time budgets and tells the model why", async () => {
    ...
    const tiny = await executeTool(
      tool,
      ctx({ value: 1, unit: "milliseconds" }),
    );
  ...
  it("stops the batch and turn when the new budget is already exhausted", async () => {
    ...
    if (execution.isError === true)
      throw new Error("execution should not be an error");
...
describe("UpdateGoalTool", () => {
  it("guards against premature blocked status", () => {
    const description = new UpdateGoalTool(
      fakeAgent(),
    ).description.toLowerCase();
    ...
    expect(description).toContain(
      "same turn instead of running more goal turns",
    );
  ...
  it("exposes the blocked-audit rule in the status parameter schema", () => {
    ...
    expect(statusDescription).toContain(
      "impossible, unsafe, or contradictory objectives",
    );
  });
  ...
  it("discourages calling UpdateGoal after a non-terminal work slice", () => {
    ...
    expect(description).toContain(
      "Most active goal turns should not call this tool",
    );
    expect(description).toContain(
      "end the turn normally without calling UpdateGoal",
    );
    expect(description).toContain(
      "actual objective and every explicit requirement",
    );
  ...
  it("accepts only active / complete / blocked", () => {
    ...
    expect(
      UpdateGoalToolInputSchema.safeParse({ status: "blocked", reason: "x" })
        .success,
    ).toBe(false);
  ...
  it.each([
    ...
    "reports a no-goal result for `%s` without stopping the turn",
    async (status, output) => {
...
describe("ToolManager goal tool registration", () => {
  function loopToolNames(type: "main" | "sub"): readonly string[] {
    ...
    ctxAgent.configure({
      tools: ["Read", "CreateGoal", "GetGoal", "SetGoalBudget", "UpdateGoal"],
    });
  ...
  it("keeps goal mutation tools visible across goal lifecycle states", async () => {
    ...
    ctxAgent.agent.tools.initializeBuiltinTools();
    expect(ctxAgent.agent.tools.loopTools.map((t) => t.name)).toContain(
      "UpdateGoal",
    );
    expect(ctxAgent.agent.tools.loopTools.map((t) => t.name)).toContain(
      "SetGoalBudget",
...
describe("CreateGoalToolInputSchema", () => {
  it("accepts a minimal objective and a full payload", () => {
    expect(
      CreateGoalToolInputSchema.safeParse({ objective: "x" }).success,
    ).toBe(true);
    expect(
