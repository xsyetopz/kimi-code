...
describe("BashTool", () => {
  ...
  it("uses args.cwd when provided", async () => {
    const execWithEnv = vi
      .fn()
      .mockResolvedValue(processWithOutput({ stdout: "sub\n" }));
    const tool = bashTool(
      createFakeKaos({ execWithEnv, osEnv: posixEnv }),
      "/workspace",
      createBackgroundManager().manager,
  ...
  it("can detach a foreground command through the background manager", async () => {
    ...
    await vi.waitFor(async () => {
      await expect(manager.readOutput(task.taskId)).resolves.toContain(
        "after detach\n",
      );
    });
  ...
  it("moves a timed-out foreground command to the background instead of killing it", async () => {
    ...
    try {
      ...
      await expect(manager.readOutput(taskId!)).resolves.toContain(
        "after timeout\n",
      );
    } finally {
      vi.useRealTimers();
  ...
  describe("background timeout default (backgroundTimeoutS)", () => {
    ...
    it("uses the configured seconds as the default deadline and detach re-arm", async () => {
      const manager = createBackgroundManager().manager;
      const options = await launchBackground(manager, {
        backgroundTimeoutS: 30,
      });
      expect(options).toMatchObject({
        timeoutMs: 30_000,
        detachTimeoutMs: 30_000,
      });
    });

    it("honors an explicit per-call timeout even when the default is disabled", async () => {
      const manager = createBackgroundManager().manager;
      const options = await launchBackground(
        manager,
        { backgroundTimeoutS: 0 },
        { timeout: 42 },
      );
  ...
  it("adds a truncation note when stdout exceeds the cap", async () => {
    ...
    const tool = bashTool(
      createFakeKaos({
        execWithEnv: vi
          .fn()
          .mockResolvedValue(processWithOutput({ stdout: huge })),
        osEnv: posixEnv,
      }),
      "/workspace",
    );
    ...
    try {
      ...
      const tool = bashTool(
        createFakeKaos({
          execWithEnv: vi
            .fn()
            .mockResolvedValue(processWithOutput({ stdout: fullOutput })),
          osEnv: posixEnv,
        }),
        "/workspace",
        manager,
  ...
  it("does not inject GIT_SSH_COMMAND into the spawn environment", async () => {
    ...
    try {
      const execWithEnv = vi
        .fn()
        .mockResolvedValue(processWithOutput({ stdout: "ok\n" }));
      const tool = bashTool(
        createFakeKaos({ execWithEnv, osEnv: posixEnv }),
        "/workspace",
      );
  ...
  it("rewrites nul-redirect on Windows so the spawned argv has /dev/null", async () => {
    const execWithEnv = vi
      .fn()
      .mockResolvedValue(processWithOutput({ stdout: "" }));
    const tool = bashTool(
      createFakeKaos({ execWithEnv, osEnv: windowsBashEnv }),
      "C:\\Users\\me\\project",
    );
