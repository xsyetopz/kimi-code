...
describe("current builtin file and shell tools", () => {
  it("Read exposes parameters and reads text content", async () => {
    ...
    expect(
      ReadInputSchema.safeParse({ path: "/workspace/a.txt" }).success,
    ).toBe(true);
  ...
  it("Write exposes parameters and writes through kaos", async () => {
    ...
    const tool = new WriteTool(
      createFakeKaos({
        writeText,
        stat: vi.fn<Kaos["stat"]>().mockResolvedValue(directoryStat),
      }),
      workspace,
    );

    expect(
      WriteInputSchema.safeParse({ path: "/workspace/a.txt", content: "hello" })
        .success,
    ).toBe(true);
    expect(tool.parameters).toMatchObject({
      type: "object",
    ...
    const result = await executeTool(
      tool,
      context({ path: "/workspace/a.txt", content: "hello" }),
    );
  ...
  it("Glob exposes parameters and walks pure-wildcard patterns capped at MAX_MATCHES", async () => {
    ...
    const exec = vi
      .fn()
      .mockResolvedValue(processWithOutput("/workspace/a.ts\n"));
    const stat = vi
      .fn()
      .mockResolvedValue({ ...regularFileStat, stMode: 0o040000 });
  ...
  it("Grep exposes parameters and rejects relative workspace escapes before spawning rg", async () => {
    ...
    const result = await executeTool(
      tool,
      context({ pattern: "needle", path: "../outside" }),
    );
    expect(result).toMatchObject({ isError: true });
  ...
  it("Bash exposes parameters and returns foreground stdout", async () => {
    ...
    expect(BashInputSchema.safeParse({ command: "printf ok" }).success).toBe(
      true,
    );
    expect(tool.parameters).toMatchObject({
      type: "object",
    ...
    const result = await executeTool(
      tool,
      context({ command: "printf ok", timeout: 1000 }),
    );
...
describe("current builtin collaboration tools", () => {
  it("AskUserQuestion exposes parameters and asks through rpc in yolo mode", async () => {
    ...
    expect(result.output).toBe(
      JSON.stringify({ answers: { "Which path?": "A" } }),
    );
  });
  ...
  it("AskUserQuestion documents the answers result shape and dismissal handling", () => {
    ...
    const description = new AskUserQuestionTool(
      {} as unknown as Agent,
  ...
  it("AgentSwarm applies one subagent_type across templated subagents", async () => {
    ...
    expect(
      Object.keys(tool.parameters["properties"] as Record<string, unknown>).at(
        -1,
      ),
    ).toBe("resume_agent_ids");
  ...
  it("AgentSwarm description states the enforced input requirements", () => {
    const description = new AgentSwarmTool(
      mockSubagentHost({}),
      mockSwarmMode(),
    ).description;
  ...
  it.each([
    ...
    "AgentSwarm rejects $name at execution time",
    async ({ input, output }) => {
      const host = mockSubagentHost({ runQueued: vi.fn() });
      const swarmMode = mockSwarmMode();
      const tool = new AgentSwarmTool(host, swarmMode);
  ...
  it("Skill exposes parameters and reports unknown skills as tool errors", async () => {
    ...
    expect(SkillToolInputSchema.safeParse({ skill: "missing" }).success).toBe(
      true,
    );
    expect(tool.parameters).toMatchObject({
      type: "object",
...
describe("current builtin background tool schemas", () => {
  it("background task schemas and manager-backed tools are covered", () => {
    ...
    expect(TaskListInputSchema.safeParse({ active_only: true }).success).toBe(
      true,
    );
    expect(TaskOutputInputSchema.safeParse({ task_id: "bash-1" }).success).toBe(
      true,
    );
    expect(TaskStopInputSchema.safeParse({ task_id: "bash-1" }).success).toBe(
      true,
    );
