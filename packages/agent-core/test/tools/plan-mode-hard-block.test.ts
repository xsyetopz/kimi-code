...
async function activePlanAgent(): Promise<{
  agent: Agent;
  planMode: PlanMode;
}> {
  const agent = {
...
function evaluatePlanPolicy(
  ...
  return new PlanModeGuardDenyPermissionPolicy(agent).evaluate(
    policyContext(toolName, args, mode),
  );
}
...
describe("Plan mode permission policy", () => {
  it("allows Write and Edit to the active plan file", async () => {
    ...
    expect(
      evaluatePlanPolicy(agent, "Write", { path: planPath }),
    ).toBeUndefined();
    expect(
  ...
  it("blocks file edits when plan mode has no selected plan file path", async () => {
    const { agent, planMode } = await activePlanAgent();
    (planMode as unknown as { _planFilePath: string | null })._planFilePath =
      null;
  ...
  it("allows multiple writes when every write access targets the active plan file", async () => {
    ...
    const result = new PlanModeGuardDenyPermissionPolicy(agent).evaluate(
      policyContext("Write", { path: planPath, content: "x" }, "manual", [
  ...
  it.each(["manual", "yolo", "auto"] as const)(
    ...
    async (mode) => {
      ...
      expect(
        evaluatePlanPolicy(agent, "Bash", { command: "rm foo.txt" }, mode),
      ).toBeUndefined();
      expect(
        evaluatePlanPolicy(agent, "Bash", { command: "ls -la" }, mode),
      ).toBeUndefined();
    },
  );
  ...
  it("does not block anything once plan mode has exited", async () => {
    ...
    expect(
      evaluatePlanPolicy(agent, "Write", { path: "/workspace/src/main.ts" }),
    ).toBeUndefined();
    expect(
      evaluatePlanPolicy(agent, "Bash", { command: "rm foo.txt" }),
    ).toBeUndefined();
    expect(
      evaluatePlanPolicy(agent, "TaskStop", { task_id: "bash-abc12345" }),
    ).toBeUndefined();
  });
});
