...
describe("ExitPlanModeTool", () => {
  it("has name, description, and parameters from the current schema", () => {
    ...
    expect(ExitPlanModeInputSchema.safeParse({ plan: "a plan" }).success).toBe(
      false,
    );
    ...
    expect(optionsSchema.items?.properties?.["label"]?.description).toContain(
      "(Recommended)",
    );
    expect(
      optionsSchema.items?.properties?.["description"]?.description,
    ).toContain("trade-offs");
    expect(
      (tool.parameters["properties"] as Record<string, unknown>)["plan"],
    ).toBeUndefined();
  });
  ...
  it("returns an error when no plan content is available", async () => {
    ...
    expect(result.output).toContain(
      "Write your plan to /tmp/kimi-plan.md first",
    );
    expect(emit).not.toHaveBeenCalled();
  });
