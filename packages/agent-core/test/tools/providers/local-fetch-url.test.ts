...
describe("LocalFetchURLProvider content kind", () => {
  it("reports text/plain bodies as a verbatim passthrough", async () => {
    const fetchImpl = vi
      .fn<typeof fetch>()
      .mockResolvedValue(
        htmlResponse("plain body", "text/plain; charset=utf-8"),
      );
...
describe("LocalFetchURLProvider SSRF guard", () => {
  ...
  it("fails closed when DNS resolution fails", async () => {
    lookupMock.mockRejectedValue(
      new Error("getaddrinfo ENOTFOUND b0rked.example"),
    );
    const fetchImpl = vi.fn<typeof fetch>();
    const provider = new LocalFetchURLProvider({ fetchImpl });

    await expect(provider.fetch("http://b0rked.example/")).rejects.toThrow(
      "Cannot resolve host",
    );
    expect(fetchImpl).not.toHaveBeenCalled();
  });
  ...
  it("skips all checks when allowPrivateAddresses is set", async () => {
    const fetchImpl = vi
      .fn<typeof fetch>()
      .mockResolvedValue(htmlResponse("local", "text/plain"));
...
describe("LocalFetchURLProvider connection pinning", () => {
  ...
  it("passes no dispatcher for an IP literal", async () => {
    ...
    expect(
      (fetchImpl.mock.calls[0]![1] as RequestInit).dispatcher,
    ).toBeUndefined();
  });
