...
describe("persistOriginalImage", () => {
  ...
  it("maps jpeg mime types to a .jpg extension", async () => {
    const dir = await freshDir();
    const path = await persistOriginalImage(
      new Uint8Array([9, 9]),
      "image/jpeg",
      { dir },
    );
  ...
  it("gives distinct paths to distinct bytes", async () => {
    const dir = await freshDir();
    const a = await persistOriginalImage(new Uint8Array([1]), "image/png", {
      dir,
    });
    const b = await persistOriginalImage(new Uint8Array([2]), "image/png", {
      dir,
    });
    expect(a).not.toBe(b);
  ...
  it("evicts the oldest files once the cache exceeds its cap", async () => {
    const dir = await freshDir();
    const old = await persistOriginalImage(
    ...
    const fresh = await persistOriginalImage(
  ...
  it("returns null instead of throwing when the destination is unusable", async () => {
    ...
    const path = await persistOriginalImage(new Uint8Array([1]), "image/png", {
      dir: blocker,
    });
