...
describe("ReadMediaFileTool", () => {
  it("has name, parameters, and path-scoped resource accesses", () => {
    ...
    expect(
      ReadMediaFileInputSchema.safeParse({ path: "/workspace/sample.png" })
        .success,
    ).toBe(true);
    ...
    expect(execution.accesses).toEqual(
      ToolAccesses.readFile("/workspace/sample.png"),
    );
  });
  ...
  it("returns a text/image/text wrap plus a <system> note for PNG files", async () => {
    ...
    const tool = makeReadMediaTool({
      stat: vi
        .fn<Kaos["stat"]>()
        .mockResolvedValue({ ...DEFAULT_STAT, stSize: data.length }),
  ...
  it("detects an extensionless PNG via magic-byte sniffing", async () => {
    ...
    expect(parts[0]).toEqual({
      type: "text",
      text: '<image path="/workspace/sample">',
    });
    expect((parts[1] as { imageUrl: { url: string } }).imageUrl.url).toContain(
      "image/png",
    );
  });
  ...
  it("expands leading tilde paths using the kaos home directory", async () => {
    ...
    expect(readBytes).toHaveBeenCalledWith(
      "/home/test/images/sample.png",
      MEDIA_SNIFF_BYTES,
    );
    expect(readBytes).toHaveBeenCalledWith("/home/test/images/sample.png");
    expect(parts[0]).toEqual({
      type: "text",
      text: '<image path="/home/test/images/sample.png">',
    });
  });
  ...
  it("falls back to a media extension when the header cannot be sniffed", async () => {
    ...
    expect(parts[0]).toEqual({
      type: "text",
      text: '<video path="/workspace/sample.mpg">',
    });
    expect((parts[1] as { videoUrl: { url: string } }).videoUrl.url).toBe(
  ...
  it("falls back to an inline base64 video part when the upload fails", async () => {
    ...
    expect(parts[1]).toEqual({
      type: "video_url",
      videoUrl: {
        url: `data:video/mp4;base64,${MP4_HEADER.toString("base64")}`,
      },
    });
  });
  ...
  it("rejects text files with a Read hint", async () => {
    ...
    const tool = makeReadMediaTool({
      stat: vi
        .fn<Kaos["stat"]>()
        .mockResolvedValue({ ...DEFAULT_STAT, stSize: text.length }),
  ...
  it("rejects unknown binary files without legacy Python-tool wording", async () => {
    ...
    const tool = makeReadMediaTool({
      stat: vi
        .fn<Kaos["stat"]>()
        .mockResolvedValue({ ...DEFAULT_STAT, stSize: blob.length }),
  ...
  it("rejects empty files and files exceeding the media size limit", async () => {
    const empty = await executeTool(
      makeReadMediaTool({
        stat: vi
          .fn<Kaos["stat"]>()
          .mockResolvedValue({ ...DEFAULT_STAT, stSize: 0 }),
      }),
      {
        turnId: "t1",
        toolCallId: "c_empty",
  ...
  it("exposes a <system> summary with the original pixel size for sized images", async () => {
    ...
    const tool = makeReadMediaTool({
      stat: vi
        .fn<Kaos["stat"]>()
        .mockResolvedValue({ ...DEFAULT_STAT, stSize: png.length }),
  ...
  it("description by capabilities lockdown — image + video points at Read for text fallback", () => {
    ...
    expect(tool.description).toContain(
      "supports image and video files for the current model",
    );
  });
  ...
  it("downsamples an oversized image but reports original dimensions", async () => {
    ...
    const tool = makeReadMediaTool({
      stat: vi
        .fn<Kaos["stat"]>()
        .mockResolvedValue({ ...DEFAULT_STAT, stSize: big.length }),
    ...
    expect(Math.max(sentDims!.width, sentDims!.height)).toBeLessThanOrEqual(
      2000,
    );
  ...
  it("reports an EXIF-rotated original in the decoded coordinate space", async () => {
    ...
    const tool = makeReadMediaTool({
      stat: vi
        .fn<Kaos["stat"]>()
        .mockResolvedValue({ ...DEFAULT_STAT, stSize: portrait.length }),
  ...
  it("reports the decoded size for a region read of an EXIF-rotated image", async () => {
    ...
    const portrait = withExifOrientation(
      new Uint8Array(
        await new Jimp({ width: 120, height: 80, color: 0x3366ccff }).getBuffer(
          "image/jpeg",
          {
            quality: 90,
          },
  ...
  describe("region and full_resolution", () => {
    async function bigPng(width: number, height: number): Promise<Buffer> {
      return Buffer.from(
        await new Jimp({ width, height, color: 0x3366ccff }).getBuffer(
          "image/png",
        ),
      );
    }
    ...
    it("announces a downsampled delivery and the region readback in the <system> block", async () => {
      ...
      expect(systemText).toMatch(
        /The attached image was downsampled to 2000x2000/,
      );
  ...
  describe("provider-unsupported formats", () => {
    ...
    it("refuses AVIF (still and animated brands) with conversion guidance", async () => {
      for (const brand of ["avif", "avis"]) {
        const result = await executeTool(
          unsupportedTool("macOS", ftypHeader(brand)),
          {
            turnId: "t1",
    ...
    it("refuses AVIF on Linux with ImageMagick guidance (no heif-convert)", async () => {
      const result = await executeTool(
        unsupportedTool("Linux", ftypHeader("avif")),
        {
          turnId: "t1",
    ...
    it("refuses BMP, TIFF, and ICO with per-OS conversion guidance", async () => {
      const cases: readonly { data: Buffer; mime: string; path: string }[] = [
        {
          data: Buffer.concat([Buffer.from("BM"), Buffer.from("bmpdata")]),
          mime: "image/bmp",
          path: "/workspace/photo.bmp",
        },
        {
          data: Buffer.from([0x49, 0x49, 0x2a, 0x00, 0x00]),
          mime: "image/tiff",
          path: "/workspace/scan.tiff",
        },
        {
          data: Buffer.from([0x00, 0x00, 0x01, 0x00, 0x00]),
          mime: "image/x-icon",
          path: "/workspace/favicon.ico",
        },
      ];
  ...
  describe("read byte budget", () => {
    ...
    it("returns shrink guidance without attaching the original when a decode guard blocks compression", async () => {
      ...
      expect(JSON.stringify(result)).not.toMatch(
        /image_url|blobref|base64|region/,
      );
    });
    ...
    it("two tools honor their own limits independently (no shared process state)", async () => {
      const data = await noisePng(512, 512);
      const tight = toolFor(
        data,
        new ImageLimits(process.env, { readByteBudget: 48 * 1024 }),
      );
      const roomy = toolFor(
        data,
        new ImageLimits(process.env, { maxEdgePx: 1200 }),
      );
