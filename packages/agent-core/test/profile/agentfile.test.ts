...
describe("parseAgentFileText", () => {
  ...
  it("parses a minimal agent file", () => {
    const definition = parse(
      agentFileText({ name: "reviewer", description: "Reviews code." }),
    );
  ...
  it("rejects non-kebab-case names", () => {
    expect(() =>
      parse(agentFileText({ name: "Code Reviewer", description: "d" })),
    ).toThrow(AgentFileParseError);
  });

  it("rejects a missing description", () => {
    expect(() => parse(agentFileText({ name: "reviewer" }))).toThrow(
      /description/,
    );
  });
  ...
  it("rejects missing frontmatter and an empty prompt body", () => {
    ...
    expect(() => parse(agentFileText({ description: "d" }, "   "))).toThrow(
      /prompt body/,
    );
  });
  ...
  it("rejects an invalid model_preference", () => {
    expect(() =>
      parse(agentFileText({ description: "d", model_preference: "cheapest" })),
    ).toThrow(/model_preference/);
  });

  it("ignores unknown frontmatter fields", () => {
    const definition = parse(
      agentFileText({ description: "d", future_field: "x" }),
    );
...
describe("agentProfileFromFile", () => {
  ...
  it("leaves unknown placeholders and nunjucks syntax verbatim", () => {
    const definition = parseAgentFileText({
      ...
      text: agentFileText(
        { description: "d" },
        "Keep ${unknown_var} and {{ not_a_var }} literal.",
      ),
    });
    const profile = agentProfileFromFile(definition, defaultTools, basePrompt);
    expect(profile.systemPrompt(promptContext)).toBe(
  ...
  it("injects skills only when the Skill tool survives the tool list", () => {
    ...
    expect(withSkill.systemPrompt(promptContext)).toContain(
...
describe("SessionAgentProfileCatalog", () => {
  ...
  it("discovers agents from the user brand dir and the project dir", async () => {
    ...
    await writeAgent(
      ...
      "user-agent.md",
      agentFileText({ description: "User agent." }),
    );
    await writeAgent(
  ...
  it("lets the project source shadow the user source on the same name", async () => {
    ...
    await writeAgent(
      join(brandHome, "agents"),
      "shared.md",
      agentFileText({ description: "From user." }),
    );
    await writeAgent(
      join(workDir, ".kimi-code", "agents"),
      "shared.md",
  ...
  it("discovers plugin agents and lets the user source shadow them", async () => {
    ...
    await writeAgent(
      pluginDir,
      "shared.md",
      agentFileText({ description: "From plugin." }),
    );
    await writeAgent(
      pluginDir,
      "plugin-only.md",
      agentFileText({ description: "Plugin agent." }),
    );
    await writeAgent(
      join(brandHome, "agents"),
      "shared.md",
  ...
  it("requires override: true before a file replaces a same-name builtin", async () => {
    ...
    expect(rejected.get("coder")?.description).toBe(
      DEFAULT_AGENT_PROFILES["coder"]!.description,
    );
    ...
    await writeAgent(
      ...
      "coder.md",
      agentFileText(
        { description: "Rogue coder.", override: true },
        "Rogue prompt.",
      ),
    );
    ...
    expect(accepted.get("coder")?.systemPrompt(promptContext)).toBe(
      "Rogue prompt.",
    );
  });
  ...
  it("replaces the default profile prompt with SYSTEM.md", async () => {
    const { workDir, brandHome, osHome } = await makeLayout();
    await writeFile(
      join(brandHome, "SYSTEM.md"),
      "Custom system for ${os}.",
      "utf-8",
    );

    const c = catalog({ workDir, brandHomeDir: brandHome, osHomeDir: osHome });
    await c.ready;
    expect(c.getDefault().systemPrompt(promptContext)).toBe(
      "Custom system for Linux.",
    );
    // Only the prompt changed; capabilities and description stay builtin.
    expect(c.getDefault().tools).toEqual(
      DEFAULT_AGENT_PROFILES["agent"]!.tools,
    );
  ...
  it("extends SYSTEM.md delegation with custom agents without allowing self-delegation", async () => {
    ...
    expect(Object.keys(restored.delegatableSubagents("agent"))).toEqual(
      expectedNames,
    );
  ...
  it("lets a project agent.md win over SYSTEM.md", async () => {
    ...
    await writeAgent(
      ...
      "agent.md",
      agentFileText(
        { description: "Project default.", override: true },
        "From project agent.md",
      ),
    );
  ...
  it("treats an explicit file as an implicit builtin override", async () => {
    ...
    await writeFile(
      explicitPath,
      agentFileText(
        { name: "coder", description: "Explicit coder." },
        "Explicit prompt.",
      ),
      "utf-8",
    );
  ...
  it("backs ${base_prompt} with the SYSTEM.md override when present", async () => {
    ...
    expect(c.get("reviewer")!.systemPrompt(promptContext)).toBe(
      "[Org default on Linux.]",
    );
  });
  ...
  it("never recurses when a file overrides the default and references ${base_prompt}", async () => {
    ...
    await writeAgent(
      ...
      "agent.md",
      agentFileText(
        { description: "Override default.", override: true },
        "Override.\n${base_prompt}",
      ),
    );
  ...
  it("warns about dead tool patterns in agent files", async () => {
    ...
    expect(
      warnings.some(
        (w) =>
          w.includes('"read"') && w.includes("no registered or built-in tool"),
      ),
    ).toBe(true);
    expect(
      warnings.some(
        (w) =>
          w.includes('"*"') && w.includes("wildcards only work inside mcp__"),
      ),
    ).toBe(true);
    expect(
      warnings.some(
        (w) =>
          w.includes('"mcp__github"') && w.includes("mcp__<server>__<tool>"),
      ),
    ).toBe(true);
...
describe("Session agentfile wiring", () => {
  ...
  it("binds a file-defined main profile selected by profileName", async () => {
    ...
    try {
      ...
      expect(main.config.systemPrompt).toContain(
        `You review code in ${workDir}`,
      );
      // The default delegation set includes the file-defined agent.
      expect(
        Object.keys(session.agentCatalog.delegatableSubagents("agent")),
      ).toContain("reviewer");
    } finally {
      await session.close();
    }
  ...
  it("fails createMain for an unknown profileName", async () => {
    ...
    try {
      await expect(session.createMain()).rejects.toThrow(
        /Agent profile "nope" was not found/,
      );
    } finally {
      await session.close();
    }
  ...
  it("fails createMain for an invalid explicit agent file", async () => {
    ...
    const session = new Session({
      ...
      providerManager: testProviderManager(),
      agents: {
        userHomeDir: brandHome,
        explicitFiles: [join(workDir, "missing.md")],
      },
    });
    try {
      await expect(session.createMain()).rejects.toThrow();
  ...
  it("restores a bound custom subagent when its agent files are removed before resume", async () => {
    ...
    await writeFile(
      ...
      agentFileText(
        {
          name: "leader",
          description: "Delegates focused work.",
          subagents: ["worker"],
        },
    ...
    try {
      ...
      const worker = main.subagentHost!.delegatableSubagents(
