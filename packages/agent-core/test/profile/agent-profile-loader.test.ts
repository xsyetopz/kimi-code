...
`,
  ...
  it("reports invalid profile graphs without relying on loader internals", () => {
    ...
    expect(() =>
      resolveAgentProfiles([{ name: "agent" }, { name: "agent" }]),
    ).toThrow(/Duplicate agent profile name: "agent"/);
...
describe("default agent profiles", () => {
  ...
  it("renders the model-invocable skill listing for bundled prompts", () => {
    ...
    skills.register(
      skill("review", { whenToUse: "When code review is requested." }),
    );
    skills.register({
      ...skill("nested-review", {
        isSubSkill: true,
    ...
    expect(prompt).not.toContain(
      "When to use: When nested review is requested.",
    );
    expect(prompt).not.toContain("- private:");
    expect(prompt).not.toContain("flow-only");
    expect(prompt).not.toContain("body of review");
    expect(prompt).not.toContain(
      "Nested review body must not enter system prompt.",
    );
  });
