...
describe("Session.init", () => {
  ...
  it('loads AGENTS.md via the persistence kaos when the tool kaos rejects readText (Zed ACP "Internal error" regression)', async () => {
    ...
    await writeFile(
      join(workDir, "AGENTS.md"),
      "project instructions from disk",
      "utf-8",
    );
    ...
    const capturedContext: { agentsMd: string | undefined } = {
      agentsMd: undefined,
    };
    const events: Array<Record<string, unknown>> = [];
    const session = new Session({
    ...
    try {
      ...
      expect(capturedContext.agentsMd).toContain(
        "project instructions from disk",
      );
    } finally {
      await session.close();
  ...
  it("refreshes AGENTS.md from a resumed native session system prompt", async () => {
    ...
    try {
      ...
      expect(agent.config.systemPrompt).toContain(
        "initial resume instructions",
      );
    } finally {
    ...
    try {
      ...
      expect(resumedAgent.config.systemPrompt).toContain(
        "initial resume instructions",
      );
      ...
      expect(resumedAgent.config.systemPrompt).not.toContain(
        "initial resume instructions",
      );
    } finally {
  ...
  it("resumes a v2 session from the standard path instead of persisted homedir", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        cwd: workDir,
        agents: {
          main: { homedir: "/stale/session/agents/main", type: "main" },
        },
        custom: {},
      }),
      "utf-8",
  ...
  it("rebuilds builtin tools when rebinding the session tool kaos", async () => {
    ...
    try {
      ...
      const readTool = agent.tools.loopTools.find(
  ...
  it("tracks connected and failed MCP server totals after initial load", async () => {
    ...
    try {
      ...
      await expect(
        new SessionAPIImpl(session).getMcpStartupMetrics({}),
      ).resolves.toEqual({
...
describe("AgentAPI.startBtw", () => {
  it("runs a side subagent from a stable parent context snapshot without writing btw history", async () => {
    ...
    mainAgent.context.appendUserMessage([
      { type: "text", text: "Main task: implement /btw." },
    ]);
    ...
    try {
      ...
      if (childAgent === undefined)
        throw new Error("Expected /btw child agent");
      const inheritedHistory = trimTrailingOpenToolExchange(
        mainAgent.context.project(mainAgent.context.history),
      );
      expect(
        childAgent.context.history.slice(0, inheritedHistory.length),
      ).toEqual(inheritedHistory);
      ...
      expect(
        events.filter((event) => String(event["type"]).startsWith("subagent.")),
      ).toEqual([]);
      expect(events).toContainEqual(
        expect.objectContaining({
          type: "turn.started",
      ...
      expect(
        JSON.stringify(session.getReadyAgent("agent-0")?.context.history),
      ).toContain("What are you working on right now?");
      scripted.mockNextResponse({
        type: "text",
        text: "Follow-up answer from the same side agent.",
      });
      await api.prompt({
        agentId,
      ...
      expect(followUpHistoryText).toContain(
        "What are you working on right now?",
      );
      expect(followUpHistoryText).toContain("Can you say that another way?");
      await expect(
        access(join(sessionDir, "agents", "agent-0", "wire.jsonl")),
      ).rejects.toThrow();
    } finally {
      await session.close();
    }
  ...
  it("declares parent tools but rejects side-question tool calls before a second text turn", async () => {
    ...
    registerLookupNoteTool(mainAgent);
    mainAgent.context.appendUserMessage([
      { type: "text", text: "Main task context." },
    ]);
    events.length = 0;
  ...
  it("discovers sub-skills and builtins", async () => {
    ...
    try {
      ...
      expect(disabledSkills.map((skill) => skill.name)).toContain(
        "outer.inner",
      );
      expect(disabledSkills.map((skill) => skill.name)).toContain(
    ...
    try {
      ...
      expect(enabledSkills.map((skill) => skill.name)).toContain(
        "sub-skill.consolidate",
      );
    } finally {
...
describe("Session secondary-model live config", () => {
  ...
  it("live-applies the recipe to the session snapshot and live agents", async () => {
    ...
    try {
      ...
      expect(agent.kimiConfig?.secondaryModel).toEqual({
        model: MOCK_PROVIDER.model,
      });
      ...
      expect(second.kimiConfig?.secondaryModel).toEqual({
        model: MOCK_PROVIDER.model,
      });
    } finally {
      await session.close();
  ...
  it("refreshes collaboration tool descriptions when live-applying a recipe", async () => {
    ...
    try {
      ...
      expect(descriptionsBefore["AgentSwarm"]).not.toContain(
        "Available models",
      );
      ...
      expect(descriptionsAfter["AgentSwarm"]).toContain(
...
function wrapReadTextWithError(inner: Kaos, cause: Error): Kaos {
  return new Proxy(inner, {
    get(target, prop, receiver) {
      ...
      if (prop === "withCwd") {
        return (cwd: string) =>
          wrapReadTextWithError(target.withCwd(cwd), cause);
      }
