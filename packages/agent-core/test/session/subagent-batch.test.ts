...
describe("SubagentBatch scheduling contract", () => {
  ...
  it("rate-limit phase requeues 429 tasks, emits suspended, and throttles launches", async () => {
    ...
    try {
      ...
      const running = runBatch(
        Array.from({ length: 8 }, (_, index) => queuedTask(index + 1)),
        {
          signal: controller.signal,
        },
      );
  ...
  it("task timeout fails only that task", async () => {
    ...
    try {
      const { runBatch, attempts } = createMockBatchRunner();
      const running = runBatch([{ ...queuedTask(1), timeout: 10_000 }], {
        signal,
      });
...
type MockAttemptRecord = {
  ...
  readonly outcome: ReturnType<
    typeof createControlledPromise<MockAttemptOutcome<number>>
  >;
};
...
function createMockBatchRunner(options: MockBatchRunnerOptions = {}): {
  readonly runBatch: <T>(
...
function mockAgentId(
  task: QueuedSubagentTask<unknown>,
  attemptIndex: number,
): string {
