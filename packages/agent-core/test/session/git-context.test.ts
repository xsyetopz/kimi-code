...
describe("sanitizeRemoteUrl", () => {
  it("strips credentials from an allowed HTTPS host", () => {
    expect(
      sanitizeRemoteUrl("https://user:pass@github.com/acme/widgets.git"),
  ...
  it("rejects an unrecognized HTTPS host", () => {
    expect(
      sanitizeRemoteUrl("https://internal.corp/acme/widgets.git"),
    ).toBeNull();
  });
  ...
  it("passes through the SourceHut git host", () => {
    expect(sanitizeRemoteUrl("git@git.sr.ht:~user/repo")).toBe(
...
describe("parseProjectName", () => {
  it("extracts owner/repo from an SSH URL", () => {
    expect(parseProjectName("git@github.com:acme/widgets.git")).toBe(
      "acme/widgets",
    );
  });

  it("extracts owner/repo from an HTTPS URL", () => {
    expect(parseProjectName("https://github.com/acme/widgets.git")).toBe(
      "acme/widgets",
    );
  });

  it("extracts owner/repo from an HTTPS URL without a .git suffix", () => {
    expect(parseProjectName("https://gitee.com/acme/widgets")).toBe(
      "acme/widgets",
    );
  });
  ...
  it("keeps the full namespace for nested GitLab groups (SSH)", () => {
    expect(parseProjectName("git@gitlab.com:group/subgroup/repo.git")).toBe(
      "group/subgroup/repo",
    );
  });
});
