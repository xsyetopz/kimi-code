...
describe("reduceWireRecords", () => {
  ...
  it("keeps shell and local-command output in the transcript but not foldedLength", () => {
    const { entries, foldedLength } = reduceWireRecords([
      appendMessage(userMessage("u1")),
      appendMessage(
        userMessage("! pwd", { kind: "shell_command", phase: "input" }),
      ),
      appendMessage(
        userMessage("local output", {
          kind: "injection",
  ...
  it("defers messages appended during an open tool exchange", () => {
    const { entries } = reduceWireRecords([
      ...
      appendMessage(
        userMessage("steer", { kind: "injection", variant: "steer" }),
      ),
      loopEvent({
        type: "tool.result",
        parentUuid: "s1",
  ...
  it("rehydrates blobref media urls from the blobs dir", async () => {
    ...
    const message: ContextMessage = {
      ...
      content: [
        ...
        {
          type: "image_url",
          imageUrl: { url: "blobref:image/png;0123456789abcdef0123" },
        },
      ],
      toolCalls: [],
    } as ContextMessage;
...
describe("MessageService over a compacted wire log", () => {
  ...
  beforeEach(async () => {
    ...
    const rpc: Partial<CoreRPC> = {
      ...
      getContext: vi
        .fn()
        .mockImplementation(async (): Promise<AgentContextData> => {
