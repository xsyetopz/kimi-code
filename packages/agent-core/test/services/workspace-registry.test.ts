...
describe("WorkspaceRegistryService", () => {
  ...
  it("keeps a derived bucket visible even when its root no longer exists on disk", async () => {
    ...
    const goneRoot = normalizeWorkDir(
      join(tmpdir(), "kimi-ws-gone-never-created"),
    );
  ...
  it("tombstones a derived workspace on delete so it stays removed", async () => {
    ...
    expect((await ctx.registry.list()).map((w) => w.id)).not.toContain(
      derivedId,
    );
  });
  ...
  it("resolveAliasWorkDirs returns [] for an id unknown to registry and index", async () => {
    await expect(
      ctx.registry.resolveAliasWorkDirs("wd_unknown_deadbeef0000"),
    ).resolves.toEqual([]);
  });
  ...
  it("resolveAliasWorkDirs keeps POSIX case variants distinct (singleton)", async () => {
    ...
    await expect(ctx.registry.resolveAliasWorkDirs(upperId)).resolves.toEqual([
      "/Home/Dev/Project",
    ]);
    await expect(ctx.registry.resolveAliasWorkDirs(lowerId)).resolves.toEqual([
      "/home/dev/project",
    ]);
  });
  ...
  it("delete removes and tombstones every folded alias of a Windows root", async () => {
    ...
    await appendSessionIndexEntry(ctx.homeDir, {
      ...
      sessionDir: join(
        ctx.homeDir,
        "sessions",
        encodeWorkDirKey(indexOnlyRoot),
        "s1",
      ),
    ...
    const file = JSON.parse(
      await readFile(join(ctx.homeDir, "workspaces.json"), "utf-8"),
    ) as {
  ...
  it("session_count sums active sessions across alias buckets for one Windows root", async () => {
    ...
    const registeredSessionDir = join(
      ctx.homeDir,
      "sessions",
      registeredId,
      "sess-sum-1",
    ...
    await seedSessionBucket("c:\\users\\dev\\sumproj", "sess-sum-3", {
      archived: true,
    });

    expect((await ctx.registry.get(registeredId)).session_count).toBe(2);
    const listed = (await ctx.registry.list()).find(
      (w) => w.id === registeredId,
    );
...
describe("touchWorkspaceRegistry", () => {
  ...
  async function readRegistryFile(): Promise<{
    ...
    return JSON.parse(
      await readFile(join(homeDir, "workspaces.json"), "utf-8"),
    ) as never;
  }
  ...
  it("clears the deletion tombstone for the touched workspace", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({
        version: 1,
        workspaces: {},
        deleted_workspace_ids: [workspaceId],
      }),
      "utf-8",
    );
  ...
  it("folds a Windows case-variant spelling onto the existing entry instead of minting a duplicate", async () => {
    ...
    const result = await touchWorkspaceRegistry(
      homeDir,
      "c:\\users\\foo\\proj",
    );
    ...
    expect(
      Date.parse(file.workspaces[diskId]?.last_opened_at ?? ""),
    ).toBeGreaterThan(Date.parse("2026-01-01T00:00:00.000Z"));
  });
