  ...
  const rpc: Partial<CoreRPC> = {
    ...
    removeKimiProvider: vi.fn(async ({ providerId }) => {
      ...
      const models = Object.fromEntries(
        Object.entries(configRef.current.models ?? {}).filter(
...
describe("ModelCatalogService", () => {
  ...
  it("projects latest Opus efforts for unknown Claude-marked Anthropic-compatible models", async () => {
    ...
    const compatible = (await svc.listModels()).find(
  ...
  it("gets one provider or throws ProviderNotFoundError", async () => {
    ...
    await expect(svc.getProvider("kimi")).resolves.toMatchObject({
      id: "kimi",
    });
  ...
  it("refreshes managed OAuth models and preserves always-thinking defaults", async () => {
    ...
    const svc = ModelCatalogService._createForTest(
      makeEnv(),
      core,
      authFacade(),
    );
  ...
  it("keeps the kimi-code provider on the REST base and records the model protocol when anthropic", async () => {
    ...
    const providers = last["providers"] as Record<
      string,
      { type: string; baseUrl: string }
    >;
    const models = last["models"] as Record<
      string,
      { provider: string; protocol?: string }
    >;
    ...
    expect(providers[KIMI_CODE_PROVIDER_NAME]?.baseUrl).toBe(
      "https://api.example.test/coding/v1",
    );
    expect(models["kimi-code/kimi-for-coding"]?.provider).toBe(
      KIMI_CODE_PROVIDER_NAME,
    );
  ...
  it("sends the host User-Agent on custom-registry fetches", async () => {
    ...
    const { core } = makeCore(configRef);
    (
      core as { kimiRequestHeaders?: Record<string, string> }
    ).kimiRequestHeaders = {
