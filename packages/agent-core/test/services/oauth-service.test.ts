...
interface LoginCall {
  ...
  onDeviceCode:
    | ((auth: DeviceAuthorization) => void | Promise<void>)
    | undefined;
  ...
  const facade = {
    login: vi.fn(
      (
        ...
        const promise = new Promise<{ providerName: string; ok: true }>(
          (resolve, reject) => {
            resolveFn = resolve;
            rejectFn = reject;
          },
...
describe("OAuthService.cancelLogin", () => {
  it("cancels a pending flow and fires the AbortController", async () => {
    ...
    const aborted = new Promise<boolean>((resolve) => {
      mock.loginCalls[0]!.signal!.addEventListener("abort", () =>
        resolve(true),
      );
    });
