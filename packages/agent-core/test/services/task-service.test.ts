...
interface FakeState {
  ...
  stopCalls: Array<
    StopBackgroundPayload & { sessionId: string; agentId: string }
  >;
}
...
function makeBridge(state: FakeState): ICoreProcessService {
  const rpc: Partial<CoreRPC> = {
    ...
    getBackground: async (p: {
      sessionId: string;
      agentId: string;
      activeOnly?: boolean;
    }) => state.tasksBySession.get(p.sessionId) ?? [],
    getBackgroundOutput: async (p: {
      sessionId: string;
      agentId: string;
      taskId: string;
      tail?: number;
...
describe("TaskService.list", () => {
  it("throws SessionNotFoundError on unknown session", async () => {
    ...
    await expect(svc.list("unknown", {})).rejects.toBeInstanceOf(
      SessionNotFoundError,
    );
  });
  ...
  it("filters by status when query.status is set", async () => {
    ...
    const svc = new TaskService(makeBridge(state));
    expect(
      (await svc.list("s1", { status: "running" })).map((t) => t.id),
    ).toEqual(["t1"]);
    expect(
      (await svc.list("s1", { status: "cancelled" })).map((t) => t.id),
    ).toEqual(["t3"]);
  });
});
...
describe("TaskService.get", () => {
  it("throws TaskNotFoundError for unknown id", async () => {
    ...
    state.tasksBySession.set("s1", []);
    const svc = new TaskService(makeBridge(state));
    await expect(svc.get("s1", "nope")).rejects.toBeInstanceOf(
      TaskNotFoundError,
    );
  });
...
describe("TaskService.cancel", () => {
  it("throws TaskNotFoundError for unknown id", async () => {
    ...
    state.tasksBySession.set("s1", []);
    const svc = new TaskService(makeBridge(state));
    await expect(svc.cancel("s1", "nope")).rejects.toBeInstanceOf(
      TaskNotFoundError,
    );
  });
  ...
  it("throws TaskAlreadyFinishedError when status is completed", async () => {
    ...
    const svc = new TaskService(makeBridge(state));
    await expect(svc.cancel("s1", "t1")).rejects.toBeInstanceOf(
      TaskAlreadyFinishedError,
    );
  });
