...
function makeFakeBridge(state: FakeBridgeState): ICoreProcessService {
  const rpc: Partial<CoreRPC> = {
    ...
    listMcpServers: async (_p: EmptyPayload & { sessionId: string }) =>
      state.mcpServers,
...
describe("toProtocolMcpServer adapter", () => {
  ...
  it("maps 'pending' → 'connecting'", () => {
    expect(toProtocolMcpServer(base({ status: "pending" })).status).toBe(
      "connecting",
    );
  });
  it("passes 'connected' through", () => {
    expect(toProtocolMcpServer(base({ status: "connected" })).status).toBe(
      "connected",
    );
  });
  it("maps 'failed' → 'error'", () => {
    expect(toProtocolMcpServer(base({ status: "failed" })).status).toBe(
      "error",
    );
  });
  it("maps 'disabled' → 'disconnected'", () => {
    expect(toProtocolMcpServer(base({ status: "disabled" })).status).toBe(
      "disconnected",
    );
  });
  ...
  it("adopts name-as-id", () => {
    expect(
      toProtocolMcpServer(base({ status: "connected", name: "foo" })).id,
    ).toBe("foo");
  });
  it("does not set last_error when info.error is undefined or empty", () => {
    expect(
      toProtocolMcpServer(base({ status: "connected" })).last_error,
    ).toBeUndefined();
    expect(
...
describe("McpService.restart", () => {
  it("throws McpServerNotFoundError when no sessions exist", async () => {
    const svc = new McpService(makeFakeBridge(freshState()));
    await expect(svc.restart("lark")).rejects.toBeInstanceOf(
      McpServerNotFoundError,
    );
  });
  ...
  it("throws McpServerNotFoundError when the id is not in the registrar", async () => {
    ...
    const svc = new McpService(makeFakeBridge(state));
    await expect(svc.restart("unknown")).rejects.toBeInstanceOf(
      McpServerNotFoundError,
    );
  });
