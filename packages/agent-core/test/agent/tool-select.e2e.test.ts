...
const DISCLOSURE_PROVIDER = {
  type: "kimi",
  apiKey: "test-key",
  model: "select-capable-model",
} as const;
...
function grafanaClient(callLog: Array<[string, unknown]> = []): MCPClient {
  return {
    ...
    async callTool(name, args) {
      ...
      return {
        content: [{ type: "text", text: "error_rate=0.02" }],
        isError: false,
      };
    },
  };
...
function schemaMessages(ctx: TestAgentContext) {
  return ctx.agent.context.history.filter(
    (m) => m.tools !== undefined && m.tools.length > 0,
  );
}
...
describe("gate closed — inline regression baseline (S0)", () => {
  it("without the flag, MCP tools stay inline and nothing about disclosure appears", async () => {
    ...
    expect(ctx.agent.tools.loopTools.every((t) => t.deferred !== true)).toBe(
      true,
    );
  ...
  it("keeps deferred user tools inline when disclosure is unavailable", async () => {
    ...
    expect(ctx.agent.tools.loopTools.map((tool) => tool.name)).toContain(
      DASHBOARD_TOOL,
    );
    expect(
      ctx.agent.tools.loopTools.find((tool) => tool.name === DASHBOARD_TOOL)
        ?.deferred,
    ).toBeUndefined();
    ...
    expect(ctx.llmCalls[0]!.tools.map((tool) => tool.name)).toContain(
      DASHBOARD_TOOL,
    );
    expect(historyText(ctx)).not.toContain("<tools_added>");
  });
...
describe("disclosure mode — top-level convergence and announcements", () => {
  it("keeps MCP tools out of the top level, registers select_tools, and announces the manifest", async () => {
    ...
    expect(historyText(ctx)).toContain(
      `<tools_added>\n${GRAFANA_TOOL}\n</tools_added>`,
    );
  ...
  it("does not re-announce when the loadable set is unchanged", async () => {
    ...
    expect(
      ctx.agent.context.history.filter(isLoadableToolsAnnouncement),
    ).toHaveLength(1);
  });
  ...
  it("honors a disallowedTools deny of select_tools itself", async () => {
    ...
    expect(loopNames).toContain("Read");
    expect(
      ctx.agent.tools.data().find((info) => info.name === "select_tools")
        ?.active,
    ).toBe(false);
    ...
    ctx.mockNextResponse(
      { type: "text", text: "try" },
      selectCall("call-1", [GRAFANA_TOOL]),
    );
    ctx.mockNextResponse({ type: "text", text: "ok" });
    await runTurn(ctx, "load it");

    expect(ctx.llmCalls[0]!.tools.map((t) => t.name)).not.toContain(
      "select_tools",
    );
    expect(
...
describe("disclosure mode — select_tools three branches and dispatch", () => {
  it("loads a schema, makes it dispatchable on the next step of the same turn", async () => {
    ...
    expect(
      step2.tools.map((t) => t.name).some((n) => n.startsWith("mcp__")),
    ).toBe(false);
    expect(
      step2.history.some((m) => m.tools !== undefined && m.tools.length > 0),
    ).toBe(true);
  });
  ...
  it("loads and dispatches an opted-in user tool through the existing host RPC", async () => {
    ...
    expect(ctx.agent.tools.loopTools.map((tool) => tool.name)).not.toContain(
      DASHBOARD_TOOL,
    );
    ...
    await ctx.rpc.prompt({
      input: [{ type: "text", text: "create a dashboard" }],
    });
    await ctx.untilToolCall({ output: "dashboard-created" });
    await ctx.untilTurnEnd();

    expect(ctx.llmCalls[0]!.tools.map((tool) => tool.name)).not.toContain(
      DASHBOARD_TOOL,
    );
    ...
    expect(ctx.llmCalls[1]!.tools.map((tool) => tool.name)).not.toContain(
      DASHBOARD_TOOL,
    );
  ...
  it("stops exposing a loaded deferred user tool after unregister", async () => {
    ...
    expect(ctx.agent.tools.loadedDynamicToolNames().has(DASHBOARD_TOOL)).toBe(
      false,
    );
    ...
    expect(firstCallAfterRemoval.tools.map((tool) => tool.name)).not.toContain(
      DASHBOARD_TOOL,
    );
    expect(historyText(ctx)).toContain(
      `<tools_removed>\n${DASHBOARD_TOOL}\n</tools_removed>`,
  ...
  it("keeps the top-level tools snapshot stable across select_tools loads", async () => {
    ...
    const snapshots = persistence.records.filter(
      (
        record,
      ): record is Extract<AgentRecord, { type: "llm.tools_snapshot" }> =>
...
describe("disclosure mode — preflight wording", () => {
  it("distinguishes not-loaded from loaded-but-disconnected", async () => {
    ...
    ctx.mockNextResponse(
      { type: "text", text: "load" },
      selectCall("call-2", [GRAFANA_TOOL]),
    );
    ctx.mockNextResponse({ type: "text", text: "ok" });
    await runTurn(ctx, "load it");
    ctx.agent.tools.unregisterMcpServer("grafana");
    ...
    ctx.mockNextResponse(
      { type: "text", text: "call again" },
      mcpCall("call-3", "errors"),
    );
    ctx.mockNextResponse({ type: "text", text: "ok" });
...
describe("disclosure mode — undo semantics", () => {
  it("keeps schema messages across undo, drops announcements, and self-heals", async () => {
    ...
    expect(ctx.agent.tools.loadedDynamicToolNames().has(GRAFANA_TOOL)).toBe(
      true,
    );
    ...
    expect(toolResultTexts(ctx)).toContainEqual(
      `Already available: ${GRAFANA_TOOL}`,
    );
    expect(schemaMessages(ctx)).toHaveLength(1);
  });
...
describe("disclosure mode — model switch projection", () => {
  it("strips dynamic-tool context for a non-supporting model and restores it on switch-back", async () => {
    ...
    expect(
      projected.some((m) =>
        m.content.some(
          (p) => p.type === "text" && p.text.includes("<tools_added>"),
        ),
      ),
    ).toBe(false);
    ...
    expect(ctx.agent.context.messages.some((m) => m.tools !== undefined)).toBe(
      true,
    );
    ...
    const extra = ctx.agent.tools.loopTools.find(
      (t) => t.name === GRAFANA_TOOL,
    );
...
describe("disclosure mode — executable table freshness", () => {
  it("keeps goal tools visible without waiting for goal state changes", async () => {
    ...
    expect(ctx.agent.tools.loopTools.map((t) => t.name)).toContain(
      "UpdateGoal",
    );
    expect(ctx.agent.tools.loopTools.map((t) => t.name)).toContain(
      "SetGoalBudget",
...
describe("disclosure mode — compaction", () => {
  it("filters protocol context from the summarizer input and discards loaded schemas after compaction", async () => {
    ...
    expect(summarizerCall.history.some((m) => m.tools !== undefined)).toBe(
      false,
    );
    expect(JSON.stringify(summarizerCall.history)).not.toContain(
      "<tools_added>",
    );
    ...
    expect(ctx.agent.tools.loopTools.map((t) => t.name)).not.toContain(
      GRAFANA_TOOL,
    );
    expect(
      ctx.agent.context.history.filter(isLoadableToolsAnnouncement),
    ...
    const reAnnouncement = ctx.agent.context.history
      .filter(isLoadableToolsAnnouncement)
      .at(-1)!;
  ...
  it("survives a runtime tool-select flag flip without a builtin refresh", async () => {
    ...
    expect(
      disclosureCall.tools
        .map((t) => t.name)
        .some((n) => n.startsWith("mcp__")),
    ).toBe(false);
    expect(callLog).toEqual([["query_range", { query: "errors" }]]);
  ...
  it("discards heavy loaded schemas at compaction instead of re-entering the trigger band", async () => {
    ...
    (fatClient as { callTool: unknown }).callTool = async (
      ...args: unknown[]
    ) => {
