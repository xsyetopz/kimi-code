...
describe("ToolCallDeduplicator", () => {
  describe("same-step dedup", () => {
    it("returns a placeholder synchronously and resolves to the real result on finalize", async () => {
      ...
      const finalDup = await dedup.finalizeResult(
        "c2",
        "Read",
        { path: "/a" },
        cached!,
    ...
    it("finalizes original before dup (provider order)", async () => {
      ...
      const origFinal = await dedup.finalizeResult(
        "c1",
        "Read",
        { path: "/a" },
        okResult("A"),
      );
      const dupFinal = await dedup.finalizeResult(
        "c2",
        "Read",
        { path: "/a" },
        dupCached!,
  ...
  describe("cross-step streak", () => {
    ...
    it("same-step dups inherit reminder1 when streak triggers on original", async () => {
      ...
      expect(original.output as string).toContain(
        "what new information you expect",
      );
      expect(finalDup.output as string).toContain("<system-reminder>");
      expect(finalDup.output as string).toContain(
        "what new information you expect",
      );
    });
  ...
  describe("repeat telemetry", () => {
    ...
    it("does not emit telemetry on the first call", async () => {
      ...
      dedup.endStep();
      expect(events.filter((e) => e.event === "tool_call_repeat")).toHaveLength(
        0,
      );
    });
  ...
  describe("skipped registration (calls rejected before prepareToolExecution)", () => {
    ...
    it("counts skipped calls toward the streak and force-stops at 12, keeping the error flag", async () => {
      ...
      for (let i = 0; i < 12; i += 1) {
        dedup.beginStep();
        last = await runRejected(
          dedup,
          `c${String(i)}`,
          errResult("Invalid args"),
        );
        dedup.endStep();
      }
      expect(last!.isError).toBe(true);
