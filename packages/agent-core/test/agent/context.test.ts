...
describe("Agent context", () => {
  it("stores prompt origins without leaking them to LLM projection", () => {
    ...
    expect(
      ctx.agent.context.history.map(({ role, origin }) => ({ role, origin })),
    ).toEqual([
    ...
    expect(
      ctx.agent.context.messages.some((message) => "origin" in message),
    ).toBe(false);
  });
  ...
  it("preserves tool call extras (Gemini thought_signature) through to projection", () => {
    ...
    const recorded = ctx.agent.context.history.find(
      (m) => m.role === "assistant",
    );
    expect(recorded?.toolCalls[0]?.extras).toEqual(expectedExtras);
    const projected = ctx.agent.context.messages.find(
      (m) => m.role === "assistant",
    );
  ...
  it("reroutes an inline image-compression caption into a hidden system reminder", () => {
    ...
    expect(userMessage!.content.some((part) => part.type === "image_url")).toBe(
      true,
    );
  });
  ...
  it("runs a shell command via the Bash tool and records its output", async () => {
    ...
    const kaos = createFakeKaos({
      execWithEnv: vi
        .fn()
        .mockImplementation(async () => fakeProcess("hello\n")),
    });
    ...
    expect(textOf(ctx.agent.context.history[1]!)).toContain(
      "<bash-stdout>hello",
    );
  });
  ...
  it("renders tool error and empty-output status as model-visible text", () => {
    ...
    expect(ctx.agent.context.messages).toMatchObject([
      {
        role: "assistant",
        toolCalls: [{ id: "call_error" }, { id: "call_empty" }],
      },
      {
        role: "tool",
        content: [
  ...
  it("keeps raw tool result output in history; error status is added only in projection", () => {
    ...
    const stored = ctx.agent.context.history.find(
      (m) => m.toolCallId === "call_raw",
    )!;
  ...
  it("carries a tool result note into history and appends it in the LLM projection", () => {
    ...
    ctx.dispatch({
      ...
      event: {
        ...
        result: {
          output: "file body",
          note: "<system>10 lines read.</system>",
        },
      },
    });
  ...
  it("drops empty and whitespace-only text parts in LLM projection", () => {
    const history: ContextMessage[] = [
      ...
      },
      {
        role: "assistant",
        content: [
          { type: "think", think: "", encrypted: "enc_empty_thinking" },
        ],
        toolCalls: [],
      },
      {
  ...
  it("keeps system reminders separate from real user prompts", async () => {
    ...
    await ctx.rpc.prompt({
      input: [{ type: "text", text: "Real user prompt" }],
    });
  ...
  it("repairs a tool_use/tool_result adjacency broken by an injected user message", async () => {
    ...
    expect(
      projected[assistantIndex]?.toolCalls.map((toolCall) => toolCall.id),
    ).toEqual(["call_drive"]);
  ...
  it("clears context before the next LLM request", async () => {
    ...
    ctx.configure();
    ctx.agent.context.appendUserMessage([
      { type: "text", text: "stale user message" },
    ]);
  ...
  it("uses compacted summary plus recent messages", async () => {
    ...
    ctx.configure();
    ctx.agent.context.appendUserMessage([
      { type: "text", text: "old user message" },
    ]);
    ctx.agent.context.appendUserMessage([
      { type: "text", text: "recent user message" },
    ]);
    ctx.agent.context.applyCompaction({
  ...
  it("includes new user messages as pending until the next usage update", () => {
    ...
    ctx.agent.context.appendUserMessage([
      { type: "text", text: "next user prompt".repeat(20) },
    ]);
  ...
  it("keeps tool results pending when step usage covers only through the assistant message", () => {
    ...
    ctx.agent.context.appendUserMessage([
      { type: "text", text: "lookup pending tokens" },
    ]);
    ctx.dispatch({
      type: "context.append_loop_event",
      event: { type: "step.begin", uuid: stepUuid, turnId: "0", step: 1 },
...
describe("strictMessages duplicate tool call ids", () => {
  it("keeps duplicates on the normal projection but dedupes them in the strict one", () => {
    ...
    ctx.configure();
    ctx.agent.context.appendUserMessage([
      { type: "text", text: "run the tool twice" },
    ]);
