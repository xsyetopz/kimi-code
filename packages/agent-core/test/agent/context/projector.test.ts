 ...
 */
function findMisplacedToolUses(
 ...
 */
function everyToolUseImmediatelyAnswered(
  messages: readonly Message[],
): boolean {
  for (let i = 0; i < messages.length; i++) {
    const message = messages[i]!;
    ...
    if (
      message.toolCalls.some((toolCall) => !adjacentResultIds.has(toolCall.id))
    ) {
      return false;
    }
  }
...
describe("project tool_use/tool_result adjacency", () => {
  ...
  it("repairs multiple misplaced exchanges in a single history", () => {
    ...
    const aIndex = projected.findIndex((m) =>
      m.toolCalls.some((tc) => tc.id === "a"),
    );
    expect(projected[aIndex + 1]).toMatchObject({
      role: "tool",
      toolCallId: "a",
    });
    const bIndex = projected.findIndex((m) =>
      m.toolCalls.some((tc) => tc.id === "b"),
    );
    expect(projected[bIndex + 1]).toMatchObject({
      role: "tool",
      toolCallId: "b",
    });
  });
...
describe("project repair reporting", () => {
  ...
  it("reports a displaced result that had to be moved up", () => {
    ...
    expect(anomalies).toEqual([
      { kind: "tool_result_reordered", toolCallId: "a" },
    ]);
  });
  ...
  it("does not report adjacent parallel results that are merely out of order", () => {
    const anomalies: ProjectionAnomaly[] = [];
    project(
      [user("u1"), assistant(["a", "b", "c"]), tool("c"), tool("a"), tool("b")],
      {
        onAnomaly: (a) => anomalies.push(a),
      },
    );
  ...
  it("reports a mid-history synthesis as a non-trailing (defect) repair", () => {
    const anomalies: ProjectionAnomaly[] = [];
    project(
      [user("u1"), assistant(["a"]), user("u2"), assistant(["b"]), tool("b")],
      {
        onAnomaly: (a) => anomalies.push(a),
      },
    );
  ...
  it("reports a dropped orphan result when dropOrphanResults is set", () => {
    ...
    project(history, {
      ...
      onAnomaly: (a) => wire.push(a),
    });
    expect(wire).toEqual([
      { kind: "orphan_tool_result_dropped", toolCallId: "stray" },
    ]);
  });
  ...
  it("reports a whitespace-only text drop but not a truly-empty one", () => {
    ...
    expect(anomalies).toEqual([
      { kind: "whitespace_text_dropped", role: "assistant" },
    ]);
  });
...
describe("project drops vacuous (thinking-only) messages", () => {
  it("drops an assistant message whose only part is an empty think block", () => {
    ...
    expect(anomalies).toEqual([
      { kind: "vacuous_message_dropped", role: "assistant" },
    ]);
  });
  ...
  it("keeps a message with real text intact — including its empty think part", () => {
    ...
    const projected = project([
      user("u1"),
      thinkingAssistant([thinkPart(""), textPart("answer")]),
    ]);
  ...
  it("keeps a message whose think block has real content", () => {
    const projected = project([
      user("u1"),
      thinkingAssistant([thinkPart("real reasoning")]),
    ]);
  ...
  it("keeps a signed think block even when its text is empty", () => {
    const projected = project([
      user("u1"),
      thinkingAssistant([thinkPart("", "sig")]),
    ]);
    expect(projected.map((m) => m.role)).toEqual(["user", "assistant"]);
  ...
  it("reports no anomaly for a silently dropped empty-content message", () => {
    const anomalies: ProjectionAnomaly[] = [];
    project([user("u1"), emptyAssistant(), user("u2")], {
      onAnomaly: (a) => anomalies.push(a),
    });
    expect(anomalies).toEqual([]);
  });
...
describe("project strict-provider sanitizers", () => {
  ...
  it("only drops leading non-user under the strict flag (normal path keeps them)", () => {
    const history: ContextMessage[] = [
      assistantText("stray opener"),
      user("hi"),
    ];
    expect(project(history)[0]?.role).toBe("assistant");
    expect(project(history, { dropLeadingNonUser: true })[0]?.role).toBe(
      "user",
    );
  });
  ...
  it("merges consecutive assistant messages under the strict flag", () => {
    const projected = project(
      [user("hi"), assistantText("part one"), assistantText("part two")],
      {
        mergeConsecutiveAssistants: true,
      },
    );
...
describe("project duplicate tool_use ids", () => {
  ...
  it("leaves duplicate ids and their results untouched on the normal path", () => {
    ...
    expect(assistants[0]?.toolCalls.map((toolCall) => toolCall.id)).toEqual([
      "call_a",
    ]);
    expect(assistants[1]?.toolCalls.map((toolCall) => toolCall.id)).toEqual([
      "call_a",
      "call_b",
    ]);
    expect(projected.filter((message) => message.role === "tool")).toHaveLength(
      3,
    );
  });
  ...
  it("under the strict flag, drops a duplicate call id within one assistant message", () => {
    ...
    const assistantMessage = projected.find(
      (message) => message.role === "assistant",
    );
    expect(assistantMessage?.toolCalls.map((toolCall) => toolCall.id)).toEqual([
      "call_dup",
    ]);
    expect(everyToolUseImmediatelyAnswered(projected)).toBe(true);
  });
    ...
    const projected = project(
      [
        ...
        tool("call_a", "first result"),
        {
          ...
          content: [thinkPart("")],
          toolCalls: [
            { type: "function", id: "call_a", name: "Run", arguments: "{}" },
          ],
        },
        tool("call_a", "second result"),
        user("u2"),
  ...
  it("under the strict flag, keeps a duplicate-stripped assistant whose remaining content is sendable", () => {
    ...
    expect(anomalies).not.toContainEqual({
      kind: "vacuous_message_dropped",
      role: "assistant",
    });
    expect(everyToolUseImmediatelyAnswered(projected)).toBe(true);
...
function label(message: Message): string {
  const id =
    message.toolCallId ??
    message.toolCalls.map((toolCall) => toolCall.id).join(",");
    ...
    switch (kind) {
      ...
      case "exchange": {
        ...
        if (rng() > 0.5) {
          messages.push(
            pick(rng, [
              user(`sandwich-${seed}-${b}`),
              notification(`sandwich-n-${seed}-${b}`),
            ]),
          );
        }
...
describe("degradeOlderMediaParts", () => {
  function imageMessage(name: string): Message {
    return {
      ...
      content: [
        ...
        {
          type: "image_url",
          imageUrl: { url: `data:image/png;base64,${name}` },
        },
  ...
  it("replaces all but the most recent N media parts with placeholder text", () => {
    const messages: Message[] = [
      imageMessage("old"),
      {
        ...
        content: [
          {
            type: "video_url",
            videoUrl: { url: "data:video/mp4;base64,VVVV" },
          },
          {
            type: "audio_url",
            audioUrl: { url: "data:audio/mp3;base64,SSSS" },
          },
        ],
        toolCalls: [],
      },
    ...
    expect(survivor?.type === "image_url" && survivor.imageUrl.url).toContain(
      "recent",
    );
  });
  ...
  it("keeps untouched messages by reference and does not mutate degraded ones", () => {
    ...
    expect(messages[0]!.content.some((part) => part.type === "image_url")).toBe(
      true,
    );
  });
});
...
describe("media strip snapshots", () => {
  it("strips a cloned media container when its provider-visible identity is unchanged", () => {
    ...
    const projected = stripMediaPartsBySnapshot(
  ...
  it("preserves media when its id or part type differs from the captured identity", () => {
    const sharedContainer = {
      id: "shared",
      url: "data:application/octet-stream;base64,U0FNRQ==",
    };
