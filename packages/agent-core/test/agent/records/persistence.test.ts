...
async function makeWirePath(): Promise<string> {
  const dir = join(
    tmpdir(),
    `wire-jsonl-test-${randomBytes(6).toString("hex")}`,
  );
...
describe("FileSystemAgentRecordPersistence", () => {
  ...
  it("offloads large data URIs to blobsDir during append", async () => {
    const dir = join(
      tmpdir(),
      `wire-blob-test-${randomBytes(6).toString("hex")}`,
    );
    await mkdir(dir, { recursive: true });
    cleanups.push(dir);
    ...
    const url = (
      (record["input"] as unknown[])[0] as { imageUrl: { url: string } }
    ).imageUrl.url;
    ...
    expect(
      (await readFile(join(blobsDir, blobFiles[0]!))).toString("base64"),
    ).toBe(payload);
  });
});
