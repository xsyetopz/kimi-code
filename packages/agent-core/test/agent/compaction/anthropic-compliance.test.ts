 ...
 */
async function toAnthropicWire(
  history: Message[],
  tools: Tool[] = [],
): Promise<WireMessage[]> {
  ...
  (
    provider as unknown as { _client: { messages: { create: unknown } } }
  )._client.messages.create = vi.fn().mockImplementation((params: unknown) => {
  ...
  if (captured?.messages === undefined) {
    throw new Error(
      "Expected provider.generate() to call messages.create with messages",
    );
  }
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    for (const block of message.content) {
      if (block.type === "tool_use") {
        expect(message.role, "tool_use must be on an assistant message").toBe(
          "assistant",
        );
        ...
        expect(
          answered,
          `tool_use ${String(block.id)} must be answered in the next message`,
        ).toBe(true);
      }
      if (block.type === "tool_result") {
        expect(message.role, "tool_result must be on a user message").toBe(
          "user",
        );
...
describe("compaction — Anthropic wire compliance", () => {
  it("post-compaction context plus a follow-up tool turn is a valid Anthropic request", async () => {
    ...
    expect(wire.some((m) => m.content.some((b) => b.type === "tool_use"))).toBe(
      true,
    );
    expect(
      wire.some((m) => m.content.some((b) => b.type === "tool_result")),
    ).toBe(true);
  });
  ...
  it("collapses mixed-origin kept users and the summary into a single Anthropic user turn", async () => {
    ...
    expect(projected.filter((m) => m.role === "user").length).toBeGreaterThan(
      1,
    );
  ...
  it("produces a valid summarizer request when a tool result is non-adjacent to its call", async () => {
    ...
    const projected = ctx.agent.context.project(messy, {
  ...
  it("closes a mid-history tool call whose result is missing on the normal send path", async () => {
    ...
    const orphaned: ContextMessage[] = [
      ...
      },
      {
        ...
        toolCalls: [
          { type: "function", id: "call_1", name: "Bash", arguments: "{}" },
        ],
      },
      {
        role: "user",
      ...
      },
      {
        ...
        toolCalls: [
          { type: "function", id: "call_2", name: "Bash", arguments: "{}" },
        ],
      },
      {
        role: "tool",
  ...
  it("closes a still-open tool call in the summarizer request with a synthetic result", async () => {
    ...
    const dangling: ContextMessage[] = [
      ...
      },
      {
        ...
        content: [{ type: "text", text: "calling" }],
        toolCalls: [
          { type: "function", id: "call_x", name: "Bash", arguments: "{}" },
        ],
      },
    ];

    const projected = ctx.agent.context.project(dangling, {
      synthesizeMissing: true,
    });
    const wire = await toAnthropicWire(projected, [BASH_TOOL]);
    assertValidAnthropic(wire);
    ...
    expect(
      lastUser.content.some(
        (b) => b.type === "tool_result" && b.tool_use_id === "call_x",
      ),
    ).toBe(true);
  });
});
