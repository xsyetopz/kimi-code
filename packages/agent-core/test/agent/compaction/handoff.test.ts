...
const EXPECTED_DISPOSITION: Record<
  PromptOrigin["kind"],
...
describe("isCompactionSummaryMessage", () => {
  ...
  it("ignores ordinary user messages", () => {
    expect(isCompactionSummaryMessage(textMessage("user", "hello"))).toBe(
      false,
    );
  });
});
...
describe("compactionUserMessageDisposition", () => {
  it("classifies every prompt origin kind", () => {
    for (const kind of Object.keys(ALL_PROMPT_ORIGIN_KINDS) as Array<
      PromptOrigin["kind"]
    >) {
      expect(compactionUserMessageDisposition(originForKind(kind))).toBe(
        EXPECTED_DISPOSITION[kind],
      );
    }
  });
  ...
  it("keeps genuine user input and drops other origins", () => {
    expect(
      isRealUserInput({
        ...
        origin: originForKind("user"),
      }),
    ).toBe(true);
    expect(
      isRealUserInput({
        ...textMessage("user", "hello"),
        origin: originForKind("skill_activation"),
      }),
    ).toBe(true);
    expect(
      isRealUserInput({
        ...textMessage("user", "hello"),
        origin: originForKind("injection"),
      }),
    ).toBe(false);
    expect(
      isRealUserInput({
        ...textMessage("user", "hello"),
        origin: originForKind("shell_command"),
      }),
    ).toBe(false);
    expect(
      isRealUserInput({
        ...textMessage("user", "hello"),
        origin: originForKind("background_task"),
      }),
    ).toBe(false);
  });
});
...
describe("selectRecentUserMessages", () => {
  it("keeps the most recent messages within the budget", () => {
    ...
    const budget =
      ...
      estimateTokensForMessage(messages[2]!);

    expect(selectRecentUserMessages(messages, budget).map(messageText)).toEqual(
      ["mid", "recent"],
    );
  });
  ...
  it("truncates a CJK-heavy oldest message within the budget in one pass", () => {
    ...
    expect(selected).toHaveLength(2);
    expect(messageText(selected[1]!)).toBe("recent");
    expect(estimateTokens(messageText(selected[0]!))).toBeLessThanOrEqual(
      1_000,
    );
...
describe("selectCompactionUserMessages", () => {
  ...
  it("keeps the oldest messages within the head budget and the newest within the rest", () => {
    ...
    // tokens + 1 role token).
    const messages = ["a", "b", "c", "d", "e"].map((c) =>
  ...
  it("splits an oversized conversation with the default budgets", () => {
    const big = textMessage(
      "user",
      "x".repeat(COMPACT_USER_MESSAGE_MAX_TOKENS * 5),
    );
...
describe("buildCompactionSummaryText", () => {
  it("prefixes the summary", () => {
    expect(buildCompactionSummaryText("Summary.")).toBe(
      `${COMPACTION_SUMMARY_PREFIX}\nSummary.`,
    );
  });

  it("falls back when the summary is empty", () => {
    expect(buildCompactionSummaryText("   ")).toBe(
