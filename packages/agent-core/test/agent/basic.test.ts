...
it("runs an agent turn through builtin tool approval and execution", async () => {
  ...
  ctx.mockNextResponse({
    type: "text",
    text: "The command printed lookup-result.",
  });
  expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
...
describe("prompt-attached video resolution", () => {
  ...
  function firstUserContent(ctx: ReturnType<typeof testAgent>): ContentPart[] {
    return (
      ctx.agent.context.messages.find((m) => m.role === "user")?.content ?? []
    );
  }
    ...
    const server = createServer((req, res) => {
      if (req.method === "POST" && req.url === "/files") {
        ...
        req.on("end", () => {
          ...
          res.end(
            JSON.stringify({ error: { message: "stub upload failure" } }),
          );
        });
        return;
      }
  ...
  it("uploads a local file:// video and sends the issued ms:// reference to the model", async () => {
    ...
    try {
      ...
      await ctx.rpc.prompt({
        input: [{ type: "video_url", videoUrl: { url: fileUrl(tempVideo()) } }],
      });
      await ctx.untilTurnEnd();
  ...
  it("ends the turn as cancelled when the user aborts before the inline fallback", async () => {
    ...
    vi.spyOn(ctx.agent.kaos, "readBytes").mockImplementation(
      async (p, length) => {
    ...
    await ctx.rpc.prompt({
      input: [{ type: "video_url", videoUrl: { url: fileUrl(path) } }],
    });
    const events = await ctx.untilTurnEnd();
  ...
  it("degrades to a <video path> tag when the model lacks video_in (no upload)", async () => {
    ...
    try {
      ...
      expect(firstUserContent(ctx).some((p) => p.type === "video_url")).toBe(
        false,
      );
  ...
  it("falls back to an inline base64 part when the provider has no upload channel", async () => {
    ...
    expect(part?.type === "video_url" && part.videoUrl.url).toMatch(
  ...
  it("fails the turn on an auth (401) upload rejection without poisoning history", async () => {
    ...
    try {
      ...
      expect(ctx.agent.context.messages.some((m) => m.role === "user")).toBe(
        false,
      );
    } finally {
      await stub.close();
  ...
  it("resolves a video attached to a steer message", async () => {
    ...
    try {
      ...
      ctx.agent.turn.steer([
        { type: "video_url", videoUrl: { url: fileUrl(path) } },
      ]);
      await ctx.untilTurnEnd();
      ...
      const steered = ctx.agent.context.messages.filter(
        (m) => m.role === "user",
      );
      const hasFileUrl = steered.some((m) =>
        m.content.some(
          (p) => p.type === "video_url" && p.videoUrl.url.startsWith("file:"),
        ),
      );
      expect(hasFileUrl).toBe(false);
      const anyTag = steered.some((m) =>
        m.content.some(
          (p) => p.type === "text" && p.text.includes(`<video path="${path}">`),
        ),
      );
