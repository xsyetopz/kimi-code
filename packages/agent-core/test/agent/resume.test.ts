...
describe("Agent resume", () => {
  ...
  it("replays inline skill reminders after pending tool results before the next prompt", async () => {
    const persistence = new RecordingAgentPersistence(
      resumeDeferredSystemReminderHistory(),
    );
    const ctx = testAgent({ persistence });
  ...
  it("keeps delivered background notifications indexed after compaction replay", async () => {
    ...
    const persistence = new RecordingAgentPersistence([
      {
        ...
        message: {
          role: "user",
          content: [
            { type: "text", text: "already delivered background notification" },
          ],
          toolCalls: [],
          origin,
        },
    ...
    const sessionDir = await mkdtemp(
      join(tmpdir(), "kimi-bg-resume-delivered-"),
    );
    try {
      ...
      expect(
        ctx.agent.context.history.some(
          (message) => message.origin?.kind === "background_task",
        ),
      ).toBe(false);
  ...
  it("closes interrupted trailing tool calls with synthetic error results after resume", async () => {
    ...
    await ctx.rpc.prompt({
      input: [{ type: "text", text: "continue after resume" }],
    });
    await ctx.untilTurnEnd();
    ...
    expect(
      resumedAgain.agent.context.history.map((message) => message.role),
    ).toEqual(["user", "assistant", "tool", "tool", "user", "assistant"]);
    expect(textContent(resumedAgain.agent.context.history[3])).toContain(
      "Tool execution was interrupted before its result was recorded",
    );
    expect(textContent(resumedAgain.agent.context.history[4])).toBe(
...
describe("limitAgentReplayByTurns", () => {
  ...
  it("keeps the most recent user turns, treating system-triggered user messages as continuations", () => {
    const records = [
      ...
      replayMessage("user", "goal continuation", {
        kind: "system_trigger",
        name: "goal",
      }),
