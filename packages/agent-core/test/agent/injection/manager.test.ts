...
function installInjectors(
  manager: InjectionManager,
  injectors: DynamicInjector[],
): void {
  (manager as unknown as { injectors: DynamicInjector[] }).injectors =
    injectors;
}
...
describe("InjectionManager.onContextCompacted", () => {
  ...
  it("isolates compaction hook failures so later injectors still receive the notification", () => {
    ...
    installInjectors(ctx.agent.injection, [
      new BoomInjector(ctx.agent),
      recorder,
    ]);
...
describe("InjectionManager registration", () => {
  it("registers TodoListReminderInjector in the default injector chain", () => {
    ...
    const injectors = (
      ctx.agent.injection as unknown as { injectors: DynamicInjector[] }
    ).injectors;

    expect(
      injectors.some(
        (injector) => injector instanceof TodoListReminderInjector,
      ),
    ).toBe(true);
  });
});
...
describe("InjectionManager.injectAfterCompaction — active background tasks", () => {
  ...
  function backgroundReminderTexts(
