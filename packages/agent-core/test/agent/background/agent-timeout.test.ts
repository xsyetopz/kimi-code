...
describe("AgentBackgroundTask — timeoutMs", () => {
  ...
  it("external deadline marks task timed_out", async () => {
    ...
    const taskId = manager.registerTask(agentTask(hangForever, "hang"), {
      timeoutMs: 2_000,
    });
  ...
  it("internal TimeoutError rejection = generic failure with error reason", async () => {
    ...
    const taskId = manager.registerTask(
      agentTask(rejecting, "internal timeout"),
      {
        timeoutMs: 900_000,
      },
    );
  ...
  it("explicit timeoutMs is persisted on the task info", () => {
    ...
    vi.useFakeTimers({ toFake: ["setTimeout", "clearTimeout"] });
    const taskId = manager.registerTask(
      agentTask(new Promise(() => {}), "persist timeout"),
      {
        timeoutMs: 1_800_000,
      },
    );
    const info = manager.getTask(taskId);
    expect((info as unknown as { timeoutMs?: number }).timeoutMs).toBe(
      1_800_000,
    );
  });
  ...
  it("omitted timeoutMs leaves the task info field undefined", () => {
    const { manager } = createBackgroundManager();
    const taskId = manager.registerTask(
      agentTask(new Promise(() => {}), "default timeout"),
    );
    const info = manager.getTask(taskId);
    expect(
      (info as unknown as { timeoutMs?: number }).timeoutMs,
    ).toBeUndefined();
  });
  ...
  it("timeoutMs=0 is preserved on the task info and does not arm a deadline", async () => {
    const { manager } = createBackgroundManager();
    const taskId = manager.registerTask(
      agentTask(new Promise(() => {}), "zero timeout"),
      {
        timeoutMs: 0,
      },
    );
