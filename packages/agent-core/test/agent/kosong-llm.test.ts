...
describe("KosongLLM stream timing", () => {
  it("returns timing measured from provider request start to stream end", async () => {
    ...
    expect(response.streamTiming?.firstTokenLatencyMs).toBeGreaterThanOrEqual(
      0,
    );
  ...
  it("splits first-token latency across the request-dispatch boundary", async () => {
    ...
    expect(
      (timing?.requestBuildMs ?? 0) + (timing?.serverFirstTokenMs ?? 0),
    ).toBe(timing?.firstTokenLatencyMs);
  });
  ...
  it("applies the model context window as the completion cap", async () => {
    ...
    const providerWithBudget: ChatProvider = {
      ...
      withMaxCompletionTokens(n: number) {
        ...
        return {
          ...this,
          withMaxCompletionTokens: this.withMaxCompletionTokens,
        };
      },
    };
...
describe("downgradeUnsupportedMedia", () => {
  ...
  const audioPart = {
    type: "audio_url",
    audioUrl: { url: "data:audio/mpeg;base64,AAA" },
  } as const;
  ...
  it("replaces video parts when the model lacks video_in and keeps the rest", () => {
    ...
    const input = [
      mediaMessage([{ type: "text", text: '<video path="a.mp4">' }, videoPart]),
    ];
  ...
  it("does not downgrade for UNKNOWN_CAPABILITY or an undefined capability", () => {
    ...
    expect(
      downgradeUnsupportedMedia(input, UNKNOWN_CAPABILITY)[0]?.content,
    ).toEqual([videoPart]);
    expect(downgradeUnsupportedMedia(input, undefined)[0]?.content).toEqual([
      videoPart,
    ]);
  });
