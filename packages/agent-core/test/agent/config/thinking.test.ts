...
const alwaysThinkingModel = model({
...
describe("defaultThinkingEffortFor", () => {
  ...
  it("falls back to the middle supportEfforts entry when defaultEffort is absent", () => {
    ...
    expect(
      defaultThinkingEffortFor(
        model({ capabilities: ["thinking"], supportEfforts: ["low"] }),
      ),
    ).toBe("low");
  });
  ...
  it("returns on for boolean thinking models (thinking support without supportEfforts)", () => {
    ...
    expect(
      defaultThinkingEffortFor(model({ capabilities: ["always_thinking"] })),
    ).toBe("on");
    expect(defaultThinkingEffortFor(model({ adaptiveThinking: true }))).toBe(
      "on",
    );
  });
});
...
describe("resolveThinkingEffort", () => {
  it("returns the requested effort verbatim when one is provided", () => {
    ...
    expect(resolveThinkingEffort("on", { enabled: false }, booleanModel)).toBe(
      "on",
    );
  ...
  it("returns off when config.enabled is false and no effort is requested", () => {
    expect(
      resolveThinkingEffort(undefined, { enabled: false }, effortModel),
    ).toBe("off");
    expect(
      resolveThinkingEffort(
        undefined,
  ...
  it("uses config.effort as the default effort", () => {
    expect(
      resolveThinkingEffort(undefined, { effort: "high" }, effortModel),
    ).toBe("high");
    expect(
      resolveThinkingEffort(
        undefined,
  ...
  it("falls back to defaultThinkingEffortFor(model) when no effort is configured", () => {
    expect(resolveThinkingEffort(undefined, undefined, effortModel)).toBe(
      "medium",
    );
  ...
  it("forces always-thinking models back on when the resolved effort is off", () => {
    expect(
      resolveThinkingEffort("off", undefined, alwaysThinkingModel, true),
    ).toBe("on");
    expect(
      resolveThinkingEffort(
        undefined,
  ...
  it("clamps always-thinking models to their default effort on every protocol", () => {
    ...
    expect(
      resolveThinkingEffort("off", undefined, alwaysThinkingEffortModel, false),
    ).toBe("high");
    expect(
      resolveThinkingEffort(
        undefined,
    ...
    expect(
      resolveThinkingEffort("off", undefined, alwaysThinkingModel, false),
    ).toBe("on");
  });

  it("normalizes a configured off value (case/whitespace) instead of sending it upstream", () => {
    expect(
      resolveThinkingEffort(undefined, { effort: " OFF " }, effortModel, false),
    ).toBe("off");
    expect(
      resolveThinkingEffort(undefined, { effort: "Off" }, booleanModel, false),
    ).toBe("off");
  ...
  it("reads a whitespace-only configured effort as absent, not as an empty effort", () => {
    expect(
      resolveThinkingEffort(undefined, { effort: "   " }, effortModel, false),
    ).toBe("medium");
    expect(
      resolveThinkingEffort(
        undefined,
  ...
  it("normalizes the requested effort (case/whitespace) on every wire", () => {
    expect(resolveThinkingEffort(" OFF ", undefined, effortModel, false)).toBe(
      "off",
    );
    expect(resolveThinkingEffort(" Max ", undefined, effortModel, false)).toBe(
      "max",
    );
    expect(resolveThinkingEffort("  ", undefined, effortModel, false)).toBe(
      "medium",
    );
  });
  ...
  it("does not force on for models that are not always-thinking", () => {
    expect(resolveThinkingEffort("off", undefined, booleanModel)).toBe("off");
    expect(
      resolveThinkingEffort(undefined, { enabled: false }, booleanModel),
    ).toBe("off");
  });

  it("falls back to the model default for an unsupported Kimi effort", () => {
    expect(resolveThinkingEffort("ultra", undefined, effortModel, true)).toBe(
      "medium",
    );
  });

  it("projects a concrete effort to on for a boolean-only Kimi model", () => {
    expect(resolveThinkingEffort("ultra", undefined, booleanModel, true)).toBe(
      "on",
    );
  });
