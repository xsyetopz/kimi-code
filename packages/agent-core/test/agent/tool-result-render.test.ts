...
describe("renderToolResultForModel", () => {
  describe("string output (and its single-text-part history form)", () => {
    it("passes successful output through unchanged", () => {
      expect(renderToolResultForModel({ output: "hello" })).toEqual([
        text("hello"),
      ]);
      expect(renderToolResultForModel({ output: [text("hello")] })).toEqual([
        text("hello"),
      ]);
    });
    ...
    it("replaces empty or whitespace-only success output with the placeholder", () => {
      expect(renderToolResultForModel({ output: "" })).toEqual([
        text(EMPTY_STATUS),
      ]);
    ...
    it("recognizes the plain record placeholder and emits the wrapped form", () => {
      ...
      expect(
        renderToolResultForModel({ output: "Tool output is empty." }),
  ...
  describe("content-part array output", () => {
    it("passes a media-bearing array through unchanged on success", () => {
      const parts = [
        ...
        {
          type: "image_url",
          imageUrl: { url: "data:image/png;base64,x" },
        } as const,
      ];
    ...
    it("prepends the wrapped error status as its own part on a multi-part error", () => {
      ...
      expect(
        renderToolResultForModel({ output: parts, isError: true }),
      ).toEqual([text(ERROR_STATUS), ...parts]);
    });
    ...
    it("collapses an empty-equivalent array to the placeholder", () => {
      ...
      expect(
        renderToolResultForModel({ output: [text("")], isError: true }),
  ...
  describe("note", () => {
    ...
    it("does not wrap or alter the note text", () => {
      expect(
        renderToolResultForModel({ output: "body", note: "plain words" }),
      ).toEqual([text("body\nplain words")]);
    });
    ...
    it("ignores an empty note", () => {
      expect(renderToolResultForModel({ output: "body", note: "" })).toEqual([
        text("body"),
      ]);
    });
  });
