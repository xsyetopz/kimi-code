...
describe("skill discovery", () => {
  it("resolves documented roots in precedence order with brand merging enabled by default", async () => {
    ...
    await mkdir(path.join(repoDir, ".kimi-code", "skills"), {
      recursive: true,
    });
    await mkdir(path.join(repoDir, ".agents", "skills"), { recursive: true });
    await mkdir(path.join(homeDir, ".kimi-code", "skills"), {
      recursive: true,
    });
    ...
    expect(roots.map((root) => path.relative(realRepoDir, root.path))).toEqual([
      ...
      path.relative(
        realRepoDir,
        await realpath(path.join(homeDir, ".agents", "skills")),
      ),
      "team-skills",
    ]);
  ...
  it("lets explicit skill dirs replace automatic project and user discovery", async () => {
    ...
    expect(
      roots.map((root) => [path.relative(realRepoDir, root.path), root.source]),
    ).toEqual([
  ...
  it("discovers flat markdown skills, keeps directory skills over same-name flat files, and preserves source precedence", async () => {
    ...
    await writeSkill(projectRoot, "review.md", [
      "Project review body first line.",
      "",
      "Details.",
    ]);
    ...
    await writeSkill(projectRoot, "deploy.md", [
      "Flat deploy should be ignored.",
    ]);
    ...
    expect(
      skills.map((skill) => [skill.name, skill.description, skill.source]),
    ).toEqual([
      ["deploy", "Directory deploy", "project"],
      ["review", "Project review body first line.", "project"],
    ]);
    expect(
      warnings.some((message) => message.includes("Ignoring flat skill")),
    ).toBe(true);
  });
  ...
  it("skips directory skills with missing frontmatter metadata", async () => {
    ...
    await writeSkill(
      projectRoot,
      path.join("missing-description", "SKILL.md"),
      [
        "---",
    ...
    expect(
      warnings.some((message) => message.includes("Missing frontmatter")),
    ).toBe(true);
    expect(warnings.some((message) => message.includes('"name"'))).toBe(true);
    expect(warnings.some((message) => message.includes('"description"'))).toBe(
      true,
    );
  });
});
...
describe("discoverSkills shape and ordering", () => {
  it("parses frontmatter name/description for subdir skills", async () => {
    ...
    expect(
      skills.map((skill) => [skill.name, skill.description, skill.source]),
    ).toEqual([["alpha-skill", "Alpha description", "user"]]);
  });
  ...
  it("discovers both flat and subdir skills alongside in the same root", async () => {
    ...
    expect(skills.map((s) => s.name).toSorted()).toEqual([
      "flat-skill",
      "subdir-skill",
    ]);
  });
  ...
  it("discovers nested SKILL.md files inside a skill bundle when has-sub-skill is declared", async () => {
    ...
    await writeSkill(
      root,
      path.join("outer", "references", "inner", "SKILL.md"),
      [
        "---",
        "name: inner",
    ...
    expect(
      skills.find((s) => s.name === "outer")?.metadata.isSubSkill,
    ).toBeUndefined();
    expect(
      skills.find((s) => s.name === "outer.inner")?.metadata.isSubSkill,
    ).toBe(true);
  });
  ...
  it("skips node_modules when scanning nested directories", async () => {
    ...
    await writeSkill(
      root,
      path.join("node_modules", "pkg", "vendored", "SKILL.md"),
      ["---", "name: vendored", "description: Should never load", "---"],
    );
...
describe("resolveSkillRoots ordering and priority", () => {
  ...
  it("treats empty explicit dirs identically to omitting them", async () => {
    ...
    const omitted = await resolveSkillRoots({
      paths: { userHomeDir: homeDir, workDir },
    });
...
describe("resolveSkillRoots brand home (KIMI_CODE_HOME)", () => {
  it("resolves the user brand skills root under brandHomeDir while keeping generic under the real home", async () => {
    ...
    const userRoots = roots
      .filter((r) => r.source === "user")
      .map((r) => r.path);
    expect(userRoots).toContain(
      await realpath(path.join(brandHomeDir, "skills")),
    );
    expect(userRoots).toContain(
      await realpath(path.join(homeDir, ".agents", "skills")),
    );
  ...
  it("never nests a second .kimi-code under the brand home", async () => {
    ...
    await mkdir(path.join(brandHomeDir, ".kimi-code", "skills"), {
      recursive: true,
    });
...
describe("resolveSkillRoots extra dirs", () => {
  ...
  it("silently drops missing extra-dir entries", async () => {
    ...
    const roots = await resolveSkillRoots({
      ...
      extraDirs: [
        real,
        path.join(repoDir, "nowhere"),
        path.join(repoDir, "nope"),
      ],
    });
  ...
  it("keeps plugin-specific skill lookup when a project skill has the same name", async () => {
    ...
    expect(registry.getSkill("using-superpowers")?.content).toBe(
      "project body",
    );
    expect(
      registry.getPluginSkill("superpowers", "using-superpowers")?.content,
    ).toBe("plugin body");
  });
...
describe("project root discovery (.git walk-up)", () => {
  ...
  it("falls back to the work dir when no .git marker is found anywhere up the chain", async () => {
    ...
    await mkdir(path.join(project, ".kimi-code", "skills"), {
      recursive: true,
    });
    ...
    expect(
      projectPaths.some((p) =>
        p.includes(path.join("project", ".kimi-code", "skills")),
      ),
    ).toBe(false);
  });
});
