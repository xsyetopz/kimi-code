...
describe("builtin skill: update-config", () => {
  ...
  it("registers through registerBuiltinSkills and shows up as model-invocable", () => {
    ...
    expect(
      registry
        .listInvocableSkills()
        .some((skill) => skill.name === "update-config"),
    ).toBe(true);
  });
});
...
describe("builtin skill: check-kimi-code-docs", () => {
  ...
  it("is model-invocable (does not disable model invocation)", () => {
    expect(CHECK_KIMI_CODE_DOCS_SKILL.metadata.disableModelInvocation).not.toBe(
      true,
    );
  });
  ...
  it("registers through registerBuiltinSkills and shows up as model-invocable", () => {
    ...
    expect(
      registry
        .listInvocableSkills()
        .some((skill) => skill.name === "check-kimi-code-docs"),
    ).toBe(true);
  });
});
