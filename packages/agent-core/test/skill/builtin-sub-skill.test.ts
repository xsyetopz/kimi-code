...
describe("builtin skill: sub-skill", () => {
  ...
  it("registers through registerBuiltinSkills but stays out of the model skill listing", () => {
    ...
    expect(
      registry
        .listInvocableSkills()
        .some((skill) => skill.name === "sub-skill"),
    ).toBe(false);
  });
  ...
  it("remains visible in the full skill list for CLI display", () => {
    ...
    expect(
      registry.listSkills().some((skill) => skill.name === "sub-skill"),
    ).toBe(true);
  });
...
describe("builtin skill: sub-skill.review", () => {
  ...
  it("registers through registerBuiltinSkills", () => {
    ...
    expect(
      registry
        .listInvocableSkills()
        .some((skill) => skill.name === "sub-skill.review"),
    ).toBe(false);
  });
});
...
describe("builtin skill: sub-skill.consolidate", () => {
  ...
  it("registers through registerBuiltinSkills", () => {
    ...
    expect(
      registry
        .listInvocableSkills()
        .some((skill) => skill.name === "sub-skill.consolidate"),
    ).toBe(false);
  });
});
