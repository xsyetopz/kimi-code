...
describe("KimiCore plugin RPCs", () => {
  ...
  it("installPlugin ignores forged marketplace context from public RPC callers", async () => {
    ...
    expect(
      (installed as { marketplace?: unknown }).marketplace,
    ).toBeUndefined();
  });
  ...
  it("setPluginMcpServerEnabled toggles plugin MCP state", async () => {
    ...
    await core.setPluginMcpServerEnabled({
      id: "demo",
      server: "finance",
      enabled: true,
    });
...
`,
  ...
  it("throws PLUGIN_LOAD_FAILED on every RPC when installed.json is corrupt", async () => {
    ...
    await expect(
      core.installPlugin({ source: "/tmp/nonexistent" }),
    ...
    await expect(
      core.setPluginEnabled({ id: "demo", enabled: false }),
    ).rejects.toThrow(/load/i);
    await expect(
      core.setPluginMcpServerEnabled({
        id: "demo",
        server: "finance",
        enabled: true,
      }),
