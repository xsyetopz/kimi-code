...
const slowToolStdioFixture = join(
  here,
  "fixtures",
  "slow-tool-stdio-server.mjs",
);
const crashAfterConnectFixture = join(
  here,
  "fixtures",
  "crash-after-connect-stdio-server.mjs",
);
const stderrThenExitFixture = join(
  here,
  "fixtures",
  "stderr-then-exit-stdio-server.mjs",
);
...
describe("McpConnectionManager", () => {
  ...
  it("marks HTTP servers failed when configured bearer token env var is missing", async () => {
    ...
    try {
      ...
      expect(entry?.error).toContain(
        '"REMOTE_MCP_TOKEN" is not set or is empty',
      );
    } finally {
      await cm.shutdown();
    }
  ...
  it("marks SSE servers failed when configured bearer token env var is missing", async () => {
    ...
    try {
      ...
      expect(entry?.status).toBe("failed");
      expect(entry?.error).toContain(
        '"LEGACY_MCP_TOKEN" is not set or is empty',
      );
    } finally {
      await cm.shutdown();
    }
  ...
  it("does not let stale in-flight startup failures overwrite a reconnect attempt", async () => {
    ...
    try {
      ...
      expect(
        seen
          .filter((event) => event.name === "slow")
          .map((event) => event.status),
      ).toEqual(["pending", "pending", "connected"]);
    } finally {
      await cm.shutdown();
  ...
  it("reconnect throws a coded KimiError when the server name is unknown", async () => {
    ...
    try {
      ...
      await expect(cm.reconnect("nope")).rejects.toMatchObject({
  ...
  it("shutdown cancels in-flight startup without late status updates", async () => {
    const cm = new McpConnectionManager();
    const hangingListFixture = join(
      here,
      "fixtures",
      "hanging-list-stdio-server.mjs",
    );
    const seen: Array<{ name: string; status: McpServerEntry["status"] }> = [];
    cm.onStatusChange((entry) => {
      seen.push({ name: entry.name, status: entry.status });
  ...
  it("applies defaultToolTimeoutMs when the server entry omits toolTimeoutMs", async () => {
    ...
    try {
      ...
      await expect(
        client.callTool("slow_echo", { text: "hi" }),
  ...
  it("marks an explicitly OAuth HTTP server as needs-auth when non-auth headers accompany a 401", async () => {
    ...
    const oauthService = new McpOAuthService({
      store: new JsonFileStore(storeDir),
    });
    ...
    try {
      await cm.connectAll({
        crashy: {
          ...
          env: {
            KIMI_TEST_MCP_EXIT_AFTER_MS: "500",
            KIMI_TEST_MCP_STDERR: "fatal: out of memory",
          },
    ...
    try {
      ...
      const internalClient = (
        cm as unknown as {
          entries: Map<
            string,
            { client?: { client: { onerror?: (e: Error) => void } } }
          >;
        }
      ).entries.get("remote")?.client?.client;
      internalClient?.onerror?.(
...
describe("Session MCP startup", () => {
  ...
  it("times out tool calls using the Session's global MCP config", async () => {
    const tmp = await mkdtemp(
      join(tmpdir(), "kimi-session-mcp-global-timeout-"),
    );
    const session = new Session({
  ...
  it("waits for initial MCP startup before the first prompt reaches the model", async () => {
    ...
    try {
      ...
      expect(events.some((event) => event.type === "turn.step.started")).toBe(
        false,
      );
  ...
  it("emits tool.list.updated(mcp.disconnected) when reconnect drops the live tools", async () => {
    ...
    try {
      ...
      const disconnects = events.filter(
        (e) =>
          e.type === "tool.list.updated" && e.reason === "mcp.disconnected",
      );
