...
describe("MCP OAuth credential identity", () => {
  ...
  it("scopes hasTokens to the server URL, not just the configured name", () => {
    ...
    expect(service.hasTokens("linear", "https://first.example.com/mcp")).toBe(
      true,
    );
    expect(service.hasTokens("linear", "https://second.example.com/mcp")).toBe(
      false,
    );
  });
  ...
  it("removes stored credentials when a server authorization is reset", () => {
    ...
    expect(service.hasTokens("linear", "https://mcp.example.com/mcp")).toBe(
      false,
    );
  });
