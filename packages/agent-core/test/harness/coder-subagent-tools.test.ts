...
function createVerifyGenerate(steps: StepRecord[]): GenerateFn {
  ...
  const generate: GenerateFn = async (
    ...
    const wireTools = tools
      ...
      .map((tool) => tool.name);
    const firstUserText =
      toolText(history.find((message) => message.role === "user")) ?? "";
    const isExploreChild = firstUserText.includes(
      "Probe nested subagent support",
    );
    ...
    if (isExploreChild) {
      ...
      const prevToolIsError =
        (last as { isError?: boolean } | undefined)?.isError === true;
      const step =
      ...
      switch (step) {
        ...
        case 2:
          parts = callTool("TodoList", {
            todos: [{ title: "verify-tools-e2e", status: "in_progress" }],
          });
          break;
        case 3:
          parts = callTool("Bash", {
        ...
        case 10:
          parts = callTool("Agent", {
            ...
            prompt:
              "Probe nested subagent support: confirm you can run and report back.",
...
describe("coder subagent aligned tools (real Session e2e)", () => {
  it("runs every newly aligned tool to success inside a real coder subagent", async () => {
    ...
    for (const expected of [
      ...
      expect(firstCallTools, `wire tools should include ${expected}`).toContain(
        expected,
      );
    }
    ...
    const byStep = (n: number) =>
      steps.find((record) => record.label === `step-${String(n)}`);
    const taskId =
      /task_id: (\S+)/.exec(byStep(4)?.prevToolOutput ?? "")?.[1] ?? "";

    expect(byStep(2)?.prevToolOutput).toContain(
      'Skill "demo-skill" loaded inline',
    );
    ...
    for (const record of steps) {
      expect(
        record.prevToolIsError,
        `${record.label} saw an error tool result`,
      ).toBe(false);
    }
  }, 30_000);
  ...
  it("blocks completion until the child background bash task settles", async () => {
    ...
    const handle = await mainAgent.subagentHost!.spawn({
      ...
      prompt:
        "Start a short background sleep, then finish without waiting for it.",
    ...
    expect(JSON.stringify(child.context.history)).not.toContain(
      "<notification ",
    );
  }, 30_000);
  ...
  it("aborting the run during the background drain rejects the completion", async () => {
    ...
    const handle = await mainAgent.subagentHost!.spawn({
      ...
      prompt:
        "Start a long background sleep, then finish without waiting for it.",
  ...
  it("completes cleanly when the background task settles before the final turn ends", async () => {
    ...
    const handle = await mainAgent.subagentHost!.spawn({
      ...
      prompt:
        "Start a quick background sleep, keep working briefly, then finish.",
