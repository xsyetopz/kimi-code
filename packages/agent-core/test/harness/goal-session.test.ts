...
describe("goal session end-to-end", () => {
  it("drives a goal across sequential turns until the model marks it complete", async () => {
    ...
    const continuationHistory = JSON.stringify(
      scripted.calls[1]?.history ?? [],
    );
    ...
    expect(continuationHistory).toContain(
      "impossible, unsafe, or contradictory",
    );
    expect(continuationHistory).toContain(
      "do not run more goal turns just to satisfy the audit",
    );
    ...
    const completionToolResult = events.find(
      (event) =>
        event["type"] === "tool.result" && event["toolCallId"] === "c1",
    );
    expect(String(completionToolResult?.["output"])).toContain(
    ...
    expect(summaryHistory).toContain(
      "Write a concise final message for the user",
    );
    ...
    const completion = records.find(
      (record) =>
        record["type"] === "goal.update" && record["status"] === "complete",
    );
  ...
  it("blocks at a turn budget (no wrap-up segment)", async () => {
    ...
    await agent.goal.setBudgetLimits(
    ...
    expect((await api.getGoal({ agentId: "main" })).goal?.status).toBe(
      "blocked",
    );
  ...
  it("continues goal mode after the model resumes a paused goal", async () => {
    ...
    scripted.mockNextResponse({
      type: "text",
      text: "I completed the resumed goal.",
    });
    ...
    expect(JSON.stringify(scripted.calls[3]?.history ?? [])).toContain(
      "Write a concise final message for the user",
    );
    expect((await api.getGoal({ agentId: "main" })).goal).toBeNull();
  });
  ...
  it("drives a goal the model creates mid-turn with CreateGoal", async () => {
    ...
    agent.turn.prompt([
      { type: "text", text: "Please start a goal to do the work" },
    ]);
    await agent.turn.waitForCurrentTurn();
    ...
    const completionEvent = events.find((event) => {
      ...
      return (
        event["type"] === "goal.updated" && change?.["kind"] === "completion"
      );
    });
  ...
  it("does not start a synthetic continuation when creating the goal exhausts its turn budget", async () => {
    ...
    scripted.mockNextResponse({
      type: "text",
      text: "Goal created with a one-turn budget.",
    });
  ...
  it("asks the model to explain why it marked a goal blocked", async () => {
    ...
    const blockedToolResult = events.find(
      (event) =>
        event["type"] === "tool.result" && event["toolCallId"] === "blocked",
    );
    ...
    expect(events).not.toContainEqual(
      expect.objectContaining({
        ...
        error: expect.objectContaining({
          code: ErrorCodes.LOOP_MAX_STEPS_EXCEEDED,
        }),
      }),
    );
    expect((await api.getGoal({ agentId: "main" })).goal).toBeNull();
  ...
  it("continues the goal when a goal turn hits maxStepsPerTurn", async () => {
      ...
      events.filter(
        (event) =>
          ...
          ["paused", "blocked"].includes(
            String(
              (event["snapshot"] as Record<string, unknown> | null)?.["status"],
            ),
          ),
      ),
    ).toEqual([]);
    ...
    const completion = events.find(
      (event) =>
        event["type"] === "goal.updated" &&
        (event["change"] as Record<string, unknown> | undefined)?.["kind"] ===
          "completion",
    );
    expect(
      (completion?.["change"] as Record<string, unknown>)?.["stats"],
    ).toMatchObject({
      turnsUsed: 3,
    });
  ...
  it("pauses the goal on provider connection errors", async () => {
    ...
    expect(goal?.terminalReason).toBe(
      "Paused after provider connection error: socket hang up",
    );
  });
  ...
  it("pauses the goal on runtime errors", async () => {
    ...
    expect(goal?.status).toBe("paused");
    expect(goal?.terminalReason).toBe(
      "Paused after runtime error: unexpected failure",
    );
  });
      ...
      [
        {
          ...
          command:
            "node -e \"process.stderr.write('blocked by policy'); process.exit(2)\"",
        },
      ],
    );
    const api = new SessionAPIImpl(session);
  ...
  it("preserves buffered steered input when the goal budget is reached", async () => {
    ...
    const generate: NonNullable<AgentOptions["generate"]> = async (
      ...
      calls += 1;
      if (calls > 1)
        throw new Error("Budget should stop before another model step");
      options?.signal?.throwIfAborted();
      options?.onRequestStart?.();
  ...
  it("preserves terminal status and demotes active goals across resume", async () => {
    ...
    expect(
      (await new SessionAPIImpl(resumed).getGoal({ agentId: "main" })).goal
        ?.status,
    ).toBe("paused");
  ...
  it("retains terminal blocked reason across resume", async () => {
    const sessionDir = await makeTempDir();
    const events: Array<Record<string, unknown>> = [];
    const { session, agent } = await setupSession(sessionDir, events, [
      "GetGoal",
    ]);
    await new SessionAPIImpl(session).createGoal({
      agentId: "main",
      objective: "work",
    });
    ...
    await resumed.resume();
    const goal = (
      await new SessionAPIImpl(resumed).getGoal({ agentId: "main" })
    ).goal;
    expect(goal?.status).toBe("blocked");
  ...
  it("supports user lifecycle controls without a model turn", async () => {
    ...
    expect(JSON.stringify(cancelReminder?.content)).toContain(
