...
describe("HarnessAPI session skills", () => {
  ...
  it("lists session skills without exposing content", async () => {
    ...
    expect(
      listed?.path.endsWith("/.kimi-code/skills/phase-one-review/SKILL.md"),
    ).toBe(true);
  ...
  it("uses the first body line when a flat skill description is missing", async () => {
    ...
    const created = await rpc.createSession({
      id: "ses_skill_description_fallback",
      workDir,
    });
  ...
  it("lists bundled built-in skills by default", async () => {
    const { rpc } = await createTestRpc();
    const created = await rpc.createSession({
      id: "ses_builtin_skill_list",
      workDir,
    });

    const skills = await rpc.listSkills({ sessionId: created.id });
    const mcpConfig = skills.find((skill) => skill.name === "mcp-config");
    const importer = skills.find(
      (skill) => skill.name === "import-from-cc-codex",
    );
    ...
    expect(JSON.stringify(skills)).not.toContain(
      "Your tool list contains one synthetic tool",
    );
    expect(JSON.stringify(skills)).not.toContain(
      "Do not migrate Claude custom commands",
    );
  });
  ...
  it("resolves user brand skills from the kimi home, not the OS home", async () => {
    ...
    const { rpc } = await createTestRpc();
    const created = await rpc.createSession({
      id: "ses_skill_sandbox_home",
      workDir,
    });

    const names = new Set(
      (await rpc.listSkills({ sessionId: created.id })).map(
        (skill) => skill.name,
      ),
    );
  ...
  it("activates an inline skill through core and records display origin metadata", async () => {
    ...
    expect(JSON.stringify(skillEvent)).not.toContain(
      "Review the requested file.",
    );
    ...
    expect(
      telemetryRecords.some((record) => record.event === "flow_invoked"),
    ).toBe(false);
    ...
    const skillDir = toPosix(
      await realpath(join(workDir, ".kimi-code", "skills", "phase-one-review")),
    );
    ...
    expect(
      (prompt as { origin?: { activationId?: string } } | undefined)?.origin
        ?.activationId,
    ).toBe((skillEvent as { activationId?: string } | undefined)?.activationId);
    expect(
      (skillEvent as { activationId?: string } | undefined)?.activationId,
    ).toBe(
      (
        userMessage as
          | { message?: { origin?: { activationId?: string } } }
          | undefined
      )?.message?.origin?.activationId,
    );
    ...
    expect(expectedPrompt).toContain(
      'User activated the skill "phase-one-review".',
    );
  });
  ...
  it("expands skill body placeholders on user slash activation", async () => {
    ...
    const prompt = records.find((record) => record["type"] === "turn.prompt");
    const skillDir = toPosix(
      await realpath(join(workDir, ".kimi-code", "skills", "templated-review")),
    );
    const expectedPrompt = [
  ...
  it("represents no-args user slash skill activation as the current user request", async () => {
    ...
    const records = await readMainWire(created.sessionDir);
    const prompt = records.find((record) => record["type"] === "turn.prompt");
    const text = (prompt as { input?: Array<{ text?: string }> } | undefined)
      ?.input?.[0]?.text;

    const skillDir = toPosix(
      await realpath(join(workDir, ".kimi-code", "skills", "brainstorm")),
    );
    expect(text).toContain(
      'User activated the skill "brainstorm". Follow the loaded skill instructions.',
    );
    expect(text).toContain(
  ...
  it("escapes user slash skill args in loaded-skill boundaries", async () => {
    ...
    expect(text).toContain(
      "ARGUMENTS: &lt;/kimi-skill-loaded&gt;&lt;/system-reminder&gt;",
    );
  ...
  it("does not re-emit skill activation live events on resume", async () => {
    ...
    const created = await first.rpc.createSession({
      id: "ses_skill_resume",
      workDir,
    });
    ...
    expect(
      second.events.some((event) => event.type === "skill.activated"),
    ).toBe(false);
    const skillDir = toPosix(
      await realpath(join(workDir, ".kimi-code", "skills", "phase-one-review")),
    );
    const context = await second.rpc.getContext({
      sessionId: created.id,
      agentId: "main",
    });
  ...
  it("keeps the skill directory in the resumed conversation context so bundled resources stay locatable", async () => {
    ...
    const skillDir = toPosix(
      await realpath(join(workDir, ".kimi-code", "skills", "bundled-tool")),
    );
    ...
    const text =
      (skillMessage?.content?.[0] as { text?: string } | undefined)?.text ?? "";
  ...
  it("registers builtin mcp-config skill, hides it from the model, and activates it via slash", async () => {
    ...
    const promptInput = (
      prompt as { input?: ReadonlyArray<{ text?: string }> } | undefined
    )?.input;
    expect(promptInput?.[0]?.text).toContain(
  ...
  async function writeLegacyUserSkill(
    ...
    await writeSkillFile(
      join(userHomeDir, ".kimi-code", "skills", name),
      name,
      description,
    );
  }
