...
function experimentalFeatureEnabled(
  core: KimiCore,
  id: string,
): boolean | undefined {
  return core.getExperimentalFeatures().find((feature) => feature.id === id)
    ?.enabled;
}
        ...
        return (
          ...
          if (String(path).endsWith("local.toml")) {
            return Promise.reject(
              new Error(
                `acp: readTextFile failed for ${path}: unknown session (issue #988)`,
              ),
            );
          }
...
`,
    ...
    const first = new KimiCore(async () => ({}) as never, {
      homeDir: firstHome,
    });
    const second = new KimiCore(async () => ({}) as never, {
      homeDir: secondHome,
    });
...
`,
    ...
    expect(
      mainAgent?.tools.data().some((tool) => tool.name === "CreateGoal"),
    ).toBe(true);
    ...
    const reloadedMainAgent = core.sessions
      .get(created.id)
      ?.getReadyAgent("main");
    expect(
      reloadedMainAgent?.tools
        .data()
        .some((tool) => tool.name === "CreateGoal"),
    ).toBe(true);
  });
  ...
  it("live-applies the complete persisted secondary recipe", async () => {
    ...
    const config = core.sessions
      .get(created.id)
      ?.getReadyAgent("main")?.kimiConfig;
    expect(config?.secondaryModel).toEqual({
      model: "default-mock",
      maxContextSize: 65_536,
    });
    expect(config?.models?.["__secondary__"]?.overrides?.maxContextSize).toBe(
      65_536,
    );
  });
  ...
  it("reads workspace local.toml through persistenceKaos during createSession", async () => {
    ...
    const created = await core.createSessionWithOverrides(
      {
        id: "ses_runtime_local_toml_bootstrap",
        workDir,
        model: "default-mock",
      },
...
`,
  ...
  it("enables Moonshot web services from KIMI_WEB_* env vars without a services config section", async () => {
    ...
    const [searchUrl, searchInit] = fetchImpl.mock.calls[0] as [
      string,
      RequestInit,
    ];
    expect(searchUrl).toBe("https://search-env.example/v1");
    expect(
      (searchInit.headers as Record<string, string>)["Authorization"],
    ).toBe("Bearer env-search-key");

    await urlFetcher!.fetch("https://example.com/page", {});
    const [fetchUrl, fetchInit] = fetchImpl.mock.calls[1] as [
      string,
      RequestInit,
    ];
...
`,
    ...
    const created = await rpc.createSession({
      id: "ses_runtime_service_env_precedence",
      workDir,
    });
    const session = core.sessions.get(created.id);
    ...
    expect(init.headers).toMatchObject({
      Authorization: "Bearer env-search-key",
    });
...
`,
  ...
  it("loads project local additional dirs into the session and main agent", async () => {
    ...
    expect(mainAgent?.config.systemPrompt).toContain(
  ...
  it("returns additionalDirs when resuming an active session", async () => {
    ...
    expect(core.sessions.get(created.id)?.getAdditionalDirs()).toEqual([
      extraDir,
    ]);
  });
  ...
  it("records a local-command-stdout message when adding a remembered dir", async () => {
    ...
    expect(
      core.sessions.get(created.id)?.getReadyAgent("main")?.getAdditionalDirs(),
    ).toEqual([extraDir]);
  });
  ...
  it("adds an additional dir through the session RPC", async () => {
    ...
    const localToml = await readFile(
      join(workDir, ".kimi-code", "local.toml"),
      "utf-8",
    );
    const session = core.sessions.get(created.id);
  ...
  it("adds a session-only additional dir without writing local.toml", async () => {
    ...
    await expect(
      readFile(join(workDir, ".kimi-code", "local.toml"), "utf-8"),
    ).rejects.toThrow();
  });
...
`,
  ...
  it("rejects reloadSession while the active session has a running turn", async () => {
    ...
    await expect(
      rpc.reloadSession({ sessionId: created.id }),
    ).rejects.toMatchObject({
  ...
  it("appends a fresh plugin_session_start reminder on forced reload", async () => {
    ...
    expect(reminders[0]).toContain(
      '<plugin_session_start plugin="demo" skill="greeter">',
    );
    expect(reminders[0]).toContain("NEW BODY");
    expect(reminders[0]).not.toContain("OLD BODY");
    expect(reminders[0]).toContain(
      "supersedes any earlier plugin_session_start",
    );
  ...
  it("neutralizes a stale plugin_session_start reminder when the plugin is removed", async () => {
    ...
    expect(reminders.at(-1)).toContain(
      "supersedes any earlier plugin_session_start",
    );
  });
  ...
  it("replaces only the plugin agent layer when a plugin is disabled before reload", async () => {
    ...
    expect(
      core.sessions.get(created.id)?.agentCatalog.get("plugin-reviewer"),
    ).toBeDefined();
    expect(
      core.sessions.get(created.id)?.agentCatalog.get("local-reviewer"),
    ).toBeDefined();
    ...
    expect(reloadedCatalog?.get("local-reviewer")?.description).toBe(
      "Local reviewer",
    );
  });
  ...
  it("neutralizes stale plugin guidance after compaction when no sessionStart is active", async () => {
    ...
    main?.context.appendMessage({
      ...
      content: [
        {
          type: "text",
          text: "summary of earlier conversation with plugin guidance",
        },
      ],
      toolCalls: [],
...
describe("KimiCore print-mode defaults", () => {
  ...
  async function setupDirs(
    configToml: string,
  ): Promise<{ homeDir: string; workDir: string }> {
...
async function writeSessionStartPlugin(
  root: string,
  skillBody: string,
): Promise<void> {
...
function pluginSessionStartReminders(
  core: KimiCore,
  sessionId: string,
): string[] {
