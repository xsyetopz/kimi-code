...
describe("createChild", () => {
  it("binds ctx that travels with every entry", async () => {
    await getRootLogger().configure(defaultConfig());
    const sessionLog = log.createChild({
      sessionId: "ses_a",
      model: "kimi-k2",
    });
...
describe("session routing", () => {
  ...
  it("keeps same-id sessions in different directories isolated", async () => {
    ...
    try {
      ...
      const firstText = await readFile(
        join(firstDir, "logs", "kimi-code.log"),
        "utf-8",
      );
      const secondText = await readFile(
        join(secondDir, "logs", "kimi-code.log"),
        "utf-8",
      );
  ...
  it("waits for a closing session sink when flushing by session id", async () => {
    ...
    try {
      ...
      await expect(getRootLogger().flushSession("ses_closing")).resolves.toBe(
        true,
      );
      await closing;
