...
describe("harness config TOML loader", () => {
  it("parses the current config.toml shape through explicit field mappings", () => {
    ...
    expect(config.services?.moonshotSearch?.customHeaders).toEqual({
      "X-Search": "1",
    });
    ...
    expect(config.raw?.["notifications"]).toEqual({
      claim_stale_after_ms: 15000,
    });
  });
...
`;
    ...
    expect(config.services?.moonshotSearch?.oauth?.oauthHost).toBe(
...
`;
    ...
    expect(parseConfigString(text, configPath).experimental).toEqual(
      config.experimental,
    );
  });
...
describe("config path env override", () => {
  it("uses KIMI_CODE_HOME when no explicit homeDir is supplied", () => {
    ...
    try {
      ...
      expect(resolveConfigPath({ configPath: "/tmp/custom.toml" })).toBe(
        "/tmp/custom.toml",
      );
    } finally {
...
`);
    ...
    expect(result.config.providers["kimi"]).toMatchObject({
      type: "kimi",
      apiKey: "sk-good",
    });
    expect(result.config.models?.["k2"]).toMatchObject({
    ...
    expect(result.config.raw?.["loop_control"]).toEqual({
      max_steps_per_turn: "not-a-number",
    });
  });
...
`);
  ...
  it("drops invalid top-level scalars and keeps the rest", async () => {
    const configPath =
      await writeTempConfig(`default_permission_mode = "not-a-mode"
${VALID_TOML}`);
    const result = loadRuntimeConfigSafe(configPath, {});
...
`);
    ...
    const overrides = models["kimi-code/kimi-k2"]?.["overrides"] as Record<
      string,
      unknown
    >;
...
describe("migrateThinkingEffortMaxToHigh", () => {
  ...
  async function readMarkers(home: string): Promise<Record<string, string>> {
    return JSON.parse(
      await readFile(join(home, "migrations-effort.json"), "utf-8"),
  ...
  it("rewrites a persisted max to high once and records the marker", async () => {
    ...
    expect(readConfigFile(configPath).thinking).toEqual({
      enabled: true,
      effort: "high",
    });
  ...
  it("skips an unparsable config without writing the marker", async () => {
    ...
    await expect(
      readFile(join(home, "migrations-effort.json"), "utf-8"),
    ).rejects.toThrow();
  });
});
