...
describe("applyEnvModelConfig", () => {
  ...
  it("defaults max_context_size to 262144 (256K) when unset", () => {
    expect(
      apply({ KIMI_MODEL_NAME: "m", KIMI_MODEL_API_KEY: "k" }).models?.[
        ENV_MODEL_ALIAS_KEY
      ]?.maxContextSize,
    ).toBe(262144);
  });
  ...
  it("applies provider type and its default base url", () => {
    expect(
      apply({ ...MIN, KIMI_MODEL_PROVIDER_TYPE: "openai" }).providers[
        ENV_MODEL_PROVIDER_KEY
      ],
    ).toMatchObject({
      type: "openai",
  ...
  it("sets display_name only when provided", () => {
    ...
    expect(
      apply({ ...MIN, KIMI_MODEL_DISPLAY_NAME: "Custom Model" }).models?.[
        ENV_MODEL_ALIAS_KEY
      ]?.displayName,
    ).toBe("Custom Model");
  });
  ...
  it("maps KIMI_MODEL_ADAPTIVE_THINKING onto the alias", () => {
    expect(
      apply({ ...MIN, KIMI_MODEL_ADAPTIVE_THINKING: "true" }).models?.[
        ENV_MODEL_ALIAS_KEY
      ]?.adaptiveThinking,
    ).toBe(true);
    expect(
      apply({ ...MIN, KIMI_MODEL_ADAPTIVE_THINKING: "false" }).models?.[
        ENV_MODEL_ALIAS_KEY
      ]?.adaptiveThinking,
    ).toBe(false);
    expect(
...
describe("stripEnvModelConfig (write-back guard)", () => {
  ...
  it("keeps user providers/models and a non-env default_model", () => {
    ...
    config.providers[ENV_MODEL_PROVIDER_KEY] = {
      type: "kimi",
      apiKey: "env-key",
    };
    config.models = {
