...
describe("workspace local config", () => {
  ...
  it("appends multiple directories and deduplicates normalized paths", async () => {
    ...
    const appended = await appendWorkspaceAdditionalDir(
      testKaos,
      root,
      "shared",
      [],
    ...
    const duplicate = await appendWorkspaceAdditionalDir(
      testKaos,
      root,
      "./shared",
      [],
    ...
    const second = await appendWorkspaceAdditionalDir(
      testKaos,
      root,
      "other",
      duplicate.additionalDirs,
    );
  ...
  it("expands a ~/ path to the home directory when appending", async () => {
    ...
    const homeProjectDir = await mkdtemp(
      join(homeDir, "kimi-workspace-local-home-"),
    );
