...
describe("secondaryModelPatch", () => {
  it("returns undefined when no patch field is set", () => {
    ...
    expect(
      secondaryModelPatch({ model: "cheap", defaultEffort: undefined }),
    ).toBeUndefined();
  });
...
describe("applySecondaryModelConfig", () => {
  ...
  it("merges the patch over the base entry overrides", () => {
    ...
    base.models = {
      cheap: {
        ...baseAlias,
        overrides: { maxContextSize: 32768, displayName: "base-name" },
      },
    };
...
describe("stripSecondaryModelConfig", () => {
  ...
  it("restores env-injected recipe fields from raw on write", () => {
    ...
    expect(runtime.secondaryModel).toEqual({
      model: "main",
      defaultEffort: "high",
    });
    ...
    expect(stripped.secondaryModel).toEqual({
      model: "cheap",
      defaultEffort: "low",
    });
  });
