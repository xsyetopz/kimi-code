...
describe("createDecorator (P0.3)", () => {
  ...
  it("toString() returns the diagnostic name", () => {
    const ILogger = createDecorator<{ log(m: string): void }>(
      "tostring-logger",
    );
  ...
  it("applying @IFoo on a single ctor parameter records {id, index: 0}", () => {
    ...
    (IFoo as unknown as (t: unknown, k: string, i: number) => void)(
      Target,
      "",
      0,
    );
    ...
    const deps = _util.getServiceDependencies(
      Target as unknown as _util.DI_TARGET_OBJ,
    );
  ...
  it("two decorators on the same ctor record both with correct indexes", () => {
    ...
    (IBar as unknown as (t: unknown, k: string, i: number) => void)(
      Target,
      "",
      1,
    );
  ...
  it("subclass does NOT inherit parent ctor metadata (storeServiceDependency reset)", () => {
    ...
    const parentDeps = _util.getServiceDependencies(
      Parent as unknown as _util.DI_TARGET_OBJ,
    );
    const childDeps = _util.getServiceDependencies(
      Child as unknown as _util.DI_TARGET_OBJ,
    );
