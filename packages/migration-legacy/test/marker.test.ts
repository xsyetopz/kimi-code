...
describe("marker", () => {
  ...
  it("writeMarker creates a new marker file with first_migrated_at = last_migrated_at", async () => {
    const summaryStub = {
      sessionsAttempted: 5,
    } as MarkerData["runs"][number]["summary"];
  ...
  it("suppresses migration for both targets when appending a run to a legacy marker", async () => {
    ...
    expect(
      shouldSuppressMigration({ sourceHome: dir, targetHome: firstTarget }),
    ).toBe(true);
    expect(
      shouldSuppressMigration({ sourceHome: dir, targetHome: secondTarget }),
    ).toBe(true);
  });
  ...
  it("does not suppress migration when the completed marker names another target", async () => {
    ...
    expect(
      shouldSuppressMigration({
        sourceHome: dir,
        targetHome: join(dir, "second-target"),
      }),
    ).toBe(false);
  });
  ...
  it("suppresses migration when the target contains the skip marker", async () => {
    ...
    await writeFile(
      join(targetHome, ".skip-migration-from-kimi-cli"),
      "",
      "utf-8",
    );
