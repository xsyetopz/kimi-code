...
describe("extractToolCallDisplays", () => {
  it("recovers the file diff from a real legacy wire fixture", async () => {
    const wire = await readFile(
      join(FIXTURES, "archived", "wire.jsonl"),
      "utf-8",
    );
...
describe("containsUsableMessage", () => {
  ...
  it("true when a user / assistant / tool row is present", () => {
    expect(containsUsableMessage(['{"role":"user","content":"hi"}'])).toBe(
      true,
    );
    expect(containsUsableMessage(['{"role":"assistant","content":[]}'])).toBe(
      true,
    );
    expect(containsUsableMessage(['{"role":"tool","content":"out"}'])).toBe(
      true,
    );
  });
  ...
  it("ignores malformed JSON and unknown roles", () => {
    ...
    expect(
      containsUsableMessage(["not-json", '{"role":"user","content":"x"}']),
    ).toBe(true);
  });
});
...
describe("analyzeContextContent", () => {
  it("'real' when there is at least one user / assistant / tool row", () => {
    expect(analyzeContextContent(['{"role":"user","content":"hi"}'])).toBe(
      "real",
    );
    expect(
      analyzeContextContent([
  ...
  it("'corrupt' when every non-blank line fails to parse", () => {
    ...
    expect(analyzeContextContent(["not-json", "{broken", "}}}"])).toBe(
      "corrupt",
    );
  });
