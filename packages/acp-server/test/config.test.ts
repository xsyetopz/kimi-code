...
describe("acp-server config surface", () => {
  ...
  it("session/set_mode pushes current_mode_update alongside config_option_update", async () => {
    ...
    const modeUpdatePromise = client!.waitForSessionUpdate(
      "current_mode_update",
    );
    const configUpdatePromise = client!.waitForSessionUpdate(
    ...
    const modeUpdate = (
      modeNotification.params as { update?: { currentModeId?: string } }
    ).update;
    ...
    const configUpdate = (
      configNotification.params as {
        update?: { configOptions?: readonly ConfigOption[] };
      }
    ).update;
    expect(
      configUpdate?.configOptions?.find((o) => o.id === "mode")?.currentValue,
    ).toBe("yolo");
  }, 30_000);
  ...
  it("session/set_mode propagates plan errors without reporting a new mode", async () => {
    ...
    expect(
      (session as unknown as { currentModeId: string }).currentModeId,
    ).toBe("default");
  ...
  it('the merged "<id>,thinking" form sets the bare model and flips thinking on at its default effort', async () => {
    ...
    expect(
      listed.configOptions.find((o) => o.id === "model")?.currentValue,
    ).toBe(FAKE_MODEL_ALT_ID);
    await expect(agent.getModel()).resolves.toBe(FAKE_MODEL_ALT_ID);
    await expect(agent.getThinking()).resolves.toBe("high");
  }, 30_000);
  ...
  it("a bare set_model id does not turn thinking off", async () => {
    ...
    await client!.send("session/set_model", {
      sessionId,
      modelId: FAKE_MODEL_ALT_ID,
    });
    await expect(agent.getModel()).resolves.toBe(FAKE_MODEL_ALT_ID);
  ...
  it("session/set_config_option thinking takes effect and pushes config_option_update", async () => {
    ...
    expect(
      result.configOptions.find((o) => o.id === "thinking")?.currentValue,
    ).toBe("off");
    ...
    expect(
      update?.configOptions?.find((o) => o.id === "thinking")?.currentValue,
    ).toBe("off");
  }, 30_000);
  ...
  it("an effort-capable model advertises off + every declared effort", async () => {
    ...
    const { configOptions } = await newSession();
    const thinking = configOptions.find((o) => o.id === "thinking");
    expect(thinking?.options?.map((o) => o.value)).toEqual([
      "off",
      "low",
      "high",
    ]);
...
`,
  ...
  it("a $/cancel_request notification is accepted without error", async () => {
    ...
    const list = (await client!.send("session/list", {})) as {
      sessions: unknown[];
    };
