...
function createFakeOAuthToolkit(): {
  ...
  const fake = {
    login: () =>
      Promise.reject(new Error("fakeOAuthToolkit: login not implemented")),
    ...
    getManagedUsage: () =>
      Promise.reject(new Error("fakeOAuthToolkit: not implemented")),
    getManagedUserInfo: () =>
      Promise.reject(new Error("fakeOAuthToolkit: not implemented")),
...
describe("acp-server session lifecycle", () => {
  ...
  it("session/new creates a live session and session/list returns it", async () => {
    ...
    expect(listed.sessions.some((s) => s.sessionId === created.sessionId)).toBe(
      true,
    );
  }, 30_000);
  ...
  it("session/delete removes the session and a second delete reports invalid_params", async () => {
    ...
    const deleted = await c.send("session/delete", {
      sessionId: created.sessionId,
    });
  ...
  it("session/delete on an unknown sessionId fails with invalid_params", async () => {
    const c = await boot();
    await expect(
      c.send("session/delete", { sessionId: "does-not-exist" }),
    ).rejects.toThrow(/-32602/);
  }, 30_000);
  ...
  it("a title change pushes session_info_update", async () => {
    ...
    await c.server.klient
      .session(created.sessionId)
      .setTitle("Renamed Session");

    const notification = await c.waitForSessionUpdate(
      "session_info_update",
      10_000,
    );
    const update = (
      notification.params as { update?: { title?: string | null } }
    ).update;
  ...
  it("logout drops the token and the auth gate closes again", async () => {
    ...
    await expect
      .poll(
        async () =>
          (await c.server.klient.global.auth.summarize()).some(
            (s) => s.loggedIn,
          ),
        { timeout: 10_000 },
      )
    ...
    await expect
      ...
      .toBe(false);
    await expect(
      c.send("session/new", { cwd: homeDir, mcpServers: [] }),
    ).rejects.toThrow(/[Aa]uthentication required/);
  }, 30_000);
  ...
  it("session/list filters by cwd when the client supplies one", async () => {
    ...
    const first = (await c.send("session/new", {
      cwd: homeDir,
      mcpServers: [],
    })) as {
      sessionId: string;
    };
    const second = (await c.send("session/new", {
      cwd: otherDir,
      mcpServers: [],
    })) as {
      sessionId: string;
...
describe("filterSessionSummariesByCwd", () => {
  ...
  it("keeps cwd-less legacy sessions under an explicit filter", () => {
    ...
    expect(filterSessionSummariesByCwd(items, "/x").map((s) => s.id)).toEqual([
      "a",
      "b",
    ]);
    expect(filterSessionSummariesByCwd(items, "/y").map((s) => s.id)).toEqual([
      "b",
      "c",
    ]);
  });
