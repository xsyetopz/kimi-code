...
describe("outcomeToQuestionAnswer", () => {
  it("returns the selected label keyed by the question text", () => {
    expect(
      outcomeToQuestionAnswer(sampleQuestion, selected("q0_opt_1")),
    ).toEqual({
      "Pick a color": "Green",
    });
  });
  ...
  it("returns null on skip", () => {
    expect(
      outcomeToQuestionAnswer(sampleQuestion, selected("q0_skip")),
    ).toBeNull();
  });

  it("returns null on an out-of-bounds or unknown optionId", () => {
    expect(
      outcomeToQuestionAnswer(sampleQuestion, selected("q0_opt_99")),
    ).toBeNull();
    expect(
      outcomeToQuestionAnswer(sampleQuestion, selected("mystery")),
    ).toBeNull();
  });
});
...
describe("elicitationResponseToQuestionAnswers", () => {
  ...
  it("drops values outside the declared options", () => {
    expect(
      elicitationResponseToQuestionAnswers(
        [sampleQuestion],
        elicitation({ q0: "Purple" }),
      ),
    ).toBeNull();
    expect(
      elicitationResponseToQuestionAnswers(
        [multiQuestion],
        elicitation({ q0: ["Auth", "Hack"] }),
      ),
  ...
  it("returns null on decline / cancel / content-less accept", () => {
    expect(
      elicitationResponseToQuestionAnswers([sampleQuestion], {
        action: "decline",
      }),
    ).toBeNull();
    expect(
