...
function commandsOf(notification: unknown): readonly AvailableCommand[] {
  const params = (
    notification as { params?: { update?: { availableCommands?: unknown } } }
  ).params;
  return (params?.update?.availableCommands ??
...
describe("acp-server skills / available commands", () => {
  ...
  async function bootWithFixtureSkill(): Promise<TestClient> {
    ...
    await mkdir(join(homeDir, ".kimi-code", "skills", "acp-fixture"), {
      recursive: true,
    });
    await writeFile(
  ...
  it("session/new pushes builtins first, then the session skills", async () => {
    ...
    expect(
      commands
        .slice(0, ACP_BUILTIN_SLASH_COMMANDS.length)
    ...
    expect(commands.some((command) => command.name === "write-goal")).toBe(
      true,
    );
    const compact = commands.find((command) => command.name === "compact");
    expect(compact?.input?.hint).toBe(
  ...
  it("pushes available_commands_update only after the session/new response settles", async () => {
    ...
    const responseIndex = c.received.findIndex(
      (m) =>
        (m.result as { sessionId?: string } | undefined)?.sessionId !==
        undefined,
    );
  ...
  it("advertises a workspace skill as `skill:<name>` with its description", async () => {
    ...
    const commands = commandsOf(notification);
    const fixture = commands.find(
      (command) => command.name === "skill:acp-fixture",
    );
  ...
  it("/skill:acp-fixture activates the skill and drives a normal turn", async () => {
    ...
    expect(
      (chunk.params as { update?: { content?: { text?: string } } }).update
        ?.content?.text,
    ).toContain("FIXTURE");
  ...
  it("supports a host-provided skill alias in the advertised palette", async () => {
    ...
    const aliasCommand = {
      name: alias,
      description: "Activate the fixture skill",
    };
    await c.close();
    ...
    client = aliasClient;
    await aliasClient.send("initialize", {
      protocolVersion: 1,
      clientCapabilities: {},
    });
    ...
    expect(result.stopReason).toBe("end_turn");
    expect(
      scripted!.callHistory()[0] && JSON.stringify(scripted!.callHistory()[0]),
    ).toContain("acp-fixture");
  }, 30_000);
});
