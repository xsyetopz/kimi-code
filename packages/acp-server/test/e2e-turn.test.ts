...
} from "./_helpers/scriptedProvider";
...
describe("acp-server real prompt turn (scripted LLM)", () => {
  ...
  it("drives initialize → new → prompt and streams the assistant text as agent_message_chunk", async () => {
    ...
    const update = (
      chunk.params as { update?: { content?: { text?: string } } }
    ).update;
    ...
    const usageUpdate = (
      usage.params as {
        update?: { used?: number; size?: number; cost?: unknown };
      }
    ).update;
  ...
  it("runs a tool call and bridges the approval request to the client", async () => {
    ...
    const text =
      terminal?.content?.map((c) => c.content?.text ?? "").join("\n") ?? "";
  ...
  it("bridges AskUserQuestion through elicitation/create for form-capable clients", async () => {
    ...
    expect(
      permissionRequests.filter((p) =>
        JSON.stringify(p).includes("AskUserQuestion"),
      ),
    ).toHaveLength(0);
    ...
    expect(elicitation.requestedSchema?.properties?.["q0"]?.type).toBe(
      "string",
    );
  ...
  it("attaches locations to a file tool call and its terminal update", async () => {
    ...
    const updateOf = (m: unknown) =>
      (m as { params?: { update?: ToolCallWire } }).params?.update;
    ...
    const createdNotification = await c.waitForSessionUpdate(
      "tool_call",
      10_000,
    );
    ...
    const upgrade = updates.find(
      (u) =>
        u?.sessionUpdate === "tool_call_update" && u?.locations !== undefined,
    );
  ...
  it("refreshes the tool card title on a status progress update and drops other progress kinds", async () => {
    ...
    const wireId = (create.params as { update?: { toolCallId?: string } })
      .update?.toolCallId;
    const turnId = Number(wireId?.split(":")[0]);
    const session = getLiveSessionById(
      c.server.core.accessor,
      created.sessionId,
    );
    const agentHandle = session?.accessor
...
describe("mapPromptLaunchError", () => {
  ...
  it("maps turn.agent_busy to invalidRequest (-32600)", () => {
    const error = Object.assign(
      new Error("Cannot activate skill while another turn is active"),
      {
  ...
  it("maps a generic rejection to a fixed internalError without leaking the raw message", () => {
    const mapped = mapPromptLaunchError(
      new Error("boom internal secret"),
      "sess-x",
    );
...
describe("acp-server prompt error hygiene", () => {
  ...
  it("a launch failure settles as a fixed internalError and never leaks the engine message", async () => {
    ...
    const serialized = JSON.stringify(
      (captured as Error)?.message ?? String(captured),
    );
...
describe("acp-server builtin slash commands (local execution, no LLM turn)", () => {
  ...
  it("/compact reports the completion summary as a follow-up agent_message_chunk", async () => {
    ...
    const { chunk, stopReason } = await runSlash(
      c,
      sessionId,
      "/compact keep it short",
    );
    expect(stopReason).toBe("end_turn");
    expect(chunk).toContain("Context compaction started");
      ...
      const hit = c
        ...
        .find(
          (u) =>
            ...
            u.content?.text?.includes(needle),
        );
      if (hit !== undefined) return hit.content?.text ?? "";
      if (Date.now() > deadline) {
        throw new Error(
          `timed out waiting for an agent_message_chunk containing '${needle}'`,
        );
      }
...
describe("acp-server terminal reverse-RPC (clientCapabilities.terminal)", () => {
  ...
  function fakeTerminalClient(
    ...
    c.onRequest("terminal/output", (params) => {
      const t = terminals.find(
        (x) => x.id === (params as { terminalId?: string }).terminalId,
      );
      return {
        output: t?.output ?? "",
        truncated: false,
  ...
  const toolCallUpdates = (c: TestClient): ToolCallUpdate[] =>
    c
      ...
      .map((m) => (m.params as { update?: ToolCallUpdate }).update)
      .filter(
        (u): u is ToolCallUpdate => u?.sessionUpdate === "tool_call_update",
      );
