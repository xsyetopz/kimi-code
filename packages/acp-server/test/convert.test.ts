...
describe("acpBlocksToContentParts", () => {
  it("projects file links and embedded text resources with provenance", () => {
    ...
    expect(parts).toEqual([
      ...
      {
        type: "text",
        text: '<resource uri="memory://note/1">remember this</resource>',
      },
    ]);
  });
});
...
describe("compressPromptImageParts", () => {
  ...
  afterEach(async () => {
    await Promise.all(
      trash.splice(0).map((dir) => rm(dir, { recursive: true, force: true })),
    );
  });
  ...
  it("drops an unsupported image format and stands a text notice in", async () => {
    ...
    expect((notice as { text: string }).text).toContain(
  ...
  it("rewrites accepted MIME aliases to the canonical form", async () => {
    ...
    const out = await compressPromptImageParts([
      imagePart(`data:IMAGE/PNG;base64,${base64}`),
    ]);
