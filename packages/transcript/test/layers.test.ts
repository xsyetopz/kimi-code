...
describe("granularity", () => {
  ...
  it("detachGrades writes explicit off so a wildcard default cannot resurrect the agent", () => {
    expect(detachGrades({ "*": "delta" }, ["main"])).toEqual({
      "*": "delta",
      main: "off",
    });
  ...
  it("detachGrades deletes a listed wildcard entry", () => {
    expect(detachGrades({ "*": "delta", main: "turn" }, ["*"])).toEqual({
      main: "turn",
    });
  });
  ...
  it("redactSnapshotForGrade strips step detail below block, keeps it at block+", () => {
    const snapshot: AgentTranscriptSnapshot = {
      ...
      todos: [
        {
          todoId: "todo",
          items: [{ title: "write tests", status: "in_progress" as const }],
        },
      ],
      prompts: [
        {
...
describe("contract schemas", () => {
  it("roundtrips every op kind", () => {
    const ops: TranscriptOperation[] = [
      ...
      {
        op: "todo.upsert",
        todo: { todoId: "todo", items: [{ title: "x", status: "done" }] },
      },
      promptOp,
      {
        op: "meta.merge",
        meta: { goal: { objective: "x", status: "active" } },
      },
  ...
  it("rejects path-hostile agent ids in the transcript query", () => {
    ...
    expect(
      transcriptQuerySchema.safeParse({ ...base, agent_id: "sub-1" }).success,
    ).toBe(true);
    expect(
      transcriptQuerySchema.safeParse({
        ...base,
        agent_id: "01HF7YAT31J7SMRT1QXGJWKR8D",
      }).success,
    ).toBe(true);
    for (const hostile of [
      ...
      expect(
        transcriptQuerySchema.safeParse({ ...base, agent_id: hostile }).success,
      ).toBe(false);
    }
  });
});
...
describe("groupMessagesIntoSnapshot (cold path)", () => {
  ...
  it("keeps cold tool calls running until a result is persisted", () => {
    ...
    const pendingTool = pendingTurn.steps[0]?.frames.find(
      (f) => f.kind === "tool",
    );
  ...
  it("opens a turn for subagent run prompts recorded as system triggers", () => {
    const snapshot = groupMessagesIntoSnapshot([
      ...
      },
      {
        ...
        toolCalls: [],
        origin: { kind: "system_trigger", name: "subagent" } as {
          kind: string;
        },
      },
      {
  ...
  it("starts a new turn for user-slash skill activations, keeps other triggers as markers only", () => {
    const snapshot = groupMessagesIntoSnapshot([
      ...
      {
        role: "assistant",
        content: [{ type: "text", text: "docs done" }],
        toolCalls: [],
      },
      {
        role: "user",
      ...
      {
        role: "assistant",
        content: [{ type: "text", text: "still same turn" }],
        toolCalls: [],
      },
    ]);
  ...
  it("starts a promptless turn for turn-opening system triggers (goal continuation)", () => {
    const snapshot = groupMessagesIntoSnapshot([
      ...
      },
      {
        ...
        toolCalls: [],
        origin: { kind: "system_trigger", name: "goal_continuation" } as {
          kind: string;
        },
      },
      {
      ...
      },
      {
        ...
        toolCalls: [],
        origin: { kind: "injection", variant: "permission_mode" } as {
          kind: string;
        },
      },
      {
    ...
    expect(
      first.steps.map((step) => step.frames.map((frame) => frame.kind)),
    ).toEqual([["text"]]);
  ...
  it("maps legacy background_task origins to task turns, preserving the taskId", () => {
    const snapshot = groupMessagesIntoSnapshot([
      ...
      },
      {
        ...
        toolCalls: [],
        origin: { kind: "background_task", taskId: "b83rhswvs" } as {
          kind: string;
        },
      },
    ]);
    ...
    expect(taskTurn.origin).toMatchObject({
      kind: "task",
      taskId: "b83rhswvs",
    });
  });
...
describe("foldWireRecordFacts (cold facts)", () => {
  ...
  it("folds goal create/update/clear into meta.goal with markers, last write wins", () => {
    ...
    expect(
      base.items.some(
        (item) => item.kind === "marker" && item.markerId === "m1",
      ),
    ).toBe(true);
    ...
    expect(
      goalMarkers.map((item) => item.kind === "marker" && item.markerId),
    ).toEqual(["m2", "m3", "m4"]);
  ...
  it("marks user-cancelled turns with interruption markers, skipping unattributable cancels", () => {
    ...
    expect(
      folded.items.filter(
        (item) => item.kind === "marker" && item.marker === "interruption",
      ),
    ).toEqual([
      {
        kind: "marker",
  ...
  it("maps unknown task kinds to other and survives malformed task records", () => {
    ...
    expect(folded.tasks[0]).toMatchObject({
      taskId: "task_q",
      kind: "other",
      state: "running",
    });
  ...
  it("cancels interactions still pending at the end of the scan (crash == cancelled)", () => {
    ...
    expect(
      folded.interactions.every((entity) => entity.state !== "pending"),
    ).toBe(true);
  });
