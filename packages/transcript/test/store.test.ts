...
describe("AgentTranscript", () => {
  ...
  it("appendAtOffset matches web alignDelta semantics", () => {
    expect(appendAtOffset("abc", 3, "d")).toEqual({
      text: "abcd",
      changed: true,
    });
    expect(appendAtOffset("abc", 1, "bc").changed).toBe(false);
    expect(appendAtOffset("abc", 1, "bcd")).toEqual({
      text: "abcd",
      changed: true,
    });
  ...
  it("appendAtOffset treats a mismatched overlap as a gap, never a rewrite", () => {
    ...
    expect(appendAtOffset("hello wo", 6, "world")).toEqual({
      text: "hello world",
      changed: true,
    });
  });
  ...
  it("tracks pending interactions as a derived index (entity channel)", () => {
    const tx = new AgentTranscript("main");
    const interaction = (
      state: TranscriptInteraction["state"],
    ...
    tx.apply([
      turn1,
      { op: "interaction.upsert", interaction: interaction("pending") },
    ]);
    expect(tx.listPendingInteractions()).toEqual(["appr-1"]);
    tx.apply([
      { op: "interaction.upsert", interaction: interaction("approved") },
    ]);
    ...
    tx.apply([
      { op: "interaction.upsert", interaction: unanchored("pending") },
    ]);
    expect(tx.listPendingInteractions()).toEqual(["appr-2"]);
    tx.apply([
      { op: "interaction.upsert", interaction: unanchored("answered") },
    ]);
    expect(tx.listPendingInteractions()).toEqual([]);
  });
  ...
  it("upserts attachment and todo entities idempotently", () => {
    ...
    const todo = {
      todoId: "todo",
      items: [{ title: "x", status: "pending" as const }],
    };
  ...
  it("upserts prompt queue entities by id, idempotently", () => {
    ...
    expect(
      tx.apply([{ op: "prompt.upsert", prompt: queued }]).accepted,
    ).toHaveLength(1);
    ...
    expect(
      tx.apply([{ op: "prompt.upsert", prompt: running }]).accepted,
    ).toHaveLength(1);
  ...
  it("tool frames keep streamed inputText and the newest progress update", () => {
    ...
    tx.apply(toolFrame("running"));
    const streamed = (
      frame: Partial<ToolCallFrame> &
        Pick<ToolCallFrame, "inputText" | "state">,
    ): TranscriptOperation => ({
      op: "frame.upsert",
      turnId: "t1",
      stepId: "t1.1",
    ...
    expect(
      tx.apply([streamed({ inputText: '{"path"', state: "running" })]).accepted,
    ).toHaveLength(1);
  ...
  it("receive() equals full reset seed; snapshot windowing keeps newest turns", () => {
    ...
    for (let n = 1; n <= 5; n += 1) {
      tx.apply([
        {
          op: "marker.upsert",
          item: { kind: "marker", markerId: `m${n}`, marker: "goal" },
        },
        {
          op: "turn.upsert",
          turn: {
  ...
  it("meta.merge merges goal/modes shallowly", () => {
    ...
    tx.apply([
      {
        op: "meta.merge",
        meta: { goal: { objective: "ship it", status: "active" } },
      },
  ...
  it("meta.merge shallow-merges the agent status key one level deep", () => {
    ...
    tx.apply([
      {
        op: "meta.merge",
        meta: { agent: { model: "k2", permission: "auto" } },
      },
    ...
    tx.apply([
      {
        op: "meta.merge",
        meta: { agent: { model: "k3", phase: { kind: "idle" } } },
      },
    ]);
    expect(tx.getMeta().agent).toEqual({
      model: "k3",
  ...
  it("snapshot immutability: later applies do not mutate earlier reads", () => {
    ...
    const beforeFrame =
      before[0]?.kind === "turn" ? before[0].steps[0]?.frames[0] : undefined;
  ...
  it("places anchored standalone items before their following turn, not at the end", () => {
    ...
    expect(tx.getItems().map(itemLabel)).toEqual([
      "t0",
      "m1",
      "t1",
      "r1",
  ...
  it("appends standalone items without an anchor at the end (live order)", () => {
    ...
    tx.apply([
      ...
      {
        op: "marker.upsert",
        item: { kind: "marker", markerId: "m9", marker: "notice" },
      },
    ]);
    const items = tx.getItems();
...
describe("TranscriptStore", () => {
  ...
  it("markDisposed stamps disposedAt on the existing descriptor only", () => {
    ...
    store.onRosterChange((agents) =>
      rosters.push(agents.map((a) => a.agentId)),
    );
