...
export function groupMessagesIntoSnapshot(
  ...
  for (const message of messages) {
    ...
    if (message.role === "user") {
      ...
      const markerKey =
        originKind !== undefined ? MARKER_USER_ORIGINS[originKind] : undefined;
      if (markerKey !== undefined) {
        pushMarker(markerKey, {
          text: textOf(message),
...
function syncTurnItem(items: TranscriptItem[], draft: TurnDraft): void {
  const index = items.findIndex(
    (entry) => entry.kind === "turn" && entry.turnId === draft.turnId,
  );
...
function replaceToolFrame(
  turn: TurnDraft,
  toolCallId: string,
  next: TranscriptFrame,
): void {
  for (let s = turn.steps.length - 1; s >= 0; s -= 1) {
