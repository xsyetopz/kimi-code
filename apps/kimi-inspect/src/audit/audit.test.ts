...
describe("diffValue", () => {
  ...
  it("marks added, removed, and modified object keys", () => {
    ...
    expect(node.children?.get("gone")).toMatchObject({
      status: "removed",
      prev: "x",
    });
  ...
  it("matches entity arrays by id instead of index", () => {
    ...
    const next = [
      turnItem(1),
      { ...turnItem(2), state: "running" as const },
      turnItem(3),
    ];
...
describe("serializeState", () => {
  it("turns maps into sorted plain objects and sets into arrays", () => {
    ...
    expect(Object.keys(out.tasks as Record<string, unknown>)).toEqual([
      "a-task",
      "b-task",
    ]);
...
describe("AuditTrail", () => {
  ...
  it("records entries with increasing indices, timestamps, and state references", () => {
    ...
    trail.recordOps(
      [{ op: "turn.upsert", turn: turnItem(2) }],
      "live",
    ...
    expect(
      entries.every(
        (entry) => typeof entry.at === "string" && entry.at.length > 0,
      ),
    ).toBe(true);
