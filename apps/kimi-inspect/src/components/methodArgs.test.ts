...
describe("parseParamFields", () => {
  it("parses a single named parameter", () => {
    expect(parseParamFields("sessionId")).toEqual([
      { kind: "value", name: "sessionId" },
    ]);
  });
  ...
  it("expands a destructured object parameter into keys", () => {
    expect(parseParamFields("{ workspaceId, limit }")).toEqual([
      {
        kind: "object",
        name: "{ workspaceId, limit }",
        keys: ["workspaceId", "limit"],
      },
    ]);
  });
  ...
  it("falls back to raw for rest and array patterns", () => {
    expect(parseParamFields("...args")).toEqual([
      { kind: "raw", label: "...args" },
    ]);
    expect(parseParamFields("[a, b]")).toEqual([
      { kind: "raw", label: "[a, b]" },
...
describe("buildArgs", () => {
  ...
  it("assembles a destructured object from its key fields", () => {
    ...
    expect(
      buildArgs(fields, { "0.workspaceId": "ws1", "0.limit": "5" }),
    ).toEqual([{ workspaceId: "ws1", limit: 5 }]);
  });
  ...
  it("truncates trailing empty params but keeps interior holes", () => {
    ...
    expect(
      JSON.parse(JSON.stringify(buildArgs(fields, { "0": "x", "2": "z" }))),
    ).toEqual(["x", null, "z"]);
  });
