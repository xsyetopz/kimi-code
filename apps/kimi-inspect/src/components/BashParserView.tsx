...
export function BashParserView() {
  ...
  const nodeCount =
    result !== null && result.ok ? countNodes(result.root) : null;
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col">
        <div className="flex items-center gap-2 border-b border-neutral-800 px-3 py-2">
          ...
          {result !== null && !result.ok ? (
            <Badge tone="red">aborted</Badge>
          ) : null}
          {result !== null && result.ok && result.hasError ? (
            <Badge tone="amber">hasError</Badge>
          ) : null}
          {nodeCount !== null ? (
            <Badge tone="neutral">{nodeCount} nodes</Badge>
          ) : null}
        </div>
        <div className="min-h-0 flex-1 overflow-auto p-2">
          {error !== null ? (
            ...
            <div className="text-[11px] text-neutral-500">
              ...
              <span className="font-mono">reason: {result.reason}</span>) — the
              tree cannot be analyzed; raise the budget or shrink the input.
            </div>
          ) : (
  ...
  return (
    <div>
      ...
      >
        ...
        <span className={node.isNamed ? "text-sky-300" : "text-neutral-500"}>
          {node.type}
        </span>
