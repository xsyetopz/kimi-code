...
async function writeInstance(
  ...
  await writeFile(
    join(dir, `${id}.json`),
    JSON.stringify({ server_id: id, ...disk }),
  );
}
...
describe("readLiveInstances", () => {
  it("lists live instances sorted by started_at, dropping dead pids and bad files", async () => {
    await writeInstance("b-older", {
      pid: ALIVE_PID,
      port: 58628,
      started_at: 100,
    });
    await writeInstance("a-newer", {
      pid: ALIVE_PID,
      port: 58629,
      started_at: 200,
    });
    await writeInstance("x-dead", {
      pid: DEAD_PID,
      port: 58630,
      started_at: 50,
    });
    ...
    await writeFile(
      join(home, "server", "instances", "garbage.json"),
      "{not json",
    );
...
describe("readLiveLock / readServerToken", () => {
  it("reads the legacy lock only when its pid is alive", async () => {
    ...
    await writeFile(
      join(home, "server", "lock"),
      JSON.stringify({ pid: DEAD_PID, port: 58627 }),
    );
...
describe("discoverLocalServers", () => {
  it("merges instances + lock + proxy target, deduped by url, with the home token", async () => {
    ...
    await writeFile(
      join(home, "server", "lock"),
      JSON.stringify({ pid: ALIVE_PID, port: 59000 }),
    );
...
describe("resolveKimiHomeDir", () => {
  it("honors KIMI_CODE_HOME, else falls back to ~/.kimi-code", () => {
    ...
    expect(resolveKimiHomeDir({})).toBe(
      join(process.env["HOME"] ?? "", ".kimi-code"),
    );
  });
});
