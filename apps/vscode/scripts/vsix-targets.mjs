...
export const extensionRoot = resolve(
...
export function normalizeVsixTargets(values) {
  const requested =
    values.length === 0 || values.includes("all") ? VSIX_TARGETS : values;
  if (values.includes("all") && values.length > 1) {
    throw new Error(
      'Target "all" cannot be combined with an explicit VSIX target.',
    );
  }
...
export function isMainModule(metaUrl) {
  ...
  return (
    argvPath !== undefined && pathToFileURL(resolve(argvPath)).href === metaUrl
  );
}
