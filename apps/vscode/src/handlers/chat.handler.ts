...
const streamChat: Handler<StreamChatParams, { done: boolean }> = async (
  params,
  ctx,
) => {
  if (!ctx.workDir) {
  ...
  try {
    const result = await runtime.prompt(
      prependSystemContext(params.content, systemContext),
    );
...
const respondApproval: Handler<RespondApprovalParams, { ok: boolean }> = async (
  params,
  ctx,
) => {
  return {
    ok:
      ctx.getSession()?.respondApproval(params.requestId, params.response) ??
      false,
  };
};

const respondQuestion: Handler<RespondQuestionParams, { ok: boolean }> = async (
  params,
  ctx,
) => {
...
const setPlanMode: Handler<
  { enabled: boolean },
  { ok: boolean; planMode: boolean }
> = async (params, ctx) => {
  const runtime = ctx.getSession();
