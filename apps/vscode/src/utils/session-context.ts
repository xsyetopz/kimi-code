...
export function buildExportMarkdown(input: {
  ...
  const topic =
    firstUser === undefined
      ? ""
      : shorten(stringifyParts(firstUser.content), 80);
...
export function stringifyContextHistory(
  ...
  for (const message of history) {
    ...
    const callId =
      message.role === "tool" && message.toolCallId
        ...
        : "";
    messages.push(
      `[${message.role.toUpperCase()}]${callId}\n${sections.join("\n")}`,
    );
  }
...
function isInternalMessage(message: ContextMessage): boolean {
  return (
    message.origin !== undefined && INTERNAL_ORIGINS.has(message.origin.kind)
  );
}
...
function formatTurn(
  ...
  for (const message of messages) {
    ...
    if (message.role === "tool") {
      const info = toolInfo.get(message.toolCallId ?? "") ?? {
        name: "unknown",
        hint: "",
      };
