...
function ServerItem({
  ...
  const handleUpdate = async () => {
    try {
      const servers = await bridge.updateMCPServer(
        server.name,
        formToConfig(form),
      );
  ...
  const handleResetAuth = () =>
    handleAction(() => bridge.resetAuthMCP(server.name));
          ...
          {testOutput && (
            <div className="text-[10px] font-mono bg-muted/50 rounded p-2 mb-2 max-h-48 overflow-auto border border-border/50">
              {testOutput.split("\n").map((line, i) => (
                <div
                  key={i}
                  className="whitespace-pre-wrap break-all min-h-[1.2em]"
                >
                  {line}
                </div>
              ))}
...
export function MCPServersModal() {
  const { mcpServers, mcpModalOpen, setMCPServers, setMCPModalOpen } =
    useSettingsStore();
  const [showAdd, setShowAdd] = useState(false);
  const [addForm, setAddForm] = useState<FormData>(() => emptyForm());
  const [installingRecommended, setInstallingRecommended] = useState<
    string | null
  >(null);
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="max-w-2xl mx-auto px-3 py-3 space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              {RECOMMENDED_MCP_SERVERS.every((s) =>
                installedNames.has(s.id),
              ) && (
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will remove "{deleteTarget}" from your configuration. This
