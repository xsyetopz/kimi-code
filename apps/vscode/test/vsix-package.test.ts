...
describe("VSIX verifier CLI (package contract and failure details)", () => {
  it("passes an unpacked Windows package when the entry is self-contained", async () => {
    ...
    expect(result.stdout).toContain(
      "static audit and entry import smoke passed (package-only)",
    );
  });
  ...
  it("reports the expected target when the VSIX manifest has a different target", async () => {
    ...
    expect(result.stderr).toContain(
      "VSIX manifest target is darwin-arm64, expected linux-x64",
    );
  });
  ...
  it("names a missing required Webview resource", async () => {
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).toContain(
      "Required VSIX resource is missing: extension/dist/webview.js",
    );
  });
...
describe("Extension Development Host setup (isolated local state)", () => {
  it("creates the complete isolated directory layout for a debug launch", async () => {
    ...
    expect(result.status).toBe(0);
    await expect(
      readFile(join(baseDir, "workspace", "README.md"), "utf8"),
    ).resolves.toContain("Isolated Kimi Code extension development workspace");
    await expect(directoryExists(join(baseDir, "user-data"))).resolves.toBe(
      true,
    );
    await expect(directoryExists(join(baseDir, "extensions"))).resolves.toBe(
      true,
    );
    await expect(directoryExists(join(baseDir, "kimi-home"))).resolves.toBe(
      true,
    );
  });
  ...
  it("refuses to clear a directory without the dedicated safety suffix", async () => {
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).toContain(
      "Refusing to reset unsafe development directory",
    );
    await expect(readFile(join(unsafeDir, "keep.txt"), "utf8")).resolves.toBe(
      "keep",
    );
  });
});
...
async function makeVsixFixture(target: string): Promise<string> {
  ...
  await Promise.all([
    ...
    writeFile(
      join(distDir, "webview.js"),
      "globalThis.__kimiWebview = true;\n",
    ),
