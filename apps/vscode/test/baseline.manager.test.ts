...
describe("file baselines (capture, compare, keep, and undo)", () => {
  ...
  it("isolates the same session id between different Kimi homes", async () => {
    ...
    await expect(firstHome.getContent(session, filePath)).resolves.toBe(
      "home A original\n",
    );
    await expect(secondHome.getContent(session, filePath)).resolves.toBe(
      "home B original\n",
    );
  });
  ...
  it("restores every tracked original when undo all runs", async () => {
    ...
    await expect(readFile(join(workDir, "one.ts"), "utf-8")).resolves.toBe(
      "one before",
    );
    await expect(readFile(join(workDir, "two.ts"), "utf-8")).resolves.toBe(
      "two before",
    );
  });
  ...
  it("refuses undo all when a tracked directory now links outside the workspace", async () => {
    ...
    await expect(manager.undoAll(session)).rejects.toThrow(
      "outside the session workspace",
    );
    await expect(readFile(join(outsideDir, "app.ts"), "utf-8")).resolves.toBe(
      "outside",
    );
  });
...
describe("legacy baselines (fallback, tombstones, and fork isolation)", () => {
  it("returns the legacy original when no local baseline exists", async () => {
    ...
    await expect(manager.getContent(session, "app.ts")).resolves.toBe(
      "legacy original\n",
    );
  });
  ...
  it("restores a legacy original without mutating its snapshot when undo runs", async () => {
    const { session, baselinePath } =
      await createLegacySession("legacy original\n");
    await writeFile(join(workDir, "app.ts"), "current content\n", "utf-8");

    await manager.undo(session, "app.ts");

    await expect(readFile(join(workDir, "app.ts"), "utf-8")).resolves.toBe(
      "legacy original\n",
    );
    await expect(readFile(baselinePath, "utf-8")).resolves.toBe(
      "legacy original\n",
    );
  });
  ...
  it("uses a materialized local original before the fork target legacy fallback", async () => {
    ...
    const { session: target } = await createLegacySession(
      "legacy original\n",
      "ses-target",
    );
    ...
    await expect(manager.getContent(target, "app.ts")).resolves.toBe(
      "local original\n",
    );
  });
  ...
  it("keeps the source baseline visible when the fork accepts its own copy", async () => {
    const { session: source } = await createLegacySession(
      "legacy original\n",
      "ses-source",
    );
    const target = createSession("ses-target", source.metadata);
    ...
    await expect(manager.getContent(source, "app.ts")).resolves.toBe(
      "legacy original\n",
    );
  });
  ...
  it("preserves accepted legacy paths when a fork is materialized", async () => {
    ...
    await expect(manager.getContent(target, "app.ts")).rejects.toThrow(
      "No baseline exists",
    );
  });
});
...
describe("baseline boundaries (errors, cleanup, and platform paths)", () => {
  ...
  it("throws a clear error when requested baseline content does not exist", async () => {
    await expect(
      manager.getContent(createSession(), "missing.ts"),
    ).rejects.toThrow('No baseline exists for "missing.ts"');
  });

  it("rejects a relative path that escapes the workspace", async () => {
    await expect(
      manager.capture(createSession(), "../outside.ts"),
  ...
  it("rejects a Windows path on another drive", async () => {
    ...
    await expect(
      manager.capture(session, "D:\\Workspace\\file.ts"),
    ).rejects.toThrow("is outside workspace");
  });
