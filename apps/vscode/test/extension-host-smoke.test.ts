...
const { runExtensionHostSmoke } = await import(
  "../scripts/extension-host-smoke.mjs"
);
...
beforeEach(() => {
  ...
  vscodeTest.runTests.mockImplementation(async (options) => {
    await writeFile(
      ...
      JSON.stringify({
        vscode: options.version === "stable" ? "1.127.0" : options.version,
      }),
      "utf8",
    );
  });
...
describe("installed VSIX Extension Host smoke", () => {
  ...
  it("rejects a cached host that does not match an exact requested version", async () => {
    ...
    await expect(
      ...
      "Extension Host ran VS Code 1.99.3, expected requested version 1.100.0",
    );
  });
});
