  ...
  it("sends a lossless structured MCP edit request to the extension host", async () => {
    ...
    vi.stubGlobal("acquireVsCodeApi", () => ({
      postMessage: (message: { id: string }) => {
        posted.push(message);
        queueMicrotask(() =>
          receiveMessage?.({ data: { id: message.id, result: [] } }),
        );
      },
      getState: () => undefined,
      setState: () => undefined,
...
describe("Webview chat error recovery", () => {
  ...
  it("stops the response and retains provider detail when a running turn fails", () => {
    ...
    useChatStore
      .getState()
      .processEvent({ type: "StepBegin", payload: { n: 1 } });
...
describe("Webview thinking mode parity with the TUI", () => {
  it("derives thinking modes from metadata only, mirroring the TUI rules", () => {
    ...
    expect(
      getModelThinkingMode({ ...base, capabilities: ["always_thinking"] }),
    ).toBe("always");
    expect(getModelThinkingMode({ ...base, capabilities: ["thinking"] })).toBe(
      "switch",
    );
    expect(getModelThinkingMode({ ...base, adaptive_thinking: true })).toBe(
      "switch",
    );
    expect(getModelThinkingMode({ ...base, name: "Kimi Thinking Pro" })).toBe(
      "none",
    );
