...
export async function loadPluginMarketplace(
  ...
  const configuredSource =
    options.source ?? process.env[KIMI_CODE_PLUGIN_MARKETPLACE_URL_ENV];
  ...
  try {
    ...
    const fallback =
      configuredSource === undefined
        ? await getSourceCheckoutMarketplaceLocation()
        : undefined;
    if (fallback === undefined) {
      if (options.builtInEntries !== undefined) {
        ...
        return withBuiltInEntries(
          { source: location.resolved, plugins: [] },
    ...
    const marketplace = await withLatestVersions(
      parsePluginMarketplace(raw, fallback),
      fetchImpl,
    );
  ...
  const marketplace = await withLatestVersions(
    parsePluginMarketplace(raw, location),
    fetchImpl,
  );
  return options.builtInEntries !== undefined
    ? withBuiltInEntries(marketplace, options.builtInEntries)
...
function withBuiltInEntries(
  ...
  const catalog = marketplace.plugins.filter(
    (entry) => !builtInIds.has(entry.id),
  );
...
async function getSourceCheckoutMarketplaceLocation(): Promise<
...
function parseMarketplaceEntry(
  ...
  return {
    ...
    version:
      ...
      deriveVersionFromGithubSource(resolvedSource),
    description:
...
function validateMarketplaceEntryType(
