...
function readClipboardImageViaXclip(): ClipboardImage | null {
  ...
  const preferred =
    candidates.length > 0 ? selectPreferredImageMimeType(candidates) : null;
  const tryTypes =
    preferred !== null
...
function readClipboardFilePathsViaMacOs(run: RunCommand): string[] {
  const result = run(
    "osascript",
    ["-l", "JavaScript", "-e", MACOS_FILE_PATH_SCRIPT],
    {
      timeoutMs: DEFAULT_LIST_TIMEOUT_MS,
    },
  );
...
export async function readClipboardMedia(options?: {
  ...
  if (platform === "linux") {
    ...
    if (wayland || wsl) {
      const fileMedia =
        readClipboardFileMediaViaWlPaste() ?? readClipboardFileMediaViaXclip();
