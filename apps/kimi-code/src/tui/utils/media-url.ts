...
export function mediaUrlPartToText(kind: MediaUrlKind, url: string): string {
  ...
  if (summary !== undefined) {
    const size =
      summary.bytes !== undefined ? `, ${formatByteSize(summary.bytes)}` : "";
    return `[${kind} ${summary.mime}${size}]`;
  }
  return `<${kind} url="${escapeAttribute(url)}">`;
}

export function summarizeDataUrl(
  url: string,
): { mime: string; bytes?: number } | undefined {
  if (!url.startsWith("data:")) return undefined;
  const commaIndex = url.indexOf(",");
  const header =
    commaIndex >= 0
      ? url.slice("data:".length, commaIndex)
      : url.slice("data:".length);
  const data = commaIndex >= 0 ? url.slice(commaIndex + 1) : "";
  const [rawMime, ...params] = header.split(";");
  const mime =
    rawMime !== undefined && rawMime.length > 0
      ? rawMime
