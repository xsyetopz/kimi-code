...
export class SessionPickerComponent extends Container implements Focusable {
  ...
  constructor(opts: {
    ...
    const initialIndex = this.resolveInitialSelectedIndex(
    ...
    this.visibleCount = Math.min(
      ...
      initialLoadedPages * this.pageSize,
    );
  ...
  private loadedSessions(
    sessions: readonly SessionRow[] = this.filteredSessions(),
  ): SessionRow[] {
  ...
  private syncVisibleCount(previousQuery: string): void {
    ...
    if (
      view.selectedIndex >= loadedCount - 1 &&
  ...
  override render(width: number): string[] {
    return this.renderLines(width).map((line) =>
  ...
  private renderLines(width: number): string[] {
    ...
    if (this.loading) {
      lines.push(
        currentTheme.boldFg("primary", truncateToWidth(title, width, ELLIPSIS)),
      );
      lines.push(
        currentTheme.fg(
          "textMuted",
    ...
    if (this.sessions.length === 0) {
      ...
      lines.push(
        currentTheme.fg(
          "textMuted",
          truncateToWidth(hintParts.join(" · "), width, ELLIPSIS),
        ),
      );
      lines.push("");
      lines.push(
        currentTheme.fg(
          "textMuted",
          truncateToWidth("No sessions found.", width, ELLIPSIS),
        ),
      );
      lines.push(currentTheme.fg("primary", "─".repeat(width)));
      return lines;
    ...
    if (loadedSessions.length === 0) {
      lines.push(
        currentTheme.fg(
          "textMuted",
          truncateToWidth("No matches", width, ELLIPSIS),
        ),
      );
      lines.push(currentTheme.fg("primary", "─".repeat(width)));
      return lines;
    ...
    if (
      ...
      lines.push(
        currentTheme.fg("textMuted", truncateToWidth(footer, width, ELLIPSIS)),
      );
    }
  ...
  private renderSessionCard(
    ...
    const titleStyle = (text: string) =>
      isSelected
        ? currentTheme.boldFg(titleColor, text)
        : currentTheme.fg(titleColor, text);
    ...
    const titleSource = formatSessionLabel({
      title: rawTitle,
    ...
    const shownTitle = truncateToWidth(
      singleLine(titleSource),
      titleBudget,
      ELLIPSIS,
    );

    let header = currentTheme.fg(
      ...
      card.push(
        indent +
          currentTheme.fg(
            ...
            truncateToWidth(
              fullId,
              Math.max(idWidth, width - indentWidth),
              ELLIPSIS,
            ),
          ),
      );
