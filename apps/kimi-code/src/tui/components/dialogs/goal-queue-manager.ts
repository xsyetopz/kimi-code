...
export class GoalQueueEditDialogComponent
  extends Container
    ...
    if (safeWidth < 4) {
      return [
        ...
        ...contentLines.map((line) =>
          truncateToWidth(line, safeWidth, ELLIPSIS),
        ),
      ];
    }
...
class MultilineGoalInput {
  ...
  render(width: number): string[] {
    ...
    if (range.start > 0) {
      rendered.push(
        padInputLine(
          `  ${ELLIPSIS} ${String(range.start)} previous`,
          safeWidth,
        ),
      );
    }
    ...
    if (remaining > 0) {
      rendered.push(
        padInputLine(`  ${ELLIPSIS} ${String(remaining)} more`, safeWidth),
      );
    }
  ...
  private cursorLocation(starts = lineStarts(this.value)): {
    line: number;
    column: number;
  } {
    let line = 0;
...
function nextGraphemeEnd(text: string, offset: number): number {
  ...
  const segment = SEGMENTER.segment(text.slice(offset))
    [Symbol.iterator]()
    .next().value;
