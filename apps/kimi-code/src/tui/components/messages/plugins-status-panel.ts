...
export function buildPluginsListLines(
  input: PluginsListPanelInput,
): readonly string[] {
  ...
  for (const plugin of input.plugins) {
    ...
    const diagnostics = plugin.hasErrors
      ? warning(" | diagnostics: see /plugins info")
      : "";
    ...
    const mcp =
      plugin.mcpServerCount > 0
        ...
        : "";
    lines.push(
      `  ${muted("skills:")} ${value(String(plugin.skillCount))}${muted(mcp)}${diagnostics}`,
    );
  }
  return lines;
}
...
export function buildPluginsInfoLines(
  input: PluginsInfoPanelInput,
): readonly string[] {
  const { info } = input;
  const muted = (text: string) => currentTheme.fg("textDim", text);
  const value = (text: string) => currentTheme.fg("text", text);
  ...
  if (info.source === "github" && info.github !== undefined) {
    ...
    lines.push(
      `${muted("GitHub:")} ${value(`${info.github.owner}/${info.github.repo}`)} ${muted(`@${refLabel}`)}`,
    );
    if (info.github.installedSha !== undefined) {
      lines.push(
        `${muted("Installed SHA:")} ${value(info.github.installedSha)}`,
      );
    }
  }
  if (info.originalSource !== undefined)
    lines.push(`${muted("Original source:")} ${value(info.originalSource)}`);
  ...
  if (info.manifestPath !== undefined) {
    const kindSuffix =
      info.manifestKind !== undefined
        ? ` ${muted(`(${info.manifestKind})`)}`
        : "";
    lines.push(
      `${muted("Manifest:")} ${value(info.manifestPath)}${kindSuffix}`,
    );
  }
  ...
  for (const dir of info.manifest?.skills ?? [])
    lines.push(`  ${muted("-")} ${value(dir)}`);
  ...
  if (info.mcpServers.length > 0) {
    ...
    lines.push(
      muted(
        `  Enabled by default; disable with /plugins mcp disable ${info.id} <server>.`,
      ),
    );
      ...
      if (server.transport === "stdio") {
        const args =
          server.args !== undefined && server.args.length > 0
            ...
            : "";
        lines.push(
          `    ${muted("command:")} ${value(`${server.command ?? ""}${args}`.trim())}`,
        );
        if (server.cwd !== undefined)
          lines.push(`    ${muted("cwd:")} ${value(server.cwd)}`);
        if (server.envKeys !== undefined && server.envKeys.length > 0) {
          lines.push(
            `    ${muted("env:")} ${value(server.envKeys.join(", "))}`,
          );
        }
      } else {
        ...
        if (server.headerKeys !== undefined && server.headerKeys.length > 0) {
          lines.push(
            `    ${muted("headers:")} ${value(server.headerKeys.join(", "))}`,
          );
        }
      }
    }
  ...
  if (iface !== undefined) {
    ...
    if (iface.developerName !== undefined)
      lines.push(`  ${muted("-")} ${value(`by ${iface.developerName}`)}`);
    if (iface.websiteURL !== undefined)
      lines.push(`  ${muted("-")} ${value(iface.websiteURL)}`);
  }

  if (
    info.manifest?.keywords !== undefined &&
    info.manifest.keywords.length > 0
  ) {
    lines.push("");
