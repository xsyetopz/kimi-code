...
import { visibleWidth, wrapTextWithAnsi } from "@moonshot-ai/kimi-tui";
...
export class BannerComponent implements Component {
  ...
  render(width: number): string[] {
    ...
    const tagStyled =
      tagLabel.length > 0 ? currentTheme.boldFg("primary", tagLabel) : "";
    ...
    const descIndent = showTag
      ? " ".repeat(visibleWidth(PREFIX_STAR + PADDING))
      : "";
    const bodyContentWidth = width - (showTag ? tagWidth : 0);
    const descContentWidth =
      width - (showTag ? visibleWidth(PREFIX_STAR + PADDING) : 0);
    ...
    const subSegments = this.state.subText
      ? this.state.subText.split("\n")
      : [];
    ...
    for (const sub of subSegments) {
      const available =
        descContentWidth <= 0 ? bodyContentWidth : descContentWidth;
