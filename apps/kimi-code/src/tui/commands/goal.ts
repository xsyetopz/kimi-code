...
function parseNextGoalCommand(tokens: readonly string[]): ParsedGoalCommand {
  if (tokens.length === 2 && tokens[1] === "manage")
    return { kind: "next-manage" };
  let index = 1;
...
async function queueNextGoal(
  ...
  try {
    ...
    host.showError(
      `Failed to inspect current goal: ${formatErrorMessage(error)}`,
    );
    return;
  }
...
async function showGoalQueueManager(
  ...
  try {
    ...
    host.showError(
      `Failed to load upcoming goals: ${formatErrorMessage(error)}`,
    );
    return;
  }
    ...
    new GoalQueueManagerComponent({
      ...
      onAction: async (action) => {
        try {
          ...
          host.showError(
            `Failed to update upcoming goals: ${formatErrorMessage(error)}`,
          );
          return undefined;
        }
      },
    ...
    new GoalQueueEditDialogComponent({
      ...
      onDone: (result) => {
        void handleGoalQueueEditResult(host, result).catch((error: unknown) => {
          host.showError(
            `Failed to update upcoming goal: ${formatErrorMessage(error)}`,
          );
        });
      },
    }),
...
async function setPermissionForGoal(
  ...
  try {
    ...
    host.showError(
      `Failed to set permission mode: ${formatErrorMessage(error)}`,
    );
    return false;
  }
