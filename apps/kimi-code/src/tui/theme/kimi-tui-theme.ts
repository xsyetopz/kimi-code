...
export function createMarkdownTheme(options?: {
  transient?: boolean;
}): MarkdownTheme {
  const transient = options?.transient === true;
  const stripHash = (text: string): string =>
    text.replace(HEADING_HASH_PREFIX, "$1");

  return {
    heading: (text) =>
      chalk.bold.hex(currentTheme.color("text"))(stripHash(text)),
    ...
    listBullet: (text) =>
      chalk.hex(currentTheme.color("text"))(text.replace(/^-/, "•")),
    ...
    highlightCode: (code: string, lang?: string) => {
      ...
      const language =
        normalizedLang !== undefined && supportsLanguage(normalizedLang)
          ? normalizedLang
          : "text";
      try {
