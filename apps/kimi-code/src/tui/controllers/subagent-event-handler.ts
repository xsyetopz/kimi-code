...
export class SubAgentEventHandler {
  ...
  private readonly agentSwarmProgress: Map<
    string,
    AgentSwarmProgressComponent
  > = new Map();
      ...
      toolCall.updateSubagentMetrics({
        ...
        modelDisplay:
          event.model === undefined
            ...
            : modelDisplayName(
                event.model,
                this.host.state.appState.availableModels[event.model],
              ),
      });
    }
    return true;
  ...
  private handleSubagentCompleted(
    ...
    if (backgroundMeta !== undefined) {
      ...
      if (
        taskId !== undefined &&
        this.deps.backgroundTaskTranscriptedTerminal.has(taskId)
      ) {
        return;
      }
      if (taskId !== undefined) {
  ...
  private handleSubagentFailed(
    ...
    if (backgroundMeta !== undefined) {
      ...
      const task =
        taskId === undefined
          ? undefined
          : this.deps.backgroundTasks.get(taskId);
      this.backgroundAgentMetadata.delete(event.subagentId);
      this.deps.syncBackgroundAgentBadge();
      ...
      this.appendBackgroundAgentEntry("failed", backgroundMeta, {
        error: event.error,
      });
      return;
    }
  ...
  private buildBackgroundAgentMetadata(
    ...
    const parent = this.host.streamingUI.getActiveToolCall(
  ...
  private handleForegroundSubagentFailed(
    ...
    if (
      this.updateAgentSwarmProgress(parentToolCallId, (progress) => {
        this.markAgentSwarmFailedOrCancelled(
          progress,
          event.subagentId,
          event.error,
        );
...
function isSubagentLifecycleEvent(
  event: Event,
