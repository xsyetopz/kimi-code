...
export function createInstallPromptChoices(
  target: UpdateTarget,
): readonly InstallPromptChoice[] {
  return [
...
export function getDefaultInstallPromptSelection(
  choices: readonly InstallPromptChoice[],
): number {
  const installIndex = choices.findIndex(
    (choice) => choice.value === "install",
  );
...
function renderInstallPrompt(
  ...
  const currentVersion = chalk
    .hex(UPDATE_PROMPT_WARNING)
    .bold(options.currentVersion);
  const targetVersion = chalk
    .hex(UPDATE_PROMPT_SUCCESS)
    .bold(options.target.version);
  const sourceLabel = chalk
    .hex(UPDATE_PROMPT_PRIMARY)
    .bold(options.installSource);
  const command = chalk.hex(UPDATE_PROMPT_PRIMARY)(options.installCommand);
  const changelogText = chalk
    .hex(UPDATE_PROMPT_PRIMARY)
    .underline(`View changelog: ${CHANGELOG_URL}`);
  const lines = [
    chalk.hex(UPDATE_PROMPT_PRIMARY).bold("Kimi Code Update Available"),
    chalk.hex(UPDATE_PROMPT_MUTED)(
      `${PRODUCT_NAME} has a newer release ready.`,
    ),
...
export async function promptForInstallChoice(
  ...
  return new Promise<InstallPromptChoiceValue>((resolve) => {
    ...
    const onKeypress = (
      ...
      if (key.name === "up") {
        selectedIndex = moveInstallPromptSelection(
          selectedIndex,
          "up",
          choices.length,
        );
