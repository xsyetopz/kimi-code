 ...
 */
export function classifyByPathHeuristic(
  packageRoot: string,
): InstallSource | null {
...
export function classifyInstallSource(
  ...
  const normalizedPackageRoot = normalizePathForComparison(
    packageRoot,
    platform,
  );
  ...
  for (const candidate of candidateGlobalPackageDirs(globalPrefix, platform)) {
    if (
      normalizePathForComparison(candidate, platform) === normalizedPackageRoot
    ) {
      return "npm-global";
    }
  }
...
export async function detectInstallSource(
  ...
  const resolved: DetectInstallSourceDeps = {
    ...
    getGlobalPrefix:
      ...
      (() =>
        execFileText(npmCommand(platform), ["prefix", "-g"]).then((text) =>
          text.trim(),
        )),
