    ...
    accessor: {
      get: (token: unknown) => {
        if (!services.has(token))
          throw new Error(`unexpected service request: ${String(token)}`);
...
function makeFakeHarness() {
  ...
  const profileState: { profileName: string | undefined } = {
    ...
    [
      IConfigService,
      {
        ...
        get: vi.fn((section: string) =>
          section === "defaultModel" ? "k2" : undefined,
        ),
...
describe("runV2Print", () => {
  ...
  it("seeds explicit agent files from --agentFile and binds the --agent profile", async () => {
    const stdout = writer();
    const stderr = writer();
    const { app, agent, appServices, agentServices, handlerServices } =
      makeFakeHarness();
  ...
  it("does not materialize a main agent after fresh profile binding fails", async () => {
    ...
    await expect(
      runV2Print(opts({ agent: "missing" }) as never, "1.2.3-test", {
        stdout,
        stderr,
      }),
  ...
  it("fails before any turn when --agent-file is invalid", async () => {
    ...
    await expect(
      runV2Print(opts({ agentFiles: [agentFile] }) as never, "1.2.3-test", {
        stdout,
        stderr,
      }),
  ...
  it("treats re-selecting the already-bound profile on resume as a no-op", async () => {
    const stdout = writer();
    const stderr = writer();
    const { app, agent, agentServices, appServices, profileState } =
      makeFakeHarness();
    profileState.profileName = "reviewer";

    const index = appServices.get(ISessionIndex) as {
    ...
    await runV2Print(
      opts({ session: "ses_1", agent: "reviewer" }) as never,
      "1.2.3-test",
      {
        stdout,
        stderr,
