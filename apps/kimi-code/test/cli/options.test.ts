...
describe("CLI options parsing", () => {
  ...
  describe("KIMI_MODEL_OUTPUT_FORMAT", () => {
    ...
    it("fails validation fast for an invalid env value in prompt mode", () => {
      ...
      expect(() =>
        validateOptions(opts, { [OUTPUT_FORMAT_ENV]: "json" }),
      ).toThrow(OptionConflictError);
    });
  ...
  describe("--skills-dir", () => {
    it("collects repeated skill directories", () => {
      expect(
        parse(["--skills-dir", "/one", "--skills-dir=/two"]).skillsDirs,
      ).toEqual(["/one", "/two"]);
    });
  });
  ...
  describe("--agent / --agent-file", () => {
    it("describes agent selectors as new-session-only", () => {
      ...
      expect(normalizedHelp).toContain(
        "Agent profile to start the new session with.",
      );
    ...
    it("rejects repeated --agent", () => {
      expect(() =>
        parse(["-p", "hi", "--agent", "reviewer", "--agent", "writer"]),
      ).toThrow("--agent may only be specified once.");
    });
    ...
    it("rejects combining --agent with --agent-file", () => {
      expect(() =>
        ...
        "option '--agent <name>' cannot be used with option '--agent-file <path>'",
      );
    });
    ...
    it("rejects multiple agent files passed directly to validation", () => {
      const opts = parse(["-p", "hi", "--agent-file", "a.md"]);
      expect(() =>
        validateOptions({ ...opts, agentFiles: ["a.md", "b.md"] }),
      ).toThrow("--agent-file may only be specified once.");
    });
    ...
    it("rejects mixed agent selectors passed directly to validation", () => {
      const opts = parse(["-p", "hi", "--agent", "reviewer"]);
      expect(() =>
        validateOptions({ ...opts, agentFiles: ["reviewer.md"] }),
      ).toThrow("Cannot combine --agent with --agent-file.");
    });
    ...
    it("accepts the flags in shell mode", () => {
      expect(validateOptions(parse(["--agent", "reviewer"]), {}).uiMode).toBe(
        "shell",
      );
      expect(validateOptions(parse(["--agent-file", "a.md"]), {}).uiMode).toBe(
        "shell",
      );
    });
  ...
  describe("--add-dir", () => {
    ...
    it("parses repeated additional workspace directories", () => {
      expect(parse(["--add-dir", "/one", "--add-dir=/two"]).addDirs).toEqual([
        "/one",
        "/two",
      ]);
    });
