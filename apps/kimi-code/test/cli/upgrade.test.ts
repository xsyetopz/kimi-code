...
function createDeps(
  ...
  return {
    refreshUpdateCache: vi
      .fn()
      .mockResolvedValue(
        cacheWith(overrides.latest ?? "0.5.0", overrides.manifest ?? null),
      ),
    detectInstallSource: vi
      .fn()
      .mockResolvedValue(overrides.source ?? "npm-global"),
...
describe("handleUpgrade", () => {
  it("prompts before installing the latest version when the install source supports it", async () => {
    ...
    await expect(
      handleUpgrade("0.4.0", { ...deps, ...writable }),
    ).resolves.toBe(0);
  ...
  it("returns a failing exit code when the foreground install fails", async () => {
    ...
    const deps = createDeps({
      ...
      installUpdate: vi
        .fn()
        .mockRejectedValue(new Error("npm exited with code 1")),
    });
