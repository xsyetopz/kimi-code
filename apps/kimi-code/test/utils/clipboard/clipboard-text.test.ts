...
const clipboardMock = clipboard as unknown as {
...
describe("copyTextToClipboard", () => {
  it("copies text with the native clipboard when available", async () => {
    ...
    await expect(copyTextToClipboard('cd "/tmp/proj-b"')).resolves.toBe(
      "native",
    );
  ...
  it("keeps native clipboard method context when copying text", async () => {
    clipboardMock.setText.mockImplementation(function (
      this: unknown,
      text: string,
    ): void {
  ...
  it("throws an Error when all platform clipboard commands fail", async () => {
    ...
    await expect(
      copyTextToClipboard('cd "/tmp/proj-b"'),
...
describe("buildClipboardOSC52", () => {
  it("emits a bare OSC 52 sequence outside tmux", () => {
    expect(buildClipboardOSC52("hi", false)).toBe(
      `\u001B]52;c;${base64("hi")}\u0007`,
    );
  });
