...
describe("buildPluginMarketplaceCdn", () => {
  it("packages WebBridge without publishing other unlisted official directories", async () => {
    ...
    await expect(
      access(join(outDir, "official/listed-plugin.zip")),
    ).resolves.toBeUndefined();
    await expect(
      access(join(outDir, "official/kimi-webbridge.zip")),
    ).resolves.toBeUndefined();
    await expect(
      access(join(outDir, "official/not-listed.zip")),
    ).rejects.toThrow();
    const marketplace = JSON.parse(
      await readFile(join(outDir, "marketplace.json"), "utf8"),
    );
