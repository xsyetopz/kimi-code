...
describe("buildManifestKey", () => {
  it("namespaces by target", () => {
    expect(buildManifestKey("darwin-arm64")).toBe(
      "native/darwin-arm64/manifest.json",
    );
    expect(buildManifestKey("linux-x64")).toBe(
      "native/linux-x64/manifest.json",
    );
  });
});
...
describe("isManifestVersionSupported", () => {
  it("accepts current version", () => {
    expect(isManifestVersionSupported(NATIVE_ASSET_MANIFEST_VERSION)).toBe(
      true,
    );
  });

  it("rejects other versions", () => {
    expect(isManifestVersionSupported(NATIVE_ASSET_MANIFEST_VERSION + 1)).toBe(
      false,
    );
