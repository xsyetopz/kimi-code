...
describe("collectWebAssets", () => {
  it("collects dist-web files into deterministic SEA asset keys", async () => {
    ...
    try {
      ...
      writeFileSync(
        join(appRoot, "dist-web", "index.html"),
        '<div id="app"></div>\n',
      );
      writeFileSync(
        join(appRoot, "dist-web", "assets", "app.js"),
        'console.log("ok");\n',
      );
      ...
      expect(webAssetManifestKey("test-target")).toBe(
        "web/test-target/manifest.json",
      );
      ...
      expect(assets).toEqual({
        "web/test-target/dist-web/assets/app.js": join(
          appRoot,
          "dist-web",
          "assets",
          "app.js",
        ),
        "web/test-target/dist-web/index.html": join(
          appRoot,
          "dist-web",
          "index.html",
        ),
  ...
  it("fails clearly when dist-web has not been built", async () => {
    ...
    try {
      await expect(
        collectWebAssets({ appRoot, target: "test-target" }),
      ).rejects.toThrow(/Kimi web build output was not found/);
    } finally {
      rmSync(appRoot, { recursive: true, force: true });
    }
  });
  ...
  it("keeps manifest JSON parseable and stable", async () => {
    ...
    try {
      ...
      const { manifestJson } = await collectWebAssets({
        appRoot,
        target: "test-target",
      });

      expect(
        readFileSync(join(appRoot, "dist-web", "index.html"), "utf-8"),
      ).toBe("<html></html>");
