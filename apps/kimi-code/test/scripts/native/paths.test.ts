...
describe("targetTriple", () => {
  it("returns platform-arch when env unset", () => {
    expect(targetTriple({ platform: "darwin", arch: "arm64", env: {} })).toBe(
      "darwin-arm64",
    );
    expect(targetTriple({ platform: "linux", arch: "x64", env: {} })).toBe(
      "linux-x64",
    );
    expect(targetTriple({ platform: "win32", arch: "x64", env: {} })).toBe(
      "win32-x64",
    );
  });
...
describe("path helpers", () => {
  ...
  it("returns absolute bin dir per target", () => {
    expect(nativeBinDir("darwin-arm64")).toBe(
      p("dist-native/bin/darwin-arm64"),
    );
  });
  ...
  it("returns manifest key for target", () => {
    expect(nativeManifestKey("darwin-arm64")).toBe(
      "native/darwin-arm64/manifest.json",
    );
  });
