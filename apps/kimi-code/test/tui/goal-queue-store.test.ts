...
describe("goal queue store", () => {
  ...
  it("appends a trimmed upcoming goal and writes the session file", async () => {
    const snapshot = await appendGoalQueueItem(session(), {
  ...
  it("preserves concurrent appends to the same session queue", async () => {
    await Promise.all(
      Array.from({ length: 10 }, (_, index) =>
        appendGoalQueueItem(session(), {
          objective: `Queued goal ${index + 1}`,
        }),
      ),
    );
    ...
    expect(snapshot.goals.map((goal) => goal.objective).toSorted()).toEqual(
      Array.from(
        { length: 10 },
        (_, index) => `Queued goal ${index + 1}`,
      ).toSorted(),
    );
  });

  it("updates an upcoming goal objective", async () => {
    const first = await appendGoalQueueItem(session(), {
  ...
  it("removes an upcoming goal by id", async () => {
    ...
    const second = await appendGoalQueueItem(session(), {
      objective: "Second",
    });

    const snapshot = await removeGoalQueueItem(session(), {
  ...
  it("restores a removed upcoming goal at the front without duplicating it", async () => {
    ...
    expect(restored.goals.map((goal) => goal.objective)).toEqual([
      "First",
      "Second",
    ]);

    const deduped = await restoreGoalQueueItem(session(), removed);
    expect(deduped.goals.map((goal) => goal.objective)).toEqual([
      "First",
      "Second",
    ]);
  });
  ...
  it("moves an upcoming goal up and down", async () => {
    ...
    expect(movedUp.goals.map((goal) => goal.objective)).toEqual([
      "First",
      "Third",
      "Second",
    ]);
    ...
    expect(movedDown.goals.map((goal) => goal.objective)).toEqual([
      "Third",
      "First",
      "Second",
    ]);
  ...
  it("rejects empty and over-long objectives", async () => {
    await expect(
      appendGoalQueueItem(session(), { objective: "  " }),
  ...
  it("does not clear the queue file when JSON cannot be parsed", async () => {
    ...
    await expect(readGoalQueue(session())).rejects.toThrow(
      "Invalid JSON in goal queue",
    );
    await expect(readFile(join(dir, QUEUE_FILE), "utf-8")).resolves.toBe(
      partial,
    );
  });
  ...
  it("throws when the session summary does not expose a session directory", async () => {
    await expect(
      readGoalQueue({ id: "missing", summary: undefined }),
    ).rejects.toThrow("Session missing does not expose a session directory");
  });

  it("throws a goal-not-found error when the target item is missing", async () => {
    await expect(
      removeGoalQueueItem(session(), { goalId: "missing" }),
