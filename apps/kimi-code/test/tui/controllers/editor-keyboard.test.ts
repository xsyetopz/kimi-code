...
function createHarness(
  options: { streamingPhase?: string; isCompacting?: boolean } = {},
): Harness {
  ...
  const host = {
    ...
    session,
    btwPanelController: {
      cancelRunning: btwCancelRunning,
      closeOrCancel: btwCloseOrCancel,
    },
    openUndoSelector,
...
function pressNonEscape(editor: Harness["editor"]): void {
  ...
  if (handler === undefined)
    throw new Error("onNonEscapeInput handler not installed");
  (handler as () => void)();
}
...
describe("EditorKeyboardController double-Esc undo", () => {
  ...
  it("does not trigger undo while streaming; Esc cancels the stream instead", () => {
    const { editor, host, openUndoSelector, cancelRunningShellCommand } =
      createHarness({
...
describe("EditorKeyboardController btw panel priority", () => {
  it("Esc closes the btw panel first while compacting, without cancelling compaction", () => {
    const { editor, btwCloseOrCancel, cancelCompaction } = createHarness({
      isCompacting: true,
    });
  ...
  it("Ctrl+C cancels a running btw question first while compacting", () => {
    const { editor, btwCancelRunning, cancelCompaction } = createHarness({
      isCompacting: true,
    });
  ...
  it("Ctrl+C cancels compaction when no btw panel is open", () => {
    const { editor, btwCancelRunning, btwCloseOrCancel, cancelCompaction } =
      createHarness({
        isCompacting: true,
      });
...
describe("EditorKeyboardController shell history recall", () => {
  ...
  it("installs a filter that allows shell entries only in bash mode", () => {
    ...
    const [filter] = setHistoryFilter.mock.calls[0] as [
  ...
  it("strips the leading ! and switches to bash mode when recalling a shell entry", () => {
    ...
    expect(editor["setInputMode"] as unknown as Mock).toHaveBeenCalledWith(
      "bash",
    );
  });
  ...
  it("restores the input mode from the saved draft host state", () => {
    const { editor } = createHarness();
    const restore = editor["onHistoryDraftRestore"] as unknown as (
      state: unknown,
    ) => void;
...
describe("EditorKeyboardController Shift-Tab plan toggle", () => {
  function createShiftTabHarness(
    options: { sessionless?: boolean; engineV2?: boolean } = {},
  ) {
    const editor: Record<string, ((...args: never[]) => unknown) | undefined> =
      {
        setHistoryFilter: vi.fn() as unknown as (...args: never[]) => unknown,
    ...
    const ensureSession = vi.fn(
      async (): Promise<{ id: string } | undefined> => ({ id: "ses-lazy" }),
    );
    const host = {
      state: {
        editor,
  ...
  it("toggles plan mode directly with an active session", () => {
    const { onShiftTab, handlePlanToggle, ensureSession } =
