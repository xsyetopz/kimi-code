...
describe("highlightFirstSlashToken", () => {
  ...
  it("colours next in /goal next", () => {
    const out = highlightFirstSlashToken(
      "/goal next Ship feature X",
      "primary",
    );
    expect(out).toBeDefined();
  ...
  it("returns undefined when slash is not at the leading position", () => {
    expect(
      highlightFirstSlashToken("  hello /not-cmd", "primary"),
    ).toBeUndefined();
  });

  it("returns undefined for path-like slash tokens", () => {
    expect(
      highlightFirstSlashToken("/user/desktop/ foo", "primary"),
    ).toBeUndefined();
  });
