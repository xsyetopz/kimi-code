...
function renderText(
  component: AgentSwarmProgressComponent,
  width = 100,
): string {
...
describe("AgentSwarmProgressComponent", () => {
    ...
    try {
      ...
      const titleOf = (): string => {
        const line = component
          .render(100)
          .find((l) => strip(l).includes("Agent Swarm"));
  ...
  it("keeps streamed pending items as text even when compact layout is selected", () => {
    ...
    component.updateArgs({
      items: Array.from(
        { length: 30 },
        (_item, index) => `f${String(index + 1)}.ts`,
      ),
    });
  ...
  it("renders rate-limited subagents as cancelled when cancelled", () => {
    ...
    const cellLine = renderLines(component).find((line) =>
      line.includes("001 ["),
    );
  ...
  it("strips nested AgentSwarm prefixes from failure details", () => {
    ...
    expect(output).toContain(
      "✗ [provider.rate_limit] 429 request reached user+model max RPM.",
    );
  ...
  it("uses natural status label width for prompting text", () => {
    ...
    prompting.updateArgs(
      ...
      {
        streamingArguments:
          '{"prompt_template":"Review the changed TypeScript files carefully',
      },
    );

    const promptLine = renderLines(prompting, 80).find((line) =>
      line.includes("Prompting..."),
    );
    ...
    const workingLine = renderLines(working, 80).find((line) =>
  ...
  it("renders the activity spinner before the total status line", () => {
    ...
    const statusLine = renderLines(component, 80).find((line) =>
      line.includes("Working..."),
    );
  ...
  it("renders terminal total status lines after the tool call ends", () => {
    ...
    expect(
      renderLines(completed, 80).some((line) =>
    ...
    expect(
      renderLines(failed, 80).some((line) => line.startsWith("  ✗ Failed.")),
    ).toBe(true);
  ...
  it("reserves one trailing cell for prompting streaming text", () => {
    ...
    const promptLine = renderLines(prompting, 50).find((line) =>
      line.includes("Prompting..."),
    );
  ...
  it("creates pending rows from partial streaming arguments", () => {
    ...
    component.updateArgs(
      ...
      {
        streamingArguments:
          '{"description":"Review changed files","items":["src/a.ts","src/b',
      },
    );
    const output = renderText(component);
