...
describe("buildGoalMarker", () => {
  it("builds lifecycle markers for paused / resumed / blocked", () => {
    ...
    const blocked = buildGoalMarker(
      { kind: "lifecycle", status: "blocked" } as GoalChange,
      false,
    );
  ...
  it("keeps long provider pause markers within the terminal width", () => {
    ...
    const width = 80;
    expect(strip(marker!.render(width))).toContain(
      "Goal paused after provider API error",
    );
  ...
  it("returns null for a completion change (it posts its own message)", () => {
    expect(
      buildGoalMarker(
        { kind: "completion", status: "complete" } as GoalChange,
        false,
      ),
    ).toBeNull();
  });
