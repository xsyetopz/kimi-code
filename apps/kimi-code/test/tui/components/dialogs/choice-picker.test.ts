...
describe("ChoicePickerComponent", () => {
  ...
  it("renders optional descriptions below choice labels", () => {
    const picker = new ChoicePickerComponent({
      ...
      options: [
        ...
        },
        {
          ...
          label: "Auto",
          description:
            "Automatically approve tool actions and plan transitions.",
        },
      ],
      currentValue: "manual",
      onSelect: vi.fn(),
    ...
    expect(out).toContain(
      "    Ask before commands, edits, and other risky actions.",
    );
    expect(out).toContain(
      "    Automatically approve tool actions and plan transitions.",
    );
  ...
  it("renders domain selector wrappers with their configured options", () => {
    ...
    expect(upgradePreferenceOutput).toContain(
      "    Install new versions in the background.",
    );
  });
  ...
  it("renders the selected option description in descriptionTone, others in textMuted", () => {
    const options: ChoiceOption[] = [
      {
        value: "none",
        label: "No attachment",
        description: "Text feedback only",
      },
      {
        value: "logs+codebase",
        label: "Logs + codebase",
    ...
    const renderDescLine = (currentValue: string): string | undefined => {
      ...
      return picker
        .render(120)
        .find((line) => strip(line).includes("Include your codebase"));
    };
