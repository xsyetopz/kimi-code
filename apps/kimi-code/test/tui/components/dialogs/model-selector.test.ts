...
describe("ModelSelectorComponent", () => {
  ...
  it("renders the unavailable thinking segment muted", () => {
    ...
    expect(raw).toContain(
      currentTheme.fg("textMuted", "  Off (Unsupported)  "),
    );
  });
  ...
  it('shows a "more" indicator when the list overflows a page', () => {
    ...
    for (let i = 0; i < 12; i++)
      models[`m${String(i)}`] = model(`Model ${String(i)}`);
    const picker = new ModelSelectorComponent({
      models,
      currentValue: "m0",
      currentThinkingEffort: "off",
  ...
  it("never renders a line wider than the terminal", () => {
    const picker = new ModelSelectorComponent({
      models: {
        long: model(
          "A Very Long Model Display Name That Should Be Truncated Hard",
        ),
  ...
  it("invokes onSessionOnlySelect on Alt+S with the effective thinking state", () => {
    ...
    expect(onSessionOnlySelect).toHaveBeenCalledWith({
      alias: "kimi",
      thinking: "off",
    });
  ...
  it("always-on effort models hide Off and clamp selection at the last effort", () => {
    ...
    const picker = new ModelSelectorComponent({
      models: {
        kimi: effortModel("Kimi K2", ["low", "high", "max"], "high", [
          "always_thinking",
        ]),
      },
      currentValue: "kimi",
  ...
  it("renders the warning line directly below the key-hint line when provided", () => {
    ...
    expect(lines.findIndex((l) => l.includes("Kimi K2"))).toBeGreaterThan(
      hintIdx + 1,
    );
  });
