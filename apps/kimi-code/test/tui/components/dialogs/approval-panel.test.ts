...
function makeDialog(): {
  ...
  const dialog = new ApprovalPanelComponent(makePending(), (response) =>
...
describe("ApprovalPanelComponent", () => {
  ...
  it("renders choice descriptions beneath the label when present", () => {
        ...
        choices: [
          {
            ...
            selected_label: "auto",
            description:
              "Tools are approved automatically, and questions are skipped.",
          },
          {
            label: "Do not start",
            response: "cancelled",
    ...
    const out = strip(
      new ApprovalPanelComponent(pending, () => {}).render(80).join("\n"),
    );
  ...
  it("numeric shortcuts still drive approval actions", () => {
    ...
    expect(responses).toEqual([
      { response: "approved_for_session", feedback: undefined },
    ]);
  });
  ...
  it("renders Write as a syntax-highlighted code block (file_content), not a diff", () => {
    ...
    const lines: string[] = [];
    for (let i = 1; i <= 30; i++)
      lines.push(`const x${String(i)} = ${String(i)};`);
  ...
  it("returns feedback for plan-review revise choice", () => {
    ...
    const dialog = new ApprovalPanelComponent(pending, (response) =>
      responses.push(response),
    );
