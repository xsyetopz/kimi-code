...
function warningMark(): string {
  ...
  return withAnsiColors(
    () => chalk.hex(darkColors.warning)("\u0001").split("\u0001")[0]!,
  );
}
...
describe("plugins selector dialogs", () => {
  it("trusts only built-in Kimi CDN plugin paths", () => {
    ...
    expect(
      pluginTrustLabel({
        ...
        source: "local-path",
        originalSource:
          "https://code.kimi.com/kimi-code/plugins/official/local",
      }),
    ).toBe("third-party");
  });
  ...
  it("recognizes installed plugins by official provenance", () => {
    ...
    expect(isOfficialPluginInstall({ ...base, source: "local-path" })).toBe(
      false,
    );
  ...
  it("treats only the official Kimi CDN path as a trusted install source", () => {
    ...
    expect(
      isOfficialPluginSource("https://code.kimi.com/kimi-code/plugins/foo.zip"),
    ).toBe(false);
    expect(
      isOfficialPluginSource("https://cdn.kimi.com/unrelated/plugin.zip"),
    ).toBe(false);
    // Non-Kimi hosts (loopback included), non-https schemes, local paths, and
    // GitHub sources are unofficial.
    expect(
      isOfficialPluginSource(
        "https://example.test/kimi-code/plugins/official/x.zip",
      ),
    ).toBe(false);
    expect(
      isOfficialPluginSource(
        "http://code.kimi.com/kimi-code/plugins/official/x.zip",
      ),
    ).toBe(false);
    expect(
      isOfficialPluginSource(
        "http://127.0.0.1:58627/kimi-code/plugins/official/x.zip",
      ),
    ).toBe(false);
  ...
  it("renders a same-id custom catalog row as a normal plugin, without capability state", () => {
    ...
    const capabilities = [
      makeCapability({ id: "kimi-webbridge", displayName: "Kimi WebBridge" }),
    ];
    const entries = [
      {
        id: "kimi-webbridge",
  ...
  it("shows incomplete capability setup on installed and official rows", () => {
    ...
    expect(installedOut).toContain(
      "Kimi Computer Use  enabled  setup incomplete",
    );
  ...
  it("defaults the third-party install trust prompt to exit", () => {
    ...
    expect(out.some((line) => line.includes("Kimi has not reviewed"))).toBe(
      true,
    );
