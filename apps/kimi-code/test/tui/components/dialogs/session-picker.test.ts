...
describe("SessionPickerComponent", () => {
  ...
  it("prepends [imported] badge before the title for sessions migrated from kimi-cli", () => {
    ...
    const lines = component.render(120).map((line) => stripAnsi(line));
    const importedLine = lines.find((line) =>
      line.includes("Migrated session"),
    );
      ...
      sessions: [
        {
          ...
          title:
            "现在要重构一下 TUI 的 sessions 列表，要渲染几个字段，让 UI 更好看",
          last_prompt:
            "我们要渲染几个：sessionid title lastPrompt。工作目录，修改时间。需要重新设计下 UI。",
          work_dir:
            "/Users/someone/Desktop/中文目录/very-long-project-folder-name",
          updated_at: now - 5 * 60 * 1000,
        },
      ],
      loading: false,
      ...
      sessions: [
        {
          id,
          title:
            "refactor the sessions list so the UI looks much nicer than before",
          last_prompt:
  ...
  it("renders all-sessions scope header and Ctrl+A current-cwd hint", () => {
    ...
    expect(output).toContain(
      "↑↓ navigate · Ctrl+A current cwd · Enter select · Esc cancel",
    );
  });
