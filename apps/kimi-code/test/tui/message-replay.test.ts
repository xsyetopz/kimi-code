...
describe("KimiTUI resume message replay", () => {
  ...
  it("renders persisted goal replay records as goal transcript UI", async () => {
    const driver = await replayIntoDriver([
      ...
      goalReplay(goalSnapshot({ status: "active" }), {
        kind: "lifecycle",
        status: "active",
      }),
      goalReplay(
    ...
    expect(transcript).toContain(
      "Worked 1 turn over 7m15s, using 4.2k tokens.",
    );
  });
  ...
  it("does not replay model-facing goal completion prompts as transcript messages", async () => {
    ...
    const content = driver.state.transcriptEntries
  ...
  it("does not replay system-trigger prompts such as goal continuation as user messages", async () => {
    ...
    expect(
      driver.state.transcriptEntries.filter((entry) => entry.kind === "user"),
    ).toEqual([]);
    const transcript = stripAnsi(
      driver.state.transcriptContainer.render(140).join("\n"),
    );
  ...
  it("does not replay the model-blocked lifecycle marker when the follow-up is replayed", async () => {
    ...
    expect(
      driver.state.transcriptEntries.filter((entry) => entry.kind === "goal"),
    ).toEqual([]);
    const content = driver.state.transcriptEntries
      .map((item) => item.content)
      .join("\n");
  ...
  it("groups replayed Agent calls from one assistant message using live grouping", async () => {
    ...
    const output = stripAnsi(
      (group as AgentGroupComponent).render(120).join("\n"),
    );
  ...
  it("groups replayed Read calls from one assistant message using live grouping", async () => {
    const replay: AgentReplayRecord[] = [
      ...
      message("assistant", [], {
        toolCalls: [
          toolCall("call_read_1", "Read", {
            file_path: "/tmp/proj-a/src/a.ts",
          }),
          toolCall("call_read_2", "Read", {
            file_path: "/tmp/proj-a/src/b.ts",
          }),
        ],
      }),
  ...
  it("hydrates todo and background snapshot state from resumed main agent", async () => {
    ...
    expect(driver.sessionEventHandler.backgroundTasks.has("agent-bg1")).toBe(
      true,
    );
    expect(driver.sessionEventHandler.backgroundTasks.has("bash-bg1")).toBe(
      true,
    );
    expect(
      driver.sessionEventHandler.backgroundTaskTranscriptedTerminal.has(
        "bash-bg1",
      ),
    ).toBe(true);
  });
  ...
  it("matches completed resumed background agents by agent id when task id differs", async () => {
    ...
    expect(status?.backgroundAgentStatus?.headline).toBe(
      "agent completed in background",
    );
    expect(status?.backgroundAgentStatus?.detail).toContain(
  ...
  it("keeps timed-out status when an aborted resumed background agent later fails", async () => {
    ...
    expect(applyTerminalStatus.mock.calls.map(([args]) => args.status)).toEqual(
      ["timed_out"],
    );
    expect(
      driver.sessionEventHandler.subAgentEventHandler.backgroundAgentMetadata.has(
  ...
  it("renders user-slash skill activation once without exposing injected prompt text", async () => {
    ...
    expect(
      driver.sessionEventHandler.renderedSkillActivationIds.has("act-review"),
    ).toBe(true);
  });
    ...
    const driver = await replayIntoDriver([
      ...
      message(
        ...
        [
          {
            type: "text",
            text: "Plan rejected by user. Plan mode remains active.",
          },
        ],
        {
          toolCallId: "call_exit_reject",
  ...
  it("folds oversized goal rounds even though continuation boundaries are hidden", async () => {
    ...
    for (let t = 0; t < 40; t++) {
      replay.push(
        message(
          "assistant",
          t < 5 ? [{ type: "text", text: `final text ${t}` }] : [],
          {
    ...
    expect(assistants).toHaveLength(
      9 + TRANSCRIPT_KEEP_RECENT_ASSISTANT_COMPLETED,
    );
    ...
    expect(summaryText).toContain(
      `call ${40 - TRANSCRIPT_KEEP_RECENT_STEPS} tools`,
    );
    expect(summaryText).toContain(
      `${5 - TRANSCRIPT_KEEP_RECENT_ASSISTANT_COMPLETED} messages`,
    );
