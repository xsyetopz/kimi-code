...
describe("terminal notification helpers", () => {
  ...
  it("wraps OSC 9 in a tmux DCS passthrough when running inside tmux", () => {
    ...
    expect(terminal.write).toHaveBeenCalledTimes(1);
    expect(terminal.write).toHaveBeenCalledWith(
      "Ptmux;]9;Kimi Code: Approval required\\",
    );
  });
  ...
  it("suppresses notifications while the terminal is focused", () => {
    ...
    expect(state.terminalState.notificationKeys.has("approval:req-1")).toBe(
      true,
    );
  });
  ...
  it("falls back to body when the title is empty", () => {
    expect(formatNotification({ title: "", body: "Question?" })).toBe(
      "Question?",
    );
  });
...
describe("supportsOsc9Notification", () => {
  it("detects iTerm2 / WezTerm / Ghostty / Warp via TERM_PROGRAM", () => {
    ...
    expect(supportsOsc9Notification({ TERM_PROGRAM: "WarpTerminal" })).toBe(
      true,
    );
  });
  ...
  it("returns false for terminals known not to support OSC 9", () => {
    expect(supportsOsc9Notification({ TERM_PROGRAM: "Apple_Terminal" })).toBe(
      false,
    );
...
describe("supportsTerminalProgress", () => {
  ...
  it("rejects terminals that show every OSC 9 payload as a notification", () => {
    ...
    expect(supportsTerminalProgress({ TERM_PROGRAM: "Apple_Terminal" })).toBe(
      false,
    );
    expect(supportsTerminalProgress({ TERM_PROGRAM: "WarpTerminal" })).toBe(
      false,
    );
...
describe("isInsideTmux", () => {
  it("detects tmux via the TMUX env var", () => {
    expect(isInsideTmux({ TMUX: "/private/tmp/tmux-501/default,1234,0" })).toBe(
      true,
    );
  });
