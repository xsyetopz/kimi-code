...
describe("built-in slash command registry", () => {
  ...
  it("keeps swarm mode changes and swarm tasks idle-only", () => {
    ...
    expect(resolveSlashCommandAvailability(swarm!, "Ship feature X")).toBe(
      "idle-only",
    );
  });
  ...
  it("offers add-dir list and directory argument completions", () => {
    ...
    expect(
      directoryCompletions.every(
        (value) => value.startsWith("/") && value.endsWith("/"),
      ),
    ).toBe(true);
    expect(directoryCompletions.some((value) => value.startsWith("/."))).toBe(
      false,
    );
    ...
    expect(
      homeCompletions.every(
        (value) => value.startsWith("~/") && value.endsWith("/"),
      ),
    ).toBe(true);
    expect(homeCompletions.some((value) => value.startsWith("~/."))).toBe(
      false,
    );
    expect(homeCompletions.some((value) => value.startsWith("~/sers/"))).toBe(
      false,
    );
  });
  ...
  it("registers goal with subcommand-aware availability", () => {
    ...
    expect(resolveSlashCommandAvailability(goal!, "next Ship feature Y")).toBe(
      "always",
    );
    expect(resolveSlashCommandAvailability(goal!, "next manage")).toBe(
      "always",
    );
    expect(resolveSlashCommandAvailability(goal!, "status report")).toBe(
      "idle-only",
    );
    expect(resolveSlashCommandAvailability(goal!, "pause the rollout")).toBe(
      "idle-only",
    );
    expect(resolveSlashCommandAvailability(goal!, "cancel the migration")).toBe(
      "idle-only",
    );
    ...
    expect(resolveSlashCommandAvailability(goal!, "Ship feature X")).toBe(
      "idle-only",
    );
    expect(
      resolveSlashCommandAvailability(goal!, "replace Ship feature Y"),
    ).toBe("idle-only");
  });
  ...
  it("gates secondary_model behind the secondary-model experiment, always available", () => {
    ...
    expect((command as KimiSlashCommand).experimentalFlag).toBe(
      "secondary-model",
    );
