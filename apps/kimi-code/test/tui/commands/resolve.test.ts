...
describe("resolveSlashCommandInput", () => {
  ...
  it("resolves built-in commands by name and alias", () => {
    ...
    expect(resolve("/q")).toMatchObject({
      kind: "builtin",
      name: "exit",
      args: "",
    });
    expect(resolve("/clear")).toMatchObject({
      kind: "builtin",
      name: "new",
      args: "",
    });
    expect(resolve("/fork")).toMatchObject({
      kind: "builtin",
      name: "fork",
      args: "",
    });
    ...
    expect(resolve("/init")).toMatchObject({
      kind: "builtin",
      name: "init",
      args: "",
    });
  ...
  it("resolves skill commands and blocks them while busy", () => {
    ...
    expect(
      resolve("/skill:review src/app.ts", {
        skillCommandMap,
        isStreaming: true,
      }),
    ).toEqual({
  ...
  it("resolves unprefixed built-in skill commands and blocks them while busy", () => {
    ...
    expect(
      resolve("/mcp-config", { skillCommandMap, isCompacting: true }),
    ).toEqual({
      kind: "blocked",
      commandName: "mcp-config",
      reason: "compacting",
...
describe("slash command busy helpers", () => {
  ...
  it("formats busy messages", () => {
    expect(
      slashCommandBusyReason({ isStreaming: true, isCompacting: false }),
    ).toBe("streaming");
    expect(
      slashCommandBusyReason({ isStreaming: false, isCompacting: true }),
    ).toBe("compacting");
    expect(slashBusyMessage("new", "streaming")).toContain(
      "Cannot /new while streaming",
    );
    expect(slashBusyMessage("new", "compacting")).toContain(
      "Cannot /new while compacting",
    );
  });

  it("resolves a namespaced plugin command to a plugin-command intent", () => {
    const pluginCommandMap = new Map([
      ["my-plugin:deploy", "Deploy $ARGUMENTS"],
    ]);
  ...
  it("resolves a nested plugin command whose name contains a slash", () => {
    const pluginCommandMap = new Map([
      ["my-plugin:frontend/component", "body"],
    ]);
    expect(
      resolve("/my-plugin:frontend/component spin", { pluginCommandMap }),
    ).toEqual({
      kind: "plugin-command",
  ...
  it("blocks a plugin command while streaming", () => {
    ...
    expect(
      resolve("/my-plugin:deploy", { pluginCommandMap, isStreaming: true }),
    ).toEqual({
      kind: "blocked",
