...
function makeHost(
  ...
  const session = {
    ...
    getGoal: vi.fn(
      async (): Promise<{ goal: ReturnType<typeof fakeSnapshot> | null }> => ({
        goal: null,
      }),
    ),
...
describe("parseGoalCommand", () => {
  ...
  it("treats text after -- as the objective", () => {
    expect(
      parseGoalCommand("-- --leading-option is part of the goal"),
    ).toMatchObject({
      kind: "create",
      objective: "--leading-option is part of the goal",
    });
    expect(parseGoalCommand("-- cancel")).toMatchObject({
      kind: "create",
      objective: "cancel",
    });
  });
...
describe("handleGoalCommand", () => {
  ...
  it("asks before starting a goal in Manual mode", async () => {
    const { host: manualHost, session: s } = makeHost({
  ...
  it("defaults to Auto when confirming a Manual-mode goal start", async () => {
    ...
    expect(manualHost.setAppState).toHaveBeenCalledWith({
      permissionMode: "auto",
    });
    expect(manualHost.sendNormalUserInput).toHaveBeenCalledWith(
      "Ship feature X",
    );
  });
  ...
  it("restores the previous permission mode when the goal fails to start", async () => {
    ...
    expect(s.setPermission).toHaveBeenCalledWith("yolo");
    expect(manualHost.setAppState).toHaveBeenLastCalledWith({
      permissionMode: "manual",
    });
  });
  ...
  it("returns the command to the input box when a Manual-mode goal start is cancelled", async () => {
    ...
    expect(manualHost.restoreInputText).toHaveBeenCalledWith(
      "/goal Ship feature X",
    );
  ...
  it("defaults to Auto when confirming a YOLO-mode goal start", async () => {
    ...
    expect(s.setPermission).toHaveBeenCalledWith("auto");
    expect(yoloHost.setAppState).toHaveBeenCalledWith({
      permissionMode: "auto",
    });
  ...
  it("returns the command to the input box when a YOLO-mode goal start is cancelled", async () => {
    ...
    expect(yoloHost.restoreInputText).toHaveBeenCalledWith(
      "/goal replace Ship feature Y",
    );
    expect(s.createGoal).not.toHaveBeenCalled();
  });
  ...
  it("does not pass budget limits (flags were removed)", async () => {
    await handleGoalCommand(host, "Ship feature X");
    const arg = (session.createGoal as ReturnType<typeof vi.fn>).mock
  ...
  it("/goal next queues an upcoming goal and does not send it to the agent", async () => {
    ...
    const addChild = host.state.transcriptContainer.addChild as ReturnType<
      typeof vi.fn
    >;
    const message = addChild.mock.calls[0]?.[0] as {
      render(width: number): string[];
    };
  ...
  it("surfaces duplicate-goal errors with replace guidance", async () => {
    ...
    expect(host.showError).toHaveBeenCalledWith(
      expect.stringContaining("/goal replace"),
    );
    expect(host.sendNormalUserInput).not.toHaveBeenCalled();
  });
