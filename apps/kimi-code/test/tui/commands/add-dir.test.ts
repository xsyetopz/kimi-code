...
function makeHost(additionalDirs: readonly string[] = []) {
  ...
  const session = {
    ...
    addAdditionalDir: vi.fn(
      async (path: string, options: { persist: boolean }) => ({
...
describe("handleAddDirCommand", () => {
  ...
  it("renders the add-dir confirmation without option descriptions", async () => {
    ...
    expect(rendered).not.toContain(
      "Use this directory in the current session only",
    );
    expect(rendered).not.toContain(
      "Save this directory to the project workspace config",
    );
  ...
  it("adds a workspace dir for this session only after confirmation", async () => {
    ...
    await vi.waitFor(() => {
      expect(session.addAdditionalDir).toHaveBeenCalledWith("../shared", {
        persist: false,
      });
    });
  ...
  it("does not add a workspace dir when the confirmation is cancelled", async () => {
    ...
    expect(host.showStatus).toHaveBeenCalledWith(
      "Did not add ../shared as a working directory.",
    );
  });
