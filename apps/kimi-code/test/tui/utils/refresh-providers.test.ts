...
function makeRefreshHost(initial: KimiConfig): {
  current: () => KimiConfig;
  removeProvider: ReturnType<
    typeof vi.fn<(providerId: string) => Promise<KimiConfig>>
  >;
  setConfig: ReturnType<
    typeof vi.fn<(patch: Partial<KimiConfig>) => Promise<KimiConfig>>
  >;
} {
...
describe("refreshAllProviderModels", () => {
  ...
  it("refreshes managed Kimi Code against environment endpoints over persisted config", async () => {
    ...
    const configuredOauthKey = resolveKimiCodeOAuthKey({
  ...
  it("can refresh only the managed OAuth provider without fetching third-party registries", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledTimes(1);
    expect(
      host.current().models?.["kimi-code/kimi-for-coding"]?.displayName,
    ).toBe("Fresh Kimi");
  ...
  it("adds custom-registry providers that appear under an existing source URL", async () => {
    ...
    expect(host.setConfig).toHaveBeenCalledTimes(1);
    expect(Object.keys(host.current().providers).toSorted()).toEqual([
      "a",
      "b",
    ]);
  ...
  it("forces default thinking on when the refreshed default model cannot disable thinking", async () => {
    ...
    expect(result.failed).toEqual([]);
    expect(
      host.current().models?.["kimi-code/kimi-deep-coder"]?.capabilities,
    ).toEqual(["thinking", "always_thinking", "tool_use"]);
  ...
  it("refreshes a hand-configured API-key provider pointing at the managed endpoint", async () => {
    ...
    expect(
      host.current().models?.["my-kimi/kimi-for-coding"]?.displayName,
    ).toBe("Fresh Kimi");
    expect(
      host.current().models?.["my-kimi/kimi-for-coding"]?.capabilities,
  ...
  it("matches the managed endpoint even with a trailing slash on the configured baseUrl", async () => {
    vi.stubEnv(
      "KIMI_CODE_BASE_URL",
      "https://api.managed.example.test/coding/v1",
    );
    const host = makeRefreshHost({
      providers: {
        "my-kimi": {
    ...
    const fetchMock = vi.fn<FetchMock>(async (input) => {
      expect(fetchInputUrl(input)).toBe(
        "https://api.managed.example.test/coding/v1/models",
      );
      return new Response(
        JSON.stringify({
