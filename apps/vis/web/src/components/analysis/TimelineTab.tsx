          ...
          <tbody>
            {analysis.toolStats.map((t) => (
              <tr key={t.name} className="border-b border-border/50">
                ...
                <Td>{t.count}</Td>
                <Td
                  tone={t.errorCount > 0 ? "var(--color-sev-error)" : undefined}
                >
                  {t.errorCount}
                </Td>
                <Td
    ...
    <section className="mt-6">
      ...
      <div className="mt-2 flex flex-col gap-1">
        {gaps.map((g, i) => (
          ...
          >
            <Pill
              tone={g.kind === "between_turns" ? "meta" : "warning"}
              variant="outline"
            >
            ...
            <span className="ml-auto text-fg-3 tabular">
              line {g.afterLineNo} → {g.beforeLineNo}
            </span>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="border border-border bg-surface-0">
      ...
      >
        ...
        <Pill
          tone={turn.trigger === "steer" ? "turn" : "conversation"}
          variant="outline"
        >
          turn {turn.index}
          {turn.trigger === "steer" ? " (steer)" : ""}
        </Pill>
        ...
        {turn.toolErrorCount > 0 ? (
          <Pill tone="error">{turn.toolErrorCount} err</Pill>
        ) : null}
        <span
        ...
        <span className="flex shrink-0 items-center gap-3 font-mono text-[11px] text-fg-3 tabular">
          ...
          <span title="total tokens processed this turn">
            {formatTokens(totalTokens)} tok
          </span>
          <span title="active execution time">
...
function StepRow({
  ...
  const widthPct =
    turnDurationMs && step.durationMs
      ? Math.max(2, (step.durationMs / turnDurationMs) * 100)
      : 0;
  return (
    <div
      className="border-l-2 pl-2"
      style={{
        borderColor: step.isError
          ? "var(--color-sev-error)"
          : "var(--color-border)",
      }}
    >
      <div className="flex flex-wrap items-center gap-2 font-mono text-[11px]">
        ...
        {step.finishReason ? (
          <span
            className={
              step.isError ? "text-[var(--color-sev-error)]" : "text-fg-3"
            }
          >
            {step.finishReason}
          </span>
        ) : null}
        <span className="text-fg-3 tabular" title="step wall-clock duration">
        ...
        {step.llmFirstTokenLatencyMs !== undefined ? (
          <span
            ...
            title={
              step.llmServerFirstTokenMs !== undefined &&
              step.llmRequestBuildMs !== undefined
        ...
        {step.llmServerDecodeMs !== undefined &&
        step.llmClientConsumeMs !== undefined ? (
          <span
            className="text-fg-3 tabular"
        ...
        {step.contextTokens !== undefined ? (
          <span
            className="text-fg-3 tabular"
            title="context-window fill after step"
          >
            ctx {formatTokens(step.contextTokens)}
          </span>
        ) : null}
        {step.content.thinkChars > 0 ? (
          <span
            className="text-[var(--color-cat-meta)]"
            title="reasoning chars"
          >
...
function ToolRow({
  ...
  const widthPct =
    stepDurationMs && tc.durationMs
      ? Math.max(2, (tc.durationMs / stepDurationMs) * 100)
      : 0;
  return (
    <div className="flex flex-wrap items-center gap-2 pl-3 font-mono text-[11px]">
      ...
      <span className="text-fg-3 tabular" title="call → result elapsed">
        {formatDuration(tc.durationMs)}
      </span>
      {tc.outputBytes !== undefined ? (
        <span className="text-fg-3 tabular" title="result output size">
