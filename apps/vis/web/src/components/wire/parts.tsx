...
export function Mono({
  ...
  return (
    <span className={`font-mono text-[12px] text-fg-0 ${className}`}>
      {children}
    </span>
  );
}
...
export function Dim({
  ...
  return (
    <span className={`font-mono text-[11px] text-fg-3 ${className}`}>
      {children}
    </span>
  );
}
...
export function FieldRow({
  ...
  return (
    <>
      <span className="font-mono text-[11px] text-fg-2 text-right">
        {label}
      </span>
      <div className="font-mono text-[12px] text-fg-0 min-w-0 break-words">
        {children}
      </div>
    </>
  );
  ...
  return (
    <div className="space-y-2">
      <div className="grid grid-cols-[140px_1fr] gap-x-3 gap-y-[2px]">
        <FieldRow label="role">
          <span className="text-[var(--color-cat-ephemeral)]">
            "{message.role}"
          </span>
        </FieldRow>
        {message.toolCallId ? (
      ...
      {message.content.length > 0 ? (
        <div>
          <div className="mb-1 text-fg-2">
            content ({message.content.length} part
            {message.content.length === 1 ? "" : "s"})
          </div>
    ...
    case "tool.call": {
      ...
      return (
        <div className="space-y-2">
          <div className="grid grid-cols-[140px_1fr] gap-x-3 gap-y-[2px]">
            <FieldRow label="name">
              <Mono className="text-[var(--color-cat-tools)]">
                {event.name}
              </Mono>
            </FieldRow>
            <FieldRow label="toolCallId">
          ...
          <div className="grid grid-cols-[140px_1fr] gap-x-3 gap-y-[2px]">
            ...
            {event.llmFirstTokenLatencyMs !== undefined ? (
              <FieldRow label="firstToken">
                <span className="text-fg-1">
                  {event.llmFirstTokenLatencyMs} ms
                </span>
              </FieldRow>
            ) : null}
            {event.llmServerFirstTokenMs !== undefined ? (
              <FieldRow label="firstToken/api">
                <span className="text-fg-1">
                  {event.llmServerFirstTokenMs} ms
                </span>
              </FieldRow>
            ) : null}
