...
export function SessionDetailPage() {
  ...
  const subagentCount = session.agents.filter(
    (a) => a.agentId !== "main",
  ).length;
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      <div className="shrink-0 border-b border-border bg-surface-1 px-4 py-3">
        <div className="flex items-center gap-3">
          <span className="font-mono text-[14px] text-fg-0">
            {session.sessionId}
          </span>
        ...
        {session.imported && session.importMeta ? (
          <div className="mt-1 flex flex-wrap items-center gap-3 font-mono text-[10.5px] text-fg-3">
            {session.importMeta.manifest?.kimiCodeVersion ? (
              <span>
                kimi-code v{session.importMeta.manifest.kimiCodeVersion}
              </span>
            ) : null}
            ...
            {session.importMeta.manifest?.exportedAt ? (
              <span>
                · exported{" "}
                {formatRelativeTime(
                  Date.parse(session.importMeta.manifest.exportedAt),
                )}
              </span>
            ) : null}
        ...
        {state?.lastPrompt ? (
          <div
            className="mt-1 truncate font-mono text-[11px] text-fg-3"
      ...
      <TabBar
        ...
        tabs={[
          ...
          { id: "agents", label: "Agents", count: subagentCount },
          {
            id: "tasks",
            label: "Tasks",
            count: tasksData?.tasks.length ?? null,
          },
  ...
  return (
    <button
      ...
      title={
        state === "err" && errMsg
          ? errMsg
          : "reveal session folder in OS file manager"
      }
    >
      {state === "opening"
        ? "opening…"
