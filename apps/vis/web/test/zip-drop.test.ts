...
describe("isZipFile", () => {
  it("accepts a .zip extension regardless of declared type", () => {
    ...
    expect(
      isZipFile({ name: "SESSION.ZIP", type: "application/octet-stream" }),
    ).toBe(true);
  });
  ...
  it("accepts zip MIME types even without a .zip name", () => {
    ...
    expect(
      isZipFile({ name: "bundle", type: "application/x-zip-compressed" }),
    ).toBe(true);
  });
  ...
  it("rejects non-zip files", () => {
    ...
    expect(
      isZipFile({ name: "archive.tar.gz", type: "application/gzip" }),
    ).toBe(false);
  });
});
