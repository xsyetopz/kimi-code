...
describe("context route", () => {
  ...
  it("?history=full returns the pre-compaction messages (full reconstructed history)", async () => {
    ...
    const fullRes = await app.request(
      "/session_fixture/context?agent=main&history=full",
    );
    ...
    expect(fullBody.messages[0]!.message.content[0]).toMatchObject({
      text: "before compaction",
    });
    expect(fullBody.messages[1]!.message.content[0]).toMatchObject({
      text: "assistant reply",
    });
    expect(fullBody.messages[3]!.message.content[0]).toMatchObject({
      text: "after compaction",
    });
  });
});
