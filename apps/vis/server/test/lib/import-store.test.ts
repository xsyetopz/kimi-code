...
function buildZip(entries: Record<string, string>): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    zip.end();
    const chunks: Buffer[] = [];
    (zip.outputStream as NodeJS.ReadableStream).on("data", (c: Buffer) =>
      chunks.push(c),
    );
...
describe("import-store", () => {
  ...
  it("imports a valid bundle and lists it with manifest metadata", async () => {
    ...
    expect(
      (await stat(join(dir, "agents", "main", "wire.jsonl"))).isFile(),
    ).toBe(true);
    expect((await stat(join(dir, "logs", "kimi-code.log"))).isFile()).toBe(
      true,
    );
  ...
  it("rejects a zip with no main wire and cleans up", async () => {
    ...
    await expect(importSessionZip(home, zip, null, new Date())).rejects.toThrow(
      /session bundle/,
    );
...
describe("resolveSafeTarget (zip-slip guard)", () => {
  ...
  it("accepts in-tree paths", () => {
    expect(resolveSafeTarget(root, "state.json")).toBe(
      "/tmp/imp/abc/state.json",
    );
    expect(resolveSafeTarget(root, "agents/main/wire.jsonl")).toBe(
      "/tmp/imp/abc/agents/main/wire.jsonl",
    );
  });
