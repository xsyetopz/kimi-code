...
describe("wire-reader", () => {
  ...
  it("accepts v1.0 wire and migrates nested tool calls to flat shape", async () => {
    ...
    try {
      ...
      const dataMsg = (entry.data as { message: { toolCalls: unknown[] } })
        .message;
      ...
      const rawMsg = (
        entry.raw as {
          message: { toolCalls: Array<{ function: unknown; name?: unknown }> };
        }
      ).message;
      expect(rawMsg.toolCalls[0]).toHaveProperty("function");
      expect(rawMsg.toolCalls[0]!.function).toEqual({
        name: "Read",
        arguments: '{"path":"/x"}',
      });
  ...
  it("best-effort parses unknown protocol versions with a warning", async () => {
    ...
    expect(
      result.warnings.some((w) =>
        /unrecognised protocol_version.*0\.9/i.test(w),
      ),
    ).toBe(true);
  });
