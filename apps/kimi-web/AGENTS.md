...
The browser web UI for Kimi Code — a peer to the TUI in `apps/kimi-code`. It talks to the local server over REST + WebSocket under `/api/v1`. Stack: React 19 shell + Vue 3 legacy island + Vite 6 + TypeScript (strict) + vue-i18n v11. The React shell owns `#app`; Vue remains isolated under `src/react/LegacyVueIsland.tsx` while visible features migrate. (Tailwind was removed; all styling is via design tokens in `src/style.css` + scoped component styles.) There is no client router and no Pinia; legacy state lives in composables/refs and provide/inject.
...
- **Use the tokens, not ad-hoc values.** Colors, fonts, radii, spacing, shadows, z-index, and motion come from the CSS custom properties in `src/style.css`. Canonical names are `--color-*` / `--radius-*` / `--space-*` / `--text-*` / `--font-*` / `--z-*` / `--shadow-*` / `--ease-*` / `--duration-*` / `--weight-*` / `--leading-*`. A small set of layout/focus tokens keep the `--p-` prefix: `--p-focus-ring`, `--p-selection`, `--p-ic-sm/md/lg`, `--p-sidebar-w`, `--p-content-max/-wide`, `--p-bp-sm/-md`.
...
- **Run `bun run --cwd apps/kimi-web check:style`** (`scripts/check-style.mjs`) — it enforces the §06 anti-pattern rules (no-gradient, no-glassmorphism except TopBar `frost`, no-emoji-icon except moon, no-hardcoded-hex/font, radius/z/weight from scale). Do not add new violations.
- **Verify visually.** For any UI change, render it in the browser (light + dark, plus hover/focus states) and confirm it matches existing product patterns and introduces no regression before considering it done. Build/typecheck/check-style are necessary but not sufficient.

## Layout (`src/`)

- `src/react/main.tsx` — React bootstrap (installs error capture, owns `#app`). `src/react/LegacyVueIsland.tsx` mounts the current `App.vue` tree as one isolated Vue island; `App.vue` still holds most legacy app state.
...
All via `bun run --cwd apps/kimi-web …` (the checkout is Bun-managed):

- `dev` — Vite dev server (port `WEB_PORT`, default 5175; proxies `/api/v1` to `KIMI_SERVER_URL`, default `http://127.0.0.1:58627`).
- `build` — production build into `dist/`.
- `typecheck` — `vue-tsc --noEmit` (covers both legacy `.vue` and React `.tsx`).
...
Debugging against kap-server: start it from the repository root with `bun run dev:server` (port 58627). The Vite dev server proxies `/api/v1` to `KIMI_SERVER_URL` when set, otherwise `http://127.0.0.1:58627`.
