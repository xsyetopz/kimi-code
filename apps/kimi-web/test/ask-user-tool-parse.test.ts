...
describe("parseAskInput", () => {
  ...
  it("defaults missing optional fields and tolerates malformed input", () => {
    ...
    const qs = parseAskInput(
      JSON.stringify({ questions: [{ options: [{ label: "A" }, null] }] }),
    );
...
describe("parseAskOutput", () => {
  it("recognizes an answer payload and reads answers (question-text keys, label values)", () => {
    const out = parseAskOutput([
      JSON.stringify({
        answers: { "Which auth provider?": "Auth0" },
        note: "",
      }),
    ]);
  ...
  it("recognizes a legacy answer payload (q_<i> keys, opt ids)", () => {
    const out = parseAskOutput([
      JSON.stringify({ answers: { q_0: "opt_0_1" }, note: "" }),
    ]);
    expect(out.recognized).toBe(true);
  ...
  it("keeps string and true values, drops others", () => {
    const out = parseAskOutput([
      JSON.stringify({ answers: { a: "x", b: true, c: 3, d: null } }),
    ]);
    expect(out.recognized).toBe(true);
  ...
  it("does not recognize plain-text error output", () => {
    expect(
      parseAskOutput([
        "Interactive questions are not supported in this session.",
      ]).recognized,
    ).toBe(false);
  });

  it("does not recognize JSON that is not the answer payload", () => {
    expect(parseAskOutput([JSON.stringify({ foo: "bar" })]).recognized).toBe(
      false,
    );
    expect(
      parseAskOutput([JSON.stringify({ answers: "nope" })]).recognized,
    ).toBe(false);
    expect(parseAskOutput([JSON.stringify(["x"])]).recognized).toBe(false);
  });

  it("tolerates missing output", () => {
    expect(parseAskOutput(undefined)).toEqual({
      recognized: false,
      answers: {},
      note: "",
    });
    expect(parseAskOutput([])).toEqual({
      recognized: false,
      answers: {},
      note: "",
    });
...
describe("answerFor", () => {
  ...
  it("passes through the literal true (indeterminate answer)", () => {
    expect(
      answerFor({ "Which auth provider?": true }, "Which auth provider?", 0),
    ).toBe(true);
  });
});
