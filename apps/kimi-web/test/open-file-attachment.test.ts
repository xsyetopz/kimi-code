...
describe("openFileAttachment", () => {
  ...
  beforeEach(() => {
    ...
    mocks.getFileBlob.mockResolvedValue(
      new Blob(["<h1>x</h1>"], { type: "text/html" }),
    );
  });
  ...
  it("refuses non-text xml types outright", async () => {
    expect(await openFileAttachment("f_1", "a.xml", "application/xml")).toBe(
      "unsupported",
    );
    expect(
      await openFileAttachment("f_1", "a.xhtml", "application/xhtml+xml"),
    ).toBe("unsupported");
  });
  ...
  it("previews pdf / safe images / media with their whitelisted MIME", async () => {
    expect(await openFileAttachment("f_1", "a.pdf", "application/pdf")).toBe(
      "previewed",
    );
    expect(previewedBlobType()).toBe("application/pdf");
    expect(await openFileAttachment("f_1", "a.png", "image/png")).toBe(
      "previewed",
    );
    expect(previewedBlobType()).toBe("image/png");
    expect(await openFileAttachment("f_1", "a.mp4", "video/mp4")).toBe(
      "previewed",
    );
