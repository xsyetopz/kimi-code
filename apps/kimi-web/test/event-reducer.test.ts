...
describe("reduceAppEvent messageCreated", () => {
  it("bumps the session updatedAt so it floats to the top of the sidebar", () => {
    ...
    const next = reduceAppEvent(
      ...
      {
        type: "messageCreated",
        message: makeMessage("s-old", "2026-06-01T12:00:00.000Z"),
      },
  ...
  it("does not move a session backwards when an older message arrives", () => {
    ...
    const next = reduceAppEvent(
      ...
      {
        type: "messageCreated",
        message: makeMessage("s-new", "2026-01-01T00:00:00.000Z"),
      },
  ...
  it("leaves other sessions untouched", () => {
    ...
    const next = reduceAppEvent(
      ...
      {
        type: "messageCreated",
        message: makeMessage("s-a", "2026-06-01T12:00:00.000Z"),
      },
      { sessionId: "s-a", seq: 1 },
    );
    expect(next.sessions.find((s) => s.id === "s-a")?.updatedAt).toBe(
      "2026-06-01T12:00:00.000Z",
    );
    expect(next.sessions.find((s) => s.id === "s-b")?.updatedAt).toBe(
  ...
  it("reconciles a resolved video echo into the optimistic user message", () => {
    ...
    const echo: AppMessage = {
      ...
      content: [
        ...
        {
          type: "text",
          text: '<video path="/Users/me/.kimi-code/cache/f_abc.mp4"></video>',
        },
      ],
      createdAt: "2026-06-01T12:00:00.000Z",
      promptId: "p1",
...
describe("reduceAppEvent taskProgress", () => {
  ...
  it("preserves accumulated text across a taskCreated replacement", () => {
    const state = {
      ...createInitialState(),
      tasksBySession: {
        s1: [{ ...makeSubagentTask("t1", "s1"), text: "partial" }],
      },
    };
    const next = reduceAppEvent(
      state,
  ...
  it("keeps the roster-seeded description when a re-projected task carries the placeholder", () => {
    ...
    expect(next.tasksBySession["s1"]?.[0]?.description).toBe(
...
describe("reduceAppEvent sessions reference stability", () => {
  ...
  it("produces a new sessions array for an event that changes sessions", () => {
    ...
    const next = reduceAppEvent(
      ...
      {
        type: "sessionCreated",
        session: makeSession("s2", "2026-02-01T00:00:00.000Z"),
      },
      { sessionId: "s2", seq: 3 },
    );
...
describe("reduceAppEvent unknown agent error", () => {
  ...
  it("surfaces a rate-limit failure as a structured notice with full diagnostics", () => {
    ...
    expect(byLabel.get(i18n.global.t("warnings.details.code"))).toBe(
      "provider.rate_limit",
    );
    expect(byLabel.get(i18n.global.t("warnings.details.status"))).toBe("429");
    expect(byLabel.get(i18n.global.t("warnings.details.requestId"))).toBe(
      "req_1",
    );
    expect(byLabel.get(i18n.global.t("warnings.details.errorName"))).toBe(
      "RateLimitError",
    );
  });
  ...
  it("shows a connection failure without status/requestId rows", () => {
    ...
    expect(notice.details?.map((d) => d.value)).toEqual([
  ...
  it("still renders agent warnings as plain strings", () => {
    ...
    expect(next.warnings[0]).toBe(
      `${i18n.global.t("warnings.noteLabel")}: heads up`,
    );
  });
});
