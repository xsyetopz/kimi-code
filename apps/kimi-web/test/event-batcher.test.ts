...
describe("coalesceAppRenderEvents (lossless stream grouping)", () => {
  it("reduces 10,000 contiguous deltas in capped groups while preserving every character", () => {
    ...
    expect(
      groupLengths.every(
        (length) => length <= REASONABLE_MAX_STREAM_GROUP_CHARS,
      ),
    ).toBe(true);
    ...
    expect(
      processed.every(
        ({ appEvent }) =>
          appEvent.type === "assistantDelta" &&
          (appEvent.delta.text?.length ?? 0) <=
            REASONABLE_MAX_STREAM_GROUP_CHARS,
      ),
    ).toBe(true);
    let expectedOffset = 0;
...
describe("useKimiWebClient (resync integration)", () => {
  it("flushes queued deltas around an authoritative snapshot before live streaming resumes", async () => {
    ...
    const authoritativeSnapshotResponse = new Promise<AppSessionSnapshot>(
      (resolve) => {
    ...
    try {
      const { useKimiWebClient } = await import(
        "../src/composables/useKimiWebClient"
      );
