...
describe("rendersToolCard", () => {
  it("hides the card only for a successful tool that carries inline media", () => {
    ...
    expect(
      rendersToolCard(
        toolBlock("e", { status: "error", media: { kind: "image", url: "x" } }),
      ),
    ).toBe(true);
  });
});
...
describe("assistantRenderBlocks", () => {
  it("groups consecutive renderable tools into one tool-stack", () => {
    const rendered = assistantRenderBlocks(
      assistantTurn([toolBlock("a"), toolBlock("b")]),
    );
  ...
  it("renders a lone tool as a standalone tool, not a stack", () => {
    ...
    expect(rendered).toEqual([
      { kind: "tool", tool: tool("a"), sourceIndex: 0 },
    ]);
  });
...
describe("renderBlockKey", () => {
  it("derives stable keys per block kind", () => {
    expect(renderBlockKey({ kind: "text", text: "x", sourceIndex: 2 }, 0)).toBe(
      "text-2",
    );
    expect(
      renderBlockKey({ kind: "tool", tool: tool("a"), sourceIndex: 3 }, 0),
    ).toBe("a");
    expect(
      renderBlockKey(
        { kind: "tool-stack", tools: [{ tool: tool("a"), sourceIndex: 5 }] },
        0,
      ),
    ).toBe("tool-stack-5");
  });
