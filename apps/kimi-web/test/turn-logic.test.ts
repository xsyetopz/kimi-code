...
describe("messagesToTurns", () => {
  ...
  it("renders a completed multi-member swarm inline as a tool card", () => {
    const turns = messagesToTurns(
      [
        ...
        message("t1", "tool", [
          { type: "toolResult", toolCallId: "swarm-2", output: "all done" },
        ]),
      ],
      [],
      undefined,
        ...
        message("a1", "assistant", [
          {
            ...
            input: {
              ...
              prompt: "list the top-level dirs",
            },
          },
        ]),
        message("t1", "tool", [
          { type: "toolResult", toolCallId: "agent-call-1", output: "done" },
        ]),
      ],
      [],
      undefined,
  ...
  it("drops a caption-only text part and strips captions merged into prose", () => {
    ...
    const merged = messagesToTurns(
      [
        message("u2", "user", [
          { type: "text", text: `before ${caption} after` },
        ]),
      ],
      [],
      undefined,
  ...
  it("preserves a literal `<system>` block the user typed themselves", () => {
    ...
    const turns = messagesToTurns(
      [
        message("u1", "user", [
          {
            type: "text",
            text: "hi <system>some example markup</system> there",
          },
        ]),
      ],
      [],
    ...
    expect(turns[0]?.text).toBe(
      "hi <system>some example markup</system> there",
    );
  });
  ...
  it("leaves ordinary user text and stray angle brackets untouched", () => {
    ...
    expect(turns[0]).toMatchObject({
      role: "user",
      text: "a < b and c > d, no system tag here",
    });
  });
});
...
describe("messagesToTurns resync dedup", () => {
  ...
  it("keeps the seeded message when the transcript has no copy of the current step yet", () => {
    const turns = messagesToTurns(
      [
        message("a1", "assistant", [{ type: "text", text: "step one done" }], {
          promptId: "p1",
        }),
        message(
          "seed",
          "assistant",
          [{ type: "text", text: "step two streami" }],
          {
            promptId: "p1",
          },
        ),
  ...
  it("drops a seed whose finished parallel tools left running_tools (subset of the persisted message)", () => {
    const turns = messagesToTurns(
      [
        ...
        message("t1", "tool", [
          { type: "toolResult", toolCallId: "tool-a", output: "a done" },
        ]),
