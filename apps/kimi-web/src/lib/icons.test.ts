...
describe("ICONS registry", () => {
  ...
  it("every entry has a component and a non-empty raw svg", () => {
    for (const [name, entry] of Object.entries(ICONS)) {
      ...
      expect(entry.svg.toLowerCase(), `${name} svg contains <svg`).toContain(
        "<svg",
      );
    }
  });
...
describe("iconSvg", () => {
  ...
  it("maps size tokens to pixel width/height", () => {
    expect(iconSvg("plus", "sm")).toContain(
      `width="${SIZE_PX.sm}" height="${SIZE_PX.sm}"`,
    );
    expect(iconSvg("plus", "md")).toContain(
      `width="${SIZE_PX.md}" height="${SIZE_PX.md}"`,
    );
    expect(iconSvg("plus", "lg")).toContain(
      `width="${SIZE_PX.lg}" height="${SIZE_PX.lg}"`,
    );
  });
