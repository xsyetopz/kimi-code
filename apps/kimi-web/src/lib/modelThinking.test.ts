...
describe("modelThinking", () => {
  describe("modelThinkingAvailability", () => {
    ...
    it("detects always_thinking capability", () => {
      expect(
        modelThinkingAvailability(model({ capabilities: ["always_thinking"] })),
      ).toBe("always-on");
    });

    it("detects thinking capability", () => {
      expect(
        modelThinkingAvailability(model({ capabilities: ["thinking"] })),
      ).toBe("toggle");
    });

    it("detects adaptive thinking", () => {
      expect(modelThinkingAvailability(model({ adaptiveThinking: true }))).toBe(
        "toggle",
      );
    });
    ...
    it("marks models without thinking support as unsupported", () => {
      expect(
        modelThinkingAvailability(model({ capabilities: ["vision"] })),
      ).toBe("unsupported");
    });
  });
  ...
  describe("defaultThinkingLevelFor", () => {
    ...
    it("returns on for boolean thinking models", () => {
      expect(
        defaultThinkingLevelFor(model({ capabilities: ["thinking"] })),
      ).toBe("on");
    });
  });
  ...
  describe("segmentsFor", () => {
    it("shows off/on for boolean toggle models", () => {
      expect(segmentsFor(model({ capabilities: ["thinking"] }))).toEqual([
        "on",
        "off",
      ]);
    });
    ...
    it("shows only on for always-on models", () => {
      expect(segmentsFor(model({ capabilities: ["always_thinking"] }))).toEqual(
        ["on"],
      );
    });
  ...
  describe("thinkingLevelForModelSwitch", () => {
    it("pre-selects the target model default effort on a switch", () => {
      expect(thinkingLevelForModelSwitch(effortModel, "off", true)).toBe(
        "high",
      );
      expect(thinkingLevelForModelSwitch(effortModel, "max", true)).toBe(
        "high",
      );
      expect(thinkingLevelForModelSwitch(effortModel, undefined, true)).toBe(
        "high",
      );
    });

    it("keeps the current level when re-selecting the same model", () => {
      expect(thinkingLevelForModelSwitch(effortModel, "off", false)).toBe(
        "off",
      );
      expect(thinkingLevelForModelSwitch(effortModel, "max", false)).toBe(
        "max",
      );
      expect(
        thinkingLevelForModelSwitch(effortModel, undefined, false),
      ).toBeUndefined();
    });
    ...
    it("pre-selects on for boolean and always-on models on a switch", () => {
      ...
      expect(thinkingLevelForModelSwitch(alwaysOnModel, "off", true)).toBe(
        "on",
      );
    });

    it("pre-selects off for unsupported models on a switch", () => {
      expect(thinkingLevelForModelSwitch(unsupportedModel, "high", true)).toBe(
        "off",
      );
    });
    ...
    it("keeps the current level when the target model is unknown", () => {
      ...
      expect(
        thinkingLevelForModelSwitch(undefined, undefined, true),
      ).toBeUndefined();
    });
  });
  ...
  describe("thinkingLevelToConfig", () => {
    ...
    it("records only enabled for the model top tier", () => {
      expect(thinkingLevelToConfig("max", ["low", "high", "max"])).toEqual({
        enabled: true,
      });
    ...
    it("persists concrete levels as-is when the model levels are unknown", () => {
      expect(thinkingLevelToConfig("max")).toEqual({
        enabled: true,
        effort: "max",
      });
      expect(thinkingLevelToConfig("ultra")).toEqual({
        enabled: true,
        effort: "ultra",
      });
    });
...
describe("useModelProviderState thinking on model selection", () => {
  ...
  it("applies the resolved level to the session profile before activating a skill", async () => {
    ...
    expect(apiMock.activateSkill).toHaveBeenCalledWith(
      "session-1",
      "gen-changesets",
      undefined,
    );
  ...
  it("persists the thinking pick as the global default on setThinking", async () => {
    ...
    expect(apiMock.setConfig).toHaveBeenCalledWith({
  ...
  it("waits for the status fold when the session level has not landed", async () => {
    ...
    const level = await provider.resolveThinkingForPrompt(
      "session-1",
      effortAppModel.id,
    );
