...
describe("mergeWorkspaces — folded root identity", () => {
  ...
  it("hides POSIX roots only by exact directory (trailing slash tolerated)", () => {
    const hidden = mergeWorkspaces(
      input({
        workspaces: [ws("/home/Foo")],
        hiddenWorkspaceRoots: ["/home/Foo/"],
      }),
    );
...
describe("workspaceRootKey", () => {
  it("folds drive-letter casing", () => {
    expect(workspaceRootKey("C:\\Users\\Foo")).toBe(
      workspaceRootKey("c:\\Users\\Foo"),
    );
  ...
  it("folds UNC paths", () => {
    ...
    expect(workspaceRootKey("\\\\HOST\\Share\\Dir")).toBe(
      workspaceRootKey("//host/share/dir"),
    );
  });
  ...
  it("never folds POSIX paths", () => {
    expect(workspaceRootKey("/home/Foo")).toBe("/home/Foo");
    expect(workspaceRootKey("/home/Foo")).not.toBe(
      workspaceRootKey("/home/foo"),
    );
  });
});
