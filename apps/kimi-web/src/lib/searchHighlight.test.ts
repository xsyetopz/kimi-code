...
describe("snippet", () => {
  ...
  it("matches at the start: no leading ellipsis, trailing ellipsis when clipped", () => {
    expect(snippet("hello world this is a long sentence", "hello", 4)).toBe(
      "hello wor…",
    );
  });
  ...
  it("matches in the middle: leading and trailing ellipses", () => {
    expect(
      snippet("the quick brown fox jumps over the lazy dog", "fox", 4),
    ).toBe("…own fox jum…");
  });

  it("matches at the end: leading ellipsis, no trailing ellipsis", () => {
    expect(
      snippet("the quick brown fox jumps over the lazy dog", "dog", 4),
    ).toBe("…azy dog");
  });
...
describe("highlightHtml", () => {
  ...
  it("matches a query that contains HTML-significant characters", () => {
    expect(highlightHtml("a&b&c", "&")).toBe(
