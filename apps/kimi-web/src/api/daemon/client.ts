...
export class DaemonKimiWebApi implements KimiWebApi {
  ...
  async createSession(input: {
    ...
    if (input.model !== undefined)
      body["agent_config"] = { model: input.model };
  ...
  async updateSession(
    ...
    if (input.permissionMode !== undefined)
      agentConfig["permission_mode"] = input.permissionMode;
    if (input.planMode !== undefined) agentConfig["plan_mode"] = input.planMode;
    if (input.swarmMode !== undefined)
      agentConfig["swarm_mode"] = input.swarmMode;
    if (input.goalObjective !== undefined)
      agentConfig["goal_objective"] = input.goalObjective;
    if (input.goalControl !== undefined)
      agentConfig["goal_control"] = input.goalControl;
  ...
  async exportSession(
    ...
    const webLogBytes =
      webLog === undefined ? 0 : new TextEncoder().encode(webLog).byteLength;
    const webLogEntries =
      webLog === undefined || webLog.length === 0
        ? 0
        : webLog.split("\n").length;
  ...
  async undoSession(sessionId: string, count = 1): Promise<void> {
    await this.http.post(`/sessions/${encodeURIComponent(sessionId)}:undo`, {
      count,
    });
  }
  ...
  async listDirectory(
    ...
    if (input.includeGitStatus !== undefined)
      body["include_git_status"] = input.includeGitStatus;
    ...
    const childrenByPath = data.children_by_path
      ? Object.fromEntries(
          Object.entries(data.children_by_path).map(([k, v]) => [
            k,
            v.map(toAppFsEntry),
          ]),
        )
  ...
  async grepFiles(
    ...
    if (input.caseSensitive !== undefined)
      body["case_sensitive"] = input.caseSensitive;
  ...
  async browseFs(path?: string): Promise<FsBrowseResult> {
    try {
      const data = await this.http.get<WireFsBrowseResult>("/fs:browse", {
        path,
      });
      return {
        path: data.path,
  ...
  async refreshAllProviders(): Promise<ProviderRefreshResult> {
    const data =
      await this.http.post<WireProviderRefreshResult>("/providers:refresh");
    return toProviderRefreshResult(data);
  }

  async refreshOAuthProviderModels(): Promise<ProviderRefreshResult> {
    const data = await this.http.post<WireProviderRefreshResult>(
      "/providers:refresh_oauth",
    );
    return toProviderRefreshResult(data);
  }
  ...
  connectEvents(handlers: KimiEventHandlers): KimiEventConnection {
    ...
    return {
      ...
      seedSnapshot(sessionId: string, snapshot: AppSessionSnapshot): void {
        ...
        const appEvents = projector.seedInFlight(
          sessionId,
...
function toProviderRefreshResult(
  data: WireProviderRefreshResult,
): ProviderRefreshResult {
  return {
