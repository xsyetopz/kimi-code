...
export function toAppMessageContent(
  wire: WireMessageContent,
): AppMessageContent {
  switch (wire.type) {
    case "text":
...
export function toWirePromptSubmission(
...
export function toWireApprovalResponse(
...
export function toAppApprovalRequest(
...
export function toAppQuestionRequest(
  ...
  switch (app.kind) {
    ...
    case "multiWithOther":
      return {
        ...
        option_ids: app.optionIds,
        other_text: app.otherText,
      };
    case "skipped":
      return { kind: "skipped" };
  }
...
export function toWireQuestionResponse(
...
export function toAppTask(wire: WireTask): AppTask {
  return {
    ...
    runInBackground:
      wire.run_in_background ?? (wire.kind === "subagent" ? true : undefined),
...
export function toAppGoal(snapshot: unknown): AppGoal | null {
  ...
  return {
    goalId:
      recordString(source, "goalId") ??
      recordString(source, "goal_id") ??
      "goal",
    objective: recordString(source, "objective") ?? "",
    completionCriterion:
      recordString(source, "completionCriterion") ??
      recordString(source, "completion_criterion"),
    status,
    turnsUsed:
      recordNumber(source, "turnsUsed") ??
      recordNumber(source, "turns_used") ??
      0,
    tokensUsed:
      recordNumber(source, "tokensUsed") ??
      recordNumber(source, "tokens_used") ??
      0,
    wallClockMs:
      recordNumber(source, "wallClockMs") ??
      recordNumber(source, "wall_clock_ms") ??
      0,
    terminalReason:
      recordString(source, "terminalReason") ??
      recordString(source, "terminal_reason"),
    budget: {
      tokenBudget:
        recordNullableNumber(budget, "tokenBudget") ??
        recordNullableNumber(budget, "token_budget"),
      remainingTokens:
        recordNullableNumber(budget, "remainingTokens") ??
        recordNullableNumber(budget, "remaining_tokens"),
      turnBudget:
        recordNullableNumber(budget, "turnBudget") ??
        recordNullableNumber(budget, "turn_budget"),
      remainingTurns:
  ...
  switch ((wire as { type: string }).type) {
    ...
    case "event.workspace.created":
      return {
        ...
        workspace: toAppWorkspace(w.payload.workspace),
      };
    ...
    case "event.session.status_changed":
      return {
        ...
        mainTurnActive:
          w.payload.status !== "idle" && w.payload.status !== "aborted",
        pendingInteraction:
          w.payload.status === "awaiting_approval"
            ...
            : w.payload.status === "awaiting_question"
              ...
              : "none",
        lastTurnReason:
          w.payload.status === "aborted" ? "cancelled" : undefined,
      };
    ...
    case "event.message.created":
      return {
        type: "messageCreated",
        message: toAppMessage(w.payload.message),
      };
    ...
    case "event.tool.progress":
      if (
        typeof w.payload.message === "string" &&
        w.payload.message.length > 0
      ) {
        return {
          type: "toolOutput",
          sessionId: w.session_id,
