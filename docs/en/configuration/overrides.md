...
2. **Runtime switches**: A small set of variables like `KIMI_DISABLE_CRON` directly shut down the corresponding subsystem — even if the scheduled-task tool is otherwise available, setting this variable to a truthy value disables new schedules. The semantics are "additionally disable", not "ordinary override".
